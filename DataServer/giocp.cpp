...
DWORD WINAPI
ServerRecvWorkerThread_DS_Q1(DWORD
p);
DWORD WINAPI
ServerRecvWorkerThread_DS_Q2(DWORD
p);
DWORD WINAPI
ServerRecvWorkerThread_DS_Q3(DWORD
p);
DWORD WINAPI
ServerRecvWorkerThread_DS_Q4(DWORD
p);
DWORD WINAPI
ServerRecvWorkerThread_DS_Q5(DWORD
p);
DWORD WINAPI
ServerRecvWorkerThread_JS(DWORD
p);
DWORD WINAPI
ServerRecvWorkerThread_EXDS(DWORD
p);
...
InternetAddr.
sin_family = AF_INET;
InternetAddr.sin_addr.S_un.
S_addr = htonl(0);
InternetAddr.
sin_port = htons(g_ServerPort[dwServerCountIndex]);
...
if (nRet == -1)
{
g_Log.Add("listen() failed with error %d",
...
unsigned long __stdcall IocpServerWorker(DWORD

pIocpServerParameter)
{
...
g_Log.
AddC(TColor::Orange,
"[WARNING]: IOCP Thread Handles set to 16");
}

__try
{

g_CompletionPort[dwServerCountIndex] =
CreateIoCompletionPort(INVALID_HANDLE_VALUE,
0, 0, 0);

if ( g_CompletionPort[dwServerCountIndex] == NULL )
{
g_Log.Add("CreateIoCompletionPort failed with error: %d",
...
for (
DWORD n = 0;
n<g_dwThreadCount[dwServerCountIndex];
n++ )
{
HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) ServerWorkerThread,
                              (LPVOID) g_CompletionPort[dwServerCountIndex], 0, &ThreadID);

if ( hThread == NULL )
{
g_Log.Add("CreateThread() failed with error %d",
...
g_ThreadHandles[dwServerCountIndex][n] =
hThread;
CloseHandle(hThread);
}
...
{
HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) ServerRecvWorkerThread_DS_Q1, (LPVOID) eType, 0,
                              &ThreadID);
...
{
g_Log.Add("SetThreadPriority() failed with error %d",
...
if ( hThread == NULL )
{
g_Log.Add("CreateThread() (2) failed with error %d",
...
if ( hThread == NULL )
{
g_Log.Add("CreateThread() (3) failed with error %d",
...
if ( hThread == NULL )
{
g_Log.Add("CreateThread() (4) failed with error %d",
...
if (hThread == NULL)
{
g_Log.Add("CreateThread() (5) failed with error %d",
...
{
HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) ServerRecvWorkerThread_JS, (LPVOID) eType, 0,
                              &ThreadID);
...
{
HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) ServerRecvWorkerThread_EXDS, (LPVOID) eType, 0,
                              &ThreadID);
...
if (
CreateListenSocket(dwServerCountIndex)
== FALSE )
{
...
g_Log.Add("WSAAccept() failed with error %d",
...
EnterCriticalSection(&criti[eType]);
memcpy(&cInAddr, &cAddr
.sin_addr  , sizeof(cInAddr));
...
if (
UpdateCompletionPort(Accept, ClientIndex, dwServerCountIndex
) == 0 )
{
g_Log.
AddC(TColor::Red,
"error-L1 : %d %d CreateIoCompletionPort failed with error %d", Accept, ClientIndex,
...
if (
gObjServerAdd(Accept, inet_ntoa(cInAddr), ClientIndex, eSERVER_TYPE(eType)
) == -1 )
{
g_Log.
AddC(TColor::Red,
"error-L1 : %d %d gObjAdd() failed with error %d", Accept, ClientIndex,
...
g_Server[ClientIndex].
m_ConnectPort = g_ServerPort[dwServerCountIndex];

LeaveCriticalSection(&scriti);

memset(&g_Server[ClientIndex]
.PerSocketContext->IOContext[0].Overlapped, 0, sizeof(WSAOVERLAPPED));
memset(&g_Server[ClientIndex]
.PerSocketContext->IOContext[1].Overlapped, 0, sizeof(WSAOVERLAPPED));

g_Server[ClientIndex].PerSocketContext->IOContext[0].wsabuf.
buf = (char *) &g_Server[ClientIndex].PerSocketContext->IOContext[0].Buffer;
g_Server[ClientIndex].PerSocketContext->IOContext[0].wsabuf.
len = MAX_IO_BUFFER_SIZE;
g_Server[ClientIndex].PerSocketContext->IOContext[0].
nTotalBytes = 0;
g_Server[ClientIndex].PerSocketContext->IOContext[0].
nSentBytes = 0;
g_Server[ClientIndex].PerSocketContext->IOContext[0].
...
g_Server[ClientIndex].PerSocketContext->IOContext[0].
IOOperation = 0;
g_Server[ClientIndex].PerSocketContext->IOContext[1].wsabuf.
buf = (char *) g_Server[ClientIndex].PerSocketContext->IOContext[0].Buffer;
g_Server[ClientIndex].PerSocketContext->IOContext[1].wsabuf.
len = MAX_IO_BUFFER_SIZE;
g_Server[ClientIndex].PerSocketContext->IOContext[1].
nTotalBytes = 0;
g_Server[ClientIndex].PerSocketContext->IOContext[1].
nSentBytes = 0;
g_Server[ClientIndex].PerSocketContext->IOContext[1].
...
g_Server[ClientIndex].PerSocketContext->IOContext[1].
IOOperation = 1;
g_Server[ClientIndex].PerSocketContext->
m_socket = Accept;
g_Server[ClientIndex].PerSocketContext->
nIndex = ClientIndex;

nRet = WSARecv(Accept, &g_Server[ClientIndex].PerSocketContext->IOContext[0].wsabuf, 1, (unsigned long *) &RecvBytes,
               &Flags,
...
g_Log.
AddC(TColor::Red,
"error-L1 : WSARecv() failed with error %d",
...
g_Server[ClientIndex].PerSocketContext->IOContext[0].
nWaitIO = 4;
CloseClient(g_Server[ClientIndex]
.PerSocketContext, 0);
LeaveCriticalSection(&criti[eType]);
continue;
}
...
for (
DWORD i = 0;
i<g_dwThreadCount[dwServerCountIndex];
i++ )
{
PostQueuedCompletionStatus( g_CompletionPort[dwServerCountIndex],
0, 0, 0);
}
}
...
unsigned long __stdcall ServerWorkerThread(HANDLE
...
g_Log.Add("Error Thread : GetQueueCompletionStatus( %d )",
...
if (
RecvDataParse(lpIOContext, lpPerSocketContext
->nIndex ) == 0 )
{
g_Log.Add("error-L1 : Socket Header error %d, %d",

WSAGetLastError(), lpPerSocketContext
...
Flags = 0;
memset(&lpIOContext
->Overlapped, 0, sizeof (WSAOVERLAPPED));
lpIOContext->wsabuf.
len = MAX_IO_BUFFER_SIZE - lpIOContext->nSentBytes;
lpIOContext->wsabuf.
buf = (char *) &lpIOContext->Buffer[lpIOContext->nSentBytes];
lpIOContext->
IOOperation = 0;
...
!= WSA_IO_PENDING)
{
g_Log.Add("WSARecv() failed with error %d",
        ...
        if (size <= lpIOContext->nSentBytes) {
            if (g_Server[uIndex].m_Type == ST_DATASERVER) {
                if (headcode == 0x52 || headcode == 0x53 || headcode == 0x55) {
                    m_DSQueue3.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                          g_DelayHandler.GetQuerySessionId());
                } else if (headcode == 0x03 || headcode == 0x08 || headcode == 0x09 || headcode == 0x12) {
                    m_DSQueue2.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                          g_DelayHandler.GetQuerySessionId());
                } else if (headcode == 0x01 || headcode == 0x04 || headcode == 0x05 || headcode == 0x06 ||
                           headcode == 0x07 || headcode == 0x11 || headcode == 0x60 || headcode == 0xAE ||
                           headcode == 0xDA || headcode == 0xFA || headcode == 0xFB || headcode == 0xC4) {
                    m_DSQueue4.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                          g_DelayHandler.GetQuerySessionId());
                } else if (headcode == 0xF6 || headcode == 0xF7) {
                    m_DSQueue5.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                          g_DelayHandler.GetQuerySessionId());
                } else {
                    m_DSQueue1.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                          g_DelayHandler.GetQuerySessionId());
                }
            } else if (g_Server[uIndex].m_Type == ST_JOINSERVER) {
                m_JSQueue.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                     g_DelayHandler.GetQuerySessionId());
            } else if (g_Server[uIndex].m_Type == ST_EXDATASERVER) {
                m_EXDSQueue.AddToQueue((LPBYTE)(recvbuf + lOfs), size, headcode, uIndex,
                                       g_DelayHandler.GetQuerySessionId());
            }
    ...
    if (WSASend(g_Server[aIndex].m_Socket, &lpIoCtxt->wsabuf, 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1) {
        ...
        if (WSAGetLastError() != WSA_IO_PENDING) {
            ...
            if (lpIoCtxt->wsabuf.buf[0] == 0xC1) {
                g_Log.Add("(%d)WSASend(%d) failed with error [%x][%x] %d %s ", __LINE__, aIndex,
                          (BYTE) lpIoCtxt->wsabuf.buf[0],
...
BOOL UpdateCompletionPort(SOCKET sd, int ClientIndex, DWORD dwServerTypeCount) {
    ...
    HANDLE
    cp = CreateIoCompletionPort((HANDLE)
    sd, g_CompletionPort[dwServerTypeCount], ClientIndex, 0);
...
if (m_DSQueue1.
GetFromQueue((LPBYTE)
RecvData, &nSize, &headcode, &uindex, &iSessionId) == TRUE)
{
if (g_Server[uindex].m_ProtocolCore != NULL)
{
g_Server[uindex].
m_ProtocolCore(uindex, headcode, RecvData, nSize
);
}

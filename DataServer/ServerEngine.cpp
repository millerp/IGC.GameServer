...
void gObjServerInit() {
    ...
    if (g_iConnectStatSyncEnable == TRUE) {
        DWORD ThreadId;
        CreateThread(nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(gObjConnectStatSyncThread), nullptr, 0,
                     &ThreadId);
    }
}
...
void LoadAllowableIpList(LPSTR
filename)
{
memset(g_IpList,
0,sizeof(g_IpList));
...
{
g_Log.MsgBox("LoadAllowedIpList() error - file load error (%s)", res.
...
for (
pugi::xml_node ip = main.child("IP");
ip;
ip = ip.next_sibling()
)
{
strcpy(g_IpList[count]
.m_IP, ip.attribute("Address").

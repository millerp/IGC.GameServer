...
void EGAnsEventChipInfo(int aIndex, PMSG_REQ_VIEW_EC_MN *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("SELECT EventChips, Check_Code, MuttoNumber FROM T_MU2003_EVENT WHERE AccountID='%s'",
                            lpMsg->szUID) == FALSE) {
        pMsg.bSUCCESS = FALSE;
    } else {
...
void EGAnsRegEventChipInfo(int aIndex, PMSG_REQ_REGISTER_EVENTCHIP *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("UPDATE T_MU2003_EVENT SET EventChips = EventChips + 1 WHERE AccountID = '%s'",
                            pMsg.szUID)) {
        g_EventDB.Close();
...
void EGAnsEventStoneInfo(int aIndex, PMSG_REQ_VIEW_EC_MN *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("SELECT StoneCount, Check_Code FROM T_BLOOD_CASTLE WHERE AccountID='%s'", pMsg.szUID) ==
        FALSE) {
        pMsg.bSUCCESS = FALSE;
    } else {
        if (g_EventDB.Fetch() != SQL_NO_DATA) {
...
void EGAnsRegEventStoneInfo(int aIndex, PMSG_REQ_REGISTER_STONES *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("UPDATE T_BLOOD_CASTLE SET StoneCount = StoneCount + 1 WHERE AccountID = '%s'",
                            pMsg.szUID)) {
        g_EventDB.Close();
...
void EGAnsDeleteStones(int aIndex, PMSG_REQ_DELETE_STONES *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("SELECT StoneCount FROM T_BLOOD_CASTLE WHERE AccountID='%s'", pMsg.szUID) == FALSE ||
        g_EventDB.Fetch() == SQL_NO_DATA) {
        pMsg.bSUCCESS = FALSE;
    } else {
        ...
        if (Stones != -1 && Stones >= lpMsg->iStoneCount) {
            if (g_EventDB.ExecQuery("UPDATE T_BLOOD_CASTLE SET StoneCount = StoneCount - %d WHERE AccountID='%s'",
...
void EGAns2AnivRegSerial(int aIndex, PMSG_REQ_2ANIV_SERIAL *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("EXEC SP_REG_SERIAL '%s', '%d', '%s', '%s', '%s'", lpMsg->szUID, lpMsg->iMEMB_GUID,
                            lpMsg->SERIAL1, lpMsg->SERIAL2, lpMsg->SERIAL3) && g_EventDB.Fetch() != SQL_NO_DATA) {
...
void EGAnsRegCCOfflineGift(int aIndex, PMSG_REQ_REG_CC_OFFLINE_GIFT *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("EXEC SP_REG_CC_OFFLINE_GIFT '%s', '%s', %d", lpMsg->szUID, lpMsg->szNAME,
                            lpMsg->wServerCode) == FALSE || g_EventDB.Fetch() == SQL_NO_DATA) {
...
void EGAnsRegDLOfflineGift(int aIndex, PMSG_REQ_REG_DL_OFFLINE_GIFT *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("EXEC SP_REG_DL_OFFLINE_GIFT '%s', '%s', %d", lpMsg->szUID, lpMsg->szNAME,
                            lpMsg->wServerCode) == FALSE || g_EventDB.Fetch() == SQL_NO_DATA) {
...
void EGAnsRegHTOfflineGift(int aIndex, PMSG_REQ_REG_HT_OFFLINE_GIFT *lpMsg) {
    ...
    if (g_EventDB.ExecQuery("EXEC SP_REG_HT_OFFLINE_GIFT '%s', '%s', %d", lpMsg->szUID, lpMsg->szNAME,
                            lpMsg->wServerCode) == FALSE || g_EventDB.Fetch() == SQL_NO_DATA) {

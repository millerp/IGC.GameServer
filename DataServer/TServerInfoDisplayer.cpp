...
void TServerInfoDisplayer::Run(HWND
hWnd)
{
if (
...
- this->m_dwCheckTime > 60000 )
{
this->
SetState(SERVER_STANDBY);
}
...
void TServerInfoDisplayer::PaintAllInfo(HWND
hWnd,
int iTopLeftX,
int iTopLeftY
)
...
TextOut(hDC, iTopLeftX, iTopLeftY
+iLineCount*15, ErrorMessge[0], (int)
strlen(ErrorMessge[0])
);
iLineCount++;
}
...
TextOut(hDC,
250, 20, ServerTypeText[iIndex], (int)
strlen(ServerTypeText[iIndex])
);

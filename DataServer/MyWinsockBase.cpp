...
BOOL MyWinsockBase::Startup() {
    ...
    if (err != 0) {
        MessageBox(NULL, "WINSOCK �ʱ�ȭ ����", "Error", MB_OK | MB_APPLMODAL);
        return FALSE;
    }

    if (((wsaData.wVersion & 0xFF & 0xFF) != (2)) || ((((DWORD)wsaData.wVersion >> (DWORD)
    8) &0xFF )  != (2)))    // Lacking Some Code Here	It said SAR and is SHR
    {
        WSACleanup();
        MessageBox(NULL, "WINSOCK ������ �����ϴ�.", "Error", MB_OK | MB_APPLMODAL);
        return FALSE;
    }
    else
    {
...
BOOL MyWinsockBase::CreateSocket(HWND
hWnd)
{
int size;
size = 4;

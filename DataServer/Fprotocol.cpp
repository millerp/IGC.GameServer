...
BOOL CFriendSystem::GetDBFriendList(char *szMaster) {
    ...
    if (this->m_FriendDB.ExecQuery("SELECT FriendGuid, FriendName, Del FROM T_FriendList where GUID = %d",
                                   lpMaster->m_iGUID)) {
        while (this->m_FriendDB.Fetch() != SQL_NO_DATA) {
            if (i == this->m_iMaxFriends) {
                g_Log.AddC(TColor::Red, "DB contains more then %d friends for [%s] character", this->m_iMaxFriends,
                           szMaster);
                break;
            }
...
void CFriendSystem::FriendListRequest(int aIndex, FHP_FRIENDLIST_REQ *lpMsg) {
    ...
    if (m_EXDSProtocol.m_PartyMatchingEDS.ConnectMember(lpMsg->Name, lpMsg->Number, lpMsg->pServer, ServerIndex) ==
        TRUE) {
                ...
                for (int i = 1; i < 5; i++) {
                    if (lpPartyData->m_UserData[i].iUsedInfo == TRUE &&
                        ...
                        if (lpPartyData->m_UserData[i].iServerIndex == aIndex) {
                            m_EXDSProtocol.FixAddPartyMember(lpPartyData->m_UserData[0].iUserIndex,
                                                             lpPartyData->m_UserData[i].iUserIndex, aIndex);
                        }
                    }
                }
            }
...
void CFriendSystem::MemoListSend(int aIndex, WORD wNumber, char *szName) {
    ...
    int cnt = 0;
    if (this->m_MailDB.ExecQuery("SELECT MemoIndex, FriendName, wDate, Subject, bRead FROM T_FriendMail where GUID=%d",
                                 guid)) {
...
int CFriendSystem::DBWriteMail(MEMO_SEND_HEADER *lpMemoSendHdr, LPBYTE Photo, BYTE btPhotoSize, char *sMemo,
                               int memo_size) {
...
BOOL CFriendSystem::DBReadMail(int iMemoId, int iGuid, MEMO_READ *lpMemoRead, LPDWORD lpdwMemoSize, LPBYTE lpPhoto,
                               LPBYTE lpPhotoSize) {
    if (this->m_MailDB.ExecQuery("SELECT bRead, Dir, Act FROM T_FriendMail where MemoIndex=%d AND GUID =%d", iMemoId,
                                 iGuid)) {
        int bRead = 0;
        if (this->m_MailDB.Fetch() != SQL_NO_DATA) {
        ...
        *lpdwMemoSize = this->m_MailDB.GetAsBinary(szMsg, (LPBYTE) lpMemoRead->Memo, sizeof(lpMemoRead->Memo));
        ...
        if (bRead == 0) {
            this->m_MailDB.ExecQuery("UPDATE T_FriendMail SET bRead = 1 where MemoIndex=%d AND GUID=%d", iMemoId,
                                     iGuid);
            this->m_MailDB.Close();
        }

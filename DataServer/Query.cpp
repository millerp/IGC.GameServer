...
BOOL CQuery::Connect(TCHAR *lpszDNS, TCHAR *lpszUser, TCHAR *lpszPassword, TCHAR *lpszServer) {
    ...
    SQLRETURN Result = SQLDriverConnect(this->m_hConnection, NULL, InCon, sizeof(InCon), OutCon, sizeof(OutCon),
    ...
    if (Result == SQL_ERROR) {
        ...
        while (SQLGetDiagRec(SQL_HANDLE_DBC, this->m_hConnection, sRecord, SqlState, &NativeError, SQLMsgError,
                             sizeof(SQLMsgError), &MsgLen) != SQL_NO_DATA) {
...
void CQuery::Diagnosis(bool &bReconnect) {
    ...
    while (SQLGetDiagRec(SQL_HANDLE_STMT, this->m_hStmt, sRecord, SqlState, &NativeError, SQLMsgError,
                         sizeof(SQLMsgError), &MsgLen) != SQL_NO_DATA) {
        g_Log.AddC(TColor::Blue, "SQLSTATE: %s, Diagnosis: %s", SqlState, SQLMsgError);
        CQuery::m_LogToFile.Output("SQLSTATE: %s, Diagnosis: %s", SqlState, SQLMsgError);
        sRecord++;
...
void CQuery::SetAsBinary(LPTSTR lpszStatement, LPBYTE lpBinaryBuffer, SQLUINTEGER BinaryBufferSize) {
    ...
    SQLBindParameter(this->m_hStmt, 1, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_LONGVARBINARY, BinaryBufferSize, 0,
                     (SQLPOINTER) 1, 0, &cbValueSize);

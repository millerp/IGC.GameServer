...
void CWarehouseUserData::AddUserData(char *szAccountID) {
    ...
    for (std::vector<USERWAREHOUSE_DATA>::iterator it = this->m_vtWarehouseData.begin();
         it != this->m_vtWarehouseData.end(); it++) {
...
void CWarehouseUserData::DGGetWarehouseList(int aIndex, SDHP_GETWAREHOUSEDB *aRecv) {
    ...
    if (this->m_WareDB.Fetch() == SQL_NO_DATA || bRet == FALSE) {
        this->m_WareDB.Close();
        this->m_WareDB.ExecQuery(
                "INSERT INTO warehouse (AccountID, Money, EndUseDate, WHOpen) VALUES ('%s',0, getdate(),1)",
                szAccountID);
    ...
    switch (HeadCode) {
        ...
        case 0x4C:
            if (aRecv[3] == 0x00)
                ...
                m_DSProtocol.GDReqMineCheckIsUPTWhenUserConnect(aIndex,
                                                                (SDHP_REQ_LOAD_MINESYSTEM_UPT_USERINFO *) aRecv);
            break;
        case 0x4F: {
        ...
        case 0xBD: {
            ...
            switch (lpMsg->subcode) {
                case 0x01:
                    ...
                    break;
                case 0x02:
                    m_DSProtocol.GDReqBloodCastleEnterCount(aIndex,
                                                            reinterpret_cast<PMSG_REQ_BLOODCASTLE_ENTERCOUNT *>(aRecv));
                    break;
                case 0x03:
        ...
        case 0xF8: {
            ...
            switch (lpMsg->subcode) {
                case 0x30:
                    ...
                    break;
                case 0x32:
                    m_DSProtocol.GDReqArcaBattleGuildMemberJoin(aIndex,
                                                                (_tagPMSG_REQ_ARCA_BATTLE_GUILD_MEMBER_JOIN_DS *) (aRecv));
                    break;
                case 0x34:
                    m_DSProtocol.GDReqArcaBattleEnter(aIndex, (_tagPMSG_REQ_ARCA_BATTLE_ENTER_DS *) (aRecv));
                    break;
                case 0x38:
                    m_DSProtocol.GDReqArcaBattleWinGuildInfoInsert(aIndex,
                                                                   (_tagPMSG_REQ_AB_WIN_GUILD_INFO_INSERT_DS *) (aRecv));
                    break;
                case 0x39:
                ...
                case 0x3F:
                    ...
                    break;
                case 0x41:
                    m_DSProtocol.GDReqArcaBattleJoinMemberUnder(aIndex,
                                                                (_tagPMSG_REQ_AB_JOIN_MEMBER_UNDER_DS *) (aRecv));
                    break;
                case 0x43:
                    m_DSProtocol.GDReqArcaBattleJoinMemberUnderReq(aIndex, (_tagPMSG_REQ_AB_JOIN_CANCEL_DS *) (aRecv));
                    break;
                case 0x45:
                    m_DSProtocol.GDReqArcaBattleRegisteredMemberCnt(aIndex,
                                                                    (_tagPMSG_REQ_AB_REG_MEMBER_CNT_DS *) (aRecv));
                    break;
                case 0x47:
                    m_DSProtocol.GDReqRemoveAllGuildBuffMultiCast(aIndex,
                                                                  (_tagPMSG_REQ_REMOVE_ALL_GUILD_BUFF_DS *) (aRecv));
                    break;
                case 0x4D:
...
void CDataServerProtocol::DataServerLogin(int aIndex, SDHP_SERVERINFO *lpMsg) {
    ...
    g_Log.Add("[DataServer] GameServer(%d)(%d) connect", lpMsg->ServerCode,
              g_MapServerManager.GetMapSvrGroup(lpMsg->ServerCode));
...
void CDataServerProtocol::JGPGetCharList(int aIndex, SDHP_GETCHARLIST *aRecv) {
    ...
    pCount->MoveCnt = (BYTE)
    this->m_AccDB.GetAsInteger("MoveCnt");
    ...
    g_Log.Add("[%s] - characters: [%s][%s][%s][%s][%s]", szAccountID, CharName[0], CharName[1], CharName[2],
              CharName[3], CharName[4]);

    this->m_AccDB.ExecQuery(
            "SELECT WarehouseExpansion, Summoner, RageFighter, SecCode FROM AccountCharacter WHERE Id='%s'",
            szAccountID);
    this->m_AccDB.Fetch();
    ...
    for (int i = 0; i < 5; i++) {
        if (CharName[i][0] != 0) {
            if (lstrlen(CharName[i]) >= 4) {
                if (this->m_AccDB.ExecQuery(
                        "SELECT cLevel, Class, CtlCode, RESETS, ResetCountDay, ResetCountWeek, ResetCountMonth FROM vCharacterPreview WHERE Name='%s'",
                        CharName[i]) == TRUE) {
                    char szTemp[200];
                    ...
                    this->m_AccDB.Fetch();
                    pCL->Level = (WORD)
                    this->m_AccDB.GetAsInteger("cLevel");
                    pCL->Class = (BYTE)
                    this->m_AccDB.GetAsInteger("Class");
                    pCL->Resets = (WORD)
                    this->m_AccDB.GetAsInteger("RESETS");
                    pCL->ResetsDay = (WORD)
                    this->m_AccDB.GetAsInteger("ResetCountDay");
                    pCL->ResetsWeek = (WORD)
                    this->m_AccDB.GetAsInteger("ResetCountWeek");
                    pCL->ResetsMonth = (WORD)
                    this->m_AccDB.GetAsInteger("ResetCountMonth");
                    ...
                    pCL->CtlCode = (BYTE)
                    this->m_AccDB.GetAsInteger("CtlCode");
                    this->m_AccDB.Close();
                    ...
                    for (int j = 0; j < 12; j++) {
                        if (btInvetory[0 + j * dbsize] == 0xFF && (btInvetory[7 + j * dbsize] & 0x80) == 0x80 &&
                            (btInvetory[10 + j * dbsize] & 0xF0) == 0xF0) {
                    ...
                    this->m_AccDB.ExecQuery(
                            "SELECT RESETS, ResetCountDay, ResetCountWeek, ResetCountMonth FROM Character WHERE Name='%s'",
                            CharName[i]);
                    ...
                    if (aRecv->IsUnityBattleFieldServer == FALSE) {
                        ...
                        if (this->m_AccDB.Fetch() != SQL_NO_DATA) {
                            pCL->btGuildStatus = (BYTE)
                            this->m_AccDB.GetAsInteger("G_Status");
                        } else {
...
void CDataServerProtocol::JGCharacterCreateRequest(int aIndex, SDHP_CREATECHAR *aRecv) {
    ...
    if (SpaceSyntexCheck(szName) == FALSE || QuoteSpaceSyntexCheck(szName) == FALSE ||
        PercentSyntaxCheck(szName) == FALSE) {
        pResult.Result = 0;
        DataSend(aIndex, (LPBYTE) & pResult, pResult.h.size, __FUNCTION__);
        return;
    }
    ...
    if (iIndex == 5) {
        ...
        if (pResult.Result == 1) {
            ...
            if (this->m_AccDB.ExecQuery("SELECT cLevel FROM vCharacterPreview WHERE Name='%s'", szName) == TRUE &&
                this->m_AccDB.Fetch() != SQL_NO_DATA) {
...
void CDataServerProtocol::JGGetCharacterInfo(int aIndex, SDHP_DBCHARINFOREQUEST *aRecv) {
    ...
    if (this->m_CharDB.ExecQuery(
            "SELECT cLevel, mLevel, Class, LevelUpPoint, mlPoint, Experience, mlExperience, mlNextExp, Strength, Dexterity, Vitality, Energy, Money, Life, MaxLife, Mana, MaxMana, MapNumber, MapPosX, MapPosY, MapDir, PkCount, PkLevel, PkTime, CtlCode, Leadership, ChatLimitTime, FruitPoint, RESETS, ResetCountDay, ResetCountWeek, ResetCountMonth, Married, MarryName, InventoryExpansion, WinDuels, LoseDuels, BlockChatTime, PenaltyMask FROM Character WHERE Name='%s' AND AccountID='%s'",
            szName, szAccountID) == FALSE) {
        this->m_CharDB.Close();
        return;
    }
    ...
    pResult.Class = (BYTE)
    this->m_CharDB.GetAsInteger("Class");
    ...
    pResult.Str = (WORD)
    this->m_CharDB.GetAsInteger("Strength");
    pResult.Dex = (WORD)
    this->m_CharDB.GetAsInteger("Dexterity");
    pResult.Vit = (WORD)
    this->m_CharDB.GetAsInteger("Vitality");
    pResult.Energy = (WORD)
    this->m_CharDB.GetAsInteger("Energy");
    pResult.Money = this->m_CharDB.GetAsInteger("Money");
    pResult.Life = (DWORD)
    this->m_CharDB.GetAsFloat("Life");
    pResult.MaxLife = (DWORD)
    this->m_CharDB.GetAsFloat("MaxLife");
    pResult.Mana = (DWORD)
    this->m_CharDB.GetAsFloat("Mana");
    pResult.MaxMana = (DWORD)
    this->m_CharDB.GetAsFloat("MaxMana");
    pResult.MapNumber = (BYTE)
    this->m_CharDB.GetAsInteger("MapNumber");
    pResult.MapX = (BYTE)
    this->m_CharDB.GetAsInteger("MapPosX");
    pResult.MapY = (BYTE)
    this->m_CharDB.GetAsInteger("MapPosY");
    pResult.Dir = (BYTE)
    this->m_CharDB.GetAsInteger("MapDir");
    ...
    pResult.CtlCode = (BYTE)
    this->m_CharDB.GetAsInteger("CtlCode");
    pResult.Leadership = (WORD)
    this->m_CharDB.GetAsInteger("Leadership");
    pResult.ChatLitmitTime = (WORD)
    this->m_CharDB.GetAsInteger("ChatLimitTime");
    ...
    this->m_CharDB.ExecQuery(
            "SELECT AuthorityMask, DATEDIFF(DAY, GETDATE(), Expiry) AS ExpirationTime FROM T_GMSystem WHERE Name='%s'",
            szName);
    ...
    if (pResult.result == 1) {
        ...
        if (this->m_OptionDataDB.Fetch() != SQL_NO_DATA) {
            ...
            pSkillData.GameOption = (BYTE)
            this->m_OptionDataDB.GetAsInteger("GameOption");
            pSkillData.QkeyDefine = (BYTE)
            this->m_OptionDataDB.GetAsInteger("Qkey");
            pSkillData.WkeyDefine = (BYTE)
            this->m_OptionDataDB.GetAsInteger("Wkey");
            pSkillData.EkeyDefine = (BYTE)
            this->m_OptionDataDB.GetAsInteger("Ekey");
            pSkillData.RkeyDefine = (BYTE)
            this->m_OptionDataDB.GetAsInteger("Rkey");
            pSkillData.ChatWindow = (BYTE)
            this->m_OptionDataDB.GetAsInteger("ChatWindow");
...
void CDataServerProtocol::GJSetCharacterInfo(int aIndex, SDHP_DBCHAR_INFOSAVE *aRecv) {
    ...
    this->m_CharDB.ExecQuery(
            "UPDATE Character SET cLevel=%d, mLevel=%d, Class=%d,LevelUpPoint=%d, mlPoint=%d, Experience=%I64d, mlExperience=%I64d, mlNextExp=%I64d, Strength=%d, Dexterity=%d, Vitality=%d, Energy=%d, Money=%d, Life=%f, MaxLife=%f, Mana=%f, MaxMana=%f, MapNumber=%d, MapPosX=%d, MapPosY=%d, MapDir=%d, PkCount=%d, PkLevel=%d, PkTime=%d, Leadership=%d, ChatLimitTime=%d, FruitPoint=%d, RESETS = %d, ResetCountDay = %d, ResetCountWeek = %d, ResetCountMonth = %d, Married = %d, MarryName = '%s', WinDuels=%d, LoseDuels=%d, BlockChatTime=%I64d, PenaltyMask=%d WHERE Name = '%s'",
            aRecv->Level, aRecv->mLevel, aRecv->Class, aRecv->LevelUpPoint, aRecv->mlPoint, aRecv->Exp, aRecv->mlExp,
            aRecv->mlNextExp, aRecv->Str, aRecv->Dex, aRecv->Vit, aRecv->Energy, aRecv->Money, float(aRecv->Life),
            float(aRecv->MaxLife), float(aRecv->Mana), float(aRecv->MaxMana), aRecv->MapNumber, aRecv->MapX,
            aRecv->MapY, aRecv->Dir, aRecv->PkCount, aRecv->PkLevel, aRecv->PkTime, aRecv->Leadership,
            aRecv->ChatLitmitTime, aRecv->iFruitPoint, aRecv->resets, aRecv->resetsDay, aRecv->resetsWeek,
            aRecv->resetsMonth, aRecv->Married, aRecv->MarryName, aRecv->WinDuels, aRecv->LoseDuels,
            aRecv->ChatBlockTime, aRecv->PenaltyMask, szName);
...
void CDataServerProtocol::DGRecvPetItemInfo(int aIndex, SDHP_REQUEST_PETITEM_INFO *aRecv) {
    ...
    if (aRecv->ServerType == 0) // normal GS
    {
        for (int i = 0; i < aRecv->nCount; i++) {
            ...
            if (this->m_PetDB.Fetch() == SQL_NO_DATA) {
                this->m_PetDB.Close();
                this->m_PetDB.ExecQuery(
                        "INSERT INTO T_PetItem_Info (ItemSerial, Pet_Level, Pet_Exp) VALUES (%I64d, %d, %I64d)",
    ...
    PHeadSetW((LPBYTE) pRecvPetInfoCount, 0x56,
              sizeof(SDHP_RECV_PETITEM_INFO) + pRecvPetInfoCount->nCount * sizeof(Recv_PetItem_Info));
    ...
    DataSend(aIndex, (LPBYTE) cBUFFER,
             sizeof(SDHP_RECV_PETITEM_INFO) + pRecvPetInfoCount->nCount * sizeof(Recv_PetItem_Info), __FUNCTION__);
}
...
void CDataServerProtocol::GDSavePetItemInfo(int aIndex, SDHP_SAVE_PETITEM_INFO *aRecv) {
    ...
    } else // battlecore gs
    {
        for (int i = 0; i < aRecv->nCount; i++) {
            this->m_PetDB.ExecQuery("EXEC IGC_BattleCore_SavePetItemInfo %I64d, %d, %d, %I64d",
                                    pSavePetInfo[i].nSerial, aRecv->ServerCode, pSavePetInfo[i].Level,
                                    pSavePetInfo[i].Exp);
            this->m_PetDB.Close();
        }
    }
...
void CDataServerProtocol::DGOptionDataRecv(int aIndex, SDHP_SKILLKEYDATA *aRecv) {
    ...
    if (this->m_OptionDataDB.Fetch() == SQL_NO_DATA) {
        this->m_OptionDataDB.Close();
        this->m_OptionDataDB.ExecQuery(
                "INSERT INTO OptionData (Name, GameOption, Qkey, Wkey, Ekey, ChatWindow, Rkey, QWERLevel, EnableChangeMode) VALUES ('%s', %d, %d, %d, %d, %d, %d, %d, %d)",
                szName, aRecv->GameOption, aRecv->QkeyDefine, aRecv->WkeyDefine, aRecv->EkeyDefine, aRecv->ChatWindow,
                aRecv->RkeyDefine, aRecv->QWERLevelDefine, aRecv->EnableTransformMode);
    } else {
        this->m_OptionDataDB.Close();
        this->m_OptionDataDB.ExecQuery(
                "UPDATE OptionData SET GameOption=%d, Qkey=%d, Wkey=%d, Ekey=%d, ChatWindow=%d, Rkey=%d, QWERLevel=%d, EnableChangeMode=%d WHERE Name='%s'",
                aRecv->GameOption, aRecv->QkeyDefine, aRecv->WkeyDefine, aRecv->EkeyDefine, aRecv->ChatWindow,
                aRecv->RkeyDefine, aRecv->QWERLevelDefine, aRecv->EnableTransformMode, szName);
    }
...
void CDataServerProtocol::GS_DGAnsCastleInitData(int aIndex, CSP_REQ_CSINITDATA *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetCastleTotalInfo %d, %d", aRecv->wMapSvrNum,
                                   aRecv->iCastleEventCycle) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {

        lpMsg->iResult = TRUE;

        lpMsg->wStartYear = (WORD)
        this->m_CastleDB.GetAsInteger("SYEAR");
        lpMsg->btStartMonth = (BYTE)
        this->m_CastleDB.GetAsInteger("SMONTH");
        lpMsg->btStartDay = (BYTE)
        this->m_CastleDB.GetAsInteger("SDAY");
        lpMsg->wEndYear = (WORD)
        this->m_CastleDB.GetAsInteger("EYEAR");
        lpMsg->btEndMonth = (BYTE)
        this->m_CastleDB.GetAsInteger("EMONTH");
        lpMsg->btEndDay = (BYTE)
        this->m_CastleDB.GetAsInteger("EDAY");
        lpMsg->btIsSiegeGuildList = (BYTE)
        this->m_CastleDB.GetAsInteger("SIEGE_GUILDLIST_SETTED");
        lpMsg->btIsSiegeEnded = (BYTE)
        this->m_CastleDB.GetAsInteger("SIEGE_ENDED");
        lpMsg->btIsCastleOccupied = (BYTE)
        this->m_CastleDB.GetAsInteger("CASTLE_OCCUPY");
        ...
        if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetCastleNpcInfo %d", aRecv->wMapSvrNum) == TRUE &&
            this->m_CastleDB.Fetch() != SQL_NO_DATA) {
            do {
...
void CDataServerProtocol::GS_DGAnsOwnerGuildMaster(int aIndex, CSP_REQ_OWNERGUILDMASTER *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetOwnerGuildMaster %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        ...
        this->m_CastleDB.GetAsString("OwnerGuildMaster", pMsg.szCastleOwnGuildMaster,
                                     sizeof(pMsg.szCastleOwnGuildMaster));
    }
...
void CDataServerProtocol::GS_DGAnsCastleNpcBuy(int aIndex, CSP_REQ_NPCBUY *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqNpcBuy %d, %d, %d, %d, %d, %d, %d, %d, %d, %d",
                                   aRecv->wMapSvrNum, aRecv->iNpcNumber, aRecv->iNpcIndex, aRecv->iNpcDfLevel,
                                   aRecv->iNpcRgLevel, aRecv->iNpcMaxHp, aRecv->iNpcHp, aRecv->btNpcX, aRecv->btNpcY,
                                   aRecv->btNpcDIR) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsCastleNpcRepair(int aIndex, CSP_REQ_NPCREPAIR *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqNpcRepair %d, %d, %d", aRecv->wMapSvrNum, aRecv->iNpcNumber,
                                   aRecv->iNpcIndex) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
...
void CDataServerProtocol::GS_DGAnsCastleNpcUpgrade(int aIndex, CSP_REQ_NPCUPGRADE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqNpcUpgrade %d, %d, %d, %d, %d",
                                   aRecv->wMapSvrNum, aRecv->iNpcNumber, aRecv->iNpcIndex, aRecv->iNpcUpType,
                                   aRecv->iNpcUpValue) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsTaxInfo(int aIndex, CSP_REQ_TAXINFO *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetCastleTaxInfo %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = 1;
...
void CDataServerProtocol::GS_DGAnsTaxRateChange(int aIndex, CSP_REQ_TAXRATECHANGE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyTaxRate %d, %d, %d",
                                   aRecv->wMapSvrNum, aRecv->iTaxKind, aRecv->iTaxRate) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
...
void CDataServerProtocol::GS_DGAnsCastleMoneyChange(int aIndex, CSP_REQ_MONEYCHANGE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyMoney %d, %d", aRecv->wMapSvrNum, aRecv->iMoneyChanged) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
        pMsg.i64CastleMoney = this->m_CastleDB.GetAsInteger64("MONEY");
    }
...
void CDataServerProtocol::GS_DGAnsSiegeDateChange(int aIndex, CSP_REQ_SDEDCHANGE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyCastleSchedule %d, '%d-%d-%d 00:00:00', '%d-%d-%d 00:00:00'",
                                   ...
                                   aRecv->wEndYear, aRecv->btEndMonth, aRecv->btEndDay) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsSiegeEndedChange(int aIndex, CSP_REQ_SIEGEENDCHANGE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifySiegeEnd %d, %d", aRecv->wMapSvrNum, aRecv->bIsSiegeEnded) ==
        TRUE && this->m_CastleDB.Fetch()) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsCastleOwnerChange(int aIndex, CSP_REQ_CASTLEOWNERCHANGE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyCastleOwnerInfo %d, %d, '%s'", aRecv->wMapSvrNum,
                                   aRecv->bIsCastleOccupied, szOwnerGuildName) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsRegAttackGuild(int aIndex, CSP_REQ_REGATTACKGUILD *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqRegAttackGuild %d, '%s'", aRecv->wMapSvrNum, szEnemyGuildName) ==
        TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsRestartCastleState(int aIndex, CSP_REQ_CASTLESIEGEEND *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ResetCastleSiege %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsRegGuildMark(int aIndex, CSP_REQ_GUILDREGMARK *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqRegGuildMark %d, '%s'", aRecv->wMapSvrNum, szGuildName) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
        pMsg.iRegMarkCount = this->m_CastleDB.GetAsInteger("REG_MARKS");
    }
...
void CDataServerProtocol::GS_DGAnsGuildMarkReset(int aIndex, CSP_REQ_GUILDRESETMARK *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyGuildMarkReset %d, '%s'", aRecv->wMapSvrNum, szGuildName) ==
        TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
...
void CDataServerProtocol::GS_DGAnsGuildSetGiveUp(int aIndex, CSP_REQ_GUILDSETGIVEUP *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyGuildGiveUp %d, '%s', %d", aRecv->wMapSvrNum, szGuildName,
                                   aRecv->bIsGiveUp) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
...
void CDataServerProtocol::GS_DGAnsNpcRemove(int aIndex, CSP_REQ_NPCREMOVE *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqNpcRemove %d, %d, %d", aRecv->wMapSvrNum, aRecv->iNpcNumber,
                                   aRecv->iNpcIndex) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsCastleTributeMoney(int aIndex, CSP_REQ_CASTLETRIBUTEMONEY *aRecv) {
	...
	***********************************/
    ...
    if (aRecv->iCastleTributeMoney > 0) {
        ...
        if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ModifyMoney %d, %d", aRecv->wMapSvrNum,
                                       aRecv->iCastleTributeMoney) == TRUE && this->m_CastleDB.Fetch() != SQL_NO_DATA) {
            pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
            pMsg.i64CastleMoney = this->m_CastleDB.GetAsInteger64("MONEY");
        }
    }
...
void CDataServerProtocol::GS_DGAnsResetCastleTaxInfo(int aIndex, CSP_REQ_RESETCASTLETAXINFO *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ResetCastleTaxInfo %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsResetSiegeGuildInfo(int aIndex, CSP_REQ_RESETSIEGEGUILDINFO *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ResetSiegeGuildInfo %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsResetRegSiegeInfo(int aIndex, CSP_REQ_RESETREGSIEGEINFO *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ResetRegSiegeInfo %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        pMsg.iResult = this->m_CastleDB.GetAsInteger("QueryResult");
    }
...
void CDataServerProtocol::GS_DGAnsAllGuildMarkRegInfo(int aIndex, CSP_REQ_ALLGUILDREGINFO *aRecv) {
    ...
    lpMsg->iCount = 0;
    int iCount = 0;
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetAllGuildMarkRegInfo %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        do {
            this->m_CastleDB.GetAsString("REG_SIEGE_GUILD", lpMsgBody[iCount].szGuildName,
                                         sizeof(lpMsgBody->szGuildName));
    ...
    if (this->m_CastleDB.ExecQuery("DELETE FROM MuCastle_NPC WHERE MAP_SVR_GROUP = %d", aRecv->wMapSvrNum) == TRUE) {
        ...
        for (int i = 0; i < iCount; i++) {
            ...
            if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_ReqNpcBuy %d, %d, %d, %d, %d, %d, %d, %d, %d, %d",
                                           ...
                                           lpMsgBody[i].iNpcHp, lpMsgBody[i].btNpcX, lpMsgBody[i].btNpcY,
                                           lpMsgBody[i].btNpcDIR) != TRUE) {
                pMsg.iResult = 0;
                break;
            }
        }
...
void CDataServerProtocol::GS_DGAnsCsGulidUnionInfo(int aIndex, CSP_REQ_CSGUILDUNIONINFO *aRecv) {
    ...
    for (int i = 0; i < aRecv->iCount; i++) {
        ...
        if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetCsGuildUnionInfo '%s'", szGuildName) == TRUE &&
            this->m_CastleDB.Fetch() != SQL_NO_DATA) {
            do {
                this->m_CastleDB.GetAsString("GUILD_NAME", lpMsgBody[iCount].szGuildName,
                                             sizeof(lpMsgBody->szGuildName));
...
void CDataServerProtocol::GS_DGAnsCsSaveTotalGuildInfo(int aIndex, CSP_REQ_CSSAVETOTALGUILDINFO *aRecv) {
    ...
    CSP_CSSAVETOTALGUILDINFO *lpMsgBody = (CSP_CSSAVETOTALGUILDINFO *) ((PBYTE) aRecv +
    ...
    for (int i = 0; i < aRecv->iCount; i++) {
        ...
        if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_SetSiegeGuildInfo %d, '%s', %d, %d, %d",
                                       aRecv->wMapSvrNum, szGuildName, lpMsgBody[i].iCsGuildID,
                                       lpMsgBody[i].iGuildInvolved, lpMsgBody[i].iGuildScore) == TRUE) {
...
void CDataServerProtocol::GS_DGAnsCsLoadTotalGuildInfo(int aIndex, CSP_REQ_CSLOADTOTALGUILDINFO *aRecv) {
    ...
    if (this->m_CastleDB.ExecQuery("EXEC WZ_CS_GetSiegeGuildInfo %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CastleDB.Fetch() != SQL_NO_DATA) {
        lpMsg->iResult = TRUE;
        do {
...
void CDataServerProtocol::DGAnsCrywolfInfoLoad(int aIndex, CWP_REQ_CRYWOLFINFOLOAD *aRecv) {
    ...
    if (this->m_CrywolfDB.ExecQuery("EXEC WZ_CW_InfoLoad %d", aRecv->wMapSvrNum) == TRUE &&
        this->m_CrywolfDB.Fetch() != SQL_NO_DATA) {
        pMsg.btResult = TRUE;
...
void CDataServerProtocol::DGAnsCrywolfInfoSave(int aIndex, CWP_REQ_CRYWOLFINFOSAVE *aRecv) {
    ...
    if (this->m_CrywolfDB.ExecQuery("EXEC WZ_CW_InfoSave %d, %d, %d", aRecv->wMapSvrNum, aRecv->iCrywolfState,
                                    aRecv->iOccupationState) == TRUE) {
        pMsg.btResult = TRUE;
    }
...
void CDataServerProtocol::DGAnsPeriodItemExInsert(int aIndex, PMSG_REQ_PERIODITEMEX_INSERT *aRecv) {
    ...
    if (this->m_PeriodItemDB.ExecQuery(
            "EXEC IGC_PeriodItemInsertEx %d, '%s', %d, %d, %d, %d, %d, %I64d, %d, %I64d, %I64d",
            aRecv->dwUserGuid, aRecv->chCharacterName, aRecv->btItemType, aRecv->wItemCode, aRecv->btEffectCategory,
            aRecv->btEffectType1, aRecv->btEffectType2, pMsg.Serial, aRecv->dwDuration, aRecv->lBuyDate,
            aRecv->lExpireDate) && this->m_PeriodItemDB.Fetch() != SQL_NO_DATA) {
...
void CDataServerProtocol::DGAnsPeriodItemExDelete(int aIndex, PMSG_REQ_PERIODITEMEX_DELETE *aRecv) {
    ...
    if (this->m_PeriodItemDB.ExecQuery("EXEC IGC_PeriodItemDeleteEx %d, '%s', %d, %d, %I64d", aRecv->dwUserGuid,
                                       aRecv->chCharacterName, aRecv->btItemType, aRecv->wItemCode, aRecv->Serial) ==
        TRUE) {
        pMsg.btResultCode = 0;
    }
...
void CDataServerProtocol::ReqInGameShopItemBuy(short aIndex, ISHOP_ITEM_BUY *aRecv) {
    ...
    this->m_ItemShopDB.ExecQuery("SELECT %s AS Result FROM T_InGameShop_Point WHERE AccountID = '%s'",
                                 aRecv->CoinType == 0 ? "WCoinC" : aRecv->CoinType == 1 ? "WCoinP" : "GoblinPoint",
                                 aRecv->AccountID);
    ...
    if (Coin < aRecv->Price) {
        ...
        if (this->m_ItemShopDB.GetAsInteger("Result") == 1) {
            ...
            this->m_ItemShopDB.ExecQuery("UPDATE T_InGameShop_Point SET %s = %d WHERE AccountID = '%s'",
                                         aRecv->CoinType == 0 ? "WCoinC" : aRecv->CoinType == 1 ? "WCoinP"
                                                                                                : "GoblinPoint", Coin,
                                         aRecv->AccountID);
...
void CDataServerProtocol::ReqInGameShopItemGift(short aIndex, ISHOP_ITEM_GIFT *aRecv) {
    ...
    this->m_ItemShopDB.ExecQuery("EXEC WZ_IBS_AddGift '%s', %d, %d, %d, %d, '%s', '%s'", aRecv->TargetName, aRecv->ID1,
                                 aRecv->ID2, aRecv->ID3, 2, aRecv->Name, aRecv->Message);
...
void CDataServerProtocol::ReqInGameShopItemUse(short aIndex, ISHOP_ITEM_USE *aRecv) {
    ...
    this->m_ItemShopDB.ExecQuery("EXEC WZ_IBS_GetItemToUse '%s', %d, %d", aRecv->AccountID, aRecv->UniqueCode,
                                 aRecv->AuthCode);
    this->m_ItemShopDB.Fetch();
...
void CDataServerProtocol::ReqInGameShopPoint(short aIndex, ISHOP_REQ_POINT *aRecv) {
    ...
    this->m_ItemShopDB.ExecQuery("SELECT WCoinP, WCoinC, GoblinPoint FROM T_InGameShop_Point WHERE AccountID = '%s'",
                                 aRecv->AccountID);

    if (this->m_ItemShopDB.Fetch() == SQL_NO_DATA) {
        this->m_ItemShopDB.Close();
        this->m_ItemShopDB.ExecQuery("INSERT INTO T_InGameShop_Point VALUES ('%s', 0.00, 0.00, 0.00)",
                                     aRecv->AccountID);
        this->m_ItemShopDB.Close();
    } else {
...
void CDataServerProtocol::ReqInGameShopPackageBuy(short aIndex, LPBYTE aRecv) {
    ...
    this->m_ItemShopDB.ExecQuery("SELECT %s AS Result FROM T_InGameShop_Point WHERE AccountID = '%s'",
                                 lpMsg->CoinType == 0 ? "WCoinC" : lpMsg->CoinType == 1 ? "WCoinP" : "GoblinPoint",
                                 lpMsg->AccountID);
    this->m_ItemShopDB.Fetch();
...
void CDataServerProtocol::ReqInGameShopItemDelete(short aIndex, ISHOP_ITEM_DELETE *aRecv) {
    this->m_ItemShopDB.ExecQuery("EXEC WZ_IBS_DeleteItem '%s', %d, %d", aRecv->AccountID, aRecv->UniqueCode,
                                 aRecv->AuthCode);
    this->m_ItemShopDB.Close();
}

void CDataServerProtocol::ReqInGameShopItemRollbackUse(short aIndex, ISHOP_ITEM_ROLLBACK *aRecv) {
    this->m_ItemShopDB.ExecQuery(
            "SELECT UsedItem FROM T_InGameShop_Items WHERE UniqueCode = %d AND AuthCode = %d AND AccountID = '%s'",
            aRecv->UniqueCode, aRecv->AuthCode, aRecv->AccountID);
    this->m_ItemShopDB.Fetch();
    ...
    if (iUsedFlag == TRUE) {
        this->m_ItemShopDB.ExecQuery(
                "UPDATE T_InGameShop_Items SET UsedItem = 0 WHERE UniqueCode = %d AND AuthCode = %d AND AccountID = '%s'",
                aRecv->UniqueCode, aRecv->AuthCode, aRecv->AccountID);
    }
}
...
void CDataServerProtocol::ReqRegLuckyCoin(int aIndex, PMSG_REQ_REGISTER_LUCKYCOIN *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("UPDATE T_LuckyCoin SET LuckyCoin = LuckyCoin + 1 WHERE AccountID = '%s'",
                                  pMsg.szUID)) {
        this->m_EventDB.Close();
...
void CDataServerProtocol::DevilSqureScore(PMSG_ANS_EVENTUSERSCORE *lpMsg) {
    ...
    g_Log.Add("[DEVIL] SERVER:%d-%d, SQUARE:%d, ACCNT:%s, CHAR:%s, CLASS:%d, SCORE:%d",
              lpMsg->ServerCode / 20 + 1, lpMsg->ServerCode % 20 + 1, lpMsg->SquareNum, szAccount, szName, lpMsg->Class,
              lpMsg->Score);

    this->m_RankingDB.ExecQuery("EXEC SP_POINT_ACCUMULATION '%d', '%d', '%s', '%s', '%d', '%d'",
                                lpMsg->ServerCode / 20 + 1, lpMsg->SquareNum, szAccount, szName, lpMsg->Class,
                                lpMsg->Score);
...
void CDataServerProtocol::GDReqBloodCastleEnterCount(int aIndex, PMSG_REQ_BLOODCASTLE_ENTERCOUNT *lpMsg) {
    ...
    if (this->m_RankingDB.ExecQuery("EXEC SP_LEFT_ENTERCOUNT_BC '%s', '%s', '%d'", lpMsg->AccountID, lpMsg->GameID,
                                    lpMsg->ServerCode) == TRUE) {
...
void CDataServerProtocol::BloodCastleScore_5TH(PMSG_ANS_BLOODCASTLESCORE_5TH *lpMsg) {
    ...
    g_Log.Add("[BLOOD] SERVER:%d-%d, BRIDGE:%d, ACCNT:%s, CHAR:%s, CLASS:%d, SCORE:%d, LEFTTIME:%d, ALIVEPARTY:%d",
              lpMsg->ServerCode / 20 + 1, lpMsg->ServerCode % 20 + 1, lpMsg->BridgeNum, szAccount, szName, lpMsg->Class,
              lpMsg->Score, lpMsg->iLeftTime, lpMsg->iAlivePartyCount);

    this->m_RankingDB.ExecQuery("EXEC SP_POINT_ACCM_BC_5TH '%d', '%d', '%s', '%s', '%d', '%d', '%d', '%d'",
                                lpMsg->ServerCode / 20 + 1, lpMsg->BridgeNum, szAccount, szName, lpMsg->Class,
                                lpMsg->Score, lpMsg->iLeftTime, lpMsg->iAlivePartyCount);
}
...
void CDataServerProtocol::IllusionTempleScore(PMSG_ANS_ILLUSIONTEMPLE_RANKING *lpMsg) {
    ...
    g_Log.Add(
            "[ILLUSION] SERVER:%d-%d, TEMPLE:%d ACCNT:%s, CHAR:%s, CLASS:%d, SCORE:%d, KILLCOUNT:%d, RELICS:%d, EXPERIENCE:%d, ISWINNER:%d",
            lpMsg->ServerCode / 20 + 1, lpMsg->ServerCode % 20 + 1, lpMsg->TempleNum, szAccountID, szName, lpMsg->Class,
            lpMsg->TotalScore, lpMsg->KillCount, lpMsg->RelicsGivenCount, lpMsg->Experience, lpMsg->IsWinner);

    this->m_RankingDB.ExecQuery("EXEC SP_POINT_ACCM_IT '%d', '%d', '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d'",
                                lpMsg->ServerCode / 20 + 1, lpMsg->TempleNum, szAccountID, szName, lpMsg->Class,
                                lpMsg->TotalScore, lpMsg->KillCount, lpMsg->RelicsGivenCount, lpMsg->Experience,
                                lpMsg->IsWinner);
}
...
void CDataServerProtocol::ChaosCastleScore(PMSG_ANS_CHAOSCASTLE_RANKING *lpMsg) {
    ...
    g_Log.Add(
            "[CHAOS] SERVER:%d-%d, CASTLE:%d, ACCNT:%s CHAR:%s, CLASS:%d, PKILL:%d, MKILL:%d, EXPERIENCE:%d, WINNER:%d",
            lpMsg->ServerCode / 20 + 1, lpMsg->ServerCode % 20 + 1, lpMsg->Castle, szAccountID, szName, lpMsg->Class,
            lpMsg->PlayerKill, lpMsg->MonsterKill, lpMsg->Experience, lpMsg->IsWinner);

    this->m_RankingDB.ExecQuery("EXEC SP_POINT_ACCM_CC '%d', '%d', '%s', '%s', '%d', '%d', '%d', '%d', '%d'",
                                lpMsg->ServerCode / 20 + 1, lpMsg->Castle, szAccountID, szName, lpMsg->Class,
                                lpMsg->PlayerKill, lpMsg->MonsterKill, lpMsg->Experience, lpMsg->IsWinner);
}
...
void CDataServerProtocol::EGAnsEventChipInfo(int aIndex, PMSG_REQ_VIEW_EC_MN *lpMsg) {
    ...
    if (this->m_CharMiscDB.ExecQuery(
            "SELECT EventChips, Check_Code, MuttoNumber FROM T_MU2003_EVENT WHERE AccountID='%s'", lpMsg->szUID) ==
        FALSE) {
        pMsg.bSUCCESS = FALSE;
    } else {
        if (this->m_CharMiscDB.Fetch() != SQL_NO_DATA) {
            ...
            if (this->m_CharMiscDB.ExecQuery("INSERT INTO T_MU2003_EVENT (AccountID) VALUES ('%s')", pMsg.szUID) ==
                FALSE) {
                pMsg.bSUCCESS = FALSE;
            } else {
                pMsg.nEVENT_CHIPS = 0;
...
void CDataServerProtocol::EGAnsRegEventChipInfo(int aIndex, PMSG_REQ_REGISTER_EVENTCHIP *lpMsg) {
    ...
    if (this->m_CharMiscDB.ExecQuery("UPDATE T_MU2003_EVENT SET EventChips = EventChips + 1 WHERE AccountID = '%s'",
                                     pMsg.szUID)) {
        this->m_CharMiscDB.Close();
...
void CDataServerProtocol::EGAnsEventStoneInfo(int aIndex, PMSG_REQ_VIEW_EC_MN *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("SELECT StoneCount, Check_Code FROM T_BLOOD_CASTLE WHERE AccountID='%s'",
                                  pMsg.szUID) == FALSE) {
        pMsg.bSUCCESS = FALSE;
    } else {
        if (this->m_EventDB.Fetch() != SQL_NO_DATA) {
...
void CDataServerProtocol::EGAnsRegEventStoneInfo(int aIndex, PMSG_REQ_REGISTER_STONES *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("UPDATE T_BLOOD_CASTLE SET StoneCount = StoneCount + 1 WHERE AccountID = '%s'",
                                  pMsg.szUID)) {
        this->m_EventDB.Close();
...
void CDataServerProtocol::EGAnsDeleteStones(int aIndex, PMSG_REQ_DELETE_STONES *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("SELECT StoneCount FROM T_BLOOD_CASTLE WHERE AccountID='%s'", pMsg.szUID) == FALSE ||
        this->m_EventDB.Fetch() == SQL_NO_DATA) {
        pMsg.bSUCCESS = FALSE;
    } else {
        ...
        if (Stones != -1 && Stones >= lpMsg->iStoneCount) {
            if (this->m_EventDB.ExecQuery("UPDATE T_BLOOD_CASTLE SET StoneCount = StoneCount - %d WHERE AccountID='%s'",
                                          lpMsg->iStoneCount, pMsg.szUID) == FALSE) {
...
void CDataServerProtocol::EGAns2AnivRegSerial(int aIndex, PMSG_REQ_2ANIV_SERIAL *lpMsg) {
    ...
    if (SpaceSyntexCheck(lpMsg->SERIAL1) == FALSE || QuoteSpaceSyntexCheck(lpMsg->SERIAL1) == FALSE ||
        PercentSyntaxCheck(lpMsg->SERIAL1) == FALSE) {
        pMsg.btIsRegistered = 4;
        DataSend(aIndex, (LPBYTE) & pMsg, sizeof(pMsg), __FUNCTION__);
        return;
    }

    if (SpaceSyntexCheck(lpMsg->SERIAL2) == FALSE || QuoteSpaceSyntexCheck(lpMsg->SERIAL2) == FALSE ||
        PercentSyntaxCheck(lpMsg->SERIAL2) == FALSE) {
        pMsg.btIsRegistered = 4;
        DataSend(aIndex, (LPBYTE) & pMsg, sizeof(pMsg), __FUNCTION__);
        return;
    }

    if (SpaceSyntexCheck(lpMsg->SERIAL3) == FALSE || QuoteSpaceSyntexCheck(lpMsg->SERIAL3) == FALSE ||
        PercentSyntaxCheck(lpMsg->SERIAL3) == FALSE) {
        pMsg.btIsRegistered = 4;
        DataSend(aIndex, (LPBYTE) & pMsg, sizeof(pMsg), __FUNCTION__);
        return;
    }

    if (this->m_EventDB.ExecQuery("EXEC SP_REG_SERIAL '%s', '%d', '%s', '%s', '%s'", lpMsg->szUID, lpMsg->iMEMB_GUID,
                                  lpMsg->SERIAL1, lpMsg->SERIAL2, lpMsg->SERIAL3) &&
        this->m_EventDB.Fetch() != SQL_NO_DATA) {
        pMsg.btIsRegistered = (BYTE)
        this->m_EventDB.GetAsInteger("RegResult");
...
void CDataServerProtocol::EGAnsResetStoneInfo(int aIndex, PMSG_REQ_RESET_EVENTCHIP *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("UPDATE T_BLOOD_CASTLE SET Check_Code = 2 WHERE AccountID = '%s'", lpMsg->szUID) ==
        FALSE) {
        pMsg.bSUCCESS = FALSE;
    } else {
        pMsg.bSUCCESS = TRUE;
...
void CDataServerProtocol::EGAnsRegCCOfflineGift(int aIndex, PMSG_REQ_REG_CC_OFFLINE_GIFT *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("EXEC SP_REG_CC_OFFLINE_GIFT '%s', '%s', %d", lpMsg->szUID, lpMsg->szNAME,
                                  lpMsg->wServerCode) == FALSE || this->m_EventDB.Fetch() == SQL_NO_DATA) {
...
void CDataServerProtocol::EGAnsRegDLOfflineGift(int aIndex, PMSG_REQ_REG_DL_OFFLINE_GIFT *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("EXEC SP_REG_DL_OFFLINE_GIFT '%s', '%s', %d", lpMsg->szUID, lpMsg->szNAME,
                                  lpMsg->wServerCode) == FALSE || this->m_EventDB.Fetch() == SQL_NO_DATA) {
...
void CDataServerProtocol::EGAnsRegHTOfflineGift(int aIndex, PMSG_REQ_REG_HT_OFFLINE_GIFT *lpMsg) {
    ...
    if (this->m_EventDB.ExecQuery("EXEC SP_REG_HT_OFFLINE_GIFT '%s', '%s', %d", lpMsg->szUID, lpMsg->szNAME,
                                  lpMsg->wServerCode) == FALSE || this->m_EventDB.Fetch() == SQL_NO_DATA) {
...
void CDataServerProtocol::ReqSetExpandedWarehouse(short aIndex, SDHP_EXPANDEDWAREHOUSE_SET *aRecv) {
    if (aRecv->ExpansionType == 2) {
        this->m_AccDB.ExecQuery("UPDATE AccountCharacter SET WarehouseExpansion=%d WHERE Id='%s'",
                                aRecv->ExpansionLevel, aRecv->AccountID);
    } else if (aRecv->ExpansionType == 1) {
        this->m_AccDB.ExecQuery("UPDATE Character SET InventoryExpansion=%d WHERE Name='%s'", aRecv->ExpansionLevel,
                                aRecv->AccountID);
    }
}
...
void CDataServerProtocol::ReqSavePlayerKiller(short aIndex, DS_SAVE_PLAYERKILLER *aRecv) {
    this->m_CharMiscDB.ExecQuery(
            "INSERT INTO C_PlayerKiller_Info (Victim, Killer,KillDate) VALUES ('%s','%s',GETDATE() )", aRecv->Victim,
            aRecv->Killer);
    this->m_CharMiscDB.Close();
}
...
void CDataServerProtocol::GDReqArcaBattleGuildJoin(int aIndex, _tagPMSG_REQ_ARCA_BATTLE_GUILD_JOIN_DS *aRecv) {
    ...
    this->m_ArcaDB.ExecQuery("EXEC IGC_ArcaBattleGuildInsert '%s', '%s', %d", aRecv->szGuildName, aRecv->szGuildMaster,
                             aRecv->dwGuild);
    this->m_ArcaDB.Fetch();
...
void
CDataServerProtocol::GDReqArcaBattleGuildMemberJoin(int aIndex, _tagPMSG_REQ_ARCA_BATTLE_GUILD_MEMBER_JOIN_DS *aRecv) {
    _tagPMSG_ANS_ARCA_BATTLE_GUILD_MEMBER_JOIN_DS pMsg;

    PHeadSubSetB((LPBYTE) & pMsg, 0xF8, 0x33, sizeof(pMsg));

    this->m_ArcaDB.ExecQuery("EXEC IGC_ArcaBattleGuildMemberInsert '%s', '%s', %d", aRecv->szGuildName,
                             aRecv->szCharName, aRecv->dwGuild);
    this->m_ArcaDB.Fetch();
...
void
CDataServerProtocol::GDReqArcaBattleWinGuildInfoInsert(int aIndex, _tagPMSG_REQ_AB_WIN_GUILD_INFO_INSERT_DS *aRecv) {
    ...
    for (int i = 0; i < aRecv->btGuildCnt; i++) {
        this->m_ArcaDB.ExecQuery("EXEC IGC_ArcaBattleWinGuildInsert '%s', %d, %d, %d, %I64d",
                                 ...
                                 aRecv->m_stABWinGuildInfoDS[i].wOccupyObelisk,
                                 aRecv->m_stABWinGuildInfoDS[i].wObeliskGroup, 0);
...
void CDataServerProtocol::GDReqArcaBattleJoinMemberUnderReq(int aIndex, _tagPMSG_REQ_AB_JOIN_CANCEL_DS *aRecv) {
    ...
    for (int i = 0; i < iGuildNamesCnt; i++) {
        this->m_ArcaDB.ExecQuery("EXEC IGC_ArcaBattleMinGuildSelect '%s', %d", szGuildNames[i],
                                 aRecv->btMinGuildMemNum);
        this->m_ArcaDB.Fetch();
...
void CDataServerProtocol::GDReqArcaBattleMarkReg(int aIndex, _tagPMSG_REQ_ARCA_BATTLE_MARK_REG_DS *aRecv) {
    ...
    this->m_ArcaDB.ExecQuery("EXEC IGC_ArcaBattleMarkInsert '%s', %d, '%s', %I64d", aRecv->szGuildName,
                             aRecv->dwGuildNum, aRecv->szGuildMaster, (INT64) aRecv->dwMarkCnt);
    this->m_ArcaDB.Fetch();
...
void CDataServerProtocol::GDReqArcaBattleMarkRank(int aIndex, _tagPMSG_REQ_ARCA_BATTLE_MARK_RANK_DS *aRecv) {
    ...
    while (this->m_ArcaDB.Fetch() == SQL_SUCCESS) {
        ...
        this->m_ArcaDB.GetAsString("G_Name", pMsg.ArcaBattleMarkTopRank[pMsg.btGuildCnt].szGuildNames,
                                   MAX_GUILD_LEN + 1);
...
void CDataServerProtocol::GDReqArcaBattleMarkRegSet(int aIndex, _tagPMSG_REQ_AB_MARK_REG_UPDATE_DS *aRecv) {
    this->m_ArcaDB.ExecQuery("UPDATE IGC_ARCA_BATTLE_GUILDMARK_REG SET MarkCnt = %d WHERE G_Number = %d",
                             aRecv->dwMarkCnt, aRecv->dwGuildNum);
}
...
void CDataServerProtocol::GDReqPeriodBuffInsert(int aIndex, PMSG_REQ_PERIODBUFF_INSERT *aRecv) {
    this->m_PeriodItemDB.ExecQuery("EXEC IGC_PeriodBuffInsert '%s', %d, %d, %d, %d, %I64d", aRecv->szCharacterName,
                                   aRecv->wBuffIndex, aRecv->btEffectType1, aRecv->btEffectType2, aRecv->dwDuration,
                                   aRecv->lExpireDate);
}
...
void CDataServerProtocol::GDReqQuestExpInfoSave(int aIndex, LPBYTE lpRecv) {
    ...
    for (int i = 0; i < lpMsg->btQuestCnt; i++) {
        ...
        sprintf(szTemp, "EXEC IGC_QuestExpUserInfoSave '%s', %d, %d, %d, %I64d, %I64d, ?, ?, ?", lpMsg->szCharName,
                iEpisode, iQuestSwitch, wProgState, lStatrDate, lEndDate);
...
void CDataServerProtocol::GDReqLuckyItemDelete(int aIndex, PMSG_REQ_LUCKYITEM_DELETE *lpMsg) {
    this->m_LuckyItemDB.ExecQuery("EXEC IGC_LuckyItemDelete '%s', %d, %u", lpMsg->szCharName, lpMsg->wItemCode,
                                  lpMsg->Serial);
    this->m_LuckyItemDB.Fetch();
    this->m_LuckyItemDB.Close();
}
...
void CDataServerProtocol::GDReqGetPentagramJewel(int aIndex, PMSG_REQ_PENTAGRAMJEWEL *lpMsg) {
    ...
    this->m_PentagramDB.ExecQuery("EXEC IGC_PentagramInfoSelect %d, '%s', '%s', %d", lpMsg->iUserGuid,
                                  lpMsg->szAccountID, lpMsg->szName, lpMsg->btJewelPos);
...
void CDataServerProtocol::GDReqSetPentagramJewel(int aIndex, LPBYTE lpRecv) {
    ...
    for (int i = 0; i < lpMsg->btJewelCnt; i++) {
        PENTAGRAMJEWEL_INFO *lpMsgBody = (PENTAGRAMJEWEL_INFO *) (lpRecv + sizeof(PMSG_REQ_SETPENTAGRAMJEWEL) +
                                                                  (i * sizeof(PENTAGRAMJEWEL_INFO)));

        this->m_PentagramDB.ExecQuery(
                "EXEC IGC_PentagramInfoUpdate %d, '%s', '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d",
                lpMsg->iUserGuid, lpMsg->szAccountID, lpMsg->szName, lpMsg->btJewelPos, lpMsgBody->btJewelIndex,
                lpMsgBody->btItemType, lpMsgBody->wItemIndex,
                lpMsgBody->btMainAttribute, lpMsgBody->btLevel, lpMsgBody->btRank1OptionNum, lpMsgBody->btRank1Level,
                lpMsgBody->btRank2OptionNum, lpMsgBody->btRank2Level,
                lpMsgBody->btRank3OptionNum, lpMsgBody->btRank3Level, lpMsgBody->btRank4OptionNum,
                lpMsgBody->btRank4Level, lpMsgBody->btRank5OptionNum, lpMsgBody->btRank5Level);
...
void CDataServerProtocol::GDReqDelPentagramJewel(int aIndex, PMSG_DEL_PENTAGRAMJEWEL *lpMsg) {
    this->m_PentagramDB.ExecQuery("EXEC IGC_PentagramInfoDelete %d, '%s', '%s', %d, %d",
                                  lpMsg->iUserGuid, lpMsg->szAccountID, lpMsg->szName, lpMsg->btJewelPos,
                                  lpMsg->btJewelIndex);
...
void CDataServerProtocol::GDReqInsertPentagramJewel(int aIndex, PMSG_INSERT_PENTAGRAMJEWEL *lpMsg) {
    this->m_PentagramDB.ExecQuery(
            "EXEC IGC_PentagramInfoUpdate %d, '%s', '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d",
            lpMsg->iUserGuid, lpMsg->szAccountID, lpMsg->szName, lpMsg->btJewelPos, lpMsg->btJewelPos,
            lpMsg->btItemType, lpMsg->wItemIndex,
            lpMsg->btMainAttribute, lpMsg->btLevel, lpMsg->btRank1OptionNum, lpMsg->btRank1Level,
            lpMsg->btRank2OptionNum, lpMsg->btRank2Level,
            lpMsg->btRank3OptionNum, lpMsg->btRank3Level, lpMsg->btRank4OptionNum, lpMsg->btRank4Level,
            lpMsg->btRank5OptionNum, lpMsg->btRank5Level);

    this->m_PentagramDB.Close();
}

void CDataServerProtocol::GDReqChaosCastleFinalSave(int aIndex, _tagPMSG_REQ_SAVE_CCF_RESULT *lpMsg) {
    this->m_CCFinalDB.ExecQuery("EXEC IGC_ChaosCastleFinal_Save '%s', %d, %d, %d, %I64d, %d", lpMsg->szCharName,
                                lpMsg->nPoint, lpMsg->nCharClass, lpMsg->nCharLevel, lpMsg->nCharExp, lpMsg->byCCFType);
...
void CDataServerProtocol::GDReqChaosCastleFinalPermission(int aIndex, _tagPMSG_REQ_CCF_PERMISSION *lpMsg) {
    ...
    this->m_CCFinalDB.ExecQuery("EXEC IGC_ChaosCastleFinal_GetPermission '%s', %d", lpMsg->szCharName,
                                lpMsg->byCCFType);
...
void CDataServerProtocol::GDReqLoadMuunInvenItem(int aIndex, SDHP_REQ_DBMUUN_INVEN_LOAD *lpMsg) {
    ...
    if (this->m_MuunDB.Fetch() == SQL_NO_DATA) {
        this->m_MuunDB.Close();
        this->m_MuunDB.ExecQuery(
                "INSERT INTO PetWarehouse (AccountID,Name,DbVersion,SubEquip) VALUES ('%s', '%s','%d','%d')",
                lpMsg->AccountID, lpMsg->Name, 3, 0);
...
void CDataServerProtocol::GDReqLoadEventInvenItem(int aIndex, _tagSDHP_REQ_DBEVENT_INVEN_LOAD *aRecv) {
    ...
    if (this->m_EventInvDB.Fetch() == SQL_NO_DATA) {
        ...
        this->m_EventInvDB.ExecQuery("INSERT INTO T_Event_Inventory (AccountID, Name) VALUES ('%s', '%s')",
                                     aRecv->AccountID, aRecv->Name);
...
void CDataServerProtocol::GDReqCardInfoInsert(int aIndex, _tagPMSG_REQ_MURUMMY_INSERT_DS *aRecv) {
    ...
    for (int i = 0; i < 24; i++) {
        this->m_RummyDB.ExecQuery("EXEC IGC_SetInsertMuRummy '%s', '%s',%d, %d, %d, %d, %d", aRecv->AccountID,
                                  aRecv->Name,
                                  aRecv->stMuRummyCardInfoDS[i].btNumber, aRecv->stMuRummyCardInfoDS[i].btColor,
                                  aRecv->stMuRummyCardInfoDS[i].btSlotNum, aRecv->stMuRummyCardInfoDS[i].btStatus,
                                  aRecv->stMuRummyCardInfoDS[i].btSeq);
...
void CDataServerProtocol::GDReqScoreUpdate(int aIndex, _tagPMSG_REQ_MURUMMY_SCORE_UPDATE_DS *aRecv) {
    this->m_RummyDB.ExecQuery("EXEC IGC_SetSaveMuRummyInfo '%s', '%s', %d", aRecv->AccountID, aRecv->Name,
                              aRecv->wScore);
    this->m_RummyDB.Fetch();
    this->m_RummyDB.Close();
    ...
    for (int i = 0; i < 3; i++) {
        ...
        this->m_RummyDB.ExecQuery("EXEC IGC_SetUpdateMuRummy  '%s', '%s', %d, %d, %d", aRecv->AccountID, aRecv->Name,
                                  aRecv->stCardUpdateDS[i].btSlotNum, aRecv->stCardUpdateDS[i].btStatus,
                                  aRecv->stCardUpdateDS[i].btSeq);
        this->m_RummyDB.Fetch();
        this->m_RummyDB.Close();
    }
}
...
void CDataServerProtocol::GDReqCardInfoUpdate(int aIndex, _tagPMSG_REQ_MURUMMY_UPDATE_DS *aRecv) {
    this->m_RummyDB.ExecQuery("EXEC IGC_SetUpdateMuRummy  '%s', '%s', %d, %d, %d", aRecv->AccountID, aRecv->Name,
                              aRecv->btSlotNum, aRecv->btStatus, aRecv->btSequence);
    this->m_RummyDB.Fetch();
    this->m_RummyDB.Close();
}
...
void CDataServerProtocol::GDReqSlotInfoUpdate(int aIndex, _tagPMSG_REQ_MURUMMY_SLOTUPDATE_DS *aRecv) {
    ...
    this->m_RummyDB.ExecQuery("EXEC IGC_SetUpdateMuRummy  '%s', '%s', %d, %d, %d", aRecv->AccountID, aRecv->Name,
                              aRecv->stCardUpdateDS.btSlotNum, aRecv->stCardUpdateDS.btStatus,
                              aRecv->stCardUpdateDS.btSeq);
    this->m_RummyDB.Fetch();
    this->m_RummyDB.Close();
}
...
void CDataServerProtocol::GDReqMuRummyInfoUpdate(int aIndex, _tagPMSG_REQ_MURUMMY_INFO_UPDATE_DS *aRecv) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        this->m_RummyDB.ExecQuery("EXEC IGC_SetUpdateMuRummy  '%s', '%s', %d, %d, %d", aRecv->AccountID, aRecv->Name,
                                  aRecv->stMuRummyCardUpdateDS[i].btSlotNum, aRecv->stMuRummyCardUpdateDS[i].btStatus,
                                  aRecv->stMuRummyCardUpdateDS[i].btSeq);
        this->m_RummyDB.Fetch();
        this->m_RummyDB.Close();
    }
}
...
void CDataServerProtocol::GDReqMuRummyDBLog(int aIndex, _tagPMSG_REQ_MURUMMY_LOG_INSERT_DS *aRecv) {
    this->m_RummyDB.ExecQuery("EXEC IGC_SetSaveMuRummyLog '%s', '%s', %d", aRecv->AccountID, aRecv->Name,
                              aRecv->wScore);
    this->m_RummyDB.Fetch();
    this->m_RummyDB.Close();
}
...
void CDataServerProtocol::GDReqMineModifyUPTUserInfo(int aIndex, SDHP_REQ_MINESYSTEM_UPT_USERINFO *aRecv) {
    if (aRecv->byRequestType == 0) {
        this->m_MineDB.ExecQuery("EXEC IGC_MineSystem_Insert_UPTUserInfo '%s', %d, %d", aRecv->szCharName,
                                 aRecv->wTwinkleType, aRecv->iCurrentStage);
        this->m_MineDB.Fetch();
        this->m_MineDB.Close();
...
void CDataServerProtocol::GDAllSavePShopItemValue(int aIndex, PMSG_UPDATE_PSHOPITEMVALUE_INFO *aRecv) {
    for (int i = 0; i < aRecv->btItemCnt; i++) {
        this->m_PShopDB.ExecQuery("EXEC IGC_PShopItemValueInfoSave '%s', '%s', %d, %I64d, %d, %d, %d, %d",
                                  aRecv->AccountId, aRecv->szName, aRecv->PShopItemValueInfo[i].nPShopItemInvenNum,
                                  aRecv->PShopItemValueInfo[i].ItemSerial,
                                  aRecv->PShopItemValueInfo[i].nMoney, aRecv->PShopItemValueInfo[i].sBlessJewelValue,
                                  aRecv->PShopItemValueInfo[i].sSoulJewelValue,
...
void CDataServerProtocol::GDDelPShopItemValue(int aIndex, PMSG_DEL_PSHOPITEM *aRecv) {
    this->m_PShopDB.ExecQuery("EXEC IGC_PShopItemValueInfoDel '%s', '%s', %d", aRecv->AccountId, aRecv->szName,
                              aRecv->nPShopItemInvenNum);
    this->m_PShopDB.Fetch();
    this->m_PShopDB.Close();
}

void CDataServerProtocol::GDMovePShopItem(int aIndex, PMSG_MOVE_PSHOPITEM *aRecv) {
    this->m_PShopDB.ExecQuery("EXEC IGC_PShopItemMove '%s', '%s', %d, %d", aRecv->AccountId, aRecv->szName,
                              aRecv->nOldPShopItemInvenNum, aRecv->nNewPShopItemInvenNum);
    this->m_PShopDB.Fetch();
    this->m_PShopDB.Close();
}
...
void CDataServerProtocol::GDReqReBuyItemList(int aIndex, SDHP_REQ_SHOP_REBUY_LIST *aRecv) {
    this->m_ReBuyDB.ExecQuery("EXEC IGC_CancelItemSale_ClearExpiredItems '%s', '%s', %I64d", aRecv->szAccountID,
                              aRecv->szName, aRecv->CurrTime);
    ...
    if (this->m_ReBuyDB.ExecQuery("EXEC IGC_CancelItemSale_GetItemList '%s', '%s'", aRecv->szAccountID,
                                  aRecv->szName) == TRUE) {
        lpMsg->btResult = 0;
    }

    while (this->m_ReBuyDB.Fetch() == SQL_SUCCESS) {
        lpItem = (SDHP_SHOP_REBUY_ITEM *) (BUFFER + sizeof(SDHP_ANS_SHOP_REBUY_LIST) +
                                           (lpMsg->iCount * sizeof(SDHP_SHOP_REBUY_ITEM)));
...
void CDataServerProtocol::GDReqReBuyAddItem(int aIndex, SDHP_REQ_SHOP_REBUY_ADD_ITEM *aRecv) {
    this->m_ReBuyDB.ExecQuery("EXEC IGC_CancelItemSale_ClearExpiredItems '%s', '%s', %I64d", aRecv->szAccountID,
                              aRecv->szName, aRecv->SellDate);
    this->m_ReBuyDB.Close();

    this->m_ReBuyDB.ExecQuery(
            "EXEC IGC_CancelItemSale_AddItem '%s', '%s', %I64d, %I64d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %I64d",
            aRecv->szAccountID, aRecv->szName, aRecv->SellDate, aRecv->SellExpireDate, aRecv->dwSellPrice,
            aRecv->wItemCode, aRecv->btItemLevel,
            aRecv->btItemOp1, aRecv->btItemOp2, aRecv->btItemOp3, aRecv->btItemDur, aRecv->btItemExcOp,
            aRecv->btItemSetOp, aRecv->btItemHarmonyOp, aRecv->btItemOpEx,
            aRecv->btItemSocketOp[0], aRecv->btItemSocketOp[1], aRecv->btItemSocketOp[2], aRecv->btItemSocketOp[3],
            aRecv->btItemSocketOp[4], aRecv->btItemBonusSocketOp, aRecv->ItemSerial);
    this->m_ReBuyDB.Close();
}
...
void CDataServerProtocol::GDReqReBuyGetItem(int aIndex, SDHP_REQ_SHOP_REBUY_GET_ITEM *aRecv) {
    ...
    if (this->m_ReBuyDB.ExecQuery("EXEC IGC_CancelItemSale_GetItemToReBuy '%s', '%s', %d, %d, %d", aRecv->szAccountID,
                                  aRecv->szName, aRecv->btItemNumber, aRecv->wItemCode, aRecv->dwItemPrice) == FALSE) {
        pMsg.btResult = 1;
...
void CDataServerProtocol::GDReqDeleteSoldItem(int aIndex, SDHP_REQ_SHOP_REBUY_DELETE_ITEM *aRecv) {
    this->m_ReBuyDB.ExecQuery("EXEC IGC_CancelItemSale_DeleteItem '%s', '%s', %d, %d, %d", aRecv->szAccountID,
                              aRecv->szName, aRecv->btItemNumber, aRecv->wItemCode, aRecv->dwItemPrice);
    this->m_ReBuyDB.Close();
}
...
void CDataServerProtocol::GDReqGremoryCaseItemList(int aIndex, _stReqGremoryCaseItemList *aRecv) {
    ...
    if (this->m_GremoryCaseDB.ExecQuery("EXEC IGC_GremoryCase_GetItemList '%s', '%s'", aRecv->szAccountID,
                                        aRecv->szName) == TRUE) {
        lpMsg->btResult = 0;
    } else {
        lpMsg->btCount = 0;
    ...
    if (lpMsg->btResult == 0) {
        ...
        while (this->m_GremoryCaseDB.Fetch() == SQL_SUCCESS) {
            lpItem = (_stGremoryCaseItem *) (BUFFER + sizeof(_stAnsGremoryCaseItemList) +
                                             (lpMsg->btCount * sizeof(_stGremoryCaseItem)));
...
void CDataServerProtocol::GDReqGremoryCaseAddItem(int aIndex, _stReqAddItemToGremoryCase *aRecv) {
    ...
    if (this->m_GremoryCaseDB.ExecQuery(
            "EXEC IGC_GremoryCase_AddItem '%s', '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %I64d, %I64d",
            aRecv->szAccountID, aRecv->szName, aRecv->m_GremoryCaseItem.btStorageType,
            aRecv->m_GremoryCaseItem.btRewardSource,
            aRecv->m_GremoryCaseItem.wItemID, aRecv->m_GremoryCaseItem.btItemLevel,
            aRecv->m_GremoryCaseItem.btItemDurability,
            aRecv->m_GremoryCaseItem.btItemSkill, aRecv->m_GremoryCaseItem.btItemLuck,
            aRecv->m_GremoryCaseItem.btItemOption,
            aRecv->m_GremoryCaseItem.btItemExcOption, aRecv->m_GremoryCaseItem.btItemSetOption,
            aRecv->m_GremoryCaseItem.btItemSocketCount,
            aRecv->m_GremoryCaseItem.btItemMainAttribute, aRecv->m_GremoryCaseItem.wMuunEvoItemType,
            aRecv->m_GremoryCaseItem.btMuunEvoItemIndex,
...
void CDataServerProtocol::GDReqCheckUseItemGremoryCase(int aIndex, _stReqCheckUseItemGremoryCase *aRecv) {
    ...
    this->m_GremoryCaseDB.ExecQuery("EXEC IGC_GremoryCase_CheckUseItem %d, %d, %d", aRecv->wItemID, aRecv->dwItemGUID,
                                    aRecv->dwAuthCode);
    this->m_GremoryCaseDB.Fetch();
...
void CDataServerProtocol::GDReqGremoryCaseDeleteItem(int aIndex, _stReqDeleteItemFromGremoryCase *aRecv) {
    this->m_GremoryCaseDB.ExecQuery("EXEC IGC_GremoryCase_DeleteItem %d, %d, %d", aRecv->wItemID, aRecv->dwItemGUID,
                                    aRecv->dwAuthCode);
    this->m_GremoryCaseDB.Fetch();
}
...
void CDataServerProtocol::GDReqUBFCheckIsJoinedUser(int aIndex, PMSG_REQ_UBF_ACCOUNT_USERINFO *aRecv) {
    ...
    this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_GetUserInfo '%s', '%s', %d, %d", aRecv->szAccountID,
                                   aRecv->szName, aRecv->iServerCode, aRecv->IsUnityBattleFieldServer);
    this->m_BattleCoreDB.Fetch();
...
void CDataServerProtocol::GDReqUBFCancelUser(int aIndex, PMSG_UBF_REQ_CANCEL_REGISTER_USER *aRecv) {
    ...
    this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_CancelUser '%s', '%s', %d", aRecv->szAccountID, aRecv->szName,
                                   aRecv->ServerCode);
    this->m_BattleCoreDB.Fetch();
    ...
    if (pMsg.btCanceledResult == 1) {
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_DeleteCharacter '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->ServerCode);
        this->m_BattleCoreDB.Fetch();
    ...
    if (pMsg.btCanceledResult == TRUE && pMsg.btDeletedResult == TRUE) {
        this->m_BattleCoreDB.ExecQuery("SELECT * FROM [BattleCore].[dbo].AccountCharacter WHERE Id='%s'",
                                       aRecv->szAccountID);
        this->m_BattleCoreDB.Fetch();
        ...
        if (iIndex != 5) {
            this->m_BattleCoreDB.ExecQuery("UPDATE [BattleCore].[dbo].AccountCharacter SET GameID%d=NULL WHERE Id='%s'",
                                           iIndex + 1, aRecv->szAccountID);
            this->m_BattleCoreDB.Close();
        }
    }
...
void CDataServerProtocol::GDReqUBFCopyPetItem(int aIndex, LPBYTE lpRecv) {
    ...
    for (int i = 0; i < lpMsg->btCount; i++) {
        lpItem = (PMSG_UBF_COPY_PETITEM *) (lpRecv + sizeof(PMSG_UBF_ACCOUNT_USER_COPY_PETITEM) +
                                            (i * sizeof(PMSG_UBF_COPY_PETITEM)));

        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_CopyPetItemInfo %I64d, %d", lpItem->i64ItemSerial,
                                       lpMsg->ServerCode);
        this->m_BattleCoreDB.Fetch();
        this->m_BattleCoreDB.Close();
    }
...
void CDataServerProtocol::GDReqUBFSetCCFReward(int aIndex, SDHP_REQ_SET_CCF_WINNER_INFO *aRecv) {
    this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_SetCCFReward '%s', %d, %d", aRecv->UBFName, aRecv->btCCFType,
                                   aRecv->btRewardType);
    this->m_BattleCoreDB.Fetch();
    this->m_BattleCoreDB.Close();
}
...
void CDataServerProtocol::GDReqUBFSetDSFReward(int aIndex, PMSG_REQ_SET_DSF_WINNER_INFO *aRecv) {
    this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_SetDSFReward '%s', %d, %d", aRecv->UBFName, aRecv->btDSFType,
                                   aRecv->btRewardType);
    this->m_BattleCoreDB.Fetch();
    this->m_BattleCoreDB.Close();
}
...
void CDataServerProtocol::GDReqUBFGetReward(int aIndex, PMSG_REQ_UBF_GET_REWARD *aRecv) {
    ...
    if (aRecv->btBattleKind == 1) // CCF
    {
        ...
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_GetCCFReward '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->iServerCode);
    ...
    } else if (aRecv->btBattleKind == 2) // Tormented Square
    {
        ...
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_GetDSFReward '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->iServerCode);
    ...
    } else if (aRecv->btBattleKind == 3) // CC Battle
    {
        ...
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_GetCCReward '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->iServerCode);
...
void CDataServerProtocol::GDReqUBFSetGainReward(int aIndex, PMSG_REQ_UBF_SET_RECEIVED_REWARD *aRecv) {
    ...
    if (aRecv->btBattleKind == 1) // CCF
    {
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_SetCCFGainReward '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->iServerCode);
        this->m_BattleCoreDB.Fetch();
    ...
    {
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_SetDSFGainReward '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->iServerCode);
        this->m_BattleCoreDB.Fetch();
    ...
    {
        this->m_BattleCoreDB.ExecQuery("EXEC IGC_BattleCore_SetCCGainReward '%s', '%s', %d", aRecv->szAccountID,
                                       aRecv->szName, aRecv->iServerCode);
        this->m_BattleCoreDB.Fetch();
...
void CDataServerProtocol::GDReqDSFCanPartyEnter(int aIndex, PMSG_REQ_DSF_CAN_PARTY_ENTER *aRecv) {
    ...
    this->m_DSFinalDB.ExecQuery("EXEC IGC_DevilSquareFinal_CanPartyEnter '%s', '%s', '%s', '%s', %d",
                                aRecv->szAccountID1, aRecv->szUserName1, aRecv->szAccountID2, aRecv->szUserName2,
                                aRecv->btDSFType);
...
void CDataServerProtocol::GDReqDSFInsertRewardUser(int aIndex, PMSG_REQ_SAVE_DSF_REWARD_USER *aRecv) {
    this->m_DSFinalDB.ExecQuery("EXEC IGC_DevilSquareFinal_SetReward '%s', '%s', %d, %d, %d, %d, %d, %d",
                                ...
                                aRecv->nRewardYear, aRecv->btRewardMonth, aRecv->btRewardStartDay,
                                aRecv->btRewardEndDay);
...
void CDataServerProtocol::GDReqDSFGetReward(int aIndex, PMSG_REQ_GET_DSF_REWARD *aRecv) {
    ...
    this->m_DSFinalDB.ExecQuery("EXEC IGC_DevilSquareFinal_GetReward '%s', '%s', %d, %d, %d",
                                aRecv->szAccountID, aRecv->szUserName, aRecv->nRewardYear, aRecv->btRewardMonth,
                                aRecv->btRewardDay);
...
void CDataServerProtocol::DGChaosMachineLogLostItems(int aIndex, DSMSG_CHAOSMACHINE_LOSTITEMS *aRecv) {
    char szTemp[256];
    sprintf(szTemp,
            "INSERT INTO IGC_ChaosMachine_LostItems (AccountID, Name, LostDate, ?) VALUES ('%s', '%s', GETDATE())",
            aRecv->szAccountID, aRecv->szName);
}

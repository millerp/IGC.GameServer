...
void CConServ::ServerListSend(int aIndex) {
    ...
    for (std::vector<ServerList>::iterator It2 = g_CServerList.begin(); It2 != g_CServerList.end(); It2++) {
        ...
        pServer = (PMSG_SERVERLIST_SERVER *) (cBUFF + sizeof(PMSG_SERVERSLIST_COUNT) +
                                              (Count * sizeof(PMSG_SERVERLIST_SERVER)));
...
void CConServ::GetServerInfo(int aIndex, USHORT id) {
    ...
    for (std::vector<ServerList>::iterator It = g_CServerList.begin(); It != g_CServerList.end(); It++) {
        if (It->btServerCode == id) {
            ...
            g_Log.Add(0, szModule, __FUNCTION__, "INDEX : %d; SERVER SELECTED (%d) (%s:%d)", aIndex, id, pMsg.IP,
                      pMsg.Port);
...
void CConServ::LoadServerList(LPSTR
szFile)
{
if ((
...
g_Log.Add(0, szModule, __FUNCTION__, "Loaded %d servers", g_CServerList.
...
int newscount = 0;
void CConServ::LoadNewsFile(LPSTR
szFile)
{
for(
int i = 0;
...
textLen = 0;
memset(gObjNews[i]
.title,0,sizeof(gObjNews[i].title));
gObjNews[i].
titleColor = 0;
gObjNews[i].
year = 0;
...
Token = GetToken();
memcpy(szTitle, TokenString,
sizeof(szTitle));

Token = GetToken();
memcpy(szText, TokenString,
sizeof(szText));
...
// store data
gObjNews[iIndex].
dateColor = RGB(dateR, dateG, dateB);
gObjNews[iIndex].
titleColor = RGB(TitleR, TitleG, TitleB);
gObjNews[iIndex].
textColor = RGB(TextR, TextG, TextB);

strcpy_s(gObjNews[iIndex]
.Text,szText);
strcpy_s(gObjNews[iIndex]
.title,szTitle);
gObjNews[iIndex].

...
void TLog::LogDataSet_Start() {
    ...
    wsprintf(szTemp, "Log\\%02d.%02d.%04d_%s.log", this->m_StartDate.Day, this->m_StartDate.Month,
             this->m_StartDate.Year, "DataServer");
    ...
    if (IsFile(szTemp) == TRUE) {
        for (int i = 2;; i++) {
            ...
            wsprintf(szTemp, "Log\\%02d.%02d.%04d_%s_start-%d.log", this->m_StartDate.Day, this->m_StartDate.Month,
                     this->m_StartDate.Year, "DataServer", i);
...
void TLog::LogDataSet_Check() {
    ...
    if ((iDay == this->m_CurDate.Day) && (iMonth == this->m_CurDate.Month) && (iYear == this->m_CurDate.Year)) {
        ...
        CreateDirectory("Log", NULL);
        wsprintf(szTemp, "Log\\%02d.%02d.%04d_-_%02d.%02d.%04d_%s.log", this->m_StartDate.Day, this->m_StartDate.Month,
                 this->m_StartDate.Year, this->m_CurDate.Day, this->m_CurDate.Month, this->m_CurDate.Year,
                 "DataServer");

        char szTemp2[255];
        sprintf(szTemp2, "Log\\%02d.%02d.%04d_%s.log", this->m_CurDate.Day, this->m_CurDate.Month, this->m_CurDate.Year,
                "DataServer");
...
FillRect(hDC, &m_Rect, m_StatusColors
.StandByBG);
...
SetTextColor(hDC, m_LogStr[n]
.Color);

iCorY        -= 15;

TextOut(hDC, iCorX, iCorY, m_LogStr[n]
.Text, (int)iStrLen);

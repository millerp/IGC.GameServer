...
void CExDataServerProtocol::ProtocolCore(int aIndex, BYTE HeadCode, LPBYTE aRecv, int iSize) {
    ...
    switch (HeadCode) {
        ...
        case 0xEB: {
            ...
            switch (lpDef->subcode) {
                case 0x01:
                    m_EXDSProtocol.DGRelationShipAnsKickOutUnionMember(aIndex,
                                                                       (EXSDHP_KICKOUT_UNIONMEMBER_REQ *) aRecv);
                    break;
            }
        }
            break;
        ...
        case 0xA3: {
            ...
            switch (lpMsg->subcode) {
                case 0x00:
                    ...
                    break;
                case 0x01:
                    m_EXDSProtocol.GDReqGuildMatchingListSearchWord(aIndex,
                                                                    (_stReqGuildMatchingListSearchWord *) aRecv);
                    break;
                case 0x02:
                ...
                case 0x07:
                    ...
                    break;
                case 0x08:
                    m_EXDSProtocol.GDReqGetWaitStateListGuildMatching(aIndex,
                                                                      (_stReqWaitStateListGuildMatching *) aRecv);
                    break;
            }
        }
            break;
        ...
        case 0xF8: {
            ...
            switch (lpMsg->subcode) {
                ...
                case 0x03:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqRegGensMember(aIndex,
                                                                      (_tagPMSG_REQ_REG_GENS_MEMBER_EXDB *) aRecv);
                    break;

                case 0x05:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqSecedeGensMember(aIndex,
                                                                         (_tagPMSG_REQ_SECEDE_GENS_MEMBER_EXDB *) aRecv);
                    break;

                case 0x07:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqSaveContributePoint(aIndex,
                                                                            (_tagPMSG_REQ_SAVE_CONTRIBUTE_POINT_EXDB *) aRecv);
                    break;

                case 0x08:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqSaveAbusingKillUserName(aIndex,
                                                                                (_tagPMSG_REQ_SAVE_ABUSING_KILLUSER_EXDB *) aRecv);
                    break;
                ...
                case 0x0C:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqGensRewardCheck(aIndex,
                                                                        (_tagPMSG_REQ_GENS_REWARD_CHECK_EXDB *) aRecv);
                    break;

                case 0x0E:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqGensRewardComplete(aIndex,
                                                                           (_tagPMSG_REQ_GENS_REWARD_COMPLETE_EXDB *) aRecv);
                    break;

                case 0x0F:
                    m_EXDSProtocol.m_GensSystemEDS.GDReqGensMemberCount(aIndex,
                                                                        (_tagPMSG_REQ_GENS_MEMBER_COUNT *) aRecv);
                    break;
            ...
            if (Result.GuildNumber != -1) {
                ...
                if (UpdateGuildMemberStatus(aRecv->GuildName, aRecv->Master, 128)) {
                    if (UpdateGuildType(aRecv->GuildName, aRecv->btGuildType)) {
                        ...
                        for (int i = 0; i < g_dwMaxServerGroups; i++) {
                            if (g_Server[i].m_Index != -1
                                ...
                                if (i == aIndex)
                                    ...
                                    Result.Flag = 0;
                                g_Log.Add("[Guild Create Request] Send Result [%d] to Server [%d]", Result.Result,
                                          g_Server[i].m_ServerCode);
                        ...
                        g_Log.Add("[Guild Create Request] Create Guild: [%s], Master: [%s]", aRecv->GuildName,
                                  aRecv->Master);
                        return;
                    } else {
                        g_Log.Add("[Guild Create Request] [ERROR] UpdateGuildType: Guild: [%s], Type: [%d]",
                                  aRecv->GuildName, aRecv->btGuildType);
                    }
                } else {
                    g_Log.Add("[Guild Create Request] [ERROR] UpdateGuildMemberStatus: Guild: [%s], Member: [%s]",
                              aRecv->GuildName, aRecv->Master);
                }
            }
        } else {
            g_Log.Add("[Guild Create Request] Send Result [%d].", Result.Result);
...
BOOL
CExDataServerProtocol::AddGuild(char *szGuild, char *szMaster, int iNumber, char *lpNotice, int iScore, LPBYTE pMark,
                                int iType) {
    ...
    g_Log.Add("[AddGuild] Guild [%s][%d] Create Guild Master: [%s]", Guild.m_szGuildName, iNumber,
              Guild.m_szGuildMaster);
...
BOOL CExDataServerProtocol::UpdateGuildMemberStatus(char *szGuild, char *szName, BYTE btStatus) {
    ...
    if (lpMemb == NULL) {
        if (GuildExists(szGuild)) {
            g_Log.AddC(TColor::Firebrick, "[ERROR] Guild Member Status: member [%s] doesn't exist in guild [%s]",
                       szName, szGuild);
            return FALSE;
        } else {
    ...
    if (DelResult.Result == 3) {
        for (int i = 0; i < g_dwMaxServerGroups; i++) {
            if (g_Server[i].m_Index != -1
                ...
                g_Log.Add("[Guild Destroy Request] Send Result [%d] to Server [%d].", DelResult.Result,
                          g_Server[i].m_ServerCode);
    ...
    if (MasterDBExists(DelResult.Master)) {
        if (DelAllDBGuildMember(DelResult.GuildName)) {
            if (DelDBGuild(DelResult.GuildName)) {
                ...
                g_Log.Add("[Guild Destroy Request] [DelAllDBGuildMember] Failed to delete All GuildMember in DB : %s",
                          DelResult.GuildName);
            }
        } else {
...
void CExDataServerProtocol::GDReqGuildPeriodBuffInsert(int aIndex, PMSG_REQ_GUILD_PERIODBUFF_INSERT *aRecv) {
    ...
    for (std::map<string, GUILD_MEMBER, strCmp>::iterator It = lpGuild->m_mapGuildMember.begin();
         It != lpGuild->m_mapGuildMember.end(); It++) {
        this->m_GuildDB.ExecQuery("EXEC IGC_PeriodBuffInsert '%s', %d, %d, %d, %d, %I64d", It->first.c_str(),
    ...
    if (Result.btResult) {
        ...
        for (int i = 0; i < g_dwMaxServerGroups; i++) {
            if (g_Server[i].m_Index != -1
                ...
                && g_Server[i].m_State == SS_GUILD) {
                if (i != aIndex) {
                    g_Log.Add("[GuildMember Assign Status Request] Send Result [%d] to Server [%d].", Result.btResult,
                              g_Server[i].m_ServerCode);
                    DataSend(i, (LPBYTE) & Result, Result.h.size, __FUNCTION__);
                }
            }
    ...
    if (Result.btResult) {
        ...
        for (int i = 0; i < g_dwMaxServerGroups; i++) {
            if (g_Server[i].m_Index != -1
                ...
                && g_Server[i].m_State == SS_GUILD) {
                if (i != aIndex) {
                    g_Log.Add("[Guild Assign Type Request] Send Result [%d] to Server [%d].", Result.btResult,
                              g_Server[i].m_ServerCode);
                    DataSend(i, (LPBYTE) & Result, Result.h.size, __FUNCTION__);
                }
            }
...
void CExDataServerProtocol::GDGuildMemberAdd(int aIndex, SDHP_GUILDMEMBERADD *aRecv) {
    ...
    if (AddDBGuildMember(Result.GuildName, Result.MemberID) == FALSE) {
        Result.Result = 4;
        g_Log.AddC(TColor::Firebrick, "[Guild Join Request] [AddDBGuildMember] failed to add [%s] in guild [%s]",
                   Result.MemberID, Result.GuildName);
...
void CExDataServerProtocol::GDGuildMemberDel(int aIndex, SDHP_GUILDMEMBERDEL *aRecv) {
    ...
    if (DelDBGuildMember(Result.MemberID) == FALSE) {
        g_Log.Add("[Guild Member Withdraw Request] [DelDBGuildMember] Failed delete member [%s] from guild [%s]",
                  Result.MemberID, Result.GuildName);
    ...
    if (DelGuildMember(Result.GuildName, Result.MemberID) == FALSE) {
        g_Log.Add("[Guild Member Withdraw Request] [DelGuildMember] Guild[%s] Member[%s] delete fail.",
                  Result.GuildName, Result.MemberID);
    } else
        Result.Result = 1;
    ...
    for (int i = 0; i < g_dwMaxServerGroups; i++) {
        if (g_Server[i].m_Index != -1
            ...
            g_Log.Add("[Guild Member Withdraw Request] Send Result [%d] to Server[%d]", Result.Result,
                      g_Server[i].m_ServerCode);
...
void CExDataServerProtocol::DGRelationShipAnsJoin(int aIndex, EXSDHP_RELATIONSHIP_JOIN_REQ *aRecv) {
    ...
    if (lpRG && lpTG) {
        ...
        Result.btResult = RelationShipOperation(aRecv->iRequestGuildNum, aRecv->iTargetGuildNum,
                                                aRecv->btRelationShipType, 0);
    } else {
        if (lpRG == NULL)
...
int
CExDataServerProtocol::RelationShipBreak(LPGUILD_INFO_STRUCT lpMasterGuild, LPGUILD_INFO_STRUCT lpKickGuild, int type) {
    switch (type) {
        case 1:
...
void CExDataServerProtocol::DGRelationShipAnsKickOutUnionMember(int aIndex, EXSDHP_KICKOUT_UNIONMEMBER_REQ *aRecv) {
    ...
    if (lpMasterGuild && lpKickGuild) {
        Result.btResult = RelationShipOperation(lpKickGuild->m_iNumber, lpMasterGuild->m_iNumber,
                                                aRecv->btRelationShipType, 1);
    } else {
        if (lpMasterGuild == NULL)
            g_Log.Add("[Kick Union Member Request] error: UnionMasterGuild [%s] not found.",
                      Result.szUnionMasterGuildName);

        if (lpKickGuild)
            g_Log.Add("[Kick Union Member Request] error: UnionMemberGuild [%s] not found.",
                      Result.szUnionMemberGuildName);
    }
    ...
    if (Result.btResult != 0 && Result.btResult != 0x10) {
        ...
        for (int i = 0; i < g_dwMaxServerGroups; i++) {
            if (g_Server[i].m_Index != -1
                ...
                && g_Server[i].m_State == SS_GUILD) {
                if (i != aIndex) {
                    g_Log.Add("[Kick Union Member Request] Send Result [%d] to Server [%d]", Result.btResult,
                              g_Server[i].m_ServerCode);
                    DataSend(i, (LPBYTE) & Result, Result.h.size, __FUNCTION__);
                }
            }
...
void CExDataServerProtocol::FCHRoomCreateReq(int aIndex, char *szName, char *szFriendName, short Number, short ServerId,
                                             short FriendNumber, short FriendServerId) {
...
void CExDataServerProtocol::FCHChatRoomInvitationReq(int aIndex, short RoomNumber, char *szName, short Number,
                                                     short ServerId, BYTE Type) {
...
void CExDataServerProtocol::FriendChatRoomInvitationReq(int aIndex, FHP_FRIEND_INVITATION_REQ *lpMsg) {
    ...
    if (this->m_FriendSystemEDS.FriendExists(szMaster, szFriend)) {
        int FriendServerId = this->m_FriendSystemEDS.ExDBGetIndexByCode(
                this->m_FriendSystemEDS.GetFriendState(szFriend));
        if (FriendServerId > -1) {
...
BOOL GensSystem_EDS::InitGensSystem() {
    ...
    GetPrivateProfileString("GensSystem", "GensRankingPath", "..\\IGCData\\IGC_GensSystem.xml", g_GensRankingPath,
                            sizeof(g_GensRankingPath), ".\\IGCDS.ini");
...
BOOL GensSystem_EDS::LoadGensData(LPSTR
lpszFileName)
{
...
{
g_Log.MsgBox("[ GensRanking ] - Can't Load %s (%s)", lpszFileName, res.
...
this->
m_GensRewardStartDay = reward.attribute("StartDay").as_int();
this->
m_GensRewardEndDay = reward.attribute("EndDay").as_int();
this->
m_MinRankToGainReward = reward.attribute("MinRankToGainReward").as_int(300);

pugi::xml_node ranking = main.child("RankingSettings");

for (
pugi::xml_node gens = ranking.child("Gens");
gens;
gens = gens.next_sibling()
)
{
...
this->m_GensRankingData[iClass - 1].
GensClass = iClass;
this->m_GensRankingData[iClass - 1].
ContributePoint = gens.attribute("ReqContributePoint").as_int();
this->m_GensRankingData[iClass - 1].
RankingPosition = gens.attribute("RankingPosition").as_int();
this->m_GensRankingData[iClass - 1].
RewardItemGroup = gens.attribute("RewardItemCat").as_int();
this->m_GensRankingData[iClass - 1].
RewardItemNumber = gens.attribute("RewardItemIndex").as_int();
this->m_GensRankingData[iClass - 1].
RewardItemCount = gens.attribute("RewardItemCount").as_int();
}
...
void GensSystem_EDS::GDReqGensInfo(int aIndex, _tagPMSG_REQ_ABUSING_INFO *aRecv) {
    ...
    if (this->m_MainDB->Fetch() == SQL_NO_DATA) {
        ...
        g_Log.Add("[Gens System] [%s] Get Data from DB [I:%d] [P:%d] [C:%d] [RANK:%d]", aRecv->Name, pMsg.bInfluence,
                  pMsg.iContributePoint, pMsg.iGensClass, pMsg.iGensRanking);
    }
...
void GensSystem_EDS::GDReqRegGensMember(int aIndex, _tagPMSG_REQ_REG_GENS_MEMBER_EXDB *aRecv) {
    ...
    g_Log.Add("[Gens System] Request to Register: Name:%s Influence:%d -> RESULT: %d", aRecv->Name, aRecv->bInfluence,
              pMsg.bResult);
...
void GensSystem_EDS::GDReqSaveContributePoint(int aIndex, _tagPMSG_REQ_SAVE_CONTRIBUTE_POINT_EXDB *aRecv) {
    ...
    this->m_MainDB->ExecQuery("EXEC IGC_GensSave '%s', %d, %d", aRecv->Name, aRecv->iContributePoint,
                              aRecv->iGensClass);
    this->m_MainDB->Fetch();
    ...
    if (iResult == 1) {
        g_Log.Add("[Gens System] Save Result: OK (Name:%s ContributePoint:%d GensClass:%d)", aRecv->Name,
                  aRecv->iContributePoint, aRecv->iGensClass);
    } else {
        g_Log.Add("[Gens System] Save Result: FAIL (Name:%s ContributePoint:%d GensClass:%d)", aRecv->Name,
                  aRecv->iContributePoint, aRecv->iGensClass);
    }
...
void GensSystem_EDS::GDReqSaveAbusingKillUserName(int aIndex, _tagPMSG_REQ_SAVE_ABUSING_KILLUSER_EXDB *aRecv) {
    ...
    while (strlen(&aRecv->KillName[Count * 11]) > 0) {
        this->m_MainDB->ExecQuery("EXEC IGC_GensAbuseSave '%s', '%s', '%d'", aRecv->Name, &aRecv->KillName[Count * 11],
                                  aRecv->KillCount[Count]);
        this->m_MainDB->Close();
        Count++;
    }
...
void GensSystem_EDS::MakeRanking() {
    ...
    while (this->m_RankingDB_1->Fetch() != SQL_NO_DATA) {
        ...
        if (pRank.iGensClass != Class) {
            this->m_RankingDB_2->ExecQuery("UPDATE IGC_Gens SET Class=%d WHERE Name = '%s'", pRank.iGensClass,
                                           pRank.Name);
        }
    ...
    if (this->m_RewardMonth != m_CurrentTime.wMonth) {
        ...
        this->m_RankingDB_2->ExecQuery("UPDATE GameServerInfo SET GensRankingMonth=%d WHERE Number=0",
                                       this->m_RewardMonth);
        ...
        if (m_CurrentTime.wDay >= this->m_GensRewardStartDay && m_CurrentTime.wDay <= this->m_GensRewardEndDay) {
            ...
            for (It = this->m_mapGensRankDuprian.begin(); It != this->m_mapGensRankDuprian.end(); It++) {
                if (It->second.iRank <= this->m_MinRankToGainReward) {
                    if (this->m_GensRankingData[It->second.iGensClass - 1].RewardItemCount > 0) {
                        g_Log.AddC(TColor::Green, "[Gens System] [DUPRIAN] [%s] Reward available [RANK:%d]",
                                   It->first.c_str(), It->second.iRank);
                        this->m_RankingDB_2->ExecQuery("UPDATE IGC_Gens SET Reward = 1 WHERE Name = '%s'",
                                                       It->first.c_str());
                    } else {
                        this->m_RankingDB_2->ExecQuery("UPDATE IGC_Gens SET Reward = 0 WHERE Name = '%s'",
                                                       It->first.c_str());
                    }
                } else {
                    this->m_RankingDB_2->ExecQuery("UPDATE IGC_Gens SET Reward = 0 WHERE Name = '%s'",
            ...
            for (It = this->m_mapGensRankVanert.begin(); It != this->m_mapGensRankVanert.end(); It++) {
                if (It->second.iRank <= this->m_MinRankToGainReward) {
                    if (this->m_GensRankingData[It->second.iGensClass - 1].RewardItemCount > 0) {
                        g_Log.AddC(TColor::Blue, "[Gens System] [VANERT] [%s] Reward available [RANK:%d]",
                                   It->first.c_str(), It->second.iRank);
                        this->m_RankingDB_2->ExecQuery("UPDATE IGC_Gens SET Reward = 1 WHERE Name = '%s'",
                                                       It->first.c_str());
                    } else {
    ...
    for (std::map<string, GENS_USER, strCmp>::iterator It = this->m_mapGensRankDuprian.begin();
         It != this->m_mapGensRankDuprian.end(); It++) {
        g_Log.AddC(TColor::Green, "[Gens System] RANKING (DUPRIAN): [NAME:%s] [CONTRIBUTION:%d] [CLASS:%d] [RANK:%d]",
                   It->second.Name, It->second.iContributePoint, It->second.iGensClass, It->second.iRank);
        this->m_RankingLog.Output("RANKING (DUPRIAN): [NAME:%s] [CONTRIBUTION:%d] [CLASS:%d] [RANK:%d]",
                                  It->second.Name, It->second.iContributePoint, It->second.iGensClass,
                                  It->second.iRank);
    }

    for (std::map<string, GENS_USER, strCmp>::iterator It = this->m_mapGensRankVanert.begin();
         It != this->m_mapGensRankVanert.end(); It++) {
        g_Log.AddC(TColor::Blue, "[Gens System] RANKING (VANERT): [NAME:%s] [CONTRIBUTION:%d] [CLASS:%d] [RANK:%d]",
                   It->second.Name, It->second.iContributePoint, It->second.iGensClass, It->second.iRank);
        this->m_RankingLog.Output("RANKING (VANERT): [NAME:%s] [CONTRIBUTION:%d] [CLASS:%d] [RANK:%d]", It->second.Name,
                                  It->second.iContributePoint, It->second.iGensClass, It->second.iRank);
    }
...
void GensSystem_EDS::ManualRefreshRanking(BYTE Type) {
    g_Log.AddC(TColor::Yellow, "[Gens System] Processing Ranking refresh (Manual, by: %s)",
               Type == 0 ? "System Administrator" : "Game Master");
...
void CExDataServerProtocol::GDReqGuildMatchingList(int aIndex, _stReqGuildMatchingList *lpMsg) {
    ...
    do {
        ...
        this->m_GMatchingDB.GetAsString("GuildMasterName", pMsg.stGuildMatchingList[nCount].szRegistrant,
                                        MAX_ACCOUNT_LEN);
        ...
        this->m_GMatchingDB.GetAsString("Memo", pMsg.stGuildMatchingList[nCount].szMemo,
                                        sizeof(pMsg.stGuildMatchingList[nCount].szMemo));
...
void CExDataServerProtocol::GDReqGuildMatchingListSearchWord(int aIndex, _stReqGuildMatchingListSearchWord *lpMsg) {
    ...
    this->m_GMatchingDB.ExecQuery("EXEC IGC_GetGuildMatchingListKeyword %d, %d, '%s'", lpMsg->nPage, 9,
                                  lpMsg->szSearchWord);
    int nCount = 0;
...
void CExDataServerProtocol::GDReqRegGuildMatchingList(int aIndex, _stReqGuildMatchingData *lpMsg) {
    ...
    this->m_GMatchingDB.ExecQuery("EXEC IGC_InsertGuildMatchingList '%s','%s',%d,%d,%d,%d,'%s',%d,%d,%d,%d",
                                  lpMsg->_stGuildMatchingList.szGuildName, lpMsg->_stGuildMatchingList.szRegistrant,
                                  lpMsg->_stGuildMatchingList.nGuildNumber,
                                  lpMsg->_stGuildMatchingList.nGuildMasterLevel,
                                  lpMsg->_stGuildMatchingList.btGuildMasterClass,
                                  lpMsg->_stGuildMatchingList.btGuildMemberCnt, lpMsg->_stGuildMatchingList.szMemo,
                                  lpMsg->_stGuildMatchingList.btInterestType,
                                  lpMsg->_stGuildMatchingList.btLevelRange, lpMsg->_stGuildMatchingList.btClassType,
                                  lpMsg->_stGuildMatchingList.btGensType);
...
void CExDataServerProtocol::GDReqDelMatchingList(int aIndex, _stReqDelGuildMatchingList *lpMsg) {
    ...
    while (this->m_GMatchingDB.Fetch() == SQL_SUCCESS) {
        ...
        this->m_GMatchingDB.GetAsString("ApplicantName", stAllowList[Count].szApplicantName,
                                        sizeof(stAllowList[Count].szApplicantName));
...
void CExDataServerProtocol::GDReqJoinGuildMatchingList(int aIndex, _stRegWaitGuildMatching *lpMsg) {
    ...
    this->m_GMatchingDB.ExecQuery("EXEC IGC_GetGuildMatchingWaitState '%s', %d", lpMsg->stAllowList.szApplicantName,
                                  lpMsg->stAllowList.btState);
    ...
    if (State >= 0) {
        ...
        if (this->m_GMatchingDB.ExecQuery("EXEC IGC_InsertGuildMatchingWaitList %d,'%s',%d,%d,%d",
                                          lpMsg->stAllowList.nGuildNumber, lpMsg->stAllowList.szApplicantName,
                                          lpMsg->stAllowList.btApplicantClass,
...
void CExDataServerProtocol::GDReqAllowJoinGuildMatching(int aIndex, _stReqAllowJoinGuildMatching *lpMsg) {
    ...
    if (bIsAlreadyInGuild == FALSE) {
        ...
        if (State != 0) // Fail - wrong state
        {
            ...
            this->m_GMatchingDB.ExecQuery("EXEC IGC_SetGuildMatchingWaitState '%s', %d", lpMsg->szName,
                                          SetMatchingState);
            this->m_GMatchingDB.Fetch();
...
void CExDataServerProtocol::GDReqWaitGuildMatchingList(int aIndex, _stReqWaitGuildMatchingList *lpMsg) {
    ...
    while (this->m_GMatchingDB.Fetch() == SQL_SUCCESS) {
        ...
        this->m_GMatchingDB.GetAsString("ApplicantName", pMsg.stAllowList[Count].szApplicantName,
                                        sizeof(pMsg.stAllowList[Count].szApplicantName));
...
void CExDataServerProtocol::GuildMatchingSendAcceptAndRejectInfo(int aIndex, char *szUserName, int nUserIndex,
                                                                 int nServerIndex) {
    _stAnsNotiGuildMatching pNotiMsg;
...
void CExDataServerProtocol::GDReqRegWantedPartyMember(int aIndex, _stReqRegWantedPartyMember *lpMsg) {
    ...
    this->m_PMatchingDB.ExecQuery(
            "EXEC IGC_InsertPartyMatchingList '%s', '%s','%s', %d, %d, %d, %d,%d, %d,%d , %d,%d,%d,%d,%d,%d,%d, %d,%d,%d,%d",
            lpMsg->szPartyLeaderName, lpMsg->szTitle, lpMsg->szPassword, lpMsg->nMinLevel, lpMsg->nMaxLevel,
            lpMsg->nHuntingGround,
            lpMsg->btWantedClass, lpMsg->btCurrentPartyCnt, lpMsg->btUsePassWord, lpMsg->btApprovalType,
            lpMsg->btWantedClassDetailInfo[0], lpMsg->btWantedClassDetailInfo[1], lpMsg->btWantedClassDetailInfo[2],
            lpMsg->btWantedClassDetailInfo[3],
            lpMsg->btWantedClassDetailInfo[4], lpMsg->btWantedClassDetailInfo[5], lpMsg->btWantedClassDetailInfo[6],
            lpLeaderData->m_iServer + 1,
            lpMsg->btGensType, lpMsg->nLeaderLevel, lpMsg->btLeaderClass);

    if (this->m_PMatchingDB.Fetch() != SQL_SUCCESS) {
        pMsg.nResult = -1;
    } else {
        this->m_PartyMatchingEDS.CreateParty(lpLeaderData->m_iServer, aIndex, lpMsg->nUserIndex,
                                             lpMsg->szPartyLeaderName);
        pMsg.nResult = 0;
    }
...
void CExDataServerProtocol::GDReqGetPartyMatchingList(int aIndex, _stReqGetPartyMatchingList *lpMsg) {
    ...
    if (lpMsg->nType == 0) {
        ...
        this->m_PMatchingDB.ExecQuery("EXEC IGC_GetPartyMatchingListKeyword %d, %d, '%s'", lpMsg->nPage, 6,
                                      lpMsg->szSearchWord);
    } else if (lpMsg->nType == 2) {
        this->m_PMatchingDB.ExecQuery("EXEC IGC_GetPartyMatchingListJoinAble %d, %d,%d,%d, %d", lpMsg->nLevel,
                                      lpMsg->btClass, lpMsg->btGens, lpMsg->nPage, 6);
    }
    ...
    do {
        ...
        this->m_PMatchingDB.GetAsString("PartyLeaderName", pMsg.stPartyInfoList[nCount].szLeaderName,
                                        sizeof(pMsg.stPartyInfoList[nCount].szLeaderName));
        this->m_PMatchingDB.GetAsString("Title", pMsg.stPartyInfoList[nCount].szTitle,
                                        sizeof(pMsg.stPartyInfoList[nCount].szTitle));
        ...
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[0] = this->m_PMatchingDB.GetAsInteger(
                "WantedClassDetailInfo1");
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[1] = this->m_PMatchingDB.GetAsInteger(
                "WantedClassDetailInfo2");
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[2] = this->m_PMatchingDB.GetAsInteger(
                "WantedClassDetailInfo3");
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[3] = this->m_PMatchingDB.GetAsInteger(
                "WantedClassDetailInfo4");
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[4] = this->m_PMatchingDB.GetAsInteger(
                "WantedClassDetailInfo5");
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[5] = this->m_PMatchingDB.GetAsInteger(
                "WantedClassDetailInfo6");
        pMsg.stPartyInfoList[nCount].btWantedClassDetailInfo[6] = this->m_PMatchingDB.GetAsInteger(
    ...
    if (lpMsg->nType == 2) {
        this->m_PMatchingDB.ExecQuery("EXEC IGC_GetPartyMatchingListJoinAbleTotalCount %d, %d, %d", lpMsg->nLevel,
                                      lpMsg->btClass, lpMsg->btGens);
    } else {
...
void CExDataServerProtocol::GDReqJoinMemberPartyMatching(int aIndex, _stReqJoinMemberPartyMatching *lpMsg) {
    ...
    if (bExistOnWaitList == TRUE) {
        ...
        if (lpMsg->btRandomParty == TRUE) {
            ...
            this->m_PMatchingDB.ExecQuery("EXEC IGC_GetPartyMatchingRandom %d, %d, %d", lpMsg->nLevel, lpMsg->btClass,
                                          lpMsg->btGensType);
            ...
            if (this->m_PMatchingDB.Fetch() == SQL_NO_DATA) {
                ...
                if (nCount >= 10) {
                    ...
                    this->m_PMatchingDB.ExecQuery("EXEC IGC_InsertWaitPartyMatching '%s', '%s', %d, %d, %d",
                                                  szLeaderName, lpMsg->szMemberName, lpMsg->btChangeUpClass,
                                                  lpMsg->nLevel, lpMsg->nUserDBNumber);
                    this->m_PMatchingDB.Fetch();
                    this->m_PMatchingDB.Close();
                    pMsg.nResult = 0;
                }
            ...
            if (!strcmp(lpMsg->szLeaderName, lpMsg->szMemberName)) {
                ...
                if (this->m_PMatchingDB.Fetch() == SQL_NO_DATA || lpPartyMaster == NULL) {
                    ...
                    this->m_PMatchingDB.ExecQuery("EXEC IGC_GetPartyMatchingWaitListForLeader '%s'",
                                                  lpMsg->szLeaderName);
                    ...
                    if (nCount >= 10) {
                        ...
                        if (UsePassword == TRUE) {
                            if (strcmp(szPassword, lpMsg->szPassword)) {
                                ...
                                if (AcceptType == 1) {
                                    this->AutoAddPartyMember(lpMsg->szLeaderName, lpMsg->szMemberName,
                                                             lpMsg->nUserIndex, aIndex);
                                    return;
                                }

                                this->m_PMatchingDB.ExecQuery("EXEC IGC_InsertWaitPartyMatching '%s', '%s', %d, %d, %d",
                                                              lpMsg->szLeaderName, lpMsg->szMemberName,
                                                              lpMsg->btChangeUpClass, lpMsg->nLevel,
                                                              lpMsg->nUserDBNumber);
                                this->m_PMatchingDB.Fetch();
                                this->m_PMatchingDB.Close();
                                pMsg.nResult = 0;
                            ...
                            if (AcceptType == 1) {
                                this->AutoAddPartyMember(lpMsg->szLeaderName, lpMsg->szMemberName, lpMsg->nUserIndex,
                                                         aIndex);
                                return;
                            }
                            ...
                            this->m_PMatchingDB.ExecQuery("EXEC IGC_InsertWaitPartyMatching '%s', '%s', %d, %d, %d",
                                                          lpMsg->szLeaderName, lpMsg->szMemberName,
                                                          lpMsg->btChangeUpClass, lpMsg->nLevel, lpMsg->nUserDBNumber);
                            this->m_PMatchingDB.Fetch();
                            this->m_PMatchingDB.Close();
                            pMsg.nResult = 0;
                        }
...
void CExDataServerProtocol::GDReqAcceptMemberJoin(int aIndex, _stReqAddPartyMember *lpMsg) {
    ...
    if (lpMsg->btAlreadyParty == TRUE) {
        int iResult = this->m_PartyMatchingEDS.AddPartyMember(lpMsg->szLeaderName, lpMsg->szMemberName, nMemberIndex,
                                                              nMemberServer, nMemberServerIndex);

        if (iResult == 0) {
            PARTYMATCHING_PARTYDATA *lpPartyData = this->m_PartyMatchingEDS.GetPartyDataByMaster(lpMsg->szLeaderName);

            if (lpPartyData) {
                this->m_PMatchingDB.ExecQuery("EXEC IGC_UpdatePartyMatchingMemberCount '%s', %d", lpMsg->szLeaderName,
        ...
        if (lpMsg->btType == 1) {
            pMsg.nResult = this->m_PartyMatchingEDS.AddPartyMember(lpMsg->szLeaderName, lpMsg->szMemberName,
                                                                   nMemberIndex, nMemberServer, nMemberServerIndex);
            pMsg.btSendType = 0;
            pMsg.btType = lpMsg->btType;

            if (pMsg.nResult == 0) {
                PARTYMATCHING_PARTYDATA *lpPartyData = this->m_PartyMatchingEDS.GetPartyDataByMaster(
                        lpMsg->szLeaderName);

                if (lpPartyData) {
                    this->m_PMatchingDB.ExecQuery("EXEC IGC_UpdatePartyMatchingMemberCount '%s', %d",
                                                  lpMsg->szLeaderName, lpPartyData->m_UserCount);
                    this->m_PMatchingDB.Fetch();
                    this->m_PMatchingDB.Close();
                }
            }
...
void CExDataServerProtocol::GDReqCancelPartyMatching(int aIndex, _stReqCancelPartyMatching *lpMsg) {
    ...
    if (lpMsg->btType) {
        ...
        if (lpPartyData) {
            ...
            this->m_PMatchingDB.ExecQuery("EXEC IGC_UpdatePartyMatchingMemberCount '%s', %d", lpPartyData->szLeaderName,
                                          lpPartyData->m_UserCount);
            this->m_PMatchingDB.Fetch();
            this->m_PMatchingDB.Close();
        ...
        if (lpPartyData) {
            if (strcmp(lpPartyData->szLeaderName, lpMsg->szName)) {
                ...
                this->m_PMatchingDB.ExecQuery("EXEC IGC_UpdatePartyMatchingMemberCount '%s', %d",
                                              lpPartyData->szLeaderName, lpPartyData->m_UserCount);
                this->m_PMatchingDB.Fetch();
                this->m_PMatchingDB.Close();
            } else {
                if (lpPartyData->m_UserCount > 1) {
                    for (int i = 1; i < 5; i++) {
                        if (lpPartyData->m_UserData[i].iUsedInfo == TRUE &&
                            ...
                            DataSend(lpPartyData->m_UserData[i].iServerIndex, (LPBYTE) & pMsg, pMsg.h.size,
                                     __FUNCTION__);
                        }
                    }
                }
...
void CExDataServerProtocol::GDReqDeletePartyUser(int aIndex, _stReqDelPartyUserPartyMatching *lpMsg) {
    ...
    if (nUserIndex < 0) {
        ...
        if (strcmp(szLeaderName, lpMsg->szTargetName)) {
            ...
            this->m_PMatchingDB.ExecQuery("EXEC IGC_UpdatePartyMatchingMemberCount '%s', %d", szLeaderName,
                                          lpPartyData->m_UserCount);
            this->m_PMatchingDB.Fetch();
            this->m_PMatchingDB.Close();
...
void
CExDataServerProtocol::AutoAddPartyMember(char *szLeaderName, char *szMemberName, int nUserIndex, int nServerIndex) {
    EnterCriticalSection(&this->m_FriendSystemEDS.m_csMapFriendMaster);
    ...
    pMsg.nResult = this->m_PartyMatchingEDS.AddPartyMember(szLeaderName, szMemberName, nUserIndex, lpUser->m_iServer,
                                                           nServerIndex);
            ...
            if (btType) {
                ...
                for (int j = 0; j < 5; j++) {
                    if (lpPartyData->m_UserData[j].iUsedInfo == TRUE) {
                        ...
                        pMsg.stList[j].bFlag = lpPartyData->m_UserData[j].iServerIndex ==
                                               lpPartyData->m_UserData[i].iServerIndex; // j == i
                    }
                }
            }
...
PARTYMATCHING_PARTYDATA *CPartyMatchingData::GetPartyDataByMaster(char *szLeaderName) {
    for (std::vector<PARTYMATCHING_PARTYDATA>::iterator It = this->m_vtPartyData.begin();
         It != this->m_vtPartyData.end(); It++) {
...
void CPartyMatchingData::CreateParty(int Server, int ServerIndex, int nLeaderIndex, char *szLeaderName) {
    ...
    g_Log.Add("[Party Matching] Create Party (Leader: %s) (Total Party Count:%d)", szLeaderName,
              this->m_vtPartyData.size());
...
void CPartyMatchingData::DeleteParty(char *szLeaderName) {
    ...
    for (std::vector<PARTYMATCHING_PARTYDATA>::iterator it = this->m_vtPartyData.begin();
        ...
        if (strcmp(szLeaderName, it->szLeaderName) == 0) {
            ...
            g_Log.Add("[Party Matching] Delete Party (Leader: %s) (Total Party Count:%d)", szLeaderName,
                      this->m_vtPartyData.size());
            break;
        }
    }
...
int CPartyMatchingData::AddPartyMember(char *szLeaderName, char *szMemberName, int nMemberIndex, int nMemberServer,
        ...
        if (bResult != -4) {
            ...
            for (int i = 0; i < 5; i++) {
                if (lpPartyData->m_UserData[i].iUsedInfo == FALSE) {
                    ...
                    g_Log.Add("[Party Matching] Add Party Member (Leader: %s) (Member: %s) (Total users in party: %d)",
                              szLeaderName, szMemberName, lpPartyData->m_UserCount);
                    bResult = 0;
                    break;
                }
            }
    ...
    for (int i = 0; i < 5; i++) {
        if (lpPartyData->m_UserData[i].iUsedInfo == TRUE) {
            if (!strcmp(szMemberName, lpPartyData->m_UserData[i].szUserName)) {
                ...
                g_Log.Add("[Party Matching] Delete Party Member (Leader: %s) (Member: %s) (Total users in party: %d)",
                          szLeaderName, szMemberName, lpPartyData->m_UserCount);
...
int CPartyMatchingData::DisconnectMember(char *szMemberName) {
    ...
    for (int i = 0; i < 5; i++) {
        if (!strcmp(szMemberName, lpPartyData->m_UserData[i].szUserName)) {
            ...
            g_Log.Add("[Party Matching] Member disconnected (Name:%s) (Leader:%s)", szMemberName,
                      lpPartyData->szLeaderName);
            bMemberFound = TRUE;
        }

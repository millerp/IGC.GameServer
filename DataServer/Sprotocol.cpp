...
BOOL CLoginServerData::MuLoginAddServer(int ServerIndex, LPTSTR ServerName, WORD ServerCode, WORD Port, BYTE ServerVIP,
...
BOOL CLoginUserData::MuLoginAddUser(WORD ServerCode, WORD ServerGroup, char *szAccountID, int UserIndex, char *szIp,
                                    char *szHWID) {
...
void CLoginUserData::MuLoginDeleteUser(WORD ServerCode, WORD ServerGroup) {
    for (int n = 0; n < MAX_LOGIN_USER; n++) {
        if (this->m_MuLoginUserData[n].m_ServerCode == ServerCode &&
            this->m_MuLoginUserData[n].m_ServerGroup == ServerGroup) {
...
int CLoginUserData::SetVIPData(LPTSTR szAccountID, int VIPType, LPTSTR szDate, int Minutes) {
    ...
    if (VIPType == 0) {
        ...
        memset(this->m_MuLoginUserData[UserIndex].szEndDays, 0x00,
               sizeof(this->m_MuLoginUserData[UserIndex].szEndDays));
...
void CLoginServerProtocol::InsertDataMuLog(LPTSTR ServerName, LPTSTR Id, LPTSTR Ip, LPTSTR State, LPTSTR HWID) {
    this->m_LogDB.ExecQuery(
            "INSERT INTO ConnectionHistory (AccountID, ServerName, IP, Date, State, HWID) Values('%s','%s','%s', GETDATE(), '%s', '%s')",
...
void CLoginServerProtocol::JoinServerLogin(int aIndex, SDHP_SERVERINFO *lpMsg) {
    ...
    if (this->m_ServerData.MuLoginAddServer(aIndex, lpMsg->ServerName, lpMsg->ServerCode, lpMsg->Port, lpMsg->ServerVIP,
                                            lpMsg->MaxHWIDUseCount) == TRUE) {
    ...
    if (SpaceSyntexCheck(szAccountID) == TRUE) {
        ...
        if (bErrorFlag == FALSE) {
            if (this->m_AccountDB.ExecQuery("SELECT * FROM MEMB_INFO WHERE memb___id='%s'", szAccountID) == TRUE) {
                ...
                if (bErrorFlag == FALSE && this->m_ServerData.GetVIPLevel(
                        this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)) != 0) {
                    ...
                    } else if (Type < this->m_ServerData.GetVIPLevel(
                            this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode))) {
                        pResult.result = 64;
                        bErrorFlag = TRUE;
        ...
        if (bErrorFlag == FALSE && pResult.result != 0) {
            ...
            if (this->m_UserData.CheckAccountID(szAccountID) == TRUE &&
                this->m_UserData.CheckMoveTimeOut(szAccountID) == FALSE) {
            ...
            if (this->m_UserData.CheckHWIDLimit_Local(g_Server[aIndex].m_ServerCode, aRecv->HWID,
                                                      this->m_ServerData.GetHWIDUseCount(
                                                              this->m_ServerData.MuLoginFindServer(
                                                                      g_Server[aIndex].m_ServerCode))) == FALSE) {
                g_Log.Add("[MeMuOnline] Machine ID Limit Reached (Local) (%d) ServerCode (%d) : Account:%s HWID:%s",
                          this->m_ServerData.GetHWIDUseCount(
                                  this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)),
                          g_Server[aIndex].m_ServerCode, szAccountID, aRecv->HWID);
                JGOtherJoin(aIndex, szAccountID);
                pResult.result = 4;
                bErrorFlag = TRUE;
            }

            if (this->m_UserData.CheckHWIDLimit_Group(g_MapServerManager.GetMapSvrGroup(g_Server[aIndex].m_ServerCode),
                                                      aRecv->HWID) == FALSE) {
                g_Log.Add("[MeMuOnline] Machine ID Limit Reached (Group) (%d) MapSvrGroup (%d) : Account:%s HWID:%s",
                          g_MachineIDConnectionLimitPerGroup,
                          g_MapServerManager.GetMapSvrGroup(g_Server[aIndex].m_ServerCode), szAccountID, aRecv->HWID);
                JGOtherJoin(aIndex, szAccountID);
                pResult.result = 4;
                bErrorFlag = TRUE;
            }
        ...
        if (bErrorFlag == FALSE) {
            BOOL bResult = this->m_UserData.MuLoginAddUser(g_Server[aIndex].m_ServerCode,
                                                           g_MapServerManager.GetMapSvrGroup(
                                                                   g_Server[aIndex].m_ServerCode), szAccountID,
                                                           aRecv->Number, aRecv->IpAddress, aRecv->HWID);
            ...
            if (bResult == FALSE) {
                ...
                InsertDataMuLog(this->m_ServerData.GetServerName(
                        this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)), szAccountID,
                                aRecv->IpAddress, "Connect", aRecv->HWID);
                this->m_AccountDB.ExecQuery("EXEC WZ_CONNECT_MEMB '%s','%s','%s'",
                                            szAccountID, this->m_ServerData.GetServerName(
                                this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)), szIp);
            }
        }
    }
...
void CLoginServerProtocol::GJPAccountFail(int aIndex, SDHP_JOINFAIL *aRecv) {
    ...
    this->InsertDataMuLog(
            this->m_ServerData.GetServerName(this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)),
            szAccountID, this->m_UserData.GetIpAddr(UserIndex), "Login Fail", "N/A");
...
void CLoginServerProtocol::GJPUserClose(int aIndex, SDHP_USERCLOSE_ID *aRecv) {
    ...
    this->InsertDataMuLog(
            this->m_ServerData.GetServerName(this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)),
            szAccountID, this->m_UserData.GetIpAddr(this->m_UserData.MuLoginFindUser(szAccountID)), "Disconnect",
            "N/A");
    this->m_UserData.MuLoginDeleteUser(szAccountID);
    this->m_AccountDB.ExecQuery("EXEC WZ_DISCONNECT_MEMB '%s'", szAccountID);
}
...
void CLoginServerProtocol::GCUserKillSend(int MuLoginIndex, bool IsForceDC) {
    ...
    int aIndex = this->m_ServerData.GetServerIndex(
            this->m_ServerData.MuLoginFindServer(this->m_UserData.GetServerCode(MuLoginIndex)));
...
void CLoginServerProtocol::LoveHeartEventRecv(int aIndex, SDHP_LOVEHEARTEVENT *aRecv) {
    ...
    this->m_AccountDB.ExecQuery("INSERT INTO LoveHeartAll ( Number, Id, Server, Name ) Values (%d, '%s','%s','%s')",
                                dwHeartCount, szAccountID, this->m_ServerData.GetServerName(
                    this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)), szName);
    ...
    if (iIndex >= 0)    // if Is Connected
    {
        ...
        {
            this->m_AccountDB.ExecQuery(
                    "INSERT INTO LoveHeart ( Number, Id, Server, Name ) Values (%d, '%s','%s','%s')",
                    dwHeartCount, szAccountID, this->m_ServerData.GetServerName(
                            this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)), szName);
            pResult.Result = 1;

            g_Log.AddC(TColor::Red, "[MeMuOnlineDB] [LOVE HEART] Event Winner : %s:%s - Server : %s",
                       szAccountID, szName, this->m_ServerData.GetServerName(
                            this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode)));
        }
    }
...
void CLoginServerProtocol::GJReqMapSvrMove(int aIndex, PMSG_REQ_MAPSVRMOVE *aRecv) {
    ...
    if (iOldServerIndex == -1 || iNewServerIndex == -1) {
        fResult = 2;
        g_Log.Add("[JoinServer] GJReqMapSvrMove() -> ServerIndex = -1 ( cur:%d | dest: %d) Account:(%s)",
        ...
        g_Log.Add("[JoinServer] GJReqMapSvrMove -> Server(%d) UserLimit reached (cur:%d | max:%d)",
                  aRecv->wDstMapSvrCode, CurUser, MaxUser);
    } else {
        ...
        if (iUserIndex == -1) {
            fResult = 1;
            g_Log.Add("[JoinServer] GJReqMapSvrMove -> iUserIndex == -1 (%s)(%s)", aRecv->szAccountID,
                      aRecv->szCharName);
        } else if (this->m_UserData.CheckHWIDLimit_Local(aRecv->wDstMapSvrCode, this->m_UserData.GetHWID(iUserIndex),
                                                         this->m_ServerData.GetHWIDUseCount(iNewServerIndex)) ==
                   FALSE) {
            fResult = 3;
            g_Log.Add("[JoinServer] GJReqMapSvrMove -> Server(%d) HWID limit reached (max:%d)", aRecv->wDstMapSvrCode,
                      this->m_ServerData.GetHWIDUseCount(iNewServerIndex));
        } else if (this->m_UserData.CheckMapServerMove(iUserIndex) != false) {
            fResult = 4;
            g_Log.Add("[JoinServer] GJReqMapSvrMove -> m_bRequestMapSvrMove == TRUE (%s)(%s)", aRecv->szAccountID,
                      aRecv->szCharName);
        } else {
...
void CLoginServerProtocol::GJReqMapSvrAuth(int aIndex, PMSG_REQ_MAPSVRAUTH *aRecv) {
    ...
    if (fResult == 0) {
        ...
        if (iUserIndex == -1) {
            ...
            for (std::vector<tagUSER_DATA>::iterator _Iter = this->m_UserData.m_vecMapMove.begin();
                 _Iter != this->m_UserData.m_vecMapMove.end(); _Iter++) {
                    ...
                    if (aRecv->wDstMapSvrCode == pDataUser.pMapServerMoveData.wDstMapSvrCode) {
                        if (aRecv->iJoinAuthCode1 == pDataUser.pMapServerMoveData.iJoinAuthCode1 &&
                            ...
                            if (this->m_AccountDB.Fetch() != SQL_NO_DATA) {
                                ...
                                pResult.btBlockCode = (BYTE)
                                this->m_AccountDB.GetAsInteger("bloc_code");
                            }
                ...
                if (this->m_UserData.m_MuLoginUserData[i].iUserNumber != -1) {
                    if (GetTickCount() - this->m_UserData.m_MuLoginUserData[i].dwCheckTime >= 300000) {
                        switch (this->m_UserData.m_MuLoginUserData[i].iPayCode) {
                            ...
                            case 4: // VIP by date
                            {
                                this->m_VIPDB.ExecQuery("EXEC VIPSystem_CheckAccount '%s'",
                                                        this->m_UserData.m_MuLoginUserData[i].m_AccoundID);
                                this->m_VIPDB.Fetch();
                                ...
                                if (VIP == 0) {
                                    ...
                                    g_Log.Add("[VIP] Account expired - ID : %s",
                                              this->m_UserData.m_MuLoginUserData[i].m_AccoundID);
                                } else if (Type < this->m_ServerData.GetVIPLevel(
                                        this->m_ServerData.MuLoginFindServer(g_Server[aIndex].m_ServerCode))) {
                                    this->GCUserKillSend(i, false);

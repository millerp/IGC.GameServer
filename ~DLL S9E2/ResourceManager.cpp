...
void __fastcall CResourceManager::XorLine(char *buff, int len) {
    BYTE xorTable[] = {0xA1, 0x11, 0x8B, 0x0F, 0x28, 0x17, 0x95, 0xEE, 0x1B, 0xCD, 0x58, 0x31, 0x0E, 0x4D, 0x7E, 0x41,
                       0x21, 0xFD, 0xFE, 0x7C, 0xC1, 0xC2, 0xAF, 0x3C, 0xFA, 0x14};
    for (int i = 0; i < len; i++) {
        buff[i] ^= xorTable[i % 5];
    }
}
...
void
__fastcall CResourceManager::LoadResource(CResourceLoader &loader, int Id, std::string path, std::string file, int num,
                                          std::string folder) {
    try {
        RegisterResource(loader.Type(), path + file, Id);

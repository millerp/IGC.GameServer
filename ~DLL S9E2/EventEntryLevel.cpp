...
void LoadBCEntryLevel() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        g_BloodCastleEnterLevel_Normal[i][0] = GetPrivateProfileInt("BloodCastle", szTemp, 0,
                                                                    "./Data/Local/EventEntry.bmd");

        sprintf(szTemp, "BC%dLevelMax_Normal", i + 1);
        g_BloodCastleEnterLevel_Normal[i][1] = GetPrivateProfileInt("BloodCastle", szTemp, 0,
                                                                    "./Data/Local/EventEntry.bmd");

        sprintf(szTemp, "BC%dLevelMin_MGDLRF", i + 1);
        g_BloodCastleEnterLevel_Magumsa[i][0] = GetPrivateProfileInt("BloodCastle", szTemp, 0,
                                                                     "./Data/Local/EventEntry.bmd");

        sprintf(szTemp, "BC%dLevelMax_MGDLRF", i + 1);
        g_BloodCastleEnterLevel_Magumsa[i][1] = GetPrivateProfileInt("BloodCastle", szTemp, 0,
                                                                     "./Data/Local/EventEntry.bmd");
    }
...
void LoadDSEntryLevel() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        g_DevilSquareEnterLevel_Normal[i][0] = GetPrivateProfileInt("DevilSquare", szTemp, 0,
                                                                    "./Data/Local/EventEntry.bmd");

        sprintf(szTemp, "DS%dLevelMax_Normal", i + 1);
        g_DevilSquareEnterLevel_Normal[i][1] = GetPrivateProfileInt("DevilSquare", szTemp, 0,
                                                                    "./Data/Local/EventEntry.bmd");

        sprintf(szTemp, "DS%dLevelMin_MGDLRF", i + 1);
        g_DevilSquareEnterLevel_Magumsa[i][0] = GetPrivateProfileInt("DevilSquare", szTemp, 0,
                                                                     "./Data/Local/EventEntry.bmd");

        sprintf(szTemp, "DS%dLevelMax_MGDLRF", i + 1);
        g_DevilSquareEnterLevel_Magumsa[i][1] = GetPrivateProfileInt("DevilSquare", szTemp, 0,
                                                                     "./Data/Local/EventEntry.bmd");
    }

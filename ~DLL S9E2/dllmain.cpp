...
void SetValues() {
    ...
    MemSet(MAPSRV_DELACCID_FIX, 0x90,
           5); // nop call to memcpy in mapserver, because buff source is empty, due to no account id from webzen http (remove new login system)
...
DLL_PROCESS_ATTACH: {
    ...
    0x00401000, 12681214, PAGE_EXECUTE_READ, &OldProtect);
    CheckMainFunctions();
    DoHooks();
    SetHook();
    SetValues();

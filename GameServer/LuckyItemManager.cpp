...
int LuckyItemManager::IsLuckyItemEquipment(int iItemNum) {
    ...
    for (std::map<int, _LUCKY_ITEM_INFO>::iterator Iter = this->m_Map_LuckyItemInfo.begin();
         Iter != this->m_Map_LuckyItemInfo.end(); Iter++) {
...
void LuckyItemManager::LuckyItemTicketExchange(LPOBJ lpObj) {
    ...
    if (!CheckInventoryEmptySpace(lpObj, iItemHeight, iItemWidth)) {
        g_Log.Add("[LuckyItem] - Fail - Not Empty Inventory [%s][%s] CharClass[%d]", lpObj->AccountID, lpObj->Name,
                  lpObj->Class);
        pMsg.Result = 0xF1;
    ...
    if (iValidItemCount != 1 || iInvalidItemCount != 0 || iItemPos == -1) {
        g_Log.Add("[LuckyItem] - Can Not be Exchanged [%s][%s] CharClass[%d] ItemNum[%d] ItemName[%s]",
                  lpObj->AccountID, lpObj->Name, lpObj->Class,
    ...
    if (pLuckyItemEquipment == 0 || pItemLevelRandRate == 0 || piAddSetOptRate == 0) {
        g_Log.Add("[LuckyItem] --- Error --- [%s][%s] CharClass[%d] ItemNum[%d] ItemName[%s]", lpObj->AccountID,
    ...
    if (pLuckyItemEquipment->iAddOption) {
        if (dwOptionTemp <= pLuckyItemInfo->iAddOpt3Rate[0]) {
            ...
            Op3 = 1;
                                    ...
                                    pLuckyItemInfo->iAddOpt3Rate[2])) {
            Op3 = 2;
        } else if (dwOptionTemp <= (pLuckyItemInfo->iAddOpt3Rate[0] + pLuckyItemInfo->iAddOpt3Rate[1] +
                                    pLuckyItemInfo->iAddOpt3Rate[2] + pLuckyItemInfo->iAddOpt3Rate[3])) {
            Op3 = 3;
        } else if (dwOptionTemp <= (pLuckyItemInfo->iAddOpt3Rate[0] + pLuckyItemInfo->iAddOpt3Rate[1] +
                                    pLuckyItemInfo->iAddOpt3Rate[2] + pLuckyItemInfo->iAddOpt3Rate[3] +
                                    pLuckyItemInfo->iAddOpt3Rate[4])) {
            Op3 = 4;
        } else if (dwOptionTemp <= (pLuckyItemInfo->iAddOpt3Rate[0] + pLuckyItemInfo->iAddOpt3Rate[1] +
                                    pLuckyItemInfo->iAddOpt3Rate[2] + pLuckyItemInfo->iAddOpt3Rate[3] +
                                    pLuckyItemInfo->iAddOpt3Rate[4] + pLuckyItemInfo->iAddOpt3Rate[5])) {
            Op3 = 5;
        } else if (dwOptionTemp <= (pLuckyItemInfo->iAddOpt3Rate[0] + pLuckyItemInfo->iAddOpt3Rate[1] +
                                    pLuckyItemInfo->iAddOpt3Rate[2] + pLuckyItemInfo->iAddOpt3Rate[3] +
                                    pLuckyItemInfo->iAddOpt3Rate[4] + pLuckyItemInfo->iAddOpt3Rate[5] +
                                    pLuckyItemInfo->iAddOpt3Rate[6])) {
            Op3 = 6;
        } else {
            Op3 = 7;
        }
    ...
    ItemSerialCreateSend(lpObj->m_Index, 0xFF, 0, 0, Type, level, dur, Op1, Op2, Op3, lpObj->m_Index, ExOption,
                         SetOption, 0, 0, 0);
    ...
    g_Log.Add(
            "[LuckyItem] - Exchange End [%s][%s][%d] ItemName[%s] ItemNum[%d] Level[%d] Dur[%d] skill[%d] luck[%d] option[%d] ExOpt[%d] SetOption[%d]",
            lpObj->AccountID, lpObj->Name, lpObj->Class, szSetItemName, Type, level, dur, Op1, Op2, Op3, ExOption,
            SetOption);
...
void LuckyItemManager::LuckyItemSmelting(LPOBJ lpObj) {
    ...
    if (iValidItemCount != 1 || iInvalidItemCount != 0 || iItemPos == -1) {
        ...
        int bGambleLuckyItemSmelting = this->GambleLuckyItemSmelting(lpObj->pChaosBox[iItemPos].m_Type,
                                                                     (int) lpObj->pChaosBox[iItemPos].m_Durability);
        this->GDReqLuckyItemDelete(lpObj->pChaosBox[iItemPos].m_Type, lpObj->pChaosBox[iItemPos].m_Number,
                                   lpObj->m_Index);
        ...
        g_Log.Add(
                "[LuckyItem][Smelt Item Mix] Mix Success [%s][%s] ItemName[%s] ItemNum[%d] Level[%d] Dur[%d] skill[%d] luck[%d] option[%d] ExOpt[%d]",

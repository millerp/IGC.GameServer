    ...
    switch (lpMsg->result) {
        ...
        case 15:
            if (gObjSetAccountLogin(aIndex, szId, lpMsg->UserNumber, lpMsg->DBNumber, szJN) == 0) {
                ...
                g_Log.Add("Login [%s] : (%s) Numer: %d / DB: %d", gObj[aIndex].m_PlayerData->Ip_addr, szId,
                          lpMsg->UserNumber, lpMsg->DBNumber);
            }
...
void GJPBillCeckRecv(SDHP_BILLSEARCH_RESULT *lpMsg) {
    ...
    if (m_ObjBill[aIndex - g_ConfigRead.server.GetObjectStartUserIndex()].SetBill(lpMsg->cCertifyType, lpMsg->PayCode,
                                                                                  lpMsg->EndTime, EndsDays) == TRUE) {
        gObjAddMsgSendDelay(&gObj[aIndex], 1000, aIndex, 100, 0);

        LPOBJ lpObj = &gObj[aIndex];
        g_VipSystem.SetVipForUser(lpObj, lpMsg->cVipType);

        g_Log.Add("[%s] VIP Info Set [Type: %d] [Effect: %d]", lpObj->AccountID, lpObj->m_PlayerData->VipType,
                  lpObj->m_PlayerData->VipEffect);
    }
}
...
void JGAnsMapSvrMove(PMSG_ANS_MAPSVRMOVE *lpMsg) {
    ...
    if (strcmp(lpObj->AccountID, lpMsg->szAccountID) || strcmp(lpObj->Name, lpMsg->szCharName)) {
        g_Log.AddC(TColor::Red, "[MapServerMng] Packet Error JG [0x7A] - User Info Mismatch [%s][%s]-[%s][%s] (%d)",
                   lpMsg->szAccountID, lpMsg->szCharName, lpObj->AccountID, lpObj->Name, lpObj->m_Index);
        return;
    }

    if (lpObj->Type != OBJ_USER) {
        g_Log.AddC(TColor::Red, "[MapServerMng] Packet Error JG [0x7A] - User is not character [%s][%s]-[%s][%s] (%d)",
                   lpMsg->szAccountID, lpMsg->szCharName, lpObj->AccountID, lpObj->Name, lpObj->m_Index);
        return;
    }
    ...
    if (lpObj->m_bMapSvrMoveReq_2 == false) {
        g_Log.AddC(TColor::Red,
                   "[MapServerMng] Packet Error JG [0x7A] - Request was not send by GS [%s][%s]-[%s][%s] (%d)",
                   lpMsg->szAccountID, lpMsg->szCharName, lpObj->AccountID, lpObj->Name, lpObj->m_Index);
        return;
    }
    ...
    if (lpMsg->iResult == 0) {
        ...
        BOOL bRESULT = g_MapServerManager.GetSvrCodeData(lpMsg->wDstMapSvrCode, pMsg.szMapSvrIpAddress,
                                                         &pMsg.wMapSvrPort);
    ...
    switch (lpMsg->iResult) {
        case 0:
            ...
            if (!gObjSetAccountLogin(iIndex, szId, lpMsg->iUserNumber, lpMsg->iDBNumber, szPw)) {
                ...
                g_Log.Add("Login [%s] : (%s) Numer: %d / DB: %d", gObj[iIndex].m_PlayerData->Ip_addr, szId,
                          lpMsg->iUserNumber, lpMsg->iDBNumber);
                iSendResult = 1;
                bAuthOK = TRUE;
            }

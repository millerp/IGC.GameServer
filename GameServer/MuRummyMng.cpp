...
void CMuRummyMng::CGReqMuRummyStart(_tagPMSG_REQ_MURUMMY_INFO *lpMsg, int aIndex) {
    ...
    if (!pMuRummyInfo) {
        g_Log.Add("[MuRummy][Error] pMuRummyInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name, __FILE__,
                  __LINE__);
        return;
    }
    ...
    if (pMuRummyInfo->IsGetPlayCard() == true) {
        if (this->SetPlayCardInfo(pMuRummyInfo, stCardInfo) == true) {
            ...
            g_Log.Add("[%s][%s][Mu Rummy] Event Card Cnt(%d)", lpObj->AccountID, lpObj->Name,
                      pMuRummyInfo->GetCardDeckCnt());
        ...
        for (int x = 0; x < EVENT_INVENTORY_SIZE; x++) {
            if (lpObj->pEventInventory[x].IsItem() == TRUE && lpObj->pEventInventory[x].m_Type == ITEMGET(14, 216)) {
                ...
                g_Log.Add("[%s][%s][Mu Rummy] Event Card register success (%I64d)", lpObj->AccountID, lpObj->Name,
                          lpObj->pEventInventory[x].m_Number);
...
void CMuRummyMng::CGReqCardReveal(_tagPMSG_REQ_REVEAL_CARD *lpMsg, int aIndex) {
    ...
    if (!pCCardInfo) {
        g_Log.Add("[MuRummy][Error] pCCardInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name, __FILE__,
                  __LINE__);
        return;
    }
    ...
    if (!pSlotIndexInfo) {
        g_Log.Add("[MuRummy][Error] pSlotIndexInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name, __FILE__,
                  __LINE__);
        return;
    }
    ...
    if (btSlotNum == 255) {
        ...
        if (bRet) {
            g_Log.Add("[%s][%s][Mu Rummy] Event Card Cnt(%d) (%d)", lpObj->AccountID, lpObj->Name, iBeforeCardCnt,
                      pMuRummyInfo->GetCardDeckCnt());
...
bool
CMuRummyMng::FillEmptySlotCard(CMuRummyInfo *pMuRummyInfo, _tagMuRummyCardInfo *pOutCardInfo, OBJECTSTRUCT *lpObj) {
    CMuRummyCardInfo *pCCardInfo = pMuRummyInfo->GetCardInfo();
    ...
    for (int i = 0; i < 5; i++) {
        if (pSlotIndexInfo[i] == 255) {
            ...
            if (pMuRummyInfo->GetCardDeckCnt() < 0) {
                g_Log.Add("[MuRummy][Error] CardDeck cnt [%d] [%s, %d]", pMuRummyInfo->GetCardDeckCnt(), __FILE__,
                          __LINE__);
                return false;
            }

            this->GDReqSlotInfoUpdate(lpObj, nCardSeq, pCCardInfo[nCardSeq].GetSlotNum(),
                                      pCCardInfo[nCardSeq].GetState());
...
BYTE CMuRummyMng::CardSlotMove(CMuRummyInfo *pCMuRummyInfo, int sSlot, int tSlot, CMuRummyCardInfo *pCOutCardInfo,
                               OBJECTSTRUCT *lpObj) {
    ...
    if (sCardArr < 0 || sCardArr > 23) {
        g_Log.Add("[MuRummy][Error] CardInfo Index is Overflow [%s][%s][%d] [%s, %d]", lpObj->AccountID, lpObj->Name,
                  sCardArr, __FILE__, __LINE__);
        return -2;
    }
...
void CMuRummyMng::CGReqCardReMove(_tagPMSG_REQ_CARD_REMOVE *lpMsg, int aIndex) {
    ...
    if (!pCMuRummyInfo) {
        g_Log.Add("[MuRummy][Error] pCMuRummyInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name, __FILE__,
                  __LINE__);
        return;
    }
...
BYTE CMuRummyMng::CardMatchCheck(CMuRummyInfo *pCMuRummyInfo, int *iOutScore, _tagMuRummyCardUpdateDS *pOutCardUpdateDS,
                                 OBJECTSTRUCT *lpObj) {
    CMuRummyCardInfo *pCCardInfo = pCMuRummyInfo->GetCardInfo();
...
void CMuRummyMng::CGReqMuRummyEnd(_tagPMSG_REQ_MURUMMY_END *lpMsg, int aIndex) {
    ...
    if (pCMuRummyInfo->GetScore() < this->GetRewardScore1() || CheckInventoryEmptySpace(lpObj, 4, 2)) {
        if (pCMuRummyInfo->GetScore() >= this->GetRewardScore1()) {
            if (pCMuRummyInfo->GetScore() < this->GetRewardScore1() ||
                (pCMuRummyInfo->GetScore() >= this->GetRewardScore2())) {
                if (pCMuRummyInfo->GetScore() >= this->GetRewardScore2()) {
                    CItem Item;
                    time_t Duration;

                    if (g_BagManager.GetItemFromBag(aIndex, BAG_EVENT, EVENTBAG_MURUMMY_2, aIndex, Item, Duration) ==
                        false) {
                        return;
                    }
                    ...
                    ItemSerialCreateSend(aIndex, lpAttr->ItemKindA == 12 ? 220 : 225, 0, 0, Item.m_Type, Item.m_Level,
                                         Item.m_Durability, Item.m_Option1, Item.m_Option2, Item.m_Option3, aIndex,
                                         Item.m_NewOption, Item.m_SetOption, Duration, 0, 0);
                }
            } else {
                CItem Item;
                time_t Duration;

                if (g_BagManager.GetItemFromBag(aIndex, BAG_EVENT, EVENTBAG_MURUMMY_1, aIndex, Item, Duration) ==
                    false) {
                    return;
                }
            ...
            if (gObjCheckMaxZen(aIndex, 500000)) {
                ...
                g_Log.Add("[%s][%s][Mu Rummy] Event Reward Result [Point : %d, RewardZen : %d]", lpObj->AccountID,
                          lpObj->Name, pCMuRummyInfo->GetScore(), 500000);
...
void CMuRummyMng::CheatSetCardColorAsc(CMuRummyInfo *pMuRummyInfo, int aIndex) {
    ...
    if (!pMuRummyInfo) {
        g_Log.Add("[MuRummy][Error] pMuRummyInfo is NULL [%s][%s] [%s, %d]", gObj[aIndex].AccountID, gObj[aIndex].Name,
                  __FILE__, __LINE__);
        return;
    }
...
void CMuRummyMng::Slot5Log(OBJECTSTRUCT *lpObj, int iLogType) {
    ...
    for (int n = 0; n < 5; n++) {
        if (pSlotIndexInfo[n] == 255) {
            ...
            sprintf(szSlotInfo, "%c(%d)", this->GetColorName(pCCardInfo[pSlotIndexInfo[n]].GetColor() - 1),

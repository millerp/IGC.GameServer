...
void CCashLotterySystem::Load(LPSTR pchFilename) {
    ...
    for (pugi::xml_node section = main.child("Section"); section; section = section.next_sibling()) {
        ...
        for (pugi::xml_node item = section.child("Item"); item; item = item.next_sibling()) {
            ...
            this->InsertItem(iCategory, iItemType, iItemIndex, iItemLevelMin, iItemLevelMax, iSkillOption, iLuckOption,
                             iAddOption, iExOption);
        }
    }
}


void CCashLotterySystem::SetitemDropRate(int iCategory, int iDropRate, int iSkillOptionRate, int iLuckOptionRate,
                                         int iAddOptionRate, int iExOptionRate) {
...
BOOL
CCashLotterySystem::InsertItem(int iItemCategory, int iItemType, int iItemIndex, int iItemLevelMin, int iItemLevelMax,
                               int iSkillOption, int iLuckOption, int iAddOption, int iExOption) {
...
int CCashLotterySystem::GetItem(CItem *lpItem) {
    ...
    if (iItemType == -1 || iItemIndex == -1 || iItemLevelMin == -1 || iItemLevelMax == -1 || iSkillOption == -1 ||
        iLuckOption == -1 || iAddOption == -1 || iExOption == -1)
        return -1;
    ...
    lpItem->Convert(iItemNumber, btSkillOption, btLuckOption, btAddOption, btExOption, 0, 0, 0, -1, 0,
                    CURRENT_DB_VERSION);
    ...
    g_Log.Add(
            "[LotteryItem] Get Item Category:%d,Index:%d (Name:%s,Type:%d,Index:%d,Level:%d) Skill:%d,Luck:%d,AddOption:%d,ExOption(%d,%d,%d,%d,%d,%d)",

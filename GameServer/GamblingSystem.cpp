    ...
    if (GetBagCount() > 0) {
        if (m_iGamblingItemGetRate[btItemType] >= GetLargeRand() % 1000000) {
            ...
            if (dwOptionTemp <= m_iAddOption[0]) {
                ...
                iOption3 = 4;
            } else if (dwOptionTemp <=
                       m_iAddOption[0] + m_iAddOption[1] + m_iAddOption[2] + m_iAddOption[3] + m_iAddOption[4]) {
                iOption3 = 5;
            } else if (dwOptionTemp <=
                       m_iAddOption[0] + m_iAddOption[1] + m_iAddOption[2] + m_iAddOption[3] + m_iAddOption[4] +
                       m_iAddOption[5]) {
            ...
            } else if (dwExOptionTemp <=
                       m_iGamblingOptionCountRate[0] + m_iGamblingOptionCountRate[1] + m_iGamblingOptionCountRate[2]) {
                iExOption = GamblingOptionRand(3);
            } else if (dwExOptionTemp <=
                       m_iGamblingOptionCountRate[0] + m_iGamblingOptionCountRate[1] + m_iGamblingOptionCountRate[2] +
                       m_iGamblingOptionCountRate[3]) {
                iExOption = GamblingOptionRand(4);
            } else if (dwExOptionTemp <=
                       m_iGamblingOptionCountRate[0] + m_iGamblingOptionCountRate[1] + m_iGamblingOptionCountRate[2] +
                       m_iGamblingOptionCountRate[3] + m_iGamblingOptionCountRate[4]) {
                iExOption = GamblingOptionRand(5);
            } else if (dwExOptionTemp <=
                       m_iGamblingOptionCountRate[0] + m_iGamblingOptionCountRate[1] + m_iGamblingOptionCountRate[2] +
                       m_iGamblingOptionCountRate[3] + m_iGamblingOptionCountRate[4] + m_iGamblingOptionCountRate[5]) {
        ...
        ItemSerialCreateSend(lpObj->m_Index, btMapNumber, X, Y, iType, iLevel, fDur, iOption1, iOption2, iOption3,
                             lpObj->m_Index, iExOption, 0, 0, 0, 0);
        g_Log.Add("[ GAMBLING ][ TryGambling ] %s(%s) Try Item : [%s]%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
                  lpObj->AccountID, lpObj->Name, ItemAttribute[iType].Name, iType, iLevel, iOption1, iOption2, iOption3,
                  iExOption);
    }
    return 1;
}

...
BOOL TMonsterAIElement::LoadData(LPSTR lpszFileName) {
    ...
    try {
        ...
        for (pugi::xml_node element = main.child("Element"); element; element = element.next_sibling()) {
            ...
            if (iElementNumber < 0 || iElementNumber >= MAX_AI_ELEMENT) {
                g_Log.MsgBox("[Monster AI Element] - ElementNumber(%d) Error (%s) File. ", iElementNumber,
                             lpszFileName);
                continue;
            }
            ...
            memcpy(TMonsterAIElement::s_MonsterAIElementArray[iElementNumber].m_szElementName, szElementName,
                   sizeof(szElementName));
        }
    ...
    TMonsterAIElement::s_MonsterAIMovePath[MAP_INDEX_CRYWOLF_FIRSTZONE].LoadData(
            g_ConfigRead.GetPath("\\Monsters\\AI\\IGC_Monster_AI_MovePath.xml"), "CryWolf");
    return FALSE;
}
...
BOOL TMonsterAIElement::ApplyElementMoveTarget(int iIndex, int iTargetIndex, TMonsterAIState *pAIState) {
    ...
    int iTargetDistance = sqrt(static_cast<float>(((lpObj->X - iTargetX) * (lpObj->X - iTargetX)) +
                                                  ((lpObj->Y - iTargetY) * (lpObj->Y - iTargetY))));
...
BOOL TMonsterAIElement::ApplyElementAttack(int iIndex, int iTargetIndex, TMonsterAIState *pAIState) {
    ...
    if (TMonsterSkillManager::CheckMonsterSkill(lpObj->Class) &&
        lpObj->Class != 519) // Medic has only BUFF in MONSTERSKILL, so disable check for him
    {
...
BOOL TMonsterAIElement::ApplyElementAttackArea(int iIndex, int iTargetIndex, TMonsterAIState *pAIState) {
    ...
    for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++) {
        ...
        int iTargetDistance = sqrt(static_cast<float> (((lpTargetObj->X - iTargetX) * (lpTargetObj->X - iTargetX)) +
                                                       ((lpTargetObj->Y - iTargetY) * (lpTargetObj->Y - iTargetY))));
...
BOOL TMonsterAIElement::ApplyElementHealGroup(int iIndex, int iTargetIndex, TMonsterAIState *pAIState) {
    LPOBJ lpObj = &gObj[iIndex];
    UTIL.SendCrywolfChattingMsg(iIndex, "Element-�׷�ġ��");
    TMonsterAIGroupMember *pMemb = TMonsterAIGroup::FindGroupMemberToHeal(lpObj->m_Index, lpObj->m_iGroupNumber,
                                                                          lpObj->m_iGroupMemberGuid, 6);
...
BOOL TMonsterAIElement::ApplyElementSpecialSommon(int iIndex, int iTargetIndex, TMonsterAIState *pAIState) {
    ...
    TMonsterAIGroupMember *pMemb = TMonsterAIGroup::FindGroupMemberToSommon(lpObj->m_Index, lpObj->m_iGroupNumber,
                                                                            lpObj->m_iGroupMemberGuid);

...
int CBuffEffectSlot::SetEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex, BYTE btEffectType1, BYTE btEffectType2,
                               int iEffectValue1, int iEffectValue2, int iDuration) {
    ...
    for (int i = 0; i < MAX_BUFFEFFECT; i++) {
        if (bIsRemoveEffect == true) {
            if ((lpObj->m_BuffEffectList[i].BuffIndex != BUFFTYPE_NONE) && (i > 0)) {
                memcpy(&lpObj->m_BuffEffectList[i - 1], &lpObj->m_BuffEffectList[i],
                       sizeof(lpObj->m_BuffEffectList[i])); //Change the Buff Slot to the removed one...
        ...
        {
            g_BuffEffect.ClearBuffEffect(lpObj, lpObj->m_BuffEffectList[i].EffectType1,
                                         lpObj->m_BuffEffectList[i].EffectValue1);
            g_BuffEffect.ClearBuffEffect(lpObj, lpObj->m_BuffEffectList[i].EffectType2,
                                         lpObj->m_BuffEffectList[i].EffectValue2);
            memset(&lpObj->m_BuffEffectList[i], 0, sizeof(lpObj->m_BuffEffectList[i]));
...
void gObjCheckBuffEffectList(LPOBJECTSTRUCT lpObj) {
    ...
    if ((lpObj->Type == OBJ_USER && lpObj->Connected >= PLAYER_PLAYING) ||
        lpObj->Type == OBJ_MONSTER) //Connected Users or Monsters General
    {
        for (int i = 0; i < MAX_BUFFEFFECT; i++) {
            ...
            switch (lpObj->m_BuffEffectList[i].BuffIndex) {
                case BUFFTYPE_POISON: //55	27	255	255	Poison	1	1	1	Poisoned status of condition
                {
                    ...
                    {
                        g_BuffEffect.SetActiveBuffEffect(lpObj, lpObj->m_BuffEffectList[i].EffectType1,
                                                         lpObj->m_BuffEffectList[i].EffectValue1);
                        g_BuffEffect.SetActiveBuffEffect(lpObj, lpObj->m_BuffEffectList[i].EffectType2,
                                                         lpObj->m_BuffEffectList[i].EffectValue2);
                    }
                }
                    break;
...
bool gObjAddBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex) {
    ...
    iRemoveBuffIndex = g_BuffEffectSlot.SetEffect(lpObj, iBuffIndex, lpItemEffectInfo->wEffectType1,
                                                  lpItemEffectInfo->wEffectType2, lpItemEffectInfo->iEffectValue1,
                                                  lpItemEffectInfo->iEffectValue2, lpItemEffectInfo->iEffectValidTime);
    ...
    if (iRemoveBuffIndex != -1) {
        ...
        GCUseBuffEffect(lpObj, iBuffIndex, BUFFSTATE_BUFF_DELETE, lpItemEffectInfo->wOptionType,
                        lpItemEffectInfo->wEffectType1, lpItemEffectInfo->iEffectValidTime,
                        lpItemEffectInfo->iEffectValue1);
        return true;
    }
...
bool gObjAddBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex, BYTE EffectType1, int EffectValue1, BYTE EffectType2,
                       int EffectValue2, int Duration) {
    if (lpObj == NULL) return false;
    ...
    iRemoveBuffIndex = g_BuffEffectSlot.SetEffect(lpObj, iBuffIndex, EffectType1, EffectType2, EffectValue1,
                                                  EffectValue2, Duration);

    if (iRemoveBuffIndex != -1) {
        if (iRemoveBuffIndex > 0) {
            GCUseBuffEffect(lpObj, iRemoveBuffIndex, BUFFSTATE_BUFF_NEW, lpBuffData->BuffEffectType, 0, 0, 0);
        } else lpObj->m_BuffEffectCount++;
        GCUseBuffEffect(lpObj, iBuffIndex, BUFFSTATE_BUFF_DELETE, lpBuffData->BuffEffectType, EffectType1, Duration,
                        EffectValue1);
        return true;
    }
...
bool gObjAddBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex, int Duration) {
    ...
    iRemoveBuffIndex = g_BuffEffectSlot.SetEffect(lpObj, iBuffIndex, lpItemEffectInfo->wEffectType1,
                                                  lpItemEffectInfo->wEffectType2, lpItemEffectInfo->iEffectValue1,
                                                  lpItemEffectInfo->iEffectValue2, Duration);
    ...
    if (iRemoveBuffIndex != -1) {
        ...
        GCUseBuffEffect(lpObj, iBuffIndex, BUFFSTATE_BUFF_DELETE, lpItemEffectInfo->wOptionType,
                        lpItemEffectInfo->wEffectType1, Duration, lpItemEffectInfo->iEffectValue1);
...
bool gObjAddPeriodBuffEffect(OBJECTSTRUCT *lpObj, _tagPeriodBuffEffectInfo *lpPeriBuff, unsigned int dwDuration) {
    ...
    iRemoveBuffIndex = g_BuffEffectSlot.SetEffect(lpObj, lpPeriBuff->wBuffIndex, lpPeriBuff->btEffectType1,
                                                  lpPeriBuff->btEffectType2, lpPeriBuff->wEffectValue1,
                                                  lpPeriBuff->wEffectValue2, dwDuration);
    ...
    if (iRemoveBuffIndex != -1) {
        ...
        GCUseBuffEffect(lpObj, lpPeriBuff->wBuffIndex, BUFFSTATE_BUFF_DELETE, lpBuffData->BuffEffectType,
                        lpPeriBuff->btEffectType1, dwDuration, lpPeriBuff->wEffectValue1);
        return true;
    }
...
bool gObjAddBuffEffectForInGameShop(LPOBJECTSTRUCT lpObj, WORD wItemCode, int Duration) {
    ...
    int iRemoveBuffIndex = g_BuffEffectSlot.SetEffect(lpObj, lpBuffData->BuffIndex, lpItemEffectInfo->wEffectType1,
                                                      lpItemEffectInfo->wEffectType2, lpItemEffectInfo->iEffectValue1,
                                                      lpItemEffectInfo->iEffectValue2, Duration);
    ...
    GCUseBuffEffect(lpObj, lpBuffData->BuffIndex, BUFFSTATE_BUFF_DELETE, lpItemEffectInfo->wOptionType,
                    lpItemEffectInfo->wEffectType1, Duration, lpItemEffectInfo->iEffectValue1);
    return true;
}
...
bool gObjRemoveBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex) {
    ...
    if (g_BuffEffectSlot.RemoveEffect(lpObj, iBuffIndex) == true) {
        ...
        if (iBuffIndex == BUFFTYPE_MONK_INCREASE_HEALTH || iBuffIndex == BUFFTYPE_MONK_INCREASE_HEALTH_STR ||
            iBuffIndex == BUFFTYPE_BLESS ||
            iBuffIndex == BUFFTYPE_ACHERON_FIRE || iBuffIndex == BUFFTYPE_ACHERON_FROST ||
...
void GCUseBuffEffect(LPOBJECTSTRUCT lpObj, BYTE BuffEffectIndex, BYTE EffectUseOption, WORD OptionType, WORD EffectType,
                     int LeftTime, DWORD EffectValue) {
...
BOOL gObjAddBuffEffectWideArea(LPOBJECTSTRUCT lpTargetObj, int nDistance, int &nAffectedCount, int iBuffIndex,
                               BYTE EffectType1, int EffectValue1, BYTE EffectType2, int EffectValue2, int Duration) {
        ...
        if (lpObj->Type == OBJ_USER && gObjIsConnected(i) != FALSE) {
            if (lpObj->MapNumber == lpTargetObj->MapNumber) {
                if (gObjCalDistance(lpTargetObj, lpObj) < nDistance) {
                    gObjAddBuffEffect(lpObj, iBuffIndex, EffectType1, EffectValue1, EffectType2, EffectValue2,
                                      Duration);
                    nAffectedCount++;
                }
            }
...
void CheckItemOptForGetExpExRenewal(LPOBJ lpObj, LPOBJ lpTargetObj, UINT64 &iExp, UINT64 iDefaultExp, BOOL bRewardExp) {
    ...
    if (lpObj->pInventory[8].IsItem() == TRUE && lpObj->pInventory[8].m_Type == ITEMGET(13, 80) &&
    ...
    if (gObjCheckUsedBuffEffect(lpObj, 179) || gObjCheckUsedBuffEffect(lpObj, 180) ||
        gObjCheckUsedBuffEffect(lpObj, 181) || gObjCheckUsedBuffEffect(lpObj, 182) ||
        gObjCheckUsedBuffEffect(lpObj, 183)) {
        nAddExp += 20;
    }

    if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_ASCENSION_MARK1) ||
        gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_ASCENSION_MARK2) ||
        gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_ASCENSION_MARK3)) {
    ...
    if (lpObj->pInventory[8].IsItem() == TRUE && lpObj->pInventory[8].m_Type == ITEMGET(13, 123) &&
        lpObj->pInventory[8].m_Durability > 0.0) {
        if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_PCS_MARK3) ||
            gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_PCBANG_POINT_MARK3)) {
            iRetExp = 0;
    ...
    if (((lpObj->pInventory[8].IsItem() == TRUE && lpObj->pInventory[8].m_Type == ITEMGET(13, 123) &&
          lpObj->pInventory[8].m_Durability > 0.0) ||
         (lpObj->pInventory[8].IsItem() == TRUE && lpObj->pInventory[8].m_Type == ITEMGET(13, 80) &&
          lpObj->pInventory[8].m_Durability > 0.0)) &&

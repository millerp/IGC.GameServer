...
bool CItemSocketOptionSystem::LoadOptionScript(char *pchFileName) {
    ...
    for (pugi::xml_node option = item_option.child("Option"); option; option = option.next_sibling()) {
        ...
        m_SeedData[iSeedOptionDataCount].SetOption(btSeedIndex, btSeedType, btItemLevel, btOptionValueType,
                                                   wOptionValue, iExtractRate);
...
bool CItemSocketOptionSystem::GetSeedSphereEffect(BYTE btSeedSphereIndex, BYTE &pbSeedType, BYTE &pbEffectValueType,
                                                  WORD &pwEffectValue) {
    BYTE SeedIndex = 0;
    BYTE SphereLevel = 0;
...
void CItemSocketOptionSystem::GetSocketOption(CItem *lpItemData, BYTE *pbSocketOption, BYTE &pbSocketBonusOption) {
    ...
    if (lpItemData->IsItem() == FALSE ||
        IsEnableSocketItem(lpItemData) == false)
        return;
...
int CItemSocketOptionSystem::ApplySeedSphereEffect(LPOBJECTSTRUCT lpObj) {
    ...
    for (int iEquip = 0; iEquip < INVETORY_WEAR_SIZE; iEquip++) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            SetEffect(lpObj, SOCKET_OPTION_TYPE_SEED, SeedIndex, SeedSphere.m_SeedOptionValueType,
                      SeedSphere.m_SeedOptionValue);
...
void CItemSocketOptionSystem::ApplyBonusOptionEffect(LPOBJECTSTRUCT lpObj) {
    ...
    for (int iEquip = 0; iEquip < INVETORY_WEAR_SIZE; iEquip++) {
        ...
        g_Log.Add(
                "[SocketItem] [%s][%s] Set Bonus Socket Option - OptionIndex : %d, EffectType : %d, OptionType : %d, OptionValue : %d",
                lpObj->AccountID, lpObj->Name, lpBonusOptionData->OptionIndex, lpBonusOptionData->EffectType,
                lpBonusOptionData->OptionValueType, lpBonusOptionData->OptionValue);

        //							EffectType : %d, OptionType : %d, OptionValue : %d
        SetEffect(lpObj, SOCKET_OPTION_TYPE_BONUS, lpBonusOptionData->EffectType, lpBonusOptionData->OptionValueType,
...
void CItemSocketOptionSystem::ApplySetOptionEffect(LPOBJECTSTRUCT lpObj) {
    ...
    for (int iUserSocketIndex = 0; iUserSocketIndex < 35; iUserSocketIndex++) {
        ...
        btTotalSeedTypeList[btSeedType -
                            1] += lpObj->m_PlayerData->m_SeedOptionList[iUserSocketIndex].m_SocketOptionCount;
    }
    ...
    for (int iSetOptionIndex = 0; iSetOptionIndex < 2; iSetOptionIndex++) {
        ...
        if (btMatchedCount == 6) {
            SetEffect(lpObj, SOCKET_OPTION_TYPE_SET, m_SetSocketOptionData[iSetOptionIndex].EffectType,
                      m_SetSocketOptionData[iSetOptionIndex].OptionValueType,
                      m_SetSocketOptionData[iSetOptionIndex].OptionValue);
        }
        btMatchedCount = 0;
    }
}
...
void
CItemSocketOptionSystem::SetEffect(LPOBJECTSTRUCT lpObj, BYTE btOptionType, BYTE btEffectType, BYTE btEffectValueType,
                                   WORD wEffectValue) {
...
void CItemSocketOptionSystem::_SetFireEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                             WORD wEffectValue) {
...
void CItemSocketOptionSystem::_SetWaterEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                              WORD wEffectValue) {
    int iCalcEffectValue = 0;
...
void CItemSocketOptionSystem::_SetIceEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                            WORD wEffectValue) {
    int iAddValue = 0;
...
void CItemSocketOptionSystem::_SetWindEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                             WORD wEffectValue) {
    int iAddValue = 0;
    ...
    switch (btEffectType) {
        ...
        case SEED_EFFECT_TYPE_MAX_HP_INC: {
            ...
            GSProtocol.GCReFillSend(lpObj->m_Index, (lpObj->AddLife + lpObj->MaxLife), 0xFE, 0,
                                    (lpObj->iMaxShield + lpObj->iAddShield));
        ...
        case SEED_EFFECT_TYPE_MAX_MP_INC: {
            ...
            GSProtocol.GCManaSend(lpObj->m_Index, (lpObj->AddMana + lpObj->MaxMana), 0xFE, 0,
...
void CItemSocketOptionSystem::_SetThunderEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                                WORD wEffectValue) {
    int iAddValue = 0;
...
void CItemSocketOptionSystem::_SetEarthEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                              WORD wEffectValue) {
    if (lpObj->Type != OBJ_USER) {
        return;
    }
    ...
    switch (btEffectType) {
        case SEED_EFFECT_TYPE_STRENGTH_INC: {
            iAddValue = CalcEffectValue(lpObj->m_PlayerData->Strength + lpObj->AddStrength, wEffectValue,
                                        btEffectValueType);
            lpObj->AddStrength += iAddValue;
        }
            break;
        case SEED_EFFECT_TYPE_DEXTERITY_INC: {
            iAddValue = CalcEffectValue(lpObj->m_PlayerData->Dexterity + lpObj->AddDexterity, wEffectValue,
                                        btEffectValueType);
        ...
        case SEED_EFFECT_TYPE_VITALITY_INC: {
            iAddValue = CalcEffectValue(lpObj->m_PlayerData->Vitality + lpObj->AddVitality, wEffectValue,
                                        btEffectValueType);
            lpObj->AddVitality += iAddValue;
        }
            break;
        case SEED_EFFECT_TYPE_ENERGY_INC: {
            iAddValue = CalcEffectValue(lpObj->m_PlayerData->Energy + lpObj->AddEnergy, wEffectValue,
                                        btEffectValueType);
...
void CItemSocketOptionSystem::_SetUniqueEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                               WORD wEffectValue) {
...
void CItemSocketOptionSystem::_SetSetEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType,
                                            WORD wEffectValue) {
    int iCalcEffectValue = 0;
...
void CItemSocketOptionSystem::SetRequireStatOption(CItem *lpItemdata) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (SeedIndex == SEED_EFFECT_TYPE_REQUIRE_STRENGTH_DEC) {
            iReduceValue = CalcEffectValue(lpItemdata->m_HJOpStrength, SeedSphere.m_SeedOptionValue,
                                           SeedSphere.m_SeedOptionValueType);
            lpItemdata->m_HJOpStrength += iReduceValue;
        } else if (SeedIndex == SEED_EFFECT_TYPE_REQUIRE_DEXTERITY_DEC) {
            iReduceValue = CalcEffectValue(lpItemdata->m_HJOpDexterity, SeedSphere.m_SeedOptionValue,
                                           SeedSphere.m_SeedOptionValueType);
...
bool CItemSocketOptionSystem::SetSocketSlot(CItem *lpTargetItem, BYTE btSeedIndex, BYTE btSphereLevel,
...
bool CItemSocketOptionSystem::SetBonusSocketOption(CItem *lpItemData) {
    ...
    for (int iBonusIndex = 0; iBonusIndex < 6; iBonusIndex++) {
        if (btItemType >= m_BonusSocketOptionData[iBonusIndex].ItemTypeBegin &&
            ...
            for (int iSlotIndex = 0; iSlotIndex < 5; iSlotIndex++) {
                if (m_BonusSocketOptionData[iBonusIndex].SocketSlotType[iSlotIndex] == SeedSphereType[iSlotIndex] &&
                    SeedSphereType[iSlotIndex] != NULL) {
                    btSocketSlotCount++;
                }
                ...
                if (btSocketSlotCount >= m_BonusSocketOptionData[iBonusIndex].SocketSlotCount) {
                    if (rand() % 100 < m_BonusSocketOptionData[iBonusIndex].ExtractRate) {
                        ...
                        g_Log.Add(
                                "[SocketItem] Set Bonus Socket Option Success - OptionIndex : %d, ItemCode : %d, SocketOption[%d,%d,%d,%d,%d]",
                                lpItemData->m_BonusSocketOption, lpItemData->m_Type, lpItemData->m_SocketOption[0],
                                lpItemData->m_SocketOption[1], lpItemData->m_SocketOption[2],
                                lpItemData->m_SocketOption[3], lpItemData->m_SocketOption[4]);
                        return true;
                    }
                ...
                for (int iSlotIndex = 0; iSlotIndex < 5; iSlotIndex++) {
                    ...
                    if (btSocketSlotCount >= m_BonusSocketOptionData[iBonusIndex].SocketSlotCount) {
                        if (rand() % 100 < m_BonusSocketOptionData[iBonusIndex].ExtractRate) {
                            lpItemData->m_BonusSocketOption = m_BonusSocketOptionData[iBonusIndex].OptionIndex;
                            g_Log.Add(
                                    "[SocketItem] Set Bonus SocketEx Option Success - OptionIndex : %d, ItemCode : %d, SocketOption[%d,%d,%d,%d,%d]",
                                    lpItemData->m_BonusSocketOption, lpItemData->m_Type, lpItemData->m_SocketOption[0],
                                    lpItemData->m_SocketOption[1], lpItemData->m_SocketOption[2],
                                    lpItemData->m_SocketOption[3], lpItemData->m_SocketOption[4]);
                            return true;

...
int CMoveCommand::GetMoveLevel(int mapnumber, int x, int y, int Class) {
    for (int i = 0; i < MAX_MOVE_COMMAND; i++) {
        if (this->m_MoveLevel[i].MapNumber == mapnumber) {
            if (x >= this->m_MoveLevel[i].X && x <= this->m_MoveLevel[i].TX && y >= this->m_MoveLevel[i].Y &&
                y <= this->m_MoveLevel[i].TY) {
...
int CMoveCommand::FindIndex(LPSTR mapname) {
    for (int i = 0; i < MAX_MOVE_COMMAND; i++) {
        ...
        if (strcmp(mapname, this->m_MoveCommandData[i].Name) == 0 ||
            strcmpi(mapname, this->m_MoveCommandData[i].EngName) == 0) {
            return i;
        }
    }
...
BOOL CMoveCommand::Move(LPOBJ lpObj, int iMapIndex) {
        ...
        if (NeedLevel <= lpObj->Level) {
            if ((lpObj->m_PlayerData->Money - NeedZen) >= 0) {
                if (gObjMoveGate(lpObj->m_Index, GateNumber) != FALSE) {
                    ...
                    g_Log.Add("[%s][%s] [Using Warp to: %s] Success : reduces money %d - %d", lpObj->AccountID,
                              lpObj->Name, this->m_MoveCommandData[index].Name, lpObj->m_PlayerData->Money, NeedLevel);
    ...
    g_Log.Add("[%s][%s] [Using Warp to: %s] Fail GateNumber = %d", lpObj->AccountID, lpObj->Name,
              this->m_MoveCommandData[index].Name, GateNumber);
    return FALSE;
...
int GetKalimaGateLevel(LPOBJ lpObj) {
    if (lpObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_RAGEFIGHTER) {
        for (int i = 0; i < KALIMA_FLOORS; i++) {
            if (lpObj->Level >= g_sttKALIMAGATE_LEVEL[i].iLOWER_BOUND_MAGUMSA &&
                lpObj->Level <= g_sttKALIMAGATE_LEVEL[i].iUPPER_BOUND_MAGUMSA) {
                return i;
            }
        }
    } else {
        for (int i = 0; i < KALIMA_FLOORS; i++) {
            if (lpObj->Level >= g_sttKALIMAGATE_LEVEL[i].iLOWER_BOUND &&
                lpObj->Level <= g_sttKALIMAGATE_LEVEL[i].iUPPER_BOUND) {
                return i;
            }
        }
    }

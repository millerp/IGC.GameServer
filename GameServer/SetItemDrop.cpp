...
bool CSetItemDrop::DropItem(LPOBJ lpMonster, LPOBJ lpUser) {
    ...
    for (int i = 0; i < lpDropItem->ItemCount; i++) {
        ...
        if (!p) {
            g_Log.AddC(TColor::Red, "ERROR - Item not found (%d) CSetItemDrop::DropItem (%s)(%d)", iItemType,
                       lpUser->Name, lpMonster->Class);
            return false;
        }

        if (iSetItemOption == 0) {
            g_Log.AddC(TColor::Red, "ERROR - Set Item Option not found (%d) (Cat:%d) CSetItemDrop::DropItem (%s)(%d)",
                       iItemType, lpDropItem->ItemCat, lpUser->Name, lpMonster->Class);
            return false;
        }
        ...
        ItemSerialCreateSend(lpMonster->m_Index, lpMonster->MapNumber, lpMonster->X, lpMonster->Y, iItemType,
                             btItemLevel, 0, btSkill, btLuck, btOption, lpUser->m_Index, btExc, iSetItemOption, 0, 0,
                             0);
    }
...
SETITEM_DROP_RULE *CSetItemDrop::GetDropRule(OBJECTSTRUCT *lpUser, OBJECTSTRUCT *lpMonster) {
    ...
    for (std::vector<SETITEM_DROP_RULE>::iterator It = this->m_vtDropRules.begin();
         It != this->m_vtDropRules.end(); It++, SectionID++) {
...
SETITEM_DROP_ITEM *CSetItemDrop::GetItemData(SETITEM_DROP_RULE *lpRule) {
    ...
    for (std::vector<SETITEM_DROP_ITEM>::iterator It = lpRule->m_vtSetItems.begin();
         It != lpRule->m_vtSetItems.end(); It++) {

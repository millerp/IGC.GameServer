...
void CMarry::LoadFile(char *filename) {
    ...
    for (pugi::xml_node item = wedding.child("Item"); item; item = item.next_sibling()) {
        ...
        if (iItemID == -1) {
            g_Log.MsgBox("ERROR - Wrong item in %s file (%d %d)", filename, item.attribute("ItemCat").as_int(),
                         item.attribute("ItemIndex").as_int());
            continue;
        }
...
void CMarry::Propose(int aIndex, int uIndex) {
    ...
    if (this->m_iMarryNeedMoney > gObj[aIndex].m_PlayerData->Money ||
        this->m_iMarryNeedMoney > gObj[uIndex].m_PlayerData->Money) {
    ...
    g_Log.Add("[Marry][%s][%s] Request to marry with [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name,
...
bool CMarry::Accept(int aIndex) {
    ...
    if (gObj[uIndex].m_PlayerData->Money < gObj[uIndex].m_PlayerData->Money - this->m_iMarryNeedMoney ||
        gObj[aIndex].m_PlayerData->Money < gObj[aIndex].m_PlayerData->Money - this->m_iMarryNeedMoney) {
    ...
    g_Log.Add("[Marry] New Marriage: %s [%d]  %s [%d]", gObj[uIndex].Name, gObj[aIndex].Married, gObj[aIndex].Name,
              gObj[uIndex].Married);
    return true;
}
...
bool CMarry::CheckPosition(int aIndex, int uIndex) {
    ...
    if ((aX >= this->m_btMarryStartX && aX <= this->m_btMarryEndX) &&
        (aY >= this->m_btMarryStartY && aY <= this->m_btMarryEndY) &&
        (uX >= this->m_btMarryStartX && uX <= this->m_btMarryEndX) &&
        (uY >= this->m_btMarryStartY && uY <= this->m_btMarryEndY)) {
        return true;
    }
...
bool CMarry::CheckDivorceItem(int aIndex) {
    ...
    for (std::vector<MARRY_ITEM>::iterator It = this->m_vtDivorceItem.begin();
         It != this->m_vtDivorceItem.end(); It++) {
        if (gObjGetItemCountInIventory(aIndex, It->m_wItemID) < It->m_iItemCount) {
            LeaveCriticalSection(&this->m_criti);
            return false;
        }

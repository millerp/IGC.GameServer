...
bool CItemDrop::LoadFile(const char *szFile) {
    ...
    for (pugi::xml_node monster = main.child("Monster"); monster; monster = monster.next_sibling()) {
        ...
        if (strcmp(monster.attribute("PlayerMaxLevel").as_string(), "MAX") == 0) {
            itemdrop_monster_ptr_->wPlayerMaxLevel =
...
bool CItemDrop::DropItem(LPOBJ lpUser, LPOBJ lpMonster) {
    ...
    for (int i = 0; i < iItemCount; i++) {
        ...
        if (ItemNumber == -1) {
            delete[] lpItems;
            g_Log.AddC(TColor::Red, "ERROR - Bad Index (%d) CItemDrop::DropItem (%s)(%d)", ItemNumber, lpUser->Name,
                       lpMonster->Class);
            return false;
        }
        ...
        if (!p) {
            delete[] lpItems;
            g_Log.AddC(TColor::Red, "ERROR - Item not found (%d) CItemDrop::DropItem (%s)(%d)", ItemNumber,
                       lpUser->Name, lpMonster->Class);
            return false;
        }
        ...
        ItemSerialCreateSend(lpMonster->m_Index, lpMonster->MapNumber, lpMonster->X, lpMonster->Y, ItemNumber,
                             btItemLevel, 0, btSkill, btLuck, btOption, lpUser->m_Index, btExc, btSetItem, dwDuration,
                             btSocketOption, btMainAttribute);
    }
...
boost::shared_ptr <ITEMDROP_MONSTER> CItemDrop::GetMonsterData(OBJECTSTRUCT *lpUser, OBJECTSTRUCT *lpMonster) {
    ...
    for (std::vector < boost::shared_ptr < ITEMDROP_MONSTER >> ::iterator It = this->m_vtMonsterList.begin(); It !=
                                                                                                              this->m_vtMonsterList.end();
    It++, SectionID++)
    {
        ...
        if (itemdrop_monster_->btMonsterMapNumber != lpMonster->MapNumber &&
            itemdrop_monster_->btMonsterMapNumber != (BYTE) - 1) {
            continue;
        }

        if (lpMonster->Level < itemdrop_monster_->wMonsterMinLevel &&
            itemdrop_monster_->wMonsterMinLevel != (WORD) - 1) {
            continue;
        }

        if (lpMonster->Level > itemdrop_monster_->wMonsterMaxLevel &&
            itemdrop_monster_->wMonsterMaxLevel != (WORD) - 1) {
            continue;
        }
...
boost::shared_ptr <ITEMDROP_ITEM>
CItemDrop::GetItemFromMonster(boost::shared_ptr <ITEMDROP_MONSTER> itemdrop_monster_ptr) {
    int iSuccessRate = 0;
    int iSuccessMaxRate = 0;
    int itemDropRate = 0;

    for (std::vector < boost::shared_ptr <
         ITEMDROP_ITEM >> ::iterator It = itemdrop_monster_ptr.get()->m_vtItems.begin(); It !=
                                                                                         itemdrop_monster_ptr.get()->m_vtItems.end();
    It++)
    {

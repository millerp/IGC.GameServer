...
BOOL CShop::InsertItem(int type, int index, int level, int dur, int op1, int op2, int op3, int newop, int setop,
                       int socketcount, int elemental, int serial) {
    int itemp;
    int width;
    int height;
    int x;
    ...
    if (g_SocketOptionSystem.IsEnableSocketItem(itemp) == true)
        ...
        if (socketcount == 0) {
            ...
            g_PentagramSystem.MakePentagramSocketSlot(&this->m_item[blank], Slot[0], Slot[1], Slot[2], Slot[3],
                                                      Slot[4]);
        }
    ...
    this->m_item[blank].Convert(itemp, op1, op2, op3, newop, setop, 0, this->m_item[blank].m_SocketOption,
                                this->m_item[blank].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
...
BOOL CShop::LoadShopItem(char *filename) {
    ...
    for (pugi::xml_node item = mainsection.child("Item"); item; item = item.next_sibling()) {
        ...
        if (this->InsertItem(type, index, level, dur, op1, op2, op3, newop, setop, socketcount, elemental, serial) ==
            FALSE) {
...
SHOP_DATA *CShopMng::GetShop(LPOBJ lpObj, LPOBJ lpNpc) {
    ...
    for (; ItShopData != this->m_vecShopData.end(); ItShopData++) {
        if (ItShopData->iNpcIndex == lpNpc->Class && ItShopData->btMapNumber == lpNpc->MapNumber &&
            ItShopData->btPosX == lpNpc->X && ItShopData->btPosY == lpNpc->Y) {
            break;
        }
    }
    ...
    if (lpObj->m_PlayerData->VipType < ItShopData->btReqVipLevel && ItShopData->btOnlyForGameMaster == FALSE) {
        if (lpObj->m_PlayerData->VipType == 0) {
            ...
            char szMsg[256];
            sprintf(szMsg, Lang.GetText(0, 605), g_VipSystem.GetVipName(lpObj->m_PlayerData->VipType),
                    g_VipSystem.GetVipName(ItShopData->btReqVipLevel));
    ...
    if (g_ConfigRead.pk.bPkPenaltyDisable == FALSE && g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == FALSE &&
        g_ConfigRead.pk.bPkCanUseShops == false) {
...
bool CShopMng::IsShopNpc(int iNpcIndex, BYTE btMapNumber, BYTE btPosX, BYTE btPosY) {
    for (std::vector<SHOP_DATA>::iterator ItShopData = this->m_vecShopData.begin();
         ItShopData != this->m_vecShopData.end(); ItShopData++) {
        if (ItShopData->iNpcIndex == iNpcIndex && ItShopData->btMapNumber == btMapNumber &&
            ItShopData->btPosX == btPosX && ItShopData->btPosY == btPosY) {
            return true;
        }
    }
...
void CCancelItemSale::DGAnsSoldItemList(LPBYTE lpRecv) {
    ...
    if (lpMsg->btResult != 0) {
        ...
        for (int i = 0; i < lpMsg->iCount; i++) {
            lpItemList = (SDHP_SHOP_REBUY_ITEM *) (lpRecv + sizeof(SDHP_ANS_SHOP_REBUY_LIST) +
                                                   (i * sizeof(SDHP_SHOP_REBUY_ITEM)));
...
void CCancelItemSale::DGAnsGetReBuyItem(SDHP_ANS_SHOP_REBUY_GET_ITEM *lpMsg) {
    ...
    if (lpMsg->btResult != 0) {
        ...
        Item.Convert(lpMsg->wItemCode, lpMsg->btItemOp1, lpMsg->btItemOp2, lpMsg->btItemOp3, lpMsg->btItemExcOp,
                     lpMsg->btItemSetOp, Item.m_ItemOptionEx, lpMsg->btItemSocketOp, lpMsg->btItemBonusSocketOp, 0, 3);

        LPITEM_ATTRIBUTE p = &ItemAttribute[Item.m_Type];

        if (CheckInventoryEmptySpace(&gObj[iIndex], p->Height * lpMsg->wItemCount, p->Width * lpMsg->wItemCount) ==
            FALSE) {

...
void CMasterLevelSkillTreeSystem::AddToValueTable(int iSkillValueID, const char *szFunctionName, int iSkillValueType) {
    if (iSkillValueID < 0 || iSkillValueID > MAX_MASTER_SKILL_TYPE) {
        g_Log.Add("[MasterLevelSkillTreeSystem] - Error - Wrong iSkillValueID [%d] - [%s] [%d]", iSkillValueID,
                  __FILE__, __LINE__);
        return;
    }
...
BOOL CMasterLevelSkillTreeSystem::CheckCompleteBrandOfSkill(LPOBJ lpObj, int iMLSkill) {
    ...
    std::pair<int, int> iBrandOfSkill = std::pair<int, int>(this->GetParentSkill1(lpObj->Class, iMLSkill),
                                                            this->GetParentSkill2(lpObj->Class, iMLSkill));
...
int CMasterLevelSkillTreeSystem::gObjMagicAdd_MLS(OBJECTSTRUCT *lpObj, int iMLSkill, int iSkillLevel) {
    ...
    if (MagicDamageC.SkillGetRequireClass(lpObj->Class, lpObj->m_PlayerData->ChangeUP, iMLSkill) < 1 &&
    ...
    for (int n = 0; n < MAX_MAGIC; n++) {
        if (lpObj->Magic[n].IsMagic() == TRUE && lpObj->Magic[n].m_Skill == iMLSkill &&
        ...
        if (lpObj->Magic[n].IsMagic() == FALSE) {
            if (lpObj->Magic[n].Set(iMLSkill, iSkillLevel) >= 0) {
                ...
                if (MagicDamageC.GetSkillUseType(iMLSkill) == 4 && !MagicDamageC.GetBrandOfSkill(iMLSkill)) {
                    GSProtocol.GCMagicListOneSend(lpObj->m_Index, n, lpObj->Magic[n].m_Skill, lpObj->Magic[n].m_Level,
                                                  0, 0);
                }
...
void CMasterLevelSkillTreeSystem::CGReqGetMasterLevelSkillTree(int aIndex) {
    ...
    for (int i = 0; i < MAX_MAGIC; i++) {
        if (lpObj->Magic[i].IsMagic() == TRUE && this->CheckMasterLevelSkill(lpObj->Magic[i].m_Skill) == true) {
            ...
            if (this->GetMasterSkillValue(MagicDamageC.SkillGet(lpObj->Magic[i].m_Skill), lpObj->Magic[i].m_Level) <=
                0.0) {
                pElementMsg.fMasterSkillCurValue = -1.0;
            } else {
                pElementMsg.fMasterSkillCurValue = this->GetMasterSkillValue(
                        MagicDamageC.SkillGet(lpObj->Magic[i].m_Skill), lpObj->Magic[i].m_Level);
            }

            if (this->GetMasterSkillValue(MagicDamageC.SkillGet(lpObj->Magic[i].m_Skill),
                                          lpObj->Magic[i].m_Level + 1) <= 0.0) {
                pElementMsg.fMasterSkillNextValue = -1.0;
            } else {
                pElementMsg.fMasterSkillNextValue = this->GetMasterSkillValue(
                        MagicDamageC.SkillGet(lpObj->Magic[i].m_Skill), lpObj->Magic[i].m_Level + 1);
            }
...
void CMasterLevelSkillTreeSystem::CGReqGetMasterLevelSkill(PMSG_REQ_MASTERLEVEL_SKILL *lpMsg, int aIndex) {
    ...
    if (iResult != TRUE) {
        g_Log.Add("[MasterSkill] [%s][%s] Fail - Result:%d, Skill:%d Level:%d MLPoint:%d",
                  lpObj->AccountID, lpObj->Name, iResult, lpMsg->iMasterLevelSkill, iSkillLevel,
                  lpObj->m_PlayerData->MasterPoint);
        pMsg.btResult = iResult;
    } else {
        if (this->gObjMagicAdd_MLS(lpObj, lpMsg->iMasterLevelSkill, iSkillLevel) < 0) {
            g_Log.Add("[MasterSkill] [%s][%s] Fail - Add Magic List, Skill:%d Level:%d", lpObj->AccountID, lpObj->Name,
            ...
            if (this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMsg->iMasterLevelSkill), iSkillLevel) <= 0.0) {
                ...
                pMsg.fMasterSkillCurValue = this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMsg->iMasterLevelSkill),
                                                                      iSkillLevel);
            }
            ...
            if (this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMsg->iMasterLevelSkill), iSkillLevel + 1) <= 0.0) {
                ...
                pMsg.fMasterSkillNextValue = this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMsg->iMasterLevelSkill),
                                                                       iSkillLevel + 1);
            }
            ...
            g_Log.Add("[MasterSkill] [%s][%s] Success - Add Magic List, Skill:%d Level:%d MLPoint:%d",
                      lpObj->AccountID, lpObj->Name, lpMsg->iMasterLevelSkill, iSkillLevel,
                      lpObj->m_PlayerData->MasterPoint);
        }
    }
...
BOOL
CMasterLevelSkillTreeSystem::RunningSkill_MLS(OBJECTSTRUCT *lpObj, int aTargetIndex, CMagicInf *lpMagic, BOOL bCombo,
                                              BYTE x, BYTE y, BYTE aTargetPos) {
    if (!lpObj || !lpMagic) {
        return FALSE;
    }
    ...
    switch (lpMagic->m_Skill) {
        ...
        case 560:
        case 561:
            this->MLS_SkillStrengthenDragonRoar(lpObj->m_Index, lpMagic, gObj[aTargetIndex].X, gObj[aTargetIndex].Y,
                                                aTargetIndex, 3, TRUE, 0);
            return 1;
        case 569:
...
void CMasterLevelSkillTreeSystem::MLS_WizardMagicDefense(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("WizardMagicDefense_Level1", "iiii>di", lpObj->m_Index, aTargetIndex,
                             lpObj->m_PlayerData->Dexterity + lpObj->AddDexterity,
                             lpObj->AddEnergy + lpObj->m_PlayerData->Energy, &fEffectValue, &iSkillTime);

    float fSkillValue = this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);

    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_MELEE_DEFENSE_INC, EFFECTTYPE_IMPROVE_MELEE_DEFENSE,
                      fEffectValue + fSkillValue, EFFECTTYPE_MELEE_DEFENSE_DOWN_MANA,
                      g_SkillAdditionInfo.GetSoulBarrierManaRate_Level(lpMagic->m_Level), iSkillTime);
...
void CMasterLevelSkillTreeSystem::MLS_Healing(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    if ((CC_MAP_RANGE(lpObj->MapNumber) || lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) &&
        lpObj->m_Index != lpTargetObj->m_Index) {
        return;
    }

    double fAddLife = 0.0;

    this->m_Lua.Generic_Call("ElfHeal", "iiii>d", lpTargetObj->Class, lpObj->m_Index, aTargetIndex,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &fAddLife);
            ...
            if (tObjNum >= 0) {
                ...
                if (lpObj->VpPlayer2[count].type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon < 0) {
                    ...
                    EnableAttack = TRUE;
                           ...
                           lpObj->VpPlayer2[count].type != OBJ_NPC &&
...
void
CMasterLevelSkillTreeSystem::MLS_SkillSword4_Magumsa(LPOBJ lpObj, CMagicInf *lpMagic, int aTargetIndex, BOOL isCombo) {
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenDesruction(LPOBJ lpObj, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                                int aTargetIndex, int isCombo) {
    int count = 0;
    int DuelIndex = lpObj->m_iDuelUser;
    int nHitCount = 0;
    BOOL bAttack = FALSE;
...
void CMasterLevelSkillTreeSystem::MLS_KnightSkillAddLife(LPOBJ lpObj, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("KnightSkillAddLife_Level1", "iii>di", lpObj->m_PlayerData->Vitality + lpObj->AddVitality,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, viewpercent, &faddLifepower, &iLifeTime);

    faddLifepower += fAddValue;

    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_HP_INC, EFFECTTYPE_HP, (lpObj->MaxLife * faddLifepower / 100.0), 0, 0,
                          iLifeTime);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpObj->m_Index, TRUE);
    } else {
        for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
            if (ApplyPartyIndex[i] != -1) {
                gObjAddBuffEffect(&gObj[ApplyPartyIndex[i]], BUFFTYPE_HP_INC, EFFECTTYPE_HP,
                                  (gObj[ApplyPartyIndex[i]].MaxLife * faddLifepower / 100.0), 0, 0, iLifeTime);
...
void CMasterLevelSkillTreeSystem::MLS_SkillFulKnightSkillAddLife(LPOBJ lpObj, CMagicInf *lpMagic) {
    ...
    if (iPreSkill <= 0) {
        this->m_Lua.Generic_Call("KnightSkillAddLife_Level2", "iii>di",
                                 lpObj->m_PlayerData->Vitality + lpObj->AddVitality,
                                 lpObj->m_PlayerData->Energy + lpObj->AddEnergy, viewpercent, &faddLifepower,
                                 &iLifeTime);
    } else {
    ...
    if (lpMagic->m_Skill == 360) {
        faddManapower =
                viewpercent + this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
    }

    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_HP_INC_STR, EFFECTTYPE_HP, (lpObj->MaxLife * faddLifepower / 100.0),
                          EFFECTTYPE_MANA, (lpObj->MaxMana * faddManapower / 100.0), iLifeTime);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpObj->m_Index, TRUE);
    } else {
        for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
            if (ApplyPartyIndex[i] != -1) {
                gObjAddBuffEffect(&gObj[ApplyPartyIndex[i]], BUFFTYPE_HP_INC_STR, EFFECTTYPE_HP,
                                  (gObj[ApplyPartyIndex[i]].MaxLife * faddLifepower / 100.0), EFFECTTYPE_MANA,
                                  (gObj[ApplyPartyIndex[i]].MaxMana * faddManapower / 100.0), iLifeTime);
                GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, ApplyPartyIndex[i], TRUE);
            }
        }
    }
...
void CMasterLevelSkillTreeSystem::MLS_MasteryKnightSkillAddLife(LPOBJ lpObj, CMagicInf *lpMagic) {
    ...
    if (iPreSkill <= 0) {
        ...
        if (iPrePreSkill <= 0) {
            this->m_Lua.Generic_Call("KnightSkillAddLife_Level3", "iii>di",
                                     lpObj->m_PlayerData->Vitality + lpObj->AddVitality,
                                     lpObj->m_PlayerData->Energy + lpObj->AddEnergy, viewpercent, &faddLifepower,
                                     &iLifeTime);
        } else {
    ...
    if (partynum == -1) {
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncMaxBP = this->GetMasterSkillValue(
        ...
        for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
            if (ApplyPartyIndex[i] != -1) {
                gObj[ApplyPartyIndex[i]].m_PlayerData->m_MPSkillOpt.iMpsIncMaxBP = this->GetMasterSkillValue(
                        MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
                gObjAddBuffEffect(&gObj[ApplyPartyIndex[i]], BUFFTYPE_HP_INC_STR, EFFECTTYPE_HP,
                                  (gObj[ApplyPartyIndex[i]].MaxLife * faddLifepower / 100.0), EFFECTTYPE_MANA,
                                  (gObj[ApplyPartyIndex[i]].MaxMana * faddManapower / 100.0), iLifeTime);
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenMagicPower(LPOBJ lpObj, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_MAGIC_POWER_INC_STR, EFFECTTYPE_MAGICPOWER_INC,
                      ((lpObj->m_PlayerData->Energy + lpObj->AddEnergy) / 9.0 * 0.20),
                      EFFECTTYPE_MAGICPOWERMAX_INC, (lpObj->m_MagicDamageMax / 100.0 *
                                                     this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill),
                                                                               lpMagic->m_Level)), 1800);
...
void CMasterLevelSkillTreeSystem::MLS_SkillSkillfulMagicPower(LPOBJ lpObj, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    lpObj->m_PlayerData->m_MPSkillOpt.iMpsCriticalRateInc = this->GetMasterSkillValue(
            MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
    gObjAddBuffEffect(lpObj, BUFFTYPE_MAGIC_POWER_INC_MAS, EFFECTTYPE_MAGICPOWER_INC,
                      ((lpObj->m_PlayerData->Energy + lpObj->AddEnergy) / 9.0 * 0.20),
                      EFFECTTYPE_MAGICPOWERMAX_INC,
                      (lpObj->m_MagicDamageMax / 100.0 * this->GetBrandOfMasterSkillValue(lpObj, lpMagic->m_Skill, 1)),
                      1800);
...
void
CMasterLevelSkillTreeSystem::MLS_SkillStrengthenThunder(OBJECTSTRUCT *lpObj, CMagicInf *lpMagic, int aTargetIndex) {
    if (lpObj->Type != OBJ_USER) {
        return;
    }
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenExPoison(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                              int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillWizardMagicDefense(LPOBJ lpObj, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (iPreSkill <= 0) {
        this->m_Lua.Generic_Call("WizardMagicDefense_Level2", "iiii>di", lpObj->m_Index, aTargetIndex,
                                 lpObj->AddDexterity + lpObj->m_PlayerData->Dexterity,
                                 lpObj->AddEnergy + lpObj->m_PlayerData->Energy, &fDefenseValue, &iSkillTime);
    } else {
        int iPreLevel = this->GetCurSkillLevel(lpObj, iPreSkill);
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_MELEE_DEFENSE_INC, EFFECTTYPE_IMPROVE_MELEE_DEFENSE, fDefenseValue,
                      EFFECTTYPE_MELEE_DEFENSE_DOWN_MANA,
                      g_SkillAdditionInfo.GetSoulBarrierManaRate_Level(lpMagic->m_Level),
                      iSkillTime +
                      this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level));

    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpTargetObj->m_Index, TRUE);
}

void
CMasterLevelSkillTreeSystem::MLS_SkillStrengthenPenetration(OBJECTSTRUCT *lpObj, CMagicInf *lpMagic, int aTargetIndex) {
    if (lpObj->Type != OBJ_USER) {
        return;
    }
...
void CMasterLevelSkillTreeSystem::MLS_SkillInfinityArrow(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    lpObj->m_SkillInfo.fInfinityArrowIncRate = this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill),
                                                                         lpMagic->m_Level);
    ...
    g_Log.Add("[%s][%s] Use Infinity Arrow Skill (Time:%d)(Character Level : %d)(ChangeUp: %d)",
              lpObj->AccountID, lpObj->Name, g_SkillAdditionInfo.GetInfinityArrowSkillTime(), lpObj->Level,
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenBerserker(int aIndex, CMagicInf *lpMagic) {
    ...
    if (lpObj->Type == OBJ_USER) {
        this->m_Lua.Generic_Call("SummonerBerserker_Level1", "i>iii", lpObj->m_PlayerData->Energy + lpObj->AddEnergy,
                                 &iEffectUPValue, &iEffectDownValue, &iDuration);
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncPercentBerserker1 = this->GetMasterSkillValue(
                MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
    } else {
        iDuration = 600000;
        iEffectUPValue = 0;
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_BERSERKER_STR, EFFECTTYPE_BERSERKER_UP, iEffectUPValue, EFFECTTYPE_BERSERKER_DOWN,
                      iEffectDownValue, iDuration);
...
void CMasterLevelSkillTreeSystem::MLS_SkillSkillfulBerserker(int aIndex, CMagicInf *lpMagic) {
    ...
    if (lpObj->Type == OBJ_USER) {
        this->m_Lua.Generic_Call("SummonerBerserker_Level2", "i>iii", lpObj->m_PlayerData->Energy + lpObj->AddEnergy,
                                 &iEffectUPValue, &iEffectDownValue, &iDuration);
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncPercentBerserker1 = this->GetBrandOfMasterSkillValue(lpObj,
                                                                                                      lpMagic->m_Skill,
                                                                                                      1);
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncPercentBerserker2 = this->GetMasterSkillValue(
                MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
    } else {
        iDuration = 600000;
        iEffectUPValue = 0;
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_BERSERKER_MAS, EFFECTTYPE_BERSERKER_UP, iEffectUPValue, EFFECTTYPE_BERSERKER_DOWN,
                      iEffectDownValue, iDuration);
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillMasteryBerserker(int aIndex, CMagicInf *lpMagic) {
    ...
    if (lpObj->Type == OBJ_USER) {
        this->m_Lua.Generic_Call("SummonerBerserker_Level3", "i>iii", lpObj->m_PlayerData->Energy + lpObj->AddEnergy,
                                 &iEffectUPValue, &iEffectDownValue, &iDuration);
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncPercentBerserker1 = this->GetBrandOfMasterSkillValue(lpObj,
                                                                                                      lpMagic->m_Skill,
                                                                                                      2);
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncPercentBerserker2 = this->GetBrandOfMasterSkillValue(lpObj,
                                                                                                      lpMagic->m_Skill,
                                                                                                      1);
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncValueBerserker3 = this->GetMasterSkillValue(
                MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
    } else {
        iDuration = 600000;
        iEffectUPValue = 0;
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_BERSERKER_PRO, EFFECTTYPE_BERSERKER_UP, iEffectUPValue, EFFECTTYPE_BERSERKER_DOWN,
                      iEffectDownValue, iDuration);
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrendthenAddCriticalDamage(int aIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("DarkLordCriticalDamage_Level1", "ii>ii", lpObj->Leadership + lpObj->AddLeadership,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &addcriticaldamagevalue, &SkillTime);

    addcriticaldamagevalue += this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);

    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_CRITICAL_DMG_INC, EFFECTTYPE_CRITICAL_DMG, addcriticaldamagevalue, 0, 0,
                          SkillTime);
        ...
        for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
            if (ApplyPartyIndex[n] != -1) {
                ...
                gObjAddBuffEffect(lpPartyObj, BUFFTYPE_CRITICAL_DMG_INC, EFFECTTYPE_CRITICAL_DMG,
                                  addcriticaldamagevalue, 0, 0, SkillTime);
...
void CMasterLevelSkillTreeSystem::MLS_SkillSkillFulAddCriticalDamage(int aIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("DarkLordCriticalDamage_Level2", "ii>ii", lpObj->Leadership + lpObj->AddLeadership,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &addcriticaldamagevalue, &SkillTime);
...
void CMasterLevelSkillTreeSystem::MLS_SkillMasteryAddCriticalDamage(int aIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("DarkLordCriticalDamage_Level3", "ii>ii", lpObj->Leadership + lpObj->AddLeadership,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &addcriticaldamagevalue, &SkillTime);
    ...
    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_CRITICAL_DMG_INC_STR, EFFECTTYPE_CRITICAL_DMG, addcriticaldamagevalue,
                          EFFECTTYPE_CRITICALDAMAGE, fRate, SkillTime);
        MsgOutput(aIndex, Lang.GetText(0, 134), SkillTime);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpObj->m_Index, TRUE);
    } else {
        for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
            if (ApplyPartyIndex[n] != -1) {
                ...
                gObjAddBuffEffect(lpPartyObj, BUFFTYPE_CRITICAL_DMG_INC_STR, EFFECTTYPE_CRITICAL_DMG,
                                  addcriticaldamagevalue, EFFECTTYPE_CRITICALDAMAGE, fRate, SkillTime);
                MsgOutput(ApplyPartyIndex[n], Lang.GetText(0, 134), SkillTime);
                GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpPartyObj->m_Index, TRUE);
            }
        }
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack == TRUE) {
                    if (gObjCalDistance(lpObj, &gObj[tObjNum]) < 3) {
                        gObjAddAttackProcMsgSendDelay(lpObj, 50, tObjNum, (rand() * 17) % 300 + 500, lpMagic->m_Skill,
                                                      0);
                    }
                }
            }
    ...
    while (true) {
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack == TRUE) {
                    if (SkillSpearHitBox.HitCheck(iangle, gObj[aIndex].X, gObj[aIndex].Y, gObj[tObjNum].X,
                                                  gObj[tObjNum].Y) == TRUE) {
...
void CMasterLevelSkillTreeSystem::MLS_SkillAreaMonsterAttack(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                             int aTargetIndex, int aDistance, BOOL bPVP,
                                                             int nSuccessRate, int nDelay) // function not used
{
    return;
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillBlowOfFury(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, int aTargetIndex,
                                                      BOOL isCombo) {
    LPOBJ lpObj = &gObj[aIndex];
    int count = 0;
    int DuelIndex = lpObj->m_iDuelUser;
...
void CMasterLevelSkillTreeSystem::MLS_SkillDefense(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("ElfDefense_Level1", "iiii>dd", lpTargetObj->Class, aIndex, aTargetIndex,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &skill_defense, &skill_time);

    skill_defense +=
            skill_defense * this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level) /
            100.0;
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_DEFENSE_POWER_INC, EFFECTTYPE_IMPROVE_DEFENSE, skill_defense, 0, 0,
                      skill_time);
...
void CMasterLevelSkillTreeSystem::MLS_SkillAttack(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("ElfAttack_Level1", "iiii>dd", lpTargetObj->Class, aIndex, aTargetIndex,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &skill_attack, &skill_time);

    skill_attack +=
            skill_attack * this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level) / 100.0;
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_ATTACK_POWER_INC, EFFECTTYPE_IMPROVE_DAMAGE, skill_attack, 0, 0,
                      skill_time);
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrenghtenAttack(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("ElfAttack_Level2", "iiii>dd", lpTargetObj->Class, aIndex, aTargetIndex,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &skill_attack, &skill_time);
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenDefense(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("ElfDefense_Level2", "iiii>dd", lpTargetObj->Class, aIndex, aTargetIndex,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &skill_defense, &skill_time);
...
void CMasterLevelSkillTreeSystem::MLS_PowerSlash(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE Targetangle,
                                                 int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillMonkBarrageJustOneTarget(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (ObjectMaxRange(aTargetIndex) == FALSE) {
        g_Log.Add("[InvalidTargetIndex][MLS_SkillMonkBarrageJustOneTarget] Index :%d , AccountID : %s", aIndex,
    ...
    if (EnableAttack == TRUE && this->MLS_CalDistance(lpObj->X, lpObj->Y, lpTargetObj->X, lpTargetObj->Y) < nDistance) {
        ...
        switch (lpMagic->m_Skill) {
            ...
            case 551:
                if (rand() % 100 <= 10)
                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_CURSED_ATTACK_DOWN, EFFECTTYPE_CURSED_ATTACKDOWN, 5, 0, 0,
                                      10);
                break;
            case 554:
                if (rand() % 100 <=
                    (this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level) + 10))
                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_CURSED_ATTACK_DOWN, EFFECTTYPE_CURSED_ATTACKDOWN, 5, 0, 0,
                                      10);
                break;
            case 552:
                if (rand() % 100 <= 10)
                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_DEFENSE_POWER_DEC, EFFECTTYPE_DECREASE_DEFENSE, 10, 0, 0,
                                      10);
                break;
            case 555:
                if (rand() % 100 <=
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenDragonRoar(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                                int aTargetIndex, int aDistance, BOOL bPVP,
                                                                int nDelay) {
    LPOBJ lpObj = &gObj[aIndex];
                            ...
                            if (bAttack == TRUE) {
                                if (nDelay) {
                                    ...
                                    if (lpMagic->m_Skill == 561) {
                                        if (this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill),
                                                                      lpMagic->m_Level) >= (rand() % 100)) {
                                            gObjAddBuffEffect(&gObj[tObjNum], BUFFTYPE_DEBUFF_FIRE,
                                                              EFFECTTYPE_GIVE_DMG_TICK, 100, 0, 0, 10);
                                        }
                                    }
                                }
                            }
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenBlockRate(int aIndex, CMagicInf *lpMagic) {
    ...
    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_MONK_INCREASE_DEFENSE_RATE_STR, EFFECTTYPE_IMPROVE_DEFENSE_RATE, nEffectValue,
                          0, 0, lpObj->m_PlayerData->Energy / 5 + 60);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
    } else {
        for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
            if (ApplyPartyIndex[i] != -1) {
                gObjAddBuffEffect(&gObj[ApplyPartyIndex[i]], BUFFTYPE_MONK_INCREASE_DEFENSE_RATE_STR,
                                  EFFECTTYPE_IMPROVE_DEFENSE_RATE, nEffectValue, 0, 0,
                                  lpObj->m_PlayerData->Energy / 5 + 60);
                GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, ApplyPartyIndex[i], TRUE);
            }
        }
    }
...
void CMasterLevelSkillTreeSystem::MLS_SkillMasteryBlockRate(int aIndex, CMagicInf *lpMagic) {
    ...
    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_MONK_INCREASE_DEFENSE_RATE_STR, EFFECTTYPE_IMPROVE_DEFENSE_RATE, nEffectValue,
                          EFFECTTYPE_IMPROVE_DEFENSE, fDefenseValue, lpObj->m_PlayerData->Energy / 5 + 60);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
    } else {
        for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
            if (ApplyPartyIndex[i] != -1) {
                gObjAddBuffEffect(&gObj[ApplyPartyIndex[i]], BUFFTYPE_MONK_INCREASE_DEFENSE_RATE_STR,
                                  EFFECTTYPE_IMPROVE_DEFENSE_RATE, nEffectValue, EFFECTTYPE_IMPROVE_DEFENSE,
                                  fDefenseValue, lpObj->m_PlayerData->Energy / 5 + 60);
                GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, ApplyPartyIndex[i], TRUE);
            }
        }
    }
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenIncVital(int aIndex, CMagicInf *lpMagic) {
    ...
    if (partynum == -1) {
        gObjAddBuffEffect(lpObj, BUFFTYPE_MONK_INCREASE_HEALTH_STR, EFFECTTYPE_MONK_VITALITY, nEffectValue, 0, 0,
                          lpObj->m_PlayerData->Energy / 5 + 60);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
        gObjCalCharacter.CalcCharacter(lpObj->m_Index);
    } else {
        for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
            if (ApplyPartyIndex[i] != -1) {
                gObjAddBuffEffect(&gObj[ApplyPartyIndex[i]], BUFFTYPE_MONK_INCREASE_HEALTH_STR,
                                  EFFECTTYPE_MONK_VITALITY, nEffectValue, 0, 0, lpObj->m_PlayerData->Energy / 5 + 60);
                GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, ApplyPartyIndex[i], TRUE);
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenIgnoreEnemyDefense(int aIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_MONK_IGNORE_ENEMY_DEFENSE, EFFECTTYPE_DECREASE_DEFENSE_RATE, nEffectValue,
                      EFFECTTYPE_MONK_DECREASE_DEFENSE,
                      this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level),
                      lpObj->m_PlayerData->Energy / 5 + 60);
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillBloodStorm(LPOBJ lpObj, CMagicInf *lpMagic, BYTE x, BYTE y, int aTargetIndex,
                                                      BOOL isCombo) {
    int DuelIndex = lpObj->m_iDuelUser;
    int nHitCount = 0;
    BOOL bAttack = FALSE;
...
void
CMasterLevelSkillTreeSystem::MLS_SkillPoisonArrow(LPOBJ lpObj, CMagicInf *lpMagic, int aTargetIndex, BOOL isCombo) {
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillMonsterCall(int aIndex, int MonsterType, int x, int y) {
    ...
    if (result >= 0) {
        ...
        if (gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonHp > 0.0) {
            ...
            gObj[result].MaxLife +=
                    gObj[result].MaxLife * gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonHp / 100.0;
        }

        if (gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonDefense > 0.0) {
            gObj[result].m_Defense +=
                    gObj[result].m_Defense * gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonDefense / 100.0;
        }

        if (gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonAttack > 0.0) {
            gObj[result].m_AttackDamageMin +=
                    gObj[result].m_AttackDamageMin * gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonAttack / 100.0;
            gObj[result].m_AttackDamageMax +=
                    gObj[result].m_AttackDamageMax * gObj[aIndex].m_PlayerData->m_MPSkillOpt.iMpsCallMonAttack / 100.0;
        }
...
void CMasterLevelSkillTreeSystem::MLS_SkillPartyHealing(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    if (partynum > -1 && partynum != lpTargetObj->PartyNumber) {
        g_Log.Add("[party healing] mismatch party number. target : %d, (%d - %d)", aTargetIndex, lpObj->PartyNumber,
                  lpTargetObj->PartyNumber);
        return;
    }
    ...
    for (int n = 0; n < count; n++) {
        ...
        if (lpMagic->m_Skill == 429) {
            nApplyHP +=
                    nApplyHP * this->GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level) /
                    100.0;
        }
...
void CMasterLevelSkillTreeSystem::MLS_SkillBless(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_BLESS, EFFECTTYPE_ELF_BLESS, iEffectValue, 0, 0,
                      MagicDamageC.GetSkillKeepTime(lpMagic->m_Skill));
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenBless(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_BLESS, EFFECTTYPE_ELF_BLESS, fValue, 0, 0,
                      MagicDamageC.GetSkillKeepTime(lpMagic->m_Skill));
...
void CMasterLevelSkillTreeSystem::MLS_SkillBlind(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    if (rand() % 100 <= ((lpObj->m_PlayerData->Energy + lpObj->AddEnergy) / 100 + 5)) {
        gObjAddBuffEffect(lpTargetObj, BUFFTYPE_BLIND_2, EFFECTTYPE_BLIND, 50, 0, 0,
                          MagicDamageC.GetSkillKeepTime(lpMagic->m_Skill));
...
void CMasterLevelSkillTreeSystem::MLS_SkillIronDefense(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE, EFFECTTYPE_HP, fValue, EFFECTTYPE_IMPROVE_DEFENSE, fValue,
                      MagicDamageC.GetSkillKeepTime(lpMagic->m_Skill));
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aTargetIndex, TRUE);
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenIronDefense(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE_STR, EFFECTTYPE_HP, fPrevValue, EFFECTTYPE_IMPROVE_DEFENSE,
                      fPrevValue, MagicDamageC.GetSkillKeepTime(lpMagic->m_Skill));
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aTargetIndex, TRUE);
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenBloodHowling(LPOBJ lpObj, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_BLOOD_HOWL_STR, 0, 0, 0, 0,
                      MagicDamageC.GetSkillKeepTime(lpMagic->m_Skill));
    lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsIncDamageBloodHowling = this->GetMasterSkillValue(
            MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aTargetIndex, TRUE);
}

void
CMasterLevelSkillTreeSystem::MLS_SkillfulKnightBlow(int aIndex, int aTargetIndex, CMagicInf *lpMagic, BOOL isCombo) {
    LPOBJ lpObj = &gObj[aIndex];
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
                    ...
                    if (attackcheck == TRUE) {
                        if (SkillTestFrustrum(gObj[tObjNum].X, gObj[tObjNum].Y, aIndex)) {
                            ...
                            if (fValue > (rand() % 100)) {
                                ...
                                gObjAddBuffEffect(&gObj[tObjNum], BUFFTYPE_BLEEDING, EFFECTTYPE_GIVE_DMG_TICK,
                                                  (lpObj->m_PlayerData->Strength + lpObj->AddStrength) / 10, 0, 0, 10);
                            }
                        }
                    }
                }
...
void
CMasterLevelSkillTreeSystem::MLS_MasteryKnightBlow(int aIndex, int aTargetIndex, CMagicInf *lpMagic, BOOL isCombo) {
    LPOBJ lpObj = &gObj[aIndex];
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillSkillfulDesruction(OBJECTSTRUCT *lpObj, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                              int aTargetIndex, BOOL isCombo) {
    int count = 0;
    int DuelIndex = lpObj->m_iDuelUser;
    int nHitCount = 0;
    BOOL bAttack = FALSE;
...
void CMasterLevelSkillTreeSystem::MLS_SkillMasteryDesruction(OBJECTSTRUCT *lpObj, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                             int aTargetIndex, BOOL isCombo) {
    int count = 0;
    int DuelIndex = lpObj->m_iDuelUser;
    int nHitCount = 0;
                            ...
                            if (bAttack == TRUE) {
                                ...
                                if (fValue >= (rand() % 100)) {
                                    if (!gObjCheckUsedBuffEffect(&gObj[tObjNum], BUFFTYPE_ATTACK_SPEED_DEC)) {
                                        gObjAddBuffEffect(&gObj[tObjNum], BUFFTYPE_ATTACK_SPEED_DEC,
                                                          EFFECTTYPE_REDUCE_ATTACK_SPEED, 0, 0, 0, 5);
                                    }
                                }
                            }
                        }
...
void
CMasterLevelSkillTreeSystem::MLS_MasterySuddenIce(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenMultiShot(int aIndex, CMagicInf *lpMagic, BYTE bAngle,
                                                               int aTargetIndex) {
        ...
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack) {
                    int nResult[5];
                    for (int i = 0; i < 5; i++) {
                        nResult[i] = (((int) vSkillLineX[i] - lpObj->X) * (gObj[tObjNum].Y - lpObj->Y)) -
                                     (((int) vSkillLineY[i] - lpObj->Y) * (gObj[tObjNum].X - lpObj->X));
                        if (SkillTestFrustrum(gObj[tObjNum].X, gObj[tObjNum].Y, aIndex)) {
...
void
CMasterLevelSkillTreeSystem::MLS_SkillMasteryWizardMagicDefense(LPOBJ lpObj, CMagicInf *lpMagic, int aTargetIndex) {
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
    ...
    this->m_Lua.Generic_Call("WizardMagicDefense_Level3", "iiii>dd", lpObj->m_Index, aTargetIndex,
                             lpObj->AddDexterity + lpObj->m_PlayerData->Dexterity,
                             lpObj->AddEnergy + lpObj->m_PlayerData->Energy, &fDefenseValue, &fSkillValidTime);
    ...
    lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsMasManaRate_Wizard = this->GetMasterSkillValue(
            MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);

    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_MELEE_DEFENSE_INC_STR, EFFECTTYPE_IMPROVE_MELEE_DEFENSE, fDefenseValue,
                      EFFECTTYPE_MELEE_DEFENSE_DOWN_MANA,
                      g_SkillAdditionInfo.GetSoulBarrierManaRate_Level(lpMagic->m_Skill), fSkillValidTime);
    GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aTargetIndex, TRUE);
}

void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenWeakness(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                              int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenEnervation(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                                int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenFlameStrike(int aIndex, CMagicInf *lpMagic, BYTE bAngle,
                                                                 int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
void
CMasterLevelSkillTreeSystem::MLS_SkillGiganticStorm(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
    int count = 0;
    int DuelIndex = lpObj->m_iDuelUser;
    int nHitCount = 0;
...
void CMasterLevelSkillTreeSystem::MLS_SkillMasteryDefenseDown(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    if (retCalcSkillResistance(lpTargetObj, 1)) {
        gObjAddBuffEffect(lpTargetObj, BUFFTYPE_DECREASE_DEFENSE_RATE, EFFECTTYPE_DECREASE_DEFENSE, fDownValue, 0, 0,
                          10);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aTargetIndex, TRUE);
    }
}
...
void CMasterLevelSkillTreeSystem::MLS_SkillMasteryAddCriticalDamage2(int aIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("DarkLordCriticalDamage_Level4", "ii>ii", lpObj->Leadership + lpObj->AddLeadership,
                             lpObj->m_PlayerData->Energy + lpObj->AddEnergy, &addcriticaldamagevalue, &SkillTime);
    ...
    if (partynum == -1) {
        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncExcellentDamageRate_Darklord = this->GetMasterSkillValue(
                MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
        gObjAddBuffEffect(lpObj, BUFFTYPE_CRITICAL_DMG_INC_MAS, EFFECTTYPE_CRITICAL_DMG, addcriticaldamagevalue,
                          EFFECTTYPE_CRITICALDAMAGE, fRate, SkillTime);
        MsgOutput(aIndex, Lang.GetText(0, 134), SkillTime);
        GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpObj->m_Index, TRUE);
    } else {
        for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
            if (ApplyPartyIndex[n] != -1) {
                lpPartyObj = &gObj[ApplyPartyIndex[n]];
                lpPartyObj->m_PlayerData->m_MPSkillOpt.iMpsIncExcellentDamageRate_Darklord = this->GetMasterSkillValue(
                        MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
                gObjAddBuffEffect(lpPartyObj, BUFFTYPE_CRITICAL_DMG_INC_MAS, EFFECTTYPE_CRITICAL_DMG,
                                  addcriticaldamagevalue, EFFECTTYPE_CRITICALDAMAGE, fRate, SkillTime);
                MsgOutput(ApplyPartyIndex[n], Lang.GetText(0, 134), SkillTime);
                GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, lpPartyObj->m_Index, TRUE);
            }
...
void
CMasterLevelSkillTreeSystem::MLS_SkillFrustrum3(int aIndex, BYTE bAngle, float tx1, float ty1, float tx2, float ty2) {
    LPOBJECTSTRUCT lpObj = (LPOBJECTSTRUCT) &gObj[aIndex];
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenElectricSpark(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y,
                                                                   BYTE TargetPos, int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
    ...
    while (true) {
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack != 0) {
                    if (SkillElectricSparkHitBox.HitCheck(iangle, gObj[aIndex].X, gObj[aIndex].Y, gObj[tObjNum].X,
                                                          gObj[tObjNum].Y)) {
                        delaytime = 500;
                        Hit = 1;
...
void CMasterLevelSkillTreeSystem::MLS_SkillStrengthenchaoticDesair(int aIndex, CMagicInf *lpMagic, BYTE bAngle,
                                                                   int aTargetIndex) {
    LPOBJECTSTRUCT lpObj = (LPOBJECTSTRUCT) &gObj[aIndex];
            ...
            if (this->CheckMasterLevelSkill(lpObj->Magic[iCount].m_Skill)) {
                ...
                if (iBaseMLS > 0) {
                    ...
                    if (aSkill == BrandSkill) {
                        if (MagicDamageC.IsItemAttachedSkill(BrandSkill) &&
                            this->IsExistMLSBasedSkill(lpObj, iBaseMLS) == false) {
                            GSProtocol.GCMagicListOneDelSend(lpObj->m_Index, iCount, lpObj->Magic[iCount].m_Skill,
                                                             lpObj->Magic[iCount].m_Level, 0, 0);
                            return true;
                        }
                    }
                }
            ...
            if (this->CheckMasterLevelSkill(lpObj->Magic[iCount].m_Skill)) {
                ...
                if (iBaseMLS > 0) {
                    if (aSkill == MagicDamageC.GetNormalBrandOfSkill(iBaseMLS) &&
                        ...
                        GSProtocol.GCMagicListOneSend(lpObj->m_Index, iCount, lpObj->Magic[iCount].m_Skill,
                                                      lpObj->Magic[iCount].m_Level, 0, 0);
                        return true;
                    }
                }
    ...
    while (true) {
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (lpObj->VpPlayer2[count].type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon < 0 &&
                    ((gObj[tObjNum].Class < 100 || gObj[tObjNum].Class > 110) || gObj[tObjNum].Class != 689)) {
                    EnableAttack = TRUE;
                } else if (tObjNum == aTargetIndex || DuelIndex == tObjNum) {
                    EnableAttack = TRUE;
                } else {
...
void CMasterLevelSkillTreeSystem::MLS_SkillLightningShock(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    for (int n = 0; n < MAX_VIEWPORT; n++) {
        ...
        if (CC_MAP_RANGE(lpObj->MapNumber) || IT_MAP_RANGE(lpObj->MapNumber) ||
            lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
            EnableAttack = TRUE;
    ...
    for (int iPos = 0; iPos < MAX_SKILL_POS; iPos++) {
        for (int i = 0; i < MAX_MAGIC; i++) {
            if (lpObj->Magic[i].IsMagic() == TRUE &&
                ...
                if (lpObj->Magic[i].m_Skill ==
                    this->m_MLSTable[iClass].iAbility[nTreeKind].iElement[nRank][iPos].m_dwSkillID) {
        ...
        case ITEM_TWOHAND_SWORD: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 348) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsTwoHandSwordAddDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 352) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsTwoHandsPvPAttack = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_ONEHAND_SWORD: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 349) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsSwordAddDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 353) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddAttackSpeed = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_MACE: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 350) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsScultAddDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 354) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddSturn = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_SPEAR: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 351) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsSpearAddDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 355) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddDoubleDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_ONEHAND_STAFF: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 397) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsStaffMagicPower = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 400) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsStaffAttackSpeed = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_TWOHAND_STAFF: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 398) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsTwoHandsMagicPower = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 401) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsTwoHandsStaffManAttack = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE) {
                    if (lpObj->Magic[iCount].m_Skill == 399) {
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncShield = this->GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                        break;
                    }

                    if (lpObj->Magic[iCount].m_Skill == 437) {
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsElfAddShield = this->GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                        break;
                    }
                    ...
                    if (lpObj->Magic[iCount].m_Skill == 528) {
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddShieldDefense = this->GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                        break;
                    }
                }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE) {
                    if (lpObj->Magic[iCount].m_Skill == 402) {
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsShieldBlockRate = this->GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                        return;
                    }

                    if (lpObj->Magic[iCount].m_Skill == 440) {
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsElfShieldBlockRate = this->GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                        return;
                    }
                    ...
                    if (lpObj->Magic[iCount].m_Skill == 532) {
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddShieldBlockingRage = this->GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                        return;
                    }
                }
        ...
        case ITEM_BOW: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 435) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsBowAddDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 438) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsBowAddAttackSpeed = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_CROSSBOW: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 436) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsTwoHandsBowAddDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 439) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsTwoHandsBowManAttack = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_SUMMONER_STAFF: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 465) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddMagicPowerStick = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 467) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddManAttackStick = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_SUMMONER_BOOK: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 466) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddCurseMagicBook = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 468) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddAttackSpeedMagicBook = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_DARKLORD_SCEPTER: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 527) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddCeptorDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 529) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddPetAttack = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 531) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncCeptorManAttack = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 533) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddControlAttack = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
        ...
        case ITEM_RAGEFIGHTER_WEAPON: {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 568) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsMonkWeaponDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    break;
                }
            }
            ...
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 571) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsMonkDoubleDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }
    ...
    if (iType == ITEMGET(13, 4)) {
        for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
            if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 510) {
                lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncDarkHorseDefense = this->GetMasterSkillValue(
                        MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                return;
            }
        }
        ...
        case ITEMGET(12, 36): {
            for (int iCount = 0; iCount < MAX_MAGIC; iCount++) {
                if (lpObj->Magic[iCount].IsMagic() == TRUE && lpObj->Magic[iCount].m_Skill == 324) {
                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddWingDamage = this->GetMasterSkillValue(
                            MagicDamageC.SkillGet(lpObj->Magic[iCount].m_Skill), lpObj->Magic[iCount].m_Level);
                    return;
                }
            }

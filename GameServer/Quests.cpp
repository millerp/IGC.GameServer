...
void CQuests::CGReqQuestList(int aIndex) {
    ...
    for (int i = 0; i < 50; i++) {
        if (lpObj->m_PlayerData->m_Quests[i].QuestIndex != 0xFFFF && lpObj->m_PlayerData->m_Quests[i].QuestIndex > 0 &&
            lpObj->m_PlayerData->m_Quests[i].QuestIndex < 21) {
            if (this->CheckNPC(lpObj,
                               this->m_Quests[lpObj->m_PlayerData->m_Quests[i].QuestIndex][lpObj->m_PlayerData->m_Quests[i].QuestState].wNPCID)) {
                lpQuest = (PMSG_QUESTLISTSENDCHAR *) (cBUFFER + sizeof(PBMSG_COUNT_CHAR) +
                                                      (lpMsg->count * sizeof(PMSG_QUESTLISTSENDCHAR)));
    ...
    g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest][NPC] (%s)(%s) Sent Quest List (%d)", lpObj->AccountID, lpObj->Name,
              lpMsg->count);
}
...
void CQuests::CGReqAnswer(int aIndex, PMSG_REQ_QUESTINFO *aRecv) {
    ...
    if (lpQuest == NULL) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red, "[Quest] ERROR: Unknown quest (%s)(%s) (%d)(%d)",
    ...
    if (lpQuest->btDIALOG == 4) {
        ...
        if (ArrayNum == -1) {
            ...
            if (bResult == false) {
                g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] ERROR : AddQuest() == false (%s)(%s)", lpObj->AccountID,
                          lpObj->Name);
                return;
            }
        }
        ...
        if (lpQuest->btDIALOG == 1) {
            ...
            if (ArrayNum == -1) {
                ...
                if (bResult == false) {
                    g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] ERROR : AddQuest() == false (%s)(%s)",
                              lpObj->AccountID, lpObj->Name);
                    return;
                }
            }
        ...
        g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] (%s)(%s) Started Dialog with NPC (%d/%d)", lpObj->AccountID,
                  lpObj->Name, lpQuest->btINDEX, lpQuest->btSTATE);
    }
}
...
void CQuests::CGReqDialog(int aIndex, PMSG_REQ_QUESTANSWER *aRecv) {
    ...
    if (lpNewQuest == NULL) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                   "[Quest] ERROR: Unknown quest continue data (%s)(%s) (%d)(%d)", lpObj->AccountID, lpObj->Name,
                   aRecv->QuestIndex, aRecv->QuestState);
        return;
    }
    ...
    if (bRes == false) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red, "[Quest] ERROR: Quest not exist in PlayerData (%s)(%s)",
                   lpObj->AccountID, lpObj->Name);
        return;
    }
    ...
    if (bReqRew == false) {
        ...
        IOCP.DataSend(aIndex, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] (%s)(%s) Continue Dialog with NPC (%d/%d)", lpObj->AccountID,
                  lpObj->Name, lpQuest->btINDEX, lpQuest->btSTATE);
    } else {
...
void CQuests::GCAnsQuestSpecificInfo(LPOBJ lpObj, QUEST_INTERNAL_DATA *lpQuest) {
    ...
    for (int i = 0; i < 5; i++) {
        if (lpQuest->wREWARD[i].RewardType != 0) {
            ...
            if (pMsg.Prizes[i].RewardType == 4) // Item
            {
                ...
                Item.m_Durability = ItemGetDurability(Item.m_Type, Item.m_Level, lpQuest->wREWARD[i].RewardExc,
                                                      lpQuest->wREWARD[i].RewardSet);
                Item.Convert(lpQuest->wREWARD[i].RewardID, lpQuest->wREWARD[i].RewardSkill,
                             lpQuest->wREWARD[i].RewardLuck,
                             lpQuest->wREWARD[i].RewardOption, lpQuest->wREWARD[i].RewardExc,
                             lpQuest->wREWARD[i].RewardSet, 0, -1, Item.m_SocketOption, 3,
                             g_AttributeSystem.GetElement(Item.m_Type), 0);
    ...
    g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] (%s)(%s) Sent Specific Info about Quest (%d/%d) (Req:%d/Rew:%d)",
              lpObj->AccountID, lpObj->Name, lpQuest->btINDEX, lpQuest->btSTATE, pMsg.NeedsCount, pMsg.PrizeCount);
}
...
QUEST_INTERNAL_DATA *CQuests::FindDailyQuest(LPOBJ lpObj, BYTE btDay, BYTE btReqLevel) {
    for (int i = 0; i < 30; i++) {
        if (this->m_DailyQuest[i].btDAY == btDay && this->m_DailyQuest[i].btREQUESTLEVEL == btReqLevel) {
            QUEST_INTERNAL_DATA *lpQuest = this->GetQuestData(this->m_DailyQuest[i].btSTATE,
                                                              this->m_DailyQuest[i].btINDEX);
...
void CQuests::CGReqQuestListLogin(int aIndex) {
    ...
    for (int i = 0; i < 50; i++) {
        if (lpObj->m_PlayerData->m_Quests[i].QuestIndex != 0xFFFF) {
            lpQuest = (PMSG_QUESTLISTSENDCHAR *) (cBUFFER + sizeof(PBMSG_QUESTLIST_CHAR) +
                                                  (lpMsg->count * sizeof(PMSG_QUESTLISTSENDCHAR)));
    ...
    g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest][Login] (%s)(%s) Sent Quest List (%d)", lpObj->AccountID, lpObj->Name,
              lpMsg->count);
...
void CQuests::KillMonsterProc(LPOBJ lpMonsterObj) {
    ...
    for (int i = 0; i < 50; i++) {
        if (lpObj->PartyNumber == -1) {
            if (lpObj->m_PlayerData->m_Quests[i].QuestIndex != 0xFFFF) {
                lpQuest = this->GetQuestData(lpObj->m_PlayerData->m_Quests[i].QuestState,
                                             lpObj->m_PlayerData->m_Quests[i].QuestIndex);

                if (lpQuest != NULL) {
                    for (int k = 0; k < 5; k++) {
                        if (lpQuest->wREQUEST[k].RequirementType == 1) {
                            if (lpQuest->wREQUEST[k].RequirementID == lpMonsterObj->Class) {
                                if (lpQuest->wREQUEST[k].RequiredCount >
                                    lpObj->m_PlayerData->m_Quests[i].ReqKillCount[k]) {
                                    lpObj->m_PlayerData->m_Quests[i].ReqKillCount[k]++;
                                    g_Log.Add(0, __FILE__, __FUNCTION__,
                                              "[Quest][Single] (%s)(%s) Added KillCount: CLASS:%d (%d/%d)",
                                              lpObj->AccountID, lpObj->Name, lpMonsterObj->Class,
            ...
            for (int m = 0; m < 5; m++) {
                ...
                if (pindex != -1 && gObjIsConnected(pindex)) {
                    ...
                    if (lpPartyObj->m_PlayerData->m_Quests[i].QuestIndex != 0xFFFF) {
                        lpQuest = this->GetQuestData(lpPartyObj->m_PlayerData->m_Quests[i].QuestState,
                                                     lpPartyObj->m_PlayerData->m_Quests[i].QuestIndex);
                                ...
                                if (lpQuest->wREQUEST[k].RequirementType == 1) {
                                    if (lpQuest->wREQUEST[k].RequirementID == lpMonsterObj->Class) {
                                        if (lpQuest->wREQUEST[k].RequiredCount >
                                            ...
                                            lpPartyObj->m_PlayerData->m_Quests[i].ReqKillCount[k]++;
                                            g_Log.Add(0, __FILE__, __FUNCTION__,
                                                      "[Quest][Party] (%s)(%s) Added KillCount: CLASS:%d (%d/%d)",
                                                      lpPartyObj->AccountID, lpPartyObj->Name, lpMonsterObj->Class,
                                                      lpQuest->btINDEX, lpQuest->btSTATE);
                                        }
                                    }
                                }
...
void CQuests::CGReqQuestFinish(int aIndex, PMSG_REQ_QUESTEND *aRecv) {
    ...
    if (pMsg.btResult == TRUE) {
        ...
        if (lpQuest->btINDEX == 1 && lpQuest->btSTATE == 2) {
            g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest Tutorial] (%s)(%s) Started Tutorial (%d/%d)", lpObj->AccountID,
                      lpObj->Name, lpQuest->btINDEX, lpQuest->btSTATE);
            lpObj->m_PlayerData->Tutorial = TUTORIAL_ONGOING;
        } else if (lpQuest->btINDEX == 1 && (lpQuest->btSTATE == 90 || lpQuest->btSTATE == 4)) {
            g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest Tutorial] (%s)(%s) Finished Tutorial (%d/%d)",
                      lpObj->AccountID, lpObj->Name, lpQuest->btINDEX, lpQuest->btSTATE);
            lpObj->m_PlayerData->Tutorial = TUTORIAL_FINISHED;
        }

        if (lpQuest->btSTAGE == 0xFE) {
            lpObj->m_PlayerData->m_Quest[24 + lpQuest->btINDEX] = lpQuest->btDIALOGCLASS[lpObj->Class];
        } else if (lpQuest->btINDEX >= 2 && lpQuest->btINDEX <= 5 || lpQuest->btINDEX == 8 || lpQuest->btINDEX == 9 ||
                   lpQuest->btINDEX == 10 || lpQuest->btINDEX == 11 || lpQuest->btINDEX == 12 ||
                   lpQuest->btINDEX == 20) {
    ...
    g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] (%s)(%s) Finish Quest (%d/%d) Result: %s", lpObj->AccountID,
              lpObj->Name, lpQuest->btINDEX, lpQuest->btSTATE, pMsg.btResult == TRUE ? "OK" : "FAIL");
...
bool CQuests::CheckRequirements(LPOBJ lpObj, QUEST_INTERNAL_DATA *lpQuest) {
    ...
    for (int i = 0; i < 50; i++) {
        if (lpObj->m_PlayerData->m_Quests[i].QuestIndex == lpQuest->btINDEX &&
            lpObj->m_PlayerData->m_Quests[i].QuestState == lpQuest->btSTATE) {
            for (int k = 0; k < 5; k++) {
                if (lpQuest->wREQUEST[k].RequirementType == 1) {
                    ...
                    if (this->GetItemCount(lpObj, lpQuest->wREQUEST[k].RequirementID) >=
                        lpQuest->wREQUEST[k].RequiredCount) {
                        checks[k] = true;
                    }
...
void CQuests::CGReqQuestDelete(int aIndex, PMSG_QUESTDELETE *aRecv) {
    ...
    if (this->DelQuest(lpObj, aRecv->QuestIndex, aRecv->QuestState) == true) {
        ...
        g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] (%s)(%s) Quest Delete - (%d/%d)", lpObj->AccountID, lpObj->Name,
                  lpQuest->btINDEX, lpQuest->btSTATE);
    }
}
...
bool CQuests::CreateItem(LPOBJ lpObj, QUEST_INTERNAL_DATA *lpQuest, int RewardNum) {
    ...
    for (int i = 0; i < lpQuest->wREWARD[RewardNum].RewardCount; i++) {
        ...
        ItemSerialCreateSend(lpObj->m_Index, 235, lpObj->X, lpObj->Y, lpReward->RewardID, lpReward->RewardLevel, 0,
                             lpReward->RewardSkill, lpReward->RewardLuck, lpReward->RewardOption, lpObj->m_Index,
                             lpReward->RewardExc, lpReward->RewardSet, g_AttributeSystem.GetElement(lpReward->RewardID),
                             -1);
    }
...
void CQuests::CGReqShadowPhantomBuff(int aIndex) {
    ...
    if (lpObj->Level > g_iShadowPhantomMaxLevel) {
        ...
        gObjAddBuffEffect(lpObj, BUFFTYPE_ATTDEF_POWER_INC, EFFECTTYPE_IMPROVE_DAMAGE, (lpObj->Level / 5 + 50),
                          EFFECTTYPE_IMPROVE_DEFENSE, (lpObj->Level / 3 + 45), 1800);
        g_Log.Add(0, __FILE__, __FUNCTION__, "[Quest] (%s)(%s) Received Shadow Phantom Buff (Level:%d)",
                  lpObj->AccountID, lpObj->Name, lpObj->Level);
    }
}
                        ...
                        if (lpQuest->wREQUEST[k].RequirementType == 262) {
                            if (g_GensSystem.GetGensInfluence(lpObj) != g_GensSystem.GetGensInfluence(lpTargetObj)) {
                                if (lpQuest->wREQUEST[k].RequiredCount >
                                    ...
                                    lpObj->m_PlayerData->m_Quests[i].ReqKillCount[k]++;
                                    g_Log.Add(0, __FILE__, __FUNCTION__,
                                              "[Quest][Single] (%s)(%s) Added KillCount: (GENS) (%d/%d)",
                                              lpObj->AccountID, lpObj->Name,
                                              lpQuest->btINDEX, lpQuest->btSTATE);
                                }
                            }
                            ...
                            for (int k = 0; k < 5; k++) {
                                if (lpQuest->wREQUEST[k].RequirementType == 262) {
                                    if (g_GensSystem.GetGensInfluence(lpObj) !=
                                        g_GensSystem.GetGensInfluence(lpTargetObj)) {
                                        if (lpQuest->wREQUEST[k].RequiredCount >
                                            lpPartyObj->m_PlayerData->m_Quests[i].ReqKillCount[k]) {
                                            lpPartyObj->m_PlayerData->m_Quests[i].ReqKillCount[k]++;
                                            g_Log.Add(0, __FILE__, __FUNCTION__,
                                                      "[Quest][Party] (%s)(%s) Added KillCount: (GENS) (%d/%d)",
                                                      lpPartyObj->AccountID, lpPartyObj->Name,
                                                      lpQuest->btINDEX, lpQuest->btSTATE);
                                        }
                                    }
                                }
...
void CQuests::SetEventQuest(LPOBJ lpObj) {
    ...
    g_Log.Add(0, __FILE__, __FUNCTION__,
              "[Illusion Temple] [Quest] (%s)(%s) SetQuest: (C:%d, A:%d, S1:%d, S2:%d, S3:%d, S4:%d)",
              lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->m_Quest[50], lpObj->m_PlayerData->m_Quest[51],
              lpObj->m_PlayerData->m_Quest[52], lpObj->m_PlayerData->m_Quest[53], lpObj->m_PlayerData->m_Quest[54],
              lpObj->m_PlayerData->m_Quest[55]);

    g_Log.Add(0, __FILE__, __FUNCTION__,
              "[Blood Castle] [Quest] (%s)(%s) SetQuest: (C:%d, A:%d, P1:%d, P2:%d, P3:%d, P4:%d, P5:%d, P6:%d)",
              lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->m_Quest[56], lpObj->m_PlayerData->m_Quest[57],
              lpObj->m_PlayerData->m_Quest[58], lpObj->m_PlayerData->m_Quest[59], lpObj->m_PlayerData->m_Quest[60],
              lpObj->m_PlayerData->m_Quest[61], lpObj->m_PlayerData->m_Quest[62], lpObj->m_PlayerData->m_Quest[63]);

    g_Log.Add(0, __FILE__, __FUNCTION__,
              "[Chaos Castle] [Quest] (%s)(%s) SetQuest: (C:%d, A:%d, PV1:%d, PV2:%d, PV3:%d, PV4:%d, PV5:%d)",
              lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->m_Quest[64], lpObj->m_PlayerData->m_Quest[65],
              lpObj->m_PlayerData->m_Quest[66], lpObj->m_PlayerData->m_Quest[67], lpObj->m_PlayerData->m_Quest[68],
              lpObj->m_PlayerData->m_Quest[69], lpObj->m_PlayerData->m_Quest[70]);

    g_Log.Add(0, __FILE__, __FUNCTION__,
              "[Devil Square] [Quest] (%s)(%s) SetQuest: (C:%d, A:%d, FT1:%d, FT2:%d, FT3:%d, FT4:%d, FT5:%d)",
              lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->m_Quest[71], lpObj->m_PlayerData->m_Quest[72],
              lpObj->m_PlayerData->m_Quest[73], lpObj->m_PlayerData->m_Quest[74], lpObj->m_PlayerData->m_Quest[75],
              lpObj->m_PlayerData->m_Quest[76], lpObj->m_PlayerData->m_Quest[77]);
...
bool CQuests::CheckStage(LPOBJ lpObj, WORD Index, WORD Stage) {
    ...
    } else if (this->m_Quests[Index][Stage].btINDEX == 2) // Chaos Castle
    {
        ...
        if (lpObj->m_PlayerData->m_Quest[64 + this->m_Quests[Index][Stage].btEVENTLEVEL] ==
            this->m_Quests[Index][Stage].btREQSTAGE)
            return true;
    } else if (this->m_Quests[Index][Stage].btINDEX == 3) // Blood Castle
    {
        ...
        if (lpObj->m_PlayerData->m_Quest[56 + this->m_Quests[Index][Stage].btEVENTLEVEL] ==
            this->m_Quests[Index][Stage].btREQSTAGE)
            return true;
    } else if (this->m_Quests[Index][Stage].btINDEX == 4) // Devil Square
    {
        ...
        if (lpObj->m_PlayerData->m_Quest[71 + this->m_Quests[Index][Stage].btEVENTLEVEL] ==
            this->m_Quests[Index][Stage].btREQSTAGE)
            return true;
    } else if (this->m_Quests[Index][Stage].btINDEX == 5) // Illusion
    {
        ...
        if (lpObj->m_PlayerData->m_Quest[50 + this->m_Quests[Index][Stage].btEVENTLEVEL] ==
            this->m_Quests[Index][Stage].btREQSTAGE)
            return true;
    ...
    } else if ((this->m_Quests[Index][Stage].btREQSTAGE >= lpObj->m_PlayerData->m_Quest[24 + Index] &&
                this->m_Quests[Index][Stage].btREQSTAGE != 255) ||
               (this->m_Quests[Index][Stage].btREQSTAGE == 0xFF && lpObj->m_PlayerData->m_Quest[24 + Index] == 0xFF))
        return true;
...
void CQuests::FinishEventProc(int aIndex, BYTE btEventType, BYTE btEventLevel) {
    ...
    for (int k = 0; k < 50; k++) {
        ...
        QUEST_INTERNAL_DATA *lpQuest = this->GetQuestData(lpObj->m_PlayerData->m_Quests[k].QuestState,
                                                          lpObj->m_PlayerData->m_Quests[k].QuestIndex);
        ...
        for (int i = 0; i < 5; i++) {
            if (btEventType == 2) {
                if (lpQuest->wREQUEST[i].RequirementType == 257 && lpQuest->wREQUEST[i].RequirementID == btEventLevel) {
                    ...
                    g_Log.Add(0, __FILE__, __FUNCTION__,
                              "[Chaos Castle] (%d) [Quest] (%s)(%s) Event Cleared -> Quest State SUCCESS",
                ...
                if (lpQuest->wREQUEST[i].RequirementType == 256 && lpQuest->wREQUEST[i].RequirementID == btEventLevel) {
                    lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i] = 1;
                    g_Log.Add(0, __FILE__, __FUNCTION__,
                              "[Blood Castle] (%d) [Quest] (%s)(%s) Event Cleared -> Quest State SUCCESS",
                              btEventLevel, lpObj->AccountID, lpObj->Name);
                    break;
                }
                ...
                if (lpQuest->wREQUEST[i].RequirementType == 258 && lpQuest->wREQUEST[i].RequirementID == btEventLevel) {
                    lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i] = 1;
                    g_Log.Add(0, __FILE__, __FUNCTION__,
                              "[Devil Square] (%d) [Quest] (%s)(%s) Event Cleared -> Quest State SUCCESS",
                              btEventLevel, lpObj->AccountID, lpObj->Name);
                    break;
                }
                ...
                if (lpQuest->wREQUEST[i].RequirementType == 259 && lpQuest->wREQUEST[i].RequirementID == btEventLevel) {
                    lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i] = 1;
                    g_Log.Add(0, __FILE__, __FUNCTION__,
                              "[Illusion Temple] (%d) [Quest] (%s)(%s) Event Cleared -> Quest State SUCCESS",
                              btEventLevel, lpObj->AccountID, lpObj->Name);
                    break;
                }
            }
    ...
    for (int k = 0; k < 50; k++) {
        ...
        for (int i = 0; i < 5; i++) {
            if (lpQuest->wREQUEST[i].RequirementType == 66 && lpQuest->wREQUEST[i].RequirementID == btEventLevel &&
                ...
                g_Log.Add(0, __FILE__, __FUNCTION__,
                          "[Blood Castle] (%d) [Quest] (%s)(%s) Gate Destroyed -> Quest State SUCCESS",
                          btEventLevel, lpObj->AccountID, lpObj->Name);
                break;
            }
        }
    ...
    for (int k = 0; k < 50; k++) {
        ...
        for (int i = 0; i < 5; i++) {
                ...
                btType == 0) // Player
            {
                lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i]++;
                g_Log.Add(0, __FILE__, __FUNCTION__,
                          "[Chaos Castle] (%d) [Quest] (%s)(%s) Player Killed -> Increase Kill Count (%d)",
                          btEventLevel, lpObj->AccountID, lpObj->Name,
                          lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i]);
                break;
            } else if (lpQuest->wREQUEST[i].RequirementType == 65 &&
                       lpQuest->wREQUEST[i].RequirementID == btEventLevel &&
                       lpQuest->wREQUEST[i].RequiredCount >= lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i] &&
                       btType == 1) // Monster
            {
                lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i]++;
                g_Log.Add(0, __FILE__, __FUNCTION__,
                          "[Chaos Castle] (%d) [Quest] (%s)(%s) Monster Killed -> Increase Kill Count (%d)",
                          btEventLevel, lpObj->AccountID, lpObj->Name,
                          lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i]);
                break;
            }
    ...
    for (int k = 0; k < 50; k++) {
        ...
        for (int i = 0; i < 5; i++) {
            if (lpQuest->wREQUEST[i].RequirementType == 260 && lpQuest->wREQUEST[i].RequirementID == btEventLevel) {
                if ((lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i] + btPoint) > 255 ||
                    (lpObj->m_PlayerData->m_Quests[k].ReqKillCount[i] + btPoint) > lpQuest->wREQUEST[i].RequiredCount) {
...
bool CQuests::DropQuestItem(LPOBJ lpMonsterObj) {
    ...
    for (int i = 0; i < 30; i++) {
        ...
        if (gObjGetItemCountInIventory(MaxHitUser, this->m_QuestItems[i].wItemID) >=
            this->m_QuestItems[i].btItemCount) {
            continue;
        }

        if ((rand() % 10000) < this->m_QuestItems[i].dwRate) {
            ItemSerialCreateSend(lpMonsterObj->m_Index, lpMonsterObj->MapNumber, lpMonsterObj->X, lpMonsterObj->Y,
                                 this->m_QuestItems[i].wItemID, 0, 1, 0, 0, 0, MaxHitUser, 0, 0,
                                 g_AttributeSystem.GetElement(this->m_QuestItems[i].wItemID), -1);
            return true;
        } else {
            continue;
        }
    ...
    for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
        ...
        if (aIndex >= 0 && gObj[aIndex].MapNumber == lpMonsterObj->MapNumber) {
            for (int j = 0; j < 30; j++) {
                ...
                if (gObjGetItemCountInIventory(aIndex, this->m_QuestItems[j].wItemID) >=
                    this->m_QuestItems[j].btItemCount) {
                    continue;
                }

                if ((rand() % 10000) < this->m_QuestItems[j].dwRate) {
                    ItemSerialCreateSend(lpMonsterObj->m_Index, lpMonsterObj->MapNumber, lpMonsterObj->X,
                                         lpMonsterObj->Y, this->m_QuestItems[j].wItemID, 0, 1, 0, 0, 0, aIndex, 0, 0,
                                         g_AttributeSystem.GetElement(this->m_QuestItems[j].wItemID), -1);
                    return true;
                } else {
                    continue;
                }

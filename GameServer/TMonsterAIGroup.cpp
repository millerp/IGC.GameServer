...
int TMonsterAIGroup::FindGroupMemberObjectIndex(int iGroupNumber, int iGuid) {
    if (iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP) {
        g_Log.Add("[Monster AI Group] FindGroupMemberObjectIndex() Error - (GroupNumber=%d Guid=%d)", iGroupNumber,
                  iGuid);
        return -1;
    }
...
TMonsterAIGroup::FindGroupMemberToHeal(int iObjIndex, int iGroupNumber, int iGuid, int iDistance) {

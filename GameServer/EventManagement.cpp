...
void CEventManagement::Init() {
    if (this->m_bHasData != false && this->m_bEventStart != false) {
        ...
        if (_Itor != this->m_mapEventObj.end()) {
            ...
            for (std::vector<EVENT_ID_TIME>::iterator it = this->m_vtEventTime.begin();
...
void CEventManagement::Run() {
    ...
    if (this->m_wToday_Year != sysTime.wYear || this->m_wToday_Month != sysTime.wMonth ||
        this->m_wToday_Day != sysTime.wDay) {
        for (it = this->m_vtEventTime.begin(); it != this->m_vtEventTime.end(); it++) {
            (*(it)).m_bEventStarted = false;
        }

        g_Log.Add("[Event Management] Wrong date: %02d %02d %02d %02d", this->m_wToday_Year, this->m_wToday_Month,
                  this->m_wToday_Day, this->m_wToday_DayOfWeek);    // Deathway Need Translation
    ...
    for (it = this->m_vtEventTime.begin(); it != this->m_vtEventTime.end(); it++) {
        if (sysTime.wHour == (*(it)).m_iHour && sysTime.wMinute == (*(it)).m_iMinute &&
            (sysTime.wDayOfWeek == (*(it)).m_iDayOfWeek || (*(it)).m_iDayOfWeek == -1) &&
            (*(it)).m_bEventStarted == false) {
            g_Log.Add("[Event Management] Found Event: (%d) %02d %02d (state=%d)",
                      (*(it)).m_eEventKind, (*(it)).m_iHour, sysTime.wMinute,
                      (*(it)).m_bEventStarted);    // #error Deathway Need Translation

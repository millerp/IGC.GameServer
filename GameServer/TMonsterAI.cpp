    ...
    for (int i = 0; i < MAX_MONSTER_SEND_MSG; i++) {
        if (gSMMsg[iIndex][i].MsgCode >= 0) {
            if (GetTickCount64() > gSMMsg[iIndex][i].MsgTime) {
                TMonsterAI::ProcessStateMsg(lpObj, gSMMsg[iIndex][i].MsgCode, gSMMsg[iIndex][i].SendUser,
                                            gSMMsg[iIndex][i].SubCode);
...
bool TMonsterAI::UpdateCurrentAIUnit(int iIndex) {
    ...
    if (iOldCurrentAI != iCurrentAI) {
        UTIL.SendCrywolfChattingMsg(lpObj->m_Index, "��AI %s ���� %s �� �ٲ۴�!!",

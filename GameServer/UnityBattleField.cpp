...
void CUnityBattleField::GDReqJoinUnityBattleField(int aIndex) {
    ...
    if (this->m_bUBFEnable == false) {
        g_Log.Add("[UBF][GDReqJoinUnityBattleField][%s][%s][%s][ServerCode:%d] UBF not available.",
                  ...
                  lpObj->m_PlayerData->m_nServerCodeOfHomeWorld);
    ...
    if (lpObj->m_bMapSvrMoveQuit == true || lpObj->m_bMapSvrMoveReq == true || lpObj->m_bMapSvrMoveReq_2 == true) {
        g_Log.Add(
                "[UBF][GDReqJoinUnityBattleField][%s][%s][%s][ServerCode:%d] Map Of Move request was trying to Join UnityBattleField.",
                lpObj->AccountID, lpObj->m_PlayerData->m_RealNameOfUBF, lpObj->Name,
                lpObj->m_PlayerData->m_nServerCodeOfHomeWorld);
    ...
    if (lpObj->m_IfState.use && lpObj->m_IfState.type == 1) {
        ...
        g_Log.Add(
                "[UBF][GDReqJoinUnityBattleField][%s][%s][%s][ServerCode:%d] Trading can't be to Join UnityBattleField.",
                lpObj->AccountID, lpObj->m_PlayerData->m_RealNameOfUBF, lpObj->Name,
                lpObj->m_PlayerData->m_nServerCodeOfHomeWorld);
    ...
    if (lpObj->m_bPShopOpen == true) {
        ...
        g_Log.Add(
                "[UBF][GDReqJoinUnityBattleField][%s][%s][%s][ServerCode:%d] Opened PShop can't be to Join UnityBattleField.",
                lpObj->AccountID, lpObj->m_PlayerData->m_RealNameOfUBF, lpObj->Name,
                lpObj->m_PlayerData->m_nServerCodeOfHomeWorld);
...
void CUnityBattleField::DGAnsJoinUnityBattleField(int aIndex, BYTE Result, WORD LeftSecond) {
    ...
    g_Log.Add(
            "[UBF][DGAnsJoinUnityBattleField][%s][%s] Answer is To Join UBF ResultCode: %d (0:NewJoin Succ/1:ReJoin Succ/2:Joined State/3:Already apply/4:2 PersonUpper/5:Server Limit)",
...
void CUnityBattleField::DGAnsCopyCharacterInfo(int aIndex, BYTE result, BYTE subResult) {
    ...
    if (result != TRUE) {
        ...
        g_Log.Add(
                "[UBF][DGAnsCopyCharcterInfo][%s][%s] Character Move(Copy Fail) Result: %d (2,3:Fail), ErrCode: %d (2:No UserInfo / 3: No CharSlot)",
    ...
    g_Log.Add(
            "[UBF][DGAnsCopyCharcterInfo][Copy Complete][%s][%s] CharInfoSave : Class=%d Level=%d LVPoint=%d Exp=%I64d Str=%d Dex=%d Vit=%d Energy=%d Leadership:%d Map=%d Pk=%d",
            lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->DbClass, lpObj->Level,
            lpObj->m_PlayerData->LevelUpPoint, lpObj->m_PlayerData->Experience, lpObj->m_PlayerData->Strength,
            lpObj->m_PlayerData->Dexterity,
            lpObj->m_PlayerData->Vitality, lpObj->m_PlayerData->Energy, lpObj->Leadership, lpObj->MapNumber,
            lpObj->m_PK_Level);
...
void CUnityBattleField::GDObserverLogoutManager() {
    for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
        ...
        if (gObjIsConnected(n) == TRUE && lpObj->Type == OBJ_USER) {
            this->GDReqCheckJoinedUnityBattleField(n, g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE ? TRUE
                                                                                                               : FALSE,
                                                   TRUE);
        }
    }
...
void CUnityBattleField::GDReqCancelUnityBattleField(int aIndex, BYTE btCancelType) {
    ...
    if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
        g_Log.Add("[UBF][GDReqCancelUnityBattleField][%s][%s] In UBF, Can not be canceled", lpObj->AccountID,
                  lpObj->Name);
...
void CUnityBattleField::DGAnsCancelUnityBattleField(int aIndex, BYTE aCanceledResult, BYTE deletedResult) {
    ...
    g_Log.Add(
            "[UBF][DGAnsCancelUnityBattleField][%s][%s] Answer CancelUnityBattleField CancelResult:%d (1:Success), deleteResult:%d (1:Success) ",
...
void CUnityBattleField::DGAnsSetReceivedWinnerItem(int aIndex, BYTE btReturn) {
    ...
    if (btReturn == TRUE) {
        ...
        g_Log.Add(
                "[UBF][DGAnsSetReceivedWinnerItem] Index:%d ID:%s Name:%s WinnerItem Failed to complete the process ErrCode :%d",

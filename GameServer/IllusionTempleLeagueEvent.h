...
public:
    ...
    void
    ActRelicsGetOrRegister(OBJECTSTRUCT *lpNpc, OBJECTSTRUCT *lpObj, BYTE byMapNumber, BYTE byPrePacket, BYTE byAct,
                           WORD wNpcType);
    ...
    BYTE
    Update_GuildInfo(int nGuildIdx, BYTE byMapNumber, BYTE bySlotNumber, int nPartyIdx, BYTE byITLType, int nUserIndex);
    ...
    BYTE
    Find_MyPartyAndGuildInfo(BYTE byMapNumber, BYTE byArrayOrder, int GuildIndex, BYTE &byTeamIndex, int &nPartyIdx);

    BYTE Find_AlreadyEnteredUser(char *Name, BYTE &byMapNumber);

    BYTE CheckCanReEnterTiming(BYTE byMapNumber, BYTE &byNowState);

    BYTE ReEnterITL(int aIndex, BYTE btTempleIndex, int nGuildIdx, int nPartyIdx, int nUserArrayNo, int nITLState,
                    BYTE byTeamIndex);

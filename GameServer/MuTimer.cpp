...
== SERVER_NORMAL || g_ConfigRead.server.

GetServerType()

== SERVER_CASTLE)
{
g_MonsterRegenSystem.
...
ProcDoppelganger (GetTickCount64());
...
WeatherVariationProcess();

}
wsGServer.
m_SendSec = wsGServer.m_SendSecTmp;
wsGServer.
m_RecvSec = wsGServer.m_RecvSecTmp;
wsGServer.
m_SendSecTmp = 0;
wsGServer.
m_RecvSecTmp = 0;
...
wsprintf(szTemp, Lang
.GetText(0,2), gCloseMsgTime);
GSProtocol.
AllSendServerMsg(szTemp);
g_Log.
AddC(TColor::Red,
"Server close automatically in %d second(s)", gCloseMsgTime);
}
}
...
{
for (
int i = g_ConfigRead.server.GetObjectStartUserIndex();
...
==
TRUE &&g_AcheronGuardianEvent
.

IsInitAEProcMultiCast()
...
{
g_ChaosCastleFinal.
GD_Renew_CCF_Ranking(g_ChaosCastleFinal
.
...
{
for (
int n =/* ObjectStartUserIndex*/ 0;
n<g_ConfigRead.server.
...
{
gObj[n].
m_bPShopItemChange = false;
}
}
}

for (
int n = g_ConfigRead.server.GetObjectStartUserIndex();
n<g_ConfigRead.server.

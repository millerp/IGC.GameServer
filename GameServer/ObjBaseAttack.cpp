...
BOOL CObjBaseAttack::DecreaseArrow(LPOBJ lpObj) {
    if (lpObj->Class == CLASS_ELF && lpObj->Type == OBJ_USER &&
        g_ConfigRead.data.common.joinmuIsDecreaseAllow == true) {
...
BOOL CObjBaseAttack::PkCheck(LPOBJ lpObj, LPOBJ lpTargetObj) {
    if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
        ...
        if (gObjTargetGuildWarCheck(lpObj, lpTargetObj) == FALSE && lpTargetObj->m_PlayerData->lpGuild != NULL &&
            lpTargetObj->m_PlayerData->lpGuild->WarState != 0) {
...
BOOL CObjBaseAttack::ResistanceCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int skill) {
    if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
        if (lpTargetObj->Type != OBJ_USER) {
            if (lpTargetObj->Class == 277 || lpTargetObj->Class == 283 || lpTargetObj->Class == 288 ||
                lpTargetObj->Class == 278 || lpTargetObj->Class == 215 || lpTargetObj->Class == 216 ||
                lpTargetObj->Class == 217 || lpTargetObj->Class == 218 || lpTargetObj->Class == 219) {
                return FALSE;
            }
        }

        if (lpTargetObj->Type != OBJ_USER) {
            if (CRYWOLF_ALTAR_CLASS_RANGE(lpTargetObj->Class) != FALSE ||
                CRYWOLF_STATUE_CHECK(lpTargetObj->Class) != FALSE) //HermeX Fix @28/01/2010
            {
                return FALSE;
            }
        }
    ...
    if (lpTargetObj->Type == OBJ_USER) {
        if (lpTargetObj->Class == CLASS_RAGEFIGHTER) {
            if (lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsImmuneRate_Monk > 0.0 &&
                lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsImmuneRate_Monk >= (rand() % 100) &&
                !gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_INVISIBLE2)) {
            ...
            if (lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsImmuneRate > 0.0 &&
                lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsImmuneRate >= (rand() % 100) &&
                !gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_INVISIBLE2)) {
                GSProtocol.GCMagicAttackNumberSend(lpTargetObj, 323, lpTargetObj->m_Index, 1);
                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_INVISIBLE2, 0, 0, 0, 0, 5);
            }
    ...
    if (skill == 512) {
        for (int i = 0; i < MAX_MAGIC; i++) {
            if (lpObj->Magic[i].IsMagic() == TRUE && lpObj->Magic[i].m_Skill == skill) {
                fValue = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(MagicDamageC.SkillGet(skill),
                                                                          lpObj->Magic[i].m_Level);
                break;
            }
        }
        ...
        if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_POISON) == FALSE) {
            if (g_ConfigRead.data.common.IsJoinMu == 1) {
                if (rand() % 100 < gObjUseSkill.m_SkillData.PoisonSkillSuccessRate) {
                    ...
                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_POISON, EFFECTTYPE_POISON_DMG_TICK,
                                      gObjUseSkill.m_SkillData.PoisonPercent, 0, 0,
                                      gObjUseSkill.m_SkillData.PoisonSkillTime);
                ...
                if (retResistance(lpTargetObj, 1) == 0) {
                    ...
                    lpTargetObj->m_SkillNumber = skill;
                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_POISON, EFFECTTYPE_POISON_DMG_TICK,
                                      gObjUseSkill.m_SkillData.PoisonPercent, 0, 0, 20);
                } else {
                    return FALSE;
                }
            }
    ...
    } else if (skill == 38 || skill == 387) // Death Poison
    {
        if (g_ConfigRead.data.common.IsJoinMu == 1) {
            if (rand() % 100 < gObjUseSkill.m_SkillData.PoisonSkillSuccessRate) {
                ...
                lpTargetObj->m_SkillNumber = skill;
                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_POISON, EFFECTTYPE_POISON_DMG_TICK,
                                  gObjUseSkill.m_SkillData.DeathPoisonPercent, 0, 0,
                                  gObjUseSkill.m_SkillData.PoisonSkillTime);
            } else {
                return FALSE;
            }
            ...
            if (retResistance(lpTargetObj, 1) == 0) {
                ...
                lpTargetObj->m_SkillNumber = skill;
                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_POISON, EFFECTTYPE_POISON_DMG_TICK,
                                  gObjUseSkill.m_SkillData.DeathPoisonPercent, 0, 0, 20);
            } else {
                return FALSE;
            }
        }
        ...
        return TRUE;
    } else if (skill == 7 || skill == 39 || skill == 389 || skill == 489 || skill == 391 || skill == 393 ||
               skill == 491) //Season4 add-on )
    {
...
BOOL
CObjBaseAttack::MissCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int skill, int skillSuccess, int magicsend, BOOL &bAllMiss,
                          BYTE RFAttack) {
    int iAttackRate = 0;
    ...
    __try
    {
        ...
        if (bAllMiss != FALSE)    // When All Miss
        {
            if ((rand() % 100) >= 5) {
                if (RFAttack) {
                    if (lpObj->Class == CLASS_RAGEFIGHTER) {
                        if (skill == 261 || skill == 263 || skill == 552 || skill == 555 || skill == 559 ||
                            skill == 563) {
                            if (RFAttack % 2) {
                                MSBDamage |= 0x10;
                            } else {
...
BOOL
CObjBaseAttack::MissCheckPvP(LPOBJ lpObj, LPOBJ lpTargetObj, int skill, int skillSuccess, int magicsend, BOOL &bAllMiss,
                             BYTE RFAttack) {
    if (lpObj == NULL) {
        return FALSE;
    }
    ...
    if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_MONK_IGNORE_ENEMY_DEFENSE)) {
        ...
        if (nEffectValue2 > 0) {
            if (rand() % 100 <= 10) {
                ...
                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_PENETRATE_ARMOR, EFFECTTYPE_DECREASE_DEFENSE, nEffectValue2,
                                  EFFECTTYPE_DECREASE_DEFENSE_RATE, nEffectValue, 10);
            }
        }
    }

    ...
    for (int i = 0; i < ObjectMaxUser; i++) {
        if (this->m_UserData[i].m_Index && Tick - this->m_UserData[i].m_dwTick > 1000 * OverTimeSeconds) {
            ...
            if (this->CheckInEventMap(i) == 0 || this->CheckInEventMap(i) == 1 || this->CheckInEventMap(i) == 5) {
                gParty.Delete(this->m_UserData[i].m_PartyNumber, this->m_UserData[i].m_Index,
                              gObj[this->m_UserData[i].m_Index].DBNumber);
...
void CTemporaryUserManager::DeleteUserInParty(int Index) {
    ...
    gParty.Delete(this->m_UserData[Index].m_PartyNumber, this->m_UserData[Index].m_Index,
                  gObj[this->m_UserData[Index].m_Index].DBNumber);
...
BOOL CTemporaryUserManager::ResumeIllusionTemple(int Index, int aIndex) {
    ...
    BYTE Result = g_IllusionTempleEvent.ChangeTempleUserInformation(this->m_UserData[Index].m_Index, aIndex,
                                                                    this->m_UserData[Index].m_IllusionTempleIndex,
                                                                    this->m_UserData[Index].m_MapNumber);
...
BOOL CTemporaryUserManager::ResumeImperialGuardian(int Index, int aIndex) {
    ...
    if (g_ImperialGuardian.GetPlayUserCountRightNow(this->m_UserData[Index].m_ImperialEventIndex) >= 1) {
        return g_ImperialGuardian.ChangeUserIndex(this->m_UserData[Index].m_Index, aIndex,
                                                  this->m_UserData[Index].m_ImperialEventIndex);
    }
...
BOOL CTemporaryUserManager::ResumeBloodCastle(int Index, int aIndex) {
    ...
    if (BCCount >= 1) {
        if (g_BloodCastle.ChangeUserIndex(this->m_UserData[Index].m_Index, aIndex,
                                          this->m_UserData[Index].m_BloodCastleIndex) == TRUE) {
...
BOOL CTemporaryUserManager::ResumeDevilSquare(int Index, int aIndex) {
    ...
    if (g_DevilSquare.GetPlayUserCountRightNow(this->m_UserData[Index].m_DevilSquareIndex) >= 1) {
        if (g_DevilSquare.GetState() > DevilSquare_CLOSE) {
            ...
            return g_DevilSquare.ChangeUserIndex(this->m_UserData[Index].m_Index, aIndex,
                                                 this->m_UserData[Index].m_DevilSquareIndex);
        }
    }

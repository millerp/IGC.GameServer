...
void CGate::Load(char *filename) {
    ...
    if (res.status != pugi::status_ok) {
        g_Log.MsgBox(
                "%s file not found - Please rename IGC_GateSettings.xml.s6 or IGC_GateSettings.xml.s8 to IGC_GateSettings.xml in your Data folder according to server version",
                filename);
        ExitProcess(1);
    }
...
int CGate::GetGate(int mgt, short &x, short &y, BYTE &MapNumber, BYTE &dir, short &Level) {
    ...
    while (loopcount-- != 0) {
        ...
        if ((this->m_Ex[gt] - this->m_Sx[gt]) > 0) {
            tx = this->m_Sx[gt] +
                 (rand() % (this->m_Ex[gt] - this->m_Sx[gt]));    // Choose any X position between XBegin and XEnd
        } else {
        ...
        if ((this->m_Ey[gt] - this->m_Sy[gt]) > 0) {
            ty = this->m_Sy[gt] +
                 (rand() % (this->m_Ey[gt] - this->m_Sy[gt]));    // Choose any Y position between YBegin and YEnd
        } else {
...
BOOL CGate::IsInGate(int aIndex, int GateNumber) {
    ...
    if (lpObj->X < this->m_Sx[GateNumber] - 5 || lpObj->X > this->m_Ex[GateNumber] + 5 ||
        lpObj->Y < this->m_Sy[GateNumber] - 5 || lpObj->Y > this->m_Ey[GateNumber] + 5) {
        return FALSE;
    }

...
bool CObjUseSkill::LoadSkillInfo() {
    ...
    this->m_SkillData.DeathPoisonPercent = ReadSkill.ReadInt("joinmu", "DeathPoisonPercent",
                                                             3); // PRIVATE CUSTOM - JOINMU

    this->m_SkillData.EnableSiegeOnAllMaps = ReadSkill.ReadInt("SiegeSkills", "EnableOnAllMaps", 1);
    this->m_SkillData.UseCalcForTripleShot = ReadSkill.ReadInt("joinmu", "UseLuaCalcForTripleShot",
                                                               0); // PRIVATE CUSTOM - JOINMU
    ...
    this->m_SkillData.EnableWizardSkillWhileTeleport = ReadSkill.ReadInt("SkillInfo",
                                                                         "CanDarkWizardUseSkillsWhileTeleport", 0);
...
BOOL CObjUseSkill::SpeedHackCheck(int aIndex) {
    ...
    if (iTimeCalc < lpObj->m_DetectSpeedHackTime) {
        ...
        if (lpObj->m_DetectCount > gHackCheckCount) {
            ...
            if (gIsKickDetecHackCountLimit != 0 && lpObj->m_DetectedHackKickCount > gDetectedHackKickCount) {
                g_Log.Add("[%s][%s] Kick DetecHackCountLimit Over User (%d)", lpObj->AccountID, lpObj->Name,
                          lpObj->m_DetectedHackKickCount);
            ...
            if (gIsKickDetecHackCountLimit != 0) {
                g_Log.Add("[%s][%s] Attack Speed Is Wrong MagicSkill (%d)(%d) Penalty %d", lpObj->AccountID,
                          lpObj->Name, lpObj->m_DetectSpeedHackTime, lpObj->m_SumLastAttackTime / lpObj->m_DetectCount,
    ...
    if (bIsIgnorePacketSpeedHackDetect != 0 && lpObj->m_SpeedHackPenalty > 0) {
        lpObj->m_SpeedHackPenalty--;
        g_Log.Add("[%s][%s] Apply Attack Speed Penalty (%d left)", lpObj->AccountID, lpObj->Name,
                  lpObj->m_SpeedHackPenalty);
        return false;
    }
    return true;
...
void
CObjUseSkill::UseSkill(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
    ...
    if (usemana >= 0) {
        ...
        if (lpMagic->m_Skill == AT_SKILL_EXPPOISON) {
            ...
            if (ObjectMaxRange(aTargetIndex) == FALSE) {
                g_Log.Add(
                        "[InvalidTargetIndex][CObjUseSkill.UseSkill][AT_SKILL_DRAGON_ROAR] Index :%d , AccountID : %s ",
                        aIndex, gObj[aIndex].AccountID);
                return;
            }

            this->SkillAreaMonsterAttack(aIndex, lpMagic, gObj[aTargetIndex].X, gObj[aTargetIndex].Y, aTargetIndex, 3,
                                         1, 0);
            ...
            if (ObjectMaxRange(aTargetIndex) == FALSE) {
                g_Log.Add(
                        "[InvalidTargetIndex][CObjUseSkill.UseSkill][AT_SKILL_PHOENIXSHOT] Index :%d , AccountID : %s ",
                        aIndex, gObj[aIndex].AccountID);
                return;
            }

            this->SkillAreaMonsterAttack(aIndex, lpMagic, gObj[aTargetIndex].X, gObj[aTargetIndex].Y, aTargetIndex, 2,
                                         1, 0);
...
BOOL CObjUseSkill::RunningSkill(int aIndex, int aTargetIndex, CMagicInf *lpMagic, BOOL bCombo) {
    ...
    switch (MagicNumber) {
        ...
        case AT_SKILL_DEFENSE:
            ...
            break;
        case AT_SKILL_ATTACK:
            this->SkillAttack(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_SWORD_POWER:
            this->SkillSwordPower(aIndex, lpMagic, aTargetIndex);
            break;

        case AT_SKILL_CALLMON1:
            return this->SkillMonsterCall(lpObj->m_Index, 26, lpObj->X - 1, lpObj->Y + 1);
        case AT_SKILL_CALLMON2:
            return this->SkillMonsterCall(lpObj->m_Index, 32, lpObj->X - 1, lpObj->Y + 1);
        case AT_SKILL_CALLMON3:
            return this->SkillMonsterCall(lpObj->m_Index, 21, lpObj->X - 1, lpObj->Y + 1);
        case AT_SKILL_CALLMON4:
            return this->SkillMonsterCall(lpObj->m_Index, 20, lpObj->X - 1, lpObj->Y + 1);
        case AT_SKILL_CALLMON5:
            return this->SkillMonsterCall(lpObj->m_Index, 10, lpObj->X - 1, lpObj->Y + 1);
        case AT_SKILL_CALLMON6:
            return this->SkillMonsterCall(lpObj->m_Index, 150, lpObj->X - 1, lpObj->Y + 1);
        case AT_SKILL_CALLMON7:
            return this->SkillMonsterCall(lpObj->m_Index, 151, lpObj->X - 1, lpObj->Y + 1);

        case AT_SKILL_EXPHELL:
            return this->SkillHellFire2(lpObj->m_Index, aTargetIndex, lpMagic);
        case AT_SKILL_STRIKE:
            return this->SkillKnightBlow(lpObj->m_Index, aTargetIndex, lpMagic, bCombo);

        case AT_SKILL_SPEAR:
            return this->SkillSpear(lpObj->m_Index, aTargetIndex, lpMagic);
        case AT_SKILL_FIREBURST:
            return this->SkillFireBurst(lpObj->m_Index, aTargetIndex, lpMagic);
        case AT_SKILL_DARKHORSE_ATTACK:
            return this->SkillDarkHorseAttack(lpObj->m_Index, aTargetIndex, lpMagic);
        case AT_SKILL_RECALL_PARTY:
            this->SkillRecallParty(aIndex, lpMagic->m_Level);
            break;
        case AT_SKILL_ADD_CRITICALDAMAGE:
            this->SkillAddCriticalDamage(aIndex, lpMagic->m_Level, lpMagic);
            break;

        case AT_SKILL_DEFENSEDOWN:
            this->MaGumSkillDefenseDown(lpObj->m_Index, aTargetIndex, 1);
            break;

        case AT_SKILL_SUMMON:
            this->SkillSummon(lpObj->m_Index, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_IMMUNE_TO_MAGIC:
            this->SkillImmuneToMagic(lpObj->m_Index, lpMagic);
            break;
        case AT_SKILL_IMMUNE_TO_HARM:
            this->SkillImmuneToHarm(lpObj->m_Index, lpMagic);
            break;
        case AT_SKILL_RUSH:
            this->SkillKnightRush(lpObj->m_Index, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_JAVALIN:
            this->SkillWizardJavalin(lpObj->m_Index, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_DEEPIMPACT:
            this->SkillElfRush(lpObj->m_Index, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_ONE_FLASH:
            this->SkillMagumOneFlash(lpObj->m_Index, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_DEATH_CANNON:
            this->SkillMagumDeathCannon(lpObj->m_Index, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_SPACE_SPLIT:
            this->SkillDarklordSpaceSplit(lpObj->m_Index, aTargetIndex, lpMagic);
            break;

        case AT_SKILL_BRAND_OF_SKILL:
            this->SkillBrandOfSkill(aIndex, aTargetIndex, lpMagic);
            break;

        case AT_SKILL_REMOVAL_STUN:
            this->SkillRemoveStun(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_ADD_MANA:
            this->SkillAddMana(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_INVISIBLE:
            this->SkillCloaking(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_REMOVAL_INVISIBLE:
            this->SkillRemoveCloaking(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_REMOVAL_MAGIC:
            this->SkillRemoveMagic(aIndex, aTargetIndex, lpMagic);
            break;

        case AT_SKILL_INFINITY_ARROW:
            this->SkillInfinityArrow(aIndex, aTargetIndex, lpMagic);
            break;

        case AT_SKILL_SLEEP:
            this->SkillSummonerSleep(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_DAMAGE_REFLECT:
            this->SkillDamageReflect(aIndex, lpMagic, aTargetIndex);
            break;
        case AT_SKILL_MAGIC_CIRCLE:
            this->SkillMagicCircle(aIndex, aTargetIndex, lpMagic);
            break;
        case AT_SKILL_SD_RECOVERY:
            this->SkillShieldRefill(aIndex, lpMagic, aTargetIndex);
            break;
...
void CObjUseSkill::KnightSkillAddLife(int aIndex, CMagicInf *lpMagic) {
    ...
    if ((lpObj->Type == OBJ_USER) &&
        (lpObj->Class != CLASS_KNIGHT && lpObj->Class != CLASS_MAGUMSA && lpObj->m_PlayerData->ISBOT == false)) {
        return;
    }
    ...
    this->m_Lua.Generic_Call("KnightSkillAddLife", "iii>di", (lpObj->m_PlayerData->Vitality + lpObj->AddVitality),
                             (lpObj->m_PlayerData->Energy + lpObj->AddEnergy), viewpercent, &iaddLifepower, &iLifeTime);
...
void CObjUseSkill::WizardMagicDefense(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("WizardMagicDefense", "iiii>di", aIndex, aTargetIndex,
                             (lpObj->m_PlayerData->Dexterity + lpObj->AddDexterity),
                             (lpObj->m_PlayerData->Energy + lpObj->AddEnergy), &skillInc, &skillTime);
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_MELEE_DEFENSE_INC, EFFECTTYPE_IMPROVE_MELEE_DEFENSE, skillInc,
                      EFFECTTYPE_MELEE_DEFENSE_DOWN_MANA, 20, skillTime);
...
void CObjUseSkill::SkillDefense(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("ElfDefense", "iiii>dd", lpTargetObj->Class, aIndex, aTargetIndex,
...
void CObjUseSkill::SkillAttack(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("ElfAttack", "iiii>dd", lpTargetObj->Class, aIndex, aTargetIndex,
...
BOOL CObjUseSkill::SkillChangeUse(int aIndex) {
    ...
    if (lpObj->m_Change == 41 && change != 41) {
        ...
        GSProtocol.GCReFillSend(lpObj->m_Index, lpObj->AddLife + lpObj->MaxLife, 0xFE, 0,
                                lpObj->iAddShield + lpObj->iMaxShield);
    }
...
void CObjUseSkill::SkillHealing(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    int addlife = 0;
    this->m_Lua.Generic_Call("ElfHeal", "iiii>i", lpTargetObj->Class, lpObj->m_PlayerData->Energy + lpObj->AddEnergy,
                             aIndex, aTargetIndex, &addlife);
...
int CObjUseSkill::SkillDeathPoison(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos,
                                   int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
    int tObjNum;
    int count = 0;
                            ...
                            if (gObjCalDistance(lpTargetObj, &gObj[tObjNum]) <= 3) {
                                ...
                                if (gObjCheckUsedBuffEffect(&gObj[tObjNum], BUFFTYPE_POISON) == FALSE) {
                                    if (!retResistance(&gObj[tObjNum], 1)) {
                                        ...
                                        gObjAddBuffEffect(&gObj[tObjNum], BUFFTYPE_POISON, EFFECTTYPE_POISON_DMG_TICK,
                                                          3, 0, 0, 20);
                                    }
                                }
                            }
...
int CObjUseSkill::SkillSuddenIce(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos,
                                 int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
    int tObjNum;
    int count = 0;
    ...
    while (true) {
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack != 0) {
                    if (this->CalDistance(x, y, gObj[tObjNum].X, gObj[tObjNum].Y) <=
                        MagicDamageC.GetSkillDistance(lpMagic->m_Skill)) {
                        bAttack = 0;
...
void CObjUseSkill::SkillAddCriticalDamage(int aIndex, int skill_level, CMagicInf *lpMagic) {
    ...
    this->m_Lua.Generic_Call("DarkLordCriticalDamage", "ii>ii", (lpObj->Leadership + lpObj->AddLeadership),
    ...
    if (partynum == -1 && lpObj->m_PlayerData->ISBOT == false) {
        ...
        gObjAddBuffEffect(&gObj[lpObj->BuffPlayerIndex], BUFFTYPE_CRITICAL_DMG_INC, EFFECTTYPE_CRITICAL_DMG,
...
void CObjUseSkill::SkillElectricSpark(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos,
                                      int aTargetIndex) {
    LPOBJ lpObj = &gObj[aIndex];
...
BOOL CObjUseSkill::SkillSummon(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    GSProtocol.GCMagicAttackNumberSend(lpObj, AT_SKILL_SUMMON,
                                       (((cSummonX & 0xFF) & 0xFF) | ((cSummonY & 0xFF) & 0xFF) << 8) & 0xFFFF,
                                       bSummonSuccess);
...
BOOL CObjUseSkill::SkillImmuneToMagic(int aIndex, CMagicInf *lpMagic) {
    ...
    if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_MONSTER_MAGIC_IMMUNE) == 0 &&
        gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_MONSTER_MELEE_IMMUNE) == 0) {
...
BOOL CObjUseSkill::SkillKnightRush(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    if (lpObj->MapNumber != MAP_INDEX_CASTLESIEGE && lpObj->MapNumber != MAP_INDEX_CASTLEHUNTZONE &&
        this->m_SkillData.EnableSiegeOnAllMaps == false) {
        return false;
    }
...
BOOL CObjUseSkill::SkillStun(int aIndex, int aTargetIndex, CMagicInf *lpMagic, BYTE btX, BYTE btY, BYTE btDir,
                             BYTE btTargetPos) {
    if (ObjectMaxRange(aIndex) == false) {
        return false;
    }
            ...
            if (lpObj->VpPlayer2[count].type == OBJ_USER || lpObj->VpPlayer2[count].type == OBJ_MONSTER) {
                ...
                if (ObjectMaxRange(tObjNum)) {
                    if (gObjCalDistance(lpObj, &gObj[tObjNum]) <= 4) {
                        if (SkillElectricSparkHitBox.HitCheck(iangle, lpObj->X, lpObj->Y, gObj[tObjNum].X,
                                                              gObj[tObjNum].Y)) {
        ...
        if (lpObj->VpPlayer2[iCount].state != 0) {
            ...
            if (iTarObjNum >= 0) {
                ...
                if (gObj[iTarObjNum].Type == OBJ_MONSTER && gObj[iTarObjNum].m_RecallMon < 0) {
                    bEnableAttack = 1;
                } else if (lpTargetObj->Type == OBJ_USER && lpObj->Type == OBJ_USER &&
                           gObj[iTarObjNum].Type != OBJ_NPC) {
                    bEnableAttack = 1;
                } else if (lpTargetObj->m_cChaosCastleIndex == gObj[iTarObjNum].m_cChaosCastleIndex &&
                           lpTargetObj->m_cChaosCastleIndex != -1) {
                    bEnableAttack = 1;
                } else if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE &&
                           lpObj->VpPlayer2[iCount].type != OBJ_NPC &&
...
void CObjUseSkill::SpecificSkillAdditionTreat(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf *lpMagic, int iAttackDamage) {
    ...
    if (bySkillNum == AT_SKILL_FIRESCREAM || bySkillNum == 518 || bySkillNum == 520) {
        ...
        if (lpMagic->m_Skill == 520) {
            iAddDamage = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill),
                                                                          lpMagic->m_Level);
        }
...
int CObjUseSkill::GetAddUseMana(LPOBJ lpObj, CMagicInf *lpMagic) {
    ...
    if (lpObj->Class == CLASS_ELF && lpObj->Type == OBJ_USER && lpObj->m_PlayerData->ChangeUP != FALSE &&
        (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_INFINITY_ARROW) == TRUE ||
         gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_INFINITY_ARROW_STR) == TRUE)) {
...
int CObjUseSkill::SkillDamageReflect(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    this->m_Lua.Generic_Call("SummonerDamageReflect", "i>ii", (lpObj->m_PlayerData->Energy + lpObj->AddEnergy), &RefDmg,
                             &Time);
        ...
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack == TRUE) {
                    ...
                    } else if (gObj[tObjNum].X >= gObj[aTargetIndex].X - 2 &&
                               gObj[tObjNum].X <= gObj[aTargetIndex].X + 2
                               && gObj[tObjNum].Y >= gObj[aTargetIndex].Y - 2 &&
                               gObj[tObjNum].Y <= gObj[aTargetIndex].Y + 2) {
...
int
CObjUseSkill::SkillBlowOfDestruction(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, int aTargetIndex, BOOL bCombo) {
...
int CObjUseSkill::SkillAreaMonsterAttack(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, int aTargetIndex, int Distance,
                                         int MultiAttack, int AttackDelay) {
    LPOBJ lpObj = &gObj[aIndex];
    LPOBJ lpTargetObj = &gObj[aTargetIndex];
        ...
        while (true) {
            if (lpObj->VpPlayer2[count].state != 0) {
                ...
                if (tObjNum >= 0) {
                    ...
                    if (lpObj->VpPlayer2[count].type == OBJ_NPC || lpObj->MapNumber != MAP_INDEX_CASTLESIEGE ||
                        g_CastleSiegeSync.GetCastleState() != 7) {
                        int CallMonIndex = gObj[tObjNum].m_Index;
                            ...
                            if (gObj[tObjNum].X <= x + Distance) {
                                if (gObj[tObjNum].Y >= y - Distance) {
                                    if (gObj[tObjNum].Y <= y + Distance) {
                                        ...
                                        if (Attack) {
                                            if (AttackDelay) {
                                                if (lpMagic->m_Skill == 237 && lpObj->Class == 561 &&
                                                    gObjCheckUsedBuffEffect(&gObj[tObjNum], BUFFTYPE_FREEZE) == FALSE) {
                                                ...
                                                gObjAddAttackProcMsgSendDelay(lpObj, 50, tObjNum, AttackDelay,
                                                                              lpMagic->m_Skill, 0);
                                            } else {
                                                if (lpMagic->m_Skill != AT_SKILL_DRAGON_ROAR &&
                                                    lpMagic->m_Skill != AT_SKILL_PHOENIX_SHOT &&
                                                    lpMagic->m_Skill != 560) {
                                                    gObjAttack(lpObj, &gObj[tObjNum], lpMagic, 1, 0, 0, 0, 0, 0);
                                                } else {
                                                    int Dec = 0;
                                                    BYTE btAttr = MapC[gObj[tObjNum].MapNumber].GetAttr(gObj[tObjNum].X,
                                                                                                        gObj[tObjNum].Y);

                                                    if ((btAttr & 1) != 1) {
                                                        if (lpMagic->m_Skill == AT_SKILL_PHOENIX_SHOT &&
                                                            rand() % 100 < 10) {
                                                            ...
                                                            gObjAddBuffEffect(&gObj[tObjNum],
                                                                              BUFFTYPE_MONK_DECREASE_DEFENSE_RATE,
                                                                              EFFECTTYPE_DECREASE_DEFENSE_RATE, Dec, 0,
                                                                              0, 10);
                                                        }
                                                    }
                                                    ...
                                                    for (int i = 1; i <= 4; i++) {
                                                        if (i == 1)
                                                            gObjAttack(lpObj, &gObj[tObjNum], lpMagic, 1, 1, 0, 0, 1,
                                                                       0);
                                                        else
                                                            gObjAttack(lpObj, &gObj[tObjNum], lpMagic, 0, 1, 0, 0, i,
                                                                       0);
                                                    }
                                                }
                                            }
...
int CObjUseSkill::SkillMagicCircle(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    if (gObj[aIndex].m_PlayerData->ISBOT == true) {
        gObjAddBuffEffect(&gObj[gObj[aIndex].BuffPlayerIndex], BUFFTYPE_MAGIC_POWER_INC, EFFECTTYPE_MAGICPOWER_INC, 20,
                          0, 0, g_BotSystem.GetSkillTime(aIndex, lpMagic->m_Skill));
    } else {
        LPOBJ lpObj = &gObj[aIndex];
...
int CObjUseSkill::SkillShieldRefill(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (lpTargetObj->iShield < lpTargetObj->iAddShield + lpTargetObj->iMaxShield) {
        ...
        this->m_Lua.Generic_Call("ElfShieldRecovery", "ii>d", (int) (lpObj->m_PlayerData->Energy + lpObj->AddEnergy),
                                 (int) lpObj->Level, &skill_shield);
        ...
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (bEnableAttack != FALSE) {
                    ...
                    for (int j = 0; j < 5; j++) {
                        Value3[j] = (((int) Value1[j] - lpObj->X) * (gObj[tObjNum].Y - lpObj->Y)) -
                                    (((int) Value2[j] - lpObj->Y) * (gObj[tObjNum].X - lpObj->X));
...
int CObjUseSkill::SkillSwordPower(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (lpObj->Type == OBJ_USER) {
        this->m_Lua.Generic_Call("SummonerBerserker", "i>iii", (lpObj->m_PlayerData->Energy + lpObj->AddEnergy),
                                 &EffectUPValue, &EffectDownValue, &Time);
    }
    ...
    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_BERSERKER, EFFECTTYPE_BERSERKER_UP, EffectUPValue,
                      EFFECTTYPE_BERSERKER_DOWN, EffectDownValue, Time);
...
int CObjUseSkill::SkillMonkBuff(int aIndex, CMagicInf *lpMagic) {
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_MONK_IGNORE_ENEMY_DEFENSE, EFFECTTYPE_IGNORE_OPPONENT_DEFENSE, Ignore, 0, 0,
...
int CObjUseSkill::SkillMonkBuffApplyParty(int aIndex, CMagicInf *lpMagic) {
    ...
    if (partynum == -1) {
        if (lpMagic->m_Skill == AT_SKILL_FITNESS) {
            gObjAddBuffEffect(lpObj, BuffIndex, EFFECTTYPE_MONK_VITALITY, skill_improve, 0, 0,
                              lpObj->m_PlayerData->Energy / 5 + 60);
            GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
            gObjCalCharacter.CalcCharacter(aIndex);
        } else if (lpMagic->m_Skill == AT_SKILL_DSRINCREASE) {
            gObjAddBuffEffect(lpObj, BuffIndex, EFFECTTYPE_IMPROVE_DEFENSE_RATE, skill_improve, 0, 0,
                              lpObj->m_PlayerData->Energy / 5 + 60);
            GSProtocol.GCMagicAttackNumberSend(lpObj, lpMagic->m_Skill, aIndex, TRUE);
        }
    } else {
        for (int j = 0; j < 5; j++) {
            if (partyindex[j] != -1) {
                ...
                if (lpMagic->m_Skill == AT_SKILL_FITNESS) {
                    gObjAddBuffEffect(lpPartyObj, BuffIndex, EFFECTTYPE_MONK_VITALITY, skill_improve, 0, 0,
                                      lpObj->m_PlayerData->Energy / 5 + 60);
                    ...
                    gObjAddBuffEffect(lpPartyObj, BuffIndex, EFFECTTYPE_IMPROVE_DEFENSE_RATE, skill_improve, 0, 0,
                                      lpObj->m_PlayerData->Energy / 5 + 60);
                    GSProtocol.GCMagicAttackNumberSend(lpPartyObj, lpMagic->m_Skill, partyindex[j], TRUE);
                }
            }
...
int CObjUseSkill::SkillMonkBarrageJustOneTarget(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (ObjectMaxRange(aTargetIndex) == FALSE) {
        g_Log.Add("[InvalidTargetIndex][SkillMonkBarrageJustOneTarget] Index :%d , AccountID : %s", aIndex,
    ...
    if (EnableAttack == TRUE &&
        this->CalDistance(gObj[aTargetIndex].X, gObj[aTargetIndex].Y, gObj[tObjNum].X, gObj[tObjNum].Y) < nDistance) {
        switch (lpMagic->m_Skill) {
            case AT_SKILL_LARGE_BLOWER:
                HitCount = 5;
                break;
...
int CObjUseSkill::SkillCharge(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE || lpObj->MapNumber == MAP_INDEX_CASTLEHUNTZONE ||
        this->m_SkillData.EnableSiegeOnAllMaps == true) {
...
int CObjUseSkill::SkillMonkDarkSideGetTargetIndex(int aIndex, int aTargetIndex, CMagicInf *lpMagic, WORD *Target) {
    ...
    if (lpMagic->m_Skill == 563) {
        int iPoint = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(MagicDamageC.SkillGet(lpMagic->m_Skill),
                                                                      lpMagic->m_Level);
        nDistance += iPoint / 10;
    }
            ...
            if (aTargetIndex) {
                while (true) {
                    if (lpObj->VpPlayer2[count].state == 1) {
                        ...
                        if (tObjNum >= 0) {
                            if (tObjNum != 10000 && lpObj->VpPlayer2[count].type == OBJ_USER &&
                                tObjNum == aTargetIndex) {
                                if (this->CalDistance(lpObj->X, lpObj->Y, gObj[tObjNum].X, gObj[tObjNum].Y) <
                                    nDistance) {
                                    *Target = tObjNum;
                                    HitCount = 1;
                                    break;
...
void CObjUseSkill::SplashDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int nSkill, int nAttackDamage, int nDistance,
                                int nDamageRate) {
            ...
            while (true) {
                if (lpObj->VpPlayer2[iCount].state) {
                    ...
                    if (iTarObjNum >= 0) {
                        ...
                        if (lpTargetObj->m_cChaosCastleIndex != -1 &&
    ...
    if (gObj[aIndex].CloseCount < 0 &&
        ...
        for (int i = 0; i < 5; i++) {
            if (wTargetList[i] != 10000) {
                ...
                gObjAddAttackProcMsgSendDelay(&gObj[aIndex], 61, aIndex, 150 + (iTargetCount * 150), lpMagic->m_Skill,
                                              wTargetList[i]);
                iTargetCount++;
            }
        }
    ...
    while (true) {
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0) {
                ...
                if (EnableAttack != 0) {
                    if (gObjCalDistance(lpTargetObj, &gObj[tObjNum]) <=
                        MagicDamageC.GetSkillDistance(lpMagic->m_Skill)) // 2x2 around target
                    {
                        gObjAttack(lpObj, &gObj[tObjNum], lpMagic, 1, 0, 0, 0, 0, 0);
                    }
                }
...
void CObjUseSkill::SkillGrowLancerAttackOneTarget(int aIndex, int aTargetIndex, CMagicInf *lpMagic) {
    ...
    if (ObjectMaxRange(aTargetIndex) == FALSE) {
        g_Log.Add("[InvalidTargetIndex][SkillGrowLancerAttackOneTarget] Index :%d , AccountID : %s", aIndex,
                  gObj[aIndex].AccountID);
        return;
    }
...
int CObjUseSkill::SkillBuffGrowLancer(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    if (lpMagic->m_Skill == AT_SKILL_WRATH) {
        ...
        this->m_Lua.Generic_Call("GrowLancerWrath", "iii>iii", lpObj->m_PlayerData->Strength + lpObj->AddStrength,
                                 lpObj->m_PlayerData->Dexterity + lpObj->AddDexterity,
                                 lpObj->m_PlayerData->Energy + lpObj->AddEnergy,
                                 &skill_improve, &skill_improve2, &skill_time);

        gObjAddBuffEffect(lpObj, BuffIndex, EFFECTTYPE_WRATH_INC_DAMAGE, skill_improve, EFFECTTYPE_DECREASE_DEFENSE,
                          skill_improve2, skill_time);
    } else if (lpMagic->m_Skill == AT_SKILL_CIRCLE_SHIELD) {
        BuffIndex = BUFFTYPE_CIRCLE_SHIELD;
        this->m_Lua.Generic_Call("GrowLancerCircleShield", "iii>iii",
...
int CObjUseSkill::SkillBuffPartyGrowLancer(int aIndex, CMagicInf *lpMagic, int aTargetIndex) {
    ...
    gObjUseSkill.m_Lua.Generic_Call("GrowLancerObsidian", "iii>ii",
                                    (lpObj->m_PlayerData->Strength + lpObj->AddStrength),
                                    (lpObj->m_PlayerData->Dexterity + lpObj->AddDexterity),

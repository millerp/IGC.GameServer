...
void CDoppelGangerItemBag::LoadDoppelgangerItemBag(LPSTR lpFile) {
    ...
    for (pugi::xml_node section = section_rates.child("Section"); section; section = section.next_sibling()) {
        ...
        this->vtDPItemBag.insert(
                std::pair<int, __tagDOPPELGANGER_DROP_ITEM_GROUP_INFO>(tmpItemGroupInfo.nGroupIndex, tmpItemGroupInfo));
    }
    ...
    for (pugi::xml_node item_section = main.child(
            "Section"); item_section; item_section = item_section.next_sibling()) {
...
BOOL CDoppelGangerItemBag::DoppelgangerItemDrop(OBJECTSTRUCT *lpObj, int nItemBagIndex, int nDropRate, BYTE btFlag) {
    ...
    if (bItemDrop == FALSE) {
        ...
        if (pItemBagAttr->m_maxLevel == pItemBagAttr->m_minLevel) {
            ...
            iLevel = GetLargeRand() % (pItemBagAttr->m_maxLevel - pItemBagAttr->m_minLevel + 1) +
                     pItemBagAttr->m_minLevel;
        }
            ...
            if (GetLargeRand() % 1000000 < this->vtDPItemBag[nTmpItemBagIndex].nAddOptRate) {
                ...
                if (dwOptionTemp <= this->m_nAddOptRate[0]) {
                    ...
                    iOption3 = 3;
                                           ...
                                           this->m_nAddOptRate[3]) {
                    iOption3 = 4;
                } else if (dwOptionTemp <= this->m_nAddOptRate[0] + this->m_nAddOptRate[1] + this->m_nAddOptRate[2] +
                                           this->m_nAddOptRate[3] + this->m_nAddOptRate[4]) {
                    iOption3 = 5;
                } else if (dwOptionTemp <= this->m_nAddOptRate[0] + this->m_nAddOptRate[1] + this->m_nAddOptRate[2] +
                                           this->m_nAddOptRate[3] + this->m_nAddOptRate[4] + this->m_nAddOptRate[5]) {
                    iOption3 = 6;
                } else if (dwOptionTemp <= this->m_nAddOptRate[0] + this->m_nAddOptRate[1] + this->m_nAddOptRate[2] +
                                           this->m_nAddOptRate[3] + this->m_nAddOptRate[4] + this->m_nAddOptRate[5] +
                                           this->m_nAddOptRate[6]) {
                    iOption3 = 7;
                }
            ...
            if (bExItemDrop == TRUE) {
                ...
                if (nExOptAddRate >= this->m_nExItemAddOptRate[0]) {
                    if (nExOptAddRate >= this->m_nExItemAddOptRate[1] + this->m_nExItemAddOptRate[0]) {
                        if (nExOptAddRate >= this->m_nExItemAddOptRate[2] + this->m_nExItemAddOptRate[1] +
                                             this->m_nExItemAddOptRate[0]) {
                            if (nExOptAddRate >= this->m_nExItemAddOptRate[3] + this->m_nExItemAddOptRate[2] +
                                                 this->m_nExItemAddOptRate[1] + this->m_nExItemAddOptRate[0]) {
                                if (nExOptAddRate >= this->m_nExItemAddOptRate[4] + this->m_nExItemAddOptRate[3] +
                                                     this->m_nExItemAddOptRate[2] + this->m_nExItemAddOptRate[1] +
                                                     this->m_nExItemAddOptRate[0]) {
                                    if (nExOptAddRate < this->m_nExItemAddOptRate[5] + this->m_nExItemAddOptRate[4] +
                                                        this->m_nExItemAddOptRate[3] + this->m_nExItemAddOptRate[2] +
                                                        this->m_nExItemAddOptRate[1] + this->m_nExItemAddOptRate[0]) {
                                        nExOptionCount = 6;
                                    }
                                } else {
                ...
                while (LoopCnt-- != 0) {
                    ...
                    if ((iExOption & (1 << nExOptIndex)) != 1 << nExOptIndex &&
                        (GetLargeRand() % 1000000) < this->m_nExItemOptKindRate[nExOptIndex]) {
        ...
        g_Log.Add(
                "[Doppelganger] Drop Item(%d) [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
                btFlag, lpObj->AccountID, lpObj->Name, lpObj->MapNumber, cDropX, cDropY, ItemAttribute[iType].Name,

...
void CBuffEffect::SetBuffEffect(LPOBJECTSTRUCT lpObj, BYTE EffectType, int EffectValue) {
    ...
    switch (EffectType) {
        ...
        case EFFECTTYPE_MANA:
            ...
            GSProtocol.GCManaSend(lpObj->m_Index, (lpObj->MaxMana + lpObj->AddMana), 0xFE, 0,
        ...
        case EFFECTTYPE_BERSERKER_DOWN: {
            ...
            lpObj->Life = ((float) (lpObj->Life) < ((float) (lpObj->AddLife) + (float) (lpObj->MaxLife)))
                          ? ((float) (lpObj->Life)) : (((float) (lpObj->AddLife) + (float) (lpObj->MaxLife)));
        ...
        case EFFECTTYPE_AG_UP:
            ...
            GSProtocol.GCManaSend(lpObj->m_Index, (WORD)(lpObj->MaxMana + lpObj->AddMana), 0xFE, 0,
                                  (WORD)(lpObj->MaxBP + lpObj->AddBP));
...
void CBuffEffect::ClearBuffEffect(LPOBJECTSTRUCT lpObj, BYTE EffectType, int EffectValue) {
    ...
    switch (EffectType) {
        ...
        case EFFECTTYPE_BERSERKER_UP:
            ...
            if (lpObj->AddMana <= 0.0)
                lpObj->AddMana = 0.0;
            lpObj->Mana = ((float) (lpObj->Mana) < ((float) (lpObj->AddMana) + (float) (lpObj->MaxMana)))
                          ? ((float) (lpObj->Mana)) : (((float) (lpObj->AddMana) + (float) (lpObj->MaxMana)));
...
void CBuffEffect::SetActiveBuffEffect(class OBJECTSTRUCT *lpObj, BYTE EffectType, int EffectValue) {
    ...
    switch (EffectType) {
        ...
        case EFFECTTYPE_POISON_DMG_TICK:
            ...
            break;
        case EFFECTTYPE_FILLHP:
            GiveDamageFillHPEffect(lpObj, EffectValue);
            break;
    }
}
...
void CBuffEffect::RequestGuildPeriodBuffInsert(char *szGuildName, _tagPeriodBuffInfo *lpBuffInfo) {
    ...
    g_Log.Add(
            "[PeriodBuff][Insert] Request Insert Guild PeriodBuff. GuildName : %s, BuffIndex : %d, Duration : %d, lExpireDate : %d",
...
void CBuffEffect::RequestPeriodBuffDelete(OBJECTSTRUCT *lpObj, WORD wBuffIndex) {
    ...
    g_Log.Add("[PeriodBuff][Delete] Request Delete PeriodBuff. User Id : %s(%d), Name : %s, BuffIndex : %d",
              lpObj->AccountID, lpObj->DBNumber, lpObj->Name, wBuffIndex);
}
...
void CBuffEffect::RequestPeriodBuffInsert(OBJECTSTRUCT *lpObj, _tagPeriodBuffInfo *lpBuffInfo) {
    ...
    wsDataCli.DataSend((char *) &pMsg, pMsg.head.size);
    g_Log.Add(
            "[PeriodBuff][Insert] Request Insert PeriodBuff. User Id : %s(%d), Name : %s, BuffIndex : %d, Duration %d, lExpireDate%d",
            lpObj->AccountID, lpObj->DBNumber, lpObj->Name, lpBuffInfo->wBuffIndex, lpBuffInfo->lDuration,
            pMsg.lExpireDate);
}
...
void CBuffEffect::DGPeriodItemExSelect(PMSG_ANS_PERIODBUFF_SELECT *lpMsg) {
    ...
    if (gObjAddPeriodBuffEffect(lpObj, lpPeriBuff, lLeftDate) == FALSE) {
        g_Log.Add(
                "[PeriodBuff][Error][Select] Answer Select PeriodBuff. User Id : %s(%d), Name : %s, BuffIndex : %d Type1 : %d Type2 : %d ExpireDate : %d ResultCode : %d",
                lpObj->AccountID, lpObj->DBNumber, lpObj->Name, lpMsg->wBuffIndex, lpMsg->btEffectType1,
                lpMsg->btEffectType2, lpMsg->lExpireDate, lpMsg->btResultCode);
    } else {
        g_Log.Add(
                "[PeriodBuff][Select] Answer Select PeriodBuff. User Id : %s(%d), Name : %s, BuffIndex : %d Type1 : %d Type2 : %d ExpireDate : %d ResultCode : %d",
                lpObj->AccountID, lpObj->DBNumber, lpObj->Name, lpMsg->wBuffIndex, lpMsg->btEffectType1,
                lpMsg->btEffectType2, lpMsg->lExpireDate, lpMsg->btResultCode);
    }
}
...
void GensSystem::BattleZoneChatMsgSend(LPOBJ lpObj, LPBYTE Msg, int size) {
    ...
    if (this->IsMapBattleZone(lpObj->MapNumber)) {
        ...
        for (int n = 0; n < MAX_VIEWPORT; ++n) {
            if (lpObj->VpPlayer[n].type == OBJ_USER && lpObj->VpPlayer[n].state != 0 &&
                (this->GetGensInfluence(&gObj[lpObj->VpPlayer[n].number]) == iTarInfluence ||
                 CheckAuthorityCondition(98, lpObj) || CheckAuthorityCondition(98, &gObj[lpObj->VpPlayer[n].number]) ||
                 this->CanGensSeeOppositeChat() == TRUE)) {
...
int GensSystem::GetGensInfluence(LPOBJ lpObj) {
    ...
    if (lpObj->m_PlayerData->m_GensInfluence == LEAVED_INFLUENCE ||
        lpObj->m_PlayerData->m_GensInfluence < NONE_INFLUENCE ||
        lpObj->m_PlayerData->m_GensInfluence > VANERT_INFLUENCE) {
        g_Log.Add("[GensSystem] Error [%s][%s] GensInfluence [%d]", lpObj->AccountID, lpObj->Name,
                  lpObj->m_PlayerData->m_GensInfluence);
        return 0;
    }
...
void GensSystem::CalcContributePoint(LPOBJ lpObj, LPOBJ lpTargetObj) {
    ...
    if (lpObj->Type && lpTargetObj->Type && this->IsMapBattleZone(lpObj->MapNumber) &&
        this->IsMapBattleZone(lpTargetObj->MapNumber)) {
                ...
                if (this->ChkKillUserName(lpObj, szName) == 0) {
                    ...
                    if (iKillerContributePoint > 0 || iContributePoint <= 0) {
                        ...
                        if (iGensClassGap == 1) {
                            ...
                            if (lpObj->m_PlayerData->m_GensClass > 9 &&
                                iAddBonusValue + iAddValue >= this->GetNextContributePoint(lpObj)) {
                            ...
                            g_Log.Add(
                                    "[GensSystem] [%s][%s] Add ContributePoint[%d] - Result : Lv [%d] GensName [%s] / Lv [%d] GensName [%s]",
                                    lpObj->AccountID, lpObj->Name, iAddValue, lpObj->Level,
                                    this->GetGensInfluenceName(lpObj), lpTargetObj->Level,
                                    this->GetGensInfluenceName(lpTargetObj));

                            this->SubContributePoint(lpTargetObj, iSubValue);

                            g_Log.Add(
                                    "[GensSystem] [%s][%s] Sub ContributePoint[%d] - Result : Lv [%d] GensName [%s] / Lv [%d] GensName [%s]",
                                    lpTargetObj->AccountID, lpTargetObj->Name, iSubValue, lpTargetObj->Level,
                                    this->GetGensInfluenceName(lpTargetObj), lpObj->Level,
                                    this->GetGensInfluenceName(lpObj));
                            ...
                            if (iAddBonusValue > 0) {
                                ...
                                g_Log.Add(
                                        "[GensSystem] [%s][%s] Add BonusCP[%d] - Result : Lv [%d] GensName [%s] / Lv [%d] GensName [%s]",
                                        lpObj->AccountID, lpObj->Name, iAddBonusValue, lpObj->Level,
                                        this->GetGensInfluenceName(lpObj), lpTargetObj->Level,
                                        this->GetGensInfluenceName(lpTargetObj));
                            }
                        }
...
int GensSystem::InsertKillUserName(LPOBJ lpObj, char *szCharName) {
    ...
    for (int i = 0; i < lpObj->m_PlayerData->m_KillUserTotCnt; ++i) {
        g_Log.Add("[GensSystem] Abusing List [%s][%s] KillUserName[%s] Cnt[%d]", lpObj->AccountID, lpObj->Name,
                  &lpObj->m_PlayerData->KillUserName[iKillUserToCnt * 11],
                  lpObj->m_PlayerData->m_KillUserCnt[iKillUserToCnt]);
        iKillUserToCnt = i + 1;
    }
...
void GensSystem::AbusingInfoReset(LPOBJ lpObj) {
    ...
    if (gObjIsConnected(lpObj->m_Index)) {
        if (this->IsRegGensInfluence(lpObj)) {
            if (this->GetAbusingResetTime() > 0 &&
                GetTickCount64() - lpObj->m_PlayerData->m_GensAbusingTick >= 60000 * this->GetAbusingResetTime()) {
                EnterCriticalSection((LPCRITICAL_SECTION) & criti);
...
int GensSystem::SendGensRewardItem(LPOBJ lpObj, int iGensClass) {
    ...
    if (iArrayClass >= 0) {
        if (this->m_GensRanking[iArrayClass].RewardItemCount > 0) {
            ...
            int ItemNumber = ItemGetNumberMake(this->m_GensRanking[iArrayClass].RewardItemGroup,
                                               this->m_GensRanking[iArrayClass].RewardItemNumber);
...
void GensSystem::SendBattleZoneData(LPOBJ lpObj) {
    ...
    if (g_ConfigRead.server.IsNonPvP() == FALSE) {
        for (std::map<int, BATTLEZONE_MAP_DATA>::iterator It = this->m_mapBattleZoneData.begin();
             It != this->m_mapBattleZoneData.end(); It++) {

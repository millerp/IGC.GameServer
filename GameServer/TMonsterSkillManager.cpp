...
BOOL TMonsterSkillManager::AddMonsterSkillDelayInfo(int iIndex, int iTargetIndex, int iDelayTime,
...
void
TMonsterSkillManager::UseMonsterSkill(int iIndex, int iTargetIndex, int iMonsterSkillUnitType, int iMonsterSkillUnit,
                                      CMagicInf *lpMagic) {
                ...
                if (ObjectMaxRange(iRangeTargetIndex)) {
                    if (gObj[iRangeTargetIndex].Type == OBJ_USER) {
                        ...
                        if (lpMonsterSkillUnit->m_iUnitScopeType == 0) {
                            if (gObjCalDistance(lpObj, &gObj[iRangeTargetIndex]) <
                                lpMonsterSkillUnit->m_iUnitScopeValue) {
                                bTargetOK = TRUE;
                            }
                        } else if (lpMonsterSkillUnit->m_iUnitScopeType == 1) {
                            if (SkillElectricSparkHitBox.HitCheck(iAngle, lpObj->X, lpObj->Y,
                                                                  gObj[iRangeTargetIndex].X,
                                                                  gObj[iRangeTargetIndex].Y)) {
                                bTargetOK = TRUE;
                            }

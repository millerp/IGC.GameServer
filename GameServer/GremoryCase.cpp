                        ...
                        for (int k = 0; k < MAX_GREMORYCASE_STORAGE_ITEMS; k++) {
                            if (gObj[i].m_PlayerData->m_GremoryCaseData[j][k].btRewardInventory != 0) {
                                if (gObj[i].m_PlayerData->m_GremoryCaseData[j][k].dwExpireTime < CurrTime) {
                                    this->GDReqDeleteItemFromGremoryCase(i,
                                                                         gObj[i].m_PlayerData->m_GremoryCaseData[j][k].ItemInfo.m_Type,
                                                                         gObj[i].m_PlayerData->m_GremoryCaseData[j][k].dwItemGUID,
                                                                         gObj[i].m_PlayerData->m_GremoryCaseData[j][k].dwAuthCode);
                                    this->GCSendDeleteItemFromGremoryCase(i, j, k);

                                    g_Log.Add(
                                            "[Gremory Case] (%s)(%s) Deleted Expired Item (Storage Type: %d) (ItemGUID: %d) (AuthCode: %d)",
                                            gObj[i].AccountID, gObj[i].Name, j,
                                            gObj[i].m_PlayerData->m_GremoryCaseData[j][k].dwItemGUID,
                                            gObj[i].m_PlayerData->m_GremoryCaseData[j][k].dwAuthCode);
...
void CGremoryCase::GDReqStorageItemList(int iIndex) {
    ...
    g_Log.Add("[Gremory Case] (%s)(%s) Request to load Gremory Case item list.", gObj[iIndex].AccountID,
              gObj[iIndex].Name);
}
    ...
    for (int i = 0; i < lpMsg->btCount; i++) {
        ...
        for (int j = 0; j < MAX_GREMORYCASE_STORAGE_ITEMS; j++) {
            if (lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].btRewardInventory == 0) {
                ...
                lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType -
                                                       1][j].btRewardInventory = lpItem->btStorageType;
                lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType -
                                                       1][j].btRewardSource = lpItem->btRewardSource;
                ...
                lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType -
                                                       1][j].dwReceiveDate = lpItem->iReceiveDate;
                ...
                lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType -
                                                       1][j].ItemInfo.m_Level = lpItem->btItemLevel;
                lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType -
                                                       1][j].ItemInfo.m_Durability = ItemGetDurability(lpItem->wItemID,
                                                                                                       lpItem->btItemLevel,
                                                                                                       lpItem->btItemExcOption,
                                                                                                       lpItem->btItemSetOption);

                if (g_SocketOptionSystem.IsEnableSocketItem(
                        &lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo) == true) {
                    if (lpItem->btItemSocketCount == 0) {
                        g_SocketOptionSystem.MakeSocketSlot(
                                &lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo,
                                rand() % 5 + 1);
                    } else {
                        g_SocketOptionSystem.MakeSocketSlot(
                                &lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo,
                                lpItem->btItemSocketCount);
                    }

                    SocketOptionIndex = -1;
                } else if (g_PentagramSystem.IsPentagramItem(
                        lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo.m_Type) == true) {
                    SocketOptionIndex = lpItem->btItemMainAttribute;

                    if (lpItem->btItemSocketCount == 0) {
                        g_PentagramSystem.GetMakePentagramSlotCountNKind(SocketOption,
                                                                         lpObj->m_PlayerData->m_GremoryCaseData[
                                                                                 lpItem->btStorageType -
                                                                                 1][j].ItemInfo.m_Type);
                    } else {
                        ...
                        g_PentagramSystem.MakePentagramSocketSlot(
                                &lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo, Slot[0],
                                Slot[1], Slot[2], Slot[3], Slot[4]);
                    }
                } else if (g_CMuunSystem.IsStoneofEvolution(
                        lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo.m_Type) == TRUE) {
                ...
                lpObj->m_PlayerData->m_GremoryCaseData[lpItem->btStorageType - 1][j].ItemInfo.Convert(lpItem->wItemID,
                                                                                                      lpItem->btItemSkill,
                                                                                                      lpItem->btItemLuck,
                                                                                                      lpItem->btItemOption,
                                                                                                      lpItem->btItemExcOption,
...
void CGremoryCase::GDReqAddItemToGremoryCase(int iIndex, _stGremoryCaseItem stItem, int iDaysToExpire) {
    ...
    wsDataCli.DataSend((char *) &pMsg, pMsg.h.size);
    g_Log.Add(
            "[Gremory Case] (%s)(%s) Request to Add item to Storage (Storage Type: %d) (Reward Source: %d) (ItemID: %d) (ExpireDays: %d)",
...
void CGremoryCase::DGAnsAddItemToGremoryCase(_stAnsAddItemToGremoryCase *lpMsg) {
    ...
    for (int i = 0; i < MAX_GREMORYCASE_STORAGE_ITEMS; i++) {
        if (lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType - 1][i].btRewardInventory ==
            0) {
            iItemArrayIndex = i;
            ...
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].btRewardInventory = lpMsg->m_GremoryCaseItem.btStorageType;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].btRewardSource = lpMsg->m_GremoryCaseItem.btRewardSource;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].dwItemGUID = lpMsg->m_GremoryCaseItem.btItemGUID;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].dwAuthCode = lpMsg->m_GremoryCaseItem.dwAuthCode;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].dwReceiveDate = lpMsg->m_GremoryCaseItem.iReceiveDate;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].dwExpireTime = lpMsg->m_GremoryCaseItem.iExpireDate;

            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].ItemInfo.m_Type = lpMsg->m_GremoryCaseItem.wItemID;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].ItemInfo.m_Level = lpMsg->m_GremoryCaseItem.btItemLevel;
            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                   1][i].ItemInfo.m_Durability = ItemGetDurability(
                    lpMsg->m_GremoryCaseItem.wItemID, lpMsg->m_GremoryCaseItem.btItemLevel,
                    lpMsg->m_GremoryCaseItem.btItemExcOption, lpMsg->m_GremoryCaseItem.btItemSetOption);

            if (g_SocketOptionSystem.IsEnableSocketItem(
                    &lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType - 1][i].ItemInfo) ==
                true) {
                if (lpMsg->m_GremoryCaseItem.btItemSocketCount == 0) {
                    g_SocketOptionSystem.MakeSocketSlot(
                            &lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                                    1][i].ItemInfo, rand() % 5 + 1);
                } else {
                    g_SocketOptionSystem.MakeSocketSlot(
                            &lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                                    1][i].ItemInfo,
                            lpMsg->m_GremoryCaseItem.btItemSocketCount);
                }

                SocketOptionIndex = -1;
            } else if (g_PentagramSystem.IsPentagramItem(
                    lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                           1][i].ItemInfo.m_Type) == true) {
                SocketOptionIndex = lpMsg->m_GremoryCaseItem.btItemMainAttribute;

                if (lpMsg->m_GremoryCaseItem.btItemSocketCount == 0) {
                    g_PentagramSystem.GetMakePentagramSlotCountNKind(SocketOption,
                                                                     lpObj->m_PlayerData->m_GremoryCaseData[
                                                                             lpMsg->m_GremoryCaseItem.btStorageType -
                                                                             1][i].ItemInfo.m_Type);
                } else {
                    BYTE Slot[5] = {0, 0, 0, 0, 0};
                    ...
                    g_PentagramSystem.MakePentagramSocketSlot(
                            &lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                                    1][i].ItemInfo, Slot[0], Slot[1], Slot[2], Slot[3],
                            Slot[4]);
                }
            } else if (g_CMuunSystem.IsStoneofEvolution(
                    lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType -
                                                           1][i].ItemInfo.m_Type) == TRUE) {
                int iMuunEvolutionItemID = ITEMGET(lpMsg->m_GremoryCaseItem.wMuunEvoItemType,
                                                   lpMsg->m_GremoryCaseItem.btMuunEvoItemIndex);

                SocketOption[0] = iMuunEvolutionItemID >> 8;
                SocketOption[1] = ITEM_GET_INDEX(iMuunEvolutionItemID);
            }

            lpObj->m_PlayerData->m_GremoryCaseData[lpMsg->m_GremoryCaseItem.btStorageType - 1][i].ItemInfo.Convert(
                    lpMsg->m_GremoryCaseItem.wItemID, lpMsg->m_GremoryCaseItem.btItemSkill,
                    lpMsg->m_GremoryCaseItem.btItemLuck, lpMsg->m_GremoryCaseItem.btItemOption,
                    lpMsg->m_GremoryCaseItem.btItemExcOption, lpMsg->m_GremoryCaseItem.btItemSetOption, 0, SocketOption,
                    SocketOptionIndex, 0, 3);
            break;
        }
    }

    if (iItemArrayIndex == -1) {
        g_Log.Add("[Gremory Case] (%s)(%s) Failed to Add Item to Storage - No space in array", lpObj->AccountID,
                  lpObj->Name);
        return;
    }

    g_Log.Add(
            "[Gremory Case] (%s)(%s) Success to Add Item to Storage (Storage: %d) (Source: %d) (ItemGUID: %d) (AuthCode: %d)",
            lpObj->AccountID, lpObj->Name, lpMsg->m_GremoryCaseItem.btStorageType,
            lpMsg->m_GremoryCaseItem.btRewardSource, lpMsg->m_GremoryCaseItem.btItemGUID,
            lpMsg->m_GremoryCaseItem.dwAuthCode);
...
void CGremoryCase::DGAnsCheckItemUseGremoryCase(_stAnsCheckUseItemGremoryCase *lpMsg) {
    ...
    if (lpObj->m_PlayerData->m_bGremoryCaseDBInfo == false) {
        ...
        g_Log.Add("[Gremory Case] (%s)(%s) Failed to Add Item to Inventory - Data not loaded from DB", lpObj->AccountID,
                  lpObj->Name);
        return;
    }
    ...
    if (iStorageType == -1 || iItemArrayIndex == -1) {
        ...
        g_Log.Add("[Gremory Case] (%s)(%s) Failed to Add Item to Inventory - Item not found", lpObj->AccountID,
                  lpObj->Name);
        return;
    }
    ...
    if (lpMsg->btResult == 0) {
        ...
        g_Log.Add("[Gremory Case] (%s)(%s) Failed to Add Item to Inventory - Item not Exists in DB", lpObj->AccountID,
                  lpObj->Name);
        return;
    }
    ...
    if (IsInventoryEmptySpace == false) {
        ...
        g_Log.Add("[Gremory Case] (%s)(%s) Failed to Add Item to Inventory - Not enough empty space", lpObj->AccountID,
                  lpObj->Name);
        return;
    }
    ...
    if (lpObj->m_IfState.use) {
        ...
        g_Log.Add("[Gremory Case] (%s)(%s) Failed to Add Item to Inventory - IfState check error", lpObj->AccountID,
                  lpObj->Name);
        return;
    }

    if (p->ItemKindA == 11) {
        ItemSerialCreateSend(iIndex, 226, 0, 0,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Type,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Level,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Durability,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Option1,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Option2,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Option3,
                             iIndex,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_NewOption,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_SetOption,
                             0,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_SocketOption,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_BonusSocketOption);

    } else if (p->ItemKindA == 12) {
        BYTE SocketOption[5] = {-1, -1, -1, -1, -1};

        ItemSerialCreateSend(iIndex, 224, 0, 0,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Type,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Level,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Durability,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Option1,
        ...
        ItemSerialCreateSend(iIndex, 235, 0, 0,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Type,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Level,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Durability,
                             lpObj->m_PlayerData->m_GremoryCaseData[iStorageType][iItemArrayIndex].ItemInfo.m_Option1,
...
void CGremoryCase::GCSendStorageItemList(int iIndex) {
    ...
    for (int i = 0; i < MAX_GREMORYCASE_STORAGE_TYPES; i++) {
        for (int j = 0; j < MAX_GREMORYCASE_STORAGE_ITEMS; j++) {
            if (lpObj->m_PlayerData->m_GremoryCaseData[i][j].btRewardInventory != 0) {
                lpItem = (PMSG_GREMORYCASE_ITEM *) (BUFFER + sizeof(PMSG_RECEIVE_GREMORYCASE_ITEMLIST) +
                                                    (lpMsg->btCount * sizeof(PMSG_GREMORYCASE_ITEM)));
...
void CGremoryCase::GCSendAddItemToGremoryCase(int iIndex, BYTE btStorageType, BYTE btItemArrayIndex) {
    ...
    ItemByteConvert(pMsg.m_ReceivedItem.btItemInfo,
                    lpObj->m_PlayerData->m_GremoryCaseData[btStorageType][btItemArrayIndex].ItemInfo);
...
void CGremoryCase::CheckIsStorageFull(int iIndex) {
    ...
    if (iServerStorageCount >= MAX_GREMORYCASE_STORAGE_ITEMS ||
        iCharacterStorageCount >= MAX_GREMORYCASE_STORAGE_ITEMS) {
        btStorageState = 1;
    } else if ((iServerStorageCount >= MAX_GREMORYCASE_STORAGE_ITEMS - 5 &&
                iServerStorageCount < MAX_GREMORYCASE_STORAGE_ITEMS) ||
               (iCharacterStorageCount >= MAX_GREMORYCASE_STORAGE_ITEMS - 5 &&
                iCharacterStorageCount < MAX_GREMORYCASE_STORAGE_ITEMS)) {
        btStorageState = 2;
    }

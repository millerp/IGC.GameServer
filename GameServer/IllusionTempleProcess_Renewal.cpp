...
void CIllusionTempleProcess_Renewal::Proc_ITRState_None(int nCurTime) {
    ...
    if (iTICK_MSEC >= 1000) {
        ...
        if (this->m_nRemainTimeMin <= g_IT_Event.m_nITROpenTime && this->m_nRemainTimeMsec > 5000 &&
            this->m_bEntrance == false) {
            this->m_bEntrance = 1;
        }
        ...
        if (this->m_nUserCount >= 2 && bCanStart) {
            ...
            for (int nIndex = 0; nIndex < 10; nIndex++) {
                if (this->m_UserData[nIndex].m_nIndex != -1 &&
                    ...
                    gObj[this->m_UserData[nIndex].m_nIndex].MapNumber == this->m_nTempleNumber + 45) {
...
void CIllusionTempleProcess_Renewal::Proc_ITRState_End(int nCurTime) {
    ...
    if (this->m_nRemainTimeMsec <= 60000 * g_IT_Event.m_nITREndTime - 6000 && this->m_nRemainTimeMsec > 0 &&
        this->m_bGameEndPosMove == true && this->m_bSendResult == false) {
        if (this->m_bITRRoundStartFail == false) {
            this->SendITRResult();
        }
...
void CIllusionTempleProcess_Renewal::Set_ITRState_End() {
    ...
    g_Log.Add("[ ITR ][WarResult] Allied Team : %d , IllusionTeam :%d", this->m_byAlliedTeamPoint,
...
BOOL CIllusionTempleProcess_Renewal::LeaveUserIllusionTemple(int aIndex) {
    ...
    if (gObj[aIndex].m_nITR_Index != -1 && this->m_UserData[gObj[aIndex].m_nITR_Index].m_nIndex == aIndex &&
        ...
        LeaveCriticalSection(&this->m_critUserData);
        g_Log.Add("[ ITR ] (%d) LeaveUser: (%s)(%s)", this->m_nTempleNumber + 1, gObj[aIndex].AccountID,
                  gObj[aIndex].Name);
    }
...
void CIllusionTempleProcess_Renewal::DropRelicsItem(int iIndex, BYTE byAct) {
    ...
    if (MapC[map_num].ItemDrop(type, level, dur, X, Y, Option1, Option2, Option3, NOption, SOption, item_number,
        ...
        g_Log.Add("[ ITR ] (%d) (%s)(%s) to Drop Relics Item [Serial:%I64d]", this->m_nTempleNumber + 1,
...
void CIllusionTempleProcess_Renewal::AddITRNpcInfo(WORD wNpcType, BYTE byMapNumber, BYTE byDistance, BYTE byX, BYTE byY,
                                                   BYTE byDir) {
...
void CIllusionTempleProcess_Renewal::Check_RegisterRelics(OBJECTSTRUCT *lpNpc, OBJECTSTRUCT *lpObj) {
    ...
    if (abs(lpObj->Y - lpNpc->Y) > 3 || abs(lpObj->X - lpNpc->X) > 3) {
        g_Log.AddC(TColor::Red, "[ITR] Error (%s) - abs > 3 (%d/%d) (%d/%d)", __FUNCTION__, lpObj->Y, lpNpc->Y,
                   lpObj->X, lpNpc->X);
        return;
    }
                        ...
                        if (this->m_Occupied_StoneStatus[i].m_wType == lpNpc->Class &&
                            ...
                            if (this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos > 0) {
                                if (lpObj->pInventory[this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos].m_Type ==
                                    ...
                                    this->SendRelicsUserInfo(lpObj, 1);
                                    g_Log.Add(
                                            "[ ITR ][RegisterRelics] (%d) (%s)(%s) to Delete Relics Item [Serial:%I64d]",
                                            this->m_nTempleNumber + 1, lpObj->AccountID, lpObj->Name,
    ...
    for (int i = 0; i < 11; i++) {
        ...
        if (result > 0) {
            ...
            if (this->m_ITRNpcInfo[i].wNpcType == 658) {
                ...
                this->AddITROccupiedStonePos(this->m_ITRNpcInfo[i].wNpcType, result, nOccCount,
                                             this->m_ITRNpcInfo[i].byMapTagIndex);
                nOccCount++;
            }
        }
    }
        ...
        if (this->m_Occupied_StoneStatus[i].m_nIndex != -1) {
            ...
            if (this->m_Occupied_StoneStatus[i].m_byStatus == 10 ||
                ...
                g_Log.Add("[ ITR ][RegenOccupiedStoneStatus] Round [%d] NpcIndex:[%d],Type:[%d] MAKE NEUTRAL",
                          this->m_byITRRound, this->m_Occupied_StoneStatus[i].m_nIndex,
                          this->m_Occupied_StoneStatus[i].m_wType);
            }
        }
    }
...
void CIllusionTempleProcess_Renewal::Send_ITR_StoneInfo(int userindex, WORD wNpcType, BYTE byState, int NpcIndex,
                                                        BYTE byMapTagIndex) {
...
void CIllusionTempleProcess_Renewal::IncUserKillCount(int nIndex) {
    ...
    g_Log.Add("[ ITR ][UserKill] map:%d, ACC:%s, NAME:%s ,KillCnt:%d -> %d",
              ...
              this->m_UserData[gObj[nIndex].m_nITR_Index].m_btUserKillCount + 1);
...
void
CIllusionTempleProcess_Renewal::SendUseSkillResult(int nIndex, int nTargetIndex, WORD wSkillNumber, BYTE btResult) {
...
void CIllusionTempleProcess_Renewal::CalCursetTempleResult() {
    ...
    if (byWinnerGuildIndex != 255 && byLoseGuildIndex != 255) {
        g_Log.Add("[ ITR ] (%d) Result (WinTeam: %d)(Score:[%d][%d])", this->m_nTempleNumber + 1, this->m_btWinTeam,
                  byAlliedCnt, byIllusionCnt);
    } else {
        g_Log.Add("[ ITR ] (%d) Result (Team: %d)(Score:[%d][%d]) byWinnerGuildIndex :%d,byLoseGuildIndex : %d ",
...
void CIllusionTempleProcess_Renewal::DropITR_RewardBox(int nIndex) {
    ...
    g_QuestExpProgMng.ChkUserQuestTypeEventMap(QUESTEXP_ASK_ILLUSIONTEMPLE_CLEAR, &gObj[nIndex], this->m_nTempleNumber,
                                               0);
    ...
    for (int j = 0; j < 10; j++) {
        if (this->m_UserData[j].m_nIndex != -1) {
            if (gObj[this->m_UserData[j].m_nIndex].Connected > PLAYER_LOGGED) {
                ...
                g_Log.Add(
                        "[ ITR ] MAP:(%d) ITR Result  (Account:%s, Name:%s, G.Name:%s Team:%d, Class:%d, UserKillCount:%d,InvalidationCount:%d,OccupiedCount:%d)",
                        this->m_nTempleNumber + 1, gObj[this->m_UserData[j].m_nIndex].AccountID,
                        gObj[this->m_UserData[j].m_nIndex].Name,
                        gObj[this->m_UserData[j].m_nIndex].m_PlayerData->GuildName,
                        this->m_UserData[j].m_btTeam, gObj[this->m_UserData[j].m_nIndex].m_PlayerData->DbClass,
                        this->m_UserData[j].m_btUserKillCount, this->m_UserData[j].m_byInvalidationCount,
...
void CIllusionTempleProcess_Renewal::ResetUserFlag_DoingRelicsThing(OBJECTSTRUCT *lpObj) {
    ...
    g_Log.Add(
            "[ ITR ][ResetUserFlag_DoingRelicsThing] NAME:%s, StoneState:%d, NPCTYPE:%d, GettingRelics:%d, RegisteringRelics:%d",
            lpObj->Name, lpObj->m_byITR_StoneState, lpObj->m_wITR_NpcType, lpObj->m_bITR_GettingRelics,
            lpObj->m_bITR_RegisteringRelics);

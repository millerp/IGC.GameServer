...
void classdef::DGAnsClassDefData(PMSG_ANS_CLASSDEF *lpMsg) {
    for (int i = 0; i < lpMsg->iCount; i++) {
        if (lpMsg->m_ClassData[i].Class != -1) {
            this->SetCharacter(lpMsg->m_ClassData[i].Class, lpMsg->m_ClassData[i].Str, lpMsg->m_ClassData[i].Dex,
                               lpMsg->m_ClassData[i].Vit, lpMsg->m_ClassData[i].Energy,
                               lpMsg->m_ClassData[i].Life, lpMsg->m_ClassData[i].Mana, lpMsg->m_ClassData[i].LevelLife,
                               lpMsg->m_ClassData[i].LevelMana,
                               lpMsg->m_ClassData[i].VitalityToLife, lpMsg->m_ClassData[i].EnergyToMana,
                               lpMsg->m_ClassData[i].Leadership);
        }
    }
}

void classdef::SetCharacter(int Class, int Str, int Dex, int Vit, int Energy, float Life, float Mana, float LevelLife,
                            float LevelMana, float VitalityToLife, float EnergyToMana, int Leadership) {
...
int classdef::GetDefPoint(int char_class) {
    ...
    return this->DefClass[char_class].Strength + this->DefClass[char_class].Dexterity +
           this->DefClass[char_class].Vitality + this->DefClass[char_class].Energy +
           this->DefClass[char_class].Leadership;
}
...
bool QuestExpUserMng::IsQuestComplete(int iEpisode, int iObjIndex) {
    ...
    if (iAskCnt >= MAX_QUESTEXP_ASK_INFO) {
        g_Log.Add("[QuestExp] - Error - IsQuestComplete [%s][%s] iAskCnt[%d]", gObj[iObjIndex].AccountID,
                  gObj[iObjIndex].Name, iAskCnt);
        return false;
    }
    ...
    if (iAskCnt != 0) {
        ...
        g_Log.Add("[QuestExp] AskQuestComplete [%s][%s] Ep[%d] QS[%d]", gObj[iObjIndex].AccountID, gObj[iObjIndex].Name,
                  iEpisode, iQS);
        return true;
    } else {
        int iQS = pQuestExpUserMng->GetQuestSwitch();
        g_Log.Add("[QuestExp] IsQuestComplete [%s][%s] Ep[%d] QS[%d] ***HackUser***", gObj[iObjIndex].AccountID,
                  gObj[iObjIndex].Name, iEpisode, iQS);
        return false;
    }
}
...
bool QuestExpUserMng::QuestExpCheckInventoryEmptySpace(BYTE *btTempInventoryMap, int iItemHeight, int iItemWidth,
                                                       int iObjIndex) {
    for (int h = 0; h < MAX_INVENTORY_H; ++h) {
        for (int w = 0; w < 8; ++w) {
            if (btTempInventoryMap[w + h * 8] == 0xFF) {
                BYTE blank = this->QuestExpOnlyInventoryRectCheck(btTempInventoryMap, w, h, iItemWidth, iItemHeight,
                                                                  iObjIndex);
                ...
                if (blank != 0xFF) {
                    this->QuestExpTempInventoryItemSet(btTempInventoryMap, blank, iItemWidth, iItemHeight, 1,
                                                       iObjIndex);
                    return true;
                }
            }
...
BYTE QuestExpUserMng::QuestExpOnlyInventoryRectCheck(BYTE *btTempInventoryMap, int sx, int sy, int width, int height,
                                                     int iObjIndex) {
    if (width + sx > 8) {
        return -1;
    }
...
void QuestExpUserMng::QuestExpTempInventoryItemSet(BYTE *btTempInventoryMap, int itempos, int xl, int yl, BYTE set_byte,
                                                   int iObjIndex) {
...
bool QuestExpUserMng::IsRandResultReward(int iRewardIndexID, QuestExpRewardKindRandom *pRewardKindRandom,
                                         int *iRandResultIndexID) {
    if (pRewardKindRandom == nullptr) {
        g_Log.Add("[QuestExp] - Error - %s(NULL)", __FUNCTION__);
        return false;
    }
...
int QuestExpUserMng::InvenChk_EnableReward(int iEpisode, int iObjIndex, int *pRandResultIndexID) {
    ...
    if (pQuestExpInfo == nullptr) {
        g_Log.Add("[QuestExp] - Error - InvenChk_EnableReward [%s][%s] Ep[%d] QS[%d]", gObj[iObjIndex].AccountID,
                  gObj[iObjIndex].Name, iEpisode, iQuestSwitch);
        return 0;
    }
    ...
    for (; it != end; ++it) {
        ...
        if (pRewardKind) {
            bool bRandResultReward = IsRandResultReward(pQuestExpReward->GetIndexID(), pRewardKindRandom,
                                                        RandomRewardIndex);
...
int QuestExpUserMng::SendQuestReward(int iEpisode, int iObjIndex) {
    ...
    if (!pQuestExpInfo) {
        g_Log.Add("[QuestExp] - Error - QuestExpInfo [%s][%s] Ep [%d] QS[%d] QuestInfoIndexID [0x%x]",
                  gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, iEpisode, iQuestSwitch, dwQuestInfoIndexID);
        return 0;
    }
    ...
    for (; iter != end; ++iter) {
        ...
        switch (iQuestType) {
            case QUESTEXP_REWARD_EXP: {
                ...
                if (gObjCheckUsedBuffEffect(&gObj[iObjIndex], BUFFTYPE_PCS_MARK3) ||
                    gObjCheckUsedBuffEffect(&gObj[iObjIndex], BUFFTYPE_PCBANG_POINT_MARK3)) {
                    iExp = 0;
                }
                ...
                g_Log.Add(LogBuff, gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, pRewardExp->GetRewardExp(),
                          iEpisode, iQuestSwitch);
            }
                break;
            ...
            case QUESTEXP_REWARD_ZEN: {
                ...
                g_Log.Add(LogBuff, gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, pRewardZen->GetRewardZen(),
                          iEpisode, iQuestSwitch);
            }
                break;
            ...
            case QUESTEXP_REWARD_ITEM: {
                ...
                for (int i = 0; i < RewardItemCnt; ++i) {
                    ...
                    if (itemtype != ITEMGET(12, 144) && itemtype != ITEMGET(12, 145) && itemtype != ITEMGET(12, 221)
                        && itemtype != ITEMGET(12, 231) && itemtype != ITEMGET(12, 241) &&
                        itemtype != ITEMGET(12, 251) && itemtype != ITEMGET(12, 261)) {
                        for (int j = 0; j < 5; j++) {
                    ...
                    ItemSerialCreateSend(iObjIndex, 233, gObj[iObjIndex].X, gObj[iObjIndex].Y,
                                         ...
                                         pRewardItem->GetRewardItemExOpt(), 0, 0, btSocketOption,
                                         btPentagramMainAttribute);
                    ...
                    strcat(ItemLogBuff,
                           "Reward Item [%s][%s] Ep[%d] QS[%d] ItemName[%s] Item Num[%d] Level[%d] Dur[%d] skill[%d]");

                    g_Log.Add(ItemLogBuff, gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, iEpisode,
                              iQuestSwitch, szRewardItemName, pRewardItem->GetRewardItemNum(),
                              pRewardItem->GetRewardItemLevel(), pRewardItem->GetRewardItemDur(),
                              pRewardItem->GetRewardItemSkill());
                }
            }
                break;
            ...
            case QUESTEXP_REWARD_CONTRIBUTION_POINT: {
                ...
                g_Log.Add(LogBuff, gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, iContributePt, iEpisode,
                          iQuestSwitch);

                char client_msg[128] = "";
                wsprintf(client_msg, Lang.GetText(0, 564), iContributePt,
                         g_GensSystem.GetContributePoint(&gObj[iObjIndex]));
...
void QuestExpUserMng::SetQuestSwitch(int iEpisode, int iQuestSwitch, int iObjIndex) {
    ...
    DWORD dwGetUserQuestInfoIndexID = GetQuestInfoIndexId(pUserQuestInfo->GetEpisode(),
                                                          pUserQuestInfo->GetQuestSwitch());
    ...
    if (pUserQuestInfo->SetEpisode(iEpisode) == false) {
        ...
        g_Log.Add("[QuestExp] - Error - SetQuestSwitch [%s][%s] Cur EP[%d] Cur QS[%d]", gObj[iObjIndex].AccountID,
                  gObj[iObjIndex].Name, iUserEpisode, iUserQuestSwitch);
        return;
    }
    ...
    g_Log.Add("[QuestExp] Set Quest Switch [%s][%s] EP[%d] QS[%d]", gObj[iObjIndex].AccountID, gObj[iObjIndex].Name,
              iUserEpisode, iUserQuestSwitch);
...
void QuestExpUserMng::UserAllQuestInfoSave(int iObjIndex) {
    ...
    while (iEpCnt < MAX_QUESTEXP_EPISODE) {
        ...
        if (iEpisode > 0) {
            ...
            if (iAskCnt == 0) {
                g_Log.Add("[QuestExp] AllQuestInfoSave [%s][%s] EP[%d] QS[%d]", gObj[iIndex].AccountID,
                          gObj[iIndex].Name, iEpisode, iQuestSwitch);
            }
            ...
            for (int i = 0; i < iAskCnt; ++i) {
                ...
                g_Log.Add("[QuestExp] AllQuestInfoSave [%s][%s] EP[%d] QS[%d] AskIndex[0x%x] AskValue[%d] AskState[%d]",
                          gObj[iIndex].AccountID, gObj[iIndex].Name, iEpisode, iQuestSwitch,
                          QuestInfo[iQuestCnt].btAskIndex[i], QuestInfo[iQuestCnt].btAskValue[i],
                          QuestInfo[iQuestCnt].btAskState[i]);
            }
...
void QuestExpUserMng::UserQuestInfoSave(DWORD dwQuestInfoIndexID, int iObjIndex) {
    ...
    if (iAskCnt == 0) {
        g_Log.Add("[QuestExp] QuestInfoSave [%s][%s] EP[%d] QS[%d]", gObj[iIndex].AccountID, gObj[iIndex].Name,
                  iEpisode, iQuestSwitch);
    }
    ...
    for (int i = 0; i < iAskCnt; i++) {
        ...
        g_Log.Add("[QuestExp] QuestInfoSave [%s][%s] EP[%d] QS[%d] AskIndex[0x%x] AskValue[%d] AskState[%d]",
                  gObj[iIndex].AccountID, gObj[iIndex].Name, iEpisode, iQuestSwitch, QuestInfo.btAskIndex[i],
                  QuestInfo.btAskValue[i], QuestInfo.btAskState[i]);
    }
    ...
    for (int i = 0; i < iQuestCnt; i++) {
        ...
        for (int j = 0; j < MAX_QUESTEXP_ASK_INFO; j++) {
            if (QuestInfo[i].btAskIndex[j]) {
                QuestExpAsk *pQuestExpAsk = g_QuestExpManager.GetQuestExpAsk(iEpisode, iQuestSwitch,
                                                                             QuestInfo[i].btAskIndex[j]);

                if (!pQuestExpAsk) {
                    g_Log.Add("[QuestExp] UserQuestInfo DB Error [%s][%s] - pQuestExpAsk NULL",
                ...
                g_Log.Add(
                        ...
                        gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, iEpisode, iQuestSwitch, iQuestType, btAskIndex,
                        btAskValue, btAskState, wProgState);
...
UserQuestInfo *QuestExpUserMng::GetUserQuestInfo(int iEpisode, int iObjIndex) {
    if (IsEpisode(iEpisode) == false) {
        g_Log.Add("[QuestExp] - Error - GetUserQuestInfo [%s][%s] Ep[%d]", gObj[iObjIndex].AccountID,

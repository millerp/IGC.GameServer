...
bool CItemOptionTypeMng::LoadAccessoryItemOptionScript(char *pchFileName) {
    ...
    if (res.status != pugi::status_ok) {
        g_Log.MsgBox("[ItemOptionTypeMng] AccessoryItem Option Script file Load failed. (%s) (%s)", pchFileName,
                     res.description());
        return false;
    }
    ...
    for (pugi::xml_node item = main.child("Item"); item; item = item.next_sibling()) {
        ...
        memcpy(&this->m_AccessoryItemOptionValue[iValueCount], &AccessoryItemOptionValue,
               sizeof(AccessoryItemOptionValue));
        iValueCount++;
    }
        ...
        if (iRand < OneOp || iRand >= OneOp + TwoOp) {
            if (iRand < OneOp + TwoOp || iRand >= OneOp + TwoOp + ThreeOp) {
                if (iRand < OneOp + TwoOp + ThreeOp || iRand >= OneOp + TwoOp + ThreeOp + FourOp) {
                    if (iRand < OneOp + TwoOp + ThreeOp + FourOp ||
                        iRand >= OneOp + TwoOp + ThreeOp + FourOp + FiveOp) {
                        if (iRand >= OneOp + TwoOp + ThreeOp + FourOp + FiveOp ||
                            iRand < OneOp + TwoOp + ThreeOp + FourOp + FiveOp + SixOp) {
                            iOptionCount = 6;
                        }
                    } else {
                        iOptionCount = 5;
        ...
        for (int j = 0; j < MAX_EXCELLENT_COMMON_OPTIONS; j++) {
            if (this->m_CommonExtOptionType[j].OptionID != -1) {
                if (this->m_CommonExtOptionType[j].ItemKindA_1 == lpAttr->ItemKindA ||
                    ...
                    if ((lpObj->pInventory[i].m_NewOption & this->m_CommonExtOptionType[j].OptionNumber) ==
                        this->m_CommonExtOptionType[j].OptionNumber) {
...
void
CItemOptionTypeMng::SetExcOptionEffect(OBJECTSTRUCT *lpObj, int ExcOptionID, LPITEM_ATTRIBUTE lpItem, BYTE btItemPos) {
    ...
    switch (ExcOptionID) {
        ...
        case OPTION_INC_ATTACK_PERCENT:
            if ((lpObj->pInventory[btItemPos].m_Type >= ITEMGET(5, 0) &&
                 lpObj->pInventory[btItemPos].m_Type < ITEMGET(6, 0)) ||
            ...
            if ((lpObj->pInventory[btItemPos].m_Type >= ITEMGET(5, 0) &&
                ...
                if (btItemPos == 0 || btItemPos == 9) {
                    lpObj->m_AttackDamageMinRight +=
                            (lpObj->Level + lpObj->m_PlayerData->MasterLevel) / lpOption->OptionValue;
    ...
    for (int i = 0; i < MAX_EXCELLENT_WING_OPTIONS; i++) {
        if (this->m_WingExtOptionType[i].OptionID != -1) {
            if (this->m_WingExtOptionType[i].ItemKindA == lpAttr->ItemKindA &&
                ...
                if ((lpObj->pInventory[7].m_NewOption & this->m_WingExtOptionType[i].OptionNumber) ==
                    this->m_WingExtOptionType[i].OptionNumber) {

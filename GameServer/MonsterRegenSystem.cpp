...
void CMonsterRegenSystem::LoadScript(char *lpFileName) {
    ...
    for (pugi::xml_node group = spot_settings.child("Group"); group; group = group.next_sibling()) {
        ...
        for (pugi::xml_node area = group.child("Area"); area; area = area.next_sibling()) {
            ...
            this->m_stMonsterPosition[nGroupNumber].m_stRegenArea[nAreaCnt].m_nBeginX = area.attribute(
                    "StartX").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stRegenArea[nAreaCnt].m_nBeginY = area.attribute(
                    "StartY").as_int();
    ...
    for (pugi::xml_node group = monster_settings.child("Group"); group; group = group.next_sibling()) {
        ...
        for (pugi::xml_node monster = group.child("Monster"); monster; monster = monster.next_sibling()) {
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nMonsterIndex = monster.attribute(
                    "Index").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nCnt = monster.attribute(
                    "Count").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nOverrideDefaultSettings = monster.attribute(
                    "OverrideDefaultSettings").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nEventId = monster.attribute(
                    "EventID").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nHP = monster.attribute(
                    "HP").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nDamageMin = monster.attribute(
                    "DamageMin").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nDamageMax = monster.attribute(
                    "DamageMax").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nDefense = monster.attribute(
                    "Defense").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nAttackRate = monster.attribute(
                    "AttackRate").as_int();
            this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nDefenseRate = monster.attribute(
                    "DefenseRate").as_int();
            nArrayIndex++;
        }
    ...
    for (pugi::xml_node group = spawn_settings.child("Group"); group; group = group.next_sibling()) {
        ...
        for (pugi::xml_node spawn = group.child("Spawn"); spawn; spawn = spawn.next_sibling()) {
            this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[nTimeArrayIndex].m_nBeginHour = spawn.attribute(
                    "StartHour").as_int();
            this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[nTimeArrayIndex].m_nBeginMinute = spawn.attribute(
                    "StarMinute").as_int();
            this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[nTimeArrayIndex].m_nAppearanceRate = spawn.attribute(
                    "AppearanceRate").as_int();
            nTimeArrayIndex++;
        }
...
void CMonsterRegenSystem::RegenMonster(int nGroupNumber) {
    ...
    for (int k = 0; k < MAX_MONSTER_INDEX_INFO; k++) {
        ...
        for (int n = 0; n < nMonsterCount && nMapNumber != -1; n++) {
            ...
            if (nResult < 0) {
                g_Log.Add("[MonsterGroupRegen] Add Monster fail!! [GROUP]: %d, [INDEX]: %d", nGroupNumber,
                          nMonsterIndex);
            } else {
                if (this->SetPosMonster(nResult, nMapNumber, nBeginX, nBeginY, nEndX, nEndY) == true) {
                    if (gObjSetMonster(nResult, nMonsterIndex) == TRUE) {
                        if (this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[k].m_nOverrideDefaultSettings ==
                            TRUE) {
                        ...
                        if (gObj[nResult].Class == this->m_stMonsterGroupInfo[nGroupNumber].m_nBossMonsterClass) {
                            ...
                            if (this->m_bSendMessage == true) {
                                ...
                                TNotice::MakeNoticeMsg(&pNotice, 0,
                                                       (char *) this->m_stMonsterGroupInfo[nGroupNumber].m_sSpawnNotice.c_str());
                        ...
                        g_Log.Add("[MonsterGroupRegen] Group: %d, MonsterIndex: %d, MapNumber: %d, X: %d, Y: %d",
...
void CMonsterRegenSystem::DeleteMonster(int nGroupNumber) {
    ...
    for (int i = 0; i < MAX_MONSTER_ACCOUNT_NUM_INFO &&
                    this->m_stMonsterGroupInfo[nGroupNumber].m_stMonsterAccountNumInfo[i].m_nIndex > -1; i++) {
        gObjDel(this->m_stMonsterGroupInfo[nGroupNumber].m_stMonsterAccountNumInfo[i].m_nIndex);
        g_Log.Add("[MonsterGroupRegen] Delete Monster - %d",
    ...
    for (int i = 0; i < MAX_REGEN_TIME_TABLE; i++) {
        if (this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[i].m_nBeginHour == nCurHour &&
            ...
            if (this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[i].m_bRegen == false) {
                if (bSetRegen &&
                    !this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[i].m_bFailedAppearanceRate) {
                    if (rand() % 100 <
                        this->m_stMonsterGroupInfo[nGroupNumber].m_stRegenTimeTable[i].m_nAppearanceRate) {
...
bool CMonsterRegenSystem::MonsterKillCheck(LPOBJ lpPlayer, LPOBJ lpMonster) {
    for (int i = 0; i < MAX_MONSTER_GROUP_INFO && this->m_stMonsterGroupInfo[i].m_nGroupNumber > -1; i++) {
        if (this->m_stMonsterGroupInfo[i].m_nLiveCount > 0) {
            for (int n = 0; n < MAX_MONSTER_ACCOUNT_NUM_INFO &&
                            this->m_stMonsterGroupInfo[i].m_stMonsterAccountNumInfo[n].m_nIndex > -1; n++) {
                if (this->m_stMonsterGroupInfo[i].m_stMonsterAccountNumInfo[n].m_nIndex == lpMonster->m_Index) {
                    for (int k = 0; k < MAX_MONSTER_INDEX_INFO; k++) {
                        if (this->m_stMonsterPosition[i].m_stMonsterIndexInfo[k].m_nMonsterIndex == lpMonster->Class) {
                            if (this->m_stMonsterPosition[i].m_stMonsterIndexInfo[k].m_nOverrideDefaultSettings ==
                                TRUE) {
                                g_BagManager.SearchAndUseBag(lpPlayer->m_Index, BAG_EVENT,
                                                             this->m_stMonsterPosition[i].m_stMonsterIndexInfo[k].m_nEventId,
                                                             lpMonster->m_Index);

...
int CEventDungeonItemBag::GetLevelIndex(int nUserLevel) {
    for (int i = 0; i < 10; i++) {
        if (nUserLevel >= this->eventDungeonItemInfo[i].m_nMinLevel &&
            nUserLevel <= this->eventDungeonItemInfo[i].m_nMaxLevel) {
...
BOOL CEventDungeonItemBag::CreateNormalItem(int nIndex, int nMaxUserLevel, CEventItemBagAttr BagObject) {
    ...
    if (nType == ITEMGET(13, 0) || nType == ITEMGET(13, 1) || nType == ITEMGET(13, 2) || nType == ITEMGET(13, 8) ||
        nType == ITEMGET(13, 9) || nType == ITEMGET(13, 12) || nType == ITEMGET(13, 13)) {
        nLevel = 0;
    }
    ...
    ItemSerialCreateSend(nIndex, lpObj->MapNumber, btDropX, btDropY, nType, nLevel, 0, Option1, Option2, Option3,
                         nIndex, ExOption, 0, 0, 0, 0);
    g_Log.Add("[IMPERIALGUARDIAN] Drop Item : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
              lpObj->MapNumber, btDropX, btDropY, ItemAttribute[nType].Name, nType, nLevel, Option1, Option2, Option3,
              ExOption);
...
int CEventDungeonItemBag::CreateSetItem(int nIndex, CEventItemBagAttr BagObject) {
    ...
    ItemSerialCreateSend(nIndex, btMapNumber, btDropX, btDropY, nType, 0, 0, TRUE, Option2, Option3, nIndex, 0,
                         SetOption, 0, 0, 0);
    g_Log.Add(
            "[IMPERIALGUARDIAN] Drop Set Item [%s][%s] : (%d)(%d/%d) Itemnum:[%d] skill:[%d] luck:[%d] option:[%d] SetOption:[%d], SetName:[%s]",
            lpObj->AccountID, lpObj->Name, btMapNumber, btDropX, btDropY, nType, TRUE, Option2, Option3, SetOption,
            gSetItemOption.GetSetOptionName(tmpSetOption));

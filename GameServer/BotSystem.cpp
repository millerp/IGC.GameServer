    ...
    switch (type) {
        ...
        case BOT_TRADE: {
            ...
            for (pugi::xml_node craft = ItemList.child("Craft"); craft; craft = craft.next_sibling()) {
                ...
                for (pugi::xml_node requirements = craft.child(
                        "Requirement"); requirements; requirements = requirements.next_sibling()) {
...
void CBotSystem::SetAllBots() {
    ...
    for (std::map<int, _sBOT_SETTINGS>::iterator It = this->m_BotData.begin(); It != this->m_BotData.end(); It++) {
        ...
        pBot->aIndex = iIndex;
        this->LoadBotSpecializationData(pBot->aIndex, g_ConfigRead.GetPath((char *) pBot->sPathActionFile.c_str()),
                                        pBot->btType);
        g_Log.AddC(TColor::Aqua, "[%d] BOT [%s] Type: [%d] Added Map [%d][%d][%d]", pBot->wID, pBot->sName,
                   pBot->btType, pBot->btMap, pBot->btX, pBot->btY);
    }
...
//--- assign
    ...
    lpBotObj->MaxLife = DCInfo.DefClass[lpBotObj->Class].Life +
                        ((lpBotObj->Level + lpBotObj->m_PlayerData->MasterLevel) - 1) *
                        DCInfo.DefClass[lpBotObj->Class].LevelLife +
                        ((lpBotObj->m_PlayerData->Vitality - DCInfo.DefClass[lpBotObj->Class].Vitality) *
                         DCInfo.DefClass[lpBotObj->Class].VitalityToLife);
    lpBotObj->MaxMana = DCInfo.DefClass[lpBotObj->Class].Mana +
                        ((lpBotObj->Level + lpBotObj->m_PlayerData->MasterLevel) - 1) *
                        DCInfo.DefClass[lpBotObj->Class].LevelMana +
                        ((lpBotObj->m_PlayerData->Energy - DCInfo.DefClass[lpBotObj->Class].Energy) *
                         DCInfo.DefClass[lpBotObj->Class].EnergyToMana);
...
void CBotSystem::MakePreviewCharSet(int aIndex, _sBOT_INVENTORY_WEAR_ITEMS *pInv) {
    ...
    if ((wingtype >= ITEMGET(12, 36) && wingtype <= ITEMGET(12, 40)) || wingtype == ITEMGET(12, 43) ||
        wingtype == ITEMGET(12, 50)) {
    ...
    } else if ((wingtype >= ITEMGET(12, 3) && wingtype <= ITEMGET(12, 6)) || wingtype == ITEMGET(13, 30) ||
               wingtype == ITEMGET(12, 42) || wingtype == ITEMGET(12, 49)) {
        lpObj->CharSet[5] |= 8;
...
void CBotSystem::BuffPlayer(WORD wBufferindex, short aIndex) {
    ...
    for (int i = 0; i < MAX_BUFFS_PER_BOT; i++) {
        if (gBotObj->m_BotBuffs[i].wBuffId > 0) {
            ...
            gObjAddBuffEffect(&gObj[aIndex], gBotObj->m_BotBuffs[i].wBuffId, gBotObj->m_BotBuffs[i].wEffectType,
                              gBotObj->m_BotBuffs[i].iEffect, 0, 0, gBotObj->m_BotBuffs[i].wDuration);
        }
    }
...
sBOT_REWARD_STRUCT CBotSystem::ConfirmMixSuccess(int aIndex, int botIndex) {
    ...
    for (std::map<int, sBOT_CRAFTING>::iterator It = this->m_BotData[botIndex].m_Crafting.begin();
         It != this->m_BotData[botIndex].m_Crafting.end(); It++) {
        ...
        if (iMixNeedCount == this->AlchemistTradeItemCount(aIndex)) {
            ...
            for (int i = 0; i < It->second.iReqCount; i++) {
                if (gObjGetItemCountInTradeWindow(aIndex, It->second.m_Requirements[i].wItemId,
                                                  It->second.m_Requirements[i].btLevel,
                                                  It->second.m_Requirements[i].btExc,
                                                  It->second.m_Requirements[i].btSetItem,
                                                  It->second.m_Requirements[i].btOpt,
                                                  It->second.m_Requirements[i].btLuck,
                                                  It->second.m_Requirements[i].btSkill) ==
                    It->second.m_Requirements[i].btCount) {
                    foundItems++;
                }
                if (foundItems == It->second.iReqCount) {
                    m_MixResult.m_Reward.Convert(It->second.m_Reward.wItemId, It->second.m_Reward.btSkill,
                                                 It->second.m_Reward.btLuck, It->second.m_Reward.btOpt, 0,
                                                 It->second.m_Reward.btSetItem, 0, 0, 0, 0, 3);
    ...
    for (std::map<int, sBOT_CRAFTING>::iterator It = this->m_BotData[botIndex].m_Crafting.begin();
        ...
        if (iMixNeedCount == this->AlchemistTradeItemCount(aIndex)) {
            ...
            for (int i = 0; i < It->second.iReqCount; i++) {
                ...
                if (foundItems == It->second.iReqCount) {
                    rewardItem.Convert(It->second.m_Reward.wItemId, It->second.m_Reward.btSkill,
                                       It->second.m_Reward.btLuck, It->second.m_Reward.btOpt, 0,
                                       It->second.m_Reward.btSetItem, 0, 0, 0, 0, 3);
...
void CBotSystem::AlchemistTradeOk(int aIndex, int botIndex) {
    ...
    if (reward.m_Reward.m_Type > 0) {
        int iEmptyCount = CheckInventoryEmptySpaceCount(&gObj[aIndex], ItemAttribute[reward.m_Reward.m_Type].Width,
                                                        ItemAttribute[reward.m_Reward.m_Type].Height);
        ...
        for (int i = 0; i < reward.iCount; i++) {
            ItemSerialCreateSend(aIndex, 235, gObj[aIndex].X, gObj[aIndex].Y, reward.m_Reward.m_Type,
                                 reward.m_Reward.m_Level, 0, reward.m_Reward.m_Option1, reward.m_Reward.m_Option2,
                                 reward.m_Reward.m_Option3, aIndex, reward.m_Reward.m_NewOption,
                                 reward.m_Reward.m_SetOption, 0, 0, 0);
        }
...
bool CBotSystem::StoreAddItems(int botIndex) {
    ...
    for (int i = 0; i < this->m_BotData[botIndex].m_Shop.iItemCount; i++) {
        BYTE blank = this->PShopCheckSpace(&gObj[aIndex], this->m_BotData[botIndex].m_Shop.pItems[i].wItemId,
                                           &gObj[aIndex].pInventoryMap[PSHOP_START_RANGE]);
        ...
        if (blank != 255) {
            ...
            item.Convert(this->m_BotData[botIndex].m_Shop.pItems[i].wItemId, item.m_Option1, item.m_Option2,
                         item.m_Option3, 0, item.m_SetOption, item.m_NewOption, item.m_SocketOption, 0, 0, 3);
    ...
    memcpy(gObj[aIndex].m_szPShopText, this->m_BotData[botIndex].m_Shop.szBotShopName.c_str(),
           sizeof(gObj[aIndex].m_szPShopText));
    return true;
}
...
int
gObjGetItemCountInTradeWindow(int aIndex, WORD itemtype, int itemlevel, BYTE btExc, BYTE btSet, BYTE btOpt, BYTE btLuck,
                              BYTE btSkill) {
    int Count = 0;

    for (int n = 0; n < TRADE_BOX_SIZE; n++) {
        if (gObj[aIndex].Trade[n].IsItem() == 1 && gObj[aIndex].Trade[n].m_Type == itemtype &&
            gObj[aIndex].Trade[n].m_Level == itemlevel) {

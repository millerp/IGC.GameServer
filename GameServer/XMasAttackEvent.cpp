...
BOOL CXMasMonsterHerd::MonsterHerdItemDrop(LPOBJ lpObj) {
    ...
    if (lpObj->Class == 476) {
        ...
        ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, itemnumber, 0, 0, 0, 0, 0, iIndex, 0,
                             0, 0, 0, 0);
        ...
        if (buff == 1) {
            gObjAddBuffEffectWideArea(lpObj, 10, count, BUFFTYPE_SANTA_HEAL, EFFECTTYPE_HP, 500, 0, 0,
                                      g_XMasAttackEvent.GetRestBuffDuration());
        } else if (buff == 2) {
            gObjAddBuffEffectWideArea(lpObj, 10, count, BUFFTYPE_SANTA_PROTECT, EFFECTTYPE_MANA, 500, 0, 0,
                                      g_XMasAttackEvent.GetRestBuffDuration());
        } else {
            int Damage;
            int Defense;
            ...
            gObjAddBuffEffectWideArea(lpObj, 10, count, BUFFTYPE_CHRISTMAS_BLESS, EFFECTTYPE_IMPROVE_DAMAGE, Damage,
                                      EFFECTTYPE_IMPROVE_DEFENSE, Defense, g_XMasAttackEvent.GetSantaBuffDuration());
        }
...
void CXMasAttackEvent::StartEvent() {
    ...
    for (int i = 0; i < MAX_XMASMONSTER_MAP; i++) {
        ...
        for (std::vector<XMASMONSTER_DATA>::iterator it = this->m_vtMonsterAddData.begin();
             it != this->m_vtMonsterAddData.end(); it++) {
            XMASMONSTER_DATA &pXMD = *it;
            iCount = 100;

            while (iCount-- != 0) {
                if (this->m_XMasMonsterHerd[i].AddMonster(pXMD.m_iType, pXMD.m_bDoRegen, pXMD.m_bDoAttackFirst) !=
                    FALSE) {
                    break;
                }
            }
...
void CXMasAttackEvent::CheckSync() {
    ...
    switch (bTIME_CHANGED) {
        case 1:
            this->m_iTIME_MSEC_REMAIN = ((iMIN_HOUR * 60 * 60 + iMIN_MINUTE * 60) -
                                         (sysTime.wHour * 60 * 60 + sysTime.wMinute * 60 + sysTime.wSecond)) * 1000;
            break;
        case 2:
            this->m_iTIME_MSEC_REMAIN = (((iMIN_HOUR + 24) * 60 * 60 + iMIN_MINUTE * 60) -
                                         (sysTime.wHour * 60 * 60 + sysTime.wMinute * 60 + sysTime.wSecond)) * 1000;
            break;
        default:
            g_Log.AddC(TColor::Red, "[XMas Event] No Schedule Time Data");
...
void CXMasAttackEvent::Move() {
    if (this->m_bDoEvent) {
        ...
        for (int i = 0; i < MAX_RINGMONSTER_MAP; i++) {
            ...
            this->m_XMasMonsterHerd[i].SetRadius(
                    (rand() % (this->m_iRADIUS_MAX - this->m_iRADIUS_MIN)) + this->m_iRADIUS_MIN);
        }
    }
}

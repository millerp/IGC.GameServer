...
BOOL CPeriodItemEx::IsCorrectUser(LPOBJ lpObj) {
    ...
    if (PeriodItemIndex >= 0) {
        if (this->m_PeriodData[PeriodItemIndex].btUsedInfo == TRUE &&
            this->m_PeriodData[PeriodItemIndex].dwUserGuid == lpObj->DBNumber) {
            return TRUE;
        }
    }
            ...
            if (this->m_PeriodData[i].m_Obj->DBNumber == lpObj->DBNumber) {
                for (int iItemSlotIndex = 0; iItemSlotIndex < 30; iItemSlotIndex++) {
                    if (this->m_PeriodData[i].m_ItemInfo[iItemSlotIndex].Serial == Serial) {
                        g_Log.Add(
                                "[PeriodItemEx][Remove PeriodItem By Force] Remove PeriodItem Data By Force. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %I64d, ExpireDate : %I64d",
                                lpObj->AccountID, lpObj->DBNumber, lpObj->Name,
                                ItemAttribute[this->m_PeriodData[i].m_ItemInfo[iItemSlotIndex].wItemCode].Name,
                    ...
                    if (lpObj->Connected >= PLAYER_LOGGED) {
                        for (int j = 0; j < 30; j++) {
                            if (this->m_PeriodData[i].m_ItemInfo[j].btUsedInfo == TRUE) {
                                if (this->CheckExpireDate(this->m_PeriodData[i].m_ItemInfo[j].lItemExpireDate) ==
                                    TRUE) {
                                    g_Log.Add(
                                            "[PeriodItemEx][Remove PeriodItem] Expire PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %I64d, ExpireDate : %I64d",
                                            lpObj->AccountID, lpObj->DBNumber, lpObj->Name,
                                            ItemAttribute[this->m_PeriodData[i].m_ItemInfo[j].wItemCode].Name,
                                            this->m_PeriodData[i].m_ItemInfo[j].wItemCode,
                                            this->m_PeriodData[i].m_ItemInfo[j].btItemType,
                                            this->m_PeriodData[i].m_ItemInfo[j].Serial,
                                            this->m_PeriodData[i].m_ItemInfo[j].lItemExpireDate);

                                    this->RemovePeriodItemData(lpObj, this->m_PeriodData[i].m_ItemInfo[j].btItemType,
                                                               this->m_PeriodData[i].m_ItemInfo[j].wItemCode,
                                                               this->m_PeriodData[i].m_ItemInfo[j].Serial);
...
void CPeriodItemEx::SetExpireNotMatchedData(LPOBJ lpObj) {
    ...
    for (int i = 0; i < INVENTORY_SIZE; i++) {
        if (lpObj->pInventory[i].IsItem() == TRUE &&
            ...
            for (int j = 0; j < 30; j++) {
                if (this->m_PeriodData[lpObj->m_PlayerData->m_iPeriodItemEffectIndex].m_ItemInfo[j].btUsedInfo ==
                    TRUE &&
                    this->m_PeriodData[lpObj->m_PlayerData->m_iPeriodItemEffectIndex].m_ItemInfo[j].wItemCode ==
                    lpObj->pInventory[i].m_Type &&
                    this->m_PeriodData[lpObj->m_PlayerData->m_iPeriodItemEffectIndex].m_ItemInfo[j].Serial ==
                    lpObj->pInventory[i].m_Number) {
                    bItemFound = TRUE;
                }
            }
...
void CPeriodItemEx::RequestPeriodItemInsert(LPOBJ lpObj, ITEM_DATA *lpItemData, DWORD dwDuration) {
    ...
    g_Log.Add(
            "[PeriodItemEx][Insert PeriodItem] Request Insert PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %I64d, Duration : %d",
            lpObj->AccountID, lpObj->DBNumber, lpObj->Name, ItemAttribute[lpItemData->wItemCode].Name,
            lpItemData->wItemCode, lpItemData->btItemType, lpItemData->Serial, dwDuration);
...
void CPeriodItemEx::RequestPeriodItemDelete(LPOBJ lpObj, ITEM_DATA *lpItemData) {
    ...
    g_Log.Add(
            "[PeriodItemEx][Delete PeriodItem] Request Delete PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %I64d",
            lpObj->AccountID, lpObj->DBNumber, lpObj->Name, ItemAttribute[lpItemData->wItemCode].Name,
            lpItemData->wItemCode, lpItemData->btItemType, lpItemData->Serial);
...
void CPeriodItemEx::DGPeriodItemExInsert(PMSG_ANS_PERIODITEMEX_INSERT *lpMsg) {
    ...
    if (lpMsg->btResultCode == 0) {
        g_Log.Add(
                "[PeriodItemEx][Insert PeriodItem] OnRequest Insert PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %I64d, Duration : %d, Result : %d",
                lpObj->AccountID, lpObj->DBNumber, lpObj->Name, ItemAttribute[lpMsg->wItemCode].Name, lpMsg->wItemCode,
                lpMsg->btItemtype, lpMsg->Serial, lpMsg->dwDuration, lpMsg->btResultCode);
...
void CPeriodItemEx::DGPeriodItemExDelete(PMSG_ANS_PERIODITEMEX_DELETE *lpMsg) {
    ...
    if (lpMsg->btResultCode == 0) {
        g_Log.Add(
                "[PeriodItemEx][Delete PeriodItem] OnRequest Delete PeriodItem Data. User Id : %s(%d), Name : %s, Result : %d",
...
void CPeriodItemEx::DGPeriodItemExExpiredItemList(PMSG_ANS_PERIODITEMEX_EXPIRED_ITEMLIST *lpMsg) {
    ...
    for (int i = 0; i < lpMsg->btExpiredItemCount; i++) {
        ...
        if (pos != -1) {
            g_Log.Add(
                    "[PeriodItemEx][SetExpiredItemList] OnRequest Expired Item Set Disable. User Id : %s(%d), Name : %s, Item : %s(%d), Serial : %I64d",
                    lpObj->AccountID, lpObj->DBNumber, lpObj->Name, ItemAttribute[lpMsg->wItemCode[i]].Name,
                    lpMsg->wItemCode[i], lpMsg->Serial[i]);
...
void CPeriodItemEx::DGPeriodItemExList(PMSG_ANS_PERIODITEMEX_LIST *lpMsg) {
    ...
    if (lpMsg->btItemType == PERIODITEM_BUFF) {
        if (gObjAddBuffEffectForInGameShop(lpObj, lpMsg->wItemCode, LeftDate) == true) {
            g_Log.Add(
                    "[PeriodItemEx][SelectItemList] User Id : %s(%d), Name : %s, Item Code : %d(%s), PeriodType : %d, Serial : %I64d, Duration : %d, ExpireDate : %d",
                    lpObj->AccountID, lpObj->DBNumber, lpObj->Name, lpMsg->wItemCode,
                    ItemAttribute[lpMsg->wItemCode].Name, lpMsg->btItemType, lpMsg->Serial, LeftDate,
                    lpMsg->lItemExpireDate);
        } else {
            g_Log.Add(
                    "[PeriodItemEx][SelectItemList] Failed User Id : %s(%d), Name : %s, Item Code : %d(%s), PeriodType : %d, Serial : %I64d, Duration : %d, ExpireDate : %d",
                    lpObj->AccountID, lpObj->DBNumber, lpObj->Name, lpMsg->wItemCode,
                    ItemAttribute[lpMsg->wItemCode].Name, lpMsg->btItemType, lpMsg->Serial, LeftDate,
                    lpMsg->lItemExpireDate);
        }
    ...
    if (this->m_PeriodData[PeriodIndex].btUsedInfo == TRUE) {
        for (int i = 0; i < 30; i++) {
            if (this->m_PeriodData[PeriodIndex].m_ItemInfo[i].btUsedInfo == TRUE) {
                ...
                pMsg2.Pos = this->GetItemFromInventory(lpObj, this->m_PeriodData[PeriodIndex].m_ItemInfo[i].wItemCode,
                                                       this->m_PeriodData[PeriodIndex].m_ItemInfo[i].Serial);
...
BOOL CPeriodItemEx::SendPeriodItemInfoOnce(LPOBJ lpObj, CItem *Item) {
    ...
    if (this->m_PeriodData[PeriodIndex].btUsedInfo == TRUE &&
        this->m_PeriodData[PeriodIndex].dwUserGuid == lpObj->DBNumber) {
        for (int i = 0; i < 30; i++) {
            if (this->m_PeriodData[PeriodIndex].m_ItemInfo[i].btUsedInfo == TRUE &&
                this->m_PeriodData[PeriodIndex].m_ItemInfo[i].Serial == Item->GetNumber()) {
...
BOOL CPeriodItemEx::SendPeriodItemInfoOnce(LPOBJ lpObj, UINT64 Serial) {
    ...
    if (this->m_PeriodData[PeriodIndex].btUsedInfo == TRUE &&
        ...
        for (int i = 0; i < 30; i++) {
            if (this->m_PeriodData[PeriodIndex].m_ItemInfo[i].btUsedInfo == TRUE &&
                this->m_PeriodData[PeriodIndex].m_ItemInfo[i].Serial == Serial) {
                this->OnRequestPeriodItemListOnce(lpObj, &this->m_PeriodData[PeriodIndex].m_ItemInfo[i]);
                return TRUE;
            }
        }

...
void ChaosCastleFinal::Load_CCFInfo(char *Filename) {
    ...
    this->m_bCCF_EVENT_ENABLE =
            g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_CCF) == true
            ? main_section.attribute("Enable").as_int() : false;
...
bool ChaosCastleFinal::CheckOpenOrNotOpenCCF_Final(BYTE bCCFType) {
    ...
    for (std::vector<CCF_START_TIME>::iterator it = this->m_vtCCFOpenTime.begin();
         it != this->m_vtCCFOpenTime.end(); it++) {
        pRET = *it;
    ...
    if (iTICK_MSEC >= 1000) {
        ...
        if (this->m_bCCF_EVENT_ENABLE) {
            if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE && this->Check_CCF_DayType()) {
                ...
                if (!IsNowFinal) {
                    if (this->m_stCCFData.m_nCCF_REMAIN_MSEC <= 60000 * this->m_iCCF_TIME_MIN_ALL_NOTIFY &&
                    ...
                    if (this->m_stCCFData.m_nCCF_REMAIN_MSEC <= 60000 * this->m_iCCF_TIME_MIN_OPEN &&
                        !this->m_stCCFData.m_bCCF_CAN_ENTER) {
                    ...
                    if (this->m_stCCFData.m_nCCF_REMAIN_MSEC <= 60000 * this->m_iCCF_TIME_MIN_OPEN &&
                        ...
                        this->m_stCCFData.m_nCCF_REMAIN_MSEC / 60000 != this->m_stCCFData.m_nCCF_NOTIFY_COUNT) {
                        this->m_stCCFData.m_nCCF_NOTIFY_COUNT = this->m_stCCFData.m_nCCF_REMAIN_MSEC / 60000;

                        PMSG_NOTICE pNotice;
                        TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 590),
                                                 this->m_stCCFData.m_nCCF_NOTIFY_COUNT + 1);

                        this->SendAllUserAnyMsg((LPBYTE) & pNotice, pNotice.h.size);
                        g_Log.Add("[Chaos Castle Survival] Event starts in %d minutes",
                                  this->m_stCCFData.m_nCCF_NOTIFY_COUNT + 1);
                    }

                    if (this->m_stCCFData.m_nCCF_REMAIN_MSEC <= 30000 && this->m_stCCFData.m_nCCF_REMAIN_MSEC > 0 &&
                        !this->m_stCCFData.m_bCCF_MSG_BEFORE_ENTER) {
                        ...
                        for (int n = g_ConfigRead.server.GetObjectStartUserIndex();
                            ...
                            if (gObj[n].Connected == PLAYER_PLAYING && gObj[n].Type == OBJ_USER &&
                                gObj[n].MapNumber != MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
        ...
        if (this->m_stCCFData.m_bCCF_PLAY_START == true) {
            ...
            if (iCUR_PLAY_STATE == 1) {
                ...
                if (iCCF_LEFT_MONSTER_COUNT > 0) {
                    g_Log.Add("[Chaos Castle Survival][CCS1] Has No Winner : Monster Left (%d)",
                              this->m_stCCFData.m_lCCF_CURRENT_MONSTER_COUNT);
                } else {
                    int nPoint = 0;
                    ...
                    if (iCCF_WINNER_INDEX == -1) {
                        ...
                        g_Log.Add(
                                "[Chaos Castle Survival][CCS1] [%s][%s][%s][ServerCode: %d] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
                                gObj[iCCF_WINNER_INDEX].AccountID, gObj[iCCF_WINNER_INDEX].Name,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->m_RealNameOfUBF,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->m_nServerCodeOfHomeWorld,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->DbClass,
                                gObj[iCCF_WINNER_INDEX].Level, gObj[iCCF_WINNER_INDEX].m_PlayerData->LevelUpPoint,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->Experience,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->Strength,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->Dexterity,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->Vitality,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->Energy, gObj[iCCF_WINNER_INDEX].Leadership,
                                gObj[iCCF_WINNER_INDEX].MapNumber, gObj[iCCF_WINNER_INDEX].m_PK_Level);
            ...
            if (iCUR_PLAY_STATE == 2) {
                ...
                if (iCCF_LEFT_MONSTER_COUNT > 0) {
                    g_Log.Add("[Chaos Castle Survival][CCS2] Has No Winner : Monster Left (%d)",
                              this->m_stCCFData.m_lCCF_CURRENT_MONSTER_COUNT);
                } else {
                    int nPoint = 0;
                    int iCCF_WINNER_INDEX = this->GetCurrentCCFWinUser(&nPoint);
                    ...
                    if (iCCF_WINNER_INDEX == -1) {
                        ...
                        g_Log.Add(
                                "[Chaos Castle Survival][CCS2] [%s][%s][%s][ServerCode: %d] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
                                gObj[iCCF_WINNER_INDEX].AccountID, gObj[iCCF_WINNER_INDEX].Name,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->m_RealNameOfUBF,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->m_nServerCodeOfHomeWorld,
                                gObj[iCCF_WINNER_INDEX].m_PlayerData->DbClass,
    ...
    if (this->m_stCCFData.m_nCCF_REMAIN_MSEC <= 0) {
        ...
        if (this->GetMonsterListCount() > 0) {
            ...
            if (iCCF_WINNER_INDEX == -1) {
                ...
                g_Log.Add(
                        "[Chaos Castle Survival][CCS3] [%s][%s][%s][ServerCode: %d] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
                        gObj[iCCF_WINNER_INDEX].AccountID, gObj[iCCF_WINNER_INDEX].Name,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->m_RealNameOfUBF,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->m_nServerCodeOfHomeWorld,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->DbClass,
                        gObj[iCCF_WINNER_INDEX].Level, gObj[iCCF_WINNER_INDEX].m_PlayerData->LevelUpPoint,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->Experience,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->Strength, gObj[iCCF_WINNER_INDEX].m_PlayerData->Dexterity,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->Vitality,
                        gObj[iCCF_WINNER_INDEX].m_PlayerData->Energy, gObj[iCCF_WINNER_INDEX].Leadership,
                        gObj[iCCF_WINNER_INDEX].MapNumber, gObj[iCCF_WINNER_INDEX].m_PK_Level);
    ...
    if (!this->m_bIsCCF_Ranking_Renew) {
        ...
        for (int i = 0; i < 50; i++) {
            if (this->m_CCFRanking[i].byRank > 0)
                g_Log.Add("[Chaos Castle Survival] No.%d Name: %s Point: %d", this->m_CCFRanking[i].byRank,
                          this->m_CCFRanking[i].szCharName, this->m_CCFRanking[i].nPoint);
        }
    ...
    for (int i = 0; i < 70; i++) {
        if (this->m_stCCFData.m_UserData[i].m_nIndex != -1
            ...
            if (gObj[this->m_stCCFData.m_UserData[i].m_nIndex].m_nCCF_UserIndex != -1) {
                ...
                int nPoint = gObj[this->m_stCCFData.m_UserData[i].m_nIndex].m_byKillMobCount +
                             2 * gObj[this->m_stCCFData.m_UserData[i].m_nIndex].m_byKillUserCount;

                this->GD_Req_Save_CCF_Result(this->m_stCCFData.m_UserData[i].m_nIndex,
                ...
                g_Log.Add(
                        "[Chaos Castle Survival][GAME-RESULT][FAIL-CASE] NAME:[%s] UBFNAME:[%s] SERVERCODE:[%d],MOBKILL:[%d],USERKILL:[%d],POINT:[%d]",
                        gObj[this->m_stCCFData.m_UserData[i].m_nIndex].Name,
...
void ChaosCastleFinal::SendCCFWinMessage(int iWinnerIndex, int nPoint, bool bFinalWinner) {
    ...
    if (gObj[iWinnerIndex].m_bCCF_Quit_Msg == false) {
        ...
        g_Log.Add(
                "[Chaos Castle Survival][GAME-RESULT][WIN-CASE] NAME:[%s] UBFNAME:[%s] SERVERCODE:[%d],MOBKILL:[%d],USERKILL:[%d],POINT:[%d]",
        ...
        if (gObj[iIndex].MapNumber == iMapNumber && gObj[iIndex].Connected > PLAYER_LOGGED) {
            if (gObj[iIndex].X >= iMIN_X && gObj[iIndex].X <= iMAX_X &&
                ...
                if (btMapAttr == 8) {
                    ...
                    g_Log.Add(
                            "[Chaos Castle Survival] [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
...
void ChaosCastleFinal::PullObjInnerPlace(int iTRAP_STEP) {
            ...
            if (gObjIsConnected(this->m_stCCFData.m_UserData[i].m_nIndex) == TRUE &&
                ...
                if (btMapAttr == 8) {
                    ...
                    if (vtMAP_UNTRAP[iUSER_AXIS].empty() == false) {
                        ...
                        if (this->ObjSetPosition(this->m_stCCFData.m_UserData[i].m_nIndex, ptEMPTY.x, ptEMPTY.y) ==
                            TRUE) {
    ...
    for (int iMON = 0; iMON < 100; iMON++) {
        ...
        if (gObj[iMON_INDEX].Life > 0.0f && gObj[iMON_INDEX].MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL != FALSE &&
...
void ChaosCastleFinal::SetMapAttrFill() {
    for (int iTRAP_STEP = 1; iTRAP_STEP < MAX_CCF_TRAP_STEP; iTRAP_STEP++) {
        for (int a = 0; a < MAX_CCF_HOLLOW_ZONE; a++) {
            for (int b = ::g_iCCF_MapHollowZone[iTRAP_STEP][a][0]; b <= ::g_iCCF_MapHollowZone[iTRAP_STEP][a][2]; b++) {
                for (int c = ::g_iCCF_MapHollowZone[iTRAP_STEP][a][1];
                     c <= ::g_iCCF_MapHollowZone[iTRAP_STEP][a][3]; c++) {
                    MapC[MAP_INDEX_CHAOSCASTLE_SURVIVAL].m_attrbuf[c * 256 + b] &= ~8;
                }
            }
        }
...
void ChaosCastleFinal::CheckMonsterInDieTile() {
    ...
    for (int iMON = 0; iMON < 100; iMON++) {
        ...
        if (gObj[iMON_INDEX].Life > 0.0f && gObj[iMON_INDEX].MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL &&
            gObj[iMON_INDEX].Connected > PLAYER_EMPTY) {
            int iSX = gObj[iMON_INDEX].X;
            int iSY = gObj[iMON_INDEX].Y;
...
void ChaosCastleFinal::ProcessFallUser() {
    for (int iFALL = 0; iFALL < 70; iFALL++) {
        if (this->m_stCCFData.m_lFallUser[iFALL] != -1) {
            ...
            if (gObj[iFALL_INDEX].MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL && gObjIsConnected(iFALL_INDEX) &&
...
int ChaosCastleFinal::GetFinalRemainTime(int iUserIndex) {
    ...
    for (std::vector<CCF_START_TIME>::iterator it = this->m_vtCCFOpenTime.begin();
        ...
        if (pRET.m_nFinal == 3) {
            iREMAIN_MINUTE = (60 * pRET.m_nMinute + 3600 * pRET.m_nHour -
                              (sysTime.wSecond + 60 * sysTime.wMinute + 3600 * sysTime.wHour)) / 60 + 1;
        }
    }
...
void ChaosCastleFinal::SearchCCFNDropMonsterItem(int iMonsterIndex, int iMaxHitUserIndex) {
        ...
        if (this->m_iCCF_BoxRatio1 >= nRand || nRand >= this->m_iCCF_BoxRatio2 || this->m_byBoxDropCnt != 1) {
            if (this->m_iCCF_BoxRatio2 >= nRand || nRand >= this->m_iCCF_BoxRatio3 || this->m_byBoxDropCnt != 2) {
                if (this->m_iCCF_BoxRatio3 >= nRand || nRand >= this->m_iCCF_BoxRatio4 || this->m_byBoxDropCnt != 3) {
                    if (this->m_iCCF_BoxRatio4 >= nRand || nRand >= this->m_iCCF_BoxRatio5 ||
                        this->m_byBoxDropCnt != 4) {
                        if (this->m_iCCF_BoxRatio5 >= nRand || nRand >= this->m_iCCF_BoxRatio6 ||
                            this->m_byBoxDropCnt != 5) {
    ...
    if (bDrop == TRUE) {
        ItemSerialCreateSend(iMaxHitUserIndex, gObj[iMaxHitUserIndex].MapNumber, gObj[iMaxHitUserIndex].X,
                             gObj[iMaxHitUserIndex].Y, iItemType, iLevel, 0, 0, 0, 0, iMaxHitUserIndex, 0, 0, 0, 0, 0);

        g_Log.Add("[Chaos Castle Survival] Monster Dropped ChaosCastleFinal Item To [%s][%s] (%d)(Item:%s)",
                  gObj[iMaxHitUserIndex].AccountID, gObj[iMaxHitUserIndex].Name, iMonsterIndex,
                  ItemAttribute[iItemType].Name);
    }
...
void ChaosCastleFinal::SetItemsToMonster(int iCCFType) {
    ...
    for (int iB = 0; iB < ::g_iCCF_MonsterItems[iCCFType - 1][1]; iB++) {
        for (int iC = 0; iC < 200; iC++) {
            ...
            std::map<int, _CCF_MONSTER_ITEM_DROP>::iterator it = this->m_stCCFData.m_mapCCFMonsterItemList.find(
                    iMONSTER_INDEX);
            ...
            if (it == this->m_stCCFData.m_mapCCFMonsterItemList.end()) {
                ...
                this->m_stCCFData.m_mapCCFMonsterItemList.insert(
                        std::pair<int, _CCF_MONSTER_ITEM_DROP>(iMONSTER_INDEX, MID));
    ...
    g_Log.Add("[Chaos Castle Survival] Set Monster Drop Item Count (%d)",
              this->m_stCCFData.m_mapCCFMonsterItemList.size());
}
...
void ChaosCastleFinal::GetRandomDropPos(BYTE &btPosX, BYTE &btPosY) {
    ...
    btPosX = GetLargeRand() % (g_iCCF_Jewel_Random_Drop[nRandValue][2] - g_iCCF_Jewel_Random_Drop[nRandValue][0] +
                               g_iCCF_Jewel_Random_Drop[nRandValue][0]);
    btPosY = GetLargeRand() % (g_iCCF_Jewel_Random_Drop[nRandValue][3] - g_iCCF_Jewel_Random_Drop[nRandValue][1] +
                               g_iCCF_Jewel_Random_Drop[nRandValue][1]);
}
...
void ChaosCastleFinal::UnSafetyCastleZone() {
    ...
    for (int iHOLE_NUM = 0; iHOLE_NUM < 2; iHOLE_NUM++) {
        for (int i = g_byCCFCenterHollowZoneMapXY[iHOLE_NUM][0]; i <= g_byCCFCenterHollowZoneMapXY[iHOLE_NUM][2]; i++) {
            for (int j = g_byCCFCenterHollowZoneMapXY[iHOLE_NUM][1];
                 j <= g_byCCFCenterHollowZoneMapXY[iHOLE_NUM][3]; j++) {
...
void ChaosCastleFinal::CheckSync() {
    if (this->m_vtCCFOpenTime.empty() != false) {
        g_Log.AddC(TColor::Red,
                   "[Chaos Castle Survival] No Schedule Time Data - Chaos Castle Survival Terminated (m_vtCCFOpenTime.empty())");
        this->SetState(0, 0);
        return;
    }
    ...
    switch (bTIME_CHANGED) {
        case TRUE:
            this->m_stCCFData.m_nCCF_REMAIN_MSEC = (((iMIN_HOUR * 60 * 60) + (iMIN_MINUTE * 60)) -
                                                    ((sysTime.wHour * 60 * 60) + (sysTime.wMinute * 60) +
                                                     (sysTime.wSecond))) * 1000;
            break;

        case 2:
            this->m_stCCFData.m_nCCF_REMAIN_MSEC = ((((iMIN_HOUR + 24) * 60 * 60) + (iMIN_MINUTE * 60)) -
                                                    ((sysTime.wHour * 60 * 60) + (sysTime.wMinute * 60) +
                                                     (sysTime.wSecond))) * 1000;
            break;
        ...
        default:
            g_Log.AddC(TColor::Red,
                       "[Chaos Castle Survival] No Schedule Time Data - Chaos Castle Terminated (bTIME_CHANGED=%d)",
                       bTIME_CHANGED);
            this->SetState(0, 0);
            return;
...
void ChaosCastleFinal::RewardUserEXP(BOOL bWinner) {
    for (int i = 0; i < 70; i++) {
        if (this->m_stCCFData.m_UserData[i].m_nIndex != -1
            ...
            int iREWARD_EXP = this->CalcSendRewardEXP(this->m_stCCFData.m_UserData[i].m_nIndex, iTOT_EXP,
                                                      iKILLCOUNT_USER, iKILLCOUNT_MONSTER);

            g_Log.Add(
                    "[Chaos Castle Survival] [%s][%s] Reward User EXP (USER_KILL:%d, MON_KILL:%d, TOT_EXP:%d, TOT_REWARD_EXP:%d)",
                    gObj[this->m_stCCFData.m_UserData[i].m_nIndex].AccountID,
                    gObj[this->m_stCCFData.m_UserData[i].m_nIndex].Name,
...
bool ChaosCastleFinal::CheckCanStart_CCF() {
    ...
    if (nEnteredUserCount >= this->m_nCCF_MinUserCount) {
        ...
        g_Log.Add("[Chaos Castle Survival][CheckCanStart_CCS] GetCurPlayUser() FAILED (UserCount:%d) (NeedMinCount:%d)",
                  nEnteredUserCount, this->m_nCCF_MinUserCount);
        return false;
    }
}
    ...
    for (int i = 0; i < 70; i++) {
        if (this->m_stCCFData.m_UserData[i].m_nIndex != -1 &&
            ...
            if (this->m_stCCFData.m_UserData[i].m_nIndex != -1) {
                ...
                GSProtocol.GCMoneySend(this->m_stCCFData.m_UserData[i].m_nIndex,
                                       gObj[this->m_stCCFData.m_UserData[i].m_nIndex].m_PlayerData->Money);
                gObjMoveGate(this->m_stCCFData.m_UserData[i].m_nIndex, 333);
                g_Log.Add("[Chaos Castle Survival][CCF_Start_Fail_So_RollBack] INDEX: %d ",
                          this->m_stCCFData.m_UserData[i].m_nIndex);
...
void ChaosCastleFinal::GD_Req_Save_CCF_Result(int index, char *Name, int Point, int nCCFtype) {
    ...
    g_Log.Add("[Chaos Castle Survival][GD_Req_Save_CCF_Result] pMsg.szCharName:%s, obj.Name :%s", pMsg.szCharName,
              gObj[index].Name);
...
int ChaosCastleFinal::CheckRankingReqValid(BYTE byCCFType) {
    ...
    if (byCCFType != 1 ||
        sysTime.wDay != this->m_byTryOut1 && sysTime.wDay != this->m_byTryOut2 && sysTime.wDay != this->m_byTryOut3) {
        if (byCCFType != 2 || sysTime.wDay != this->m_bySemiFinal1 && sysTime.wDay != this->m_bySemiFinal2 &&
                              sysTime.wDay != this->m_byFinal) {
...
void ChaosCastleFinal::Send_RankingInfo(BYTE byCCFType, int aIndex) {
    ...
    if (nUserCnt || CanIsSeeRankingInfo == -1) {
        if (CanIsSeeRankingInfo == -1) {
            pMsg.btResult = 2;
            g_Log.Add(
                    "[Chaos Castle Survival][Send_RangkingInfo][ERROR] CANNOT SEE RANKING==> CanIseeRankInfo:%d, nUserCnt:%d ",
                    -1, nUserCnt);
        } else {
            pMsg.btResult = 0;
            g_Log.Add("[Chaos Castle Survival][Send_RangkingInfo] RankingInfo ==> CanIseeRankInfo:%d, nUserCnt:%d ",
                      CanIsSeeRankingInfo, nUserCnt);
        }
    } else {
        pMsg.btResult = 1;
        g_Log.Add(
                "[Chaos Castle Survival][Send_RangkingInfo][ERROR] No RankingInfo ==> CanIseeRankInfo:%d, nUserCnt:%d ",
                CanIsSeeRankingInfo, nUserCnt);
    }
...
void ChaosCastleFinal::SendAllMemberOfCCF(int index) {
    ...
    for (int i = 0; i < 70; i++) {
        if (this->m_stCCFData.m_UserData[i].m_nIndex != -1 &&
            gObj[this->m_stCCFData.m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED) {
            LPOBJ lpObj = &gObj[this->m_stCCFData.m_UserData[i].m_nIndex];

            if (lpObj) {
                char szMsg[128];
                wsprintf(szMsg, "[CCF Entry]-> No.%d, Name:%s, Level:%d, MasterLevel:%d, Class%d", i, lpObj->Name,
                         lpObj->Level, lpObj->m_PlayerData->MasterLevel, lpObj->Class);
...
void ChaosCastleFinal::MakeChaosCastleRewardWing(int iUserIndex) {
    ItemSerialCreateSend(iUserIndex, gObj[iUserIndex].MapNumber, gObj[iUserIndex].X, gObj[iUserIndex].Y,
                         ITEMGET(this->m_stCCFFinalReward.m_btCat, this->m_stCCFFinalReward.m_wIndex),
                         this->m_stCCFFinalReward.m_btLevel, this->m_stCCFFinalReward.m_btDurability,
                         this->m_stCCFFinalReward.m_btSkill, this->m_stCCFFinalReward.m_btLuck,
                         this->m_stCCFFinalReward.m_btOption, iUserIndex, this->m_stCCFFinalReward.m_btNewOption, 0,
                         this->m_stCCFFinalReward.m_dwExpiryTime, 0, 0);
}
...
void ChaosCastleFinal::GDReqSetCCFReward_UBF(int iUserIndex, BYTE btCCFType, BYTE btRewardType) {
    ...
    g_Log.Add(
            "[UBF][ChaosCastleFinal][GDReqSetCCFWinerInformation]ACC:%s , NAME :%s,Real NAME :%s, CCFType:%d, RewardType:%d",

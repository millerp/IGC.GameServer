...
int CNewPVP::Reserve(OBJECTSTRUCT &requester, OBJECTSTRUCT &responsor) {
    ...
    if (!IsVulcanusMap(requester.MapNumber)) {
        if (IsVulcanusMap(requester.MapNumber) || requester.m_PlayerData->Money - lpData->NeedZen < 0)
            return ENEWPVP::E_NOT_ENOUGH_MONEY;
    }
    if (!IsVulcanusMap(responsor.MapNumber)) {
        if (IsVulcanusMap(responsor.MapNumber) || responsor.m_PlayerData->Money - lpData->NeedZen < 0)
            return ENEWPVP::E_NOT_ENOUGH_MONEY;
    }
    ...
    std::pair<std::map<int, _tagWaiting>::iterator, bool> pair = m_Waiting.insert(
            std::make_pair(requester.m_Index, waiting));
...
int CNewPVP::Join(OBJECTSTRUCT &requester, OBJECTSTRUCT &responsor) {
    ...
    if (!(iter != m_Waiting.end())) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "iter!=m_Waiting.end()", "ENEWPVP::E_NOT_EXIST_USER", "NULL", __FILE__,
                  __LINE__);
    ...
    if (m_DuelChannel[nId].nStatus != DC_IDLE) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "m_DuelChannel[nId].nStatus==DC_IDLE", "ENEWPVP::E_INVALID_STATUS", "NULL",
                  __FILE__, __LINE__);
        return ENEWPVP::E_INVALID_STATUS;
    }
    ...
    g_Log.Add("[NewPVP] [%s][%s] Duel Started [%s][%s]", responsor.AccountID, responsor.Name, requester.AccountID,
              requester.Name);
    return 0;
}
...
int CNewPVP::Leave(OBJECTSTRUCT &obj) {
    ...
    if (nId < 0 || nId >= DUEL_CHANNEL_MAX) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "nId>=0 && nId<DUEL_CHANNEL_MAX", "ENEWPVP::E_INVALID_CHANNELID", "NULL",
                  __FILE__, __LINE__);
        return ENEWPVP::E_INVALID_CHANNELID;
    }
    if (!IS_START(m_DuelChannel[nId].nStatus) && m_DuelChannel[nId].nStatus != DC_RESERVEDEND) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d",
                  "IS_START(m_DuelChannel[nId].nStatus)||m_DuelChannel[nId].nStatus==DC_RESERVEDEND",
                  "ENEWPVP::E_INVALID_STATUS", "NULL", __FILE__, __LINE__);
        return ENEWPVP::E_INVALID_STATUS;
    }
    ...
    if (lpTargetObj == NULL) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "lpTargetObj", "ENEWPVP::E_INVALID_INDEX", "NULL", __FILE__, __LINE__);
        return ENEWPVP::E_INVALID_INDEX;
    }
...
void CNewPVP::Leave(OBJECTSTRUCT &requester, OBJECTSTRUCT &responsor) {
    ...
    if (nId < 0 || nId >= DUEL_CHANNEL_MAX) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "nId>=0 && nId<DUEL_CHANNEL_MAX", "0", "NULL", __FILE__, __LINE__);
        return;
    }
    if (!IS_START(m_DuelChannel[nId].nStatus) && m_DuelChannel[nId].nStatus != DC_RESERVEDEND) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d",
                  "IS_START(m_DuelChannel[nId].nStatus)||m_DuelChannel[nId].nStatus==DC_RESERVEDEND", "0", "NULL",
                  __FILE__, __LINE__);
        return;
    }
...
void CNewPVP::SetScore(OBJECTSTRUCT &obj) {
    ...
    if (!IS_START(m_DuelChannel[nId].nStatus)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "IS_START(m_DuelChannel[nId].nStatus)", "0", "NULL", __FILE__, __LINE__);
        return;
    }
    obj.m_btDuelScore++;
}
...
void CNewPVP::ReFill(OBJECTSTRUCT &obj) {
    if (!gObjIsConnected(&obj)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "gObjIsConnected(&obj)", "0", "NULL", __FILE__, __LINE__);
        return;
    }
...
int CNewPVP::CheckUsersOnConnect(int nId) {
    if (nId < 0 || nId >= DUEL_CHANNEL_MAX) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "nId>=0 && nId<DUEL_CHANNEL_MAX", "FALSE", "NULL", __FILE__, __LINE__);
        return FALSE;
    }
    if (!IS_START(m_DuelChannel[nId].nStatus)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "IS_START(m_DuelChannel[nId].nStatus)", "FALSE", "NULL", __FILE__, __LINE__);
        return FALSE;
    }
...
void CNewPVP::ChatMsgSend(OBJECTSTRUCT &obj, BYTE *Msg, int size) {
    ...
    if (m_DuelChannel[nId].nStatus == DC_IDLE) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "m_DuelChannel[nId].nStatus!=DC_IDLE", "0", "NULL", __FILE__, __LINE__);
        return;
    }
...
void CNewPVP::GetObserverList(int nId, PMSG_DUEL_OBSERVERLIST_BROADCAST &res) {
    ...
    for (std::map<int, _tagObserverInfo>::iterator iter = m_ObserverInfoList.begin();
         iter != m_ObserverInfoList.end(); iter++) {
        ObserverInfo &info = iter->second;
        if (info.nId == nId) {
...
int CNewPVP::JoinChannel(int nId, OBJECTSTRUCT &obj) {
    if (IsDuel(obj)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "!IsDuel(obj)", "ENEWPVP::E_ALREADY_DUELLING", "NULL", __FILE__, __LINE__);
        return ENEWPVP::E_ALREADY_DUELLING;
    }
    if (IsDuelRequested(obj)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "!IsDuelRequested(obj)", "ENEWPVP::E_ALREADY_DUELREQUESTED", "NULL", __FILE__,
                  __LINE__);
        return ENEWPVP::E_ALREADY_DUELREQUESTED;
    }
    if (IsDuelReserved(obj)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "!IsDuelReserved(obj)", "ENEWPVP::E_ALREADY_DUELRESERVED", "NULL", __FILE__,
                  __LINE__);
    ...
    if (!IS_START(m_DuelChannel[nId].nStatus)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "IS_START(m_DuelChannel[nId].nStatus)", "ENEWPVP::E_INVALID_STATUS", "NULL",
                  __FILE__, __LINE__);
        return ENEWPVP::E_INVALID_STATUS;
    }
    ...
    EnterCriticalSection(&this->m_csObserver);
    std::pair<std::map<int, _tagObserverInfo>::iterator, bool> pair = m_ObserverInfoList.insert(
            std::make_pair(obj.m_Index, info));
...
void CNewPVP::BroadcastResult(int nId, BYTE nFlag, OBJECTSTRUCT &obj) {
    ...
    if (m_DuelChannel[nId].nStatus != DC_RESERVEDEND) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "m_DuelChannel[nId].nStatus==DC_RESERVEDEND", "0", "NULL", __FILE__, __LINE__);
        return;
    }
    if (!gObjIsConnected(obj.m_Index)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "gObjIsConnected(obj.m_Index)", "0", "NULL", __FILE__, __LINE__);
        return;
    }
    if (!IsDuel(obj)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "IsDuel(obj)", "0", "NULL", __FILE__, __LINE__);
        return;
    }
...
void CNewPVP::BroadcastDuelInfo(int nId, BYTE nFlag) {
    ...
    ShieldPer1 = ((float) (requester.iShield) / (float) ((int) (requester.iMaxShield) + (int) (requester.iAddShield)) *
                  100.0f);
    ShieldPer2 = ((float) (responsor.iShield) / (float) ((int) (responsor.iMaxShield) + (int) (responsor.iAddShield)) *
                  100.0f);
...
void CNewPVP::BroadcastMessage(int nId, BYTE nFlag, BYTE nMsgType, int nNotifySec) {
    ...
    switch (nMsgType) {
        case 0:
            TNotice::MakeNoticeMsgEx(&res, 1, Lang.GetText(0, 323), nNotifySec);
            break;
        case 1:
            TNotice::MakeNoticeMsgEx(&res, 1, Lang.GetText(0, 324), nNotifySec);
            break;
        case 2:
            TNotice::MakeNoticeMsgEx(&res, 1, Lang.GetText(0, 325), nNotifySec);
            break;
        default:
            return;
    }
...
BOOL CNewPVP::DropItem(LPOBJECTSTRUCT lpObj, LPOBJECTSTRUCT lpMonsterObj) {
    if (!IsVulcanusMap(lpObj->MapNumber)) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "IsVulcanusMap(lpObj->MapNumber)", "FALSE", "NULL", __FILE__, __LINE__);
        return FALSE;
    }
    if (m_bNewPVP != TRUE) {
        g_Log.Add("%s\t%s\t%s\t%s\t%d", "m_bNewPVP==TRUE", "FALSE", "NULL", __FILE__, __LINE__);
        return FALSE;
    }

...
g_Log.
LogInit(g_ConfigRead
.server.

IsLogCreate(), g_ConfigRead
...
wsprintf(Path,
"..\\data-%d\\", g_ConfigRead.server.

GetGameServerCode()

);
wsprintf(IGCPath,
"..\\IGCData-%d\\", g_ConfigRead.server.

GetGameServerCode()

);
wsprintf(CommonServerPath,
"..\\data-%d\\commonserver.cfg", g_ConfigRead.server.
...
if(
FindFirstFile(CommonServerPath, &wfd
...
{
g_SendNPCInfo.
LoadScript(g_ConfigRead
.GetPath("IGC_MiniMap_BattleCore.xml"));
}
...
g_SendNPCInfo.
LoadScript(g_ConfigRead
.GetPath("IGC_MiniMap.xml"));
}

g_MapServerManager.
LoadData(g_ConfigRead
.GetPath("IGC_MapServerInfo.xml"));

ReadCommonServerInfo();

DWORD dwThreadId;
CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)
GameServerInfoSend, NULL, NULL, &dwThreadId);
...
if (g_CastleSiege.
LoadData(g_ConfigRead
.GetPath("\\Events\\IGC_CastleSiege.xml")))
{
g_CastleSiege.
LoadPreFixData(g_ConfigRead
.GetPath("\\Events\\IGC_CastleSiege.xml"));
...
gMAttr.
LoadAttr(g_ConfigRead
.GetPath("\\Monsters\\IGC_MonsterList.xml"));
gMSetBase.
LoadSetBase(g_ConfigRead
.GetPath("\\Monsters\\IGC_MonsterSpawn.xml"));
gEventMSetBase.
LoadScript(g_ConfigRead
.GetPath("\\Monsters\\IGC_MonsterSpawn_Event.xml"));
...
TempLua->
DoFile(g_ConfigRead
.GetPath("\\Scripts\\Misc\\ExpCalc.lua"));
double exp = 0.0;

for (
int n = 1;
n<=g_ConfigRead.data.common.
UserMaxLevel;
n++)
{
...
g_ShopMng.
LoadShopList(g_ConfigRead
.GetPath("IGC_ShopList.xml"));
wsGServer.
CreateSocket(hWnd);
...
Luna<QuestExpLuaBind>::Register(g_MuLuaQuestExp
.
...
g_MuLuaQuestExp.
DoFile(g_ConfigRead
.GetPath("\\Scripts\\Quests\\Quest_Info.lua"));
g_MuLuaQuestExp.
DoFile(g_ConfigRead
.GetPath("\\Scripts\\Quests\\Quest_Main.lua"));
g_Generic_Call(g_MuLuaQuestExp
.
...
{
DWORD ThreadId;
CreateThread(nullptr, 0, reinterpret_cast
<LPTHREAD_START_ROUTINE>(OffLevelThread),
nullptr, 0, &ThreadId);
}
}
...
BOOL GMDataServerConnect(char *ConnectServer, WPARAM wMsg) {
    ...
    if (result == 0) {
        g_Log.Add("DataServer connect fail IP [ %s ] PORT [ %d ]", ConnectServer,
                  g_ConfigRead.server.GetDataServerPort());
        return FALSE;
    }
...
void GMJoinClientMsgProc(WPARAM
wParam,
LPARAM lParam
)
{
...
g_Log.
AddC(TColor::Red,
"Game server closed because Authentication server is closed.");
...
void ExDataClientMsgProc(WPARAM
wParam,
LPARAM lParam
)
{
...
pMsg.h.
set((LPBYTE)
&pMsg, 0xC4 , sizeof(pMsg));
pMsg.
State = -4;

IOCP.
DataSend(i, (
unsigned char*)&pMsg, sizeof(pMsg));

gObj[i].
m_FriendServerOnline = FRIEND_SERVER_STATE_OFFLINE;
...
gObj[i].m_PlayerData->
m_nPartyMatchingIndex = -1;
gObj[i].m_PlayerData->
m_bUsePartyMatching = false;
gObj[i].m_PlayerData->
m_bPartyMatchingLeader = false;
gObj[i].m_PlayerData->
m_nPartyMatchingMemberCount = 0;
gObj[i].m_PlayerData->
m_bUseGuildMatching = false;
}
}
...
g_Log.
AddC(TColor::Red,
"Error-L1 : Friend Server Down (State Send Ok)");
break;
}
}


void GMDataClientMsgProc(WPARAM
wParam,
LPARAM lParam
)
{
...
void ReadCommonServerInfo() {
    ...
    gStalkProtocol = GetPrivateProfileInt("GameServerInfo", "StalkProtocol", 0,
                                          gDirPath.GetNewPath("commonserver.cfg"));
    GetPrivateProfileString("GameServerInfo", "StalkProtocolId", "0", gStalkProtocolId, 10,
                            gDirPath.GetNewPath("commonserver.cfg"));
    ...
    g_Log.Add("[Option] Connect Server IP(%s) / PORT(%d)", g_ConfigRead.server.GetConnectServerIP(),
              g_ConfigRead.server.GetConnectServerPort());
    ...
    gCreateCharacter = GetPrivateProfileInt("GameServerInfo", "CreateCharacter", 1,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    gCharacterDeleteMinLevel = GetPrivateProfileInt("GameServerInfo", "CharacterDeleteMinLevel", 40,
                                                    gDirPath.GetNewPath("commonserver.cfg"));

    // Guild Settingss
    g_iServerGroupGuildChatting = GetPrivateProfileInt("GameServerInfo", "ServerGroupGuildChatting", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    g_iServerGroupUnionChatting = GetPrivateProfileInt("GameServerInfo", "ServerGroupUnionChatting", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));

    SpeedHackPlayerBlock = GetPrivateProfileInt("GameServerInfo", "SpeedHackPlayerBlock", 0,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    gItemSerialCheck = GetPrivateProfileInt("GameServerInfo", "ItemSerialCheck", 1,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    gSerialZeroCheck = GetPrivateProfileInt("GameServerInfo", "SerialZeroCheck", 1,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    bCanTrade = GetPrivateProfileInt("GameServerInfo", "Trade", 1, gDirPath.GetNewPath("commonserver.cfg"));

    gLevelUpPointNormal = GetPrivateProfileInt("GameServerInfo", "LevelUpPointNormal", 5,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    gLevelUpPointMGDL = GetPrivateProfileInt("GameServerInfo", "LevelUpPointMGDL", 7,
                                             gDirPath.GetNewPath("commonserver.cfg"));

    gItemDisapearTime = GetPrivateProfileInt("GameServerInfo", "ItemDisapearTime", 120,
                                             gDirPath.GetNewPath("commonserver.cfg"));
    ...
    gExcDropRate = GetPrivateProfileInt("GameServerInfo", "ExcItemDropPer", 2000,
                                        gDirPath.GetNewPath("commonserver.cfg"));
    gSkillDropRate = GetPrivateProfileInt("GameServerInfo", "ItemSkillDrop", 6,
                                          gDirPath.GetNewPath("commonserver.cfg"));
    gLuckDropRate = GetPrivateProfileInt("GameServerInfo", "ItemLuckDrop", 4, gDirPath.GetNewPath("commonserver.cfg"));
    gExcSkillDropRate = GetPrivateProfileInt("GameServerInfo", "ExcItemSkillDrop", 100,
                                             gDirPath.GetNewPath("commonserver.cfg"));
    gExcLuckDropRate = GetPrivateProfileInt("GameServerInfo", "ExcItemLuckDrop", 1,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    g_Log.Add("[Option] Item drop rate : %d%%", gItemDropPer);
    gZenDurationTime = GetPrivateProfileInt("GameServerInfo", "ZenDurationTime", 30,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    gEventChipEvent = GetPrivateProfileInt("GameServerInfo", "EventChipEvent", 0,
                                           gDirPath.GetNewPath("commonserver.cfg"));
    gBoxOfGoldDropRate = GetPrivateProfileInt("GameServerInfo", "BoxOfGoldDropRate", 2,
                                              gDirPath.GetNewPath("commonserver.cfg"));
    g_ItemDropRateForBoxOfGold = GetPrivateProfileInt("GameServerInfo", "ItemDropRateForBoxOfGold", 2,
                                                      gDirPath.GetNewPath("commonserver.cfg"));
    g_EventChipDropRateForBoxOfGold = GetPrivateProfileInt("GameServerInfo", "EventChipDropRateForBoxOfGold", 2,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    gSilverBoxDropRate = GetPrivateProfileInt("GameServerInfo", "SilverBoxDropRate", 10,
                                              gDirPath.GetNewPath("commonserver.cfg"));
    gGoldBoxDropRate = GetPrivateProfileInt("GameServerInfo", "GoldBoxDropRate", 10,
                                            gDirPath.GetNewPath("commonserver.cfg"));

    gSetPartyExp2 = GetPrivateProfileInt("GameServerInfo", "SetParty2ExpBonus", 120,
                                         gDirPath.GetNewPath("commonserver.cfg"));
    gSetPartyExp3 = GetPrivateProfileInt("GameServerInfo", "SetParty3ExpBonus", 230,
                                         gDirPath.GetNewPath("commonserver.cfg"));
    gSetPartyExp4 = GetPrivateProfileInt("GameServerInfo", "SetParty4ExpBonus", 270,
                                         gDirPath.GetNewPath("commonserver.cfg"));
    gSetPartyExp5 = GetPrivateProfileInt("GameServerInfo", "SetParty5ExpBonus", 300,
                                         gDirPath.GetNewPath("commonserver.cfg"));

    gPartyExp1 = GetPrivateProfileInt("GameServerInfo", "NormalParty1ExpBonus", 150,
                                      gDirPath.GetNewPath("commonserver.cfg"));
    gPartyExp2 = GetPrivateProfileInt("GameServerInfo", "NormalParty2ExpBonus", 160,
                                      gDirPath.GetNewPath("commonserver.cfg"));
    gPartyExp3 = GetPrivateProfileInt("GameServerInfo", "NormalParty3ExpBonus", 180,
                                      gDirPath.GetNewPath("commonserver.cfg"));
    gPartyExp4 = GetPrivateProfileInt("GameServerInfo", "NormalParty4ExpBonus", 200,
                                      gDirPath.GetNewPath("commonserver.cfg"));
    gPartyExp5 = GetPrivateProfileInt("GameServerInfo", "NormalParty5ExpBonus", 220,
                                      gDirPath.GetNewPath("commonserver.cfg"));

    IsMultiWareHouse = GetPrivateProfileIntA("GameServerInfo", "IsMultiWareHouse", 1,
                                             gDirPath.GetNewPath("commonserver.cfg"));
    g_WareHouseCount = GetPrivateProfileInt("GameServerInfo", "MultiWareHouseCount", 3,
                                            gDirPath.GetNewPath("commonserver.cfg"));

    bIsIgnorePacketSpeedHackDetect = GetPrivateProfileInt("GameServerInfo", "IsIgnorePacketHackDetect", 0,
                                                          gDirPath.GetNewPath("commonserver.cfg"));

    // Ring of Transform
    gIsItemDropRingOfTransform = GetPrivateProfileInt("GameServerInfo", "IsItemDropRingOfTransform", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));
    gItemDropRingOfTransform = GetPrivateProfileInt("GameServerInfo", "ItemDropRingOfTransform", 1,
                                                    gDirPath.GetNewPath("commonserver.cfg"));
    ...
    // Hack Settinhs
    gDisconnectHackUser = GetPrivateProfileInt("GameServerInfo", "DisconnectHackUser", 0,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    GetPrivateProfileString("GameServerInfo", "DecTimePerAttackSpeed", "5.33", szTemp, 5,
                            gDirPath.GetNewPath("commonserver.cfg"));
    gDecTimePerAttackSpeed = atof(szTemp);

    // Devil Square Event
    gDQChaosSuccessRateLevel1 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel1", 75,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));
    gDQChaosSuccessRateLevel2 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel2", 70,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));
    gDQChaosSuccessRateLevel3 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel3", 65,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));
    gDQChaosSuccessRateLevel4 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel4", 60,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));
    gDQChaosSuccessRateLevel5 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel5", 55,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));
    gDQChaosSuccessRateLevel6 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel6", 50,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));
    gDQChaosSuccessRateLevel7 = GetPrivateProfileInt("EventMixes", "DQChaosSuccessRateLevel7", 45,
                                                     g_ConfigRead.GetPath("IGC_ChaosBox.ini"));

    // Skill Log
    gWriteSkillLog = GetPrivateProfileInt("GameServerInfo", "WriteSkillLog", 0,
                                          gDirPath.GetNewPath("commonserver.cfg"));

    // Marlon Teleport
    gQuestNPCTeleportTime = GetPrivateProfileInt("GameServerInfo", "QuestNPCTeleportTime", 900,
                                                 gDirPath.GetNewPath("commonserver.cfg"));

    // Mark Lord
    g_iMarkOfTheLord = GetPrivateProfileInt("GameServerInfo", "MarkOfTheLord", 0,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    ...
    g_MaxStrength = GetPrivateProfileInt("GameServerInfo", "MaxStrength", 65535,
                                         gDirPath.GetNewPath("commonserver.cfg"));
    g_MaxAgility = GetPrivateProfileInt("GameServerInfo", "MaxAgility", 65535, gDirPath.GetNewPath("commonserver.cfg"));
    g_MaxVitality = GetPrivateProfileInt("GameServerInfo", "MaxVitality", 65535,
                                         gDirPath.GetNewPath("commonserver.cfg"));
    ...
    // Ribbon Box Event
    g_bRibbonBoxEvent = GetPrivateProfileInt("GameServerInfo", "RibbonBoxEvent", 0,
                                             gDirPath.GetNewPath("commonserver.cfg"));

    // Red Ribbon Box
    g_iRedRibbonBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "RedRibbonBoxDropLv_Min", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    g_iRedRibbonBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "RedRibbonBoxDropLv_Max", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    g_iRedRibbonBoxDropRate = GetPrivateProfileInt("GameServerInfo", "RedRibbonBoxDropRate", 0,
                                                   gDirPath.GetNewPath("commonserver.cfg"));

    // Green Ribbon Box
    g_iGreenRibbonBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "GreenRibbonBoxDropLv_Min", 0,
                                                         gDirPath.GetNewPath("commonserver.cfg"));
    g_iGreenRibbonBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "GreenRibbonBoxDropLv_Max", 0,
                                                         gDirPath.GetNewPath("commonserver.cfg"));
    g_iGreenRibbonBoxDropRate = GetPrivateProfileInt("GameServerInfo", "GreenRibbonBoxDropRate", 0,
                                                     gDirPath.GetNewPath("commonserver.cfg"));


    // Blue Ribbon Box
    g_iBlueRibbonBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "BlueRibbonBoxDropLv_Min", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iBlueRibbonBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "BlueRibbonBoxDropLv_Max", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iBlueRibbonBoxDropRate = GetPrivateProfileInt("GameServerInfo", "BlueRibbonBoxDropRate", 0,
                                                    gDirPath.GetNewPath("commonserver.cfg"));



    // Chocolate Event
    g_bChocolateBoxEvent = GetPrivateProfileInt("GameServerInfo", "ChocolateEvent", 0,
                                                gDirPath.GetNewPath("commonserver.cfg"));

    // Pink Chocolate Box
    g_iPinkChocolateBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "PinkChocolateBoxDropLv_Min", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iPinkChocolateBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "PinkChocolateBoxDropLv_Max", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iPinkChocolateBoxDropRate = GetPrivateProfileInt("GameServerInfo", "PinkChocolateBoxDropRate", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));


    // Red Chocolate Box
    g_iRedChocolateBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "RedChocolateBoxDropLv_Min", 0,
                                                          gDirPath.GetNewPath("commonserver.cfg"));
    g_iRedChocolateBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "RedChocolateBoxDropLv_Max", 0,
                                                          gDirPath.GetNewPath("commonserver.cfg"));
    g_iRedChocolateBoxDropRate = GetPrivateProfileInt("GameServerInfo", "RedChocolateBoxDropRate", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));


    // Blue Chocolate Box
    g_iBlueChocolateBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "BlueChocolateBoxDropLv_Min", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iBlueChocolateBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "BlueChocolateBoxDropLv_Max", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iBlueChocolateBoxDropRate = GetPrivateProfileInt("GameServerInfo", "BlueChocolateBoxDropRate", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));


    // Candy Event
    g_bCandyBoxEvent = GetPrivateProfileInt("GameServerInfo", "CandyBoxEvent", 0,
                                            gDirPath.GetNewPath("commonserver.cfg"));

    // LightPurple Candy Box
    g_iLightPurpleCandyBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "LightPurpleCandyBoxDropLv_Min", 0,
                                                              gDirPath.GetNewPath("commonserver.cfg"));
    g_iLightPurpleCandyBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "LightPurpleCandyBoxDropLv_Max", 0,
                                                              gDirPath.GetNewPath("commonserver.cfg"));
    g_iLightPurpleCandyBoxDropRate = GetPrivateProfileInt("GameServerInfo", "LightPurpleCandyBoxDropRate", 0,
                                                          gDirPath.GetNewPath("commonserver.cfg"));

    // Vermilion Candy Box
    g_iVermilionCandyBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "VermilionCandyBoxDropLv_Min", 0,
                                                            gDirPath.GetNewPath("commonserver.cfg"));
    g_iVermilionCandyBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "VermilionCandyBoxDropLv_Max", 0,
                                                            gDirPath.GetNewPath("commonserver.cfg"));
    g_iVermilionCandyBoxDropRate = GetPrivateProfileInt("GameServerInfo", "VermilionCandyBoxDropRate", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));

    // DeepBlue Candy Box
    g_iDeepBlueCandyBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "DeepBlueCandyBoxDropLv_Min", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iDeepBlueCandyBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "DeepBlueCandyBoxDropLv_Max", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iDeepBlueCandyBoxDropRate = GetPrivateProfileInt("GameServerInfo", "DeepBlueCandyBoxDropRate", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));


    // KUNDUN Settings
    giKundunRefillHPSec = GetPrivateProfileInt("GameServerInfo", "KundunRefillHPSec", 500,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    giKundunRefillHP = GetPrivateProfileInt("GameServerInfo", "KundunRefillHP", 10000,
                                            gDirPath.GetNewPath("commonserver.cfg"));
    giKundunRefillHPTime = GetPrivateProfileInt("GameServerInfo", "KundunRefillHPTime", 600,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    giKundunHPLogSaveTime = GetPrivateProfileInt("GameServerInfo", "KundunHPLogSaveTime", 120,
                                                 gDirPath.GetNewPath("commonserver.cfg"));

    // Fenrir Settings
    g_bFenrirStuffItemDrop = GetPrivateProfileInt("GameServerInfo", "FenrirStuffItemDrop", 0,
                                                  gDirPath.GetNewPath("commonserver.cfg"));

    // Fenrir Stuff 1
    g_iFenrirStuff_01_DropLv_Min = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropLv_Min", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_01_DropLv_Max = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropLv_Max", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_01_DropMap = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropMap", 0,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_01_DropRate = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropRate", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    // Fenrir Stuff 2
    g_iFenrirStuff_02_DropLv_Min = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropLv_Min", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_02_DropLv_Max = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropLv_Max", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_02_DropMap = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropMap", 0,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_02_DropRate = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropRate", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    // Fenrir Stuff 3
    g_iFenrirStuff_03_DropLv_Min = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropLv_Min", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_03_DropLv_Max = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropLv_Max", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_03_DropMap = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropMap", 0,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirStuff_03_DropRate = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropRate", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    // Fenrir Durability Settings
    g_iFenrirRepairRate = GetPrivateProfileInt("GameServerInfo", "FenrirRepairRate", 4000,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirDefaultMaxDurSmall = GetPrivateProfileInt("GameServerInfo", "FenrirDefaultMaxDurSmall", 200,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrirElfMaxDurSmall = GetPrivateProfileInt("GameServerInfo", "FenrirElfMaxDurSmall", 160,
                                                   gDirPath.GetNewPath("commonserver.cfg"));

    // Fenrir Mix Rate
    g_iFenrir_01Level_MixRate = GetPrivateProfileInt("GameServerInfo", "Fenrir_01Level_MixRate", 70,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrir_02Level_MixRate = GetPrivateProfileInt("GameServerInfo", "Fenrir_02Level_MixRate", 50,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iFenrir_03Level_MixRate = GetPrivateProfileInt("GameServerInfo", "Fenrir_03Level_MixRate", 30,
                                                     gDirPath.GetNewPath("commonserver.cfg"));

    // Hallowin Event
    g_bHallowinDayEventOn = GetPrivateProfileInt("GameServerInfo", "HallowinEventOn", 0,
                                                 gDirPath.GetNewPath("commonserver.cfg"));

    g_iHallowinDayEventItemDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventPumpkinOfLuckDropRate", 100,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    g_iHallowinDayEventJOLBlessDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLBlessDropRate", 10,
                                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iHallowinDayEventJOLAngerDropRaTe = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLAngerDropRate", 15,
                                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iHallowinDayEventJOLScreamDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLScreamDropRate", 15,
                                                                gDirPath.GetNewPath("commonserver.cfg"));
    g_iHallowinDayEventJOLFoodDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLFoodDropRate", 30,
                                                              gDirPath.GetNewPath("commonserver.cfg"));
    g_iHallowinDayEventJOLDrinkDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLDrinkDropRate", 25,
                                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iHallowinDayEventJOLPolymorphRingDropRate = GetPrivateProfileInt("GameServerInfo",
                                                                       "HallowinEventJOLPolymorphRingDropRate", 5,
                                                                       gDirPath.GetNewPath("commonserver.cfg"));

    // SantaRing Item Drop
    g_bSantaPolymorphRingDropOn = GetPrivateProfileInt("GameServerInfo", "SantaPolymorphRingDropOn", 0,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    g_iSantaPolymorphRingDropRate = GetPrivateProfileInt("GameServerInfo", "SantaPolymorphRingDropRate", 20,
                                                         gDirPath.GetNewPath("commonserver.cfg"));

    // Condor Flame Drop
    g_iCondorFlameDropRate = GetPrivateProfileInt("GameServerInfo", "CondorFlameDropRate", 20,
                                                  gDirPath.GetNewPath("commonserver.cfg"));

    // Shadow Phantom NPC
    g_iShadowPhantomMaxLevel = GetPrivateProfileInt("GameServerInfo", "ShadowPhantomMaxLevel", 220,
                                                    gDirPath.GetNewPath("commonserver.cfg"));

    // Battle Soccer
    gEnableBattleSoccer = GetPrivateProfileInt("GameServerInfo", "EnableBattleSoccer", 1,
                                               gDirPath.GetNewPath("commonserver.cfg"));

    // NPC Talk
    gEnableEventNPCTalk = GetPrivateProfileInt("GameServerInfo", "EnableEventNPCTalk", 0,
                                               gDirPath.GetNewPath("commonserver.cfg"));

    // Server Division
    gEnableServerDivision = GetPrivateProfileInt("GameServerInfo", "EnableServerDivision", 0,
                                                 gDirPath.GetNewPath("commonserver.cfg"));
    ...
    // Penetration Skill
    gEnableCheckPenetrationSkill = GetPrivateProfileInt("GameServerInfo", "EnableCheckPenetrationSkill", 1,
                                                        gDirPath.GetNewPath("commonserver.cfg"));

    // Shield System
    g_ShieldSystemOn = GetPrivateProfileInt("GameServerInfo", "ShieldSystemOn", 0,
                                            gDirPath.GetNewPath("commonserver.cfg"));

    g_iDamageDevideToSDRate = GetPrivateProfileInt("GameServerInfo", "DamageDevideToSD", 90,
                                                   gDirPath.GetNewPath("commonserver.cfg"));
    g_iDamageDevideToHPRate = GetPrivateProfileInt("GameServerInfo", "DamageDevideToHP", 10,
                                                   gDirPath.GetNewPath("commonserver.cfg"));
    g_fSuccessAttackRateOption = (double) (GetPrivateProfileInt("GameServerInfo", "SuccessAttackRateOption", 20000,
                                                                gDirPath.GetNewPath("commonserver.cfg")) / 10000);
    g_iSDChargingOption = GetPrivateProfileInt("GameServerInfo", "SDChargingOption", 1,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iConstNumberOfShieldPoint = GetPrivateProfileInt("GameServerInfo", "ConstNumberOfShieldPoint", 20,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    g_ShieldAutoRefillOn = GetPrivateProfileInt("GameServerInfo", "ShieldAutoRefillOn", 0,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    g_ShieldAutoRefillOnSafeZone = GetPrivateProfileInt("GameServerInfo", "ShieldAutoRefilOnSafeZone", 1,
                                                        gDirPath.GetNewPath("commonserver.cfg"));


    // Compund Potion
    g_CompoundPotionDropOn = GetPrivateProfileInt("GameServerInfo", "CompoundPotionDropOn", 0,
                                                  gDirPath.GetNewPath("commonserver.cfg"));

    g_iCompoundPotionLv1DropRate = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv1DropRate", 100,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iCompoundPotionLv2DropRate = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv2DropRate", 80,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    g_iCompoundPotionLv3DropRate = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv3DropRate", 50,
                                                        gDirPath.GetNewPath("commonserver.cfg"));

    g_iCompoundPotionLv1DropLevel = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv1DropLevel", 68,
                                                         gDirPath.GetNewPath("commonserver.cfg"));
    g_iCompoundPotionLv2DropLevel = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv2DropLevel", 96,
                                                         gDirPath.GetNewPath("commonserver.cfg"));
    g_iCompoundPotionLv3DropLevel = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv3DropLevel", 118,
                                                         gDirPath.GetNewPath("commonserver.cfg"));

    // Shile System Combo
    g_bShieldComboMissOptionOn = GetPrivateProfileInt("GameServerInfo", "ShieldComboMissOptionOn", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    // Shield Potions
    g_iShieldPotionLv1MixSuccessRate = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv1MixSuccessRate", 50,
                                                            gDirPath.GetNewPath("commonserver.cfg"));
    g_iShieldPotionLv1MixMoney = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv1MixMoney", 100000,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    g_iShieldPotionLv2MixSuccessRate = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv2MixSuccessRate", 30,
                                                            gDirPath.GetNewPath("commonserver.cfg"));
    g_iShieldPotionLv2MixMoney = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv2MixMoney", 500000,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    g_iShieldPotionLv3MixSuccessRate = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv3MixSuccessRate", 30,
                                                            gDirPath.GetNewPath("commonserver.cfg"));
    g_iShieldPotionLv3MixMoney = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv3MixMoney", 1000000,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    // Shield GAGE
    g_iShieldGageConstA = GetPrivateProfileInt("GameServerInfo", "ShieldGageConstA", 12,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iShieldGageConstB = GetPrivateProfileInt("GameServerInfo", "ShieldGageConstB", 30,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    ...
    wsprintf(szTemp, "[%s] [%d] [%s] [%d] [%c.%c%c.%c%c] [%s] [%s][%s]", GAMESERVER_VERSION,
             g_ConfigRead.server.GetGameServerCode(), g_ConfigRead.server.GetServerName(), Lang.GetDefaultLangID(),
             ::szClientVersion[0], szClientVersion[1], szClientVersion[2], szClientVersion[3], szClientVersion[4],
             szGameServerExeSerial, szName, szServer);
    ...
    gDoPShopOpen = GetPrivateProfileInt("GameServerInfo", "PersonalShopOpen", 0,
                                        gDirPath.GetNewPath("commonserver.cfg"));

    ReadEventInfo(MU_EVENT_ALL);

    // AutoRecuperation Level
    g_iUseCharacterAutoRecuperationSystem = GetPrivateProfileInt("GameServerInfo", "UseCharacterAutoRecuperationSystem",
                                                                 0, gDirPath.GetNewPath("commonserver.cfg"));
    g_iCharacterRecuperationMaxLevel = GetPrivateProfileInt("GameServerInfo", "CharacterRecuperationMaxLevel", 100,
                                                            gDirPath.GetNewPath("commonserver.cfg"));

    // Skill Check Settings
    g_iSkillDistanceCheck = GetPrivateProfileInt("GameServerInfo", "SkillDistanceCheck", 0,
                                                 gDirPath.GetNewPath("commonserver.cfg"));
    g_iSkillDistanceCheckTemp = GetPrivateProfileInt("GameServerInfo", "SkillDistanceCheckTemp", 2,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iSkillDistanceKick = GetPrivateProfileInt("GameServerInfo", "SkillDistanceKick", 0,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    g_iSkillDistanceKickCount = GetPrivateProfileInt("GameServerInfo", "SkillDistanceKickCount", 5,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    g_iSkillDiatanceKickCheckTime = GetPrivateProfileInt("GameServerInfo", "SkillDistanceKickCheckTime", 10,
                                                         gDirPath.GetNewPath("commonserver.cfg"));

    gAttackSpeedTimeLimit = GetPrivateProfileInt("GameServerInfo", "AttackSpeedTimeLimit", 800,
                                                 gDirPath.GetNewPath("commonserver.cfg"));
    bIsIgnorePacketSpeedHackDetect = GetPrivateProfileInt("GameServerInfo", "IsIgnorePacketHackDetect", 0,
                                                          gDirPath.GetNewPath("commonserver.cfg"));
    gHackCheckCount = GetPrivateProfileInt("GameServerInfo", "HackCheckCount", 5,
                                           gDirPath.GetNewPath("commonserver.cfg"));
    gMinimumAttackSpeedTime = GetPrivateProfileInt("GameServerInfo", "MinimumAttackSpeedTime", 200,
                                                   gDirPath.GetNewPath("commonserver.cfg"));
    gDetectedHackKickCount = GetPrivateProfileInt("GameServerInfo", "DetectedHackKickCount", 10,
                                                  gDirPath.GetNewPath("commonserver.cfg"));
    gIsKickDetecHackCountLimit = GetPrivateProfileInt("GameServerInfo", "IsKickDetecHackCountLimit", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));
    gSpeedHackPenalty = GetPrivateProfileInt("GameServerInfo", "SpeedHackPenalty", 0,
                                             gDirPath.GetNewPath("commonserver.cfg"));
...
void ReadEventInfo(MU_EVENT_TYPE eEventType) {
    ...
    gIsDropDarkLordItem = GetPrivateProfileInt("GameServerInfo", "IsDropDarkLordItem", 0,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    gSleeveOfLordDropRate = GetPrivateProfileInt("GameServerInfo", "SleeveOfLordDropRate", 10,
                                                 gDirPath.GetNewPath("commonserver.cfg"));
    gSleeveOfLordDropLevel = GetPrivateProfileInt("GameServerInfo", "SleeveOfLordDropLevel", 86,
                                                  gDirPath.GetNewPath("commonserver.cfg"));
    gSoulOfDarkHorseDropRate = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkHorseDropRate", 10,
                                                    gDirPath.GetNewPath("commonserver.cfg"));
    gSoulOfDarkHorseropLevel = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkHorseDropLevel", 125,
                                                    gDirPath.GetNewPath("commonserver.cfg"));
    gSoulOfDarkSpiritDropRate = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkSpiritDropRate", 10,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    gSoulOfDarkSpiritDropLevel = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkSpiritDropLevel", 96,
                                                      gDirPath.GetNewPath("commonserver.cfg"));
    GetPrivateProfileString("GameServerInfo", "DarkSpiritAddExperience", "1", szTemp, 10,
                            gDirPath.GetNewPath("commonserver.cfg"));
    gDarkSpiritAddExperience = (float) atof(szTemp);
    gIsDropGemOfDefend = GetPrivateProfileInt("GameServerInfo", "IsDropGemOfDefend", 0,
                                              gDirPath.GetNewPath("commonserver.cfg"));
    gGemOfDefendDropRate = GetPrivateProfileInt("GameServerInfo", "GemOfDefendDropRate", 5,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    gGemOfDefendDropLevel = GetPrivateProfileInt("GameServerInfo", "GemOfDefendDropLevel", 75,
                                                 gDirPath.GetNewPath("commonserver.cfg"));

    if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
        gIsDropSetItemInCastleHuntZone = GetPrivateProfileInt("GameServerInfo", "IsDropSetItemInCastleHuntZone", 0,
                                                              gDirPath.GetNewPath("commonserver.cfg"));
        gSetItemInCastleHuntZoneDropRate = GetPrivateProfileInt("GameServerInfo", "SetItemInCastleHuntZoneDropRate", 1,
                                                                gDirPath.GetNewPath("commonserver.cfg"));
        gSetItemInCastleHuntZoneDropLevel = GetPrivateProfileInt("GameServerInfo", "SetItemInCastleHuntZoneDropLevel",
                                                                 75, gDirPath.GetNewPath("commonserver.cfg"));
    }

    gWeaponAttackDamage = GetPrivateProfileInt("GameServerInfo", "CastleSiegeWeaponAttackDamage", 300,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    gWeaponDefenseDamage = GetPrivateProfileInt("GameServerInfo", "CastleSiegeWeaponDefenseDamage", 200,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    ...
    gEvent1ItemDropTodayMax = GetPrivateProfileInt("GameServerInfo", "Event1ItemDropTodayMax", 1,
                                                   gDirPath.GetNewPath("commonserver.cfg"));
    gEvent1ItemDropTodayCount = GetPrivateProfileInt("GameServerInfo", "Event1ItemDropTodayCount", 0,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    gEvent1ItemDropTodayPercent = GetPrivateProfileInt("GameServerInfo", "Event1ItemDropTodayPercent", 80,
                                                       gDirPath.GetNewPath("commonserver.cfg"));
    gFireCrackerEvent = GetPrivateProfileInt("GameServerInfo", "FireCrackerEvent", 0,
                                             gDirPath.GetNewPath("commonserver.cfg"));
    gFireCrackerDropRate = GetPrivateProfileInt("GameServerInfo", "FireCrackerDropRate", 5000,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    g_ItemDropRateForgFireCracker = GetPrivateProfileInt("GameServerInfo", "ItemDropRateForFireCracker", 2,
                                                         gDirPath.GetNewPath("commonserver.cfg"));
    gOnlyFireCrackerEffectUse = GetPrivateProfileInt("GameServerInfo", "OnlyFireCrackerEffectUse", 0,
                                                     gDirPath.GetNewPath("commonserver.cfg"));
    gMedalEvent = GetPrivateProfileInt("GameServerInfo", "MedalEvent", 0, gDirPath.GetNewPath("commonserver.cfg"));
    gGoldMedalDropRate = GetPrivateProfileInt("GameServerInfo", "GoldMedalDropRate", 2,
                                              gDirPath.GetNewPath("commonserver.cfg"));
    gSilverMedalDropRate = GetPrivateProfileInt("GameServerInfo", "SilverMedalDropRate", 2,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    gXMasEvent = GetPrivateProfileInt("GameServerInfo", "XMasEvent", 0, gDirPath.GetNewPath("commonserver.cfg"));
    g_XMasEvent_StarOfXMasDropRate = GetPrivateProfileInt("GameServerInfo", "XMasEvent_StarOfXMasDropRate", 5000,
                                                          gDirPath.GetNewPath("commonserver.cfg"));
    gHeartOfLoveEvent = GetPrivateProfileInt("GameServerInfo", "HeartOfLoveEvent", 0,
                                             gDirPath.GetNewPath("commonserver.cfg"));
    gHeartOfLoveDropRate = GetPrivateProfileInt("GameServerInfo", "HeartOfLoveDropRate", 5000,
                                                gDirPath.GetNewPath("commonserver.cfg"));
    g_ItemDropRateForgHeartOfLove = GetPrivateProfileInt("GameServerInfo", "ItemDropRateForHeartOfLove", 2,
                                                         gDirPath.GetNewPath("commonserver.cfg"));
    gHappyNewYearNpcEvent = GetPrivateProfileInt("GameServerInfo", "HappyNewYearTalkNpc", 0,
                                                 gDirPath.GetNewPath("commonserver.cfg"));
    ...
    gMerryXMasNpcEvent = GetPrivateProfileInt("GameServerInfo", "MerryXMasTalkNpc", 0,
                                              gDirPath.GetNewPath("commonserver.cfg"));
    ...
    g_iKundunMarkDropRate = GetPrivateProfileInt("GameServerInfo", "KundunMarkDropRate", 0,
                                                 gDirPath.GetNewPath("commonserver.cfg"));

    g_iMysteriousBeadDropRate1 = GetPrivateProfileInt("GameServerInfo", "MysteriouseBeadDropRate1", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));
    g_iMysteriousBeadDropRate2 = GetPrivateProfileInt("GameServerInfo", "MysteriouseBeadDropRate2", 0,
                                                      gDirPath.GetNewPath("commonserver.cfg"));

    g_iHiddenTreasureBoxOfflineRate = GetPrivateProfileInt("GameServerInfo", "HiddenTreasureBoxOfflineRate", 0,
                                                           gDirPath.GetNewPath("commonserver.cfg"));
    ...
    g_iDarkLordHeartDropRate = GetPrivateProfileInt("GameServerInfo", "DarkLordHeartDropRate", 0,
                                                    gDirPath.GetNewPath("commonserver.cfg"));
    g_iDarkLordHeartOffEventRate = GetPrivateProfileInt("GameServerInfo", "DarkLoadHeartOffEventRate", 0,
                                                        gDirPath.GetNewPath("commonserver.cfg"));
    ...
    //Cherry Blossom
    g_bBlossomBoxDropOn = GetPrivateProfileInt("GameServerInfo", "CherryBlossomEventOn", 0,
                                               gDirPath.GetNewPath("commonserver.cfg"));
    g_iBlossomBoxDropRate = GetPrivateProfileInt("GameServerInfo", "CherryBlossomEventItemDropRate", 50,
                                                 gDirPath.GetNewPath("commonserver.cfg"));
    ...
    g_MonsterStatCalc.LoadScript(
            g_ConfigRead.GetPath("\\Events\\ImperialGuardian\\IGC_ImperialGuardian_Monster_Info.xml"));

    g_ChaosCastleFinal.Load_CCFInfo(
            g_ConfigRead.GetPath("\\Events\\ChaosCastle_Survival\\IGC_ChaosCastle_Survival.xml"));
    g_ChaosCastleFinal.Load_CCFMonster(
            g_ConfigRead.GetPath("\\Events\\ChaosCastle_Survival\\IGC_ChaosCastle_Survival_Monster.xml"));

    g_DevilSquareFinal.Load_DSFScheduleScript(
            g_ConfigRead.GetPath("\\Events\\Tormented Square\\IGC_TormentedSquare.xml"));
    g_DevilSquareFinal.Load_DSFMonsterScript(
            g_ConfigRead.GetPath("\\Events\\Tormented Square\\IGC_TormentedSquare_Monsters.xml"));
    ...
    if (g_ArcaBattle.IsArcaBattleDay() == TRUE && g_AcheronGuardianEvent.IsAcheronGuardianDay() == TRUE) {
        g_Log.MsgBox(MB_OK | MB_ICONERROR, "Error",
                     "Error - ArcaBattle and AcheronGuardian cannot be active at the same day.");
        ExitProcess(1);
    }
}
...
void ReadGameEtcInfo(MU_ETC_TYPE eGameEtcType) {
    switch (eGameEtcType) {
        case 0:
            ...
            bCanChaosBox = GetPrivateProfileInt("Common", "IsChaosMachine", 0,
                                                g_ConfigRead.GetPath("IGC_ChaosBox.ini"));

            gItemDropPer = GetPrivateProfileInt("GameServerInfo", "ItemDropPer", 10,
                                                gDirPath.GetNewPath("commonserver.cfg"));
...
void ReloadEvent(MU_EVENT_RELOAD eEvent) {
    switch (eEvent) {
        ...
        case 1:
            g_DevilSquare.Load(g_ConfigRead.GetPath("\\Events\\DevilSquare\\IGC_DevilSquare_Classic.xml"));
            g_DevilSquare.LoadMonster(
                    g_ConfigRead.GetPath("\\Events\\DevilSquare\\IGC_DevilSquare_Classic_Monsters.xml"));
            g_DevilSquare.Init();
            break;
        case 2:
            g_ChaosCastle.Load(g_ConfigRead.GetPath("\\Events\\ChaosCastle\\IGC_ChaosCastle.xml"));
        ...
        case 8:
            if (g_ConfigRead.server.GetServerType() == SERVER_NORMAL) {
                g_Raklion.LoadData(g_ConfigRead.GetPath("\\Events\\Raklion\\IGC_RaklionHatchery.xml"));
                g_RaklionMonsterMng.LoadData(
                        g_ConfigRead.GetPath("\\Events\\Raklion\\IGC_RaklionHatchery_Monsters.xml"));
                g_Raklion.Init();
            } else {
                g_Log.AddC(TColor::Blue, "Function not supported");
            }
        ...
        case 13:
            ...
            if (g_ArcaBattle.IsArcaBattleDay() == TRUE && g_AcheronGuardianEvent.IsAcheronGuardianDay() == TRUE) {
                g_Log.MsgBox(
                        "Error - ArcaBattle and AcheronGuardian cannot be active at the same day - ArcaBattle disabled.");
        ...
        case 14:
            ...
            if (g_ArcaBattle.IsArcaBattleDay() == TRUE && g_AcheronGuardianEvent.IsAcheronGuardianDay() == TRUE) {
                g_Log.MsgBox(
                        "Error - ArcaBattle and AcheronGuardian cannot be active at the same day - Acheron Guardian disabled.");

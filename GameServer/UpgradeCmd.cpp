    ...
    for (pugi::xml_node need_item = main.child("NeedItem"); need_item; need_item = need_item.next_sibling()) {
        for (pugi::xml_node item = need_item.child("Item"); item; item = item.next_sibling()) {
            ...
            if (m_Upgrade.m_NeedItem.wItemID == (WORD) - 1) {
                g_Log.MsgBox("Upgrade Item Cmd - Wrong Need Item found (System disabled) (%d/%d)", btItemCat,
                             wItemIndex);
                this->m_bSystemEnable = false;
                return;
            }
    ...
    for (pugi::xml_node get_item = main.child("GetItem"); get_item; get_item = get_item.next_sibling()) {
        for (pugi::xml_node item = get_item.child("Item"); item; item = item.next_sibling()) {
            ...
            if (Iter == this->m_mapUpgradeData.end()) {
                g_Log.MsgBox(
                        "Upgrade Item Cmd - Wrong Get Item found (not matched with Need Item) (System disabled) (%d)",
                        iUpgradeID);
                this->m_bSystemEnable = false;
                return;
            }
            ...
            if (Iter->second.m_GetItem.wItemID == (WORD) - 1) {
                g_Log.MsgBox("Upgrade Item Cmd - Wrong Get Item found (System disabled) (%d/%d)", btItemCat,
                             wItemIndex);
                this->m_bSystemEnable = false;
                return;
            }
            ...
            if (p == NULL) {
                g_Log.MsgBox(
                        "Upgrade Item Cmd - Wrong Get Item found (Not exists in ItemList.xml) (System disabled) (%d/%d)",
                        btItemCat, wItemIndex);
                this->m_bSystemEnable = false;
                return;
            }
...
int CUpgradeCmd::DoUpgrade(int aIndex) {
    ...
    for (int i = INVETORY_WEAR_SIZE; i < INVENTORY_SIZE - 1; i++) // Pentagram excluded, so -1
    {
        ...
        for (std::map<int, UPGRADECMD_DATA>::iterator It = this->m_mapUpgradeData.begin();
             It != this->m_mapUpgradeData.end(); It++) {
            ...
            NewItem.m_Durability = ItemGetDurability(NewItem.m_Type, NewItem.m_Level, NewItem.m_NewOption,
                                                     NewItem.m_SetOption);

            ItemSerialCreateSend(aIndex, 235, 0, 0, NewItem.m_Type, NewItem.m_Level, NewItem.m_Durability,
                                 NewItem.m_Option1, NewItem.m_Option2, NewItem.m_Option3, aIndex, NewItem.m_NewOption,
                                 NewItem.m_SetOption, 0, NewItem.m_SocketOption, 0);
            return 1;
        }
    }

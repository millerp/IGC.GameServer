...
BOOL CRaklion::LoadData(char *lpszFileName) {
    ...
    try {
        ...
        if (g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_RAKLION) ==
            false) {
        ...
        for (int iCount = 0; iCount < 6; iCount++) {
            if (iBossPatternCondition[iCount] < 0) {
                g_Log.MsgBox("[ RAKLION ] - BossPatternCondition[%d] error : (%d)", iCount,
                             iBossPatternCondition[iCount]);
                return FALSE;
            }
...
void CRaklion::Run() {
    ...
    if (m_iTodayDay != pCurrentTime->tm_mday || m_iTodayMon != pCurrentTime->tm_mon ||
        m_iTodayYear != pCurrentTime->tm_year) {
        m_iTodayDay = pCurrentTime->tm_mday;
        m_iTodayMon = pCurrentTime->tm_mon;
        m_iTodayYear = pCurrentTime->tm_year;
    ...
    if (!m_bEvent && pCurrentTime->tm_hour == 0 && pCurrentTime->tm_min == 0 && pCurrentTime->tm_sec == 0) {
        for (int iCount = g_ConfigRead.server.GetObjectStartUserIndex();
             iCount < g_ConfigRead.server.GetObjectMax(); iCount++) {
            if (gObj[iCount].MapNumber == MAP_INDEX_RAKLION ||
                ...
                if (gObj[iCount].Connected == PLAYER_PLAYING &&
                    ...
                    iRaklionUserCount++;
                    g_Log.Add("[RAKLION][IsInRaklionEvent] %d/%d/%d %dth User : %s (%s)", pCurrentTime->tm_year + 1900,
                              pCurrentTime->tm_mon + 1, pCurrentTime->tm_mday, iRaklionUserCount, gObj[iCount].Name,
                              gObj[iCount].AccountID);
                }
            }
        }
        m_bEvent = TRUE;
    ...
    for (int iCount = g_ConfigRead.server.GetObjectStartUserIndex();
        ...
        if (gObj[iCount].Connected == PLAYER_PLAYING &&
            ...
            if (gObj[iCount].RegenOk == 0 && gObj[iCount].m_State == 2 && gObj[iCount].Live == 1) {
                ...
                g_Log.AddC(TColor::Red, "[ RAKLION ][ Invalid User ] Invalid Raklion Boss Map User[%s][%s]",
...
int CRaklion::CheckEnterRaklion(int iUserIndex) {
    ...
    if (GetRaklionState() == RAKLION_STATE_CLOSE_DOOR) {
        g_Log.Add("[ RAKLION ][ Entrance Fail ] [%s][%s] State(%d)", gObj[iUserIndex].AccountID, gObj[iUserIndex].Name,
                  GetRaklionState());
        return 3;
    }
    if (gObj[iUserIndex].MapNumber != MAP_INDEX_HATCHERY) {
        g_Log.Add("[ RAKLION ][ Entrance Fail ] Invalid Map Number(%d) [%s][%s] State(%d)", gObj[iUserIndex].MapNumber,
                  gObj[iUserIndex].AccountID, gObj[iUserIndex].Name, GetRaklionState());
        return 4;
    }
    return 0;
}

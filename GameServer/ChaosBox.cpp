...
BOOL CMixSystem::ChaosBoxItemDown(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].m_Type == ITEMGET(12, 15) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 13) ||
            ...
            lpObj->pChaosBox[n].m_Type == ITEMGET(13, 86) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 87) ||
            ...
            lpObj->pChaosBox[n].m_Type == ITEMGET(13, 92) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 53) ||
            lpObj->pChaosBox[n].m_Type == ITEMGET(14, 103) ||
            lpObj->pChaosBox[n].m_Type == ITEMGET(14, 103) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 104) ||
            lpObj->pChaosBox[n].m_Type == ITEMGET(14, 105) ||
            lpObj->pChaosBox[n].m_Type == ITEMGET(14, 106) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 107) ||
            lpObj->pChaosBox[n].m_Type == ITEMGET(14, 108)) {
            lpObj->pChaosBox[n].Clear();
        } else {
            ...
            float dur = ItemGetDurability(lpObj->pChaosBox[n].m_Type, lpObj->pChaosBox[n].m_Level,
                                          lpObj->pChaosBox[n].IsExtItem(), lpObj->pChaosBox[n].IsSetItem());

            lpObj->pChaosBox[n].m_Durability =
                    dur * lpObj->pChaosBox[n].m_Durability / lpObj->pChaosBox[n].m_BaseDurability;
            lpObj->pChaosBox[n].Convert(lpObj->pChaosBox[n].m_Type, lpObj->pChaosBox[n].m_Option1,
                                        ...
                                        lpObj->pChaosBox[n].m_SetOption, lpObj->pChaosBox[n].m_ItemOptionEx, 0, -1, 0,
                                        CURRENT_DB_VERSION);
        }
    }
...
int CMixSystem::ChaosBoxMix(LPOBJ lpObj, int &Result2, int &WingNum) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            ...
            if (lpObj->pChaosBox[n].m_Level >= MIN_CHAOS_ITEM_LEVEL &&
                (lpObj->pChaosBox[n].m_Option3 * 4) >= MIN_CHAOS_ITEM_LEVEL) {
                nv = lpObj->pChaosBox[n].m_OldBuyMoney;
                value += lpObj->pChaosBox[n].m_OldBuyMoney;
                add = 1;

                if (lpObj->pChaosBox[n].m_Type == ITEMGET(2, 6) || lpObj->pChaosBox[n].m_Type == ITEMGET(4, 6) ||
                    lpObj->pChaosBox[n].m_Type == ITEMGET(5, 7))    // Chaos Items
                {
                    Result2 = 1;
                }
            }
            ...
            if (lpObj->pChaosBox[n].m_Type == ITEMGET(12, 15)) {
                ...
                add = 1;
                       ...
                       lpObj->pChaosBox[n].m_Type == ITEMGET(14, 14))    // Jewel of Bless, Jewel of Soul
            {
                value += lpObj->pChaosBox[n].m_OldBuyMoney;
                nv = lpObj->pChaosBox[n].m_OldBuyMoney;
                add = 1;
            ...
            {
                g_Log.Add("[%s][%s] Fail to mix because of Item (13,14). It can't be in default ChaosMix",
    ...
    if (WingCharmItemNum > -1) //season4.5 add-on
    {
        ...
        switch (WingCharmIndex) {
            ...
            case 87:
                ...
                break;
            default:
                g_Log.Add("[MixSystem] WingCharmItemNum => %d , WingCharmIndex => %d", WingCharmItemNum,
                          WingCharmIndex);
                break;
        }
    }
...
void CMixSystem::DefaultChaosMix(LPOBJ lpObj) {
    ...
    int iChaosTaxMoney = (int) ((__int64) lpObj->ChaosMoney *
                                (__int64) g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / (__int64) 100);
        ...
        if (lpObj->ChaosSuccessRate > 0) {
            ...
            if ((rand() % 100) <= (lpObj->ChaosSuccessRate - 1)) {
                ...
                if (MixResult2 == TRUE) {
                    ...
                    ::ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, WingNum, 0, 255, Option1, Option2, Option3, -1, 0,
                                           0, 0, 0, 0);
                } else {
                    ...
                    ::ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, ChaosItemNum, Level, 255, Option1, Option2,
                                           Option3, -1, 0, 0, 0, 0, 0);
                }
            }
        }
...
void CMixSystem::LogDQChaosItem(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            ...
            g_Log.Add(
                    "[DevilSquare,%d] [%s][%s] CBMix [%d,%s,%d,%d,%d,%d]serial:[%I64d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380Ex[%d] HO:[%d,%d]",
...
BOOL CMixSystem::DevilSquareEventChaosMix(LPOBJ lpObj, BOOL bCheckType, int iItemLevel) {
    ...
    int iChaosTaxMoney = (int) ((__int64) nChaosNeedMoney *
                                (__int64) g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / (__int64) 100);
...
void CMixSystem::LogPlusItemLevelChaosItem(LPOBJ lpObj, int iPlusMixLevel) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            ...
            g_Log.Add(
                    "[PlusItemLevel,%d] [%s][%s] CBMix [%d,%s,%d,%d,%d,%d]serial:[%I64d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380Ex:[%d] HO:[%d,%d]",
                    iMixLevel,
                    lpObj->AccountID, lpObj->Name, n, lpObj->pChaosBox[n].GetName(), lpObj->pChaosBox[n].m_Level,
                    lpObj->pChaosBox[n].m_Option1, lpObj->pChaosBox[n].m_Option2, lpObj->pChaosBox[n].m_Option3,
                    lpObj->pChaosBox[n].m_Number, (int) lpObj->pChaosBox[n].m_Durability,
                    ...
                    g_kJewelOfHarmonySystem.GetItemStrengthenOption(
                            &lpObj->pChaosBox[n]),    // #error Cahnge to GetItemStrengthenOption
                    g_kJewelOfHarmonySystem.GetItemOptionLevel(&lpObj->pChaosBox[n]));
        }
    }
}
...
void CMixSystem::LogChaosItem(LPOBJ lpObj, LPSTR sLogType) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            ...
            g_Log.Add(
                    "[%s] [%s][%s] CBMix [%d,%s,%d,%d,%d,%d]serial:[%I64d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d]",
                    sLogType, lpObj->AccountID, lpObj->Name, n, lpObj->pChaosBox[n].GetName(),
                    lpObj->pChaosBox[n].m_Level,
                    lpObj->pChaosBox[n].m_Option1, lpObj->pChaosBox[n].m_Option2, lpObj->pChaosBox[n].m_Option3,
                    lpObj->pChaosBox[n].m_Number, (int) lpObj->pChaosBox[n].m_Durability,
                    ExOption[0], ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5], ExOption[6],
...
BOOL CMixSystem::PlusItemLevelChaosMix(LPOBJ lpObj, int mixType) {
    ...
    if (Plus9ItemCount == 1 && ChaosGemCount == 1 && BlessGemCount == 1 && SoulGemCount == 1 && Plus10ItemCount == 0 &&
        Plus11ItemCount == 0 && Plus12ItemCount == 0) {
        MixType = 3;
        lpObj->ChaosMoney = 2000000;
        ExtraBlessGemCount = BlessGemCount - 1;
        ExtraSoulGemCount = SoulGemCount - 1;
    } else if (Plus10ItemCount == 1 && ChaosGemCount == 1 && BlessGemCount == 2 && SoulGemCount == 2 &&
    ...
    } else if (Plus11ItemCount == 1 && ChaosGemCount == 1 && BlessGemCount == 3 && SoulGemCount == 3 &&
    ...
    } else if (Plus12ItemCount == 1 && ChaosGemCount == 1 && BlessGemCount == 4 && SoulGemCount == 4 &&
    ...
    } else if (Plus13ItemCount == 1 && ChaosGemCount == 1 && BlessGemCount == 5 && SoulGemCount == 5 &&
               Plus9ItemCount == 0 && Plus10ItemCount == 0 && Plus11ItemCount == 0) {
        MixType = 49;
        lpObj->ChaosMoney = 10000000;
        ExtraBlessGemCount = BlessGemCount - 5;
        ExtraSoulGemCount = SoulGemCount - 5;
    } else if (Plus14ItemCount == 1 && ChaosGemCount == 1 && BlessGemCount == 6 && SoulGemCount == 6 &&
               Plus9ItemCount == 0 && Plus10ItemCount == 0 && Plus11ItemCount == 0) {
        MixType = 50;
    ...
    if ((lpObj->pChaosBox[PlusItemPos].m_Type >= ITEMGET(12, 0) &&
         lpObj->pChaosBox[PlusItemPos].m_Type <= ITEMGET(12, 6)) ||
        (lpObj->pChaosBox[PlusItemPos].m_Type >= ITEMGET(12, 36) &&
         lpObj->pChaosBox[PlusItemPos].m_Type <= ITEMGET(12, 43)) ||
    ...
    if (Result < lpObj->ChaosSuccessRate) {
        ...
        Item.m_Durability =
                Dur * lpObj->pChaosBox[PlusItemPos].m_Durability / lpObj->pChaosBox[PlusItemPos].m_BaseDurability;
        ...
        Item.Convert(lpObj->pChaosBox[PlusItemPos].m_Type, lpObj->pChaosBox[PlusItemPos].m_Option1,
                     lpObj->pChaosBox[PlusItemPos].m_Option2,
                     lpObj->pChaosBox[PlusItemPos].m_Option3, lpObj->pChaosBox[PlusItemPos].m_NewOption,
                     lpObj->pChaosBox[PlusItemPos].m_SetOption, lpObj->pChaosBox[PlusItemPos].m_ItemOptionEx,
                     SocketOption, SocketIndex, 0, CURRENT_DB_VERSION);
        ...
        if (g_ConfigRead.mix.PlusItemMix15ResultNotice[nItemGrade] != 0 && MixType == 50) {
            GSProtocol.GCServerMsgStringSendAllEx(0, Lang.GetText(0, 611), lpObj->Name,
        ...
        g_Log.Add(
                "[PlusItemLevel] [%s][%s] CBMix Fail, Charm of Chaos Assembly Used, %d / %d Money : %d-%d [%d], CharmRate : %d",
                lpObj->AccountID, lpObj->Name, Result, lpObj->ChaosSuccessRate, lpObj->m_PlayerData->Money,
                lpObj->ChaosMoney, FailLevel, iCharmOfLuckCount);
    }
...
BOOL CMixSystem::WingChaosMix(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            ...
            if ((lpObj->pChaosBox[n].m_Type >= ITEMGET(12, 0) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12, 2)) ||
                lpObj->pChaosBox[n].m_Type == ITEMGET(12, 41)) {
                WingCount++;
                WingIndex = n;
    ...
    if ((rand() % 100) < lpObj->ChaosSuccessRate) {
        ...
        if (WingCharmItemNum > -1) //season4.5 add-on
        {
            ...
            switch (WingCharmIndex) {
                ...
                case 92:
                    ...
                    break;
                default:
                    g_Log.Add("[MixSystem][WingChaosMix] WingCharmItemNum => %d , WingCharmIndex => %d",
                              WingCharmItemNum, WingCharmIndex);
                    break;
            }
        }
        ...
        ::ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, iWingNum, iWingLevel, 0, 0, iOption1, iOption2, -1, iOption3,
                               0, 0, 0, 0);
        ::gObjInventoryCommit(lpObj->m_Index);
...
BOOL CMixSystem::IllusionTempleItemChaosMix(LPOBJ lpObj) {
    ...
    if (lpObj->ChaosSuccessRate == 0 && ChoasGemCount != 1 && OldScrollCount != 1 && CovenantCount != 1 &&
        OldScrollLevel != CovenantLevel) {
        pMsg.Result = CB_INCORRECT_MIX_ITEMS;
        IOCP.DataSend(lpObj->m_Index, (BYTE * ) & pMsg, pMsg.h.size);
        lpObj->ChaosLock = FALSE;
    ...
    if ((rand() % 100) < lpObj->ChaosSuccessRate) {
        ...
        ::g_Log.Add("[IllusionTemple Ticket,%d] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d",
                    OldScrollLevel, lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->m_PlayerData->Money,
                    nChaosNeedMoney, iCharmOfLuckCount);
        return TRUE;
    } else {
        for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
            lpObj->pChaosBox[n].Clear();
...
BOOL CMixSystem::CheckLevelCondition(CItem *lpItem, short Level, BYTE Op1, BYTE Op2, BYTE Op3, BYTE SetOption,
                                     BYTE NewOption) {
...
BOOL CMixSystem::AdvancedWingMix(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            if ((lpObj->pChaosBox[n].m_Type >= ITEMGET(12, 0) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12, 6)) ||
                lpObj->pChaosBox[n].m_Type == ITEMGET(13, 30)) {
                lpObj->ChaosLock = FALSE;
    ...
    if (BundleOfHarmonyCount != 0 && BundleOfCreationCount != 0) {
        if (ChoasGemCount != 1 || BundleOfSoulCount != 1 || BundleOfBlessCount != 1 || BundleOfCreationCount != 1 ||
            BundleOfHarmonyCount < 1) {
            pMsg.Result = CB_INCORRECT_MIX_ITEMS;
            IOCP.DataSend(lpObj->m_Index, (BYTE * ) & pMsg, pMsg.h.size);
            lpObj->ChaosLock = FALSE;
...
BOOL CMixSystem::ThirdWingLevel2ChaosMix(LPOBJ lpObj) {
    ...
    if (CondorFeatherCount != 0 && CondorFlameCount != 0 && JewelOfCreationCount != 0) {
        if (ChoasGemCount != 1 || BundleOfSoulCount != 1 || BundleOfBlessCount != 1 || JewelOfCreationCount != 1 ||
            ExcItemCount < 1) {
            pMsg.Result = CB_INCORRECT_MIX_ITEMS;
            IOCP.DataSend(lpObj->m_Index, (BYTE * ) & pMsg, pMsg.h.size);
            lpObj->ChaosLock = FALSE;
    ...
    if ((rand() % 100) < lpObj->ChaosSuccessRate) {
        ...
        ::ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, iWingNum, 0, 0, 0, iOption1, iOption2, -1, iExcOpt, 0, 0, 0,
                               0);
        ::gObjInventoryCommit(lpObj->m_Index);
...
void CMixSystem::ThirdWingMixFailItemPanalty(CItem *lpItem) {
    ...
    lpItem->m_Option3 = 0;
    lpItem->Convert(lpItem->m_Type, lpItem->m_Option1, lpItem->m_Option2, lpItem->m_Option3, lpItem->m_NewOption,
                    lpItem->m_SetOption, lpItem->m_ItemOptionEx, lpItem->m_SocketOption, lpItem->m_BonusSocketOption, 0,
                    CURRENT_DB_VERSION);
}
...
void CMixSystem::SetItemChaosMix(LPOBJ lpObj) {
        ...
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            ...
            if (lpObj->pChaosBox[n].m_Type == ITEMGET(14, 16)) {
                ...
                if (lpObj->pChaosBox[n].IsSetItem() != FALSE) {
                    ...
                    ((lpObj->pChaosBox[n].m_Type >= ITEMGET(13, 8) && lpObj->pChaosBox[n].m_Type < ITEMGET(13, 14)) ||
                     (lpObj->pChaosBox[n].m_Type >= ITEMGET(13, 20) && lpObj->pChaosBox[n].m_Type < ITEMGET(13, 28))) &&
                    (lpObj->pChaosBox[n].m_Option3 >= 2 && lpObj->pChaosBox[n].m_Level >= 3)) {
                if (lpObj->pChaosBox[n].IsSetItem() != FALSE) {
                    MixItemCount++;
                    MixSetItemIndex = n;
                    iMixItemChaosMoney += lpObj->pChaosBox[n].m_BuyMoney;
    ...
    if ((rand() % 100) < lpObj->ChaosSuccessRate) {
        ...
        float Dur = ItemGetDurability(lpObj->pChaosBox[MixSetItemIndex].m_Type,
                                      lpObj->pChaosBox[MixSetItemIndex].m_Level,
                                      lpObj->pChaosBox[MixSetItemIndex].IsExtItem(),
                                      gSetItemOption.IsSetItem(lpObj->pChaosBox[MixSetItemIndex].m_Type));

        Item.m_Level = lpObj->pChaosBox[MixSetItemIndex].m_Level;
        Item.m_Durability = Dur * lpObj->pChaosBox[MixSetItemIndex].m_Durability /
                            lpObj->pChaosBox[MixSetItemIndex].m_BaseDurability;
        Item.m_JewelOfHarmonyOption = lpObj->pChaosBox[MixSetItemIndex].m_JewelOfHarmonyOption;

        Item.Convert(lpObj->pChaosBox[MixSetItemIndex].m_Type, lpObj->pChaosBox[MixSetItemIndex].m_Option1,
                     ...
                     lpObj->pChaosBox[MixSetItemIndex].m_SetOption, lpObj->pChaosBox[MixSetItemIndex].m_ItemOptionEx,
                     lpObj->pChaosBox[MixSetItemIndex].m_SocketOption,
                     lpObj->pChaosBox[MixSetItemIndex].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
        ...
        g_Log.Add("[SetItemMix] [%s][%s] CBMix Success %d Money : %d-%d (SetName:%s)", lpObj->AccountID, lpObj->Name,
...
void CMixSystem::DarkHorseChaosMix(LPOBJ lpObj) {
    ...
    if (ChoasGemCount != 1 || BlessGemCount != 5 || SoulGemCount != 5 || CreatureGemCount != 1 ||
        SoulOfDarkHorse != 1 || iInvalidItemCount > 0) {
...
void CMixSystem::CastleSpecialItemMix(LPOBJ lpObj) {
    ...
    if (iBlessGemMixCount != 3 || iSoulGemMixCount != 3 || iDefendGemCount != 30 || iInvalidItemCount > 0) {
        g_Log.Add("[CastleSpecialMix] [%s][%s] Item Mix Failed - Item Error (DG:%d, BGx10:%d, SGx10:%d, Other:%d)",
                  lpObj->AccountID, lpObj->Name, iBlessGemMixCount, iSoulGemMixCount, iDefendGemCount,
                  iInvalidItemCount);
    ...
    if ((rand() % 100) < lpObj->ChaosSuccessRate) {
        ...
        if (g_BagManager.GetItemFromBag(lpObj->m_Index, BAG_EVENT, EVENTBAG_LORDMIX, lpObj->m_Index, Item, Duration) ==
            false) {
            ChaosBoxInit(lpObj);
            GSProtocol.GCUserChaosBoxSend(lpObj, 1);
        ...
        ItemSerialCreateSend(lpObj->m_Index, 255, 0, 0, Item.m_Type, Item.m_Level, Item.m_Durability, Item.m_Option1,
                             Item.m_Option2, Item.m_Option3, lpObj->m_Index, Item.m_NewOption, Item.m_SetOption,
                             Duration, Item.m_SocketOption, Item.m_BonusSocketOption);
        gObjInventoryCommit(lpObj->m_Index);
...
void CMixSystem::Fenrir_04Upgrade_Mix(LPOBJ lpObj) {
    ...
    if (iStuffCount_01 != 1 || iLifeGemCount != 5 || iChaosGemCount != 1 ||
        (iAttackStuffCount == 0 && iDefendStuffCount == 0) || iInvalidItemCount > 0) {
        IOCP.DataSend(lpObj->m_Index, (UCHAR * ) & pMsg, pMsg.h.size);
        lpObj->ChaosLock = FALSE;
...
void CMixSystem::LotteryItemMix(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            if (lpObj->pChaosBox[n].m_Type == ITEMGET(14, 54) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 92) ||
                lpObj->pChaosBox[n].m_Type == ITEMGET(14, 93) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 95)) {
                iLotteryItemCount++;
    ...
    if (iInvalidItemCount > 0 || iLotteryItemCount != 1) {
        ...
        if (iLotteryItemCount != 1) {
            g_Log.AddC(TColor::Red,
                       "[CashShop][ChaosCardMix] User Try do Mix without having Chaos Card - User(ID:%s,Name:%s)",
                       lpObj->AccountID, lpObj->Name);
        }
    ...
    g_Log.Add(
            "[CashShop][ChaosCardMix] - User(ID:%s,Name:%s) Item(Name:%s,Type:%d,Level:%d,Dur:%d,Skill:%d,Luck:%d,AddOption:%d,Ex:(%d,%d,%d,%d,%d,%d))",
...
void CMixSystem::SeedExtractMix(LPOBJ lpObj) {
    ...
    if (iExcItemCount != 1 || iSetItemCount != 1 || iHarmonyCount != 1 || iChoasGemCount != 1 ||
        iCreationGemCount != 1) {
        pMsg.Result = CB_INCORRECT_MIX_ITEMS;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        lpObj->ChaosLock = FALSE;
    ...
    iChaosTaxMoney = (int) ((__int64) g_SocketOptionSystem.m_RequireMoneyForSeedExtract *
                            (__int64) g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / (__int64) 100);
    ...
    if (lpSeedData != NULL) {
        pMsg.Result = 1;
        ItemSerialCreateSend(lpObj->m_Index, 0xFF, 0, 0, lpSeedData->m_ItemCode, lpSeedData->m_ItemLevel, 1, 0, 0, 0,
                             lpObj->m_Index, 0, 0, 0, 0, 0);
        gObjInventoryCommit(lpObj->m_Index);
        g_Log.Add("[%s][%s] Seed Extract Mix Success - SeedInfo ( Index : %d, Type : %d, Level : %d, ItemCode : %d )",
                  lpObj->AccountID, lpObj->Name, lpSeedData->m_SeedIndex, lpSeedData->m_SeedType,
                  lpSeedData->m_ItemLevel, lpSeedData->m_ItemCode);
    } else {
        lpObj->pChaosBox[iSetItemNum].m_Level -= rand() % 3;
        lpObj->pChaosBox[iExcItemNum].m_Level -= rand() % 3;
        lpObj->pChaosBox[iHarmonyItemNum].Clear();
...
void CMixSystem::SeedSphereCompositeMix(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) //loc21
    {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            if (g_SocketOptionSystem.IsSeedItem(lpObj->pChaosBox[n].m_Type) == 1) {
                ...
                iCreationGemCount++;
            } else if (lpObj->pChaosBox[n].m_Type >= ITEMGET(12, 112) &&
                       lpObj->pChaosBox[n].m_Type <= ITEMGET(12, 117)) {
                iSeedSpear3rdCount++;
                lpSeedItem = &lpObj->pChaosBox[n];
    ...
    if (iSeedSpear3rdCount != 0) {
        if (iSphereItemCount != 1 || iSeedSpear3rdCount != 1 || iChaosJewel10BunchCount != 1 ||
            iCreationJewel10BunchCount != 1) {
            pMsg.Result = CB_INCORRECT_MIX_ITEMS;
            IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
            lpObj->ChaosLock = FALSE;
            return;
    ...
    if (bMakeSeedSpear4th == FALSE &&
        (iSphereItemCount != 1 || iSeedItemCount != 1 || iChoasGemCount != 1 || iCreationGemCount != 1)) {
        pMsg.Result = CB_INCORRECT_MIX_ITEMS;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        lpObj->ChaosLock = FALSE;
        return;
    ...
    iChaosTaxMoney = (int) ((__int64) g_SocketOptionSystem.m_RequireMoneyForSeedSphereComposite *
                            (__int64) g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / (__int64) 100);
    ...
    if (rand() % 100 > iSeedSphereCompositeRate) //fail rate
    {
        ...
        if (bMakeSeedSpear4th == TRUE) {
            OptionIndex = g_SocketOptionSystem.GetSeedIndexForSeedSpearItemCode(lpSeedItem->m_Type,
                                                                                lpSeedItem->m_Level);
        } else {
        ...
        if (SeedSphereItem.m_ItemCode != 0) {
            pMsg.Result = 1;
            ItemSerialCreateSend(lpObj->m_Index, 0xFF, 0, 0, SeedSphereItem.m_ItemCode, SeedSphereItem.m_ItemLevel, 1,
                                 0, 0, 0, lpObj->m_Index, 0, 0, 0, 0, 0);
            gObjInventoryCommit(lpObj->m_Index);
            g_Log.Add(
                    "[%s][%s] SeedSphere Composite Mix Success - SeedSphere Info ( Index : %d, Type : %d, Level : %d, ItemCode : %d )",
                    lpObj->AccountID, lpObj->Name, SeedSphereItem.m_SeedSphereIndex, SeedSphereItem.m_SeedType,
                    SeedSphereItem.m_SphereLevel, SeedSphereItem.m_ItemCode);
            return;
        }
...
void CMixSystem::SetSeedSphereMix(LPOBJ lpObj, BYTE btPos) {
    ...
    if (g_SocketOptionSystem.CheckItemForSameSeedType(pSocketItem, pSeedSpearItem->m_Type, pSeedSpearItem->m_Level) ==
        0) {
        pMsg.Result = CB_INCORRECT_MIX_ITEMS;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        lpObj->ChaosLock = FALSE;
    ...
    iChaosTaxMoney = (int) ((__int64) g_SocketOptionSystem.m_RequireMoneyForSeedSphereSet *
                            (__int64) g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / (__int64) 100);
    ...
    SocketByteOption = g_SocketOptionSystem.GetSeedSphereIndexForItemCode(pSeedSpearItem->m_Type,
                                                                          pSeedSpearItem->m_Level);
    ...
    float Dur = ItemGetDurability(pSocketItem->m_Type, pSocketItem->m_Level, pSocketItem->IsExtItem(),
                                  pSocketItem->IsSetItem());
    ...
    Item.Convert(pSocketItem->m_Type, pSocketItem->m_Option1, pSocketItem->m_Option2, pSocketItem->m_Option3,
                 pSocketItem->m_NewOption,
                 pSocketItem->m_SetOption, pSocketItem->m_ItemOptionEx, pSocketItem->m_SocketOption,
                 pSocketItem->m_BonusSocketOption, 0, CURRENT_DB_VERSION);
    ...
    g_Log.Add(
            "[%s][%s] Set SeedSphere Mix Success - ItemInfo ( Name : %s, ItemCode : %d, Level : %d, SocketOption[%d,%d,%d,%d,%d], BonusOption : %d )",
            lpObj->AccountID, lpObj->Name, ItemAttribute[Item.m_Type].Name, Item.m_Type, Item.m_Level,
...
void CMixSystem::SeedSphereRemoveMix(LPOBJ lpObj, BYTE btPos) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) //loc21
    {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            if (g_SocketOptionSystem.IsEnableSocketItem(lpObj->pChaosBox[n].m_Type) == 1) {
                if (lpObj->pChaosBox[n].m_SocketOption[btPos] != 0xFE &&
                    lpObj->pChaosBox[n].m_SocketOption[btPos] != 0xFF) //Slot with OPTION
                {
                    iSocketItemCount++; //
    ...
    iChaosTaxMoney = (int) ((__int64) g_SocketOptionSystem.m_RequireMoneyForSeedSphereRemove *
                            (__int64) g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / (__int64) 100);
    ...
    if (rand() % 100 < 80) {
        ...
        ItemSerialCreateSend(lpObj->m_Index, 219, 0, 0, lpSeedData->m_ItemCode, lpSeedData->m_ItemLevel, 1, 0, 0, 0,
                             lpObj->m_Index, 0, 0, 0, 0, 0);
    } else {
        pMsg.Result = CB_ERROR;
    }

    int levelitemdur = ItemGetDurability(lpSocketItem->m_Type, lpSocketItem->m_Level, lpSocketItem->IsExtItem(),
    ...
    g_Log.Add(
            "[%s][%s] SeedSphere Remove Success - ItemInfo ( Name : %s, ItemCode : %d, Level : %d, SocketOption[%d,%d,%d,%d,%d], BonusOption : %d )",
            lpObj->AccountID, lpObj->Name, ItemAttribute[lpObj->pChaosBox[iSocketItemPosition].m_Type].Name,
            lpObj->pChaosBox[iSocketItemPosition].m_Type,
            lpObj->pChaosBox[iSocketItemPosition].m_Level, lpObj->pChaosBox[iSocketItemPosition].m_SocketOption[0],
...
void CMixSystem::CherryBlossomMix(LPOBJ lpObj) {
    ...
    ItemSerialCreateSend(lpObj->m_Index, 255, 0, 0, Item.m_Type, Item.m_Level, Item.m_Durability, Item.m_Option1,
                         Item.m_Option2, Item.m_Option3, lpObj->m_Index, Item.m_NewOption, Item.m_SetOption, Duration,
                         Item.m_SocketOption, Item.m_BonusSocketOption);
...
void CMixSystem::ItemRefineMix(LPOBJ lpObj) {
    ...
    for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
        if (lpObj->pChaosBox[n].IsItem() == TRUE) {
            if (lpObj->pChaosBox[n].m_Type == ITEMGET(14, 191)) {
                ...
                DarkIronKnightItem++;
            } else if (g_SocketOptionSystem.IsSocketItem(&lpObj->pChaosBox[n]) && lpObj->pChaosBox[n].m_Level >= 7 &&
                       lpObj->pChaosBox[n].m_Option3 >= 1) {
                SocketItem++;
    ...
    if (rand() % 100 <= lpObj->ChaosSuccessRate) {
        ...
        ItemSerialCreateSend(lpObj->m_Index, 255, 0, 0, Item.m_Type, Item.m_Level, 0, Item.m_Option1, Item.m_Option2,
                             Item.m_Option3, lpObj->m_Index, Item.m_NewOption, 0, 0, SocketOption, 0);
        gObjInventoryCommit(lpObj->m_Index);
...
void CMixSystem::MonsterWingMix(LPOBJ lpObj) {
    ...
    if (InvalidItemCount || WingCount != 1 || ChaosCount != 1 || CreationCount != 1 ||
        (DeathKingMaterialCount + HellMaineMaterialCount + DarkPhoenixMaterialCount + DeathBeamMaterialCount) != 1) {
    ...
    if (rand() % 100 < lpObj->ChaosSuccessRate) {
        ...
        ItemSerialCreateSend(lpObj->m_Index, 255, 0, 0, ITEMGET(12, WingType), 0, 0, 0, Option2, Option3, -1, NOption,
                             0, 0, 0, 0);
        gObjInventoryCommit(lpObj->m_Index);

        g_Log.Add("[MonsterWing Mix] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID,
                  lpObj->Name,
...
void CMixSystem::CCFRewardMix(LPOBJ lpObj) {
    ...
    if (ChaosItemCount != 1 || JewelOfLife10BundleCount != 1 || JewelOfBless10BundleCount != 1 ||
        JewelOfSoul10BundleCount != 1 || BadgeItemCount != 1 || InvalidItemCount != 0) {
        lpObj->ChaosLock = FALSE;
        pMsg.Result = CB_INCORRECT_MIX_ITEMS;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
...
void CMixSystem::BloodCastleItemChaosMix_Multi(LPOBJ lpObj, int iMixCount) {
    ...
    if (bIsChaosGemExist == iMixCount && bIsAngelKingPaperExist == iMixCount && bIsBloodBoneExist == iMixCount) {
        ...
        g_Log.Add(
                "[Blood Castle] BloodCastle Ticket Mass Mix Chaos Mix Start (Account:%s, Name:%s, Level:%d) (MixCount:%d)",
                gObj[iIndex].AccountID, gObj[iIndex].Name, iAngelKingPaperLevel, iMixCount);
        ...
        if (iMIX_SUCCESS_RATE < 0 || iMIX_SUCCESS_RATE > 100) {
            ...
            g_Log.Add(
                    "[Blood Castle] BloodCastle Ticket Mix Chaos Mass Mix Failed - MixRate Out of Bound (Account:%s, Name:%s, Level:%d)",
                    gObj[iIndex].AccountID, gObj[iIndex].Name, iAngelKingPaperLevel);
            return;
        }
        ...
        if (iMIX_NEED_MONEY < 0) {
            IOCP.DataSend(iIndex, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.Add(
                    "[Blood Castle] BloodCastle Ticket Mix Chaos Mass Mix Failed - MixMoney < 0 (Account:%s, Name:%s, Level:%d)",
                    gObj[iIndex].AccountID, gObj[iIndex].Name, iAngelKingPaperLevel);
            return;
        }
        ...
        if ((gObj[iIndex].m_PlayerData->Money - iMIX_NEED_MONEY) < 0) {
            ...
            IOCP.DataSend(iIndex, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.Add(
                    "[Blood Castle] BloodCastle Ticket Mix Chaos Mass Mix Failed - Not Enough Money (Account:%s, Name:%s, Level:%d)",
                    gObj[iIndex].AccountID, gObj[iIndex].Name, iAngelKingPaperLevel);
            return;
        }

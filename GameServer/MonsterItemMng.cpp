...
void CMonsterItemMng::LoadMonsterItemDropRate() {
    ...
    for (pugi::xml_node monster = droprate.child("Monster"); monster; monster = monster.next_sibling()) {
        ...
        if (nLevel >= MAX_LEVEL_MONSTER) {
            g_Log.MsgBox("Error - Wrong Monster Level (%d) (Maximum is %d) (%s)", nLevel, MAX_LEVEL_MONSTER - 1,
                         __FUNCTION__);
            continue;
        }
        ...
        this->m_MonsterItemDropRate[nLevel].m_CreationRate =
                monster.attribute("JewelOfCreation").as_float() * 10000000.0;
...
void CMonsterItemMng::MagicBookGiveItemSearch(int monsterlevel, int maxlevel) {
    if (monsterlevel >= MAX_LEVEL_MONSTER) {
        g_Log.Add("[INFO] Monster Level greater than max drop Level (%d) excluded from general drop, line: %d",
                  MAX_LEVEL_MONSTER - 1, __LINE__);
        return;
    }
...
void CMonsterItemMng::NormalGiveItemSearchEx(int monsterlevel, int maxlevel) {
    ...
    if (monsterlevel >= MAX_LEVEL_MONSTER) {
        g_Log.Add("[INFO] Monster (%s) Level greater than max drop Level (%d) excluded from general drop, line: %d",
                  this->MonsterName, MAX_LEVEL_MONSTER - 1, __LINE__);
        return;
    }

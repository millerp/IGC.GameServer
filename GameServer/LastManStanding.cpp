        ...
        if (resets >= this->m_Rooms[i].MinReset && resets <= this->m_Rooms[i].MaxReset && gObj[aIndex].Level > 6) {
            ...
            for (int n = 0; n < LMS_MAX_USER_PER_ROOM; n++) {
                if (this->m_Rooms[i].m_Data[n].iIndex == 0) {
                    ...
                    g_Log.Add("[PVP_LastManStanding][%s][%s] Register on room [%d]", gObj[aIndex].AccountID,
                              gObj[aIndex].Name, i);
                    return true;
                }
            }
...
void CLastManStanding::Run() {
    ...
    if (!bTIME_TO_START) {
        for (std::vector<sLMS_START_TIME>::iterator it = this->m_vtStartList.begin();
             it != this->m_vtStartList.end(); it++) {
    ...
    if (bTIME_TO_START) {
        for (int n = 0; n < this->m_iRoomCount; n++) {
            if (m_wCurrentMin == StartMinute && m_Rooms[n].bState == 0) {
                if (n == 0) {
                    PMSG_NOTICE pNotice;
                    TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 357), Lang.GetMap(0, this->m_Cfg.iRegNPCMap),
                                             this->m_Cfg.iRegTime);
            ...
            if (m_Rooms[n].bState == 3) {
                for (int i = 0; i < m_Rooms[n].regCount; i++) {
                    if (m_Rooms[n].m_Data[i].iIndex != 0 &&
                        gObj[m_Rooms[n].m_Data[i].iIndex].MapNumber != this->m_Cfg.iPVPMap) {
                        g_Log.Add("[PVP_LastManStanding] User %s left the map.",
                                  gObj[m_Rooms[n].m_Data[i].iIndex].Name);
                            ...
                            for (int i = 0; i < m_Rooms[n].regCount; i++) {
                                if (this->m_Rooms[n].m_Data[i].iIndex != 0) {
                                    if (m_Rooms[n].m_Data[i].iScore > 0) {
                                        ...
                                        sprintf(text, Lang.GetText(0, 360),
                                                gObj[this->m_Rooms[n].m_Data[i].iIndex].Name, n,
                                                this->m_Rooms[n].m_Data[i].iScore);
            ...
            if (m_wCurrentMin > StartMinute + this->m_Cfg.iRegTime + this->m_Cfg.iDuration + 1 &&
                m_Rooms[n].bState == 3) {
                PMSG_NOTICE pNotice;
        ...
        for (int i = 0; i < m_Rooms[Room].regCount; i++) {
            if (this->m_Rooms[Room].m_Data[i].iIndex > 0) {
                if (gObjIsConnectedGP(this->m_Rooms[Room].m_Data[i].iIndex)) {
                    ...
                    gObjTeleport(this->m_Rooms[Room].m_Data[i].iIndex, m_Rooms[Room].Map,
                                 m_Rooms[Room].X1 + rand() % xrand, m_Rooms[Room].Y1 + rand() % yrand);
...
void CLastManStanding::UserDie(int aIndex, int aTargetIndex) {
    ...
    for (int i = 0; i < this->m_Rooms[room].regCount; i++) {
        if (this->m_Rooms[room].m_Data[i].iIndex == aIndex) {
            ...
            g_Log.Add("[PVP_LastManStanding] Room [%d] [%d] Killed [%d] Death[%d] [%s]", room, aTargetIndex, aIndex,
                      this->m_Rooms[room].m_Data[i].iDeaths, gObj[this->m_Rooms[room].m_Data[i].iIndex].Name);
                    ...
                    for (int i = 0; i < m_Rooms[room].regCount; i++) {
                        if (this->m_Rooms[room].m_Data[i].iIndex != 0) {
                            if (m_Rooms[room].m_Data[i].iScore > 0) {
                                ...
                                char text[200];
                                sprintf(text, Lang.GetText(0, 362), gObj[this->m_Rooms[room].m_Data[i].iIndex].Name,
                                        room, this->m_Rooms[room].m_Data[i].iScore);
                                this->RewardUser(this->m_Rooms[room].m_Data[i].iIndex);
                                g_Log.Add("[PVP_LastManStanding] Room [%d] reward user [%s]", room,
                                          gObj[this->m_Rooms[room].m_Data[i].iIndex].Name);
                                AllPlayerMsgSend(text);
                            }
            ...
            g_Log.Add("[PVP_LastManStanding] Room [%d] [%d] Killed [%d] Score [%d] [%s]", room, aIndex, aTargetIndex,
                      this->m_Rooms[room].m_Data[i].iScore, gObj[this->m_Rooms[room].m_Data[i].iIndex].Name);

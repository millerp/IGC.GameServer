...
BOOL CJewelOfHarmonySystem::StrengthenItemByJewelOfRise(LPOBJ lpObj, int source, int target) {
    ...
    if (iItemType == JEWELOFHARMONY_ITEM_TYPE_NULL) {
        g_Log.Add(
                "[LuckyItem][Strengthen Item] Strengthen Fail [%s][%s] Name[%s] Type[%d] Serial[%I64d] Invalid ItemType[%d]",
    ...
    if (iItemOption == AT_JEWELOFHARMONY_NOT_STRENGTHEN_ITEM) {
        g_Log.Add(
                "[LuckyItem][Strengthen Item] Strengthen Fail - NOT OPTION [%s][%s] Name[%s] Type[%d] Serial[%I64d] ItemType[%d]",
                lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
                pTarget->m_Number, iItemType);
    ...
    g_Log.Add(
            "[LuckyItem][Strengthen Item] Strengthen Success [%s][%s] Name[%s] Type[%d] Serial[%I64d] Rate (%d/%d) Option %d OptionLevel %d",
            lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
            pTarget->m_Number, iSuccessRate, this->m_iRateStrengthenSuccess,
    ...
    lpObj->pInventory[target].m_Durability =
            levelitemdur * lpObj->pInventory[target].m_Durability / lpObj->pInventory[target].m_BaseDurability;
...
BOOL CJewelOfHarmonySystem::StrengthenItemByJewelOfHarmony(LPOBJ lpObj, int source, int target) {
    ...
    if (iItemType == JEWELOFHARMONY_ITEM_TYPE_NULL) {
        g_Log.Add(
                "[JewelOfHarmony][Strengthen Item] Strengthen Fail [%s][%s] Name[%s] Type[%d] Serial[%I64d] Invalid ItemType[%d]",
                lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
                pTarget->m_Number, iItemType);
    ...
    if (iItemOption == AT_JEWELOFHARMONY_NOT_STRENGTHEN_ITEM) {
        g_Log.Add(
                "[JewelOfHarmony][Strengthen Item] Strengthen Fail - NOT OPTION [%s][%s] Name[%s] Type[%d] Serial[%I64d] ItemType[%d]",
                lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
                pTarget->m_Number, iItemType);
    ...
    if (iSuccessRate >= this->m_iRateStrengthenSuccess) {
        g_Log.Add(
                "[JewelOfHarmony][Strengthen Item] Strengthen Fail [%s][%s] Name[%s] Type[%d] Serial[%I64d]  Rate (%d/%d)",
                lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
                pTarget->m_Number, iSuccessRate, this->m_iRateStrengthenSuccess);
        GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 274), lpObj->m_Index, 1);
        return TRUE;
    ...
    g_Log.Add(
            "[JewelOfHarmony][Strengthen Item] Strengthen Success [%s][%s] Name[%s] Type[%d] Serial[%I64d] Rate (%d/%d) Option %d OptionLevel %d",
            lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
            pTarget->m_Number, iSuccessRate, this->m_iRateStrengthenSuccess,
            iItemOption, iItemOptionLevel);
...
void CJewelOfHarmonySystem::StrengthenItemByMacro(LPOBJ lpObj, BYTE invenrotyTargetPos, BYTE btOptionType,
                                                  BYTE btOptionLevel) {
...
BOOL CJewelOfHarmonySystem::MakeSmeltingStoneItem_MultiMix(OBJECTSTRUCT *lpObj, int iMixCount) {
    ...
    for (int i = 0; i < iMixCount; i++) {
        ...
        if (iRate < JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE) {
            ...
            g_Log.Add(
                    "[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mass Mix (%d) Success [%s][%s], Money(%d-%d) Rate(%d/%d)",
            ...
            g_Log.Add(
                    "[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mass Mix Fail (%d) [%s][%s], Money : %d-%d Rate(%d/%d)",
                    bIsItemNormal, i, lpObj->AccountID, lpObj->Name,
                    lpObj->m_PlayerData->Money, iMakeSmeltingStoneMixPrice, iRate,
                    JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE);
        }
...
BOOL CJewelOfHarmonySystem::MakeSmeltingStoneItem(LPOBJ lpObj) {
    ...
    if (iRate < JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE) {
        ...
        g_Log.Add(
                "[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mix Success [%s][%s], Money(%d-%d) Rate(%d/%d)",
        ...
        g_Log.Add(
                "[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mix Fail [%s][%s], Money : %d-%d Rate(%d/%d)",
                bIsItemNormal, lpObj->AccountID, lpObj->Name,
                lpObj->m_PlayerData->Money, iMakeSmeltingStoneMixPrice, iRate,
                JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE);
    }
...
BOOL CJewelOfHarmonySystem::SmeltItemBySmeltingStone(LPOBJ lpObj, int source, int target) {
    ...
    if (iSuccessRate >= iRateSmeltingSuccess) {
        ...
        g_Log.Add(
                "[JewelOfHarmony][Smelt Item] Smelt Item Fail by Normal[%d] [%s][%s] Name[%s] Type [%d] Serial [%I64d] Rate(%d/%d) Level(%d->%d)",
        ...
        g_Log.Add(
                "[JewelOfHarmony][Smelt Item] Smelt Item Success by Normal[%d] [%s][%s] Name[%s] Type [%d] Serial [%I64d] Rate(%d/%d) Level(%d->%d)",
                bIsNormalSmeltingStone, lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
...
BOOL CJewelOfHarmonySystem::RestoreStrengthenItem(LPOBJ lpObj) {
    ...
    g_Log.Add(
            "[JewelOfHarmony][Restore Item] Restore Strengtened Item [%s][%s] Name[%s] ItemType[%d] Serial[%I64d] OptionType[%d] OptionLevel [%d] Money %d-%d",

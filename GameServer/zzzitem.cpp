...
void
CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3, BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx,
               BYTE *SocketOption, BYTE SocketBonusOption, BYTE PeriodItemOption, BYTE DbVersion) {
    int _type;
    int ItemLevel;
    LPITEM_ATTRIBUTE p;
    int SOption;
    ...
    if (((_type >= ITEMGET(12, 3)) && (_type <= ITEMGET(12, 6))) ||
        ((_type >= ITEMGET(12, 36)) && (_type <= ITEMGET(12, 40))) || (_type == ITEMGET(13, 30)) ||
        (_type == ITEMGET(12, 41)) || (_type == ITEMGET(12, 42)) || (_type == ITEMGET(12, 43)) ||
        (_type == ITEMGET(12, 49)) || (_type == ITEMGET(12, 50)) ||
        ((_type >= ITEMGET(12, 262) && _type <= ITEMGET(12, 265)))) //Third Wings Fix
    {
        Attribute2 = 0;
    }
    ...
    if (ItemGetDurability(this->m_Type, 0, 0, 0) != 0) {
        this->m_BaseDurability = (float) ItemGetDurability(this->m_Type, this->m_Level, Attribute2 & 0x7F,
                                                           this->m_SetOption);
    } else {
    ...
    if (p->RequireLevel != 0) {
        ...
        } else if ((_type >= ITEMGET(12, 3) && _type <= ITEMGET(12, 6)) || _type == ITEMGET(12, 42) ||
                   _type == ITEMGET(12, 49)) {
            this->m_RequireLevel = p->RequireLevel + (this->m_Level * 5);
        } else if ((_type >= ITEMGET(12, 36) && _type <= ITEMGET(12, 40)) || _type == ITEMGET(12, 43) ||
                   _type == ITEMGET(12, 50) || _type == ITEMGET(12, 270)) //Third Wings Fix Required Level
        {
    ...
    if ((Attribute2 & 0x3F) > 0) // TODO (WING) 2
    {
        if (this->m_RequireLevel > 0) {
            if ((_type >= ITEMGET(12, 36) && _type <= ITEMGET(12, 40)) || this->m_Type == ITEMGET(12, 43) ||
                this->m_Type == ITEMGET(12, 50) || this->m_Type == ITEMGET(12, 270)) //Third Wings Fix Required Level
            {
                this->m_RequireLevel;
            } else if (_type < ITEMGET(0, 0) || _type >= ITEMGET(12, 0) &&
                                                !((_type >= ITEMGET(12, 36) && _type <= ITEMGET(12, 40)) ||
                                                  this->m_Type == ITEMGET(12, 43) || this->m_Type == ITEMGET(12, 50) ||
                                                  this->m_Type == ITEMGET(12, 266) ||
                                                  this->m_Type == ITEMGET(12, 267) ||
                                                  this->m_Type == ITEMGET(12, 268) ||
                                                  this->m_Type == ITEMGET(12, 270))) //Third Wings Fix Anti-Hack
            {
                this->m_RequireLevel += 20;
            }
        }
    ...
    if (p->Defense > 0) {
        if (this->m_Type >= ITEMGET(6, 0) && this->m_Type < ITEMGET(7, 0)) {
            ...
            if ((_type >= ITEMGET(12, 36) && _type <= ITEMGET(12, 40)) || _type == ITEMGET(12, 43) ||
                _type == ITEMGET(12, 50) || _type == ITEMGET(12, 268)) {
                this->m_Defense += this->m_Level * 4;

                if (this->m_Level >= 10) {
                    this->m_Defense += (this->m_Level - 9);
                }
            } else if ((_type >= ITEMGET(12, 3) && _type <= ITEMGET(12, 6)) || _type == ITEMGET(13, 30) ||
                       _type == ITEMGET(13, 4)) {
    ...
    if (Option2 != 0) {
        ...
        if ((_type >= ITEMGET(12, 0) && _type <= ITEMGET(12, 6)) ||
            (_type >= ITEMGET(12, 41) && _type <= ITEMGET(12, 42)) || _type == ITEMGET(12, 49) ||
            _type == ITEMGET(12, 269))    // Wings
        {
            this->m_Special[this->m_SpecialNum] = 84;
            this->m_Option2 = 1;
        }
        ...
        if ((_type >= ITEMGET(12, 36) && _type <= ITEMGET(12, 40)) || _type == ITEMGET(12, 43) ||
            _type == ITEMGET(12, 50) || _type == ITEMGET(12, 270))    // Third Wings
        {
            this->m_Special[this->m_SpecialNum] = 84;
            this->m_Option2 = 1;
        }
    ...
    if (Option3 != 0) {
        ...
        if ((_type >= ITEMGET(13, 8) && _type < ITEMGET(13, 14)) ||
            ...
            (_type >= ITEMGET(13, 109) && _type <= ITEMGET(13, 115)) ||
            (_type >= ITEMGET(13, 171) && _type <= ITEMGET(13, 176))) {
        ...
        } else if (_type == ITEMGET(12, 38)) // Third Wings "Wing of Illusion"
        {
            ...
            if ((this->m_NewOption & 0x20) >= 0 && (this->m_NewOption & 0x10) !=
                                                   0) //JoL + Increase Attack Speed PS: I set both Add. Defense and Add. Damage enabled because when item is full have Additional Damage!
            {
                this->m_Special[this->m_SpecialNum] = 80; //Additional Damage
            } else if ((this->m_NewOption & 0x20) == 0 && (this->m_NewOption & 0x10) == 0) //Only JoL Option
            {
                this->m_Special[this->m_SpecialNum] = 85; //Recover Life
            } else if ((this->m_NewOption & 0x20) != 0 && (this->m_NewOption & 0x10) ==
                                                          0) //JoL + Take No Effect PS: I set 0x10 equal 0 because Additional Defense only appears with Increase Attack Speed Disabled! It's a Nice Anti-hack or Checker :D
            {
        ...
        } else if (_type == ITEMGET(12, 37)) // Third Wings "Wing of Vortex"
        {
            ...
            if ((this->m_NewOption & 0x20) >= 0 && (this->m_NewOption & 0x10) !=
                                                   0) // JoL + Increase Attack Speed PS: I set both Add. Defense and Add. Magic enabled because when item is full have Additional Damage!
            {
                this->m_Special[this->m_SpecialNum] = 81; // Additional Magic
            } else if ((this->m_NewOption & 0x20) == 0 && (this->m_NewOption & 0x10) == 0) // Only JoL Option
            {
    ...
    if ((_type >= ITEMGET(6, 0) && _type < ITEMGET(12, 0)) || (_type >= ITEMGET(13, 8) && _type <= ITEMGET(13, 9)) ||
        (_type >= ITEMGET(13, 21) && _type <= ITEMGET(13, 24)) ||
        (_type == ITEMGET(13, 110)) || (_type == ITEMGET(13, 111)) || (_type == ITEMGET(13, 112)) ||
        (_type >= ITEMGET(13, 173) && _type <= ITEMGET(13, 176))) {
    ...
    if ((_type >= ITEMGET(0, 0) && _type < ITEMGET(6, 0)) || (_type >= ITEMGET(13, 12) && _type <= ITEMGET(13, 13)) ||
        (_type >= ITEMGET(13, 25) && _type <= ITEMGET(13, 28)) ||
        _type == ITEMGET(13, 109) || _type == ITEMGET(13, 113) || _type == ITEMGET(13, 114) ||
        _type == ITEMGET(13, 115) || (_type >= ITEMGET(13, 171) && _type <= ITEMGET(13, 172))) {
        if (((this->m_NewOption >> 5) & 1) != 0) {
            this->m_Special[this->m_SpecialNum] = 0x5C;
            this->m_SpecialNum++;
        }

        if ((_type >= ITEMGET(5, 0) && _type < ITEMGET(6, 0)) || (_type == ITEMGET(13, 12)) ||
            (_type == ITEMGET(13, 25)) || (_type == ITEMGET(13, 27)) || (_type == ITEMGET(13, 109)) ||
    ...
    if (((_type >= ITEMGET(12, 3)) && (_type <= ITEMGET(12, 6))) || (_type == ITEMGET(13, 30)) ||
        (_type == ITEMGET(12, 42) || _type == ITEMGET(12, 269))) {
        if (((this->m_NewOption) & 1) != 0) {
    ...
    if (((_type >= ITEMGET(12, 36)) && (_type <= ITEMGET(12, 40))) || (_type == ITEMGET(12, 43)) ||
        (_type == ITEMGET(12, 50) || _type == ITEMGET(12, 270))) {
        if (((this->m_NewOption) & 1) != 0) {
...
void CItem::Value() {
    ...
    if (this->m_Type == ITEMGET(14, 13)) {
    ...
    } else if (this->m_Type == ITEMGET(12, 139) || this->m_Type == ITEMGET(12, 140) ||
               this->m_Type == ITEMGET(12, 142) || this->m_Type == ITEMGET(12, 143)) {
        ...
        if (this->m_Type == ITEMGET(14, 63)) {
            Gold = 200000;
        } else if (this->m_Type == ITEMGET(14, 85) || this->m_Type == ITEMGET(14, 86) ||
                   this->m_Type == ITEMGET(14, 87) || this->m_Type == ITEMGET(14, 90)) {
            Gold = (int) this->m_Durability * 300;
            ...
            Gold = 363000;
        } else if (this->m_Type == ITEMGET(14, 176) || this->m_Type == ITEMGET(14, 177) ||
                   this->m_Type == ITEMGET(14, 178) || this->m_Type == ITEMGET(14, 179)) {
...
void CItem::SetPetItemInfo(int petlevel, UINT64 petexp) {
    if (this->m_Type == ITEMGET(13, 4)) {
        ...
        if (this->m_PetItem_Exp < gPetItemExp.m_DarkHorseExpTable[this->m_PetItem_Level] ||
            this->m_PetItem_Exp >= gPetItemExp.m_DarkHorseExpTable[1 + this->m_PetItem_Level]) {
        ...
        this->Convert(this->m_Type, this->m_Option1, this->m_Option2,
                      this->m_Option3, this->m_NewOption, this->m_SetOption, this->m_ItemOptionEx, 0, -1, 0,
                      CURRENT_DB_VERSION);
        this->PetValue();
        ...
        if (this->m_PetItem_Exp < gPetItemExp.m_DarkSpiritExpTable[this->m_PetItem_Level] ||
            this->m_PetItem_Exp >= gPetItemExp.m_DarkSpiritExpTable[1 + this->m_PetItem_Level]) {
...
BOOL CItem::DecPetItemExp(int percent) {
    ...
    if (this->m_Type == ITEMGET(13, 4)) {
        UINT64 exp = gPetItemExp.m_DarkHorseExpTable[1 + this->m_PetItem_Level] -
                     gPetItemExp.m_DarkHorseExpTable[this->m_PetItem_Level];
        exp = exp * percent / 100;
        ...
        UINT64 exp = gPetItemExp.m_DarkSpiritExpTable[1 + this->m_PetItem_Level] -
                     gPetItemExp.m_DarkSpiritExpTable[this->m_PetItem_Level];
        exp = exp * percent / 100;
        this->m_PetItem_Exp -= exp;
...
void ItemByteConvert(unsigned char *buf, int type, BYTE Option1, BYTE Option2, BYTE Option3, BYTE level, BYTE dur,
                     BYTE Noption, BYTE SetOption, BYTE JewelOfHarmonyOption, BYTE ItemEffectEx, BYTE *SocketOption,
                     BYTE SocketBonusOption, BYTE PeriodItemOption) {
    memset(buf, 0, 12);
    int n = 0;
    ...
    for (pugi::xml_node section = main.child("Section"); section; section = section.next_sibling()) {
        ...
        for (pugi::xml_node item = section.child("Item"); item; item = item.next_sibling()) {
            ...
            if (ITEMGET(ItemType, ItemIndex) < 0 || ITEMGET(ItemType, ItemIndex) >= ITEMGET(16, 0)) {
                g_Log.MsgBox("Error - wrong Item configuration (ItemType:%d ItemIndex:%d) in Section:%d", ItemType,
                             ItemIndex, ItemType);
                return FALSE;
            }
...
int GetLevelItem(int type, int index, int level) {
    ...
    if (itemlevel >= level - 18 && itemlevel <= level) {
        ...
        if (type == 13) {
            if (index == 8 || index == 9 || index == 12 || index == 13 || index == 20 || index == 21 || index == 22 ||
                index == 23 || index == 24 || index == 25 || index == 26 || index == 27 || index == 28) {
                if (itemlevel > 4) {
                    itemlevel = 4;
                }
            }

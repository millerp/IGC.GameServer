    ...
    switch (npcnum) {
        ...
        case 258: {
            if (g_LastManStanding.m_Cfg.bEnable) {
                if (lpNpc->MapNumber == g_LastManStanding.m_Cfg.iRegNPCMap &&
                    lpNpc->X == g_LastManStanding.m_Cfg.iRegNPCX && lpNpc->Y == g_LastManStanding.m_Cfg.iRegNPCY) {
                    if (g_LastManStanding.m_Rooms[0].bState !=
                        1) // rooms[0] because the state at this point is same for all rooms
                    {
                    ...
                    if (g_LastManStanding.m_Cfg.iReqItem) {
                        int invitationpos = -1;
                        int removedCount = 0;
                        if (gObjGetItemCountInIventory(lpObj->m_Index, ITEMGET(g_LastManStanding.m_Cfg.iReqItemCat,
                                                                               g_LastManStanding.m_Cfg.iReqItemIdx)) <
                            g_LastManStanding.m_Cfg.iReqItemCount) {
                            MsgOutput(lpObj->m_Index, Lang.GetText(0, 398), g_LastManStanding.m_Cfg.iReqItemCount,
                                      ItemAttribute[ITEMGET(g_LastManStanding.m_Cfg.iReqItemCat,
                                                            g_LastManStanding.m_Cfg.iReqItemIdx)].Name);
                            return false;

                        }
                        for (short i = 0; i < MAIN_INVENTORY_SIZE; ++i) {
                            if (lpObj->pInventory[i].m_Type > 0 && lpObj->pInventory[i].m_Type ==
                                                                   ITEMGET(g_LastManStanding.m_Cfg.iReqItemCat,
                                                                           g_LastManStanding.m_Cfg.iReqItemIdx)) {
                ...
                switch (lpObj->m_PK_Count) {
                    ...
                    default:
                        if (lpObj->m_PK_Level > 3) {
                            dwNeedPoint = g_ConfigRead.pk.iPKPointIncForMurderer * (lpObj->m_PK_Count - 3) +
                                          g_ConfigRead.pk.GetPKReqPoint(6);
                        }
                        break;
                }
...
BOOL NpcAngelMessanger(LPOBJ lpNpc, LPOBJ lpObj) {
    ...
    if (iITEM_LEVEL != 0) {
        ...
        if (g_BloodCastle.GetCurrentState(iITEM_LEVEL - 1) == BC_STATE_CLOSED &&
            g_BloodCastle.CheckCanEnter(iITEM_LEVEL - 1) != false) {
            PMSG_TALKRESULT pMsg;
...
BOOL NpcCastleCrown(LPOBJ lpNpc, LPOBJ lpObj) //GS-CS Decompiled 100%
{
    ...
    if (gObjIsConnected(iUserIndex) == FALSE) {
        ...
        if (gObjIsConnected(iSwitchIndex1) == FALSE || gObjIsConnected(iSwitchIndex2) == FALSE) {
            g_Log.Add("[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s)", lpObj->AccountID,
                      lpObj->Name, lpObj->m_PlayerData->GuildName);
            return TRUE;
        }

        if (lpObj->m_btCsJoinSide != gObj[iSwitchIndex1].m_btCsJoinSide ||
            lpObj->m_btCsJoinSide != gObj[iSwitchIndex2].m_btCsJoinSide) {
            GSProtocol.GCAnsCsAccessCrownState(lpObj->m_Index, 4);
            g_Log.Add("[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s) (S1:%s/%s)(S2:%s/%s)",
                      lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->GuildName, gObj[iSwitchIndex1].Name,
                      gObj[iSwitchIndex1].m_PlayerData->GuildName, gObj[iSwitchIndex2].Name,
                      gObj[iSwitchIndex2].m_PlayerData->GuildName);
            return TRUE;
        } else {
            ...
            g_Log.Add("[CastleSiege] [%s][%s] Start to Register Castle Crown (GUILD:%s)", lpObj->AccountID, lpObj->Name,
                      lpObj->m_PlayerData->GuildName);
        }
...
BOOL NpcCastleSwitch(LPOBJ lpNpc, LPOBJ lpObj) //GS-CS Decompiled 100%
{
    ...
    if (!gObjIsConnected(iUserIndex)) {
        ...
        g_Log.Add("[CastleSiege] [%s][%s] Start to Push Castle Crown Switch (GUILD:%s) - CS X:%d/Y:%d",
                  lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->GuildName, lpNpc->X, lpNpc->Y);
    } else if (lpObj->m_Index != iUserIndex) {
    ...
    if (gObjIsConnected(lpObj->m_Index)) {
        ...
        switch (lpNpc->Class) // Not original, but easier
        {
            case 543: {
                ...
                g_Log.Add("[Gens System] Gens_Duprian_NPC [%s][%s] GensName [%s] Total ContributePoint [%d]",
            ...
            case 544: {
                ...
                g_Log.Add("[Gens System] Gens_Vanert_NPC [%s][%s] GensName [%s] Total ContributePoint [%d]",
                          lpObj->AccountID, lpObj->Name, GensName, pResult.dwContributePoint);
            }
                break;
...
bool NpcShadowPhantom(int iObjIndex) //-> Complete
{
    ...
    gObjAddBuffEffect(lpObj, BUFFTYPE_ATTDEF_POWER_INC, EFFECTTYPE_IMPROVE_DAMAGE, iIncleaseAttackPower,
                      EFFECTTYPE_IMPROVE_DEFENSE, iIncleaseDefensePower, (lpObj->Level / 6 + 30) * 60);
    return true;
}

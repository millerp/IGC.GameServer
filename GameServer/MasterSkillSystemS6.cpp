...
void CMasterSkillSystemS6::AddMasterSkill(int aIndex, PMSG_MASTER_SKILL_RECV *aRecv) {
    ...
    if (skillindex == -1) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                   "[Mastering] (%s)(%s) ERROR: Trying to add invalid/not exist skill (skillindex == -1) (%d)",
    ...
    if (skillnum == -1) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                   "[Mastering] (%s)(%s) ERROR: Trying to add invalid/not exist skill (skillnum == -1) (%d)",
                   lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
        LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
        return;
    }
    ...
    if (lpObj->m_PlayerData->MasterPoint < MasterP) {
        g_Log.Add(0, __FILE__, __FUNCTION__,
                  "[Mastering] (%s)(%s) AddMasterSkill Fail - not enough MLPoints (need:%d/have:%d) (skill:%d)",
    ...
    if (MagicDamageC.SkillGetRequireClass(lpObj->Class, lpObj->m_PlayerData->ChangeUP, aRecv->MasterSkill) == FALSE) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                   "[Mastering] (%s)(%s) AddMasterSkill Fail - invalid Class/ChangeUP (class:%d/CUP:%d) (skill:%d)",
    ...
    if (SkillType == 3) // passive
    {
        ...
        if (skillid == -1) {
            if (this->AddSkillInfo(lpObj, aRecv->MasterSkill, 1) != -1) {
                Result = true;
                level = 1;
                g_Log.Add(0, __FILE__, __FUNCTION__, "[Mastering] (%s)(%s) Added PASSIVE skill (%d)", lpObj->AccountID,
                          lpObj->Name, aRecv->MasterSkill);
            }
        } else {
            if (lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel >=
                this->m_SkillInfo[lpObj->m_PlayerData->m_MasterSkills[skillid].SkillIndex].btMaxLevel) {
                this->AnsAddMasterSkill(aIndex, Result, aRecv->MasterSkill, level);
                LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                           "[Mastering] (%s)(%s) ERROR: SkillLevel >= btMaxLevel (SKILL:%d) (TYPE: PASSIVE UPGRADE)",
                           lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                return;
            }
            ...
            g_Log.Add(0, __FILE__, __FUNCTION__, "[Mastering] (%s)(%s) Upgraded PASSIVE skill (%d) (%d) -> (%d)",
                      lpObj->AccountID, lpObj->Name, aRecv->MasterSkill,
                      lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel - 1,
                      lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel);
        }
    ...
    } else if (SkillType == 4) // active
    {
        ...
        if (OldSkillID == 0 || aRecv->MasterSkill == 418) {
            ...
            if (pos == -1) {
                ...
                LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                           "[Mastering] (%s)(%s) ERROR: gObjMagicAdd() == -1 (SKILL:%d) (TYPE: ACTIVE FIRST LEVEL)",
                           lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                return;
            }
            ...
            g_Log.Add(0, __FILE__, __FUNCTION__, "[Mastering] (%s)(%s) Added ACTIVE skill FIRST LEVEL (%d)",
                      lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
        } else if (skillid == -1) {
            if (MagicDamageC.GetMasterSkillType(OldSkillID) < 3) {
                if (gObjGetMagicSearch(lpObj, OldSkillID) != 0) {
                    if (gObjMagicDel(lpObj, OldSkillID, 0) == -1) {
                        ...
                        LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                                   "[Mastering] (%s)(%s) ERROR: gObjMagicDel() == -1 (SKILL:%d) (TYPE: ACTIVE STRENGTHENER)",
                                   lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                        return;
                    }
                    ...
                    if (pos == -1) {
                        ...
                        LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                                   "[Mastering] (%s)(%s) ERROR: gObjMagicAdd() == -1 (SKILL:%d) (TYPE: ACTIVE STRENGTHENER)",
                                   lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                        return;
                    }
                    ...
                    g_Log.Add(0, __FILE__, __FUNCTION__, "[Mastering] (%s)(%s) Added ACTIVE skill STRENGTHENER (%d)",
                              lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                }
            } else {
                ...
                if (gObjGetMagicSearch(lpObj, OldSkillID) != 0) {
                    ...
                    if (pos == -1) {
                        ...
                        LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                                   "[Mastering] (%s)(%s) ERROR: gObjMagicAdd() == -1 (SKILL:%d) (TYPE: ACTIVE MASTERY)",
                                   lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                        return;
                    }
                    ...
                    g_Log.Add(0, __FILE__, __FUNCTION__, "[Mastering] (%s)(%s) Added ACTIVE skill MASTERY (%d)",
                              lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                }
            }
        } else {
            ...
            if (lpMagic == NULL) {
                g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                           "[Mastering] ERROR - ACTIVE SKILL NOT EXIST IN CMagicInf (%s)(%s)(%d)", lpObj->AccountID,
                           lpObj->Name, aRecv->MasterSkill);
                LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                return;
            }
            ...
            if (lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel >=
                ...
                LeaveCriticalSection(&lpObj->m_PlayerData->m_MasterSkillCriti);
                g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                           "[Mastering] (%s)(%s) ERROR: SkillLevel >= btMaxLevel (SKILL:%d) (TYPE: ACTIVE UPGRADE)",
                           lpObj->AccountID, lpObj->Name, aRecv->MasterSkill);
                return;
            }
            ...
            Result = true;
            level = lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel;
            g_Log.Add(0, __FILE__, __FUNCTION__, "[Mastering] (%s)(%s) Upgraded ACTIVE skill (%d) (%d) -> (%d)",
                      lpObj->AccountID, lpObj->Name, aRecv->MasterSkill,
                      lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel - 1,
                      lpObj->m_PlayerData->m_MasterSkills[skillid].SkillLevel);
        }
...
void CMasterSkillSystemS6::AnsAddMasterSkill(int aIndex, BYTE btResult, int MasterSkill, int level) {
    ...
    if (level < this->m_SkillInfo[this->FindSkillArrayNum(MasterSkill)].btMaxLevel) {
        ...
        pMsg.nextvalue = this->m_SkillInfo[this->FindSkillArrayNum(MasterSkill)].LevelVal[
                this->m_SkillInfo[this->FindSkillArrayNum(MasterSkill)].btMaxLevel - 1];
    }
...
BYTE CMasterSkillSystemS6::FindSkillIndex(WORD Skillid, BYTE Class) {
    for (int i = 0; i < 512; i++) {
        if (this->m_SkillInfo[i].SkillID == Skillid) {
            if (this->m_SkillInfo[i].wType == -1 || this->m_SkillInfo[i].wType == 65535 ||
                this->m_SkillInfo[i].wType == Class ||
                (this->m_SkillInfo[i].wType == MST_MG + MST_DK && (Class == MST_MG || Class == MST_DK))) {
...
void CMasterSkillSystemS6::GCSendMasterSkillList(int aIndex) {
    ...
    for (int i = 0; i < 120; i++) {
        if (lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId != 0xFFFF) {
            ...
            lpBody.btMasterSkillId = this->FindSkillIndex(lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId,
                                                          this->GetClass(gObj[aIndex].Class));
            lpBody.value = this->m_SkillInfo[this->FindSkillArrayNum(
                    lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId)].LevelVal[
                    lpObj->m_PlayerData->m_MasterSkills[i].SkillLevel - 1]; // todo
            if (lpBody.btLevel < this->m_SkillInfo[this->FindSkillArrayNum(
                    lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId)].btMaxLevel) {
                lpBody.nextvalue = this->m_SkillInfo[this->FindSkillArrayNum(
                        lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId)].LevelVal[lpObj->m_PlayerData->m_MasterSkills[i].SkillLevel]; // todo ; // todo
            } else {
                lpBody.nextvalue = this->m_SkillInfo[this->FindSkillArrayNum(
                        lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId)].LevelVal[
                        lpObj->m_PlayerData->m_MasterSkills[i].SkillLevel - 1]; // todo ; // todo
    ...
    g_Log.Add(0, __FILE__, __FUNCTION__, "[%d][%s][%s] Master Skill List send (%d)", aIndex, lpObj->AccountID,
              lpObj->Name, count);
}
...
float CMasterSkillSystemS6::GetEffectValue(LPOBJ lpObj, int Skill) {
    ...
    for (int i = 0; i < 120; i++) {
        if (lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId == Skill) {
            return this->m_SkillInfo[lpObj->m_PlayerData->m_MasterSkills[i].SkillIndex].LevelVal[
                    lpObj->m_PlayerData->m_MasterSkills[i].SkillLevel - 1];
        }
    }
...
void CMasterSkillSystemS6::CalcEffect(OBJECTSTRUCT *lpObj) {
    ...
    for (int i = 0; i < 120; i++) {
        if (lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId != 0xFFFF &&
            lpObj->m_PlayerData->m_MasterSkills[i].SkillIndex != 0xFFFF) {
            switch (lpObj->m_PlayerData->m_MasterSkills[i].SkillMagicId) {
                ...
                case 601:
                    ...
                    GSProtocol.GCManaSend(lpObj->m_Index, lpObj->MaxMana + lpObj->AddMana, -2, 0,
                ...
                case 348:
                    ...
                    break;
                case 349:
                    if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[0].m_TwoHand == FALSE &&
                        lpObj->pInventory[0].m_Type >= ITEMGET(0, 0) && lpObj->pInventory[0].m_Type <= ITEMGET(0, 31)) {
                        lpObj->m_AttackDamageMinRight += this->GetEffectValue(lpObj, 349);
                        lpObj->m_AttackDamageMaxRight += this->GetEffectValue(lpObj, 349);
                    } else if (lpObj->pInventory[1].IsItem() == TRUE && lpObj->pInventory[1].m_TwoHand == FALSE &&
                               lpObj->pInventory[1].m_Type >= ITEMGET(0, 0) &&
                               lpObj->pInventory[1].m_Type <= ITEMGET(0, 31)) {
                ...
                case 350:
                    if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[0].m_TwoHand == TRUE &&
                        lpObj->pInventory[0].m_Type >= ITEMGET(2, 0) && lpObj->pInventory[0].m_Type <= ITEMGET(2, 7)) {
                        lpObj->m_AttackDamageMinRight += this->GetEffectValue(lpObj, 350);
                        lpObj->m_AttackDamageMaxRight += this->GetEffectValue(lpObj, 350);
                    } else if (lpObj->pInventory[1].IsItem() == TRUE && lpObj->pInventory[1].m_TwoHand == TRUE &&
                               lpObj->pInventory[1].m_Type >= ITEMGET(2, 0) &&
                               lpObj->pInventory[1].m_Type <= ITEMGET(2, 7)) {
                        lpObj->m_AttackDamageMinLeft += this->GetEffectValue(lpObj, 350);
                        lpObj->m_AttackDamageMaxLeft += this->GetEffectValue(lpObj, 350);
                    } else if (lpObj->pInventory[1].IsItem() == TRUE && lpObj->pInventory[1].m_TwoHand == TRUE &&
                               lpObj->pInventory[1].m_Type == ITEMGET(2, 20)) {
                        lpObj->m_AttackDamageMinLeft += this->GetEffectValue(lpObj, 350);
                        lpObj->m_AttackDamageMaxLeft += this->GetEffectValue(lpObj, 350);
                    } else if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[1].m_TwoHand == TRUE &&
                               lpObj->pInventory[1].m_Type == ITEMGET(2, 20)) {
                        lpObj->m_AttackDamageMinLeft += this->GetEffectValue(lpObj, 350);
                        lpObj->m_AttackDamageMaxLeft += this->GetEffectValue(lpObj, 350);
                    }
                ...
                case 351:
                    if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[0].m_TwoHand == TRUE &&
                        lpObj->pInventory[0].m_Type >= ITEMGET(3, 0) && lpObj->pInventory[0].m_Type <= ITEMGET(3, 12)) {
                ...
                case 405:
                    ...
                    break;
                case 435:
                    if (lpObj->pInventory[1].IsItem() == TRUE && lpObj->pInventory[1].m_Type >= ITEMGET(4, 0) &&
                        lpObj->pInventory[1].m_Type < ITEMGET(5, 0)) {
                        lpObj->m_AttackDamageMinLeft += this->GetEffectValue(lpObj, 435);
                        lpObj->m_AttackDamageMaxLeft += this->GetEffectValue(lpObj, 435);
                    }
                    break;
                case 436:
                    if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[0].m_Type >= ITEMGET(4, 0) &&
                        lpObj->pInventory[0].m_Type < ITEMGET(5, 0)) {
                ...
                case 457:
                    ...
                    break;
                case 465:
                    if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[0].m_Type >= ITEMGET(5, 0) &&
                        lpObj->pInventory[0].m_Type < ITEMGET(6, 0) &&
                        lpObj->pInventory[0].m_RequireClass[0] == FALSE) {
                        lpObj->m_MagicDamageMin += this->GetEffectValue(lpObj, 465);
                        lpObj->m_MagicDamageMax += this->GetEffectValue(lpObj, 465);
                    }
                    break;
                case 466:
                    if (lpObj->pInventory[1].IsItem() == TRUE && lpObj->pInventory[1].m_Type >= ITEMGET(5, 0) &&
                        lpObj->pInventory[1].m_Type < ITEMGET(6, 0) &&
                        lpObj->pInventory[1].m_RequireClass[0] == FALSE) {
                ...
                case 513:
                    ...
                    break;
                case 568:
                    if (lpObj->pInventory[0].IsItem() == TRUE && lpObj->pInventory[0].m_Type >= ITEMGET(0, 32) &&
                        lpObj->pInventory[0].m_Type <= ITEMGET(0, 35)) {
                        lpObj->m_AttackDamageMinRight += this->GetEffectValue(lpObj, 568);
                        lpObj->m_AttackDamageMaxRight += this->GetEffectValue(lpObj, 568);
                    } else if (lpObj->pInventory[1].IsItem() == TRUE && lpObj->pInventory[1].m_Type >= ITEMGET(0, 32) &&
                               lpObj->pInventory[1].m_Type <= ITEMGET(0, 35)) {
    ...
    for (int i = 0; i < 120; i++) {
        ...
        if (MagicDamageC.GetMasterSkillType(type) >= 3) {
            if (MagicDamageC.SkillGetRequireClass(lpObj->Class, lpObj->m_PlayerData->ChangeUP, type) == FALSE) {
                g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Purple,
                           "[Mastering] WARNING: Invalid skill(%d) for class (%d)(%s)(%s)", type, lpObj->Class,
            ...
            if (MagicDamageC.GetMasterSkillType(type) == 4) {
                if (gObjGetMagicSearch(lpObj, type) == NULL) {
                    ...
                    g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Purple,
                               "[Mastering] WARNING: Active skill(%d) not found in character skill -> added to MagicInfo",
                               type);
                }
            }
...
void CMasterSkillSystemS6::DecreasePoint(LPOBJ lpObj, int MasterPoint) {
    ...
    if (lpObj->m_PlayerData->MasterPoint < 0) {
        ...
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red,
                   "[Mastering] (%s)(%s) ERROR: lpObj->MasterPoint < 0 -> reset to 0",
                   lpObj->AccountID, lpObj->Name);
    }
}
...
void CMasterSkillSystemS6::ApplyActiveSkillEffect(LPOBJ lpObj, WORD skill) {
    ...
    if (skill < 300) {
        g_Log.AddC(0, __FILE__, __FUNCTION__, TColor::Red, "[Mastering] (%s)(%s) ERROR: skill is not MASTER",
                   lpObj->AccountID, lpObj->Name);
        return;
    }
}
...
int CMasterSkillSystemS6::MLS_CheckExistSkill(LPOBJ lpObj, WORD Skill) {
    for (int i = 0; i < MAX_MAGIC; i++) {
        if (lpObj->Magic[i].IsMagic() == TRUE) {
            if (MagicDamageC.CheckMasterSkill(lpObj->Magic[i].m_Skill) == TRUE &&
                MagicDamageC.GetBrandOfSkill(lpObj->Magic[i].m_Skill) == Skill) {
                return TRUE;
            }
        }
    }
    ...
    for (int n = 0; n < MAGIC_SIZE; n++) {
        if (lpObj->Magic[n].IsMagic() == 1) {
            if (lpObj->Magic[n].m_Skill == Skill) {
                g_Log.Add(0, __FILE__, __FUNCTION__, "Same skill already exists. %d %d", lpObj->Magic[n].m_Skill,
                          Skill);
                return -1;
            }
        }
...
BOOL CMasterSkillSystemS6::UseMasterSkill(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos,
                                          int aTargetIndex) {
...
BOOL CMasterSkillSystemS6::MLS_SkillBlowOfFury(int aIndex, CMagicInf *lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos,
...
BOOL
CMasterSkillSystemS6::MLS_SkillSwordOfDestructionStr(int aIndex, CMagicInf *lpMagic, BYTE X, BYTE Y, int aTargetIndex,
                                                     BOOL bCombo) {

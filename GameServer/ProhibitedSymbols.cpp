...
void CProhibitedSymbols::LoadSymbolFile(LPSTR szFile) {
    ...
    for (pugi::xml_node restrictsymbol = symbol_section.child(
            "Restrict"); restrictsymbol; restrictsymbol = restrictsymbol.next_sibling()) {
...
bool CProhibitedSymbols::Validate(char *string, int len, BYTE Type) {
    ...
    if (this->m_nMode == 0) {
        for (int i = 0; i < wstr.length(); i++) {
            std::map<wchar_t, _PROSYM_STRUCT>::iterator it = this->m_ProhibitedSymbols.find(
                    wstr.at(i)); // this is a problem right? yes xd

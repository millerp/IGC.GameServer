    ...
    while (true) {
        ...
        if (type == 0) {
            ...
            while (true) {
                ...
                if (iCount > 7) {
                    g_Log.MsgBox(
                            "ERROR CIllusionTempleLeagueEvent::LoadIllusionTempleScript Cannot over 7 leaguedays i:%d",
                            iCount);
                    return;
                }
            }
...
BOOL CIllusionTempleLeagueEvent::Enter_ITL(int aIndex, BYTE byTempleIndex, BYTE bySlotNum) {
    ...
    if (this->m_cIllusionTempleLeagueProc[byTempleIndex].EnterUserIllusionTempleLeague(aIndex, byTempleIndex,
                                                                                       lpObj->m_PlayerData->GuildNumber,
                                                                                       lpObj->PartyNumber) == TRUE) {
...
void CIllusionTempleLeagueEvent::ActRelicsGetOrRegister(OBJECTSTRUCT *lpNpc, OBJECTSTRUCT *lpObj, BYTE byMapNumber,
                                                        BYTE byPrePacket, BYTE byAct, WORD wNpcType) {
...
void CIllusionTempleLeagueEvent::AddNpcPosNum() {
    for (int i = 0; i < 22; i++) {
        if (this->m_ITL_NPC[i].byMapNumber == MAP_INDEX_ILLUSIONTEMPLE7) {
            this->m_cIllusionTempleLeagueProc[0].AddITLNpcInfo(this->m_ITL_NPC[i].wNpcType,
                                                               this->m_ITL_NPC[i].byMapNumber,
                                                               this->m_ITL_NPC[i].byDistance, this->m_ITL_NPC[i].byX,
                                                               this->m_ITL_NPC[i].byY, this->m_ITL_NPC[i].byDir);
        } else if (this->m_ITL_NPC[i].byMapNumber == MAP_INDEX_ILLUSIONTEMPLE8) {
            this->m_cIllusionTempleLeagueProc[1].AddITLNpcInfo(this->m_ITL_NPC[i].wNpcType,
                                                               this->m_ITL_NPC[i].byMapNumber,
                                                               this->m_ITL_NPC[i].byDistance, this->m_ITL_NPC[i].byX,
                                                               this->m_ITL_NPC[i].byY, this->m_ITL_NPC[i].byDir);
        }
...
void CIllusionTempleLeagueEvent::ITL_UseSkill(int iIndex, WORD MagicNumber, int wTargetObjIndex, BYTE btDis) {
    ...
    this->m_cIllusionTempleLeagueProc[gObj[iIndex].MapNumber - 98].ITL_UseSkill(iIndex, MagicNumber, wTargetObjIndex,
                                                                                btDis);
}
...
bool CIllusionTempleLeagueEvent::IsCanEnterTiming() {
    return this->m_cIllusionTempleLeagueProc[0].GetIllusionTempleLeagueState() == 0 &&
           this->m_cIllusionTempleLeagueProc[0].GetEntrance() == true;
}
...
BYTE CIllusionTempleLeagueEvent::Update_GuildInfo(int nGuildIdx, BYTE byMapNumber, BYTE bySlotNumber, int nPartyIdx,
                                                  BYTE byITLType, int nUserIndex) {
    return this->m_cIllusionTempleLeagueProc[byMapNumber].SetGuild(nGuildIdx, bySlotNumber, byITLType, nPartyIdx,
                                                                   nUserIndex);
}
...
void CIllusionTempleLeagueEvent::DG_ITL_Tournament_Get(_tagPMSG_ANS_ITL_TOURNAMENT *lpMsg) {
    ...
    for (int i = 0; i < lpMsg->byGuildCount; i++) {
        if (lpMsg->m_GuildTournamentRank[i].byRank <= 0) {
            ...
            memcpy(this->m_GuildTournamentRank[i].szGuildName, lpMsg->m_GuildTournamentRank[i].szGuildName,
                   MAX_GUILD_LEN + 1);
...
void CIllusionTempleLeagueEvent::DG_ITL_RewardList_Get(_tagPMSG_ANS_ITL_REWARDLIST *lpMsg) {
    ...
    for (int i = 0; i < 5; i++) {
        if (lpMsg->m_stRewardList[i].byRank > 0 && lpMsg->m_stRewardList[i].byRank < 6) {
            ...
            memcpy(this->m_ITL_Winner_RewardList[i].szCharName, lpMsg->m_stRewardList[i].szCharName,
                   MAX_ACCOUNT_LEN + 1);
            memcpy(this->m_ITL_Winner_RewardList[i].szGuildName, lpMsg->m_stRewardList[i].szGuildName,
                   MAX_GUILD_LEN + 1);
...
BYTE CIllusionTempleLeagueEvent::Find_MyPartyAndGuildInfo(BYTE byMapNumber, BYTE byArrayOrder, int GuildIndex,
                                                          BYTE &byTeamIndex, int &nPartyIdx) {
...
BYTE
CIllusionTempleLeagueEvent::ReEnterITL(int aIndex, BYTE btTempleIndex, int nGuildIdx, int nPartyIdx, int nUserArrayNo,
                                       int nITLState, BYTE byTeamIndex) {
    return this->m_cIllusionTempleLeagueProc[btTempleIndex].ReEnterUserIllusionTempleLeague(aIndex, btTempleIndex,
                                                                                            nGuildIdx, nPartyIdx,
                                                                                            nUserArrayNo, byTeamIndex);
}

void
CIllusionTempleLeagueEvent::Insert_RewardList(char *Name, char *GuildName, BYTE byRank, BYTE byPoint, BYTE byEnter) {
    for (int i = 0; i < 5; i++) {
...
void CIllusionTempleLeagueEvent::MakeITLRewardItem_CCFWing(int iUserIndex) {
    ItemSerialCreateSend(iUserIndex, gObj[iUserIndex].MapNumber, gObj[iUserIndex].X, gObj[iUserIndex].Y,
                         ITEMGET(12, 266), 0, 0, 0, 1, 4, iUserIndex, 7, 0, 2419200, 0, 0);

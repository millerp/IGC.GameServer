    ...
    if (lpUserObj->MapNumber == MAP_INDEX_CASTLESIEGE &&
        ...
        if (lpUserObj->Y == g_CastleSiege.GetCrownAccessUserY() &&
            ...
            if (gObjIsConnected(g_CastleSiege.GetCrownUserIndex()) &&
                ...
                if (gObj[g_CastleSiege.GetCrownUserIndex()].m_btCsJoinSide ==
                    gObj[g_CastleSiege.GetCrownSwitchUserIndex(217)].m_btCsJoinSide &&
                    gObj[g_CastleSiege.GetCrownUserIndex()].m_btCsJoinSide ==
                    gObj[g_CastleSiege.GetCrownSwitchUserIndex(218)].m_btCsJoinSide)    // #warning ??? maybe is 218???
                {
                    DWORD dwTime = GetTickCount() - g_CastleSiege.GetCrownAccessTickCount();

                    g_Log.Add(
                            "[CastleSiege] [Reg. Accumulating] Accumulated Crown AccessTime : acc(%d) + %d [%s](%s)(%s)",

...
bool CAcheronGuardianEvent::LoadScriptAcheronEvent(char *lpszFileName) {
    ...
    this->m_bAEEnable =
            g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_ACHERON) == true
            ? main_section.attribute("Enable").as_bool() : false;
    ...
    for (pugi::xml_node monster = monstergroupmember.child("Monster"); monster; monster = monster.next_sibling()) {
        ...
        this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nMonsterIndex = monster.attribute(
                "Index").as_int();
        this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nGenSqe = monster.attribute(
                "SpawnSeq").as_int();
        this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[nArrayIndex].m_nMonAttr = monster.attribute(
                "Element").as_int();
        ...
        if (this->m_iDayOfWeekCycle[wDayOfWeek] == TRUE) {
            if (this->m_vtAcheronEventTime.empty() == true) {
                g_Log.AddC(TColor::Red,
                           "[AcheronEvent] No Schedule Time Data - Acheron Event terminated (m_vtArcaBattleOpenTime.empty())");
                return false;
            }

            GetLocalTime(&systime);

            for (std::vector<_tagAcheronEventTime>::iterator It = this->m_vtAcheronEventTime.begin();
                 It != this->m_vtAcheronEventTime.end(); It++) {
                pRET = *It;
            ...
            if (bTIME_CHANGED == TRUE) {
                this->m_i64_REMAIN_MSEC = 1000 * (60 * iMIN_MINUTE + 3600 * iMIN_HOUR -
                                                  (systime.wSecond + 60 * systime.wMinute + 3600 * systime.wHour));
                this->SetProcState(2);
                this->m_i64_TICK_COUNT = GetTickCount64();

                g_Log.Add("[AcheronEvent] Sync Start Time. [%I64d] min remain (START HOUR:%d, MIN:%d)",
                          this->m_i64_REMAIN_MSEC / 60000,
                ...
                g_Log.AddC(TColor::Red,
                           "[AcheronEvent] No Schedule Time Data - Acheron Event Terminated (bTIME_CHANGED=%d)",
                           bTIME_CHANGED);
                return false;
            }
        } else {
...
void CAcheronGuardianEvent::ResetGenTime() {
    for (int nGroupNumber = 0; nGroupNumber < 5; nGroupNumber++) {
        if (this->m_stMonsterGroupInfo[nGroupNumber].m_nGroupNumber >= 0) {
            ...
            this->m_stMonsterGroupInfo[nGroupNumber].m_i64_NextRegenTime =
                    GetTickCount64() + 60000 * this->m_stMonsterGroupInfo[nGroupNumber].m_i64_RegenTimeMin;
        }
    }
}
...
void CAcheronGuardianEvent::RegenMonsterRun() {
    ...
    if (this->m_stMonsterGroupInfo[this->m_iGroupNum].m_nGroupNumber > -1) {
        if (dwTick >= this->m_stMonsterGroupInfo[this->m_iGroupNum].m_i64_NextRegenTime) {
            ...
            this->m_stMonsterGroupInfo[this->m_iGroupNum].m_i64_NextRegenTime =
                    GetTickCount64() + 60000 * this->m_stMonsterGroupInfo[this->m_iGroupNum].m_i64_RegenTimeMin;
        }
    }
}
    ...
    for (int n = 0; n < nMonsterCount; n++) {
        ...
        for (int i = 0; i < nMonsterKindCnt; i++) {
            if (this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[i].m_nGenSqe == iGenSeq
                && this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[i].m_nMonAttr ==
                   this->m_stObeliskInfo[this->m_iGroupNum].m_iAttrKind) {
        ...
        if (nResult < 0) {
            g_Log.Add("[AcheronGuardianEvent][MonsterGroupRegen] AddMonster fail!! [GROUP]:%d, [INDEX]:%d",

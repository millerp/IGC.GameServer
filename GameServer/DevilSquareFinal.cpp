            ...
            for (int i = 0; i < 25; i++) {
                if (nDSFType == this->m_DSFStartHourType[i].DSFBattleType) {
                    ...
                    if (this->m_nDSFTestType > 0) {
                        ...
                        nBattleStartTime = 60000 * this->m_DSFStartHourType[i].DSFMin +
                                           3600000 * this->m_DSFStartHourType[i].DSFHour;
        ...
        if (this->m_nCurBattleStartTime >= 0) {
            if (this->m_nNextTimeMin < nCurTime) {
                ...
                if (this->m_nRemainTimeMin <= this->m_DSFTimeRuleInfo.BeforeEnterMinute && this->m_nRemainTimeMin > 0) {
                    PMSG_NOTICE pNotice;
                    TNotice::MakeNoticeMsgEx(&pNotice, 0, "Tormented Square will close doors after %d minutes",
                                             this->m_nRemainTimeMin);
            ...
            if (iTICK_MSEC >= 1000) {
                ...
                if (this->m_nRemainTimeMin <= this->m_DSFTimeRuleInfo.BeforeEnterMinute &&
            ...
            if (this->m_nRemainTimeMin <= 0 && this->m_nRemainTimeMsec <= 0) {
                ...
                if (bCanStart == TRUE && this->m_nUserCount >= this->m_DSFUnitInfo.MinUserCount) {
                    for (int j = 0; j < 20; j++) {
                        if (this->m_UserData[j].m_nIndex != -1 &&
                            ...
                            g_Log.Add("[DSF][Proc_DSFState_None][%s][%s][%s][%d] Not PartyUser Kick Out",
                                      gObj[this->m_UserData[j].m_nIndex].AccountID,
                                      gObj[this->m_UserData[j].m_nIndex].m_PlayerData->m_RealNameOfUBF,
                                      gObj[this->m_UserData[j].m_nIndex].Name,
                                      gObj[this->m_UserData[j].m_nIndex].m_PlayerData->m_nServerCodeOfHomeWorld);
                        }
                    }
                    ...
                    g_Log.Add(
                            "[DSF][Proc_DSFState_None] Failed to Start DSF (UserCount: %d) (m_nRemainTimeMin:%d)(m_nRemainTimeMsec:%d)",
...
void CDevilSquareFinal::Proc_DSFState_End(int nCurTime) {
    ...
    if (this->m_nRemainTimeMsec <= 1000 * this->m_DSFTimeRuleInfo.AfterRoundSecond - 3000 &&
        this->m_nRemainTimeMsec > 0 && this->m_bResultPointSend == false) {
        this->SendDSFResult(-1);
        this->SaveWinnerPartyPoint();
...
void CDevilSquareFinal::Set_DSFState_Ready() {
    ...
    for (int i = 0; i < 10; i++) {
        if (this->m_PartyDataSave[i].nPartyNo != -1) {
            ...
            memcpy(PartyUserName.szUserName1,
                   gObj[this->m_PartyDataSave[i].nRequestUserIndex].m_PlayerData->m_RealNameOfUBF, MAX_ACCOUNT_LEN + 1);
            memcpy(PartyUserName.szUserName2,
                   gObj[this->m_PartyDataSave[i].n2ndPartyUserIndex].m_PlayerData->m_RealNameOfUBF,
                   MAX_ACCOUNT_LEN + 1);
            PartyUserName.iTeamLevel =
                    this->m_PartyDataSave[i].nRequestUserLevel + this->m_PartyDataSave[i].n2ndPartyUserLevel;
...
char CDevilSquareFinal::SetParty(BYTE btSlotOrder, BYTE btDSFType, int nPartyIdx, int nUserIndex) {
    ...
    memcpy(this->m_PartyDataSave[btSlotOrder].szRequestUserRealName, gObj[nUserIndex].m_PlayerData->m_RealNameOfUBF,
           MAX_ACCOUNT_LEN + 1);
    ...
    this->m_PartyDataSave[btSlotOrder].nRequestUserLevel =
            gObj[nUserIndex].m_PlayerData->MasterLevel + gObj[nUserIndex].Level;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (useridx > 0 && nUserIndex != useridx) {
            ...
            memcpy(this->m_PartyDataSave[btSlotOrder].sz2ndPartyUserRealName,
                   gObj[useridx].m_PlayerData->m_RealNameOfUBF, MAX_ACCOUNT_LEN + 1);
            ...
            this->m_PartyDataSave[btSlotOrder].n2ndPartyUserLevel =
                    gObj[useridx].m_PlayerData->MasterLevel + gObj[useridx].Level;
        }
    }
        ...
        if (this->m_UserData[i].m_nIndex == -1) {
            if (lpObj->PartyNumber == this->m_PartyDataSave[btSlotNum].nPartyNo) {
                ...
                g_Log.Add("[DSF][Enter_DSF][%s][%s][%s][%d] UserCnt:[%d], DSFIndex:[%d], Team:%d",
                          ...
                          gObj[aIndex].m_PlayerData->m_nServerCodeOfHomeWorld, this->m_nUserCount,
                          gObj[aIndex].m_nDSFIndex,
    ...
    if (this->m_bGameStart == false) {
        for (int i = 0; i < 10; i++) {
            if (this->m_UserData[gObj[aIndex].m_nDSFIndex].m_btTeam == this->m_PartyDataSave[i].btTeamIndex &&
                nUserCount == 0) {
                g_Log.Add(
                        "[DSF][Leave_DSF][%s][%s][%s][%d] All PartyUser Leave : Game is not start, TeamIndex:[%d], PartyUserName1:%s, PartyUserName2:%s",
                        gObj[aIndex].AccountID, gObj[aIndex].m_PlayerData->m_RealNameOfUBF, gObj[aIndex].Name,
                        gObj[aIndex].m_PlayerData->m_nServerCodeOfHomeWorld,
                        this->m_PartyDataSave[i].btTeamIndex, this->m_PartyDataSave[i].szRequestUserRealName,
                        this->m_PartyDataSave[i].sz2ndPartyUserRealName);
...
void CDevilSquareFinal::Send_DSF_PlayState(BYTE btTeam) {
    ...
    for (int i = 0; i < 20; i++) {
        if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED &&
            this->m_UserData[i].m_btTeam == btTeam) {
...
bool CDevilSquareFinal::CanStartDSFBattle() {
    ...
    g_Log.Add("[DSF][CanStartDSFBattle] Result %s nCanEnterPartyCount:%d", bCanStart == TRUE ? "TRUE" : "FALSE",
              nCanEnterPartyCount);
    return bCanStart;
}
        ...
        if (this->m_PartyDataSave[i].nPartyNo != -1) {
            switch (i) {
                case 0:
                    ...
                    n1stLevel =
                            this->m_PartyDataSave[0].n2ndPartyUserLevel + this->m_PartyDataSave[0].nRequestUserLevel;
                    break;
                case 1:
                    n2ndPoint = this->m_PartyDataSave[1].nPoint;
                    n2ndTeam = this->m_PartyDataSave[1].btTeamIndex;
                    n2ndLevel =
                            this->m_PartyDataSave[1].n2ndPartyUserLevel + this->m_PartyDataSave[1].nRequestUserLevel;
                    ...
                    if (n1stPoint >= n2ndPoint) {
                        if (n1stPoint == n2ndPoint && n1stLevel > n2ndLevel) {
                            ...
                            n1stLevel = this->m_PartyDataSave[1].n2ndPartyUserLevel +
                                        this->m_PartyDataSave[1].nRequestUserLevel;
                ...
                case 2:
                    ...
                    n3rdLevel =
                            this->m_PartyDataSave[2].n2ndPartyUserLevel + this->m_PartyDataSave[2].nRequestUserLevel;
                    ...
                    if (n1stPoint >= n3rdPoint) {
                        if (n1stPoint == n3rdPoint) {
                            if (n1stLevel <= n3rdLevel) {
                                ...
                                n2ndLevel = this->m_PartyDataSave[2].n2ndPartyUserLevel +
                                            this->m_PartyDataSave[2].nRequestUserLevel;
                                ...
                                n2ndTeam = nTempTeam;
                                nTempLevel = n1stLevel;
                                n1stLevel = this->m_PartyDataSave[2].n2ndPartyUserLevel +
                                            this->m_PartyDataSave[2].nRequestUserLevel;
                                n3rdLevel = n2ndLevel;
                                n2ndLevel = nTempLevel;
                            }
                ...
                case 3:
                    ...
                    n4thLevel =
                            this->m_PartyDataSave[3].n2ndPartyUserLevel + this->m_PartyDataSave[3].nRequestUserLevel;
                    ...
                    if (n1stPoint >= n4thPoint) {
                        if (n1stPoint == n4thPoint) {
                            if (n1stLevel <= n4thPoint) {
                                ...
                                n3rdTeam = nTempTeam;
                                nTempLevel = n2ndLevel;
                                n2ndLevel = this->m_PartyDataSave[3].n2ndPartyUserLevel +
                                            this->m_PartyDataSave[3].nRequestUserLevel;
                                n4thLevel = n3rdLevel;
                                n3rdLevel = nTempLevel;
                            } else {
                                nTempPoint = n1stPoint;
                                ...
                                n2ndTeam = nTempTeam;
                                nTempLevel = n1stLevel;
                                n1stLevel = this->m_PartyDataSave[3].n2ndPartyUserLevel +
                                            this->m_PartyDataSave[3].nRequestUserLevel;
                                n4thLevel = n3rdLevel;
                                n3rdLevel = n2ndLevel;
                                n2ndLevel = nTempLevel;
                            ...
                            if (n2ndPoint == n4thPoint) {
                                if (n2ndLevel <= n4thPoint) {
                                    ...
                                    n3rdLevel = this->m_PartyDataSave[3].n2ndPartyUserLevel +
                                                this->m_PartyDataSave[3].nRequestUserLevel;
                ...
                default:
                    ...
                    nNewLevel =
                            this->m_PartyDataSave[i].n2ndPartyUserLevel + this->m_PartyDataSave[i].nRequestUserLevel;
                    ...
                    if (n1stPoint >= nNewPoint) {
                        if (n1stPoint == nNewPoint) {
                            if (n1stLevel <= nNewLevel) {
                                ...
                                n3rdTeam = nTempTeam;
                                nTempLevel = n2ndLevel;
                                n2ndLevel = this->m_PartyDataSave[i].n2ndPartyUserLevel +
                                            this->m_PartyDataSave[i].nRequestUserLevel;
                                n4thLevel = n3rdLevel;
                                n3rdLevel = nTempLevel;
                            } else {
                                nTempPoint = n1stPoint;
                                ...
                                n2ndTeam = nTempTeam;
                                nTempLevel = n1stLevel;
                                n1stLevel = this->m_PartyDataSave[i].n2ndPartyUserLevel +
                                            this->m_PartyDataSave[i].nRequestUserLevel;
                                n4thLevel = n3rdLevel;
                                n3rdLevel = n2ndLevel;
                                n2ndLevel = nTempLevel;
                            ...
                            if (n2ndPoint == nNewPoint) {
                                if (n2ndLevel <= nNewLevel) {
                                    ...
                                    n4thTeam = nTempTeam;
                                    nTempLevel = n3rdLevel;
                                    n3rdLevel = this->m_PartyDataSave[i].n2ndPartyUserLevel +
                                                this->m_PartyDataSave[i].nRequestUserLevel;
                                    n4thLevel = nTempLevel;
                                } else {
                                    nTempPoint = n2ndPoint;
                                ...
                                if (n3rdPoint == nNewPoint) {
                                    if (n3rdLevel <= nNewLevel) {
                                        ...
                                        n4thLevel = this->m_PartyDataSave[i].n2ndPartyUserLevel +
                                                    this->m_PartyDataSave[i].nRequestUserLevel;
                                    } else {
                                        nTempPoint = n3rdPoint;
                                        n3rdPoint = this->m_PartyDataSave[i].nPoint;
    ...
    for (int j = 0; j < 10; j++) {
        if (this->m_nDSF_TYPE == 1) {
            if ((this->m_PartyDataSave[j].btTeamIndex == n1stTeam || this->m_PartyDataSave[j].btTeamIndex == n2ndTeam ||
                 this->m_PartyDataSave[j].btTeamIndex == n3rdTeam ||
                 this->m_PartyDataSave[j].btTeamIndex == n4thTeam) &&
                ...
                g_Log.Add(
                        "[DSF][SaveWinnerPartyPoint] - [%s][%s][%s][%d][%d][%d] / [%s][%s][%s][%d][%d][%d] DSF League Result - DSFType:%d, Point:%d, TeamIndex:%d",
            ...
            if ((this->m_PartyDataSave[j].btTeamIndex == n1stTeam ||
                 this->m_PartyDataSave[j].btTeamIndex == n2ndTeam) &&
                this->m_PartyDataSave[j].nPoint > 0) {
                this->GDSaveDSFPartyPoint(this->m_PartyDataSave[j].szRequestUserID,
                                          this->m_PartyDataSave[j].szRequestUserName,
                                          this->m_PartyDataSave[j].nRequestUserLevel,
                ...
                g_Log.Add(
                        "[DSF][SaveWinnerPartyPoint] - [%s][%s][%s][%d][%d][%d] / [%s][%s][%s][%d][%d][%d] DSF SemiFinal Result - DSFType:%d, Point:%d, TeamIndex:%d",
                        this->m_PartyDataSave[j].szRequestUserID,
                        this->m_PartyDataSave[j].szRequestUserRealName,
                        this->m_PartyDataSave[j].szRequestUserName,
                        this->m_PartyDataSave[j].nRequestUserServerCode,
            ...
            if (this->m_PartyDataSave[j].btTeamIndex == n1stTeam && this->m_PartyDataSave[j].nPoint > 0) {
                ...
                g_Log.Add(
                        "[DSF][SaveWinnerPartyPoint] - [%s][%s][%s][%d][%d][%d] / [%s][%s][%s][%d][%d][%d] DSF Final Result - DSFType:%d, Point:%d, TeamIndex:%d",
                        this->m_PartyDataSave[j].szRequestUserID,
                        this->m_PartyDataSave[j].szRequestUserRealName,
                        this->m_PartyDataSave[j].szRequestUserName,
                        this->m_PartyDataSave[j].nRequestUserServerCode,
...
void CDevilSquareFinal::GDSaveDSFPartyPoint(char *szAccountID1, char *szUserName1, int nUser1Level, int nClass1,
                                            char *szAccountID2, char *szUserName2, int nUser2Level, int nClass2,
                                            int iDSFType, int iPoint, BYTE btEnterCnt) {
    ...
    g_Log.Add(
            "[DSF][GDSaveDSFPartyPoint] AccountID1:[%s], UserName1:[%s], User1Level:[%d], AccountID2:[%s], UserName2:[%s], User2Level:[%d], DSFType:[%d], Point:[%d]",
...
void CDevilSquareFinal::DSFDropReward(OBJECTSTRUCT *lpUser, OBJECTSTRUCT *lpMonster) {
    ...
    if (rand() % 10000 < this->m_DSFUnitInfo.MonsterRewardRate[this->m_nDSF_TYPE - 1]) {
        ItemSerialCreateSend(lpUser->m_Index, lpUser->MapNumber, lpUser->X, lpUser->Y, wItemCode, 0, 0, 0, 0, 0,
                             lpUser->m_Index, 0, 0, 0, 0, 0);
    }
}
...
void CDevilSquareFinal::GDReqSetDSFReward_UBF(int iUserIndex, BYTE btDSFType, BYTE btRewardType) {
    ...
    g_Log.Add(
            "[UBF][DevilSquareFinal][GDReqSetDSFWinnerInformation]ACC:%s , NAME :%s,Real NAME :%s, DSFType:%d, RewardType:%d",

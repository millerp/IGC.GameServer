...
int CCastleSiege::LoadPreFixData(char *lpszFileName) {
    ...
    int iTODAY_DATE_NUM =
            MACRO2((MACRO1(tmToDay.wDay) | MACRO1(tmToDay.wMonth) << 8) & 0xFFFF) | MACRO2(tmToDay.wYear) << 16;
    __int64 iTODAY_DATE_NUM64 = (MACRO2(tmToDay.wMinute) | MACRO2(tmToDay.wHour) << 16) | ((__int64)(
            MACRO2((MACRO1(tmToDay.wDay) | MACRO1(tmToDay.wMonth) << 8) & 0xFFFF) | MACRO2(tmToDay.wYear) << 16)
            << 0x20);
    ...
    if (this->m_tmFixCastleCycleStartDate.wYear == 0 || this->m_tmFixCastleCycleStartDate.wMonth == 0 ||
        this->m_tmFixCastleCycleStartDate.wDay == 0) {
        this->m_bFixCastleCycleStartDate = FALSE;
    } else {
        _SYSTEMTIME tmEndDate = this->m_tmFixCastleCycleStartDate;
        GetNextDay(&tmEndDate, this->m_iCastleSiegeCycle, 0, 0, 0);

        int iEVENT_END_DATE_NUM = MACRO2((MACRO1(tmEndDate.wDay) | MACRO1(tmEndDate.wMonth) << 8) & 0xFFFF) |
                                  MACRO2(tmEndDate.wYear) << 16;

        if (iEVENT_END_DATE_NUM <= iTODAY_DATE_NUM) {
            this->m_bFixCastleCycleStartDate = FALSE;
            g_Log.MsgBox(
                    "[CastleSiege] CCastleSiege::LoadPreFixData() - iEVENT_END_DATE_NUM (%04d-%02d-%02d) <= iTODAY_DATE_NUM (%04d-%02d-%02d)",
        ...
        if (MONTH_MACRO(this->m_tmFixCastleCycleStartDate.wMonth) == FALSE ||
            DAY_MACRO(this->m_tmFixCastleCycleStartDate.wDay) == FALSE) {
            this->m_bFixCastleCycleStartDate = FALSE;
        }
    }

    if (this->m_tmFixCastleStateStartDate.wYear == 0 || this->m_tmFixCastleStateStartDate.wMonth == 0 ||
        this->m_tmFixCastleStateStartDate.wDay == 0) {
        this->m_bFixCastleStateStartDate = FALSE;
    } else if (this->m_iFixCastleSpecificState < CASTLESIEGE_STATE_REGSIEGE ||
               this->m_iFixCastleSpecificState > CASTLESIEGE_STATE_ENDCYCLE) {
        this->m_bFixCastleStateStartDate = FALSE;
    } else {
        __int64 iSIEGE_DATE_NUM64 =
                (MACRO2(m_tmFixCastleStateStartDate.wMinute) | MACRO2(m_tmFixCastleStateStartDate.wHour) << 16) |
                ((__int64)(MACRO2((MACRO1(m_tmFixCastleStateStartDate.wDay) |
                                   MACRO1(m_tmFixCastleStateStartDate.wMonth) << 8) & 0xFFFF) |
                           MACRO2(m_tmFixCastleStateStartDate.wYear) << 16) << 0x20);

        if (iSIEGE_DATE_NUM64 <= iTODAY_DATE_NUM64) {
            this->m_bFixCastleStateStartDate = FALSE;
            g_Log.MsgBox(
                    "[CastleSiege] CCastleSiege::LoadPreFixData() - iSIEGE_DATE_NUM64 (STATE:%d, %04d-%02d-%02d %02d:%02d:%02d) <= iTODAY_DATE_NUM64 (%04d-%02d-%02d %02d:%02d:%02d)",
                    this->m_iFixCastleSpecificState, this->m_tmFixCastleStateStartDate.wYear,
                    this->m_tmFixCastleStateStartDate.wMonth, this->m_tmFixCastleStateStartDate.wDay,
                    this->m_tmFixCastleStateStartDate.wHour, this->m_tmFixCastleStateStartDate.wMinute,
                    this->m_tmFixCastleStateStartDate.wSecond, tmToDay.wYear, tmToDay.wMonth, tmToDay.wDay,
                    tmToDay.wHour, tmToDay.wMinute, tmToDay.wSecond);
        } else {
        ...
        if (this->m_iFixCastleSpecificState == CASTLESIEGE_STATE_ENDSIEGE) {
            g_Log.MsgBox(
                    "[CastleSiege] CCastleSiege::LoadPreFixData() - FIX STATE 'CASTLESIEGE_STATE_ENDSIEGE' CAN'T BE USED");
            this->m_bFixCastleStateStartDate = FALSE;
        }

        if (MONTH_MACRO(this->m_tmFixCastleStateStartDate.wMonth) == FALSE ||
            DAY_MACRO(this->m_tmFixCastleStateStartDate.wDay) == FALSE ||
            HOUR_MACRO(this->m_tmFixCastleStateStartDate.wHour) == FALSE ||
            MINUTE_MACRO(this->m_tmFixCastleStateStartDate.wMinute) == FALSE) {
            this->m_bFixCastleStateStartDate = FALSE;
        }
    }
...
int CCastleSiege::LoadData(char *lpszFileName) {
    ...
    if (this->m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_1) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::LoadData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_1 (%d)",
                   this->m_iCastleDataLoadState);
        return false;
    }
    ...
    if (iCastleSiegeCycle <= 0) {
        g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::LoadData() - iCastleSiegeCycle <= 0 : %d",
                   iCastleSiegeCycle);
        return false;
    } else {
    ...
    for (pugi::xml_node cycle = cyclesettings.child("Cycle"); cycle; cycle = cycle.next_sibling()) {
        ...
        if (!vtScheduleData.empty()) {
            for (std::vector<_CS_SCHEDULE_DATA>::iterator it = vtScheduleData.begin();
                 it != vtScheduleData.end(); it++) {
                _CS_SCHEDULE_DATA pScheduleData = _CS_SCHEDULE_DATA(*it);

                if (pScheduleData.m_bIN_USE != 0) {
                    if (pScheduleData.m_iSTATE == iSTATE) {
                        g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::LoadData() - Same State Exist : %d",
                                   iSTATE);
                        return false;
                    }

                    if (pScheduleData.m_iSTATE > iSTATE) {
                        int iSTATE1_TIME = pScheduleData.m_iADD_DAY * 24 * 60 + pScheduleData.m_iADD_HOUR * 60 +
                                           pScheduleData.m_iADD_MIN;
                        int iSTATE2_TIME = iSTART_DAY * 24 * 60 + iSTART_HOUR * 60 + iSTART_MIN;

                        if (iSTATE1_TIME <= iSTATE2_TIME) {
                            g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong : %d",
                                       iSTATE2_TIME);
                            return false;
                        }
                    } else {
    ...
    for (pugi::xml_node npc = npcsettings.child("NPC"); npc; npc = npc.next_sibling()) {
        ...
        if (it != mpNpcDataCheck.end()) {
            g_Log.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Same Npc Key Exist : NUM:%d-IDX:%d", iNPC_NUM,
                         iNPC_INDEX);
            return false;
        }
    ...
    if (!m_vtScheduleData.empty()) {
        for (int iIDX = 0; iIDX < this->m_vtScheduleData.size(); iIDX++) {
            if (iIDX + 1 < this->m_vtScheduleData.size()) {
                int iIDX_SEC1 = this->m_vtScheduleData[iIDX].m_iADD_DAY * 24 * 60 * 60 +
                                this->m_vtScheduleData[iIDX].m_iADD_HOUR * 60 * 60 +
                                this->m_vtScheduleData[iIDX].m_iADD_MIN * 60;
                int iIDX_SEC2 = this->m_vtScheduleData[iIDX + 1].m_iADD_DAY * 24 * 60 * 60 +
                                this->m_vtScheduleData[iIDX + 1].m_iADD_HOUR * 60 * 60 +
                                this->m_vtScheduleData[iIDX + 1].m_iADD_MIN * 60;

                int iIDX_RESULT = iIDX_SEC2 - iIDX_SEC1;

                if (iIDX_RESULT < 0) {
                    g_Log.AddC(TColor::Red,
                               "[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong (sort fail) : %d-%d",
                               this->m_vtScheduleData[iIDX].m_iSTATE, this->m_vtScheduleData[iIDX + 1].m_iSTATE);
                } else {
    ...
    while (it1 != this->m_vtScheduleData.end()) {
        ...
        char szTEMP[256] = {0};
        wsprintf(szTEMP, "STT:%d, DAY:%d, HOUR:%d, MIN:%d, GAP:%d\n", stTEMP.m_iSTATE, stTEMP.m_iADD_DAY,
                 stTEMP.m_iADD_HOUR, stTEMP.m_iADD_MIN, stTEMP.m_iGAP_SEC);
    ...
    while (it2 != this->m_vtNpcData.end()) {
        ...
        char szTEMP[256] = {0};
        wsprintf(szTEMP,
                 "NUM:%d, IDX:%d, DBSAVE:%s, SIDE:%d, DFLEVEL:%d, RGLEVEL:%d, MXHP:%d, (%d,%d)-(%d,%d), DIR:%d\n",
                 stTEMP.m_iNPC_NUM, stTEMP.m_iNPC_INDEX, (stTEMP.m_bNPC_DBSAVE != 0) ? "YES" : "NO", stTEMP.m_iNPC_SIDE,
                 stTEMP.m_iNPC_DF_LEVEL, stTEMP.m_iNPC_RG_LEVEL, stTEMP.m_iNPC_MAXHP, stTEMP.m_iNPC_SX,
                 stTEMP.m_iNPC_SY, stTEMP.m_iNPC_DX, stTEMP.m_iNPC_DY, stTEMP.m_iNPC_DIR);
        OutputDebugString(szTEMP);
        it2++;
    }
...
int CCastleSiege::DataRequest() {
    ...
    if (this->m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::DataRequest() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",
...
int CCastleSiege::Init() {
    if (this->m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::Init() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4 (%d)",
                   this->m_iCastleDataLoadState);
        g_Log.MsgBox("[CastleSiege] CCastleSiege::Init() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4 (%d)",
                     this->m_iCastleDataLoadState);
        return false;
    }

    if (this->m_bFileDataLoadOK == FALSE || this->m_bDbDataLoadOK == FALSE) {
        g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::Init() - Data Load Fail (FILE:%d, DB:%d)",
                   this->m_bFileDataLoadOK, this->m_bDbDataLoadOK);
        g_Log.MsgBox("[CastleSiege] CCastleSiege::Init() - Data Load Fail (FILE:%d, DB:%d)", this->m_bFileDataLoadOK,
                     this->m_bDbDataLoadOK);
        return false;
    }
...
void CCastleSiege::SetState(int iCastleSiegeState, BOOL bSetRemainMsec) {
    ...
    switch (this->m_iCastleSiegeState) {
        case CASTLESIEGE_STATE_NONE:
            this->SetState_NONE(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_IDLE_1:
            this->SetState_IDLE_1(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_REGSIEGE:
            this->SetState_REGSIEGE(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_IDLE_2:
            this->SetState_IDLE_2(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_REGMARK:
            this->SetState_REGMARK(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_IDLE_3:
            this->SetState_IDLE_3(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_NOTIFY:
            this->SetState_NOTIFY(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_READYSIEGE:
            this->SetState_READYSIEGE(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_STARTSIEGE:
            this->SetState_STARTSIEGE(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_ENDSIEGE:
            this->SetState_ENDSIEGE(bSetRemainMsec);
            break;
        case CASTLESIEGE_STATE_ENDCYCLE:
            this->SetState_ENDCYCLE(bSetRemainMsec);
            break;
    }
...
void CCastleSiege::Run() //Identical
{
    ...
    if ((GetTickCount() - g_iLastSiegeUpdate) > 3000) {
        ...
        if (iLeftSiegeDate >= 0) {
            ...
            g_Log.AddC(TColor::Blue, "LEFT : SEC(%d), SIEGE-DAY(%04d-%02d-%02d %02d:%02d:%02d)", iLeftSiegeDate,
                       tmLeftDate.wYear, tmLeftDate.wMonth, tmLeftDate.wDay, tmLeftDate.wHour, tmLeftDate.wMinute,
                       tmLeftDate.wSecond);
        } else {
...
void CCastleSiege::SetState_ENDCYCLE(BOOL bSetRemainMsec) {
    ...
    g_Log.Add("[CastleSiege] State - End Cycle Schedule : (%d-%d-%d(%d:%d:%d)) (%d-%d-%d(%d:%d:%d))",
              this->m_tmStartDate.wYear, this->m_tmStartDate.wMonth, this->m_tmStartDate.wDay,
              this->m_tmStartDate.wHour, this->m_tmStartDate.wMinute, this->m_tmStartDate.wSecond,
              this->m_tmEndDate.wYear, this->m_tmEndDate.wMonth, this->m_tmEndDate.wDay, this->m_tmEndDate.wHour,
              this->m_tmEndDate.wMinute, this->m_tmEndDate.wSecond);
    g_Log.Add("[CastleSiege] State - End Cycle : Date-Changing Info (%d-%d-%d) (%d-%d-%d)", this->m_tmStartDate.wYear,
              this->m_tmStartDate.wMonth, this->m_tmStartDate.wDay, this->m_tmEndDate.wYear, this->m_tmEndDate.wMonth,
              this->m_tmEndDate.wDay);

    GS_GDReqSiegeDateChange(this->m_iMapSvrGroup, -1, this->m_tmStartDate.wYear, this->m_tmStartDate.wMonth,
                            this->m_tmStartDate.wDay, this->m_tmEndDate.wYear, this->m_tmEndDate.wMonth,
                            this->m_tmEndDate.wDay);
    ...
    if (iTICK_MSEC >= 1000) {
        ...
        if ((GetTickCount() - this->m_dwCS_LEFTTIME_TICK_COUNT) > 180000) {
            if (this->m_iCS_REMAIN_MSEC > 180000) {
                GSProtocol.GCAnsCsLeftTimeAlarm((this->m_iCS_REMAIN_MSEC / 1000) / 3600,
...
BOOL CCastleSiege::FirstCreateDbNPC() {
    ...
    if (this->m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::FirstCreateDbNPC() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4");
        return FALSE;
    }
...
BOOL CCastleSiege::SetCastleInitData(CSP_ANS_CSINITDATA *lpMsg) {
    if (this->m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_3) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::SetCastleInitData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_3");
        return FALSE;
    }
    ...
    int iSTART_DATE_NUM =
            MACRO2((MACRO1(this->m_tmStartDate.wDay) | MACRO1(this->m_tmStartDate.wMonth) << 8) & 0xFFFF) |
            MACRO2(this->m_tmStartDate.wYear) << 16;
    int iEND_DATE_NUM = MACRO2((MACRO1(this->m_tmEndDate.wDay) | MACRO1(this->m_tmEndDate.wMonth) << 8) & 0xFFFF) |
                        MACRO2(this->m_tmEndDate.wYear) << 16;
    ...
    g_Log.Add("[CastleSiege] CCastleSiege::LoadData() - Siege Schedule Start Date (%d-%d-%d)", m_tmStartDate.wYear,
              m_tmStartDate.wMonth, m_tmStartDate.wDay);

    g_Log.Add("[CastleSiege] CCastleSiege::LoadData() - Siege Schedule Date (%d-%d-%d)", EndTime.wDay, EndTime.wHour,
              EndTime.wMinute);

    GetNextDay(&m_tmSiegeEndSchedule, EndTime.wDay, EndTime.wHour, EndTime.wMinute, 0);

    g_Log.Add("[CastleSiege] CCastleSiege::LoadData() - Siege Schedule End Date (%d-%d-%d(%d:%d:%d)",
              this->m_tmSiegeEndSchedule.wYear, this->m_tmSiegeEndSchedule.wMonth, this->m_tmSiegeEndSchedule.wDay,
              this->m_tmSiegeEndSchedule.wHour, this->m_tmSiegeEndSchedule.wMinute, this->m_tmSiegeEndSchedule.wSecond);
...
BOOL CCastleSiege::SetCastleNpcData(CSP_CSINITDATA *lpMsg, int iCOUNT) {
    if (this->m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_3) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::SetCastleNpcData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_3");
        return FALSE;
    }
...
int CCastleSiege::CheckSync() {
    ...
    int iEVENT_END_DATE_NUM = MACRO2((MACRO1(m_tmEndDate.wDay) | MACRO1(m_tmEndDate.wMonth) << 8) & 0xFFFF) |
                              MACRO2(m_tmEndDate.wYear) << 16;

    if (iEVENT_END_DATE_NUM <= iTODAY_DATE_NUM) {
        this->m_bFixCastleCycleStartDate = FALSE;
        g_Log.MsgBox(
                "[CastleSiege] CCastleSiege::CheckSync() - iEVENT_END_DATE_NUM (%04d-%02d-%02d) <= iTODAY_DATE_NUM (%04d-%02d-%02d)",
                this->m_tmEndDate.wYear, this->m_tmEndDate.wMonth, this->m_tmEndDate.wDay, tmToDay.wYear,
                tmToDay.wMonth, tmToDay.wDay);
        return false;
    }
    ...
    if (this->m_bFixCastleStateStartDate != FALSE) {
        ...
        if (bSpecificStateExist != FALSE) {
            ...
            g_Log.MsgBox(
                    "[CastleSiege] CCastleSiege::CheckSync() - FIXED STATE:%d NOT FOUND IN SCHEDULE (P.S.> Check File 'MuCastleData.dat', 'commonserver.cfg')",
                    this->m_iFixCastleSpecificState);
            this->m_bFixCastleStateStartDate = FALSE;
        }
    }
    ...
    if (iEVENT_START_DATE_NUM > iTODAY_DATE_NUM) {
        ...
        while (it != this->m_vtScheduleData.end()) {
            ...
            i64ScheduleDateNum = (MACRO2(tmSchduleDate.wMinute) | MACRO2(tmSchduleDate.wHour) << 16) | ((__int64)(
                    MACRO2((MACRO1(tmSchduleDate.wDay) | MACRO1(tmSchduleDate.wMonth) << 8) & 0xFFFF) |
                    MACRO2(tmSchduleDate.wYear) << 16) << 0x20);
            i64ToDayDateNum = (MACRO2(tmToDay.wMinute) | MACRO2(tmToDay.wHour) << 16) | ((__int64)(
                    MACRO2((MACRO1(tmToDay.wDay) | MACRO1(tmToDay.wMonth) << 8) & 0xFFFF) | MACRO2(tmToDay.wYear) << 16)
                    << 0x20);
        ...
        if (bStateSetted == FALSE) {
            g_Log.MsgBox(
                    "[CastleSiege] CCastleSiege::CheckSync() - bStateSetted == FALSE  START_DATE (%04d-%02d-%02d), END_DATE (%04d-%02d-%02d)",
                    this->m_tmStartDate.wYear, this->m_tmStartDate.wMonth, this->m_tmStartDate.wDay,
                    this->m_tmEndDate.wYear, this->m_tmEndDate.wMonth, this->m_tmEndDate.wDay);
            return false;
        }
...
void CCastleSiege::GetNextDay(_SYSTEMTIME *st, int iAfterDay, int iAfterHour, int iAfterMin, int iAfterSec) //Identical
{
    ...
    i64 = i64 + (__int64) iAfterDay * 864000000000 + (__int64) iAfterHour * 36000000000 +
          (__int64) iAfterMin * 600000000 + (__int64) iAfterSec * 10000000;
...
void CCastleSiege::GetStateDate(int iCastleSiegeState, SYSTEMTIME *st) {
    ...
    for (std::vector<_CS_SCHEDULE_DATA>::iterator it = this->m_vtScheduleData.begin();
         it != this->m_vtScheduleData.end(); it++) {
...
int CCastleSiege::CalcCastleLeftSiegeDate() {
    ...
    if (this->m_iCastleSiegeState < CASTLESIEGE_STATE_IDLE_1 ||
        CASTLESIEGE_STATE_ENDCYCLE < this->m_iCastleSiegeState) {
...
void CCastleSiege::ClearNonDbNPC() {
    for (int n = 0; n < g_ConfigRead.server.GetObjectMaxMonster(); n++) {
        if (gObjIsConnected(n) && gObj[n].MapNumber == MAP_INDEX_CASTLESIEGE && gObj[n].m_btCsNpcType != NULL &&
                ...
                if (pNpcData.m_iNPC_NUM == 277) {
                    ...
                    if (bCreateLever != FALSE) {
                        ...
                        if (ObjectMaxRange(iLeverIndex)) {
                            ...
                            if (bRETVAL == FALSE) {
                                g_Log.AddC(TColor::Red,
                                           "[CastleSiege] CCastleSiege::CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",
...
int CCastleSiege::CheckAddDbNPC(int iIndex, int iNpcType, int iNpcIndex, BYTE &btResult) {
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex) {
            if (NpcData.m_bIN_USE == TRUE && NpcData.m_iNPC_LIVE > 0 && gObjIsConnected(NpcData.m_iNPC_OBJINDEX) &&
                gObj[NpcData.m_iNPC_OBJINDEX].Class == iNpcType) {
                if (gObj[NpcData.m_iNPC_OBJINDEX].Live == 1 &&
                    gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal &&
                    gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal != 0) {
                    btResult = 0;
                    iRETVAL = 0;
                    break;
                }
            ...
            GS_GDReqCastleNpcBuy(m_iMapSvrGroup, iIndex, iNpcType, iNpcIndex, NpcData.m_iNPC_DF_LEVEL,
                                 NpcData.m_iNPC_RG_LEVEL, NpcData.m_iNPC_MAXHP, NpcData.m_iNPC_HP, NpcData.m_iNPC_SX,
        ...
        if ((NpcData.m_iNPC_NUM == iNpcType) && (NpcData.m_iNPC_INDEX == iNpcIndex)) {
                    ...
                    if (gObj[NpcData.m_iNPC_OBJINDEX].Live == TRUE) {
                        if (gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal) {
                            if (gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal) {
                                g_Log.Add(
                                        "[CastleSiege] CCastleSiege::AddDbNPC() ERROR - DB NPC EXIST (CLS:%d, IDX:%d(%d), MAXHP:%d, HP:%d, DF:%d, RG:%d)",
                                        NpcData.m_iNPC_NUM, NpcData.m_iNPC_INDEX, NpcData.m_iNPC_OBJINDEX,
                                        gObj[NpcData.m_iNPC_OBJINDEX].MaxLife, gObj[NpcData.m_iNPC_OBJINDEX].Life,
            ...
            if (iObjIndex >= 0) {
                ...
                if (NpcData.m_iNPC_NUM == 277 || NpcData.m_iNPC_NUM == 283) {
                    g_Log.Add("[CastleSiege][CreateDBNPC] (Index:%d, Num:%d, DF:%d, RG:%d, MaxHP:%d, HP:%d )",
        ...
        if (pNpcData->m_iNPC_NUM == iNpcType && pNpcData->m_iNPC_OBJINDEX == iIndex) {
            if (pNpcData->m_iCsNpcExistVal == iMonsterExistVal && pNpcData->m_bIN_USE == TRUE) {
                if (pNpcData->m_iNPC_LIVE > 0) {
                    ...
                    if (pNpcData->m_iNPC_NUM == 277) {
                        this->SetGateBlockState(pNpcData->m_iNPC_SX, pNpcData->m_iNPC_SY, 1);
                        gObjAddBuffEffect(&gObj[pNpcData->m_iNPC_OBJINDEX], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0,
                                          -10);
                        g_Log.Add("[CastleSiege][ State:%d ] Delete Castle Gate ( CLS:%d, IDX:%d, X:%d. Y:%d )",
                                  this->m_iCastleSiegeState, pNpcData->m_iNPC_NUM, pNpcData->m_iNPC_INDEX,
                                  pNpcData->m_iNPC_SX, pNpcData->m_iNPC_SY);
                    }
                    ...
                    if (pNpcData->m_iNPC_NUM == 283) {
                        if (this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE) {
                            ...
                            g_Log.Add("[CastleSiege] Defend Statue is Broken (CLS:%d, IDX:%d, X:%d, Y:%d)",
                                      pNpcData->m_iNPC_NUM, pNpcData->m_iNPC_INDEX, pNpcData->m_iNPC_SX,
                                      pNpcData->m_iNPC_SY);
                        }
                    }
                    break;
...
BOOL CCastleSiege::RepairDbNPC(int iNpcType, int iNpcIndex, int iNpcHP, int iNpcMaxHP) {
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == 1 &&
            NpcData.m_iNPC_LIVE > 0) {
            it->m_iNPC_HP = iNpcHP;
            it->m_iNPC_MAXHP = iNpcMaxHP;

            int iObjIndex = NpcData.m_iNPC_OBJINDEX;

            if (gObjIsConnected(iObjIndex) != FALSE && gObj[iObjIndex].m_btCsNpcType != 0 &&
                gObj[iObjIndex].Class == iNpcType && gObj[iObjIndex].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal &&
...
int CCastleSiege::PayForUpgradeDbNpc(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue,
                                     int iNpcUpIndex) {
    ...
    if (CS_CHECK_UPGRADE(iNpcUpIndex) == FALSE) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::PayForUpgradeDbNpc() ERROR - lpMsg->iNpcUpIndex is Out of Bound : %d",
                   iNpcUpIndex);
        return FALSE;
    }
    ...
    if (bIsLive == FALSE) {
        g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::PayForUpgradeDbNpc() ERROR - bIsLive == FALSE",
                   iNpcUpIndex);
        return FALSE;
    }
...
BOOL
CCastleSiege::UpgradeDbNPC(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex) {
    if (CS_CHECK_UPGRADE(iNpcUpIndex) == FALSE) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::UpgradeDbNPC() ERROR - lpMsg->iNpcUpIndex is Out of Bound : %d",
                   iNpcUpIndex);
        return FALSE;
    }
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == 1 &&
            ...
            if (gObjIsConnected(iObjIndex) && gObj[iObjIndex].m_btCsNpcType != 0 &&
                gObj[iObjIndex].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && gObj[iObjIndex].m_iCsNpcExistVal != 0) {
...
void CCastleSiege::StoreDbNpc() {
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (pNpcData.m_bNPC_DBSAVE == TRUE && gObjIsConnected(pNpcData.m_iNPC_OBJINDEX) &&
            ...
            gObj[pNpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal != NULL && gObj[pNpcData.m_iNPC_OBJINDEX].Live == TRUE &&
            ...
            g_Log.Add(
                    "[CastleSiege] CCastleSiege::StoreDbNpc() NPC INFO (CLS:%d, IDX:%d(%d), MAXHP:%d, HP:%d, DF:%d, RG:%d)",
                    pNpcData.m_iNPC_NUM, pNpcData.m_iNPC_INDEX, pNpcData.m_iNPC_OBJINDEX, lpMsgBody[iCOUNT].iNpcMaxHp,
                    lpMsgBody[iCOUNT].iNpcHp, lpMsgBody[iCOUNT].iNpcDfLevel, lpMsgBody[iCOUNT].iNpcRgLevel);
            iCOUNT++;
        }
        it++;
    }
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (NpcData.m_iNPC_NUM == iNpcType && NpcData.m_bIN_USE == TRUE) {
            ...
            if (gObjIsConnected(iObjIndex)) {
                if (gObj[iObjIndex].Class == iNpcType && gObj[iObjIndex].Live == 1 && gObj[iObjIndex].Life > 0 &&
                    gObj[iObjIndex].m_btCsNpcType != 0 &&
                    gObj[iObjIndex].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal &&
                    gObj[iObjIndex].m_iCsNpcExistVal != 0) //HermeX Fix
                {
...
void CCastleSiege::AdjustDbNpcLevel() {
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (pNpcData.m_bIN_USE == 1 && pNpcData.m_iNPC_LIVE > 0) {
            ...
            if (gObjIsConnected(iNPC_INDEX) && gObj[iNPC_INDEX].Class == pNpcData.m_iNPC_NUM &&
        ...
        for (std::vector<_CS_NPC_DATA>::iterator it = this->m_vtNpcData.begin(); it != this->m_vtNpcData.end(); it++) {
            ...
            if (pNpcData.m_iNPC_NUM == 277) {
                ...
                if (ObjectMaxRange(iLeverIndex)) {
                    ...
                    it->m_iCS_GATE_LEVER_INDEX = -1;
                    g_Log.AddC(TColor::Red,
                               "[CastleSiege] CCastleSiege::CreateAllCsGateLever() ERROR - Failed to Create Gate Lever : GateIndex:%d",
                               pNpcData.m_iNPC_INDEX);
                }
            }
        }
        ...
        for (std::vector<_CS_NPC_DATA>::iterator it = this->m_vtNpcData.begin(); it != this->m_vtNpcData.end(); it++) {
            ...
            if ((NpcData.m_iNPC_NUM == 277 || NpcData.m_iNPC_NUM == 283) && NpcData.m_iNPC_OBJINDEX != OBJ_EMPTY) {
                if (!gObjIsConnected(NpcData.m_iNPC_OBJINDEX) ||
                    gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal != NpcData.m_iCsNpcExistVal ||
                    gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal == 0 ||
                    (gObj[NpcData.m_iNPC_OBJINDEX].Class != 277 && gObj[NpcData.m_iNPC_OBJINDEX].Class != 283) ||
                    gObj[NpcData.m_iNPC_OBJINDEX].Live == FALSE) //HermeX Fix
                {
                    ...
                    switch (NpcData.m_iNPC_NUM) {
                        case 277:
                            this->SetGateBlockState(NpcData.m_iNPC_SX, NpcData.m_iNPC_SY, 1);
                            gObjAddBuffEffect(&gObj[NpcData.m_iNPC_OBJINDEX], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0,
                                              -10);
                            g_Log.Add("[CastleSiege] Castle Gate is Broken (CLS:%d, IDX:%d, X:%d, Y:%d)",
...
int CCastleSiege::DelGemOfDefend(int iIndex, int iNeedGemOfDefend) {
    ...
    for (int x = 0; x < INVENTORY_SIZE; x++) {
        if (gObj[iIndex].pInventory[x].IsItem() == TRUE) {
            if (gObj[iIndex].pInventory[x].m_Type == ITEMGET(14, 31)) {
                g_Log.Add("[CastleSiege] [%s][%s] Deleting Gem of Defend (GUILD:%s) - Serial:%u",
...
void CCastleSiege::SetCalcRegGuildList(CSP_CALCREGGUILDLIST *lpMsg, int iCOUNT) {
    ...
    for (int iGCNT = 0; iGCNT < iCOUNT; iGCNT++) {
        ...
        int dwHighScore = ((lpMsg[iGCNT].iRegMarkCount * 5) + lpMsg[iGCNT].iGuildMemberCount +
                           (lpMsg[iGCNT].iGuildMasterLevel / 4));

#define MACRO3(value) (((__int64) value & 0xFFFFFFFF ))

        GuildData.m_i64TotolScore =
                ((__int64)(DWORD)MACRO3(lpMsg[iGCNT].iSeqNum)) | (((__int64)(DWORD)MACRO3(dwHighScore)) << 32);
    ...
    for (int iGCNT = 0; iGCNT < iGUILD_COUNT; iGCNT++) {
        ...
        GuildData.m_iGuildScore =
                (vtRegGuildScore[iGCNT].m_iRegMarkCount * 5) + (vtRegGuildScore[iGCNT].m_iGuildMemberCount) +
                (vtRegGuildScore[iGCNT].m_iGuildMasterLevel / 4); //season 2.5 add-on

        this->m_mapCsBasicGuildInfo.insert(
                std::pair<string, _CS_TOTAL_GUILD_DATA>(vtRegGuildScore[iGCNT].m_strGuildName, GuildData));
    }
    ...
    if (this->m_btIsCastleOccupied == TRUE) {
        if (strcmp(this->m_szCastleOwnerGuild, "") != 0) {
            ...
            this->m_mapCsBasicGuildInfo.insert(
                    std::pair<string, _CS_TOTAL_GUILD_DATA>(m_szCastleOwnerGuild, GuildData));
        }
    }
    ...
    lpMsgSend->h.set((LPBYTE) & cBUFFER, 0x86,
                     (lpMsgSend->iCount * sizeof(CSP_CSGUILDUNIONINFO)) + sizeof(CSP_REQ_CSGUILDUNIONINFO));
    wsDataCli.DataSend((char *) cBUFFER,
                       (lpMsgSend->iCount * sizeof(CSP_CSGUILDUNIONINFO)) + sizeof(CSP_REQ_CSGUILDUNIONINFO));
}
...
void CCastleSiege::MakeCsTotalGuildInfo(CSP_CSGUILDUNIONINFO *lpMsg, int iCOUNT) {
    ...
    if (this->m_bCsBasicGuildInfoLoadOK == FALSE) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::MakeCsTotalGuildInfo() ERROR - m_bCsBasicGuildInfoLoadOK == FALSE");
        return;
    }
    if (iCOUNT <= 0) {
...
void CCastleSiege::SaveCsTotalGuildInfo() {
    if (this->m_bCsTotalGuildInfoLoadOK == FALSE) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CCastleSiege::SaveCsTotalGuildInfo() ERROR - m_bCsTotalGuildInfoLoadOK == FALSE");
        return;
    }
    ...
    lpMsgSend->h.set((LPBYTE) cBUFFER, 0x87,
                     (lpMsgSend->iCount * sizeof(CSP_CSSAVETOTALGUILDINFO)) + sizeof(CSP_REQ_CSSAVETOTALGUILDINFO));
    wsDataCli.DataSend((char *) cBUFFER,
                       (lpMsgSend->iCount * sizeof(CSP_CSSAVETOTALGUILDINFO)) + sizeof(CSP_REQ_CSSAVETOTALGUILDINFO));
    ...
    for (int iSize = this->m_mapCsTotalGuildInfo.size(); it2 != this->m_mapCsTotalGuildInfo.end(); it2++) {
        ...
        g_Log.Add("[CastleSiege] Siege Guild List Save - GNAME:%s, CSGUID:%d, INVOLVED:%d, SCORE:%d",
                  (char *) &szGuildName[0], GuildData.m_iCsGuildID, GuildData.m_bGuildInvolved,
                  GuildData.m_iGuildScore); //season 2.5 changed
    }
...
void CCastleSiege::SetCsTotalGuildInfo(CSP_CSLOADTOTALGUILDINFO *lpMsg, int iCOUNT) {
    ...
    for (; it2 != this->m_mapCsTotalGuildInfo.end(); it2++) {
        ...
        g_Log.Add("[CastleSiege] Siege Guild List Load - GNAME:%s, CSGUID:%d, INVOLVED:%d, SCORE:%d", &GuildName[0],
                  GuildData.m_iCsGuildID, GuildData.m_bGuildInvolved, GuildData.m_iGuildScore); //season 2.5 changed
    }
...
int CCastleSiege::GetCsJoinSide(char *lpszGuildName, BYTE *btCsJoinSide, bool *bCsGuildInvolved) {
    ...
    *bCsGuildInvolved = 0;
    ...
    if (this->m_iCastleSiegeState < CASTLESIEGE_STATE_NOTIFY ||
        CASTLESIEGE_STATE_STARTSIEGE < this->m_iCastleSiegeState) {
        return FALSE;
    }
...
void CCastleSiege::SetAllUserCsJoinSide() {
    ...
    for (int iUSER = g_ConfigRead.server.GetObjectStartUserIndex();
         iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++) {
...
void CCastleSiege::NotifySelfCsJoinSide(int iIndex) {
    ...
    switch (btCsJoinSide) //season3 all changed
    {
        case 1:
            ...
            iViewState = 0x40000;
            if (this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE ||
                this->m_iCastleSiegeState == CASTLESIEGE_STATE_ENDSIEGE) {
...
int CCastleSiege::OperateGate(int iObjIndex, int iMonsterExistVal, int bOpenType) {
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (NpcData.m_iNPC_NUM == 277 && NpcData.m_iNPC_OBJINDEX == iObjIndex && NpcData.m_bIN_USE == TRUE &&
            NpcData.m_iNPC_LIVE == 2) {
...
int CCastleSiege::OperateGate(int iNpcIndex, int bOpenType) {
    ...
    while (it != this->m_vtNpcData.end()) {
        ...
        if (NpcData.m_iNPC_NUM == 277 && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == TRUE &&
            NpcData.m_iNPC_LIVE == 2) {
            int iObjNumber = NpcData.m_iNPC_OBJINDEX;

            if (gObjIsConnected(iObjNumber) && gObj[iObjNumber].m_btCsNpcType != 0 &&
...
BOOL CCastleSiege::CheckMiddleWinnerGuild() {
    ...
    if ((strcmp(gObj[this->m_iCastleCrownAccessUser].m_PlayerData->GuildName, "") == 0) ||
        (strcmp(gObj[this->m_iCastleSwitchAccessUser1].m_PlayerData->GuildName, "") == 0) ||
        (strcmp(gObj[this->m_iCastleSwitchAccessUser2].m_PlayerData->GuildName, "") == 0)) {
        return FALSE;
    }
    ...
    if ((gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide >= 2) &&
        ...
        if (gObj[this->m_iCastleCrownAccessUser].m_bCsGuildInvolved == 1) {
            if ((gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide ==
                 gObj[this->m_iCastleSwitchAccessUser1].m_btCsJoinSide) &&
                (gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide ==
                 gObj[this->m_iCastleSwitchAccessUser2].m_btCsJoinSide)) {
                if (((GetTickCount() - this->m_dwCrownAccessTime) +
                     gObj[this->m_iCastleCrownAccessUser].m_iAccumulatedCrownAccessTime) >= 60000) //season4.5 changed
                {
                    g_Log.Add("[CastleSiege] Castle Winner Has been Changed (GUILD:%s)",
                              gObj[this->m_iCastleCrownAccessUser].m_PlayerData->GuildName);
                    ...
                    memset(&this->m_szMiddleWinnerGuild, 0, sizeof(this->m_szMiddleWinnerGuild));
                    memcpy(&this->m_szMiddleWinnerGuild, gObj[this->m_iCastleCrownAccessUser].m_PlayerData->GuildName,
                           8);
...
int CCastleSiege::CheckCastleSiegeResult() {
    ...
    if (m_btIsCastleOccupied == TRUE && strcmp(m_szCastleOwnerGuild, "") != FALSE) {
        if (strcmp(m_szMiddleWinnerGuild, "") == FALSE ||
            strcmp(m_szMiddleWinnerGuild, m_szCastleOwnerGuild) == FALSE) {
    ...
    g_Log.Add("[CastleSiege] Castle Final Winner Guild : %s",
              (strcmp(this->m_szCastleOwnerGuild, "") == FALSE) ? "NONE" : this->m_szCastleOwnerGuild);
...
BOOL CCastleSiege::CheckTeleportMagicAxisY(int iStartY, int iTargetX, int iTargetY) {
    for (int iSTEP = 0; iSTEP < 3; iSTEP++) {
        if ((g_iCastleWallStepAxisY[iSTEP] >= iStartY) && (g_iCastleWallStepAxisY[iSTEP] <= iTargetY) &&
            (iTargetX < 144)) {
            return FALSE;
        }
    }
...
void CCastleSiege::NotifyCsSelfLeftTime(int iIndex) {
    GSProtocol.GCAnsSelfCsLeftTimeAlarm(iIndex, (this->m_iCS_REMAIN_MSEC / 1000) / 3600,
                                        ((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
}
...
void CCastleSiege::ReSpawnAllUser() {
    ...
    for (int iUSER = g_ConfigRead.server.GetObjectStartUserIndex();
        ...
        switch (btJoinSide) {
            case 1: {
                for (int iCNT = 0; iCNT < 100; iCNT++) {
                    iTX = (rand() % (g_iCsUserReSpawnArea[1][2] - g_iCsUserReSpawnArea[1][0])) +
                          g_iCsUserReSpawnArea[1][0];
                    iTY = (rand() % (g_iCsUserReSpawnArea[1][3] - g_iCsUserReSpawnArea[1][1])) +
                          g_iCsUserReSpawnArea[1][1];
            ...
            case 2: {
                for (int iCNT = 0; iCNT < 100; iCNT++) {
                    iTX = (rand() % (g_iCsUserReSpawnArea[2][2] - g_iCsUserReSpawnArea[2][0])) +
                          g_iCsUserReSpawnArea[2][0];
                    iTY = (rand() % (g_iCsUserReSpawnArea[2][3] - g_iCsUserReSpawnArea[2][1])) +
                          g_iCsUserReSpawnArea[2][1];
            ...
            default: {
                for (int iCNT = 0; iCNT < 100; iCNT++) {
                    iTX = (rand() % (g_iCsUserReSpawnArea[0][2] - g_iCsUserReSpawnArea[0][0])) +
                          g_iCsUserReSpawnArea[0][0];
                    iTY = (rand() % (g_iCsUserReSpawnArea[0][3] - g_iCsUserReSpawnArea[0][1])) +
                          g_iCsUserReSpawnArea[0][1];
        ...
        if (bMoveOK) {
            ...
            GSProtocol.GCTeleportSend(&gObj[iUSER], -1, MAP_INDEX_CASTLESIEGE, gObj[iUSER].X, gObj[iUSER].Y,
                                      gObj[iUSER].Dir);
        ...
        switch (btCsJoinSide) {
            case 1:
                if (bRefreshOwnerUser != FALSE) {
                    ...
                    GSProtocol.GCTeleportSend(&gObj[iUSER], -1, gObj[iUSER].MapNumber, gObj[iUSER].X, gObj[iUSER].Y,
                                              gObj[iUSER].Dir); //season 2.5 changed
...
void CCastleSiege::SetAllCastleGateState(BOOL bOpenType) {
    ...
    while (it != m_vtNpcData.end()) {
        ...
        if (pNpcData.m_iNPC_NUM == 277 && gObjIsConnected(pNpcData.m_iNPC_OBJINDEX) &&
...
void CCastleSiege::AddMiniMapDataReqUser(int iIndex) {
    ...
    if (std::find(this->m_vtMiniMapReqUser.begin(), this->m_vtMiniMapReqUser.end(), iIndex) ==
...
void CCastleSiege::DelMiniMapDataReqUser(int iIndex) {
    ...
    if ((it = std::find(this->m_vtMiniMapReqUser.begin(), this->m_vtMiniMapReqUser.end(), iIndex)) !=
        this->m_vtMiniMapReqUser.end()) {
...
void CCastleSiege::OperateMiniMapWork() {
    ...
    for (std::map<string, _CS_TOTAL_GUILD_DATA>::iterator it = this->m_mapCsBasicGuildInfo.begin();
         it != this->m_mapCsBasicGuildInfo.end(); it++) {
    ...
    lpMsgSend2->h.set(cBUFFER2, 0xBB,
                      (lpMsgSend2->iCount * sizeof(PMSG_SENDNPCMINIMAPDATA)) + sizeof(PMSG_ANS_SENDNPCMINIMAPDATA));
    ...
    for (int iIDX = g_ConfigRead.server.GetObjectStartUserIndex(); iIDX < g_ConfigRead.server.GetObjectMax(); iIDX++) {
        ...
        if (it3 != this->m_mapMiniMapData.end()) {
            ...
            if (gObj[iIDX].m_PlayerData->GuildStatus == GUILD_MASTER &&
                strcmp(gObj[iIDX].m_PlayerData->GuildName, "") != 0 &&
                strcmp(gObj[iIDX].m_PlayerData->GuildName, it3->second.m_szGuildName) == 0) {
    ...
    for (; it4 != this->m_mapMiniMapData.end(); it4++) {
        ...
        lpMsgSend1->h.set(cBUFFER1, 0xB6,
                          (lpMsgSend1->iCount * sizeof(PMSG_SENDMINIMAPDATA)) + sizeof(PMSG_ANS_SENDMINIMAPDATA));

        IOCP.DataSend(it4->second.m_iGuildMasterIndex, cBUFFER1,
                      (lpMsgSend1->iCount * sizeof(PMSG_SENDMINIMAPDATA)) + sizeof(PMSG_ANS_SENDMINIMAPDATA));
        IOCP.DataSend(it4->second.m_iGuildMasterIndex, cBUFFER2,
                      (lpMsgSend2->iCount * sizeof(PMSG_SENDNPCMINIMAPDATA)) + sizeof(PMSG_ANS_SENDNPCMINIMAPDATA));

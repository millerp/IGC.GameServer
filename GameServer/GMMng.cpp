...
void CGMMng::LoadCommandFile(LPSTR file) {
    ...
    GetPrivateProfileString("commands", "CastleSiegeSetOwnerCMD", "/cssetowner", m_CommandNames.CSSetOwnerCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeSetRegisterCMD", "/cssetregister", m_CommandNames.CSSetRegCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeSetRegMarkCMD", "/cssetregmark", m_CommandNames.CSSetMarkCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeSetNotifyCMD", "/cssetnotify", m_CommandNames.CSSetNotifyCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeSetReadyCMD", "/cssetready", m_CommandNames.CSSetReadyCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeSetEndCMD", "/cssetend", m_CommandNames.CSSetEndCMD, 20, file);
    GetPrivateProfileString("commands", "CastleSiegeGetOwnerCMD", "/csgetowner", m_CommandNames.CSGetOwnerCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeGetStateCMD", "/csgetstate", m_CommandNames.CSGetStateCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeSetStartCMD", "/cssetstart", m_CommandNames.CSSetStartCMD, 20,
                            file);
    GetPrivateProfileString("commands", "CastleSiegeChangeSideCMD", "/cschangeside", m_CommandNames.CSSetChangeSideCMD,
                            20, file);
...
int CGMMng::ManagementProc(LPOBJ lpObj, char *szCmd, int aIndex) {
    ...
    switch (command_number) {
        ...
        {
            if ((lpObj->GameMaster & GM_COMMAND_GUILD) == GM_COMMAND_GUILD &&
                (lpObj->GameMaster & GM_COMMAND_DC_MOVE) == GM_COMMAND_DC_MOVE) {
                pId = this->GetTokenString();
        ...
        case 111:    //107:
        {
            if ((lpObj->Authority & 2) == 2) {
                ...
                if (gObj[aIndex].m_PlayerData->lpGuild != NULL &&
                    gObj[aIndex].m_PlayerData->lpGuild->lpTargetGuildNode != NULL) {
                    if (strcmp(gObj[aIndex].Name, gObj[aIndex].m_PlayerData->lpGuild->Names[0]) == 0) {
                        if (gObj[aIndex].m_PlayerData->lpGuild->BattleGroundIndex >= 0 &&
                            gObj[aIndex].m_PlayerData->lpGuild->WarType == 1) {
                            ...
                            GSProtocol.GCServerMsgStringSendGuild(gObj[aIndex].m_PlayerData->lpGuild->lpTargetGuildNode,
                                                                  szTemp, 1);
                        }
                    }
                }
        ...
        case 104:    //102:
        {
            ...
            GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->m_PlayerData->Ip_addr,
                          lpObj->AccountID,
                          lpObj->Name, "Ban Chatting");
        ...
        case 202:    //111:
        {
            if ((lpObj->Authority & 2) == 2) {
                ...
                g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->m_PlayerData->Ip_addr,
                          lpObj->AccountID,
        ...
        case 429: // MultiCast Post
        {
            ...
            GS_GDReqGlobalPostMultiCast(g_MapServerManager.GetMapSvrGroup(), lpObj->m_Index,
                                        (char *) szCmd + strlen(m_CommandNames.GPostCMD));
            break;
        }
            break;
        case 430: {
        ...
        case 423: {
            ...
            GetLocalTime(&t);
            PMSG_NOTICE pNotice;
            TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 443), g_ConfigRead.server.GetServerName(),
                                     g_ConfigRead.server.GetGameServerCode(), gObjTotalUser,
                                     g_ConfigRead.server.GetObjectMaxUser());
            TNotice::SendNoticeToUser(aIndex, &pNotice);
            TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 444), t.wHour, t.wMinute, t.wSecond, t.wDay, t.wMonth,
                                     t.wYear);
            TNotice::SendNoticeToUser(aIndex, &pNotice);
            break;
        }
        ...
        case 390: {
            ...
            if ((type >= 0 && type <= 15)) {
                ...
                if (type == 13 && (index == 4 || index == 5)) {
                    PetItemSerialCreateSend(aIndex, gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y, Item.m_Type,
                                            ItemLevel, 0, ItemSkill, ItemLuck, ItemOpt, -1, ItemExc, ItemAncient);
                } else {
                    ItemSerialCreateSend(aIndex, 227, gObj[aIndex].X, gObj[aIndex].Y, Item.m_Type, ItemLevel, 0,
                                         ItemSkill, ItemLuck, ItemOpt, -1, ItemExc, ItemAncient, ItemExpireTime,
                                         Item.m_SocketOption, MainAttribute);
                }

                g_Log.Add("[%s][%s][%s] Created Item using Admin Command (%s/%d/%d) Auth: %d", lpObj->AccountID,
                          lpObj->Name, lpObj->m_PlayerData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber), lpObj->X,
                          lpObj->Y, lpObj->Authority);
                GMLog->Output("[%s][%s][%s] Created Item using Admin Command (%s/%d/%d) Auth: %d", lpObj->AccountID,
                              lpObj->Name, lpObj->m_PlayerData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber), lpObj->X,
                              lpObj->Y, lpObj->Authority);
            }
            break;
        ...
        case 399: {
            ...
            MsgOutput(aIndex, Lang.GetText(0, 446), lpObj->AccountID, lpObj->Name, lpObj->Level,
                      lpObj->m_PlayerData->MasterLevel, lpObj->Class);
            MsgOutput(aIndex, Lang.GetText(0, 447), lpObj->m_PlayerData->SwearWarning, Lang.GetMap(0, lpObj->MapNumber),
                      lpObj->X, lpObj->Y);
        ...
        case 320:    //117:
        {
            ...
            g_Log.Add("[KUNDUN] Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->m_PlayerData->Ip_addr,
                      lpObj->AccountID,
                    ...
                    if (lpObj->VpPlayer[n].type == OBJ_MONSTER) {
                        if (lpObj->VpPlayer[n].number >= 0) {
                            ...
                            if (lpTarget->Class == 275) {
                                ...
                                pNotice.SendToUser(lpObj->m_Index, "KUNDUN HP = %7.0f / %7.0f", lpTarget->Life,
                                                   lpTarget->MaxLife);
                                pNotice.SendToUser(lpObj->m_Index,
                                                   "HP Refill Sec = %d HP Refill Value = %d HP Refill Time = %d",
        ...
        case 322:    //119:
        {
            ...
            pNotice.SendToUser(lpObj->m_Index, "Kundun HP Refill Sec = %d Refill HP = %d Refill HP Time = %d",
                               giKundunRefillHPSec, giKundunRefillHP, giKundunRefillHPTime);
        ...
        case 401: {
            ...
            MsgOutput(lpObj->m_Index, "AccountID: %s, Level:%d MasterLevel:%d", gObj[iTargetIndex].AccountID,
                      gObj[iTargetIndex].Level, gObj[iTargetIndex].m_PlayerData->MasterLevel);
            MsgOutput(lpObj->m_Index, "Map: %s, X:%d Y:%d", Lang.GetMap(0, gObj[iTargetIndex].MapNumber),
                      gObj[iTargetIndex].X, gObj[iTargetIndex].Y);
        ...
        case 412: {
            ...
            lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life +
                             ((lpObj->Level + lpObj->m_PlayerData->MasterLevel) - 1) *
                             DCInfo.DefClass[lpObj->Class].LevelLife +
                             ((lpObj->m_PlayerData->Vitality - DCInfo.DefClass[lpObj->Class].Vitality) *
                              DCInfo.DefClass[lpObj->Class].VitalityToLife);
            lpObj->MaxMana = DCInfo.DefClass[lpObj->Class].Mana +
                             ((lpObj->Level + lpObj->m_PlayerData->MasterLevel) - 1) *
                             DCInfo.DefClass[lpObj->Class].LevelMana +
                             ((lpObj->m_PlayerData->Energy - DCInfo.DefClass[lpObj->Class].Energy) *
                              DCInfo.DefClass[lpObj->Class].EnergyToMana);
                    ...
                    if (g_PentagramSystem.IsPentagramItem(&lpObj->pInventory[i]) == true) {
                        for (int j = 0; j < 5; j++) {
                            if (lpObj->pInventory[i].m_SocketOption[j] < 0xFE) {
                                g_PentagramSystem.DelPentagramJewelInfo(aIndex, 0,
        ...
        case 433: {
            ...
            if (g_ConfigRead.server.GetServerType() != SERVER_NORMAL &&
                gObj[aIndex].MapNumber != MAP_INDEX_LORENMARKET) {
                MsgOutput(aIndex, Lang.GetText(0, 479));
                return 0;
            }
        ...
        case 444: {
            ...
            break;
        case 445: {
            MsgOutput(aIndex, "HP/MaxHP (%d/%d)", (int) gObj[aIndex].Life,
                      (int) (gObj[aIndex].AddLife + gObj[aIndex].MaxLife));
        }
            break;
        case 446: {
            if ((lpObj->Authority & 2) != 2 && (lpObj->Authority & 0x20) != 0x20) {
        ...
        case 449: {
            if (g_OffLevel.b_Enabled == true) {
                ...
                if (g_OffLevel.m_PerMapAttr[lpObj->MapNumber].Disable != 1) {
                    for (int i = 0; i < 5; i++) {
                        CMagicInf *lpMagic = gObjGetMagicSearch(lpObj,
                                                                g_OffLevel.m_SkillDef[lpObj->Class].SkillNumber[i]);
                        ...
                        if (lpMagic != 0) {
                            ...
                            MsgOutput(aIndex, Lang.GetText(0, 603),
                                      MagicDamageC.m_Name[g_OffLevel.m_SkillDef[lpObj->Class].SkillNumber[i]],
                                      lpMagic->m_Skill);
                            ...
                            if (g_OffLevel.m_SkillDef[lpObj->Class].SkillNumber[i] > 0) {
                                MsgOutput(aIndex, Lang.GetText(0, 604),
                                          MagicDamageC.m_Name[g_OffLevel.m_SkillDef[lpObj->Class].SkillNumber[i]],
                                          g_OffLevel.m_SkillDef[lpObj->Class].SkillNumber[i]);
                            }
                        }
        ...
        case 450: // GM Gift by Gremory Case
        {
            ...
            g_Log.Add("[%s][%s][%s] Added GM Gift to Gremory Case using Admin Command (%s/%d/%d) Auth: %d Target: %s",
                      lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber),
                      lpObj->X, lpObj->Y, lpObj->Authority, lpTargetObj->Name);
            GMLog->Output(
                    "[%s][%s][%s] Created Added GM Gift to Gremory Case using Admin Command (%s/%d/%d) Auth: %d Target: %s",
                    lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber),
                    lpObj->X, lpObj->Y, lpObj->Authority, lpTargetObj->Name);
        }
            break;
        ...
        case 452: {
            ...
            g_Log.Add("[ChaosCastleFinal][CHEAT][CCF BLOW] [%s]Changed m_bCCF_CHEAT_BLOW:%d ", lpObj->Name,
                      g_ChaosCastleFinal.m_bCCF_CHEAT_BLOW);
        }
            break;
        case 453: {
            if ((lpObj->Authority & 2) != 2 && (lpObj->Authority & 0x20) != 0x20) {
            ...
            g_Log.Add("[ChaosCastleFinal][CHEAT][CCF BLOW] [%s]Changed m_bCCF_CHEAT_TRAP:%d ", lpObj->Name,
                      g_ChaosCastleFinal.m_bCCF_CHEAT_TRAP);
        }
            break;
        case 454: {
            if ((lpObj->Authority & 2) != 2 && (lpObj->Authority & 0x20) != 0x20) {
...
BOOL CGMMng::CheckTraceMarryCondition(LPOBJ lpObj, LPOBJ lpTargetObj) {
    ...
    if (BC_MAP_RANGE(lpTargetObj->MapNumber) || DS_MAP_RANGE(lpTargetObj->MapNumber) ||
        CC_MAP_RANGE(lpTargetObj->MapNumber) || IT_MAP_RANGE(lpTargetObj->MapNumber) ||
        DG_MAP_RANGE(lpTargetObj->MapNumber) || IMPERIAL_MAP_RANGE(lpTargetObj->MapNumber) ||
        CRYWOLF_MAP_RANGE(lpTargetObj->MapNumber) || lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE ||
        lpTargetObj->MapNumber == MAP_INDEX_CASTLEHUNTZONE || g_GensSystem.IsMapBattleZone(lpTargetObj->MapNumber) ||
        lpTargetObj->MapNumber == MAP_INDEX_KANTURU_BOSS || lpTargetObj->MapNumber == MAP_INDEX_KALIMA1
        || lpTargetObj->MapNumber == MAP_INDEX_KALIMA2 || lpTargetObj->MapNumber == MAP_INDEX_KALIMA3 ||
        lpTargetObj->MapNumber == MAP_INDEX_KALIMA4 || lpTargetObj->MapNumber == MAP_INDEX_KALIMA5 ||
        lpTargetObj->MapNumber == MAP_INDEX_KALIMA6
        || lpTargetObj->MapNumber == MAP_INDEX_KALIMA7 || lpTargetObj->MapNumber == MAP_INDEX_ARCA_WAR ||
        lpTargetObj->MapNumber == MAP_INDEX_DEBENTER_ARCA_WAR) {
        return FALSE;
    }

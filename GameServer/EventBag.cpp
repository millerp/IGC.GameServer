...
bool CEventBag::UseBag(int aIndex, int iMonsterIndex) {
    ...
    if (rand() % 10000 >= this->m_BagData.dwItemDropRate) {
        ...
        g_Log.Add("[%s] Drop Zen [%s][%s] [%d Zen]", this->m_BagData.szBagName, lpObj->AccountID, lpObj->Name,
                  this->m_BagData.dwDropMoney);
        return true;
    }
    ...
    if (m_ItemSection.btItemDropCount == 1) {
        ...
        bool bResult = gLuaBag.DropCommonBag(aIndex, lpMonsterObj->MapNumber, lpMonsterObj->X, lpMonsterObj->Y,
                                             &m_Item);
...
bool
CEventBag::UseBag_GremoryCase(int aIndex, int iMonsterIndex, BYTE btStorageType, BYTE btRewardSource, int iExpireDays) {
    LPOBJ lpObj = &gObj[aIndex];
    LPOBJ lpMonsterObj = &gObj[iMonsterIndex];

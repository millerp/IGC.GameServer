...
BOOL gObjMonsterRegen(LPOBJ lpObj) {
    ...
    } else if ((lpObj->Class == 457 && lpObj->MapNumber == MAP_INDEX_HATCHERY) ||
               (lpObj->Class == 458 && lpObj->MapNumber == MAP_INDEX_HATCHERY))//Season 4.5 addon
    {
        lpObj->Live = FALSE;
        lpObj->m_State = 4;
        lpObj->RegenTime = GetTickCount();
        ...
        BLOODCASTLE_MONSTER_POSITION *lpPos = g_BloodCastle.GetMonsterPosData(lpObj->m_PosNum, iBridgeIndex,
                                                                              lpObj->Class);
        ...
        if (lpPos == nullptr) {
            g_Log.AddC(TColor::Red, "[Blood Castle][Error] lpPos == nullptr (%d) (%d) (%d)", lpObj->m_PosNum,
                       iBridgeIndex, lpObj->Class);
            return FALSE;
        }
    ...
    if (lpObj->MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE && g_CrywolfSync.GetCrywolfState() >= 2 &&
        g_CrywolfSync.GetCrywolfState() <= 5) // Crywolf
    {
        ...
        if (lpObj->sHD[n].number >= 0) {
            ...
            if (gObjIsConnected(lpObj->sHD[n].number) == TRUE) {
                ...
                if (gObj[lpObj->sHD[n].number].MapNumber != lpObj->MapNumber) {
                    delok = TRUE;
                } else if (gObjCheckTileArea(lpObj->m_Index, gObj[lpObj->sHD[n].number].X, gObj[lpObj->sHD[n].number].Y,
                                             20) == FALSE) {
                    delok = TRUE;
                }
            } else {
...
BOOL gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty) {
    ...
    if (CC_MAP_RANGE(lpObj->MapNumber) == TRUE) {
        CHAOSCASTLE_MONSTER_POSITION *lpPos = g_ChaosCastle.GetMonsterPosData(lpObj->m_PosNum,
                                                                              lpObj->m_cChaosCastleIndex);
    ...
    if (DS_MAP_RANGE(lpObj->MapNumber) == TRUE) {
        DEVILSQUARE_MONSTER_POSITION *lpPos = g_DevilSquare.GetMonsterPosData(lpObj->m_PosNum,
                                                                              lpObj->m_bDevilSquareIndex);
    ...
    if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE &&
        (lpObj->MapNumber == MAP_INDEX_KANTURU_BOSS || lpObj->MapNumber == MAP_INDEX_HATCHERY) && lpObj->m_PosNum < 0) {
...
void gObjMonsterStateProc(LPOBJ lpObj, int aMsgCode, int aIndex, int aMsgSubCode) {
    ...
    switch (aMsgCode) {
        case 0:
            ...
            if (lpObj->Class >= 504 && lpObj->Class <= 511 && lpObj->MaxLife * 0.3 >= lpObj->Life &&
                gObjCheckUsedBuffEffect(lpObj, 81) == false) {
                CMagicInf cMagicInf;
                ...
                if ((rand() % 2) == 1 && lpObj->PathStartEnd == 0) {
                    ...
                    if (ObjectMaxRange(IndexEnemy)) {
                        ...
                        if (enemydis <= range) {
                            ...
                            if (MapC[EnemyMap].CheckWall2(lpObj->X, lpObj->Y, gObj[IndexEnemy].X, gObj[IndexEnemy].Y) ==
                                1) {
                                lpObj->m_ActState.Attack = 1;
                                lpObj->TargetNumber = aIndex;
                            }
        ...
        case 1:
            ...
            if (KALIMA_MAP_RANGE(lpObj->MapNumber)) {
                if (lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 ||
                    lpObj->Class == 267) {
            ...
            if (lpObj->Class >= 678 && lpObj->Class <= 680) {
                g_Log.Add("[LORDSILVESTER]Kill %s INDEX:%d, X(%d)/Y(%d)", lpObj->Name, lpObj->Class, lpObj->X,
                          lpObj->Y);
            }
        ...
        case 2:
            if (gObj[aIndex].Live != FALSE) {
                if (BC_MAP_RANGE(gObj[aIndex].MapNumber) == FALSE) {
                    if (gObj[aIndex].Class != 131 ||
                        (((lpObj->Class - 132) < 0) ? FALSE : ((lpObj->Class - 132) > 2) ? FALSE : TRUE) == FALSE) {
...
void gObjMonsterMagicAttack(LPOBJ lpObj, int iMonsterClass) {
    ...
    if (pCount.Count > 0) {
        ...
        if (lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 ||
            lpObj->Class == 267 || lpObj->Class == 275) {
        ...
        if (lpObj->m_Attribute) {
            ...
            if (lpObj->Class != 673 || lpObj->VPCount2 >= 1) {
                if (lpObj->m_MoveRange > 0 && !gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_STONE) &&
                    !gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_STUN)
                ...
                if (lpObj->MaxLife == lpObj->Life &&
                    gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_MONSTER_MAGIC_IMMUNE) == TRUE) {
                ...
                if (g_ConfigRead.data.common.IsGuardSay) {
                    if (!(rand() % g_ConfigRead.data.common.GuardTalkChance)) {
                        if (lpObj->m_PK_Count == 0) {
                            GSProtocol.ChatSend(lpObj,
                                                (char *) g_ConfigRead.data.common.GuardSayText.c_str());    // Need Translation
                        }
                    }
                }
                    ...
                    if (g_ArcaBattle.IsArcaBattleServer() == TRUE) {
                        if (g_AcheronGuardianEvent.IsPlayStart() == false) {
                            if (lpObj->m_RecallMon != -1 && ObjectMaxRange(iTargetNumber) == true) {
                                if (g_ArcaBattle.IsEnableAttackObelisk(&gObj[lpObj->m_RecallMon],
                                                                       gObj[iTargetNumber].Class) == FALSE) {
                                    lpObj->TargetNumber = -1;
                                }
                            }
                        }
        ...
        if (item_drop) {
            ...
            if (DropItem->m_serial) {
                ...
                MapC[lpObj->MapNumber].MonsterItemDrop(type, level, dur, x, y,
                                                       Option1, Option2, Option3, NOption, 0, MaxHitUser, 0, 0, 0, -1,
                                                       0);
            }
        }
    }
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj) {
    ...
    if (CC_MAP_RANGE(lpObj->MapNumber)) {
        g_ChaosCastle.SearchNDropMonsterItem(g_ChaosCastle.GetChaosCastleIndex(lpObj->MapNumber), lpObj->m_Index,
                                             lpTargetObj->m_Index);
...
//		itemrate = 100;
    ...
    } else if (lpObj->Class == 80 || lpObj->Class == 81 || lpObj->Class == 494)    //Golden Lizard King
    {
        ...
        return;
    } else if (lpObj->Class == 82 || lpObj->Class == 83 || lpObj->Class == 495 ||
               lpObj->Class == 496)    // Golden Tantalos
    {
        int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
        x = lpObj->X;
        y = lpObj->Y;
    ...
    if (lpObj->Class == 652) {
        g_BagManager.SearchAndUseBag(lpTargetObj->m_Index, BAG_EVENT, EVENTBAG_GOLDCOLOSSUS_GOBLIN,
                                     lpTargetObj->m_Index);
        return;
    }

    if (lpObj->Class == 653) {
        g_BagManager.SearchAndUseBag(lpTargetObj->m_Index, BAG_EVENT, EVENTBAG_GOLDCOLOSSUS_TITAN,
                                     lpTargetObj->m_Index);
        return;
    }

    if (lpObj->Class == 654) {
        g_BagManager.SearchAndUseBag(lpTargetObj->m_Index, BAG_EVENT, EVENTBAG_GOLDCOLOSSUS_TANTALUS,
                                     lpTargetObj->m_Index);
        return;
    }

    if (lpObj->Class == 655) {
        g_BagManager.SearchAndUseBag(lpTargetObj->m_Index, BAG_EVENT, EVENTBAG_GOLDCOLOSSUS_EROHIM,
                                     lpTargetObj->m_Index);
        return;
    }
    ...
    if (lpObj->Class == 656) {
        g_BagManager.SearchAndUseBag(lpTargetObj->m_Index, BAG_EVENT, EVENTBAG_GOLDCOLOSSUS_HELLMAINE,
                                     lpTargetObj->m_Index);
        ...
        for (int iNum = 0; iNum < iWingCount; iNum++) {
            ItemSerialCreateSend(lpTargetObj->m_Index, lpTargetObj->MapNumber, lpTargetObj->X, lpTargetObj->Y,
                                 ITEMGET(12, 267), 0, 0, 0, 0, 0, iLootIndex, 7, 0, iWingExpireTime, 0, 0);
        }
    ...
    if (lpObj->Class == 657) {
        g_BagManager.SearchAndUseBag(lpTargetObj->m_Index, BAG_EVENT, EVENTBAG_GOLDCOLOSSUS_KUNDUN,
                                     lpTargetObj->m_Index);
        ...
        for (int iNum = 0; iNum < iWingCount; iNum++) {
            ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12, 267), 0, 0, 0, 0, 0,
                                 iLootIndex, 7, 0, iWingExpireTime, 0, 0);
        }
    ...
    int iExcDropRate = gExcDropRate + g_VipSystem.GetExcDropBonus(lpTargetObj) + g_BonusEvent.GetAddExcDrop() +
                       g_GensSystem.GetBattleZoneExcDropBonus(lpTargetObj);
    ...
    int ItemDropPer = gItemDropPer + g_VipSystem.GetDropBonus(lpTargetObj) + g_BonusEvent.GetAddDrop() +
                      g_GensSystem.GetBattleZoneDropBonus(lpTargetObj);

    if (lpTargetObj->Type == OBJ_USER) {
        ItemDropPer += gItemDropPer * lpTargetObj->m_PlayerData->SetOpImproveItemDropRate / 100;
    }

    ItemDropPer += ItemDropPer * (gObjGetTotalValueOfEffect(lpTargetObj, EFFECTTYPE_ITEMDROPRATE) /
                                  100.0f); // 7 - ADD_OPTION_DROP_RATE
    ...
    if (item_drop) {
        ...
        if (g_CrywolfSync.GetOccupationState() == 1 && g_CrywolfSync.GetApplyPenalty() == TRUE) {
            // Jewels
            if (DropItem->m_Type == ITEMGET(14, 13) || DropItem->m_Type == ITEMGET(14, 14) ||
                DropItem->m_Type == ITEMGET(14, 16) ||
                DropItem->m_Type == ITEMGET(14, 22) || DropItem->m_Type == ITEMGET(12, 15) ||
                DropItem->m_Type == ITEMGET(14, 31)) {
    ...
    if (item_drop) {
        ...
        if (type == ITEMGET(12, 15) ||    // Chaos
            ...
            (type >= ITEMGET(15, 0) && type <= ITEMGET(16, 0))) {
            Option1 = 0;
            Option2 = 0;
            Option3 = 0;
            NOption = 0;
    ...
    if ((rand() % 400) == 1) {
        for (n = 0; n < 4; n++) {
            ...
            if (MaxHitUser != -1 && gObj[MaxHitUser].Type == OBJ_USER) {
                if (g_ConfigRead.data.common.joinmuDropItemUnderCharacter[gObj[MaxHitUser].Class] == true) {
                    MapC[lpObj->MapNumber].MoneyItemDrop(lpObj->MonsterMoneyDrop, gObj[MaxHitUser].X,
                                                         gObj[MaxHitUser].Y);
                } else {
...
BOOL gEventMonsterItemDrop(LPOBJ lpObj, LPOBJ lpTargetObj) {
    ...
    if (SocketSphereItemType != -1)// Season 4.5 addon
    {
        ...
        ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, SocketSphereItemType, 0, 1, 0, 0, 0,
                             MaxHitUser, 0, 0, 0, 0, 0);
    ...
    if (TetraCrystalItemType != -1)// Season 9 addon
    {
        ...
        ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, TetraCrystalItemType, 0, 1, 0, 0, 0,
                             MaxHitUser, 0, 0, 0, 0, 0);
    ...
    if (g_CompoundPotionDropOn == TRUE) {
        ...
        if (bDropStuff == FALSE && lpObj->Level >= g_iCompoundPotionLv2DropLevel &&
            (rand() % 10000) <= g_iCompoundPotionLv2DropRate) {
            bDropStuff = TRUE;
    ...
    if ((rand() % 10000) < g_DoppelGanger.GetMirrorDimensionDropRate() &&
        lpObj->Level >= g_DoppelGanger.GetMirrorDimensionDropLevel()) {
        x = lpObj->X;
        y = lpObj->Y;
        dur = 1.0f;
    ...
    if ((rand() % 10000) < g_ImperialGuardian.GetPaperScrapDropRate() &&
        lpObj->Level >= g_ImperialGuardian.GetPaperScrapDropLevel()) {
        x = lpObj->X;
        y = lpObj->Y;
        dur = 1.0f;
                ...
                if (lpObj->Level >= gSetItemInCastleHuntZoneDropLevel) //Good
                {
                    if ((rand() % 10000) < gSetItemInCastleHuntZoneDropRate) {
                        ...
                        if (ObjectMaxRange(MaxHitUser)) {
                            g_Log.Add("[Castle HuntZone] Drop SetItem [%s][%s] ", gObj[MaxHitUser].AccountID,
                                      gObj[MaxHitUser].Name);
    ...
    if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) {
        if (lpObj->MapNumber == MAP_INDEX_KANTURU2) {
            if ((rand() % 10000) < g_Kanturu.GetMoonStoneDropRate()) {
                ...
                ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, x, y, type, level, dur, Option1, Option2,
                                     Option3, MaxHitUser, 0, 0, 0, 0, 0);

                return TRUE;
            }
        }

        if (lpObj->MapNumber == MAP_INDEX_KANTURU1 || lpObj->MapNumber == MAP_INDEX_KANTURU2 ||
    ...
    if (g_bCandyBoxEvent) {
        ...
        } else if (lpObj->Level >= g_iVermilionCandyBoxDropLevelMin &&
                   lpObj->Level <= g_iVermilionCandyBoxDropLevelMax) {
    ...
    if (lpObj->Level >= g_ConfigRead.data.common.SpiritMapDropLevel &&
        rand() % 10000 < g_ConfigRead.data.common.SpiritMapDropRate) {
...
void CQeustNpcTeleport::Run(int aIndex) {
    ...
    if (this->TimeCount > gQuestNPCTeleportTime) {
        ...
        g_Log.Add("[Quest] %s Teleport MAP : %s", this->m_strNPCName.c_str(),
                  Lang.GetMap(0, this->m_QuestNPCTeleportPos[tableindex].mapnum));
    }
}

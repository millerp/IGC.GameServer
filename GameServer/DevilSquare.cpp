...
int CDevilSquare::GetUserLevelToEnter(int iUserIndex, WORD &wMoveGate) {
    ...
    for (int i = 0; i < MAX_DEVILSQUARE_GROUND; i++) {
        if (gObj[iUserIndex].Class == 4 || gObj[iUserIndex].Class == 3 || gObj[iUserIndex].Class == 6) {
            ...
            if (gObj[iUserIndex].Level >= g_sttDEVILSQUARE_LEVEL[i].SpecialCharacterMinLevel &&
                gObj[iUserIndex].Level <= g_sttDEVILSQUARE_LEVEL[i].SpecialCharacterMaxLevel &&
                gObj[iUserIndex].m_PlayerData->ChangeUP != 2) {
                iENTER_LEVEL = i;
            ...
            if (gObj[iUserIndex].Level >= g_sttDEVILSQUARE_LEVEL[i].NormalCharacterMinLevel &&
                gObj[iUserIndex].Level <= g_sttDEVILSQUARE_LEVEL[i].NormalCharacterMaxLevel &
                gObj[iUserIndex].m_PlayerData->ChangeUP != 2) {
                iENTER_LEVEL = i;
                wMoveGate = g_sttDEVILSQUARE_LEVEL[i].MoveGate;
                break;
...
void CDevilSquare::CheckSync() {
    ...
    switch (bTIME_CHANGED) {
        ...
        case 2:
            this->m_iRemainTime = ((((iMIN_HOUR + 24) * 60) + (iMIN_MINUTE)) -
...
void CDevilSquare::gDevilSquareMonsterRegen(LPOBJ lpObj) {
    ...
    WORD monstertype = this->m_DevilSquareGround[devilsquareindex].GetMonsterType(
            this->m_iPlaytime - this->m_iRemainTime);
...
UINT64 CDevilSquare::gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg) {
    ...
    if (!g_MasterLevelSkillTreeSystem.IsMasterLevelUser(lpObj)) {
        ...
        fBaseExp = g_ExpManager.GetExpMultiplier(lpObj->Level + lpObj->m_PlayerData->MasterLevel,
                                                 lpObj->m_PlayerData->m_iResets, true);
    }
                ...
                if (dis[n] < 10) {
                    if (g_MasterLevelSkillTreeSystem.IsMasterLevelUser(lpPartyObj)) {
                        ...
                        if (lpPartyObj->m_PlayerData->MasterLevel == g_ConfigRead.data.common.MLUserMaxLevel) {
                            ...
                            myexp = g_MasterLevelSkillTreeSystem.gMasterExperience[
                                    lpPartyObj->m_PlayerData->MasterLevel + 1];
                        }
                        if (exp > myexp) {
                            exp = myexp;
                        }
                    ...
                    if (g_MasterLevelSkillTreeSystem.IsMasterLevelUser(lpPartyObj)) {
                        ...
                        fBaseExp = g_ExpManager.GetExpMultiplier(
                                lpPartyObj->Level + lpPartyObj->m_PlayerData->MasterLevel,
                                lpPartyObj->m_PlayerData->m_iResets, true);
                    } else {
                        fEventBonus = g_BonusEvent.GetAddExp();
                        fMapBonus = g_MapAttr.GetExpBonus(lpPartyObj->MapNumber);
                        fBaseExp = g_ExpManager.GetExpMultiplier(lpPartyObj->Level, lpPartyObj->m_PlayerData->m_iResets,
                                                                 false);
                    }
                    ...
                    if (exp > 0) {
                        if (lpPartyObj->Type == OBJ_USER) {
                            ...
                            if (gObjLevelUp(lpPartyObj, exp, lpTargetObj->Class, "Devil Square Monster (Party)") ==
                                false) {
                                continue;
                            }
                        }
                    ...
                    if (lpPartyObj->Type == OBJ_USER && exp > 0) {
                        GSProtocol.GCKillPlayerExpSend(lpPartyObj->m_Index, lpTargetObj->m_Index, exp, AttackDamage,
                                                       MSBFlag);
                    }
                }
            }
...
void CDevilSquare::gObjMonsterScoreDivision(LPOBJ lpMonObj, LPOBJ lpObj, int AttackDamage, BOOL MSBFlag) {
    ...
    if (LastHitObjNum != -1) {
        ...
        g_QuestExpProgMng.ChkUserQuestTypeEventMap(QUESTEXP_ASK_DEVILSQUARE_POINT_GAIN, lpTargetObj,
                                                   lpTargetObj->m_bDevilSquareIndex, 2);
    }
}

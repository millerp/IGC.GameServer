    ...
    for (int x = 0; x < INVENTORY_SIZE; x++) {
        ...
        if ((gObj[iObjIndex].pInventory[x].m_Type<ITEMGET(14, 0)
            ...
            if (gObj[iObjIndex].pInventory[x].m_Option1 == iItemSkill &&
                gObj[iObjIndex].pInventory[x].m_Option3 == iItemOpt &&
                gObj[iObjIndex].pInventory[x].m_NewOption == iItemExOpt) {
...
int QuestExpLuaBind::GetQuestSwitch(lua_State *L) {
    ...
    if (iQuestSwitch == -1) {
        g_Log.Add("[QuestExp] - Error - LuaGetQuestSwitch [%s][%s] Episode[%d]", gObj[iObjIndex].AccountID,

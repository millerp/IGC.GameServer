...
BOOL CQuestInfo::LoadQuestInfo(char *filename) {
    ...
    for (pugi::xml_node quest_info = main.child("QuestInfo"); quest_info; quest_info = quest_info.next_sibling()) {
        ...
        for (pugi::xml_node job = quest_job.child("Job"); job; job = job.next_sibling()) {
            ...
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].NeedSubType = job.attribute(
                    "ReqItemIndex").as_int();
            ...
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].NeedTargetMinLevel = job.attribute(
                    "MonsterMinLevel").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].NeedTargetMaxLevel = job.attribute(
                    "MonsterMaxLevel").as_int();
            ...
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RewardSubType = job.attribute(
                    "RewardSubType").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RewardCount = job.attribute(
                    "RewardCount").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].LinkConditionIndex = job.attribute(
                    "LinkConditionIndex").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[0] = job.attribute(
                    "DarkWizard").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[1] = job.attribute(
                    "DarkKnight").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[2] = job.attribute(
                    "FairyElf").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[3] = job.attribute(
                    "MagicGladiator").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[4] = job.attribute(
                    "DarkLord").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[5] = job.attribute(
                    "Summoner").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[6] = job.attribute(
                    "RageFighter").as_int();
            this->QuestInfo[iQuestIndex].QuestSubInfo[iSubInfoCount].RequireClass[7] = job.attribute(
                    "GrowLancer").as_int();
        ...
        for (pugi::xml_node condition = quest_condition.child(
                "Condition"); condition; condition = condition.next_sibling()) {
            ...
            this->QuestInfo[iQuestIndex].QuestCondition[iConditionCount].NeedQuestIndex = condition.attribute(
                    "Index").as_int();
            this->QuestInfo[iQuestIndex].QuestCondition[iConditionCount].MinLevel = condition.attribute(
                    "MinLevel").as_int();
            this->QuestInfo[iQuestIndex].QuestCondition[iConditionCount].MaxLevel = condition.attribute(
                    "MaxLevel").as_int();
            this->QuestInfo[iQuestIndex].QuestCondition[iConditionCount].ReqStr = condition.attribute(
                    "ReqStrength").as_int();
            this->QuestInfo[iQuestIndex].QuestCondition[iConditionCount].NeedZen = condition.attribute(
                    "ReqMoney").as_int();
            this->QuestInfo[iQuestIndex].QuestCondition[iConditionCount].StartContext = condition.attribute(
                    "StartContext").as_int();
            this->QuestInfo[iQuestIndex].QuestConditionCount++;
        }

        g_Log.Add("[Quest] (%s) Load : Index %d, SubInfoCount %d, ConditionCount %d",
                  this->QuestInfo[iQuestIndex].Name, iQuestIndex, this->QuestInfo[iQuestIndex].QuestSubInfoCount,
                  this->QuestInfo[iQuestIndex].QuestConditionCount);
...
BYTE CQuestInfo::QuestClearConditionCheck(LPOBJ lpObj, int QuestIndex) {
    ...
    for (int subquest = 0; subquest < subquestcount; subquest++) {
        ...
        if (lpSubInfo != NULL) {
            ...
            if (lpSubInfo->QuestType == 1) {
                int NumberItemQuestFound = gObjGetItemCountInIventory(lpObj->m_Index, lpSubInfo->NeedType,
            ...
            if (lpSubInfo != NULL) {
                if (lpSubInfo->QuestType == 1) {
                    if ((lpObj->Level >= lpSubInfo->NeedTargetMinLevel &&
                         lpObj->Level <= lpSubInfo->NeedTargetMaxLevel && lpSubInfo->NeedTargetMinLevel != -1) ||
                        (lpSubInfo->NeedTargetMinLevel == -1 && lpSubInfo->NeedTargetMaxLevel == lpObj->Class)) {
                    ...
                    if (lpSubInfo != NULL) {
                        if (lpSubInfo->QuestType == 1) {
                            if ((lpObj->Level >= lpSubInfo->NeedTargetMinLevel &&
                                 ...
                                 lpSubInfo->NeedTargetMinLevel != -1) || (lpSubInfo->NeedTargetMinLevel == -1 &&
                                                                          lpSubInfo->NeedTargetMaxLevel ==
                                                                          lpObj->Class)) {
                                if (this->GetQuestState(lpPartyObj, lpQuestInfo->QuestIndex) == TRUE) {
                                    if ((rand() % ITEM_QUEST_DROP_PROBABILITY) < lpSubInfo->NeedDropRate) {
                                        ...
                                        g_Log.Add("[Quest] Quest Item Drop(Party) [%s]: [%s][%s] (%s) (%d,%d)",
                                                  lpObj->Name,
                                                  lpPartyObj->AccountID, lpPartyObj->Name, lpQuestInfo->Name,
                                                  lpSubInfo->NeedType,
                                                  lpSubInfo->NeedSubType);
                                        return true;
                                    }
                        ...
                        if (lpSubInfo != NULL) {
                            if (lpSubInfo->QuestType == 2) {
                                if (lpObj->Class == lpSubInfo->NeedType &&
                                    this->GetQuestState(lpPartyObj, lpQuestInfo->QuestIndex) == TRUE) {
                                    switch (lpSubInfo->NeedType) {
                                        case 409:
                                            if (lpPartyObj->m_PlayerData->m_Quest[20] < lpSubInfo->NeedNumber) {
                                                ...
                                                if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                                    MsgOutput(lpPartyObj->m_Index, Lang.GetText(0, 540),
                                                              lpPartyObj->m_PlayerData->m_Quest[20],
                                        ...
                                        case 410:
                                            if (lpPartyObj->m_PlayerData->m_Quest[21] < lpSubInfo->NeedNumber) {
                                                ...
                                                if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                                    MsgOutput(lpPartyObj->m_Index, Lang.GetText(0, 541),
                                                              lpPartyObj->m_PlayerData->m_Quest[21],
                                                              lpSubInfo->NeedNumber);
                                                }
                                            }
                                        ...
                                        case 411:
                                            if (lpPartyObj->m_PlayerData->m_Quest[22] < lpSubInfo->NeedNumber) {
                                                ...
                                                if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                                    MsgOutput(lpPartyObj->m_Index, Lang.GetText(0, 542),
                                                              lpPartyObj->m_PlayerData->m_Quest[22],
                                                              lpSubInfo->NeedNumber);
                                                }
                                            }
                                        ...
                                        case 412:
                                            if (lpPartyObj->m_PlayerData->m_Quest[24] < lpSubInfo->NeedNumber) {
                                                ...
                                                if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                                    MsgOutput(lpPartyObj->m_Index, Lang.GetText(0, 543),
                                                              lpPartyObj->m_PlayerData->m_Quest[24],
                                                              lpSubInfo->NeedNumber);
                                                }
                                            }
            ...
            for (int n = 0; n < lpQuestInfo->QuestSubInfoCount; n++) {
                ...
                if (lpSubInfo != NULL) {
                    if (lpSubInfo->QuestType == 2) {
                        ...
                        if (lpObj->Class == lpSubInfo->NeedType &&
                            this->GetQuestState(lpTarget, lpQuestInfo->QuestIndex) == TRUE) {
                            switch (lpSubInfo->NeedType) {
                                case 409:
                                    if (lpTarget->m_PlayerData->m_Quest[20] < lpSubInfo->NeedNumber) {
                                        lpTarget->m_PlayerData->m_Quest[20]++;
                                        if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                            MsgOutput(lpTarget->m_Index, Lang.GetText(0, 540),
                                                      lpTarget->m_PlayerData->m_Quest[20], lpSubInfo->NeedNumber);
                                        }
                                ...
                                case 410:
                                    if (lpTarget->m_PlayerData->m_Quest[21] < lpSubInfo->NeedNumber) {
                                        ...
                                        if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                            MsgOutput(lpTarget->m_Index, Lang.GetText(0, 541),
                                                      lpTarget->m_PlayerData->m_Quest[21], lpSubInfo->NeedNumber);
                                        }
                                ...
                                case 411:
                                    if (lpTarget->m_PlayerData->m_Quest[22] < lpSubInfo->NeedNumber) {
                                        ...
                                        if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                            MsgOutput(lpTarget->m_Index, Lang.GetText(0, 542),
                                                      lpTarget->m_PlayerData->m_Quest[22], lpSubInfo->NeedNumber);
                                        }
                                ...
                                case 412:
                                    if (lpTarget->m_PlayerData->m_Quest[24] < lpSubInfo->NeedNumber) {
                                        ...
                                        if (g_ConfigRead.data.common.ThirdQuestCounter) {
                                            MsgOutput(lpTarget->m_Index, Lang.GetText(0, 543),
                                                      lpTarget->m_PlayerData->m_Quest[24], lpSubInfo->NeedNumber);
                                        }
    ...
    for (int subquest = 0; subquest < subquestcount; subquest++) {
        ...
        if (lpSubInfo != NULL) {
            if (lpSubInfo->RewardType == QUEST_COMPENSATION_CHANGEUP) {
                ...
                if (lpObj->m_PlayerData->DbClass != 2 && lpObj->m_PlayerData->DbClass != 18 &&
                    lpObj->m_PlayerData->DbClass != 34 && lpObj->m_PlayerData->DbClass != 50 &&
                    lpObj->m_PlayerData->DbClass != 66 && lpObj->m_PlayerData->DbClass != 82 &&
                    lpObj->m_PlayerData->DbClass != 98) {
                    if (lpObj->m_PlayerData->DbClass == 48 || lpObj->m_PlayerData->DbClass == 64 ||
                        lpObj->m_PlayerData->DbClass == 96) {
                ...
                g_Log.Add("[Quest] Quest Clear (%s) : [%s][%s] Class:%d(%d), 3rd ChangeUp",
                          lpQuestInfo->Name, lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->DbClass,
                          lpObj->m_PlayerData->DbClass - 1);
                ...
                g_Log.Add("[Quest] Quest Clear (%s) : [%s][%s] Stat(%d,%d), Class:%d PlusStat",
                          lpQuestInfo->Name, lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->LevelUpPoint,
                          lpSubInfo->RewardCount, lpObj->m_PlayerData->DbClass);
            }
...
void CQuestInfo::QuestInfoSave(LPOBJ lpObj) {
    ...
    if (iQuestState_1 == 1 || iQuestState_2 == 1) {
        g_Log.Add("[Quest] Save MonsterKillInfo [%s][%s] (%d, %d, %d, %d)", lpObj->AccountID, lpObj->Name,
                  lpObj->m_PlayerData->m_Quest[20], lpObj->m_PlayerData->m_Quest[21], lpObj->m_PlayerData->m_Quest[22],
                  lpObj->m_PlayerData->m_Quest[24]);
    }
}
                ...
                if (nType == (lpSubInfo->NeedType * 512) + lpSubInfo->NeedSubType) {
                    if (nLevel == lpSubInfo->ItemLevel) {
                        ...
                        if (iItemCount >= lpSubInfo->NeedNumber) {
                            g_Log.Add("[Quest] Too many has quest items [%s][%s] (%d/%d)", lpObj->AccountID,
                                      lpObj->Name, nType, iItemCount);
                            return FALSE;
                        }
                        return TRUE;
                    }

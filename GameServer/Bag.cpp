...
const char *szFileClassName[] = {"Wizard", "Knight", "FairyElf", "MagicGladiator", "DarkLord", "Summoner",
                                 "RageFighter", "GrowLancer"};
...
bool CBag::LoadBag(const char *lpFileName) {
    ...
    for (pugi::xml_node section = main_section.child("DropAllow"); section; section = section.next_sibling()) {
        ...
        if (strcmp("MAX", section.attribute("PlayerMaxLevel").as_string()) == 0) {
            m_DropSection.wUseSectionMaxLevel =
                    g_ConfigRead.data.common.UserMaxLevel + g_ConfigRead.data.common.MLUserMaxLevel;
        } else {
        ...
        if (m_DropSection.btUseSectionAllowedMapNum != 0xFF &&
            MAX_MAP_RANGE(m_DropSection.btUseSectionAllowedMapNum) == FALSE) {
            g_Log.MsgBox("Error: Bag (%s), DropAllow Section:%d, btUseSectionAllowedMapNum value is wrong", lpFileName,
                         m_DropSection.btUseSectionAllowedMapNum);
            continue;
        }
        ...
        for (pugi::xml_node dropsec = section.child("Drop"); dropsec; dropsec = dropsec.next_sibling()) {
            ...
            for (pugi::xml_node item = dropsec.child("Item"); item; item = item.next_sibling()) {
                ...
                if (ItemGetNumberMake(m_Item.btItemType, m_Item.wItemIndex) == -1) {
                    g_Log.MsgBox("Error: Wrong Item in %s bag (Cat: %d Index: %d)", this->m_BagData.szBagName,
                                 m_Item.btItemType, m_Item.wItemIndex);
                    continue;
                }
...
int CBag::GetDropSection(int aIndex, BAG_SECTION_DROP &pRetDrop) {
    ...
    for (std::vector<BAG_SECTION_DROP>::iterator It = this->m_BagData.m_vtSectionDrop.begin();
         It != this->m_BagData.m_vtSectionDrop.end(); It++, SectionID++) {
...
int CBag::GetItemsSection(BAG_SECTION_DROP &pDropSec, BAG_SECTION_ITEMS &pRetItems) {
    ...
    for (std::map<int, BAG_SECTION_ITEMS>::iterator It = pDropSec.m_mapSectionItems.begin();
         It != pDropSec.m_mapSectionItems.end(); It++) {
...
int CBag::GetItem(BAG_SECTION_ITEMS &pItemsSec, BAG_ITEM &pRetItem) {
    if (pItemsSec.m_vtItems.size() == 0) {
        g_Log.AddC(TColor::Red, "[ERROR] CBag::GetItem() -> Item Container is EMPTY - check XML File (%s)",
                   this->m_BagData.szBagName);
        return FALSE;
    }
...
int CBag::GetReadyItemToUse(int aIndex, CItem &pItem, time_t &DurationItem) {
    ...
    if (this->GetItem(m_ItemSection, m_Item) == FALSE) {
        g_Log.AddC(TColor::Red, "[ERROR] Bag %s -> GetItem() == FALSE -> probably no item in %d section",
                   this->m_BagData.szBagName, m_ItemSection.btSectionID);
        return false;
    }
    ...
    g_Log.Add("[%s] Create Item [%s][%s] : (%d)(%d/%d) Item:(%s)%d", this->m_BagData.szBagName,
              ...
              ItemAttribute[ITEMGET(m_Item.btItemType, m_Item.wItemIndex)].Name,
              ITEMGET(m_Item.btItemType, m_Item.wItemIndex));
...
void CBag::AddCoin(int aIndex) {
    ...
    g_Log.Add("[Bag Manager] (%s)(%s) Add Coin: (Type:%d) (Amount:%d) (Bag:%s)", lpObj->AccountID, lpObj->Name,
              this->m_BagData.btAddCoinType, this->m_BagData.dwAddCoinValue, this->m_BagData.szBagName);
}
...
void CBag::DropSummonItem(int aIndex) {
    ...
    if (GetLargeRand() % 1000000 <= this->m_BagData.dwSummonBookDropRate) {
        ...
        ItemSerialCreateSend(aIndex, gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y, iType, 0, -1, 0, 0, 0,
                             aIndex, 0, 0, 0, 0, 0);
    }
}
...
BOOL CRingMonsterHerd::MonsterHerdItemDrop(LPOBJ lpObj) {
    ...
    if (lpObj->Class == 135) {
        ...
        char szTemp[256];
        wsprintf(szTemp, Lang.GetText(0, 101), gObj[iIndex].Name,
                 Lang.GetMap(0, lpObj->MapNumber));    // #error Apply Deathway fix here
...
void CRingAttackEvent::StartEvent() {
    ...
    for (int i = 0; i < MAX_RINGMONSTER_MAP; i++) {
        ...
        for (std::vector<RINGMONSTER_DATA>::iterator it = this->m_vtMonsterAddData.begin();
             it != this->m_vtMonsterAddData.end(); it++) {
            RINGMONSTER_DATA &pRMD = *it;
            iCount = 100;

            while (iCount-- != 0) {
                if (this->m_RingMonsterHerd[i].AddMonster(pRMD.m_iType, pRMD.m_bDoRegen, pRMD.m_bDoAttackFirst) !=
                    FALSE) {
                    break;
                }
            }
...
void CRingAttackEvent::Move() {
    if (this->m_bDoEvent) {
        ...
        for (int i = 0; i < MAX_RINGMONSTER_MAP; i++) {
            ...
            this->m_RingMonsterHerd[i].SetRadius(

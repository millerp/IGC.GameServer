...
BOOL CKanturu::LoadData(LPSTR lpszFileName) {
    ...
    try {
        ...
        this->SetKanturuEnable(
                g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_KANTURU) ==
                true ? main.attribute("Enable").as_bool() : false);
...
int CKanturu::CheckEnterKanturu(int iUserIndex) {
    ...
    if (this->GetKanturuState() == KANTURU_STATE_BATTLE_OF_MAYA &&
        ...
        if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) {
            if ((gObj[iUserIndex].pInventory[7].m_Type<ITEMGET(12, 0) || gObj[iUserIndex].pInventory[7].m_Type>ITEMGET(
                    12, 6) && gObj[iUserIndex].pInventory[7].m_Type < ITEMGET(12, 36)) &&
                ...
                !(gObj[iUserIndex].pInventory[7].m_Type >= ITEMGET(12, 36) &&
                  gObj[iUserIndex].pInventory[7].m_Type <= ITEMGET(12, 43)) &&
                !(gObj[iUserIndex].pInventory[7].m_Type >= ITEMGET(12, 262) &&
                  gObj[iUserIndex].pInventory[7].m_Type <= ITEMGET(12, 265)) &&

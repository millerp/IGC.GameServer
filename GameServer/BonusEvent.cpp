...
void CBonusEvent::Run(void *lpParam) {
    while (true) {
        ...
        for (std::vector < std::shared_ptr < BONUS_EVENT_DATA >> ::iterator It = lpThis->m_vtEventData.begin(); It !=
                                                                                                                lpThis->m_vtEventData.end();
        It++)
        {
        ...
        if (active_event_ptr != NULL) {
            ...
            if (bChange == TRUE) {
                if (lpThis->m_bEventNotice == true) {
                    ...
                    PMSG_NOTICE pNotice;
                    TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 600),
                                             active_event_ptr->wEndHour - active_event_ptr->wStartHour, nRemainMin);
                    TNotice::SendNoticeToAllUser(&pNotice);

                    g_Log.Add("[Bonus Event] Started Event (AddExp:%f) (AddMLExp:%f) (AddDrop:%f) (AddExcDrop:%f)",
                              active_event_ptr->AddExp, active_event_ptr->AddMLExp, active_event_ptr->AddDrop,
                              active_event_ptr->AddExcDrop);
                }

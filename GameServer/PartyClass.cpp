...
int PartyClass::GetIndex(int party_number, int usernumber, int dbnumber) {
    ...
    for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
        if (this->m_PartyS[party_number].Number[n] == usernumber ||
            this->m_PartyS[party_number].DbNumber[n] == dbnumber) {
            return n;
        }
    }
...
int PartyClass::Create(int usernumber, int dbnumber, int level) {
    ...
    while (true) {
        if (this->m_PartyS[n].Count == 0) {
            ...
            if (gObj[usernumber].m_PlayerData->m_bUsePartyMatching &&
                gObj[usernumber].m_PlayerData->m_bPartyMatchingLeader) {
...
int PartyClass::Add(int party_number, int usernumber, int dbnumber, int level) {
    ...
    for (int i = 0; i < MAX_USER_IN_PARTY; i++) {
        if ((this->m_PartyS[party_number].Number[i] >= 0) && (this->m_PartyS[party_number].Number[i] == usernumber) &&

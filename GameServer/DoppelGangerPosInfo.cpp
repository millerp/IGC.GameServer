...
void CDoppelGangerPosInfo::GetCenterPosition(int nMapNumber, int nPosIndex, BYTE *btX, BYTE *btY) {
    BYTE btTmpX = this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][nPosIndex][2] -
                  this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][nPosIndex][0];
    *btX = btTmpX / 2 + this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][nPosIndex][0];
    BYTE btTmpY = this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][nPosIndex][3] -
                  this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][nPosIndex][1];
...
int CDoppelGangerPosInfo::GetPosIndex(int nMapNumber, BYTE btX, BYTE btY) {
    for (int i = 0; i < MAX_DOPPELGANGER_POS_INFO; i++) {
        if (btX > this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][i][0] &&
            btX <= this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][i][2] &&
            btY > this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][i][1] &&
            btY <= this->m_arDGMapInfo[nMapNumber - MAP_INDEX_DOUBLE_GOER1][i][3]) {
            return i;
        }
    }

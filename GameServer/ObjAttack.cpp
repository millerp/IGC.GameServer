...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf *lpMagic, int magicsend, unsigned char MSBFlag,
                        int AttackDamage, BOOL bCombo, BYTE RFAttack, BYTE byReflect, BYTE byPentagramAttack) {
    ...
    if (g_MineSystem.IsTwinkle(lpTargetObj->Class)) {
        g_Log.AddC(TColor::Red, "[Anti-Hack] [%d][%s][%s] Attempt to attack Pandora Twinkle", lpObj->m_Index,
                   lpObj->AccountID, lpObj->Name);
        AntiHackLog->Output("[Anti-Hack] [%d][%s][%s] Attempt to attack Pandora Twinkle", lpObj->m_Index,
                            lpObj->AccountID, lpObj->Name);
        return FALSE;
    }

    if (lpTargetObj->Class == 104 || lpTargetObj->Class == 105 || lpTargetObj->Class == 106 ||
        lpTargetObj->Class == 523 || lpTargetObj->Class == 689) {
        return FALSE;
    }
    ...
    if (g_ArcaBattle.IsArcaBattleServer() == TRUE && g_AcheronGuardianEvent.IsPlayStart() == false) {
        if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER &&
            g_ArcaBattle.IsPkEnable(lpObj, lpTargetObj) == FALSE) {
            return FALSE;
        }
    ...
    if (AttackDamage == 0) {
        if (skill != 76) {
            if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_INFINITY_ARROW) == FALSE &&
                gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_INFINITY_ARROW_STR) == FALSE) {
    ...
    } else if (lpObj->pInventory[1].m_Type == ITEMGET(2, 5) &&
               lpObj->pInventory[1].m_IsValidItem != false)    // Crystal Sword
    {
        if ((rand() % 20) == 0) {
            skill = 7;
            lpMagic = &DefMagicInf[7];
    ...
    if (AttackDamage > 0 || bPentagramBossMonster) {
        ...
        if (lpObj->Type != OBJ_USER || lpTargetObj->Type != OBJ_USER) {
            ...
            if (skill == 265 || skill == 564 || skill == 566) {
                ...
                gObjUseSkill.m_Lua.Generic_Call("DragonSlasher_DecreaseVictimSD", "i>ii", Energy, &nSuccessRate,
                                                &nDecreaseSDRate);
        ...
        if (g_ShieldSystemOn == TRUE) {
            if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
                ...
                if ((lpObj->Type != OBJ_MONSTER || !lpObj->m_iPentagramMainAttribute) &&
                    !this->MissCheck(lpObj, lpTargetObj, skill, skillSuccess, magicsend, bAllMiss, RFAttack)) // ?
                {
                    if (lpObj->m_iPentagramMainAttribute > 0 && byPentagramAttack == TRUE) {
                        this->PentagramAttack(lpObj, lpTargetObj, lpMagic, 0, MsgDamage, 0, 0);
                    }
        ...
        if (lpTargetObj->Type == OBJ_USER && (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_BERSERKER_STR) ||
                                              gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_BERSERKER_MAS) ||
                                              gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_BERSERKER_PRO))) {
            float fDefense = Dexterity / 3;
            float fValue = (float) gObjGetTotalValueOfEffect(lpTargetObj, 32);
        ...
        if (lpTargetObj->Type == OBJ_USER && lpTargetObj->pInventory[7].IsItem() == TRUE &&
            lpTargetObj->pInventory[7].m_IsValidItem == true) {
        ...
        } else if (skill == 76 && lpObj->Type == OBJ_USER)    // pluzzmanton
        {
            ...
            if (lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_RAGEFIGHTER) {
                iFenrirAttackDmg = lpObj->m_PlayerData->Strength / 3 + lpObj->m_PlayerData->Dexterity / 5 +
                                   lpObj->m_PlayerData->Vitality / 5 + lpObj->m_PlayerData->Energy / 7;    // #formula
            } else if (lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_SUMMONER) {
                iFenrirAttackDmg = lpObj->m_PlayerData->Strength / 5 + lpObj->m_PlayerData->Dexterity / 5 +
                                   lpObj->m_PlayerData->Vitality / 7 + lpObj->m_PlayerData->Energy / 3;    // #formula
            } else if (lpObj->Class == CLASS_ELF) {
                iFenrirAttackDmg = lpObj->m_PlayerData->Strength / 5 + lpObj->m_PlayerData->Dexterity / 3 +
                                   lpObj->m_PlayerData->Vitality / 7 + lpObj->m_PlayerData->Energy / 5;    // #formula
            } else    // Dark Lord
            {
                iFenrirAttackDmg = lpObj->m_PlayerData->Strength / 5 + lpObj->m_PlayerData->Dexterity / 5 +
                                   lpObj->m_PlayerData->Vitality / 7 + lpObj->m_PlayerData->Energy / 3 +
                                   lpObj->Leadership / 3;    // #formula
            }
            ...
            } else if (MsgDamage == 2)    // Excellent
            {
                ...
                AttackDamage = (iFenrirAttackDmg + lpMagic->m_DamageMin) +
                               (rand() % (lpMagic->m_DamageMax - lpMagic->m_DamageMin + 1));
                AttackDamage -= targetdefense;
            }
        } else {
        ...
        if (MsgDamage == 3) {
            ...
            if (g_CMuunSystem.GetMuunItemValueOfOptType(lpObj, MUUN_INC_CRITICAL_DAMAGE, &iMuunEffectValue, 0) ==
                TRUE) {
                AttackDamage += iMuunEffectValue;
            }

            if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER &&
                g_CMuunSystem.GetMuunItemValueOfOptType(lpObj, MUUN_INC_CRITICAL_DAMAGE_NONPVP, &iMuunEffectValue, 0) ==
                TRUE) {
                AttackDamage += iMuunEffectValue;
            }
        } else if (MsgDamage == 2) {
            ...
            if (g_CMuunSystem.GetMuunItemValueOfOptType(lpObj, MUUN_INC_EXCELLENT_DAMAGE, &iMuunEffectValue, 0) ==
                TRUE) {
                AttackDamage += iMuunEffectValue;
            }

            if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER &&
                g_CMuunSystem.GetMuunItemValueOfOptType(lpObj, MUUN_INC_EXCELLENT_DAMAGE_NONPVP, &iMuunEffectValue,
                                                        0) == TRUE) {
                AttackDamage += iMuunEffectValue;
            }
        }
        ...
        if (gObjSatanSprite(lpObj) == TRUE) {
            if (lpObj->Class != CLASS_RAGEFIGHTER) {
                ...
                if (skill == 263 ||
                    ...
                    lpObj->Life -= 100.0f;
                } else if (skill == 260 || skill == 261 || skill == 264 || skill == 551 || skill == 554 ||
                           skill == 552 || skill == 555 || skill == 560 || skill == 561 || skill == 270) {
                    lpObj->Life -= 2.0f;
                } else {
                    lpObj->Life -= 3.0f;
                }
        ...
        if (AttackDamage > 1) {
            ...
            if (g_CMuunSystem.GetMuunItemValueOfOptType(lpTargetObj, MUUN_DEC_ENEMY_ATTACK_SKILL, &iEffectValue,
                                                        NULL) == TRUE) {
        ...
        if (gObjWingSprite(lpObj) == TRUE) {
            ...
            g_ConfigRead.m_ItemCalcLua.Generic_Call("Wings_DecreaseHP", "iiii>i", Wing->m_Type,
                                                    (int) Wing->m_Durability, lpObj->Class, skill, &LifeDec);
            lpObj->Life -= LifeDec;

            if (lpObj->Life < 0.0f) {
                lpObj->Life = 0.0f;
            } else {
                g_ConfigRead.m_ItemCalcLua.Generic_Call("Wings_CalcIncAttack", "iii>i", AttackDamage, Wing->m_Type,
                                                        Wing->m_Level, &AttackDamage);
            }
        ...
        if (lpTargetObj->Live) {
            switch (skill) {
                ...
                case 55:
                case 57:
                    gObjUseSkill.m_Lua.Generic_Call("Knight_Gladiator_CalcSkillBonus", "iii>i", lpObj->Class,
                                                    AttackDamage, Energy, &AttackDamage);
                    break;
                ...
                case 47:
                    if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type == ITEMGET(13, 3) ||
                        ...
                        gObjUseSkill.m_Lua.Generic_Call("ImpaleSkillCalc", "iii>i", lpObj->Class, AttackDamage, Energy,
                                                        &AttackDamage);
                    }
                    break;
                ...
                case 24:
                    if (gObjUseSkill.m_SkillData.UseCalcForTripleShot == true) {
                        gObjUseSkill.m_Lua.Generic_Call("Elf_CalcTripleShot", "iiiii>i", AttackDamage, Strength,
                                                        Dexterity, Vitality, Energy, &AttackDamage);
                    }
                    break;
                ...
                case 76:
                    ...
                    gObjUseSkill.m_Lua.Generic_Call("FenrirSkillCalc", "iii>i", AttackDamage, lpObj->Level,
                                                    lpObj->m_PlayerData->MasterLevel, &AttackDamage);
                    break;
                case 214: {
                    int LifeChange = 0;

                    if (lpTargetObj->Type == OBJ_MONSTER) {
                        gObjUseSkill.m_Lua.Generic_Call("SummonerDrainLife_Monster", "ii>i", Energy, lpTargetObj->Level,
                                                        &LifeChange);
                    } else if (lpTargetObj->Type == OBJ_USER) {
                        gObjUseSkill.m_Lua.Generic_Call("SummonerDrainLife_PvP", "ii>i", Energy, AttackDamage,
                                                        &LifeChange);
                    }
                ...
                case 219: {
                    ...
                    if (lpTargetObj->Type == OBJ_MONSTER) {
                        gObjUseSkill.m_Lua.Generic_Call("Sleep_Monster", "iii>ii", Energy, lpObj->m_CurseSpell,
                                                        lpTargetObj->Level, &SuccessRate, &Time);
                    } else if (lpTargetObj->Type == OBJ_USER) {
                        gObjUseSkill.m_Lua.Generic_Call("Sleep_PvP", "iiii>ii", Energy, lpObj->m_CurseSpell,
                                                        lpObj->Level, lpTargetObj->Level, &SuccessRate, &Time);
                    }

                    if (SuccessRate >= rand() % 100) {
                        if (Time >= 0) {
                            if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE) == false &&
                                gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE_STR) == false) {
                ...
                case 221: {
                    ...
                    if (lpTargetObj->Type == OBJ_MONSTER) {
                        gObjUseSkill.m_Lua.Generic_Call("SummonerWeakness_Monster", "iii>iii", Energy,
                                                        lpObj->m_CurseSpell, lpTargetObj->Level, &SuccessRate, &Value,
                                                        &Time);
                    } else if (lpTargetObj->Type == OBJ_USER) {
                        gObjUseSkill.m_Lua.Generic_Call("SummonerWeakness_PvP", "iiii>iii", Energy, lpObj->m_CurseSpell,
                                                        lpObj->Level, lpTargetObj->Level, &SuccessRate, &Value, &Time);
                    }
                    ...
                    if (SuccessRate < rand() % 100) {
                        ...
                        if (Time >= 1) {
                            if (gObjCheckPowerfulEffect(lpTargetObj, BUFFTYPE_CURSED_ATTACK_DOWN, Value, 0) == TRUE) {
                                ...
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_CURSED_ATTACK_DOWN,
                                                  EFFECTTYPE_CURSED_ATTACKDOWN, Value, 0, 0, Time);
                                return 1;
                            }
                        } else {
                            GSProtocol.GCDamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, 0, 0);
                ...
                case 222: {
                    ...
                    if (lpTargetObj->Type == OBJ_MONSTER) {
                        gObjUseSkill.m_Lua.Generic_Call("SummonerInnovation_Monster", "iii>iii", Energy,
                                                        lpObj->m_CurseSpell, lpTargetObj->Level, &SuccessRate, &Value,
                                                        &Time);
                    } else if (lpTargetObj->Type == OBJ_USER) {
                        gObjUseSkill.m_Lua.Generic_Call("SummonerInnovation_PvP", "iiii>iii", Energy,
                                                        lpObj->m_CurseSpell, lpObj->Level, lpTargetObj->Level,
                                                        &SuccessRate, &Value, &Time);
                    }
                    ...
                    if (SuccessRate < rand() % 100) {
                        ...
                        if (Time >= 1) {
                            if (gObjCheckPowerfulEffect(lpTargetObj, BUFFTYPE_CURSED_DEFENSE_DOWN, Value, 0) == TRUE) {
                                ...
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_CURSED_DEFENSE_DOWN,
                                                  EFFECTTYPE_CURSED_DEFENSEDOWN, Value, 0, 0, Time);
                                return 1;
                            }
                        } else {
                            GSProtocol.GCDamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, 0, 0);
                ...
                case 223: {
                    lpTargetObj->lpAttackObj = lpObj;
                    int DOT = 0, Time = 0;
                    gObjUseSkill.m_Lua.Generic_Call("ExplosionDotDamage", "ii>ii", AttackDamage,
                                                    lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncDotDamage, &DOT, &Time);
                ...
                case 232:
                    ...
                    lpTargetObj->DelayLevel = 1;
                    lpTargetObj->DelayActionTime = 1000;
                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_COLD, EFFECTTYPE_REDUCE_MOVE_SPEED, 0, 0, 0,
                                      10); // 20 -> 10, previous was wrong
                    gObjUseSkill.m_Lua.Generic_Call("StrikeOfDestructionCalc", "ii>i", AttackDamage, Energy,
                                                    &AttackDamage);
                    break;
                case 236:
                ...
                case 238: {
                    ...
                    if (lpMagic->m_Skill == 238 && lpObj->Class == 561 &&
                        gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_POISON) == FALSE) {
                ...
                case 253: // selupan
                    ...
                    break;
                case 260:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterKillingBlow", "ii>i", AttackDamage, Vitality,
                                                    &AttackDamage);
                    break;
                case 261:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterBeastUppercut", "ii>i", AttackDamage, Vitality,
                                                    &AttackDamage);
                    break;
                case 262:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterChainDrive", "ii>i", AttackDamage, Vitality,
                                                    &AttackDamage);
                    break;
                case AT_SKILL_DARK_SIDE:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterDarkSideIncDamage", "iii>i", AttackDamage, Dexterity,
                                                    Energy, &AttackDamage);
                    break;
                case AT_SKILL_DRAGON_ROAR:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterDragonRoar", "ii>i", AttackDamage, Energy,
                                                    &AttackDamage);
                    break;
                case AT_SKILL_DRAGON_SLASH:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterDragonSlasher", "iii>i", AttackDamage, Energy,
                                                    lpTargetObj->Type, &AttackDamage);
                    break;
                case AT_SKILL_PHOENIX_SHOT:
                    gObjUseSkill.m_Lua.Generic_Call("RageFighterPhoenixShot", "ii>i", AttackDamage, Vitality,
                                                    &AttackDamage);
                    break;
                ...
                case AT_SKILL_SPIN_STEP:
                    gObjUseSkill.m_Lua.Generic_Call("GrowLancerSpinStep", "ii>i", AttackDamage, Strength,
                                                    &AttackDamage);
                    break;
                case AT_SKILL_HARSH_STRIKE:
                    gObjUseSkill.m_Lua.Generic_Call("GrowLancerHarshStrike", "ii>i", AttackDamage, Dexterity,
                                                    &AttackDamage);
                    break;
                case AT_SKILL_MAGIC_PIN:
                case AT_SKILL_MAGIC_PIN_EXPLOSION:
                    gObjUseSkill.m_Lua.Generic_Call("GrowLancerMagicPin", "ii>i", AttackDamage, Strength,
                                                    &AttackDamage);
                    break;
                ...
                case AT_SKILL_SHINING_PEAK:
                    gObjUseSkill.m_Lua.Generic_Call("GrowLancerShiningPeak", "iii>i", AttackDamage, Strength, Dexterity,
                                                    &AttackDamage);
                    break;
                case AT_SKILL_CLASH: {
                    int iAccessoryEffect = 0;
                ...
                default: {
                    if (g_MasterLevelSkillTreeSystem.CheckMasterLevelSkill(skill) == true) {
                        double fSkillValue = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
                        ...
                        switch (iMLSBase) {
                            case 330:
                            case 332:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("TwistingSlashCalc_Master_Knight",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 481:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("TwistingSlashCalc_Master_Gladiator",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 326:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("CycloneCalc_Master_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 479:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("CycloneCalc_Master_Gladiator", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 327:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SlashCalc_Master_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 328:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FallingSlashCalc_Master_Knight",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 329:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("LungeCalc_Master_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 337:
                                lpTargetObj->lpAttackObj = lpObj;
                                ...
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_COLD, EFFECTTYPE_REDUCE_MOVE_SPEED, 0, 0, 0,
                                                  10);
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call(
                                        "StrikeOfDestructionCalc_MasterLevel1_Knight", "ii>i", AttackDamage, Energy,
                                        &AttackDamage);
                                break;
                            case 344:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("BloodStormCalc_MasterLevel1_Knight",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 346:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("BloodStormCalc_MasterLevel2_Knight",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 495: {
                                int DebuffRate = 0, DebuffTime = 0;
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("EarthPrison_Level1", ">ii",
                                                                                &DebuffRate, &DebuffTime);
                            ...
                            case 497: {
                                int DebuffRate = 0, DebuffTime = 0;
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("EarthPrison_Level2", ">ii",
                                                                                &DebuffRate, &DebuffTime);
                            ...
                            case 427:
                                if (skillSuccess == TRUE) {
                                    ...
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call(
                                            "PoisonArrow_MasterLevel1_DotDamage", "i>dd", lpObj->m_AttackDamageMinRight,
                                            &fDmgValue, &fTime);
                                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_DEBUFF_POISON1, EFFECTTYPE_GIVE_DMG_TICK,
                                                      fDmgValue, 0, 0, fTime);
                                }
                                break;
                            case 434:
                                if (skillSuccess == TRUE) {
                                    ...
                                    double fDmgValue = 0.0, fTime = 0.0;
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call(
                                            "PoisonArrow_MasterLevel2_DotDamage", "id>dd",
                                            lpObj->m_AttackDamageMinRight, fSkillValue, &fDmgValue, &fTime);
                                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_DEBUFF_POISON2, EFFECTTYPE_GIVE_DMG_TICK,
                                                      fDmgValue, 0, 0, fTime);
                                }
                                break;
                            case 414:
                                if (gObjUseSkill.m_SkillData.UseCalcForTripleShot == true) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("Elf_CalcTripleShot_Level1",
                                                                                    "iiiii>i", AttackDamage, Strength,
                                                                                    Dexterity, Vitality, Energy,
                                                                                    &AttackDamage);
                                }
                                break;
                            case 418:
                                if (gObjUseSkill.m_SkillData.UseCalcForTripleShot == true) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("Elf_CalcTripleShot_Level2",
                                                                                    "iiiii>i", AttackDamage, Strength,
                                                                                    Dexterity, Vitality, Energy,
                                                                                    &AttackDamage);
                                }
                                break;
                            case 416:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("PenetrationCalc_Master_Elf", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 424:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("IceArrowCalc_Master_Elf", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 516:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("EarthShake_MasterLevel2_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                if (fSkillValue > 0) {
                                    int iAccessoryEffect = 0;
                                ...
                                break;
                            case 514:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FireBurst_MasterLevel2_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                if (fSkillValue > 0) {
                                    int iAccessoryEffect = 0;
                                ...
                                break;
                            case 509:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("ForceWave_Master_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 512:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("EarthShake_MasterLevel1_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 508:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FireBurst_MasterLevel1_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 336:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("DeathStab_MasterLevel1_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 331:
                            case 333:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("RagefulBlow_Master_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 490:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FireSlash_MasterLevel1_Gladiator",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 482:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("PowerSlash_Master_Gladiator", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 518:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FireScream_MasterLevel1_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 551:
                            case 554:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("KillingBlow_Master", "ii>i",
                                                                                AttackDamage, Vitality, &AttackDamage);
                                break;
                            case 552:
                            case 555:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("BeastUppercut_Master", "ii>i",
                                                                                AttackDamage, Vitality, &AttackDamage);
                                break;
                            case 558:
                            case 562:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("ChainDrive_Master", "ii>i",
                                                                                AttackDamage, Vitality, &AttackDamage);
                                break;
                            case 559:
                            case 563:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("RageFighterDarkSideIncDamage", "iii>i",
                                                                                AttackDamage, Dexterity, Energy,
                                                                                &AttackDamage);
                                break;
                            case 560:
                            case 561:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("DragonRoar_Master", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 564:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("DragonSlasher_Level1", "idii>i",
                                                                                AttackDamage, fSkillValue, Energy,
                                                                                lpTargetObj->Type, &AttackDamage);
                                break;
                            case 566: {
                                fPrevBase = g_MasterLevelSkillTreeSystem.GetBrandOfMasterSkillValue(lpObj, skill, 1);
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("DragonSlasher_Level2", "idii>i",
                                                                                AttackDamage, fPrevBase, Energy,
                                                                                lpTargetObj->Type, &AttackDamage);
                            ...
                            case 394:
                                ...
                                break;
                            case 339:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("DeathStab_MasterLevel2_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 342:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("DeathStab_MasterLevel3_Knight", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 340:
                                lpTargetObj->lpAttackObj = lpObj;
                                ...
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_COLD, EFFECTTYPE_REDUCE_MOVE_SPEED, 0, 0, 0,
                                                  10);
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call(
                                        "StrikeOfDestructionCalc_MasterLevel2_Knight", "ii>i", AttackDamage, Energy,
                                        &AttackDamage);
                                break;
                            case 343:
                                lpTargetObj->lpAttackObj = lpObj;
                                ...
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_COLD, EFFECTTYPE_REDUCE_MOVE_SPEED, 0, 0, 0,
                                                  10);
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call(
                                        "StrikeOfDestructionCalc_MasterLevel3_Knight", "ii>i", AttackDamage, Energy,
                                        &AttackDamage);
                                break;
                            case 431:
                                if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE) == false &&
                            ...
                            case 459: {
                                ...
                                if (lpTargetObj->Type == OBJ_MONSTER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerWeakness_Monster",
                                                                                    "iii>iii", Energy,
                                                                                    lpObj->m_CurseSpell,
                                                                                    lpTargetObj->Level, &iSuccessRate,
                                                                                    &iEffectValue, &iEffectTime);
                                } else if (lpTargetObj->Type == OBJ_USER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerWeakness_PvP", "iiii>iii",
                                                                                    Energy, lpObj->m_CurseSpell,
                                                                                    lpObj->Level, lpTargetObj->Level,
                                                                                    &iSuccessRate, &iEffectValue,
                                                                                    &iEffectTime);
                                }
                                ...
                                if (gObjCheckPowerfulEffect(lpTargetObj, BUFFTYPE_WEAKEN, iEffectValue, 0) == TRUE) {
                                    ...
                                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_WEAKEN, EFFECTTYPE_CURSED_ATTACKDOWN,
                                                      iEffectValue, 0, 0, iEffectTime);
                                    return TRUE;
                                }
                            }
                                break;
                            case 460: {
                                ...
                                if (lpTargetObj->Type == OBJ_MONSTER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerInnovation_Monster",
                                                                                    "iii>iii", Energy,
                                                                                    lpObj->m_CurseSpell,
                                                                                    lpTargetObj->Level, &iSuccessRate,
                                                                                    &iEffectValue, &iEffectTime);
                                } else if (lpTargetObj->Type == OBJ_USER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerInnovation_PvP",
                                                                                    "iiii>iii", Energy,
                                                                                    lpObj->m_CurseSpell, lpObj->Level,
                                                                                    lpTargetObj->Level, &iSuccessRate,
                                                                                    &iEffectValue, &iEffectTime);
                                }
                                ...
                                if (gObjCheckPowerfulEffect(lpTargetObj, BUFFTYPE_INNOVATE, iEffectValue, 0) == TRUE) {
                                    ...
                                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_INNOVATE, EFFECTTYPE_CURSED_DEFENSEDOWN,
                                                      iEffectValue, 0, 0, iEffectTime);
                                    return TRUE;
                                }
                            }
                            ...
                            case 463:
                                ...
                                return 0;
                            case 492:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FlameStrike_MasterLevel1_Gladiator",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_FLAMESTRKE, 0, 0, 0, 0, 1);
                                break;
                            case 494:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FlameStrike_MasterLevel2_Gladiator",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_FLAMESTRKE, 0, 0, 0, 0, 1);
                                break;
                            case 493:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FireSlash_MasterLevel2_Gladiator",
                                                                                "ii>i", AttackDamage, Energy,
                                                                                &AttackDamage);
                                break;
                            case 496:
                                ...
                                break;
                            case 519:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("ElectricSpark_Master_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 520:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("FireScream_MasterLevel2_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 523:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("ChaoticDiseier_Master_Lord", "ii>i",
                                                                                AttackDamage, Energy, &AttackDamage);
                                break;
                            case 462: {
                                fPrevBase = g_MasterLevelSkillTreeSystem.GetBrandOfMasterSkillValue(lpObj, skill, 1);
                                ...
                                if (lpTargetObj->Type == OBJ_MONSTER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerDrainLife_Monster_Level2",
                                                                                    "ii>i", Energy, lpTargetObj->Level,
                                                                                    &nAddHP2);
                                } else if (lpTargetObj->Type == OBJ_USER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerDrainLife_PvP_Level2",
                                                                                    "ii>i", Energy, AttackDamage,
                                                                                    &nAddHP2);
                                }
                                ...
                                if (rand() % 100 <= 30) {
                                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_VAMPIRIC, EFFECTTYPE_FILLHP, fSkillValue, 0,
                                                      lpObj->m_Index, 5);
                                    lpTargetObj->lpAttackObj = lpObj;
                                }
                            }
                            ...
                            case 454: {
                                ...
                                if (lpTargetObj->Type == OBJ_MONSTER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("Sleep_Monster", "iii>ii", Energy,
                                                                                    lpObj->m_CurseSpell,
                                                                                    lpTargetObj->Level, &iSuccessRate,
                                                                                    &iEffectTime);
                                } else if (lpTargetObj->Type == OBJ_USER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("Sleep_PvP", "iiii>ii", Energy,
                                                                                    lpObj->m_CurseSpell, lpObj->Level,
                                                                                    lpTargetObj->Level, &iSuccessRate,
                                                                                    &iEffectTime);
                                }
                                ...
                                if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE) == true ||
                                    gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_IRON_DEFENSE_STR) == true) {
                                    GSProtocol.GCDamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, 0, 0);
                                    return FALSE;
                                }
                                ...
                                if (gObjCheckPowerfulEffect(lpTargetObj, BUFFTYPE_SLEEP, iEffectTime, 0) == TRUE) {
                                    ...
                                    gObjAddBuffEffect(lpTargetObj, BUFFTYPE_SLEEP, EFFECTTYPE_SLEEP, iEffectTime, 0, 0,
                                                      iEffectTime);
                                    return TRUE;
                                }
                            }
                                break;
                            case 455:
                                g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("ChainLightning_Master_Summoner",
                                                                                "ii>i", AttackDamage, bCombo,
                                                                                &AttackDamage);
                                bCombo = 0;
                                break;
                            case 456:
                            ...
                            case 458: {
                                ...
                                if (lpTargetObj->Type == OBJ_MONSTER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerDrainLife_Monster_Level1",
                                                                                    "ii>i", Energy, lpTargetObj->Level,
                                                                                    &nAddHP);
                                } else if (lpTargetObj->Type == OBJ_USER) {
                                    g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("SummonerDrainLife_PvP_Level1",
                                                                                    "ii>i", Energy, AttackDamage,
                                                                                    &nAddHP);
                                }
                        ...
                        if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE &&
                            ...
                            if (lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide) {
                                if (g_CastleSiege.m_bCastleSiegeFriendlyFire == false) {
                                    ...
                                    AttackDamage =
                                            AttackDamage * g_CastleSiege.CastleSiegeSelfDmgReductionPercent / 100;
                                }
                            } else {
                                AttackDamage = AttackDamage * g_CastleSiege.CastleSiegeDmgReductionPercent / 100;
                            }
            ...
            if (g_CMuunSystem.GetMuunItemValueOfOptType(lpTargetObj, MUUN_DEC_ENEMY_DAMAGE, &nMuunEffectValue, 0) ==
                TRUE) {
            ...
            if (skill == 76) {
                if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
                    if (AttackDamage > 0) {
                        if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE || (lpObj->m_btCsJoinSide == FALSE ||
                                                                                     (lpObj->m_btCsJoinSide !=
                                                                                      lpTargetObj->m_btCsJoinSide))) {
                            ...
                            if (lpEquipment && lpEquipment->IsItem()) {
                                ...
                                if (lpTargetObj->Class == CLASS_RAGEFIGHTER) {
                                    if (lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsDownDur1_Monk > 0.0) {
                                        iDurDecValue =
                                                lpTargetObj->m_PlayerData->m_MPSkillOpt.btMpsDownDur1Level_Monk + 50;
                                    }
                                ...
                                for (int iBuffIndex = 0; iBuffIndex < lpTargetObj->m_BuffEffectCount; iBuffIndex++) {
                                    if (lpTargetObj->m_BuffEffectList[iBuffIndex].BuffIndex ==
                                        BUFFTYPE_CHARM_PROTECTITEM) {
                                ...
                                GSProtocol.GCItemDurSend(lpTargetObj->m_Index, iEquipmentPos, lpEquipment->m_Durability,
                                                         0);
                            }
                        }
                    }
    ...
    if (AttackDamage >= 1) {
        ...
        if (lpTargetObj->Type == OBJ_USER) {
            ...
            if (skill == 333) {
                float fDurDownRate = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(
                        MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
                ...
                if (rand() % 100 < fDurDownRate) {
                    if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE ||
                        ...
                        if (lpEquipment && lpEquipment->IsItem()) {
                            ...
                            if (lpTargetObj->Class == CLASS_RAGEFIGHTER &&
                                lpTargetObj->m_PlayerData->m_MPSkillOpt.iMpsDownDur1_Monk > 0.0) {
                            ...
                            GSProtocol.GCItemDurSend(lpTargetObj->m_Index, item_num[iEquipmentPos],
                                                     (BYTE) lpEquipment->m_Durability, 0);
                        }
                    }
                }
            }
    ...
    if (lpTargetObj->Type == OBJ_USER) {
        if (lpTargetObj->pInventory[lpTargetObj->m_btInvenPetPos].IsItem() == TRUE &&
            lpTargetObj->pInventory[lpTargetObj->m_btInvenPetPos].m_Type == ITEMGET(13, 37)) {
    ...
    if (AttackDamage > 0) {
        ...
        if (bCombo) {
            int iComboDamage = 0;
            gObjUseSkill.m_Lua.Generic_Call("ComboSkillDamage", "iiii>i", Strength, Dexterity, Vitality, Energy,
                                            &iComboDamage);
        ...
        if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_BLOOD_HOWL) == true) {
            ...
            g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("BloodHowl_Level1", "d>iii", lpTargetObj->Life,
                                                            &SkillEffect, &SkillSuccessRate, &SkillTime);

            if (rand() % 100 < SkillSuccessRate) {
                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_BLEEDING, EFFECTTYPE_GIVE_DMG_TICK, SkillEffect, 0, 0,
                                  SkillTime);
            }
        }
        ...
        if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_BLOOD_HOWL_STR) == true) {
            ...
            g_MasterLevelSkillTreeSystem.m_Lua.Generic_Call("BloodHowl_Level2", "d>iii", lpTargetObj->Life,
                                                            &SkillEffect, &SkillSuccessRate, &SkillTime);
        ...
        if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_CIRCLE_SHIELD) ||
            gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_CIRCLE_SHIELD_STR) ||
            gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_CIRCLE_SHIELD_MAS)) {
        ...
        if (lpObj->Type == OBJ_USER) {
            ...
            if (lpObj->Class == CLASS_RAGEFIGHTER) {
                fRate = lpObj->m_PlayerData->SetOpDoubleDamage + lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddDoubleDamage
                        + lpObj->m_PlayerData->m_MPSkillOpt.iMpsMonkDoubleDamage +
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncDoubleDamageRate_Monk;
            } else {
                fRate = lpObj->m_PlayerData->SetOpDoubleDamage + lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddDoubleDamage +
                        lpObj->m_PlayerData->m_MPSkillOpt.iMpsIncDoubleDamageRate;
            }
            ...
            if (rand() % 100 < fRate) {
                if (skill == 78 || g_MasterLevelSkillTreeSystem.GetBaseMasterLevelSkill(skill) == 518 || skill == 520) {
                    ...
                    if (lpMagic->m_Skill == 520) {
                        iAddDamage = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(
                                MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
                    }
        ...
        if (iTotalShieldDamage > 0) {
            g_Log.Add("[PvP System] Victim:[%s][%s], Attacker:[%s][%s] - SD[%d] HP[%f] -> SD[%d] HP[%f]",
                      lpTargetObj->AccountID, lpTargetObj->Name, lpObj->AccountID, lpObj->Name,
                      lpTargetObj->iShield + iTotalShieldDamage, lpTargetObj->Life + AttackDamage, lpTargetObj->iShield,
                      lpTargetObj->Life);
        }
    } else {
    ...
    if (lpMagic) {
        if (lpMagic->m_Skill == 332) {
            float fSpringValue = g_MasterLevelSkillTreeSystem.GetMasterSkillValue(
                    MagicDamageC.SkillGet(lpMagic->m_Skill), lpMagic->m_Level);
...
BOOL CObjAttack::PentagramAttack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf *lpMagic, BYTE MSBFlag, int MsgDamage,
                                 int AttackDamage, int iTargetDefense) {
    ...
    if ((lpObj->Type == OBJ_USER && lpMagic) || lpObj->Type == OBJ_MONSTER) {
        if (lpObj->Type == OBJ_USER && g_PentagramSystem.IsPentagramItem(lpObj->pInventory[236].m_Type) &&
            lpObj->pInventory[236].m_IsValidItem) {
            bPentagramEquip = true;
            PentagramAttackDamage = this->GetElementalDamage(lpObj, lpTargetObj, &PentagramDamageType1,
                                                             &PentagramDamageType2, AttackDamage, iTargetDefense);
            ...
            if (lpObj->m_PlayerData->m_PentagramOptions.m_iOnyx_5RankHalfValueSkillRate > 0) {
                if (rand() % 100 < lpObj->m_PlayerData->m_PentagramOptions.m_iOnyx_5RankHalfValueSkillRate) {
                    ...
                    switch (PentagramDamageType1) {
                        case EL_FIRE:
                            gObjUseSkill.m_Lua.Generic_Call("ElementalDebuffGetDuration", "ii>i", BUFFTYPE_ACHERON_FIRE,
                                                            (lpObj->Level + lpObj->m_PlayerData->MasterLevel),
                                                            &iDuration);
                            gObjAddBuffEffect(lpTargetObj, BUFFTYPE_ACHERON_FIRE, 83, 0, 0, 0, iDuration);
                            gObjCalCharacter.CalcCharacter(lpTargetObj->m_Index);
                            break;
                        case EL_WATER:
                            gObjUseSkill.m_Lua.Generic_Call("ElementalDebuffGetDuration", "ii>i",
                                                            BUFFTYPE_ACHERON_FROST,
                                                            (lpObj->Level + lpObj->m_PlayerData->MasterLevel),
                                                            &iDuration);
                        ...
                        case EL_EARTH:
                            ...
                            break;
                        case EL_WIND:
                            gObjUseSkill.m_Lua.Generic_Call("ElementalDebuffGetDuration", "ii>i", BUFFTYPE_ACHERON_BIND,
                                                            (lpObj->Level + lpObj->m_PlayerData->MasterLevel),
                                                            &iDuration);
...
BOOL CObjAttack::GetBuffTypePhysicalIncrease(LPOBJ lpObj, int *iAttackBerserkerMin, int *iAttackBerserkerMax,
                                             int nBuffType) {
    gObjUseSkill.m_Lua.Generic_Call("SummonerBerserkerAttackDamage", "ii>ii",
                                    (lpObj->m_PlayerData->Strength + lpObj->AddStrength),
                                    (lpObj->m_PlayerData->Dexterity + lpObj->AddDexterity), iAttackBerserkerMin,
                                    iAttackBerserkerMax);
...
int CObjAttack::GetAttackDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int targetDefense, int &effect, CMagicInf *lpMagic) {
    ...
    if (lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC) {
        ...
        if (lpMagic) {
            if ((lpMagic->m_Skill == 60 || lpMagic->m_Skill == 509) &&
                lpObj->SkillLongSpearChange)    // #error 60 and skil from GEt is 66???
            {
            ...
            if (g_CMuunSystem.GetMuunItemValueOfOptType(lpObj, MUUN_INC_SKILL_POWER, &nMuunItemEffectValue, 0) ==
                TRUE) {
            ...
            if (lpObj->Type == OBJ_USER && lpTargetObj->Type != OBJ_USER) {
                ...
                nMuunItemEffectValue = 0;
                g_CMuunSystem.GetMuunItemValueOfOptType(lpObj, MUUN_INC_MAX_ATTACK_POWER_NONPVP, &nMuunItemEffectValue,
                                                        0);
            ...
            if (lpMagic->m_Skill == 65 || lpMagic->m_Skill == 519) {
                ...
                int addskilldamage =
                        (lpObj->Leadership + lpObj->AddLeadership) / 10 + (iPartyCount * 50);    // #formula
                ...
                SkillLeftMinDamage += addskilldamage;
                       ...
                       g_MasterLevelSkillTreeSystem.GetBaseMasterLevelSkill(lpMagic->m_Skill) == 512 ||
                       lpMagic->m_Skill == 516) {
                if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type == ITEMGET(13, 4))    // Dark Horse
                {
                    int addskilldamage = (lpObj->m_PlayerData->Strength + lpObj->AddStrength) / 10 +
                                         (lpObj->Leadership + lpObj->AddLeadership) / 5 +
                                         lpObj->pInventory[lpObj->m_btInvenPetPos].m_PetItem_Level * 10;    // #formula
                ...
                int iadddamage = (lpObj->m_PlayerData->Strength + lpObj->AddStrength) / 30 +
                                 (lpObj->m_PlayerData->Energy + lpObj->AddEnergy) / 55;    // #formula
                ...
                int iadddamage = (lpObj->m_PlayerData->Strength + lpObj->AddStrength) / 25 +
                                 (lpObj->m_PlayerData->Energy + lpObj->AddEnergy) / 50;    // #formula
            ...
            if (lpObj->Class == CLASS_ELF &&
                (lpMagic->m_Skill == 235 || lpMagic->m_Skill == 411 || lpMagic->m_Skill == 431)) //season4 add-on
            {
        ...
        __try
        {
            sub = (lpObj->m_AttackDamageMaxRight + SkillRightMaxDamage + iAttackBerserkerMax + iMuunOptDamage +
                   iMuunOptDamageMax) - (lpObj->m_AttackDamageMinRight + SkillRightMinDamage + iAttackBerserkerMin +
                                         iMuunOptDamage);    // #formula
            lpObj->m_AttackDamageRight =
                    (lpObj->m_AttackDamageMinRight + SkillRightMinDamage + iAttackBerserkerMin + iMuunOptDamage) +
                    (rand() % (sub + 1));
        }
        __except(sub = 1, 1)
        {
        ...
        __try
        {
            sub = (lpObj->m_AttackDamageMaxLeft + SkillLeftMaxDamage + iAttackBerserkerMax + iMuunOptDamage +
                   iMuunOptDamageMax) - (lpObj->m_AttackDamageMinLeft + SkillLeftMinDamage + iAttackBerserkerMin +
                                         iMuunOptDamage);    // #formula
            lpObj->m_AttackDamageLeft =
                    (lpObj->m_AttackDamageMinLeft + SkillLeftMinDamage + iAttackBerserkerMin + iMuunOptDamage) +
                    (rand() % (sub + 1));
        }
        __except(sub = 1, 1)
        {
        ...
        if (cDamage) {
            lpObj->m_AttackDamageRight =
                    lpObj->m_AttackDamageMaxRight + SkillRightMaxDamage + iAttackBerserkerMax + iMuunOptDamage +
                    iMuunOptDamageMax;
            lpObj->m_AttackDamageLeft =
                    lpObj->m_AttackDamageMaxLeft + SkillLeftMaxDamage + iAttackBerserkerMax + iMuunOptDamage +
                    iMuunOptDamageMax;
    ...
    if (lpObj->Type == OBJ_USER) {
        ...
        if (lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddControlAttack > 0.0) {
            if (lpObj->pInventory[0].GetDetailItemType() == ITEM_DARKLORD_SCEPTER ||
                lpObj->pInventory[1].GetDetailItemType() == ITEM_DARKLORD_SCEPTER) {
                int nAddAttack = (lpObj->AddLeadership + lpObj->Leadership) /
                                 lpObj->m_PlayerData->m_MPSkillOpt.iMpsAddControlAttack;
                ad += nAddAttack;
            }
        }

        if (lpObj->pInventory[7].IsItem() == TRUE && lpObj->pInventory[7].m_IsValidItem == true) {
            if (lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_ELF || lpObj->Class == CLASS_MAGUMSA ||
                lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_RAGEFIGHTER) {
...
int CObjAttack::GetAttackDamageSummoner(LPOBJ lpObj, LPOBJ lpTargetObj, int targetDefense, CMagicInf *lpMagic,
                                        int &effect) {
    ...
    switch (lpMagic->m_Skill) {
        ...
        case 460: {
            if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_BERSERKER) == true) {
                ...
                gObjUseSkill.m_Lua.Generic_Call("SummonerBerserkerCurseDamage", "ii>dd",
                                                (lpObj->m_PlayerData->Energy + lpObj->AddEnergy), iValue,
                                                &fCurseDamageMin, &fCurseDamageMax);
            }
            ...
            damagemin = lpObj->m_CurseDamageMin + fCurseDamageMin + lpMagic->m_DamageMin +
                        g_SkillSpec.CalcStatBonus(lpObj, lpMagic->m_Skill);
            damagemax = lpObj->m_CurseDamageMax + fCurseDamageMax + lpMagic->m_DamageMax +
                        g_SkillSpec.CalcStatBonus(lpObj, lpMagic->m_Skill);
            ...
            if (lpObj->pInventory[1].IsItem() &&
                (lpObj->pInventory[1].m_Type >= ITEMGET(5, 0) && lpObj->pInventory[1].m_Type < ITEMGET(6, 0)) &&
                lpObj->pInventory[1].m_IsValidItem) {
        ...
        default: {
            if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_BERSERKER) == true) {
                int iValue = gObjGetTotalValueOfEffect(lpObj, 31);
                gObjUseSkill.m_Lua.Generic_Call("SummonerBerserkerMagicDamage", "ii>dd",
                                                (lpObj->m_PlayerData->Energy + lpObj->AddEnergy), iValue,
                                                &fMagicDamageMin, &fMagicDamageMax);
            }
            ...
            if (lpObj->pInventory[0].IsItem() &&
                (lpObj->pInventory[0].m_Type >= ITEMGET(5, 0) && lpObj->pInventory[0].m_Type < ITEMGET(6, 0)) &&
                lpObj->pInventory[0].m_IsValidItem) {
    ...
    if (SkillAttr != -1) {
        if ((lpObj->Authority & 0x20) != 0x20 ||
            (lpObj->pInventory[10].m_Type != ITEMGET(13, 42) && lpObj->pInventory[11].m_Type != ITEMGET(13, 42))) {
...
int
CObjAttack::GetAttackDamageWizard(LPOBJ lpObj, LPOBJ lpTargetObj, int targetDefense, CMagicInf *lpMagic, int &effect) {
    if (lpObj->Type != OBJ_USER) {
        return 0;
    }
    ...
    if (lpMagic->m_Skill == 40) {
        if (lpObj->m_PlayerData->SkillHellFire2Count >= 0) {
            ...
            if (SkillAttr != -1) {
                if ((lpObj->Authority & 0x20) != 0x20 || (lpObj->pInventory[10].m_Type != ITEMGET(13, 42) &&
...
int
CObjAttack::GetElementalDamage(LPOBJ lpObj, LPOBJ lpTargetObj, char *DamageType1, char *DamageType2, int iAttackDamage,
                               int iTargetDefense) // 1.1.0.4 decompilation
{
    int Strength = 0;
    int Dexterity = 0;
    int Vitality = 0;
    ...
    if (lpObj->Type == OBJ_USER) {
        switch (lpObj->Class) {
            case CLASS_WIZARD:
                this->m_Lua.Generic_Call("WizardElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality,
                                         Energy, PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_KNIGHT:
                this->m_Lua.Generic_Call("KnightElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality,
                                         Energy, PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_ELF:
                this->m_Lua.Generic_Call("ElfElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality, Energy,
                                         PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_MAGUMSA:
                this->m_Lua.Generic_Call("GladiatorElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality,
                                         Energy, PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_DARKLORD:
                this->m_Lua.Generic_Call("LordElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality, Energy,
                                         PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_SUMMONER:
                this->m_Lua.Generic_Call("SummonerElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality,
                                         Energy, PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_RAGEFIGHTER:
                this->m_Lua.Generic_Call("RageFighterElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality,
                                         Energy, PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
            case CLASS_GROWLANCER:
                this->m_Lua.Generic_Call("GrowLancerElementalDamageCalc", "iiiiii>ii", Strength, Dexterity, Vitality,
                                         Energy, PentagramMinDamage, PentagramMaxDamage, &MinDamage, &MaxDamage);
                break;
        }
    }
    ...
    if (lpObj->Type == OBJ_USER) {
        ...
        } else if (lpObj->m_PlayerData->m_PentagramOptions.m_iRuby_3RankOptionNum == 2 &&
        ...
        if (lpObj->m_PlayerData->m_PentagramOptions.m_iRuby_4RankOptionNum == 1) {
            ...
            } else if (lpTargetObj->Class == CLASS_WIZARD || lpTargetObj->Class == CLASS_ELF ||
                       lpTargetObj->Class == CLASS_MAGUMSA || lpTargetObj->Class == CLASS_SUMMONER) {
                AddDamage += lpObj->m_PlayerData->m_PentagramOptions.m_iRuby_4RankAddDamage;
            }
        } else if (lpObj->m_PlayerData->m_PentagramOptions.m_iRuby_4RankOptionNum == 2) {
            if (lpTargetObj->Type == OBJ_MONSTER) {
                AddDamage += lpObj->m_PlayerData->m_PentagramOptions.m_iRuby_4RankAddDamage;
            } else if (lpTargetObj->Class == CLASS_KNIGHT || lpTargetObj->Class == CLASS_DARKLORD ||
                       lpTargetObj->Class == CLASS_RAGEFIGHTER) {
                AddDamage += lpObj->m_PlayerData->m_PentagramOptions.m_iRuby_4RankAddDamage;
            }
        }
        ...
        } else if (lpObj->m_PlayerData->m_PentagramOptions.m_iEme_3RankOptionNum == 2 &&
                   lpTargetObj->Type == OBJ_MONSTER) {
            AddDamage += lpObj->m_PlayerData->m_PentagramOptions.m_iEme_3RankAddDamage;
        }
    ...
    if (lpObj->Type == OBJ_USER) {
        ...
        } else if (lpObj->m_PlayerData->m_PentagramOptions.m_iEme_5RankOptionNum == 2 &&
                   lpTargetObj->Type == OBJ_MONSTER) {
            TotalDamage += TotalDamage * lpObj->m_PlayerData->m_PentagramOptions.m_iEme_5RankAddDamageRate / 100;
        }
    }
    ...
    if (TotalDamage > 0 && lpObj->Type == OBJ_USER) {
        if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_ARCA_FIRETOWER) == TRUE && AttackerAttribute == EL_FIRE) {
            ...
            TotalDamage += 5 * TotalDamage / 100;
        } else if (gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_ARCA_DARKNESSTOWER) == TRUE &&
                   AttackerAttribute == EL_DARKNESS) {
            TotalDamage += 5 * TotalDamage / 100;
        }
    }
    ...
    if (lpTargetObj->Type == OBJ_USER) {
        this->m_Lua.Generic_Call("ElementalDefenseCalc", "ii>i", lpTargetObj->Class,
                                 lpTargetObj->m_PlayerData->Dexterity, &BaseDefense);
    ...
    if (lpTargetObj->Type == OBJ_USER && g_PentagramSystem.IsPentagramItem(&lpTargetObj->pInventory[236]) == true) {
        ...
        } else if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_3RankOptionNum == 2 &&
        ...
        if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_4RankOptionNum == 1) {
            ...
            } else if (lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_ELF || lpObj->Class == CLASS_MAGUMSA ||
            ...
            if (lpObj->Type == OBJ_MONSTER) {
                AddDefense += lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_4RankAddDefense;
            } else if (lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_DARKLORD ||
                       lpObj->Class == CLASS_RAGEFIGHTER) {
                AddDefense += lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_4RankAddDefense;
            }
        }
        ...
        } else if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_3RankOptionNum == 2 &&
                   lpObj->Type == OBJ_MONSTER) {
            AddDefense += lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_3RankAddDefense;
        }
        ...
        if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_4RankOptionNum == 1) {
            ...
            } else if (lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_ELF ||
                       lpTargetObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_SUMMONER) {
                AddDefense += lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_4RankAddDefense;
            }
    ...
    if (TotalDefense > 0 && lpTargetObj->Type == OBJ_USER) {
        ...
        } else if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_ARCA_WATERTOWER) == TRUE &&
                   DefenserAttribute == EL_WATER) {
            TotalDefense += 5 * TotalDefense / 100;
        } else if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_ARCA_EARTHTOWER) == TRUE &&
                   DefenserAttribute == EL_EARTH) {
            TotalDefense += 5 * TotalDefense / 100;
        } else if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_ARCA_WINDTOWER) == TRUE &&
                   DefenserAttribute == EL_WIND) {
            TotalDefense += 5 * TotalDefense / 100;
        } else if (gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_ARCA_DARKNESSTOWER) == TRUE &&
                   DefenserAttribute == EL_DARKNESS) {
            TotalDefense += 5 * TotalDefense / 100;
        }
    }
    ...
    if (lpTargetObj->Type == OBJ_USER) {
        if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_5RankOptionNum == 1 && lpObj->Type == OBJ_USER) {
            TotalDamage -=
                    lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_5RankDamageAbsorbRate * TotalDamage / 100;
        } else if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_5RankOptionNum == 2 &&
                   lpObj->Type == OBJ_MONSTER) {
            TotalDamage -=
                    lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_5RankDamageAbsorbRate * TotalDamage / 100;
        }
    ...
    *DamageType2 = 0;
    ...
    if (TotalDamageWithRelationShip - TotalDefenseWithRelationShip > 0 && TotalDamageWithRelationShip > 10) {
        if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
            TotalDamageWithRelationShip = TotalDamageWithRelationShip *
                                          g_ConfigRead.calc.ElementalPvPDamageRate[lpObj->Class][lpTargetObj->Class] /
                                          100;
        } else if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_MONSTER) {
            TotalDamageWithRelationShip =
                    TotalDamageWithRelationShip * g_ConfigRead.calc.ElementalPvMDamageRate[lpObj->Class] / 100;
        }
        ...
        if (lpTargetObj->Type == OBJ_USER) {
            if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_5RankOptionNum == 1 && lpObj->Type == OBJ_USER) {
                TotalDamageWithRelationShip -=
                        lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_5RankMinusTargetDamageRate *
                        TotalDamageWithRelationShip / 100;
            } else if (lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_5RankOptionNum == 2 &&
                       lpObj->Type == OBJ_MONSTER) {
                TotalDamageWithRelationShip -=
                        lpTargetObj->m_PlayerData->m_PentagramOptions.m_iSapph_5RankMinusTargetDamageRate *
                        TotalDamageWithRelationShip / 100;
        ...
        if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
            ...
            if (lpObj->Level - lpTargetObj->Level > 10) {
                TotalDamageWithRelationShip =
                        (lpObj->Level + lpObj->m_PlayerData->MasterLevel - lpTargetObj->Level) / ((rand() % 31) + 70);
            } else {
    ...
    if (lpTargetObj->Type == OBJ_USER &&
        lpTargetObj->m_PlayerData->m_PentagramOptions.m_isAddResistByStrongRelationShip == true && (rand() % 100) < 5) {
        switch (AttackerAttribute) {
            case 1:
    ...
    if (lpTargetObj->Type == OBJ_USER &&
        lpTargetObj->m_PlayerData->m_PentagramOptions.m_isAddResistByPentaAttack == true && (rand() % 100) < 5)
        return 0;

    if (lpObj->Type == OBJ_USER) {
        AttackSuccessRate = (3 * (lpObj->AddDexterity + lpObj->m_PlayerData->Dexterity) / 2) +
                            (5 * (lpObj->Level + lpObj->m_PlayerData->MasterLevel)) +
                            ((lpObj->AddStrength + lpObj->m_PlayerData->Strength) / 4);
    ...
    if (lpTargetObj->Type == OBJ_USER) {
        ...
        DefenseSuccessRate +=
                DefenseSuccessRate * lpTargetObj->m_PlayerData->m_PentagramOptions.m_iTopa_1RankAddDefenseSuccessRate /
                100;
    } else {

    ...
    if (iResult == 0) {
        ...
        if (iCurrentState == KANTURU_STATE_BATTLE_OF_MAYA) {
            ...
            if (bMoveGateSuccess == TRUE) {
                ...
                g_Log.Add(
                        "[Kanturu][User Mng] Add User [%s][%s] Current Battle User:%d State:[%d-%d] (CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%u, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d",
                        ...
                        gObj[iIndex].m_PlayerData->Experience, gObj[iIndex].m_PlayerData->Strength,
                        gObj[iIndex].m_PlayerData->Dexterity,

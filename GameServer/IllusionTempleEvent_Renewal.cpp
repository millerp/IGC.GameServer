...
int CIllusionTempleEvent_Renewal::Load_ITR_EventInfo() {
    this->Load_ITR_NPC_Position(
            g_ConfigRead.GetPath("\\Events\\IllusionTemple_Renewal\\IGC_IllusionTemple_Renewal_NPC.xml"));
...
void CIllusionTempleEvent_Renewal::Load_ITR_Script(char *filename) {
    ...
    this->SetEventEnable(
            g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_IT) == true
            ? itr.attribute("Enable").as_bool() : false);
...
void CIllusionTempleEvent_Renewal::ActRelicsGetOrRegister(OBJECTSTRUCT *lpNpc, OBJECTSTRUCT *lpObj, BYTE byMapNumber,
...
BYTE
CIllusionTempleEvent_Renewal::Update_PartyInfo(BYTE byMapNumber, BYTE bySlotNumber, int PartyIndex, int nUserIndex) {

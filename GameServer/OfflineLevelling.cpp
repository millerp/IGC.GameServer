        ...
        case MAGIC_ATTACK: {
            for (int i = 0; i < MAX_VIEWPORT; i++) {
                if (user->VpPlayer2[i].type == OBJ_MONSTER && user->VpPlayer2[i].state != 0) {
                    ...
                    if (lpMagic == NULL) {
                        g_Log.Add("[OfflineLevelling] [%s][%s] Skill %d not found", user->AccountID, user->Name,
                                  obj.wSkillNumber);
                        return;
                    }
...
void COfflineLevelling::Run() {
    ...
    for (std::map<int, OFF_LEVEL_PLAYERS>::iterator iter = m_OffPlayerData.begin();
         iter != m_OffPlayerData.end(); iter++) {
...
bool COfflineLevelling::CheckUseTime(int aIndex) {
    ...
    if (lpObj->m_PlayerData->VipType == 0) {
        if (useTime > (this->m_General.MaxDuration * 1000 * 3600)) {
            ...
            lpObj->m_bOffLevel = false;
            this->DeleteUser(lpObj->m_Index);
            g_Log.Add("[OffLevel][%d][%s][%s] Exceed max use time [Hours:%d] [Vip:%d]", aIndex, lpObj->AccountID,
                      lpObj->Name, (useTime * 1000 * 3600), lpObj->m_PlayerData->VipType);
        }
    } else {
    ...
    for (int i = 0; i < INVETORY_WEAR_SIZE; i++) {
        if (lpObj->pInventory[i].IsItem()) {
            if (lpObj->pInventory[i].m_Durability < lpObj->pInventory[i].m_BaseDurability) {
                ...
                if (lpObj->pInventory[i].m_Type == ITEMGET(13, 20) &&
                    (lpObj->pInventory[i].m_Level == 0 || lpObj->pInventory[i].m_Level == 1))
                    continue;

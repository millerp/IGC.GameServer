...
void CIllusionTempleLeagueProcess::CheckSync() {
    ...
    g_Log.Add("[ ITL ] TYPE:%d , (%d) ITL Sync Open Time. [%d]min remain, m_nRemainTimeMsec:[%d]",
              g_IllusionTempleLeagueEvent.m_nITL_TYPE, this->m_nTempleNumber + 1, this->m_nRemainTimeMin,
              this->m_nRemainTimeMsec);
...
void CIllusionTempleLeagueProcess::Proc_ITLState_None(int nCurTime) {
    ...
    if (g_IllusionTempleLeagueEvent.WhatIsTodayTypeOfITL() == 1 ||
        ...
        if (iTICK_MSEC >= 1000) {
            ...
            if (this->m_nRemainTimeMin <= CIllusionTempleLeagueEvent::m_nITLOpenTime + 1 &&
                this->m_nRemainTimeMsec == -1) {
                this->m_nRemainTimeMsec = 60000 * (CIllusionTempleLeagueEvent::m_nITLOpenTime + 1);
            }

            if (this->m_nRemainTimeMin <= CIllusionTempleLeagueEvent::m_nITLOpenTime &&
                this->m_nRemainTimeMsec > 5000 && this->m_bEntrance == false) {
                this->m_bEntrance = true;
            }

            if (this->m_nRemainTimeMsec <= 30000 && this->m_nRemainTimeMsec > 0 && this->m_nTempleNumber == 0 &&
                this->m_bSendMsgBeforeReady == false) {
            ...
            if (this->m_nUserCount >= 2 && bCanStart == true) {
                ...
                for (int i = 0; i < 10; i++) {
                    if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].m_nITLIndex == i) {
                        if (this->m_UserData[i].m_btTeam == 1) {
                            g_SendNPCInfo.SendITLNpcCoordinate(this->m_UserData[i].m_nIndex,
                                                               this->m_nTempleNumber + 98);
                ...
                if (byAllied == 0 || byIllusion == 0) {
                    if (g_IllusionTempleLeagueEvent.WhatIsTodayTypeOfITL() == 2 ||
                        ...
                        g_Log.Add(
                                "[ ITL ] FINAL/SEMIFINAL START FAIL BattleType:[%d],ALLIED:ILLUSION == [%d VS %d],WINTEAM : [%d]",
                                g_IllusionTempleLeagueEvent.WhatIsTodayTypeOfITL(), byAllied, byIllusion,
                                byWinTeamIndex);
                    }

                    if ((byWinTeamIndex == 0 || byWinTeamIndex == 1) &&
                        this->m_GuildDataSave[byWinTeamIndex].bDBSave == false) {
                ...
                for (int nIndex = 0; nIndex < 10; nIndex++) {
                    ...
                    if (this->m_UserData[nIndex].m_nIndex != -1 &&
                        gObj[this->m_UserData[nIndex].m_nIndex].Connected > PLAYER_LOGGED &&
                        gObj[this->m_UserData[nIndex].m_nIndex].MapNumber == this->m_nTempleNumber + 98) {
                        if (byWinTeamIndex == 0 || byWinTeamIndex == 1) {
                            if (g_IllusionTempleLeagueEvent.m_nITL_TYPE == 1) {
                ...
                g_Log.Add(
                        "[ ITL ] (%d) Failed to Start ITL (UserCount: %d) (m_nRemainTimeMin:%d)(m_nRemainTimeMSec:%d),AlliedCnt:[%d],IllusionCnt:[%d]",
                        this->m_nTempleNumber, this->m_nUserCount, this->m_nRemainTimeMin, this->m_nRemainTimeMsec,
                        this->m_nAlliedUserCount, this->m_nIllusionUserCount);
...
void CIllusionTempleLeagueProcess::Proc_ITLState_End(int nCurTime) {
            ...
            if (this->m_nRemainTimeMsec > 0 && this->m_bGameEndPosMove == false) {
                ...
                for (int i = 0; i < 10; i++) {
                    if (this->m_UserData[i].m_nIndex != -1 &&
                        ...
                        gObjSetPosition(this->m_UserData[i].m_nIndex, gObj[this->m_UserData[i].m_nIndex].X,
                                        gObj[this->m_UserData[i].m_nIndex].Y);
                    }

                    if (this->m_UserData[i].m_nRelicsInvenPos > 0) {
                        if (gObj[this->m_UserData[i].m_nIndex].pInventory[this->m_UserData[i].m_nRelicsInvenPos].m_Type ==
                            ITEMGET(14, 223)) {
                            gObjInventoryDeleteItem(this->m_UserData[i].m_nIndex,
                                                    this->m_UserData[i].m_nRelicsInvenPos);
                            GSProtocol.GCInventoryItemDeleteSend(this->m_UserData[i].m_nIndex,
                                                                 this->m_UserData[i].m_nRelicsInvenPos, 0);
    ...
    if (this->m_nRemainTimeMsec <= 60000 * CIllusionTempleLeagueEvent::m_nITLEndTime - 6000 &&
        this->m_nRemainTimeMsec > 0 && this->m_bGameEndPosMove == true && this->m_bSendResult == false) {
        if (this->m_bITLRoundStartFail == false) {
            this->SendITLResult();
        }
            ...
            if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED) {
                if (gObj[this->m_UserData[i].m_nIndex].MapNumber == this->m_nTempleNumber + 98) {
                    g_Log.Add("[ ITL ] MAP:[%d], ACC:[%s], NAME:[%s], TEAM:[%d], CLASS:[%d]",
                              this->m_nTempleNumber, gObj[this->m_UserData[i].m_nIndex].AccountID,
                              gObj[this->m_UserData[i].m_nIndex].Name,
...
void CIllusionTempleLeagueProcess::Set_ITLState_End() {
    ...
    g_Log.Add("[ ITL ][WarResult] Allied Team : %d , IllusionTeam :%d", this->m_byAlliedTeamPoint,
              this->m_byIllusionTeamPoint);
...
BOOL CIllusionTempleLeagueProcess::EnterUserIllusionTempleLeague(int aIndex, BYTE btTempleIndex, int nGuildIdx,
                                                                 int nPartyIdx) {
    ...
    for (int i = 0; i < 10; i++) {
        if (this->m_UserData[i].m_nIndex == -1) {
            ...
            } else if (this->m_GuildDataSave[1].nGuild ==
                       gObj[this->m_UserData[i].m_nIndex].m_PlayerData->GuildNumber) {
...
BOOL CIllusionTempleLeagueProcess::LeaveUserIllusionTempleLeague(int aIndex) {
    ...
    if (gObj[aIndex].m_nITLIndex != -1 && this->m_UserData[gObj[aIndex].m_nITLIndex].m_nIndex == aIndex &&
        ...
        LeaveCriticalSection(&this->m_critUserData);
        g_Log.Add("[ ITL ] (%d) LeaveUser: (%s)(%s)", this->m_nTempleNumber + 1, gObj[aIndex].AccountID,
                  gObj[aIndex].Name);
    }
            ...
            if (lpObj) {
                if (lpObj->pInventory[this->m_UserData[i].m_nRelicsInvenPos].m_Type == ITEMGET(14, 223)) {
                    ...
                    g_Log.Add("[ ITL ] (%d) (%s)(%s) to Delete All Relics Item [Serial:%I64d]",
                              this->m_nTempleNumber + 1, lpObj->AccountID, lpObj->Name,
                              lpObj->pInventory[this->m_UserData[i].m_nRelicsInvenPos].m_Number);
...
void CIllusionTempleLeagueProcess::DropRelicsItem(int iIndex, BYTE byAct) {
    ...
    if (MapC[map_num].ItemDrop(type, level, dur, X, Y, Option1, Option2, Option3, NOption, SOption, item_number,
                               aAntiLootIndex, PetLevel, PetExp, ItemEffectEx, NULL, 0xFF, 0) == TRUE) {
        g_Log.Add("[ ITL ] (%d) (%s)(%s) to Drop Relics Item [Serial:%I64d]", this->m_nTempleNumber + 1,
                  lpObj->AccountID, lpObj->Name, lpObj->pInventory[iItemPos].m_Number);
...
void CIllusionTempleLeagueProcess::AddITLNpcInfo(WORD wNpcType, BYTE byMapNumber, BYTE byDistance, BYTE byX, BYTE byY,
                                                 BYTE byDir) {
    ...
    for (int i = 0; i < 10; i++) {
        if (this->m_Occupied_StoneStatus[i].m_nIndex != -1) {
            g_Log.Add("[ ITL ] (%d) Remove Occupied_StoneStatus Name: %s, (%d: %d/%d)",
                      ...
                      gObj[this->m_Occupied_StoneStatus[i].m_nIndex].MapNumber,
                      gObj[this->m_Occupied_StoneStatus[i].m_nIndex].X,
...
void CIllusionTempleLeagueProcess::GetRelics(OBJECTSTRUCT *lpNpc, OBJECTSTRUCT *lpObj, WORD wNpcType) {
    ...
    if (bCanGetRelics == TRUE) {
        ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpNpc->X, lpNpc->Y, ItemGetNumberMake(14, 223), 0, 0, 0,
                             0, 0, lpObj->m_Index, 0, 0, 0, 0, 0);
    }
        ...
        if (bFail == true) {
            ...
            if (lpObj->m_nITLRelicsTick <= GetTickCount()) {
                if (lpNpc->Class >= 659 && lpNpc->Class <= 668) {
                    for (int i = 0; i < 10; i++) {
                        if (this->m_Occupied_StoneStatus[i].m_wType == lpNpc->Class &&
                            this->m_UserData[nRegisterRelicsUserArray].m_btTeam ==
                            ...
                            if (this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos > 0) {
                                if (lpObj->pInventory[this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos].m_Type ==
                                    ITEMGET(14, 223)) {
                                    this->SendRelicsUserInfo(lpObj, 1);
                                    g_Log.Add(
                                            "[ ITL ][RegisterRelics] (%d) (%s)(%s) to Delete Relics Item [Serial:%I64d]",
                                            this->m_nTempleNumber + 1, lpObj->AccountID, lpObj->Name,
                                            lpObj->pInventory[this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos].m_Number);

                                    gObjInventoryDeleteItem(lpObj->m_Index,
                                                            this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos);
                                    GSProtocol.GCInventoryItemDeleteSend(lpObj->m_Index,
                                                                         this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos,
                                                                         0);
    ...
    for (int i = 0; i < 11; i++) {
        ...
        if (result > 0) {
            ...
            if (this->m_ITLNpcInfo[i].wNpcType == 658) {
                ...
                this->AddITLOccupiedStonePos(this->m_ITLNpcInfo[i].wNpcType, result, nOccCount,
                                             this->m_ITLNpcInfo[i].byMapTagIndex);
                nOccCount++;
            }
        }
    }
        ...
        if (this->m_Occupied_StoneStatus[i].m_nIndex != -1) {
            ...
            if (this->m_Occupied_StoneStatus[i].m_byStatus == 10 ||
                ...
                g_Log.Add("[ ITL ][RegenOccupiedStoneStatus] Round [%d] NpcIndex:[%d],Type:[%d] MAKE NEUTRAL",
                          this->m_byITLRound, this->m_Occupied_StoneStatus[i].m_nIndex,
                          this->m_Occupied_StoneStatus[i].m_wType);
            }
        }
    }
...
void CIllusionTempleLeagueProcess::Send_ITL_StoneInfo(int userindex, WORD wNpcType, BYTE byState, int NpcIndex,
                                                      BYTE byMapTagIndex) {
...
void CIllusionTempleLeagueProcess::IncUserKillCount(int nIndex) {
    ...
    g_Log.Add("[ ITL ][UserKill] map:%d, ACC:%s, NAME:%s ,KillCnt:%d -> %d",
              ...
              this->m_UserData[gObj[nIndex].m_nITLIndex].m_btUserKillCount + 1);
...
void CIllusionTempleLeagueProcess::CalCursetTempleResult() {
            ...
            if (byAlliedCnt == byIllusionCnt) {
                ...
                if (byAlliedTeamKillSum <= byIllusionTeamKillSum) {
                    if (byAlliedTeamKillSum >= byIllusionTeamKillSum) {
                        ...
                        for (int i = 0; i < 10; i++) {
                            if (this->m_UserData[i].m_btTeam == 0) {
                                nAlliedLevelSum += (gObj[this->m_UserData[i].m_nIndex].Level +
                                                    gObj[this->m_UserData[i].m_nIndex].m_PlayerData->MasterLevel);
                            } else if (this->m_UserData[i].m_btTeam == 1) {
                                nIllusionLevelSum += (gObj[this->m_UserData[i].m_nIndex].Level +
                                                      gObj[this->m_UserData[i].m_nIndex].m_PlayerData->MasterLevel);
                            }
                        }
    ...
    if (byWinnerGuildIndex != 255 && byLoseGuildIndex != 255) {
        ...
        g_Log.Add("[ ITL ] (%d) Result (WinTeam: %d)(Score:[%d][%d])", this->m_nTempleNumber + 1, this->m_btWinTeam,
                  byAlliedCnt, byIllusionCnt);
    } else {
        g_Log.Add("[ ITL ] (%d) Result (Team: %d)(Score:[%d][%d]) byWinnerGuildIndex :%d,byLoseGuildIndex : %d ",
                  this->m_nTempleNumber + 1, this->m_btWinTeam, byAlliedCnt, byIllusionCnt, byWinnerGuildIndex,
                  byLoseGuildIndex);
...
void CIllusionTempleLeagueProcess::DropChaosGem(int nIndex) {
    ...
    ItemSerialCreateSend(nIndex, gObj[nIndex].MapNumber, gObj[nIndex].X, gObj[nIndex].Y, ItemGetNumberMake(12, 30), 0,
                         0, 0, 0, 0, nIndex, 0, 0, 0, 0, 0);

    g_Log.Add("[ ITL ] (%d) [DropChaosGem] [ItemSerialCreateSend] Index:[%d],Name:[%s],Acc:[%s]",
              this->m_nTempleNumber + 1, nIndex, gObj[nIndex].Name, gObj[nIndex].AccountID);
}
...
BOOL CIllusionTempleLeagueProcess::CheckSkillProdection(int iIllusionTempleIndex) {
    ...
    if (this->m_UserData[iIllusionTempleIndex].m_nIndex != -1 &&
        this->m_UserData[iIllusionTempleIndex].m_dwSkillProdectionTime != 0) {
        return TRUE;
    } else {
        return FALSE;
    }
...
BOOL CIllusionTempleLeagueProcess::CheckSkillRestraint(int iIllusionTempleIndex) {
    ...
    if (this->m_UserData[iIllusionTempleIndex].m_nIndex != -1 &&
        this->m_UserData[iIllusionTempleIndex].m_dwSkillRestraintTime != 0) {
        return TRUE;
    } else {
        return FALSE;
    }
...
void CIllusionTempleLeagueProcess::SendITLResult() {
    ...
    for (int i = 0; i < 10; i++) {
        if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED) {
            ...
            g_Log.Add(
                    "[ ITL ] (%d) ITL Result  (Account:%s, Name:%s, G.name:%s , Team:%d, Class:%d, UserKillCount:%d,InvalidationCount:%d,OccupiedCount:%d)",
                    this->m_nTempleNumber, gObj[this->m_UserData[i].m_nIndex].AccountID,
                    gObj[this->m_UserData[i].m_nIndex].Name, gObj[this->m_UserData[i].m_nIndex].m_PlayerData->GuildName,
                    this->m_UserData[i].m_btTeam, gObj[this->m_UserData[i].m_nIndex].m_PlayerData->DbClass,
                    this->m_UserData[i].m_btUserKillCount, this->m_UserData[i].m_byInvalidationCount,
            ...
            this->GDSaveITLUserPoint(this->m_UserData[i].m_nIndex, gObj[this->m_UserData[i].m_nIndex].Name,
                                     gObj[this->m_UserData[i].m_nIndex].m_PlayerData->GuildName,
                                     this->m_UserData[i].m_byOccupiedCount, this->m_UserData[i].m_btUserKillCount,
                                     this->m_UserData[i].m_byInvalidationCount,
    ...
    if (this->m_bNobodyWinLose == false) {
        for (int i = 0; i < 2; i++) {
            if (this->m_GuildDataSave[i].bDBSave == false) {
                this->GDSaveITLGuildPoint(this->m_GuildDataSave[i].szGName, this->m_GuildDataSave[i].byITLType,
                                          this->m_GuildDataSave[i].byWin,
    ...
    for (int i = 0; i < 10; i++) {
        if (this->m_UserData[i].m_btTeam != 0xFF && this->m_UserData[i].m_nIndex != -1 &&
            gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED) {
...
BYTE CIllusionTempleLeagueProcess::SetGuild(int nGuildIndex, BYTE bySlotOrder, BYTE byITLType, int nPartyIdx,
                                            int nUserIndex) {
    if (nGuildIndex <= 0) {
        return -1;
    }
...
void
CIllusionTempleLeagueProcess::GDSaveITLGuildPoint(char *GuildName, int Type, BYTE btWin, BYTE btLose, BYTE byOccCnt,
                                                  BYTE byEnterCnt) {
...
void CIllusionTempleLeagueProcess::GDSaveITLUserPoint(int index, char *UserName, char *GuildName, int nOccupiedCount,
                                                      int KillPoint, int nInvalidationCount, int nType, int nEnterCount,
                                                      int nWin) {
    ...
    g_Log.Add(
            "[ ITL ] (%d) [GDSaveITLUserPoint] Name:%s, G.Name:%s, OccCnt:%d, KillCnt:%d, InvalidationCnt:%d,Type:%d, EnterCnt:%d, Win:%d",
            this->m_nTempleNumber + 1, UserName, GuildName, nOccupiedCount, KillPoint, nInvalidationCount, nType,
            nEnterCount, nWin);
...
void CIllusionTempleLeagueProcess::MoveToStartPoint_WhenRoundBreak() {
    for (int i = 0; i < 10; i++) {
        if (this->m_UserData[i].m_nIndex != -1 &&
            ...
            g_Log.Add(
                    "[ ITL ] (%d) [RoundBreak_Move2WaittingRoom]  Round [%d], UserIndex :[%d], Account:[%s] , Name :[%s] ",
                    this->m_nTempleNumber + 1, this->m_byITLRound, this->m_UserData[i].m_nIndex,
                    gObj[this->m_UserData[i].m_nIndex].AccountID, gObj[this->m_UserData[i].m_nIndex].Name);
        }
    }
}
...
BOOL CIllusionTempleLeagueProcess::ReEnterUserIllusionTempleLeague(int aIndex, BYTE btTempleIndex, int nGuildIdx,
                                                                   int nPartyIdx, int nUserArrayNo, BYTE TeamIdx) {
    if (!ObjectMaxRange(aIndex)) {
        g_Log.Add("return %s %d", __FILE__, __LINE__);
        return FALSE;
    }
...
void CIllusionTempleLeagueProcess::ResetUserFlag_DoingRelicsThing(OBJECTSTRUCT *lpObj) {
    ...
    g_Log.Add(
            "[ ITL ][ResetUserFlag_DoingRelicsThing] NAME:%s, StoneState:%d, NPCTYPE:%d, GettingRelics:%d, RegisteringRelics:%d",
            lpObj->Name, lpObj->m_byStoneState, lpObj->m_wITLNpcType, lpObj->m_bGettingRelics,
            lpObj->m_bRegisteringRelics);

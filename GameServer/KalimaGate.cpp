    ...
    __try
    {
        ...
        if (btMapAttr != 0) {
            if (this->GetRandomLocation(iMapNumber, cX, cY) == 0) {
                g_Log.Add(
                        "[Kalima] [%s][%s] Failed to Summon Kalima Gate - Failed to get Location (MapNumber:%d, X:%d, Y:%d)",
        ...
        if (iMonsterIndex >= 0) {
            ...
            g_Log.Add(
                    "[Kalima] [%s][%s] Create Kalima Gate Successfuly (Live:%d, GateTick:%u, DieRegen:%d, Map:%d, X:%d, Y:%d, SummonIndex:%d, OwnerIndex:%d, GateNo.:%d)",
                ...
                if (gObj[tObjNum].Live != FALSE) {
                    if (abs(lpObj->Y - gObj[tObjNum].Y) < 2) {
                        if (abs(lpObj->X - gObj[tObjNum].X) < 2) {
                            if (lpObj->m_RecallMon == tObjNum || (gObj[tObjNum].PartyNumber != -1 &&
                                                                  lpObjCallOwner->PartyNumber ==
                                                                  gObj[tObjNum].PartyNumber)) {
                                ...
                                if (iOverLevel >= iMoveMapLevel) {
                                    if (gObjMoveGate(tObjNum, iGateNumber) == FALSE) {
                                        ...
                                        g_Log.Add(
                                                "[Kalima] [%s][%s] - [%s][%s] Transported by Kalima Gate (SummonIndex:%d, GateNo.:%d, MapNumber:%d)",
                                                lpObjCallOwner->AccountID, lpObjCallOwner->Name,
                                                gObj[tObjNum].AccountID, gObj[tObjNum].Name, iIndex, iMoveMapLevel + 1,
                                                iMapNumber);
...
int CKalimaGate::GetKalimaGateLevel(int iIndex) {
    if (gObj[iIndex].Class == CLASS_MAGUMSA || gObj[iIndex].Class == CLASS_DARKLORD ||
        gObj[iIndex].Class == CLASS_RAGEFIGHTER)    // DarkLord and MagicGLadiaro
    {
    ...
    __try
    {
        ...
        if (btMapAttr != 0) {
            if (this->GetRandomLocation(iMonMapNumber, cX, cY) == FALSE) {
                g_Log.Add(
                        "[Kalima] [%s][%s] Failed to Summon Kalima Gate (NextMap) - Failed to get Location (MapNumber:%d, X:%d, Y:%d)",
        ...
        if (iMonsterIndex >= 0) {
            ...
            g_Log.Add(
                    "[Kalima] [%s][%s] Create Kalima Gate Successfuly (NextMap) (Live:%d, GateTick:%u, DieRegen:%d, Map:%d, X:%d, Y:%d, SummonIndex:%d, OwnerIndex:%d)",
                    gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iMonsterIndex].Live, gObj[iMonsterIndex].RegenTime,
                    gObj[iMonsterIndex].DieRegen, gObj[iMonsterIndex].MapNumber, gObj[iMonsterIndex].X,
                    ...
                    if (abs(lpObj->Y - gObj[tObjNum].Y) < 2) {
                        if (abs(lpObj->X - gObj[tObjNum].X) < 2) {
                            if (lpObj->m_RecallMon == tObjNum || (gObj[tObjNum].PartyNumber != -1 &&
                                                                  ...
                                                                  gObj[tObjNum].PartyNumber)) {
                                int iMoveMapLevel =
                                        lpObj->MapNumber - MAP_INDEX_KALIMA1 + 1;    // #error "Deatwway - remove the -1
                                int iGateNumber = g_iKalimaGateGateIndex[iMoveMapLevel];
                                ...
                                if (!gObjMoveGate(tObjNum, iGateNumber)) {
                                    ...
                                    g_Log.Add(
                                            "[Kalima] [%s][%s] - [%s][%s] Transported by Kalima Gate (NextMap) (SummonIndex:%d, GateNo.:%d, MapNumber:%d)",
                                            lpObjCallOwner->AccountID, lpObjCallOwner->Name,
                                            gObj[tObjNum].AccountID, gObj[tObjNum].Name,

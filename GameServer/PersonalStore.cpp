...
void CPersonalStore::CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE *lpMsg, int aIndex) {
    ...
    int iItemPrice = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->sItemPrice4, lpMsg->sItemPrice3),
                                   MAKE_NUMBERW(lpMsg->sItemPrice2, lpMsg->sItemPrice1));
    ...
    if (gObjInventorySearchSerialNumber(&gObj[aIndex], sitem->GetNumber()) == FALSE) {
        GSProtocol.GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1, gObj[aIndex].iShield);
        g_Log.Add("error-L2 : CopyItem [%s][%s] return %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, __FILE__,
                  __LINE__);
    ...
    if (iItemPrice <= 0 && wBlessPrice <= 0 && wSoulPrice <= 0 && wChaosPrice <= 0) {
        g_Log.Add(
                "[PShop] [%s][%s] ERROR : Personal Shop Item Price <= 0 : %d and sBlessJewelPrice <= 0 : %d and sSoulJewelPrice <= 0 : %d and sChaosJewelPrice <= 0 : %d ",
                gObj[aIndex].AccountID, gObj[aIndex].Name, sitem->m_iPShopValue, sitem->m_wPShopBlessValue,
                sitem->m_wPShopSoulValue, sitem->m_wPShopChaosValue);
    ...
    if ((gObj[aIndex].Penalty & 4) == 4 || (gObj[aIndex].Penalty & 8) == 8) {
        g_Log.Add("[PShop] [%s][%s] ERROR : Item Price Change - Item Block", gObj[aIndex].AccountID, gObj[aIndex].Name,
                  sitem->m_iPShopValue);    // Deathway fix Here - Last parameter can cause crash
    ...
    g_Log.Add(
            "[PShop] [%s][%s] Changing Item Price : %d, BlessJewelPrice : %d, SoulJewelPrice : %d, ChaosJewelPrice : %d (Type:%d (%s), Lev:%d, Serial:%I64d, Dur:%d",
            gObj[aIndex].AccountID, gObj[aIndex].Name, sitem->m_iPShopValue, sitem->m_wPShopBlessValue,
...
void CPersonalStore::CGPShopReqOpen(PMSG_REQ_PSHOP_OPEN *lpMsg, int aIndex) {
    ...
    if (gObj[aIndex].m_IfState.use == FALSE)
        ...
        if (gObj[aIndex].m_IfState.use == TRUE) {
            ...
            if (!bEnablePShopOpen) {
                g_Log.Add("[PShop] [%s][%s] ERROR : m_IfState.type is Using : %d", gObj[aIndex].AccountID,
                          gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
            }
        } else {
            g_Log.Add("[PShop] [%s][%s] ERROR : Unknown m_IfState.type : %d", gObj[aIndex].AccountID, gObj[aIndex].Name,
    ...
    if (gObj[aIndex].pTransaction == 1) {
        g_Log.Add("[PShop] [%s][%s] ERROR : Transaction == 1, IF_TYPE : %d", gObj[aIndex].AccountID, gObj[aIndex].Name,
                  gObj[aIndex].m_IfState.type);
...
void CPersonalStore::CGPShopReqBuyList(PMSG_REQ_BUYLIST_FROM_PSHOP *lpMsg, int aSourceIndex) {
    ...
    if (lpObj->m_PlayerData->ISBOT == true && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_SHOP) {
        GSProtocol.GCAlterPShopVault(aSourceIndex,
                                     g_BotSystem.m_BotData[lpObj->m_PlayerData->wBotIndex].m_Shop.iCoinType);
    ...
    if (gObj[aSourceIndex].m_bMapSvrMoveQuit == true || gObj[aSourceIndex].m_bMapSvrMoveReq == true ||
        gObj[aSourceIndex].m_bMapSvrMoveReq_2 == true ||
        lpObj->m_bMapSvrMoveQuit == true || lpObj->m_bMapSvrMoveReq == true || lpObj->m_bMapSvrMoveReq_2 == true) {
        g_Log.Add(
                "[PShop][SuspectCheating] Map Of Move request was trying to Shop List. Buyer ID[%s]Name[%s] Seller Name[%s]",
...
void CPersonalStore::CGPShopReqBuyItem(LPBYTE lpRecv, int aSourceIndex) {
    ...
    if (lpObj->m_bPShopTransaction == true) {
        ...
        if (::gObjInventorySearchSerialNumber(&gObj[aSourceIndex], lpObj->Inventory1[lpMsg->btItemPos].GetNumber()) ==
            FALSE) {
            GSProtocol.GCReFillSend(aSourceIndex, gObj[aSourceIndex].Life, 0xFD, 1, gObj[aSourceIndex].iShield);
            g_Log.Add("error-L2 : CopyItem [%s][%s] return %s %d", gObj[aSourceIndex].AccountID,
        ...
        __try
        {
            ...
            if (lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE) {
                if (lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue <= 0 &&
                    lpObj->Inventory1[lpMsg->btItemPos].m_wPShopBlessValue <= 0
                ...
                if (lpObj->m_bOff == false && lpObj->m_PlayerData->ISBOT == false) {
                    ...
                    if (!lpObj->m_bOff && lpObj->m_PlayerData->ISBOT == false) {
                        if (gObj[aSourceIndex].m_PlayerData->Money <
                            lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue) {
                            g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
                                      gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
                                      lpObj->AccountID, lpObj->Name);
                            g_PersonalStore.CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
                        ...
                        switch (g_ConfigRead.offtrade.CoinType) {
                            case 0: {
                                ...
                                break;
                            case 1: {
                                if (gObj[aSourceIndex].m_PlayerData->m_WCoinC <
                                    lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue) {
                                    g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
                                              gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
                                              lpObj->AccountID, lpObj->Name);
                                ...
                                break;
                            case 2: {
                                if (gObj[aSourceIndex].m_PlayerData->m_WCoinP <
                                    lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue) {
                                    g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
                                              gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
                                              lpObj->AccountID, lpObj->Name);
                                ...
                                break;
                            case 3: {
                                if (gObj[aSourceIndex].m_PlayerData->m_GoblinPoint <
                                    lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue) {
                                    g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
                                              gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
                                              lpObj->AccountID, lpObj->Name);
                ...
                if (lpObj->Inventory1[lpMsg->btItemPos].m_Type != lpMsg->iItemType
                    ...
                    g_Log.Add(
                            "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Client Item Different than Server Item (Server - ItemType:%d, Pos:%d, MV:%d, BJV:%d, SJV:%d, CJV:%d), (Client - ItemType:%d, Pos:%d, MV:%d, BJV:%d, SJV:%d, CJV:%d)",
                            lpObj->AccountID, lpObj->Name, gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
                            lpObj->Inventory1[lpMsg->btItemPos].m_Type, lpMsg->btItemPos,
                            lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue,
                            ...
                            lpMsg->iItemType, lpMsg->btItemPos, lpMsg->PShopItemMoneyValue,
                            lpMsg->PShopItemBlessJewelValue, lpMsg->PShopItemSoulJewelValue,
                            lpMsg->PShopItemChaosJewelValue);
                ...
                if (gCountBlessSoulChaosJewel(aSourceIndex, sBlessJewelCount, s10BlessJewelCount, s20BlessJewelCount,
                                              s30BlessJewelCount,
                                              sSoulJewelCount, s10SoulJewelCount, s20SoulJewelCount, s30SoulJewelCount,
                                              sChaosJewelCount, s10ChaosJewelCount, s20ChaosJewelCount,
                                              s30ChaosJewelCount)) {
                    sBlessJewelTotalCount =
                            30 * s30BlessJewelCount + 20 * s20BlessJewelCount + 10 * s10BlessJewelCount +
                            sBlessJewelCount;
                    sSoulJewelTotalCount =
                            30 * s30SoulJewelCount + 20 * s20SoulJewelCount + 10 * s10SoulJewelCount + sSoulJewelCount;
                    sChaosJewelTotalCount =
                            30 * s30ChaosJewelCount + 20 * s20ChaosJewelCount + 10 * s10ChaosJewelCount +
                            sChaosJewelCount;
                ...
                if (lpObj->Inventory1[lpMsg->btItemPos].m_wPShopBlessValue > 0
                    ...
                    int iInvalidValueTestResult = gJewelBuyItemValueTest(aSourceIndex, lpObj->m_Index,
                                                                         ...
                                                                         lpObj->Inventory1[lpMsg->btItemPos].m_wPShopSoulValue,
                                                                         lpObj->Inventory1[lpMsg->btItemPos].m_wPShopChaosValue,
                                                                         sBlessJewelCount, s10BlessJewelCount,
                                                                         s20BlessJewelCount, s30BlessJewelCount,
                                                                         sSoulJewelCount, s10SoulJewelCount,
                                                                         s20SoulJewelCount, s30SoulJewelCount,
                                                                         sChaosJewelCount, s10ChaosJewelCount,
                                                                         s20ChaosJewelCount, s30ChaosJewelCount,
                                                                         sBlessJewelNeedCount, s10BlessJewelNeedCount,
                                                                         s20BlessJewelNeedCount, s30BlessJewelNeedCount,
                                                                         sSoulJewelNeedCount, s10SoulJewelNeedCount,
                                                                         s20SoulJewelNeedCount, s30SoulJewelNeedCount,
                                                                         sChaosJewelNeedCount, s10ChaosJewelNeedCount,
                                                                         s20ChaosJewelNeedCount,
                                                                         s30ChaosJewelNeedCount);
                ...
                if (g_PentagramSystem.IsPentagramItem(&lpObj->Inventory1[lpMsg->btItemPos]) == true) {
                    ...
                    if (!gTransPentagramJewelInfoTest(aSourceIndex, lpObj->m_Index,
                                                      lpObj->Inventory1[lpMsg->btItemPos])) {
                ...
                if (g_PentagramSystem.IsPentagramItem(&lpObj->Inventory1[lpMsg->btItemPos])) {
                    int nJewelCount = 0;
                    gTransPentagramJewelInfo(aSourceIndex, btNewItemPos, lpObj->m_Index,
                                             lpObj->Inventory1[lpMsg->btItemPos], &nJewelCount);
                ...
                gJewelInventoryPut(aSourceIndex, lpObj->m_Index,
                                   ...
                                   lpObj->Inventory1[lpMsg->btItemPos].m_wPShopSoulValue,
                                   lpObj->Inventory1[lpMsg->btItemPos].m_wPShopChaosValue,
                                   sBlessJewelNeedCount, s10BlessJewelNeedCount, s20BlessJewelNeedCount,
                                   s30BlessJewelNeedCount,
                                   sSoulJewelNeedCount, s10SoulJewelNeedCount, s20SoulJewelNeedCount,
                                   s30SoulJewelNeedCount,
                                   sChaosJewelNeedCount, s10ChaosJewelNeedCount, s20ChaosJewelNeedCount,
                                   s30ChaosJewelNeedCount);
                ...
                g_Log.Add(
                        "[PShop] [%s][%s][%s] PShop Item Buy Request Succeed : [%s][%s][%s](ZenPrice = %d(ZenCommision:%d), BJPrice = %d(BJCommision:%d), SJPrice = %d(SJCommision:%d), CJPrice = %d(CJCommision:%d), ItemType:%d(%s), ItemLevel : %d, ItemDur : %d, Serial : %u, BuyKind : %d",
                        ...
                        iITEM_LOG_TYPE, ItemAttribute[iITEM_LOG_TYPE].Name, iITEM_LOG_LEVEL, iITEM_LOG_DUR,
                        iITEM_LOG_SERIAL, lpMsg->btBuyKind);

                if (lpObj->m_PlayerData->ISBOT == true && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_SHOP) {
                    BotShopLog->Output("[BOT SHOP] [%s][%s][%s] Buy %s from %s for %d",
                                       gObj[aSourceIndex].m_PlayerData->Ip_addr, gObj[aSourceIndex].AccountID,
                                       gObj[aSourceIndex].Name, ItemAttribute[iITEM_LOG_TYPE].Name, lpObj->Name,
                                       dwCost);
                }
...
void CPersonalStore::PShop_ViewportListRegenarate(short aIndex) {
        ...
        if (lpObj->VpPlayer[n].state == 2 && lpObj->VpPlayer[n].type == OBJ_USER && gObj[tObjIndex].m_bPShopOpen == 1) {
            ...
            if (lpObj->m_wVpPShopPlayerCount > 0 && lpObj->m_wVpPShopPlayerCount <= MAX_VIEWPORT) {
                ...
                if (found == 0) {
                    ...
                    memcpy(lpMsgBody[iVpAddCount].szPShopText, gObj[tObjIndex].m_szPShopText,
                           sizeof(gObj[tObjIndex].m_szPShopText));
                    iVpAddCount++;
                }
                ...
                g_Log.Add("[PShop] ERROR : lpObj->m_wVpPShopPlayerCount is OUT of BOUND : %d",
                          lpObj->m_wVpPShopPlayerCount);
            }
        }
    }
    ...
    if (iVpAddCount > 0 && iVpAddCount <= MAX_VIEWPORT) {
        ...
        PHeadSubSetW((unsigned char *) lpMsg, 0x3F, 0,
                     sizeof(PMSG_ANS_PSHOP_VIEWPORT_NOTIFY) + sizeof(PMSG_PSHOP_VIEWPORT_NOTIFY) * iVpAddCount);
        IOCP.DataSend(aIndex, (unsigned char *) lpMsg,
                      ((lpMsg->h.sizeL & 0xFF) & 0xFF | ((lpMsg->h.sizeH & 0xFF) & 0xFF) << 8) & 0xFFFF);
...
void CPersonalStore::GDAnsPShopItemValue(PMSG_ANS_PSHOPITEMVALUE_INFO *lpMsg) {
    ...
    for (int i = 0; i < iItemCnt; i++) {
        if (lpMsg->PShopItemValueInfo[i].nPShopItemInvenNum >= 204 &&
            lpObj->pInventory[lpMsg->PShopItemValueInfo[i].nPShopItemInvenNum].IsItem() == TRUE) {
            if (lpObj->pInventory[lpMsg->PShopItemValueInfo[i].nPShopItemInvenNum].m_Number ==
                lpMsg->PShopItemValueInfo[i].ItemSerial) {
...
void CPersonalStore::GCPShop_AllInfo(short aIndex, int iLastUserCount) {
    ...
    for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
        ...
        if (gObj[n].Connected == PLAYER_PLAYING && lpObj->Type == OBJ_USER &&
            (lpObj->MapNumber == MAP_INDEX_RORENCIA || lpObj->MapNumber == MAP_INDEX_DEVIAS ||

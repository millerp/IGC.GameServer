...
bool CPentagramMixSystem::LoadMixNeedSourceScript(char *pchFileName) {
    ...
    if (res.status != pugi::status_ok) {
        g_Log.MsgBox("[PentagramMixSystem] PentagramSystem NeedSource Script file Load failed. (%s) (%s)", pchFileName,
                     res.description());
        return false;
    }
    ...
    for (pugi::xml_node combination = combinationmaterials.child(
    ...
    for (pugi::xml_node errtel = levelupsettings.child("Errtel"); errtel; errtel = errtel.next_sibling()) {
        ...
        memcpy(&this->m_JewelLevelUpgradeNeedItemData[iValueCount], &LevelUpgradeNeedItemData,
               sizeof(JEWEL_LEVEL_UPGRADE_NEED_ITEM));
        iValueCount++;
    }
    ...
    for (pugi::xml_node errtel = rankupsettings.child("Errtel"); errtel; errtel = errtel.next_sibling()) {
        ...
        memcpy(&this->m_JewelRankUpgradeNeedItemData[iValueCount], &RankUpgradeNeedItemData,
               sizeof(JEWEL_RANK_UPGRADE_NEED_ITEM));
        iValueCount++;
    }
    ...
    for (pugi::xml_node errtel = rankupsocketrate.child("Errtel"); errtel; errtel = errtel.next_sibling()) {
        ...
        memcpy(&this->m_JewelRankUpgradeDetermineRate[iValueCount], &JewelRankUpgradeDetermineRate,
               sizeof(JEWEL_RANK_UPGRADE_DETERMINE_RATE));
        iValueCount++;
    }
...
bool CPentagramMixSystem::LoadJewelOptionScript(char *pchFileName) {
    ...
    if (res.status != pugi::status_ok) {
        g_Log.MsgBox("[PentagramMixSystem] PentagramSystem Jewel Upgrade Script file Load failed. (%s) (%s)",
...
BYTE CPentagramMixSystem::PentagramJewelRefine(int iIndex, int iRefineType) {
    ...
    for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
        if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE) {
            for (int iCount = 0; iCount < 4; iCount++) {
                if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type ==
                    ITEMGET(lpRefineOptionData->NeedItem_Type[iCount], lpRefineOptionData->NeedItem_Index[iCount])) {
                    if (iCount == 0) {
            ...
            if (btIsCorrectItem == TRUE) {
                ...
                g_Log.Add(
                        "[PentagramRefine] User:%s(%s) Source Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], BonusOption[%d]",
                        lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->pPentagramMixBox[i].GetName(),
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Level,
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Option3,
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Number,
                        (int) lpObj->m_PlayerData->pPentagramMixBox[i].m_Durability, ExOption[0], ExOption[1],
                        ExOption[2], ExOption[3], ExOption[4], ExOption[5], ExOption[6],
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_SetOption,
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_ItemOptionEx >> 7,
                        g_kJewelOfHarmonySystem.GetItemStrengthenOption(&lpObj->m_PlayerData->pPentagramMixBox[i]),
                        g_kJewelOfHarmonySystem.GetItemOptionLevel(&lpObj->m_PlayerData->pPentagramMixBox[i]),
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[0],
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[1],
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[2],
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[3],
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[4],
                        lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption);
    ...
    if (iRandRate >= iSuccessRate) {
        ...
        g_Log.Add("[PentagramRefine] [%s][%s] Refine(Mix) Fail!!! (%d/%d)", lpObj->AccountID, lpObj->Name, iSuccessRate,
                  iRandRate);
    } else {
        ...
        for (int i = 0; i < iAcquisitionItemKind; i++) {
            ...
            if ((rand() % 10000) < iAcquisitionRate) {
                ...
                iResultItemType = ITEMGET(
                        lpRefineSuccessOptionData->Success_AcquisitionItem_Type[iFirstAcquisitionItemArrayNo],
                        lpRefineSuccessOptionData->Success_AcquisitionItem_Index[iFirstAcquisitionItemArrayNo]);
                ...
                for (int j = 0; j < iResultItemNum; j++) {
                    ItemSerialCreateSend(iIndex, 230, 0, 0, iResultItemType, 0, iItemDurability, 0, 0, 0, 0, 0, 0, 0,
                                         SocketOption, btBonusSocketOption);
                    btReturnValue = TRUE;
                }
    ...
    if (iUpgradeType == 1) {
            ...
            if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE) {
                for (int n = 0; n < 4; n++) {
                    if (this->m_JewelRankUpgradeNeedItemData[n].TargetItemRank == iTargetValue) {
                        for (int k = 0; k < 4; k++) {
                            if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                                ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[0],
                                        this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[0])
                                && lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                                   ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[1],
                                           this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[1])
                                && lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                                   ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[2],
                                           this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[2])) {
                                if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type == ITEMGET(14, 189)) {
                                    ...
                                    lpObj->m_PlayerData->m_PentagramMixLock = FALSE;
                                    g_Log.Add(
                                            "[PentagramJewel_Upgrade] [%s][%s] RankUpgrade Source Item Is Not Match!, PentagramJewelUpgrade Failed",
                                            lpObj->AccountID, lpObj->Name);

                                    return PENTAGRAM_MIX_ITEMS_COUNT_MISMATCH;
                                } else if (this->CheckLevelCondition(&lpObj->m_PlayerData->pPentagramMixBox[i], 7, 0, 0,
                                                                     1, 0, 0)) {
                                    iSetItemCount++;
                                    iSetItemMoney += lpObj->m_PlayerData->pPentagramMixBox[i].m_BuyMoney;
                                    bSourceItem = TRUE;
                                } else if (
                                        g_PentagramSystem.IsPentagramJewel(&lpObj->m_PlayerData->pPentagramMixBox[i]) ==
                                        TRUE) {
                                    iPentagramJewelID = lpObj->m_PlayerData->pPentagramMixBox[i].m_Type;
                                    iPentagramJewelCurRank =
                                            (lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption & 0xF0) >> 4;

                                    for (int l = 0; l < 5; l++) {
                                        if (iPentagramJewelCurRank == l + 1) {
                                            iSourceJewelLevel =
                                                    (lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[l] & 0xF0)
                                                            >> 4;
                                        }
                            ...
                            if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type ==
                                ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[k],
                                        this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[k])) {
                                SourceItemCount[k]++;
                                bSourceItem = TRUE;
                                break;
                            }
                        ...
                        if (bSourceItem) {
                            ...
                            g_Log.Add(
                                    "[PentagramRankUpgrade] User:%s(%s) Source Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], BonusOption[%d]",
                                    lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->pPentagramMixBox[i].GetName(),
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_Level,
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_Option3,
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_Number,
                                    (int) lpObj->m_PlayerData->pPentagramMixBox[i].m_Durability, ExOption[0],
                                    ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5], ExOption[6],
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_SetOption,
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_ItemOptionEx >> 7,
                                    g_kJewelOfHarmonySystem.GetItemStrengthenOption(
                                            &lpObj->m_PlayerData->pPentagramMixBox[i]),
                                    g_kJewelOfHarmonySystem.GetItemOptionLevel(
                                            &lpObj->m_PlayerData->pPentagramMixBox[i]),
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[0],
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[1],
                                    lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[2],
        ...
        for (int m = 0; m < 4; m++) {
            if (this->m_JewelRankUpgradeNeedItemData[m].TargetItemRank == iRankCount + 1) {
                for (int i = 0; i < 4; i++) {
                    if (this->m_JewelRankUpgradeNeedItemData[m].NeedSourceItem_Num[i] != SourceItemCount[i] &&
                        (i != 3 || this->m_JewelRankUpgradeNeedItemData[m].NeedSourceItem_Num[3] != iSetItemCount)) {
                        lpObj->m_PlayerData->m_PentagramMixLock = FALSE;
        ...
        if (iRandRate >= iPentagramMixSuccessRate) {
            for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
                if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE &&
                    ...
                    g_Log.Add(
                            "[PentagramRankUpgradeFail] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d], CurRank[%d]",
                            lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->pPentagramMixBox[i].GetName(),
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Level,
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
                            ...
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[3],
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[4],
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption & 0x0F,
                            (lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption & 0xF0) >> 4);
                    break;
            ...
            if (btPentagramChaosCharm == TRUE) {
                ...
                for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
                    if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE
                        ...
                        Item.Convert(lpObj->m_PlayerData->pPentagramMixBox[i].m_Type,
                                     lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                                     lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
                                     lpObj->m_PlayerData->pPentagramMixBox[i].m_Option3,
                                     lpObj->m_PlayerData->pPentagramMixBox[i].m_NewOption,
                                     lpObj->m_PlayerData->pPentagramMixBox[i].m_SetOption,
                                     0, lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption,
                                     lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption, 0,
                                     CURRENT_DB_VERSION);
                        break;
                    }
                }
                ...
                if (btResult == 0xFF) {
                    ...
                    g_Log.Add(
                            "[PentagramUpgrade][%s][%s] Pentagram Rank Upgrade Fail with PentagramChaosCharm - ItemType : %d, ItemIndex : %d, ItemLevel : %d --> 0",
                            lpObj->AccountID, lpObj->Name, ItemType, ItemIndex, iLevel);
                    btReturnValue = TRUE;
                }
            } else {
                this->PentagramMixBoxInit(lpObj);
                g_Log.Add("[PentagramUpgrade] [%s][%s] Rank Upgrade Fail!! (%d/%d)", lpObj->AccountID, lpObj->Name,
                          iPentagramMixSuccessRate, iRandRate);
            ...
            for (int n = 0; n < 250; n++) {
                if (ITEMGET(this->m_JewelItemOptionData[n].PentagramJewel_Type,
                            this->m_JewelItemOptionData[n].PentagramJewel_Index) == iPentagramJewelID &&
                    this->m_JewelItemOptionData[n].ItemRank == iRankCount + 1) {
                    if (iRand >= iOptionRate &&
                        iRand < this->m_JewelItemOptionData[n].RankOptionNumRate + iOptionRate) {
            ...
            for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
                if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE &&
                    ...
                    lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption =
                            btMainAttribute | 0x10 * iSuccessRank;
                    Item.m_Level = lpObj->m_PlayerData->pPentagramMixBox[i].m_Level;
                    Item.m_Durability = lpObj->m_PlayerData->pPentagramMixBox[i].m_Durability;
                    Item.m_Number = lpObj->m_PlayerData->pPentagramMixBox[i].m_Number;
                    ...
                    Item.Convert(lpObj->m_PlayerData->pPentagramMixBox[i].m_Type,
                                 ...
                                 lpObj->m_PlayerData->pPentagramMixBox[i].m_SetOption,
                                 0, lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption,
                                 lpObj->m_PlayerData->pPentagramMixBox[i].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
                    break;
                }
            }
            ...
            if (btResult == 0xFF) {
                ...
                g_Log.Add(
                        "[PentagramRankUpgrade] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d], CurRank[%d]",
                        lpObj->AccountID, lpObj->Name, Item.GetName(),
                        Item.m_Level, Item.m_Option1,
                        Item.m_Option2, Item.m_Option3,
                        Item.m_Number, (int) Item.m_Durability, ExOption[0], ExOption[1], ExOption[2], ExOption[3],
                        ExOption[4], ExOption[5], ExOption[6],
        ...
        for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
            if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE) {
                for (int j = 0; j < 4; j++) {
                    if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                        ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[0],
                                this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[0])
                        && lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                           ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[1],
                                   this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[1])
                        && lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                           ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[2],
                                   this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[2])
                        && lpObj->m_PlayerData->pPentagramMixBox[i].m_Type !=
                           ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[3],
                                   this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[3])) {
                        if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type == ITEMGET(14, 189)) {
                            ...
                            lpObj->m_PlayerData->m_PentagramMixLock = FALSE;
                            g_Log.Add(
                                    "[PentagramJewel_Upgrade] [%s][%s] LevelUpgrade Source Item Is Not Match!, PentagramJewelUpgrade Failed",
                                    lpObj->AccountID, lpObj->Name);
                    ...
                    if (lpObj->m_PlayerData->pPentagramMixBox[i].m_Type ==
                        ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[j],
                                this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[j])) {
                        iSourceItemCount[j]++;
                        bSourceItem = TRUE;
                        break;
                    }
                ...
                if (bSourceItem == TRUE) {
                    ...
                    g_Log.Add(
                            "[PentagramLevelUpgrade] User:%s(%s) Source Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], BonusOption[%d]",
                            lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->pPentagramMixBox[i].GetName(),
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Level,
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
        ...
        for (int i = 0; i < 250; i++) {
            if (ITEMGET(this->m_JewelItemOptionData[i].PentagramJewel_Type,
                        this->m_JewelItemOptionData[i].PentagramJewel_Index) == iSourceJewelType &&
        ...
        if (iRandRate >= iUpgradeSuccessRate) {
            for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
                if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE &&
                    ...
                    g_Log.Add(
                            "[PentagramLevelUpgradeFail] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d], CurRank[%d]",
                            lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->pPentagramMixBox[i].GetName(),
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Level,
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                            lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
            ...
            if (bPentagramChaosCharm == TRUE) {
                ...
                if (btResult == 0xFF) {
                    ...
                    IOCP.DataSend(iIndex, (LPBYTE) & pMsg, pMsg.h.size);
                    g_Log.Add(
                            "[PentagramUpgrade][%s][%s] Pentagram Level Upgrade Fail with PentagramChaosCharm - ItemType : %d, ItemIndex : %d, ItemLevel : %d --> 0",
                            lpObj->AccountID, lpObj->Name, ItemType, ItemIndex, iLevel);
                    btReturnValue = TRUE;
                }
            } else {
                this->PentagramMixBoxInit(lpObj);
                g_Log.Add("[PentagramUpgrade] [%s][%s] Level Upgrade Fail!! (%d/%d)", lpObj->AccountID, lpObj->Name,
                          iUpgradeSuccessRate, iRandRate);
                btReturnValue = PENTAGRAM_MIX_UPGRADE_FAILED;
            }
        } else {
            CItem pModifyItem;
            ...
            for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
                if (lpObj->m_PlayerData->pPentagramMixBox[i].IsItem() == TRUE &&
                    ...
                    lpObj->m_PlayerData->pPentagramMixBox[i].m_SocketOption[iCurRank - 1] =
                            btRankOptionNum | 0x10 * iItemTargetLevel;
                    ...
                    pModifyItem.Convert(lpObj->m_PlayerData->pPentagramMixBox[i].m_Type,
                                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Option1,
                                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Option2,
                                        lpObj->m_PlayerData->pPentagramMixBox[i].m_Option3,
                                        lpObj->m_PlayerData->pPentagramMixBox[i].m_NewOption,
            ...
            if (btResult == 0xFF) {
                ...
                g_Log.Add(
                        "[PentagramLevelUpgrade] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d], CurRank[%d]",
                        ...
                        pModifyItem.m_Number, (int) pModifyItem.m_Durability, ExOption[0], ExOption[1], ExOption[2],
                        ExOption[3], ExOption[4], ExOption[5], ExOption[6],
                        pModifyItem.m_SetOption, pModifyItem.m_ItemOptionEx >> 7,
                        g_kJewelOfHarmonySystem.GetItemStrengthenOption(&pModifyItem),
                        g_kJewelOfHarmonySystem.GetItemOptionLevel(&pModifyItem),
...
int CPentagramMixSystem::CheckLevelCondition(CItem *lpItem, WORD Level, BYTE Op1, BYTE Op2, BYTE Op3, BYTE SetOption,
                                             BYTE NewOption) {

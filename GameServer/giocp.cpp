        ...
        if (CreateListenSocket() == 0) {
            ...
            while (true) {
                ...
                if (UpdateCompletionPort(Accept, ClientIndex, 1) == 0) {
                    g_Log.AddC(TColor::Azure, "error-L1 : %d %d CreateIoCompletionPort failed with error %d", Accept,
                               ClientIndex, GetLastError());
                    closesocket(Accept);
                    LeaveCriticalSection(&criti);
                    continue;
                }
                ...
                if (gObjAdd(Accept, inet_ntoa(cInAddr), ClientIndex) == -1) {
                    g_Log.AddC(TColor::Azure, "error-L1 : %d %d gObjAdd() failed with error %d", Accept, ClientIndex,
                               GetLastError());
                    LeaveCriticalSection(&criti);
                    closesocket(Accept);
                    continue;
                ...
                nRet = WSARecv(Accept, &gObj[ClientIndex].PerSocketContext->IOContext[0].wsabuf, 1,
                               (unsigned long *) &RecvBytes, &Flags,
...
DWORD CIOCP::ServerWorkerThread() {
    ...
    while (true) {
        bSuccess = GetQueuedCompletionStatus(CompletionPort, &dwIoSize, &ClientIndex, &lpOverlapped,
                                             -1); // WAIT_FOREVER
    ...
    while (true) {
        ...
        if (size <= lpIOContext->nSentBytes) {
            if (xcode == 0xC3) {
                ...
                if (ret < 0) {
                    g_Log.AddC(TColor::Red, "[%s][Packet-Decrypt BYTE] Error: ret < 0 %x/%x/%x)",
                               gObj[uIndex].m_PlayerData->Ip_addr, recvbuf[lOfs], recvbuf[lOfs + 1], recvbuf[lOfs + 2]);
                } else {
                    ...
                    if (gObj[uIndex].m_PlayerData->PacketsPerSecond >= g_ConfigRead.data.common.PacketLimit) {
                        g_Log.AddC(TColor::Red, "[ANTI-HACK] Packets Per Second: %d / %d",
                                   gObj[uIndex].m_PlayerData->PacketsPerSecond, g_ConfigRead.data.common.PacketLimit);
                    ...
                    if (PacketStream.AddData(byDec, ret + 2) == 0) {
                        ...
                        g_Log.AddC(TColor::Red,
                                   "error-L1 : CStreamPacketEngine Adding Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d",
                    ...
                    if (PacketStream.ExtractPacket(byDec) != 0) {
                        g_Log.AddC(TColor::Red,
                                   "error-L1 : CStreamPacketEngine ExtractPacket Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d",
                                   gObj[uIndex].m_PlayerData->Ip_addr, gObj[uIndex].AccountID, gObj[uIndex].Name,
                                   headcode, __FILE__, __LINE__, gObj[uIndex].Connected);
                ...
                if (ret < 0) {
                    g_Log.AddC(TColor::Red, "[Packet-Decrypt WORD] Error: ret < 0 %x/%x/%x)", recvbuf[lOfs],
                               recvbuf[lOfs + 1], recvbuf[lOfs + 2]);
                } else {
                    unsigned char *pDecBuf = byDec;
                ...
                if (ps.AddData(&recvbuf[lOfs], size) == 0) {
                    g_Log.AddC(TColor::Red,
                               "error-L1 : CStreamPacketEngine Adding Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d",
                               gObj[uIndex].m_PlayerData->Ip_addr, gObj[uIndex].AccountID, gObj[uIndex].Name, headcode,
...
bool CIOCP::DataSend(int aIndex, unsigned char *lpMsg, DWORD dwSize, bool Encrypt) {
    ...
    if (lpMsg[0] != 0xC1 && lpMsg[0] != 0xC2 &&
        ...
        g_Log.AddC(TColor::Red, "[ERROR] Trying to send packet without HEADER (%s)(%s)", gObj[aIndex].AccountID,
                   gObj[aIndex].Name);
        LeaveCriticalSection(&criti);
        return FALSE;
    }
    ...
    if (lpIoCtxt->nWaitIO > 0) {
        if ((lpIoCtxt->nSecondOfs + dwSize) > MAX_IO_BUFFER_SIZE - 1) {
            g_Log.Add("(%d)error-L2 MAX BUFFER OVER %d %d %d [%s][%s]", aIndex, lpIoCtxt->nTotalBytes,
                      lpIoCtxt->nSecondOfs, dwSize, gObj[aIndex].AccountID, gObj[aIndex].Name);
            lpIoCtxt->nWaitIO = 0;
            CloseClient(aIndex);
            LeaveCriticalSection(&criti);
            return true;
    ...
    if ((lpIoCtxt->nTotalBytes + dwSize) > MAX_IO_BUFFER_SIZE - 1) {
        g_Log.Add("(%d)error-L2 MAX BUFFER OVER %d %d [%s][%s]", aIndex, lpIoCtxt->nTotalBytes, dwSize,
...
bool CIOCP::IoSendSecond(_PER_SOCKET_CONTEXT *lpPerSocketContext) {
    ...
    if (WSASend(gObj[aIndex].m_socket, &lpIoCtxt->wsabuf, 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1) {
        if (WSAGetLastError() != WSA_IO_PENDING) {
            g_Log.Add("WSASend(%d) failed with error %d %s ", __LINE__, WSAGetLastError(),
                      gObj[aIndex].m_PlayerData->Ip_addr);
            CloseClient(aIndex);
            LeaveCriticalSection(&criti);
            return false;
        }

...
bool CSkillSafeZoneUse::CanUseSkill(LPOBJ lpObj, CMagicInf *lpMagic) {
    ...
    for (std::vector<SKILL_SAFEZONE_USE>::iterator It = this->m_vtSkillData.begin();
         It != this->m_vtSkillData.end(); It++) {
        if (It->wSkill == lpMagic->m_Skill) {
            for (std::vector<SKILL_SAFEZONE_USE_AREA>::iterator It2 = It->m_vtArea.begin();
                 It2 != It->m_vtArea.end(); It2++) {
                if (It2->btMapNumber == lpObj->MapNumber && lpObj->X >= It2->StartX && lpObj->X <= It2->EndX &&
                    lpObj->Y >= It2->StartY && lpObj->Y <= It2->EndY) {
                    return true;
                }
            }
        }

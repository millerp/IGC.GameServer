...
BOOL MonsterHerd::SetTotalInfo(int iMapNumber, int iRadius, int iStartX, int iStartY) {
    ...
    if (((iStartX < 0) ? FALSE : (iStartX > 254) ? FALSE : TRUE) == FALSE ||
        ((iStartY < 0) ? FALSE : (iStartY > 254) ? FALSE : TRUE) == FALSE) {
        return FALSE;
    }
...
void MonsterHerd::Stop() {
    ...
    if (this->m_mapMonsterHerd.empty() == false) {
        ...
        if (this->m_mapMonsterHerd.empty() != false) {
            ...
            for (std::map<int, _MONSTER_HERD_DATA>::iterator _Iter = this->m_mapMonsterHerd.begin();
                 _Iter != this->m_mapMonsterHerd.end(); _Iter++) {
...
void MonsterHerd::BeenAttacked(LPOBJ lpObj, LPOBJ lpTargetObj) {
    ...
    for (std::map<int, _MONSTER_HERD_DATA>::iterator it = this->m_mapMonsterHerd.begin();
         it != this->m_mapMonsterHerd.end(); it++) {
                ...
                if (dis <= attackRange) {
                    ...
                    if (gObjMonsterGetTargetPos(lpObj) == TRUE) {
                        if (MAX_MAP_RANGE(lpObj->MapNumber) == FALSE) {
                            g_Log.AddC(TColor::Red, "[ERROR] MAX_MAP_RANGE (MonsterHerd) == FALSE (%d)",
                                       lpObj->MapNumber);
                            break;
                        }

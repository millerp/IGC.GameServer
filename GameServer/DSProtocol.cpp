...
void
(*ItemSerialCreateSend)(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int type, BYTE level, BYTE dur, BYTE Op1, BYTE Op2,
                        BYTE Op3, int LootIndex, BYTE NewOption, BYTE SetOption, time_t lDuration, BYTE *SocketOption,
                        BYTE MainAttribute);
    ...
    if (pCLCount.Count > 0) {
        ...
        for (int n = 0; n < pCLCount.Count; n++) {
            ...
            if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
                ...
                gObj[aIndex].m_PlayerData->m_NameConvertOfUBF.InputData(n, tmpKeyName.c_str(), tmpRealName.c_str(),
                                                                        lpCL->ServerCodeOfHomeWorld, MAX_ACCOUNT_LEN);
                g_Log.Add("[UBF][ConvertInputData][%d][%s][KeyName: %s][RealName: %s] ServerCode: %d",
                          aIndex, gObj[aIndex].AccountID, tmpKeyName.c_str(), tmpRealName.c_str(),
                          lpCL->ServerCodeOfHomeWorld);
            ...
            if (lpCL->dbInventory[0] == (BYTE) - 1 && (lpCL->dbInventory[2] & 0x80) == 0x80 &&
                (lpCL->dbInventory[3] & 0xF0) == 0xF0) {
                TempInventory[0] = -1;
            } else {
                TempInventory[0] =
                        (lpCL->dbInventory[0] + (lpCL->dbInventory[2] & 0x80) * 2) + (lpCL->dbInventory[3] & 0xF0) * 32;
            }

            if (lpCL->dbInventory[4] == (BYTE) - 1 && (lpCL->dbInventory[6] & 0x80) == 0x80 &&
                (lpCL->dbInventory[7] & 0xF0) == 0xF0) {
                TempInventory[1] = -1;
            } else {
                TempInventory[1] =
                        (lpCL->dbInventory[4] + (lpCL->dbInventory[6] & 0x80) * 2) + (lpCL->dbInventory[7] & 0xF0) * 32;
            }

            if (lpCL->dbInventory[8] == (BYTE) - 1 && (lpCL->dbInventory[10] & 0x80) == 0x80 &&
                (lpCL->dbInventory[11] & 0xF0) == 0xF0) {
                TempInventory[2] = 0x1FF;
            } else {
                TempInventory[2] = ((lpCL->dbInventory[8] + (lpCL->dbInventory[10] & 0x80) * 2) +
                                    (lpCL->dbInventory[11] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }

            if (lpCL->dbInventory[12] == (BYTE) - 1 && (lpCL->dbInventory[14] & 0x80) == 0x80 &&
                (lpCL->dbInventory[15] & 0xF0) == 0xF0) {
                TempInventory[3] = 0x1FF;
            } else {
                TempInventory[3] = ((lpCL->dbInventory[12] + (lpCL->dbInventory[14] & 0x80) * 2) +
                                    (lpCL->dbInventory[15] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }

            if (lpCL->dbInventory[16] == (BYTE) - 1 && (lpCL->dbInventory[18] & 0x80) == 0x80 &&
                (lpCL->dbInventory[19] & 0xF0) == 0xF0) {
                TempInventory[4] = 0x1FF;
            } else {
                TempInventory[4] = ((lpCL->dbInventory[16] + (lpCL->dbInventory[18] & 0x80) * 2) +
                                    (lpCL->dbInventory[19] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }

            if (lpCL->dbInventory[20] == (BYTE) - 1 && (lpCL->dbInventory[22] & 0x80) == 0x80 &&
                (lpCL->dbInventory[23] & 0xF0) == 0xF0) {
                TempInventory[5] = 0x1FF;
            } else {
                TempInventory[5] = ((lpCL->dbInventory[20] + (lpCL->dbInventory[22] & 0x80) * 2) +
                                    (lpCL->dbInventory[23] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }

            if (lpCL->dbInventory[24] == (BYTE) - 1 && (lpCL->dbInventory[26] & 0x80) == 0x80 &&
                (lpCL->dbInventory[27] & 0xF0) == 0xF0) {
                TempInventory[6] = 0x1FF;
            } else {
                TempInventory[6] = ((lpCL->dbInventory[24] + (lpCL->dbInventory[26] & 0x80) * 2) +
                                    (lpCL->dbInventory[27] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }

            if (lpCL->dbInventory[28] == (BYTE) - 1 && (lpCL->dbInventory[30] & 0x80) == 0x80 &&
                (lpCL->dbInventory[31] & 0xF0) == 0xF0) {
                TempInventory[7] = 0x1FF;
            } else {
                TempInventory[7] = ((lpCL->dbInventory[28] + (lpCL->dbInventory[30] & 0x80) * 2) +
                                    (lpCL->dbInventory[31] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }

            if (lpCL->dbInventory[32] == (BYTE) - 1 && (lpCL->dbInventory[34] & 0x80) == 0x80 &&
                (lpCL->dbInventory[35] & 0xF0) == 0xF0) {
                TempInventory[8] = 0x1FF;
            } else {
                TempInventory[8] = ((lpCL->dbInventory[32] + (lpCL->dbInventory[34] & 0x80) * 2) +
                                    (lpCL->dbInventory[35] & 0xF0) * 32) % MAX_SUBTYPE_ITEMS;
            }
            ...
            if (g_ConfigRead.SeasonX == false) {
                ...
                } else if ((TempInventory[7] >= 3 && TempInventory[7] <= 6) || TempInventory[7] == 30 ||
                           TempInventory[7] == 42 || TempInventory[7] == 49 || TempInventory[7] == 269) {
                    pCList.CharSet[5] |= 8;
                ...
                } else if ((TempInventory[7] >= 36 && TempInventory[7] <= 40) || TempInventory[7] == 43 ||
                           TempInventory[7] == 50) {
                    pCList.CharSet[5] |= 12;
            ...
            } else // New Season X Charset for Wings
            {
                ...
                } else if ((TempInventory[7] >= 49 && TempInventory[7] <= 50) ||
                           (TempInventory[7] >= 130 && TempInventory[7] <= 135)) {
...
void JGGetCharacterInfo(SDHP_DBCHAR_INFORESULT *lpMsg) {
    ...
    if ((lpMsg->CtlCode & 1) != 0) {
        g_Log.AddC(TColor::Red, "error-L1 : Load Block Character infomation", szName,
                   gObj[aIndex].AccountID);    // Deathway Fix HERE
        IOCP.CloseClient(aIndex);
    ...
    for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++) {
        if (gObj[i].Connected == PLAYER_PLAYING) {
            if (gObj[i].Type == OBJ_USER) {
                if (!strncmp(szName, gObj[i].Name, MAX_ACCOUNT_LEN) ||
                    !strncmp(szAccountId, gObj[i].AccountID, MAX_ACCOUNT_LEN)) {
    ...
    if (gObjSetCharacter((UCHAR *) lpMsg, aIndex) == false) {
        ...
        g_Log.AddC(TColor::Red, "CL:%d LV:%d MLV:%d IDX:%d CS:%d", lpMsg->Class, lpMsg->Level, lpMsg->mLevel, aIndex,
    ...
    if (lpObj->m_PlayerData->Money < 0) {
        g_Log.AddC(TColor::Red, "ERROR: MONEY < 0 [%s][%s] %d -> 0", szName, gObj[aIndex].AccountID,
    ...
    if (lpObj->m_bMapSvrMoveReq == false) {
        ...
        if (lpObj->m_PK_Level > 4) {
            ...
            g_Log.Add(
                    "[CRenewal][PKSetting][%s][%s] PkLevel:[%d],KillCount:[%d], PkPoint:[%d] PK Character move to LORENCIA",
                    lpObj->AccountID, lpObj->Name, lpObj->m_PK_Level, lpObj->m_PK_Count, lpObj->m_PK_Time);
        }

        short wGameServerCode = g_MapServerManager.CheckMoveMapSvr(lpObj->m_Index, lpObj->MapNumber,
                                                                   lpObj->m_sPrevMapSvrCode);
    ...
    g_UnityBattleField.GDReqCheckJoinedUnityBattleField(aIndex,
                                                        g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE ? TRUE
                                                                                                                 : FALSE,
                                                        0);
    ...
    GSProtocol.GCReFillSend(aIndex, gObj[aIndex].AddLife + gObj[aIndex].MaxLife, 0xFE, 0,
                            gObj[aIndex].iAddShield + gObj[aIndex].iMaxShield);
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove) {
    ...
    if (bMapServerMove == TRUE) {
        ...
        g_Log.AddC(TColor::Brown,
                   "[MapServerMng][%s][%s] GJSetCharacterInfo() - MapServerMove Character Save Start (%d)",
                   lpObj->AccountID, lpObj->Name, lpObj->m_Index);
    }
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE *lpMsg) {
    ...
    if (!gObjIsAccontConnect(aIndex, szId)) {
        g_Log.AddC(TColor::Red, "Error-L1 : Request to receive Warehouse information doesn't match the user [%s][%d]",
                   szId, aIndex);
        return;
    }
    ...
    for (int n = 0; n < WAREHOUSE_SIZE; n++) {
        ...
        if (lpMsg->dbItems[n * MAX_DBITEM_INFO] == (BYTE) - 1 &&
            (lpMsg->dbItems[n * MAX_DBITEM_INFO + 9] & 0xF0) == 0xF0 &&
            (lpMsg->dbItems[n * MAX_DBITEM_INFO + 7] & 0x80) == 0x80) {
            itype = -1;
        }
        ...
        if (itype != -1) {
            ...
            item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3,
                         ...
                         SocketOptionIndex, item.m_PeriodItemOption, CURRENT_DB_VERSION);
...
void
ItemCreate(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int type, BYTE level, BYTE dur, BYTE Op1, BYTE Op2, BYTE Op3,
           int LootIndex, BYTE NewOption, BYTE SetOption, time_t lDuration, BYTE *SocketOption, BYTE MainAttribute) {
    ...
    if (ObjectMaxRange(aIndex) != FALSE && gObj[aIndex].Type != OBJ_USER) {
        ...
        g_Log.Add(
                "Item Create Request [%s] [%d][%d][%d] : [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] MainAttr:[%d] Expire:[%I64d]",
                ...
                NewOption[3], NewOption[4], NewOption[5], NewOption[6], pMsg.SetOption, pMsg.MainAttribute,
                pMsg.lDuration);
    }
...
void
PetItemSerialCreateSend(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int type, BYTE level, BYTE dur, BYTE Op1, BYTE Op2,
                        BYTE Op3, int LootIndex, BYTE NewOption, BYTE SetOption) {
    SDHP_ITEMCREATE pMsg;
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV *lpMsg) {
    ...
    if (lpMsg->MapNumber == (BYTE) - 1 || lpMsg->MapNumber == (BYTE) - 2) {
        ...
        NewItem.Convert(lpMsg->Type, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0,
                        SocketOption, SocketBonusOption, 0, CURRENT_DB_VERSION);
        ...
        if (btPos == 0xFF) {
            ...
            g_Log.Add("[CBMix] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]",
                      ITEM_GET_TYPE(NewItem.m_Type), ITEM_GET_INDEX(NewItem.m_Type), lpObj->AccountID, lpObj->Name);
    ...
    if (lpMsg->MapNumber == 230) {
        ...
        Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption,
                     0, lpMsg->SocketOption, lpMsg->MainAttribute, 0, CURRENT_DB_VERSION);
        ...
        if (btPos == 0xFF) {
            ...
            g_Log.Add(
                    "[PentagramRefine] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d]",
                    lpObj->AccountID, lpObj->Name, Item.GetName(), Item.m_Level, Item.m_Option1, Item.m_Option2,
                    Item.m_Option3, Item.m_Number, Item.m_Durability,
                    ExOption[0], ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5], ExOption[6],
                    Item.m_SetOption, Item.m_ItemOptionEx >> 7,
                    g_kJewelOfHarmonySystem.GetItemStrengthenOption(&Item),
                    g_kJewelOfHarmonySystem.GetItemOptionLevel(&Item),
                    Item.m_SocketOption[0], Item.m_SocketOption[1], Item.m_SocketOption[2], Item.m_SocketOption[3],
        ...
        if (btRes == 0xFF) {
            ...
            g_Log.Add(
                    "[PentagramJewel Out Success] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d]",
                    lpObj->AccountID, lpObj->Name, Item.GetName(), Item.m_Level, Item.m_Option1, Item.m_Option2,
                    Item.m_Option3, Item.m_Number, Item.m_Durability,
                    ExOption[0], ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5], ExOption[6],
                    Item.m_SetOption, Item.m_ItemOptionEx >> 7,
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
            ...
            Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                         lpMsg->SetOption, 0, lpMsg->SocketOption, lpMsg->MainAttribute, 0, CURRENT_DB_VERSION);
            ...
            if (iItemPos == (BYTE) - 1) {
                ...
                GSProtocol.GCServerMsgStringSendEx(lpMsg->aIndex, 1, Lang.GetText(0, 535),
                                                   ItemAttribute[Item.m_Type].Name);
            }
        }
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
            ...
            Item.m_Durability = ItemGetDurability(ITEMGET(iType, iTypeIndex), Item.m_Level, Item.m_NewOption,
                                                  Item.m_SetOption);
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
            ...
            pCreateItem.Convert(ITEMGET(iItemType, iItemIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                                lpMsg->SetOption, 0, lpMsg->SocketOption, lpMsg->MainAttribute, 0, CURRENT_DB_VERSION);
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_LOGGED) {
            ...
            if (bIsBlank == TRUE) {
                BYTE btItemPos = gObjInventoryInsertItem(lpMsg->aIndex, pCreateItem);
                if (btItemPos == 0xFF) {
                    g_Log.Add("[Gambling Trade] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]",
                              iItemType, iItemIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
                ...
                int iBuyTaxMoney = (g_GamblingItemBag.GetGamblingValue() *
                                    (__int64) g_CastleSiegeSync.GetTaxRateChaos(gObj[lpMsg->aIndex].m_Index) /
                                    (__int64) 100);
                ...
                if (lpMsg->Type == ITEMGET(3, 11) ||
                    ...
                    g_Log.Add(
                            "[Gambling Unique] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%u][%d] Ex:[%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d]",
                            gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name, pCreateItem.GetName(),
                            pCreateItem.m_Level, pCreateItem.m_Option1, pCreateItem.m_Option2,
                            pCreateItem.m_Option3, pCreateItem.m_Number, (int) pCreateItem.m_Durability, ExOption[0],
                            ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5],
                            lpMsg->SetOption, pCreateItem.m_ItemOptionEx >> 7,
                            g_kJewelOfHarmonySystem.GetItemStrengthenOption(&pCreateItem),
                            g_kJewelOfHarmonySystem.GetItemOptionLevel(&pCreateItem),
                            pCreateItem.m_SocketOption[0], pCreateItem.m_SocketOption[1], pCreateItem.m_SocketOption[2],
                            pCreateItem.m_SocketOption[3], pCreateItem.m_SocketOption[4]);
                } else {
                    g_Log.Add(
                            "[Gambling Normal] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%u][%d] Ex:[%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d]",
                            gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name, pCreateItem.GetName(),
                            pCreateItem.m_Level, pCreateItem.m_Option1, pCreateItem.m_Option2,
                            pCreateItem.m_Option3, pCreateItem.m_Number, (int) pCreateItem.m_Durability, ExOption[0],
                            ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5],
                ...
                g_Log.Add("[Gambling Fail] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", iItemType,
                          iItemIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
                PHeadSetB((LPBYTE) & pResult, 0x32, sizeof(pResult));
                pResult.Result = 0xFF;
                IOCP.DataSend(aIndex, (LPBYTE) & pResult, pResult.h.size);
                gObj[aIndex].ChaosLock = FALSE;
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
            ...
            if (iItemPos == (BYTE) - 1) {
                ...
                g_Log.Add("[GensRanking] Reward User:[%s][%s] Influence:%d, GensClass:%d, ContributePoint:%d, Item:%s",
                          ...
                          gObj[lpMsg->aIndex].m_PlayerData->m_GensInfluence,
                          gObj[lpMsg->aIndex].m_PlayerData->m_GensClass,
                          gObj[lpMsg->aIndex].m_PlayerData->m_ContributePoint, Item.GetName());
        ...
        if (g_CMuunSystem.IsMuunItem(lpMsg->Type) == TRUE) {
            ...
            Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                         lpMsg->SetOption, 0, lpMsg->SocketOption, SocketOptionIndex, 0, CURRENT_DB_VERSION);
            g_Log.Add(
                    "[MuunSystem] Drop Muun Item (Muun Rank : %d[%d]) - %s, [%d,%d,%d,%d], Serial : %I64d, ItemCode : %d, Level : %d, MapNumber : %d(%d/%d)",
        ...
        if (g_CMuunSystem.IsMuunItem(lpMsg->Type) == TRUE) {
            int nMuunRank = g_CMuunSystem.GetMuunRankOfMuunInfo(lpMsg->Type);
            int iItemCount = MapC[gObj[lpMsg->aIndex].MapNumber].MonsterItemDrop(lpMsg->Type, lpMsg->Level, lpMsg->Dur,
                                                                                 lpMsg->x,
                                                                                 lpMsg->y, lpMsg->Op1, lpMsg->Op2,
                                                                                 lpMsg->Op3, lpMsg->NewOption,
                                                                                 lpMsg->SetOption,
                                                                                 lootindex, lpMsg->m_Number, 0,
                                                                                 lpMsg->SocketOption, nMuunRank,
                                                                                 lpMsg->lDuration);
        } else {
            int iItemCount = MapC[gObj[lpMsg->aIndex].MapNumber].MonsterItemDrop(lpMsg->Type, lpMsg->Level, lpMsg->Dur,
                                                                                 lpMsg->x,
        ...
        g_Log.Add(
                "[GameMaster Command] Item Drop [%d][%d][%d] : serial:%I64d [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
        ...
        if (gObj[lpMsg->aIndex].Connected > 1) {
            ...
            if (btItemPos == (BYTE) - 1) {
                g_Log.Add("[EventInven][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
                          iType, iTypeIndex, Item.m_Level, Item.m_Number, gObj[lpMsg->aIndex].AccountID,
                          gObj[lpMsg->aIndex].Name);
            } else {
                GSProtocol.GCEventInvenItemOneSend(lpMsg->aIndex, btItemPos);
                g_Log.Add(
                        "[EventInven][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
                        iType, iTypeIndex, Item.m_Level, Item.m_Number, gObj[lpMsg->aIndex].AccountID,
                        gObj[lpMsg->aIndex].Name);
            }
        }
        ...
        if (gObj[lpMsg->aIndex].Connected > 1) {
            ...
            if (!pCMuRummyInfo) {
                g_Log.Add("[MuRummy][Error] pCMuRummyInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name,
                          __FILE__, __LINE__);
                return;
            }
            ...
            if (g_PentagramSystem.IsPentagramItem(lpMsg->Type) == TRUE) {
                ...
                Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                             lpMsg->SetOption, 0, btSocketSlotCount, lpMsg->MainAttribute, 0, CURRENT_DB_VERSION);
            } else {
                Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                             lpMsg->SetOption, 0, lpMsg->SocketOption, lpMsg->MainAttribute, 0, CURRENT_DB_VERSION);
            }
            ...
            if (btItemPos == (BYTE) - 1) {
                ...
                g_Log.Add("[MuRummy][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
                ...
                g_Log.Add(
                        "[%s][%s][Mu Rummy] Event Reward Result [Point : %d, RewardItem:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]",
                        lpObj->AccountID, lpObj->Name, pCMuRummyInfo->GetScore(), Item.GetName(), Item.m_Level,
                        Item.m_Option1, Item.m_Option2, Item.m_Option3, Item.m_Number, (int) Item.m_Durability,
        ...
        if (gObj[lpMsg->aIndex].Connected > 1) {
            ...
            MuunItem.Convert(lpMsg->Type, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0,
                             SocketOption, nMuunRank, 0, 3);
            ...
            if (btItemPos == (BYTE) - 1) {
                ...
                g_Log.Add(
                        "[MuunSystem][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
            ...
            Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                         lpMsg->SetOption, 0, 0, -1, 0, CURRENT_DB_VERSION);
            ...
            if (iItemPos == (BYTE) - 1) {
                ...
                g_Log.Add(
                        "[MineSystem][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
                        iType, iTypeIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
                ::GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, iItemPos);
            }
        ...
        if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
            ...
            if (iItemPos == (BYTE) - 1) {
                ...
                g_Log.Add(
                        "[Shop Buy][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
                        iType, iTypeIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
            }
        ...
        if (gObj[lpMsg->aIndex].Connected > 1) {
            ...
            if (btItemPos == (BYTE) - 1) {
                ...
                g_Log.Add(
                        "[%s][%s][Mu Rummy] Event Reward Result [Point : %d, RewardMuunItem:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]",
                        lpObj->AccountID, lpObj->Name, pCMuRummyInfo->GetScore(), MuunItem.GetName(), MuunItem.m_Level,
                        MuunItem.m_Option1, MuunItem.m_Option2, MuunItem.m_Option3, MuunItem.m_Number,
                        (int) MuunItem.m_Durability,
                        NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5],
                        NewOption[6]);
        ...
        if (gObj[lpMsg->aIndex].Connected > 1) {
            ...
            Item.Convert(lpMsg->Type, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0,
                         lpMsg->SocketOption, 0, 0, 3);
            ...
            if (btItemPos == (BYTE) - 1) {
                ...
                g_Log.Add(
                        "[Extract Seed Success] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d]",
                        gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name, Item.GetName(), Item.m_Level,
                        Item.m_Option1, Item.m_Option2, Item.m_Option3, Item.m_Number,
                        Item.m_Durability, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4],
                        NewOption[5], NewOption[6], Item.m_SetOption, Item.m_ItemOptionEx >> 7,
                        g_kJewelOfHarmonySystem.GetItemStrengthenOption(&Item),
                        g_kJewelOfHarmonySystem.GetItemOptionLevel(&Item), Item.m_SocketOption[0],
        ...
        if (gObj[lpMsg->aIndex].Connected > 1) {
            ...
            Item.Convert(lpMsg->Type, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0,
                         SocketOption, nRank, 0, 3);

            BYTE btRet = g_CMuunSystem.DGMuunExchangeInsertInven(lpObj, Item, lpMsg->x);

            if (btRet == 0xFF) {
                g_Log.Add(
                        "[MuunSystem][MuunExchange][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",
                ...
                if (g_CMuunSystem.IsStoneofEvolution(lpMsg->Type) == TRUE) {
                    ...
                    if (lpItemAttribute != NULL) {
                        g_Log.Add(
                                "[%s][%s][MuunSystem][MuunExchange][ItemSerialCreateRecv][SUCCESS] Item:[%s(%s),%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]",
                                lpObj->AccountID, lpObj->Name, Item.GetName(), lpItemAttribute->Name, Item.m_Level,
                                Item.m_Option1, Item.m_Option2, Item.m_Option3, Item.m_Number, Item.m_Durability,
                                NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5],
                                NewOption[6]);
                    } else {
                    ...
                    g_Log.Add(
                            "[%s][%s][MuunSystem][MuunExchange][ItemSerialCreateRecv][SUCCESS] Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]",
                            lpObj->AccountID, lpObj->Name, Item.GetName(), Item.m_Level, Item.m_Option1, Item.m_Option2,
                            Item.m_Option3, Item.m_Number, Item.m_Durability,
                            NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5],
                            NewOption[6]);
        ...
        if (g_SocketOptionSystem.IsEnableSocketItem(lpMsg->Type) == 1) {
            ...
            g_Log.Add(
                    "[SocketItem] Drop Socket Item ( Socket Slot : %d ) - %s, [%d,%d,%d,%d], Serial : %I64d, ItemCode : %d, Level : %d, MapNumber : %d(%d/%d)",
                    SocketSlotCount, ItemAttribute[lpMsg->Type].Name, lpMsg->Level, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,
            ...
            g_Log.Add(
                    "[PentagramItem] Drop Pentagram Item ( Socket Slot : %d ) - %s, [%d,%d,%d,%d], Serial : %I64d, ItemCode : %d, Level : %d, MapNumber : %d(%d/%d)",
                    SocketSlotCount, ItemAttribute[lpMsg->Type].Name, lpMsg->Level, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,
                    lpMsg->m_Number, lpMsg->Type, lpMsg->Level, lpMsg->MapNumber, lpMsg->x, lpMsg->y);
            ...
            g_Log.Add(
                    "[MuunSystem] Drop Stone of Evolution Item - %s, [%d,%d,%d,%d], Serial : %I64d, ItemCode : %d, Level : %d, MapNumber : %d(%d/%d)",
        ...
        int iItemCount = MapC[mapnumber].MonsterItemDrop(lpMsg->Type, lpMsg->Level, lpMsg->Dur, lpMsg->x,
                                                         lpMsg->y, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption,
                                                         lpMsg->SetOption,
                                                         lootindex, lpMsg->m_Number, 0, SocketOption, SocketOptionIndex,
                                                         lpMsg->lDuration);
        ...
        if (iItemCount != -1) {
            ...
            if (((mapnumber) < MAP_INDEX_ILLUSIONTEMPLE1) ? FALSE : ((mapnumber) > MAP_INDEX_ILLUSIONTEMPLE6) ? FALSE
                                                                                                              : TRUE) //Season 2.5 add-on
            {
...
void
ItemMovePathSave(char *ActID, char *Name, BYTE level, BYTE mapnumber, BYTE x, BYTE y, char *Item, BYTE op1, BYTE op2,
                 BYTE op3, DWORD serial) {
...
void
DGOptionDataSend(int aIndex, char *szName, BYTE *KeyBuffer, BYTE GO, BYTE Qk, BYTE Wk, BYTE Ek, BYTE ChatWnd, BYTE Rk,
                 int QWER, BYTE Transform) {
...
void DGOptionDataRecv(SDHP_SKILLKEYDATA_SEND *lpMsg) {
    ...
    ::GSProtocol.GCSkillKeySend(aIndex, lpMsg->SkillKeyBuffer, lpMsg->GameOption, lpMsg->QkeyDefine, lpMsg->WkeyDefine,
                                lpMsg->EkeyDefine, lpMsg->ChatWindow, lpMsg->RkeyDefine, lpMsg->QWERLevelDefine);
...
void DGMoveOtherServer(SDHP_CHARACTER_TRANSFER_RESULT *lpMsg) {
    ...
    if (lpMsg->Result != 0) {
        ...
        GSProtocol.GCServerMsgStringSend("문제 발생시 change@webzen.co.kr로 문의해 주시기바랍니다", lpObj->m_Index, 1);
    ...
    GSProtocol.GCServerMsgStringSend("접속이 종료됩니다.", lpObj->m_Index, 1);// Deathway translation here
    GSProtocol.GCServerMsgStringSend("분할 서버로 접속해주시기 바랍니다.", lpObj->m_Index,
                                     1);// Deathway translation here
        ...
        for (int n = 0; n < lpMsg->nCount; n++) {
            ...
            if (gObj[aIndex].pInventory[pPetItemInfo[n].nPos].IsItem()) {
                if (gObj[aIndex].pInventory[pPetItemInfo[n].nPos].m_Number == pPetItemInfo[n].nSerial) {
                    gObj[aIndex].pInventory[pPetItemInfo[n].nPos].SetPetItemInfo(pPetItemInfo[n].Level,
                                                                                 pPetItemInfo[n].Exp);
                }
            }
        }
        ...
        for (int n = 0; n < lpMsg->nCount; n++) {
            ...
            if (gObj[aIndex].pWarehouse[pPetItemInfo[n].nPos].IsItem()) {
                if (gObj[aIndex].pWarehouse[pPetItemInfo[n].nPos].m_Number == pPetItemInfo[n].nSerial) {
                    gObj[aIndex].pWarehouse[pPetItemInfo[n].nPos].SetPetItemInfo(pPetItemInfo[n].Level,
                                                                                 pPetItemInfo[n].Exp);
                }
            }
        }
...
//00434220 - identical
void GS_GDReqCastleNpcBuy(int iMapSvrGroup, int aIndex, int iNpcNumber, int iNpcIndex, int iNpcDfLevel, int iNpcRgLevel,
                          int iNpcMaxHP, int iNpcHP, BYTE btNpcX, BYTE btNpcY, BYTE btNpcDIR, int iBuyCost) {
    if (iMapSvrGroup < 0) {
        return;
    }
...
void
GS_GDReqCastleNpcUpgrade(int iMapSvrGroup, int aIndex, int iNpcNumber, int iNpcIndex, int iNpcUpType, int iNpcUpValue,
                         int iNpcUpIndex) {
    if (iMapSvrGroup < 0) {
        return;
    }
...
void GS_GDReqSiegeDateChange(int iMapSvrGroup, int aIndex, WORD wStartYear, BYTE btStartMonth, BYTE btStartDay,
                             WORD wEndYear, BYTE btEndMonth, BYTE btEndDay) {
    if (iMapSvrGroup < 0) {
        return;
    }
...
void GS_GDReqCastleStateSync(int iMapSvrGroup, int iCastleState, int iTaxRateChaos, int iTaxRateStore, int iTaxHuntZone,
                             char *lpszOwnerGuild) {
    if (iMapSvrGroup < 0) {
        return;
    }
    ...
    if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
        ...
        if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup())
            g_Log.AddC(TColor::Red,
                       "[CastleSiege] PACKET-ERROR [0x80][0x00] GS_DGAnsCastleTotalInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
    } else {
        return;
    }
}
...
void GS_DGAnsOwnerGuildMaster(LPBYTE lpRecv) {
    ...
    GSProtocol.GCAnsCastleSiegeState(lpMsg->iIndex, lpMsg->iResult, lpMsg->szCastleOwnGuild,
                                     lpMsg->szCastleOwnGuildMaster);
}
...
void GS_DGAnsCastleNpcBuy(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x03] GS_DGAnsCastleNpcBuy() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
    ...
    if (lpMsg->iResult == 1) {
        ...
        if (bAddResult == TRUE) {
            ...
            g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
                      lpMsg->iNpcNumber, lpMsg->iNpcIndex);
        } else {
            g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
                      lpMsg->iNpcNumber, lpMsg->iNpcIndex);
        }
    } else {
        g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcBuy() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)", lpMsg->iResult,
                  lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    }
...
void GS_DGAnsCastleNpcRepair(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x04] GS_DGAnsCastleNpcRepair() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }

    if (lpMsg->iResult == 1) {
        BOOL bRepairResult = g_CastleSiege.RepairDbNPC(lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcHp,
                                                       lpMsg->iNpcMaxHp);
        if (bRepairResult == TRUE) {
            ...
            g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
                      lpMsg->iNpcNumber, lpMsg->iNpcIndex);
        } else {
            g_Log.Add(
                    "[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
                    lpMsg->iNpcNumber, lpMsg->iNpcIndex);
        }
    } else {
        g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcRepair() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)", lpMsg->iResult,
                  lpMsg->iNpcNumber, lpMsg->iNpcIndex);
    }

    GSProtocol.GCAnsNpcRepair(lpMsg->iIndex, lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcHp,
                              lpMsg->iNpcMaxHp);
}
...
void GS_DGAnsCastleNpcUpgrade(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x05] GS_DGAnsCastleNpcUpgrade() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }

    if (lpMsg->iResult == 0) {
        g_Log.AddC(TColor::Red, "[CastleSiege] ERROR - Castle NPC Upgrade Fail() (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
                   lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
    } else {
        g_CastleSiege.UpgradeDbNPC(lpMsg->iIndex, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType,
                                   lpMsg->iNpcUpValue, lpMsg->iNpcUpIndex);
        g_Log.Add("[CastleSiege] [0x80][0x06] GS_DGAnsTaxInfo() - Npc Upgrade OK (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
                  lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
    }

    GSProtocol.GCAnsNpcUpgrade(lpMsg->iIndex, lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType,
                               lpMsg->iNpcUpValue);
}
...
void GS_DGAnsTaxInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x06] GS_DGAnsTaxInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }

    if (lpMsg->iResult == TRUE) {
        GSProtocol.GCAnsTaxMoneyInfo(lpMsg->iIndex, lpMsg->iResult, lpMsg->iTaxRateChaos, lpMsg->iTaxRateStore,
...
void GS_DGAnsTaxRateChange(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x07] GS_DGAnsTaxRateChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsCastleMoneyChange(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x08] GS_DGAnsCastleMoneyChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
    ...
    if (lpMsg->iResult == TRUE) {
        if (gObjIsConnected(lpMsg->iIndex)) {
            if (lpMsg->iMoneyChanged < 0) {
                ...
                g_Log.Add(
                        "[CastleSiege] [0x80][0x08] GS_DGAnsCastleMoneyChange() - Withdraw Request OK [%s][%s] (ReqMoney:%d, TotMoney:%I64d)",
...
void GS_DGAnsSiegeDateChange(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x09] GS_DGAnsSiegeDateChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsGuildMarkRegInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x0A] GS_DGAnsGuildMarkRegInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsSiegeEndedChange(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x0B] GS_DGAnsSiegeEndedChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsCastleOwnerChange(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x0C] GS_DGAnsCastleOwnerChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsRegAttackGuild(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x0D] GS_DGAnsRegAttackGuild() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsRestartCastleState(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x0E] GS_DGAnsRestartCastleState() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    } else {
...
void GS_DGAnsMapSvrMsgMultiCast(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x0F] GS_DGAnsMapSvrMsgMultiCast() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsGlobalPostMultiCast(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "GS_DGAnsGlobalPostMultiCast() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsRegGuildMark(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x10] GS_DGAnsRegGuildMark() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    } else {
...
void GS_DGAnsGuildMarkReset(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x11] GS_DGAnsGuildMarkReset() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsGuildSetGiveUp(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x12] GS_DGAnsGuildSetGiveUp() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }

    GSProtocol.GCAnsGiveUpCastleSiege(lpMsg->iIndex, lpMsg->iResult, lpMsg->bIsGiveUp, lpMsg->iRegMarkCount,
                                      lpMsg->szGuildName);
}
...
void GS_DGAnsNpcRemove(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x16] GS_DGAnsNpcRemove() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsCastleTributeMoney(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x18] GS_DGAnsCastleTributeMoney() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }

    g_Log.Add("[CastleSiege] [0x80][0x18] GS_DGAnsCastleTributeMoney() - Money Tribute OK (%d)",
...
void GS_DGAnsResetCastleTaxInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x19] GS_DGAnsResetCastleTaxInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    } else {
        if (lpMsg->iResult == 1) {
...
void GS_DGAnsResetSiegeGuildInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x1A] GS_DGAnsResetSiegeGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsResetRegSiegeInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x80][0x1B] GS_DGAnsResetRegSiegeInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsCastleInitData(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x81] GS_DGAnsCastleInitData() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }

    if (g_CastleSiege.GetDataLoadState() != 2) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",
    ...
    if (bRET_VAL == FALSE) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - g_CastleSiege.SetCastleInitData() == FALSE");
        return;
    }
    ...
    if (bRET_VAL == FALSE) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - g_CastleSiege.SetCastleNpcData() == FALSE");
        return;
    }
...
void GS_DGAnsCastleNpcInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x82] GS_DGAnsCastleNpcInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
}
...
void GS_DGAnsAllGuildMarkRegInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x83] GS_DGAnsAllGuildMarkRegInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
    ...
    lpMsgSend->h.set((LPBYTE) & cBUFFER, 0xB4,
                     (lpMsgSend->iCount * sizeof(PMSG_CSREGGUILDLIST) + sizeof(PMSG_ANS_CSREGGUILDLIST)));

    IOCP.DataSend(lpMsg->iIndex, (LPBYTE) & cBUFFER,
                  (lpMsgSend->iCount * sizeof(PMSG_CSREGGUILDLIST) + sizeof(PMSG_ANS_CSREGGUILDLIST)));
}
...
void GS_DGAnsFirstCreateNPC(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x84] GS_DGAnsFirstCreateNPC() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsCalcREgGuildList(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x85] GS_DGAnsCalcREgGuildList() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
    ...
    g_Log.AddC(TColor::Red, "[CastleSiege] PACKET-ERROR [0x85] GS_DGAnsCalcREgGuildList() - lpMsg->iResult != 1 (%d)",
               lpMsg->iResult);
}
...
void GS_DGAnsCsGulidUnionInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x86] GS_DGAnsCsGulidUnionInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsCsSaveTotalGuildInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x87] GS_DGAnsCsSaveTotalGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsCsLoadTotalGuildInfo(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x88] GS_DGAnsCsLoadTotalGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void GS_DGAnsCastleNpcUpdate(LPBYTE lpRecv) {
    ...
    if (lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()) {
        g_Log.AddC(TColor::Red,
                   "[CastleSiege] PACKET-ERROR [0x89] GS_DGAnsCastleNpcUpdate() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
        return;
    }
...
void DGAnsCrywolfSync(LPBYTE lpRecv) {
    ...
    if (GetTickCount() > (dwCrywolfSyncLogTickCount + 60000)) {
        g_Log.AddC(TColor::Blue,
                   "[Crywolf Sync] Occupation: %d, State: %d, Benefit: %d, Penalty: %d, PlusChaosRate: %d, GemDropPenaltyRate: %d, ExpPenaltyRate: %d, MinusMonHPBenefitRate: %d, KundunHPRefillState: %d",
                   lpMsg->iOccupationState, lpMsg->iCrywolfState, lpMsg->iApplyBenefit, lpMsg->iApplyPenalty,
                   lpMsg->iPlusChaosRate, lpMsg->iGemDropPenaltyRate, lpMsg->iGettingExpPenaltyRate,
                   lpMsg->iMinusMonHPBenefitRate, lpMsg->iKundunHPRefillState);
...
void DGAnsInGameShopPoint(ISHOP_ANS_POINT *lpMsg) {
    if (lpMsg->aIndex < g_ConfigRead.server.GetObjectStartUserIndex() ||
        lpMsg->aIndex > g_ConfigRead.server.GetObjectMax())
        return;
...
void DGAnsInGameShopItemUse(ISHOP_ITEM_USEANS *lpMsg) {
    ...
    g_CashShop.GCCashInventoryItemUse(&gObj[lpMsg->aIndex], lpMsg->Result, lpMsg->UniqueCode, lpMsg->AuthCode,
                                      lpMsg->ID1, lpMsg->ID2, lpMsg->ID3);
}

void GDReqInGameShopItemGift(int aIndex, int ID1, int ID2, int ID3, int Price, BYTE CoinType, char Target[11],
                             char Message[200]) {
...
void DGSetWarehouseList(PMSG_ANS_WARESAVE *lpMsg) {
    ...
    g_Log.Add("[%s][%s] Warehouse save OK (WindowClose:%d)", gObj[lpMsg->iIndex].AccountID, gObj[lpMsg->iIndex].Name,
              lpMsg->CloseWindow);
}
...
void DGAnsArcaBattleGuildJoinSelect(_tagPMSG_ANS_ARCA_BATTLE_GUILD_JOIN_DS *lpMsg) {
    ...
    if (gObjIsConnected(aIndex)) {
        switch (lpMsg->btResult) {
            case 0:
                g_Log.Add("[ArcaBattle] [%s][%s][%s] ArcaBattle Guild Join", gObj[aIndex].AccountID, gObj[aIndex].Name,
                          gObj[aIndex].m_PlayerData->GuildName);
                GDReqArcaBattleGuildJoin(lpObj);
                return;

            case 1:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to Not Guild Master", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            case 2:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to Below Guild Member", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            case 3:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to Guild Over", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            case 4:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to be Guild Name Same", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            case 5:
                g_Log.Add("[ArcaBattle] [%s][%s] (Guild Master) ArcaBattle Join Fail to Time Over",
                          gObj[aIndex].AccountID, gObj[aIndex].Name);
                break;

            case 6:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to not enough Guild Rank", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            default:
                g_Log.Add("[ArcaBattle] [%s][%s] GuildJoin Error Result : %d", gObj[aIndex].AccountID,
                          gObj[aIndex].Name, lpMsg->btResult);
                break;
        }
...
void DGAnsArcaBattleGuildMemberJoin(_tagPMSG_ANS_ARCA_BATTLE_GUILD_MEMBER_JOIN_DS *lpMsg) {
    ...
    if (gObjIsConnected(aIndex)) {
        switch (lpMsg->btResult) {
            case 0:
                g_Log.Add("[ArcaBattle] [%s][%s][%s] ArcaBattle Guild Member Join", gObj[aIndex].AccountID,
                          gObj[aIndex].Name, gObj[aIndex].m_PlayerData->GuildName);
                return;

            case 7:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to Not Guild Member", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            case 8:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to be Name Same", gObj[aIndex].AccountID,
                          gObj[aIndex].Name);
                break;

            case 9:
                g_Log.Add("[ArcaBattle] [%s][%s] (Guild Member) ArcaBattle Join Fail to Guild Member Over",
                          gObj[aIndex].AccountID, gObj[aIndex].Name);
                break;

            case 10:
                g_Log.Add("[ArcaBattle] [%s][%s] (Guild Member) ArcaBattle Join Fail to Time Over",
                          gObj[aIndex].AccountID, gObj[aIndex].Name);
                break;

            case 13:
                g_Log.Add(
                        "[ArcaBattle] [%s][%s] (Guild Master) ArcaBattle Join Fail GuildMaster Will be Registered Automatically",
                        gObj[aIndex].AccountID, gObj[aIndex].Name);
                break;

            case 14:
                g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Join Fail to Guild Master Join Time ",
                          gObj[aIndex].AccountID, gObj[aIndex].Name);
                break;

            default:
                g_Log.Add("[ArcaBattle] [%s][%s] GuildMemberJoin Error Result : %d", gObj[aIndex].AccountID,
                          gObj[aIndex].Name, lpMsg->btResult);
                break;
        }
...
void DGAnsArcaBattleEnter(_tagPMSG_ANS_ARCA_BATTLE_ENTER_DS *lpMsg) {
    ...
    if (lpMsg->btResult == 11) {
        g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Enter Fail to Not Join Guild Master", gObj[aIndex].AccountID,
                  gObj[aIndex].Name);
    } else if (lpMsg->btResult == 12) {
        g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle Enter Fail to Not Play Time", gObj[aIndex].AccountID,
                  gObj[aIndex].Name);
    } else if (lpMsg->btResult == 0) {
        if (lpMsg->btEnterSeq == TRUE) {
            gObjMoveGate(aIndex, 426);
            g_Log.Add("[ArcaBattle] [%s][%s][%s] ArcaBattle Enter", gObj[aIndex].AccountID, gObj[aIndex].Name,
                      gObj[aIndex].m_PlayerData->GuildName);
            return;
        }
    } else {
        g_Log.Add("[ArcaBattle] [%s][%s] Enter Error Result : %d", gObj[aIndex].AccountID, gObj[aIndex].Name,
                  lpMsg->btResult);
        return;
    }
...
void DGAns_CCF_Info_Notify(SDHP_ANS_CCF_INFO_NOTIFY *lpMsg) {
    ...
    for (int nUser = g_ConfigRead.server.GetObjectStartUserIndex();
         nUser < g_ConfigRead.server.GetObjectMax(); nUser++) {
...
void DGAns_GetCCFPermission(SDHP_ANS_CCF_GETPERMISSION *lpMsg) {
    ...
    if (byCCF_Enter_Result) {
        ...
        g_Log.Add("[Chaos Castle Survival][DGAns_GetCCFPermission] ACC:%s, NAME:%s byCCF_Enter_Result:%d ",
                  lpObj->AccountID, lpObj->Name, byCCF_Enter_Result);
        ...
        if (nResult) {
            g_Log.Add("[CGReqCCF_EnterCheck][CGReqEnterChaosCastleFinal] ACC :%s , NAME :%s return :%d ",
                      lpObj->AccountID, lpObj->Name, nResult);
        }
    }
}
...
void GDReqSaveEventInvenItem(OBJECTSTRUCT *lpObj) {
    ...
    for (int n = 0; n < EVENT_INVENTORY_SIZE; n++) {
        if (lpObj->pEventInventory[n].IsItem() == TRUE) {
            ...
            g_Log.Add(
                    "[%s][%s]EI[%d,%s,%d,%d,%d,%d]serial:[%I64d] dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set[%d] 380:[%d] HO:[%d,%d] SC[%d,%d,%d,%d,%d] BonusOption[%d]",
                    ...
                    NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6],
                    lpObj->pEventInventory[n].m_SetOption, lpObj->pEventInventory[n].m_ItemOptionEx >> 7,
                    g_kJewelOfHarmonySystem.GetItemStrengthenOption(&lpObj->pEventInventory[n]),
                    g_kJewelOfHarmonySystem.GetItemOptionLevel(&lpObj->pEventInventory[n]),
                    lpObj->pEventInventory[n].m_SocketOption[0], lpObj->pEventInventory[n].m_SocketOption[1],
                    lpObj->pEventInventory[n].m_SocketOption[2],
                    lpObj->pEventInventory[n].m_SocketOption[3], lpObj->pEventInventory[n].m_SocketOption[4],
                    lpObj->pEventInventory[n].m_BonusSocketOption);
        }
    }
...
void DGAnsUBFAccountUserInfo(PMSG_ANS_UBF_ACCOUNT_USERINFO *lpMsg) {
    ...
    if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE && lpMsg->btObserverMode == TRUE &&
        (lpMsg->btRegisterState == 0 || lpMsg->btResult == 0)) {
        g_UnityBattleField.DGAnsCheckJoinedUnityBattleField(lpObj->m_Index, lpMsg->btRegisterState);

        g_Log.Add("[DGAnsUBFAccountUserInfo][%s][%s] LogOut, Because Not Resister State or Request To Cancel",
...
void DGUBFRegisterAccountUserResult(PMSG_UBF_REGISTER_ACCOUNT_USER_RESULT *lpMsg) {
    ...
    if (lpMsg->btResult != TRUE) {
        g_Log.Add(
                "[UBFRegisterAccountUserResult][%s][%s] UBFRegisterAccountUserResult Error :%d (0:DB fail/3: Unexpired Day)",
...
void DGAns_ChaosCastle_KillPoint_Result(SDHP_ANS_KILLPOINT_RESULT_CC_UBF *lpMsg) {
    ...
    g_Log.Add(
            "[CChaosCastle][DGAns_ChaosCastle_KillPoint_Result][%s][%s][%s] Ans Result-Return:%d / KillPoint:%d/ TotalPoint:%d",
            lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->m_RealNameOfUBF, lpMsg->nResult, lpMsg->nCurrentPoint,
            lpMsg->nTotalPoint);
}
        ...
        case 2:
            ...
            for (int j = 0; j < nPartyCnt; j++) {
                ...
                if (lpData->btRank <= 0) {
                    ...
                    memcpy(g_DevilSquareFinal.m_GoSemiFinalParty[j].szUserName1, lpData->szUserName1,
                           MAX_ACCOUNT_LEN + 1);
                    memcpy(g_DevilSquareFinal.m_GoSemiFinalParty[j].szUserName2, lpData->szUserName2,
                           MAX_ACCOUNT_LEN + 1);
...
void DGWhisperResponseRecv(DSMSG_GS_WHISPER_RESULT *aRecv) {
    ...
    if (lpObj->m_Whispering.iWhisperSent == true) {
        ...
        if (lpObj->m_Whispering.wResponseCount >= gMapSrvGroupOnlineServer &&
            lpObj->m_Whispering.iWhisperSent == true) // if all gs return false , player is not found
        {

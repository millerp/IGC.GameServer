...
void CMagicDamage::LogSkillList(char *filename) {
    ...
    for (pugi::xml_node skill = main.child("Skill"); skill; skill = skill.next_sibling()) {
        ...
        this->Set(name, number, damage, requireLevel, mana, bp, dis, attr, iSkillType, RequireClass, rEnergy,
                  rLeadership, rDelay, btItemSkill, btIsDamage, btBufIndex);
        this->SetEx(number, iSkillUseType, iSkillBrand, iKillCount, RequireStatus);
        this->SetMasterLevelSkillInfo(number, nSkillRank, nSkillGroup, nRequireMLPoint, nHP, nSD, wKeepTime,
                                      nRequireStr, nRequireDex);
...
void CMagicDamage::SetMasterLevelSkillInfo(int iSkill, int iSkillRank, int iSkillGroup, int iRequireMLPoint, int iHP,
                                           int iSD, WORD wKeepTime, int iRequireStr, int iRequireDex) {
...
void CMagicDamage::SetEx(int iSkill, int iSkillUseType, int iSkillBrand, int iKillCount, int *pReqStatus) {
    ...
    memcpy(this->m_iRequireStatus[iSkill], pReqStatus,
           sizeof(this->m_iRequireStatus[iSkill]));    // #error Deathway Fix
}

void
CMagicDamage::Set(char *name, int skill, int damage, int rlevel, int mana, int bp, int dis, int Attr, int iSkillType,
                  unsigned char *ReqClass, int rEnergy, int rLeadership, int rdelay, BYTE byItemSkill, BYTE byIsDamage,
                  BYTE byBufIndex) {

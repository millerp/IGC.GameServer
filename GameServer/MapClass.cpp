...
int MapClass::MonsterItemDrop(int type, int level, float dur, int x, int y, BYTE Option1, BYTE Option2, BYTE Option3,
                              BYTE NOption, BYTE SOption, int aIndex, UINT64 number, BYTE ItemEffectEx,
                              BYTE *SocketOption, BYTE SocketBonusOption, DWORD PeriodDuration) {
    int count;
    int counttot = 0;
    ...
    while (true) {
        if (this->m_cItem[count].IsItem() == FALSE) {
            this->m_cItem[count].CreateItem(type, level, x, y, dur, Option1, Option2, Option3, NOption, SOption, number,
                                            ItemEffectEx, SocketOption, SocketBonusOption);
    ...
    g_Log.Add("Error: Items limit on Map reached, check MapItemCount and disappear time settings, %s %d", __FILE__,
              __LINE__);
...
BOOL
MapClass::ItemDrop(int type, int level, float dur, int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption,
                   BYTE SOption, UINT64 number, int aIndex, int PetLevel, UINT64 PetExp, BYTE ItemEffectEx,
                   BYTE *SocketOption, BYTE SocketBonusOption, DWORD PeriodDuration) {
    int count;
    int counttot = 0;
    BYTE attr = this->GetAttr(x, y);
    ...
    while (true) {
        if (this->m_cItem[count].IsItem() == FALSE) {
            this->m_cItem[count].DropCreateItem(type, level, x, y, dur, Option1, Option2, Option3, NOption, SOption,
                                                number, PetLevel, PetExp, ItemEffectEx, SocketOption,
                                                SocketBonusOption);
            this->m_cItem[count].m_UserIndex = aIndex;
            this->m_cItem[count].m_PeriodItemDuration = PeriodDuration;
            this->m_ItemCount++;
...
BOOL MapClass::ItemGive(int aIndex, int item_num, bool bFailNotSend) {
    ...
    if (this->m_cItem[item_num].IsItem() == FALSE) {
        g_Log.Add("Error: Item does not exist %s %d %s", __FILE__, __LINE__,
                  gObj[aIndex].Name);    // Apply Deathway Fix herw
        return FALSE;
    }
                            ...
                            if (gObj[aIndex].PartyNumber == gObj[this->m_cItem[item_num].m_UserIndex].PartyNumber) {
                                if (BC_MAP_RANGE(gObj[aIndex].MapNumber) != FALSE) {
                                    if (this->m_cItem[item_num].m_Type == ITEMGET(12, 15) ||
                                        (this->m_cItem[item_num].m_Type == ITEMGET(13, 19) &&
                                         ((this->m_cItem[item_num].m_Level < 0) ? FALSE
                                                                                : (this->m_cItem[item_num].m_Level > 2)
                                                                                  ? FALSE : TRUE) != FALSE)) {
                                        lootresult = 0;
                                    } else {
                                        lootresult = 1;
        ...
        if (this->m_cItem[n].IsItem() != FALSE) {
            if (this->m_cItem[n].m_State != 8) {
                if (CurTime > this->m_cItem[n].m_Time) {
                    g_Log.Add("Item has disappeared (%s:%d / Level: %d / Skill: %d / Serial: %I64d)",
                              this->m_cItem[n].GetName(), this->m_cItem[n].m_Type, this->m_cItem[n].m_Level,
                              this->m_cItem[n].m_Special[0], this->m_cItem[n].m_Number);
...
BOOL MapClass::AttrLoad(char *filename) {
    ...
    fclose(fp);
    this->path->SetMapDimensions(256, 256, (UCHAR * )
    this->m_attrbuf, this->thisMapNumber);
    return true;
}
...
void MapClass::SearchStandAttr(short &x, short &y) {
    ...
    while (scount-- != 0) {
        if (((this->m_attrbuf[tindex + tx] & 4) != 4) && ((this->m_attrbuf[tindex + tx] & 2) != 2) &&
            ((this->m_attrbuf[tindex + tx] & 8) != 8)) {
            x = tx;
            y = ty;
            break;
        }

...
int CDoppelGangerMonsterHerd::AddMonsterEX(int iMonsterType, int bAttackFirst, int nMonsterLevel, int nMonsterHp,
                                           int nMonsterAttMin, int nMonsterAttMax, int nMonsterDef) {
    if (!this->m_bHasInfo) {
        return -1;
    }
    ...
    if (iIndex < 0) {
        g_Log.Add("[Doppelganger] AddMonsterEx Fail(%d) [Class: %d][%d][%d, %d]", iIndex, iMonsterType,
                  this->m_iMapNumber, cX, cY);
        return -1;
    }
                ...
                if (dis > attackRange) {
                    if (gObjMonsterGetTargetPos(lpObj) == TRUE) {
                        if (MAX_MAP_RANGE(lpObj->MapNumber) == FALSE) {
                            g_Log.AddC(TColor::Red, "[ERROR] MAX_MAP_RANGE (MonsterHerd DG) == FALSE (%d)",
                                       lpObj->MapNumber);
                            break;
                        }
...
void CDoppelGanger::DoppelgangerProcessInit() {
    ...
    this->m_PosInfo.LoadDoppelgangerPosInfo(
            g_ConfigRead.GetPath("\\Events\\DoppelGanger\\IGC_DoppelGanger_PositionInfo.xml"));
    this->m_DPEventInfo.LoadDoppelgangerEventInfo(
            g_ConfigRead.GetPath("\\Events\\DoppelGanger\\IGC_DoppelGanger_EventInfo.xml"));
    this->m_DPDropItemInfo.LoadDoppelgangerItemBag(
            g_ConfigRead.GetPath("\\Events\\DoppelGanger\\IGC_DoppelGanger_DropInfo.xml"));
    ...
    for (int i = 0; i < MAX_DOPPELGANGER_USER_INFO; i++) {
        if (this->m_UserData[i].IsUser() == TRUE) {
            if (gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED) {
                ...
                wsprintf(PlayerLog + len, "(%s)(%s) ", gObj[this->m_UserData[i].m_nIndex].AccountID,
    ...
    if (this->m_nCurUserCount > 0 && this->m_nGoalMonsterCnt < MAX_DOPPELGANGER_MONSTER_IN_PORTAL) {
        ...
        for (int Cnt = 0; Cnt < MAX_DOPPELGANGER_USER_INFO; Cnt++) {
            if (this->m_UserData[Cnt].IsUser() == TRUE &&
                ...
                this->AddLastTreasureBox(gObj[this->m_UserData[Cnt].m_nIndex].X,
                                         gObj[this->m_UserData[Cnt].m_nIndex].Y);
                break;
            }
        }
    }
...
void CDoppelGanger::ProcDoppelgangerState_Ready(ULONGLONG i64CurTime) {
    if ((60000 * this->m_i64ReadyTime - (GetTickCount64() - this->m_i64StateTime)) <= 30000 &&
        this->m_bSendMsgBeforePlay == false) {
...
void CDoppelGanger::ProcDoppelgangerState_Playing(ULONGLONG i64CurTime) {
    ...
    if (this->m_bSendIceWorkerDel == false && this->m_btIceWorkState == 0 &&
        GetTickCount64() - this->m_i64IceWorkerRegenTime >= 30000) {
...
void CDoppelGanger::ProcDoppelgangerState_End(ULONGLONG i64CurTime) {
    if (i64CurTime - this->m_i64StateTime > 10000 * this->m_i64EndTime && this->GetUserCount() > 0 &&
        this->m_bSuccessResult == false) {
        for (int i = 0; i < MAX_DOPPELGANGER_USER_INFO; i++) {
            if (this->m_UserData[i].IsUser() == TRUE) {
                if (gObj[this->m_UserData[i].m_nIndex].Connected <= PLAYER_LOGGED) {
...
void CDoppelGanger::SetUserAverageLevel() {
    ...
    if (this->GetUserCount()) {
        for (int i = 0; i < MAX_DOPPELGANGER_USER_INFO; i++) {
            if (this->m_UserData[i].IsUser() == TRUE) {
                nLevelSum += gObj[this->m_UserData[i].m_nIndex].Level +
...
void CDoppelGanger::PlatformLugardAct(LPOBJ lpNpc, LPOBJ lpObj) {
    ...
    if (this->GetDoppelgangerState() == DOPPELGANGER_STATE_READY) {
        i64EntryTime =
                this->m_i64EndTime + this->m_i64ReadyTime - (GetTickCount64() - this->m_i64StateTime) / 1000 / 60 +
                this->m_i64PlayTime + 1;
    } else if (this->GetDoppelgangerState() == DOPPELGANGER_STATE_PLAY) {
        i64EntryTime =
                this->m_i64PlayTime - (GetTickCount64() - this->m_i64StateTime) / 1000 / 60 + this->m_i64EndTime + 1;
...
void CDoppelGanger::MiddleTreasureAct(LPOBJ lpNpc, LPOBJ lpObj) {
    ...
    if (nItemDrop <= 100 || nItemDrop >= 500) {
        ...
        for (int i = 0; i < nUserCnt; i++) {
            ...
            int nRetIndex = this->AddMonsterLarva(this->m_nMapNumber, btX, btY, nMonsterLevel, nMonsterHp,
                                                  nMonsterAttMin, nMonsterAttMax, nMonsterDef);
...
void CDoppelGanger::AddMonsterHerd(int nHerdIndex, int nMonsterClass, BOOL bAttackFirst) {
    if (nHerdIndex <= MAX_DOPPELGANGER_MONSTER_INFO - 1 && (nMonsterClass >= 529 && nMonsterClass <= 539)) {
        ...
        int nIndex = this->m_MonsterHerd[nHerdIndex].AddMonsterEX(nMonsterClass, bAttackFirst, nMonsterLevel,
                                                                  nMonsterHp, nMonsterAttMin, nMonsterAttMax,
                                                                  nMonsterDef);
        ...
        if (nIndex != -1 && nMonsterClass == 530) {
            ...
            g_Log.Add("[Doppelganger] Killer monster Regen [%d][%d, %d] (%d)", gObj[nIndex].MapNumber, gObj[nIndex].X,
                      gObj[nIndex].Y, nIndex);
        } else if (nIndex != -1 && nMonsterClass == 529) {
            this->SetAngerKillerHp(nMonsterHp);
            this->SetAngerKillerState(2);
            g_Log.Add("[Doppelganger] AngerKiller monster Regen [%d][%d, %d] (%d)", gObj[nIndex].MapNumber,
                      gObj[nIndex].X, gObj[nIndex].Y, nIndex);
        } else if (nIndex != -1 && nMonsterClass == 531) {
            this->AddIceWorkerIndex(nIndex);
            g_Log.Add("[Doppelganger] Iceworker monster Regen [%d][%d, %d] (%d)", gObj[nIndex].MapNumber,
                      gObj[nIndex].X, gObj[nIndex].Y, nIndex);
        }
    }
}
...
void CDoppelGanger::SendDoppelgangerUserPos() {
    ...
    for (int i = 0; i < MAX_DOPPELGANGER_USER_INFO; i++) {
        if (this->m_UserData[i].IsUser() == TRUE && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED) {
            DOPPELGANGER_USER_POS *lpMsgBody = (DOPPELGANGER_USER_POS *) (SendByte +
                                                                          sizeof(PMSG_DOPPELGANGER_USER_POS) +
                                                                          (sizeof(DOPPELGANGER_USER_POS) *
                                                                           lpMsg->wCount));

            lpMsgBody->wIndex = this->m_UserData[i].m_nIndex;
            lpMsgBody->btMapNumber = this->m_nMapNumber;
            lpMsgBody->btPosIndex = this->GetDoppelgangerPosIndex(gObj[this->m_UserData[i].m_nIndex].X,
...
void CDoppelGanger::SendDoppelgangerResultAll() {
    ...
    for (int Cnt = 0; Cnt < MAX_DOPPELGANGER_USER_INFO; Cnt++) {
        if (this->m_UserData[Cnt].IsUser() == TRUE &&
            gObj[this->m_UserData[Cnt].m_nIndex].Connected == PLAYER_PLAYING) {
            IOCP.DataSend(this->m_UserData[Cnt].m_nIndex, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.Add("[Doppelganger] Event Result (%d) [%s][%s]", pMsg.btMissionResult,
                      gObj[this->m_UserData[Cnt].m_nIndex].AccountID, gObj[this->m_UserData[Cnt].m_nIndex].Name);
        }
    }
        ...
        if (lpObj->Connected == PLAYER_PLAYING && lpObj->m_State == 2 && lpObj->Type == OBJ_MONSTER) {
            if (DG_MAP_RANGE(lpObj->MapNumber) == TRUE) {
                if (lpObj->PathCount != 0) {
                    if (!gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_STONE) &&
                        ...
                        if ((GetTickCount() - lpObj->PathTime) > MoveTime) {
                            ...
                            if (lpObj->PathCur < 14) {
                                ...
                                if (lpObj->Type == OBJ_USER && ((attr & 4) == 4 || (attr & 8) == 8)) {
                                    g_Log.Add(
                                            "[ CHECK POSITION ] MoveMosterProc [%s][%s] Map[%d]-(%d,%d) User(%d,%d) Can not Move Position Attr[%d]",
                                            lpObj->AccountID, lpObj->Name, lpObj->MapNumber, nextX, nextY, lpObj->X,
                                            lpObj->Y, attr);
...
int CDoppelGanger::AddMonsterLarva(int nMapNumber, BYTE btX, BYTE btY, int nMonsterLevel, int nMonsterHp,
                                   int nMonsterAttMin, int nMonsterAttMax, int nMonsterDef) {
    int iIndex = gObjAddMonster(nMapNumber);
...
void CDoppelGanger::SendMapTileInfo(LPOBJ lpObj, BYTE btMapSetType) {
    ...
    this->m_PosInfo.GetStartMapAttr(this->m_nMapNumber, &lpMsgBody[0].btX, &lpMsgBody[0].btY, &lpMsgBody[1].btX,
                                    &lpMsgBody[1].btY);

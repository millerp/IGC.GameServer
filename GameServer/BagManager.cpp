...
bool CBagManager::AddItemBag(int BagType, int iParam1, int iParam2, const char *lpFileName) {
    ...
    g_Log.Add("[Bag Manager] Add Bag: %s (Type: %d) Current Bag Count: %d", lpFileName, BagType,
              this->m_multimapBags.size());
    return true;
}
...
void CBagManager::DeleteItemBags() {
    ...
    for (std::multimap<int, CBag *>::iterator It = this->m_multimapBags.begin();
         It != this->m_multimapBags.end(); It++) {
        delete It->second;
...
bool CBagManager::IsBag(int aIndex, int BagType, int iParam1, int iParam2) {
    ...
    std::pair <std::multimap<int, CBag *>::iterator, std::multimap<int, CBag *>::iterator> ret = this->m_multimapBags.equal_range(
            BagType);
...
bool CBagManager::UseBag_GremoryCase(int aIndex, int BagType, int iParam1, int iParam2, BYTE btStorageType,
                                     BYTE btRewardSource, int iExpireDays) {
    EnterCriticalSection(&this->m_criti);

...
void CExpManager::LoadScript(LPSTR szFileName) {
    ...
    g_Log.Add(
            "[Exp Manager] Loaded Data (Static Exp:%f Static MLExp:%f EventExp:%f QuestExp:%f), Count of Dynamic Exp ranges: %d, CalcType:%d, Debug:%d",
            this->m_fStaticExp, this->m_fStaticMLExp, this->m_fEventExp, this->m_fQuestExp, this->m_vtExpRanges.size(),
            this->m_iExpCalcType, this->m_bDebugMode);
}
...
float CExpManager::GetExpMultiplier(int iLevel, int iReset, bool bIsMasterLevel) {
    ...
    if (this->m_iExpCalcType == EXP_CALC_STATIC_ONLY) {
        ...
        for (std::vector<EXP_RANGES>::iterator It = this->m_vtExpRanges.begin();
             It != this->m_vtExpRanges.end(); It++) {
            if (iReset >= It->iMinReset && iReset <= It->iMaxReset && iLevel >= It->iMinLevel &&
                iLevel <= It->iMaxLevel) {
                if (bIsMasterLevel) {
                    fExp = It->fMLExp;
                } else {
    ...
    if (this->m_bDebugMode == true) {
        g_Log.AddC(TColor::Yellow, "[EXP DEBUG] Level:%d Reset:%d ML:%d ExpResult:%f (CalcType:%d)", iLevel, iReset,
                   bIsMasterLevel, fExp, this->m_iExpCalcType);
    }

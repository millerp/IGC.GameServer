...
void CDragonEvent::ClearMonster() {
    ...
    for (std::map<int, DRAGON_MONSTER_INFO>::iterator It = this->m_mapMonsterInfo.begin();
...
void CDragonEvent::DragonActive() {
    ...
    for (int i = 0; i < It->second.m_Count; i++) {
        ...
        for (std::vector<DRAGON_MAP_INFO>::iterator ItMap = It->second.m_MapInfo.begin();
             ItMap != It->second.m_MapInfo.end(); ItMap++) {
        ...
        this->GetBoxPosition(this->m_MapNumber, SpawnInfo.m_X, SpawnInfo.m_Y, SpawnInfo.m_W, SpawnInfo.m_H,

...
CCrywolf::CCrywolf() {
    ...
    memset(this->m_iMonsterGroupNumberArray, 0,
           sizeof(this->m_iMonsterGroupNumberArray));    // #error Change 20 to sizeof(this->m_iMonsterGroupNumberArray)
}
...
BOOL CCrywolf::LoadData(LPSTR lpszFileName) {
    ...
    for (pugi::xml_node cycle = cyclesettings.child("Cycle"); cycle; cycle = cycle.next_sibling()) {
        ...
        this->m_StateTimeInfo[iStateNumber].m_iContinuanceTime =
                iHour * 60 * 60 * 1000 + iMinute * 60 * 1000 + iSecond * 1000;
...
void CCrywolf::DelAllData() {
    ...
    memset(this->m_iMVPScoreTable, 0,
           sizeof(this->m_iMVPScoreTable));    // #error change to sizeof(this->m_iMVPScoreTable)
    memset(this->m_iMVPRankScoreTable, 0,
           sizeof(this->m_iMVPRankScoreTable));    // #error change to sizeof(this->m_iMVPRankScoreTable)
    memset(this->m_iMVPRankExpTable, 0,
           sizeof(this->m_iMVPRankExpTable));    // #error change to sizeof(this->m_iMVPRankExpTable)
...
void CCrywolf::ProcState_START() {
    ...
    if (this->m_bTurnUpBoss != FALSE &&
        CHECK_LIMIT(this->m_iBossIndex, g_ConfigRead.server.GetObjectMaxMonster()) != FALSE) {
        if (gObj[this->m_iBossIndex].Live == FALSE) {
...
void CCrywolf::NotifyCrywolfHeroList() {
    ...
    PHeadSubSetB((LPBYTE) lpMsg, 0xBD, 0x08,
                 iHeroCount * sizeof(PMSG_ANS_CRYWOLF_HERO_LIST_INFO) + sizeof(PMSG_ANS_CRYWOLF_HERO_LIST_INFO_COUNT));

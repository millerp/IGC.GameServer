...
void CItemShop::Initialize() {
    ...
    g_Log.Add("[ItemShop] Initializing ItemShop with Version: %d/%d/%d (Image Version: %d/%d/%d)", this->ItemVer1,
              this->ItemVer2, this->ItemVer3,
    ...
    if (this->GoblinSystem) {
        g_Log.Add("[ItemShop] Goblin Point System Enabled (AddCoin:%d) (Delay:%d seconds) (CoinType:%d)",
                  this->GoblinAddCoin, this->GoblinTime, this->GoblinCoinType);
    }
}
...
void CItemShop::GCCashInventoryItemCount(LPOBJ lpObj, LPBYTE lpRecv) {
    ...
    if (lpMsg->Count > 0) {
        if (lpMsg->InvType == 1) {
            ...
            while (count != 8) {
                lpItem = (ISHOP_ITEMLIST *) (lpRecv + sizeof(ISHOP_ANS_ITEMLIST) +
                                             ((StartItem + count) * sizeof(ISHOP_ITEMLIST)));
...
void CItemShop::CGCashInventoryItemUse(LPOBJ lpObj, PMSG_REQ_INGAMESHOP_ITEMUSE *lpMsg) {
    ...
    if (lpItem == NULL) {
        ...
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add("[ItemShop][Bug Tracker] [%s][%s] Item doesn't exist (%d)", lpObj->AccountID, lpObj->Name,
                  lpMsg->ItemID);
        return;
    }

    if (lpItem->ItemKindA == 11 && CheckEventInventoryEmptySpace(lpObj, lpItem->Height, lpItem->Width) == false) {
        g_Log.Add("[ItemShop][Bug Tracker] [%s][%s] Event Inventory space isn't sufficient (%s)(%d)(%d)",
                  lpObj->AccountID, lpObj->Name, lpItem->Name, lpItem->Height, lpItem->Width);
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
    } else if (lpItem->ItemKindA == 12 && gObjChkMuunInventoryEmpty(lpObj) == false) {
        g_Log.Add("[ItemShop][Bug Tracker] [%s][%s] Muun Inventory space isn't sufficient (%s)(%d)(%d)",
                  lpObj->AccountID, lpObj->Name, lpItem->Name, lpItem->Height, lpItem->Width);
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
        ...
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add("[ItemShop][Bug Tracker] [%s][%s] Inventory space isn't sufficient (%s)(%d)(%d)", lpObj->AccountID,
                  lpObj->Name, lpItem->Name, lpItem->Height, lpItem->Width);
        return;
    }
    ...
    if (this->CheckBuyCondition(lpObj, lpMsg->ItemID) == false) {
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add("[ItemShop][Bug Tracker] [%s][%s] Cannot use this item: (%s)", lpObj->AccountID, lpObj->Name,
                  lpItem->Name);
        return;
    }

    GDReqInGameShopItemUse(lpObj->m_Index, lpMsg->UniqueCode, lpMsg->AuthCode);
    g_Log.Add("[ItemShop] [%s][%s] Request to Use Item (Code:%d) (Auth:%d) (Name:%s)", lpObj->AccountID, lpObj->Name,
              lpMsg->UniqueCode, lpMsg->AuthCode, lpItem->Name);
}

void
CItemShop::GCCashInventoryItemUse(LPOBJ lpObj, int Result, int UniqueCode, int AuthCode, int ID1, int ID2, int ID3) {
    PMSG_ANS_INGAMESHOP_ITEMUSE pMsg;
    PHeadSubSetB((LPBYTE) & pMsg, 0xD2, 0x0B, sizeof(pMsg));
    ...
    if (Iter == this->MapItemInfo.end()) {
        g_Log.Add(
                "[ItemShop] [%s][%s] Using Item Failed: Item not found in CashItemList (ItemInfo) (%d/%d) (Code:%d) (Auth:%d)",
                lpObj->AccountID, lpObj->Name, ID2, ID3, UniqueCode, AuthCode);
        pMsg.result = CS_USE_ITEM_INTERNAL_ERROR;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
    }
    ...
    if (lpItemInfo == NULL) {
        g_Log.Add(
                "[ItemShop] [%s][%s] Using Item Failed: Item not found in ItemList (%d/%d) (ID:%d/%d) (Code:%d) (Auth:%d)",
                lpObj->AccountID, lpObj->Name, ID2, ID3, Iter->second.wItemGroup, Iter->second.wItemType, UniqueCode,
                AuthCode);
        pMsg.result = CS_USE_ITEM_INTERNAL_ERROR;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
    ...
    if (lpItemInfo->ItemKindA == 11 &&
        CheckEventInventoryEmptySpace(lpObj, lpItemInfo->Height, lpItemInfo->Width) == false) {
        GDReqInGameShopItemRollbackUse(lpObj->m_Index, UniqueCode, AuthCode);
        g_Log.Add(
                "[ItemShop] [%s][%s] Using Item Failed: Not enough space in Event Inventory (%d/%d) (ID:%d/%d) (W:%d) (H:%d) (Code:%d) (Auth:%d)",
                lpObj->AccountID, lpObj->Name, ID2, ID3, Iter->second.wItemGroup, Iter->second.wItemType,
                lpItemInfo->Width, lpItemInfo->Height, UniqueCode, AuthCode);
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
        ...
        GDReqInGameShopItemRollbackUse(lpObj->m_Index, UniqueCode, AuthCode);
        g_Log.Add(
                "[ItemShop] [%s][%s] Using Item Failed: Not enough space in Muun Inventory (%d/%d) (ID:%d/%d) (W:%d) (H:%d) (Code:%d) (Auth:%d)",
                lpObj->AccountID, lpObj->Name, ID2, ID3, Iter->second.wItemGroup, Iter->second.wItemType,
                lpItemInfo->Width, lpItemInfo->Height, UniqueCode, AuthCode);
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        ...
        GDReqInGameShopItemRollbackUse(lpObj->m_Index, UniqueCode, AuthCode);
        g_Log.Add(
                "[ItemShop] [%s][%s] Using Item Failed: Not enough space in Inventory (%d/%d) (ID:%d/%d) (W:%d) (H:%d) (Code:%d) (Auth:%d)",
                lpObj->AccountID, lpObj->Name, ID2, ID3, Iter->second.wItemGroup, Iter->second.wItemType,
                lpItemInfo->Width, lpItemInfo->Height, UniqueCode, AuthCode);
        pMsg.result = CS_USE_ITEM_CANNOT_USE;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
    ...
    if (Result == 0) {
        g_Log.Add("[ItemShop] [%s][%s] Using Item Failed. (%d/%d) (Code:%d) (Auth:%d)", lpObj->AccountID, lpObj->Name,
                  ID2, ID3, UniqueCode, AuthCode);
    ...
    if (Iter->second.btItemType == 1) {
        g_PeriodItemEx.SetPeriodItemInfo(lpObj, ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType), 0,
                                         Iter->second.dwItemPeriodTime * 60);
    } else if (Iter->second.btItemType == 2) {
        DWORD periodtime = Iter->second.dwItemPeriodTime * 60;
        ItemSerialCreateSend(lpObj->m_Index, 236, NULL, NULL, ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType),
                             Iter->second.btItemLevel,
                             Iter->second.btItemDurability, Iter->second.btItemSkill, Iter->second.btItemLuck,
                             Iter->second.btItemOption,
                             Iter->second.dwItemPeriodTime, Iter->second.btItemExOption, Iter->second.btItemSetOption,
                             periodtime, 0, 0);
        ...
        if (ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType) == ITEMGET(14, 91)) {
            ...
            pMsg.result = CS_USE_ITEM_CANNOT_USE;
            IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.AddC(TColor::Red, "[ItemShop] (%s)(%s) ERROR: Trying to use UNKNOWN character card (%d/%d)",
                       lpObj->AccountID, lpObj->Name, Iter->second.wItemGroup, Iter->second.wItemType);
            return;
        }
        ...
        if (lpObj->m_PlayerData->VipType == 0 && Iter->second.btItemLevel == 0) {
            ...
            IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.AddC(TColor::Red, "[ItemShop] (%s)(%s) ERROR: Non-VIP User cannot buy a ticket with Vip Level == 0",
                       lpObj->AccountID, lpObj->Name, Iter->second.wItemGroup, Iter->second.wItemType);
            return;
        }
        ...
        if (lpObj->m_PlayerData->VipType != 0 && lpObj->m_PlayerData->VipType > Iter->second.btItemLevel) {
            ...
            g_Log.AddC(TColor::Red,
                       "[ItemShop] (%s)(%s) ERROR: Trying to use VIP card with lower VIP level while user has higher VIP level (%d/%d)",
                       lpObj->AccountID, lpObj->Name, Iter->second.wItemGroup, Iter->second.wItemType);
            return;
        }
        ...
        if (lpItemInfo->ItemKindA == 11) {
            ItemSerialCreateSend(lpObj->m_Index, 226, NULL, NULL,
                                 ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType), Iter->second.btItemLevel, 0,
                                 0, 0, 0, lpObj->m_Index, 0, 0, 0, 0, 0);
        } else if (lpItemInfo->ItemKindA == 12) {
            BYTE SocketOption[5] = {-1, -1, -1, -1, -1};
            ItemSerialCreateSend(lpObj->m_Index, 224, NULL, NULL,
                                 ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType), Iter->second.btItemLevel,
                                 Iter->second.btItemDurability, 0, 0, 0, lpObj->m_Index, 0, 0, 0, SocketOption, 0);
        } else {
            BYTE SocketOption[5] = {-1, -1, -1, -1, -1};
            ...
            if (g_PentagramSystem.IsPentagramItem(ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType)) == TRUE ||
                g_SocketOptionSystem.IsEnableSocketItem(ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType)) ==
                TRUE) {
                if (SocketCount < 1) {
                    SocketCount = 1;
                }
            ...
            ItemSerialCreateSend(lpObj->m_Index, 235, NULL, NULL,
                                 ITEMGET(Iter->second.wItemGroup, Iter->second.wItemType), Iter->second.btItemLevel,
                                 Iter->second.btItemDurability, Iter->second.btItemSkill, Iter->second.btItemLuck,
                                 Iter->second.btItemOption,
                                 lpObj->m_Index, Iter->second.btItemExOption, Iter->second.btItemSetOption, 0,
                                 SocketOption, MainAttribute);
        }
    }
    ...
    g_Log.Add("[ItemShop] [%s][%s] Using Item Success: (%d/%d) (ID:%d/%d) (Code:%d) (Auth:%d)", lpObj->AccountID,
              lpObj->Name, ID2, ID3, Iter->second.wItemGroup, Iter->second.wItemType, UniqueCode, AuthCode);
    ITEMSHOP_LOG->Output("[ItemShop] [%s][%s] Using Item Success: (%d/%d) (ID:%d/%d) (Code:%d) (Auth:%d)",
                         lpObj->AccountID, lpObj->Name, ID2, ID3, Iter->second.wItemGroup, Iter->second.wItemType,
                         UniqueCode, AuthCode);
...
void CItemShop::CGCashItemBuy(LPOBJ lpObj, PMSG_REQ_INGAMESHOP_ITEMBUY *lpMsg) {
    ...
    if (Iter == this->VecItemList.end()) {
        ...
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.AddC(TColor::Red,
                   "[ItemShop] WARNING: Trying to buy undefined Item (INDEX:%d SUBINDEX:%d CATEGORY:%d ItemID:%d BUYOPTION:%d",
                   lpMsg->ItemIndex, lpMsg->ItemSubIndex, lpMsg->Category, lpMsg->ItemID, lpMsg->ItemOpt);
        return;
    }
    ...
    if (Iter->wPackageID == 0) {
        ...
        if (Iter2 == this->MapItemInfo.end()) {
            ...
            IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.AddC(TColor::Red,
                       "[ItemShop] WARNING: Trying to buy undefined Item (ItemInfo) (INDEX:%d SUBINDEX:%d CATEGORY:%d ItemID:%d BUYOPTION:%d",
                       lpMsg->ItemIndex, lpMsg->ItemSubIndex, lpMsg->Category, lpMsg->ItemID, lpMsg->ItemOpt);
            return;
        }
    }
    ...
    if (Iter->btEnableForSale == FALSE) {
        pMsg.Result = 6;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Failed - Item (%d/%d) is not for sale", lpObj->AccountID, lpObj->Name,
                  lpMsg->ItemIndex, lpMsg->ItemSubIndex);
        return;
    }
    ...
    if (this->CheckBuyCondition(lpObj, lpMsg->ItemID) == false) {
        pMsg.Result = 6;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Failed - Item (%d/%d): User does not meet all requirements",
                  lpObj->AccountID, lpObj->Name, lpMsg->ItemIndex, lpMsg->ItemSubIndex);
        return;
    }
    ...
    switch (Iter->btCoinType) {
        ...
        case 2:
            ...
            break;
        default:
            g_Log.AddC(TColor::Red, "[ItemShop] ERROR: Unknown Coin Type:%d for Item (%d)(%d)", Iter->btCoinType,
                       lpMsg->ItemIndex, lpMsg->ItemSubIndex);
            UserCoin = 0;
            break;
    }
    ...
    if (UserCoin < Iter->wPrice) {
        pMsg.Result = 1;
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Failed - Item (%d/%d): not enough money to purchase (need:%d/have:%d)",
                  lpObj->AccountID, lpObj->Name, lpMsg->ItemIndex, lpMsg->ItemSubIndex, Iter->wPrice, UserCoin);
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
    }
        ...
        if (Iter->btRandomItemPackage == TRUE) {
            ...
            if (GUID == -1 || ID == -1) {
                ...
                g_Log.AddC(TColor::Blue,
                           "[ItemShop] ERROR: [PackageID:%d] Cannot get RandomItem (GUID == -1 || ID == -1)",
                           Iter->wPackageID);
                return;
            }
            ...
            for (It = this->VecPackageList.begin(); It != this->VecPackageList.end(); It++) {
                if (It->PackageID == Iter->wPackageID) {
                    pItem = (ISHOP_ITEM_PACKAGE *) (cBUFF + sizeof(ISHOP_ITEM_BUY_PACKAGE) +
                                                    (pMsg->Count * sizeof(ISHOP_ITEM_PACKAGE)));
            ...
            g_Log.Add("[ItemShop] (%s)(%s) Purchase Request: [Package ID: %d] [Items: %d]", lpObj->AccountID,
                      lpObj->Name, Iter->wPackageID, pMsg->Count);
            ITEMSHOP_LOG->Output("[ItemShop] (%s)(%s) Purchase Request: [Package ID: %d] [Items: %d]", lpObj->AccountID,
                                 lpObj->Name, Iter->wPackageID, pMsg->Count);
        }
    } else {
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Request: (GUID:%d) (ID:%d) (Price:%d) (CoinType:%d)", lpObj->AccountID,
                  lpObj->Name, Iter->wItemInfoGUID, Iter->wItemInfoID, Iter->wPrice, Iter->btCoinType);
        ITEMSHOP_LOG->Output("[ItemShop] (%s)(%s) Purchase Request: (GUID:%d) (ID:%d) (Price:%d) (CoinType:%d)",
                             lpObj->AccountID, lpObj->Name, Iter->wItemInfoGUID, Iter->wItemInfoID, Iter->wPrice,
                             Iter->btCoinType);
...
void CItemShop::AddCoin(LPOBJ lpObj, char EventType) {
    ...
    g_Log.Add("[ItemShop] (%s)(%s) Add Coin: (Type:%d) (Amount:%d) (Event:%d)", lpObj->AccountID, lpObj->Name,
              this->CoinAdderType, Coin, EventType);
}
...
void CItemShop::CGCashItemGift(LPOBJ lpObj, PMSG_REQ_CASHITEM_GIFT *lpMsg) {
    ...
    if (Iter == this->VecItemList.end()) {
        ...
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.AddC(TColor::Red,
                   "[ItemShop] WARNING: Trying to gift undefined Item (INDEX:%d SUBINDEX:%d CATEGORY:%d ItemID:%d BUYOPTION:%d",
                   lpMsg->ItemIndex, lpMsg->ItemSubIndex, lpMsg->Category, lpMsg->ItemID, lpMsg->ItemOpt);
        return;
    }
    ...
    if (Iter->wPackageID == 0) {
        ...
        if (Iter2 == this->MapItemInfo.end()) {
            ...
            IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
            g_Log.AddC(TColor::Red,
                       "[ItemShop] WARNING: Trying to gift undefined Item (ItemInfo) (INDEX:%d SUBINDEX:%d CATEGORY:%d ItemID:%d BUYOPTION:%d",
                       lpMsg->ItemIndex, lpMsg->ItemSubIndex, lpMsg->Category, lpMsg->ItemID, lpMsg->ItemOpt);
            return;
        }
    }
    ...
    if (Iter->btEnableForGift == FALSE) {
        pMsg.Result = 7;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Failed (Gift) - Item (%d/%d) cannot be sent as gift", lpObj->AccountID,
                  lpObj->Name, lpMsg->ItemIndex, lpMsg->ItemSubIndex);
        return;
    }
    ...
    if (UserCoin < Iter->wPrice) {
        pMsg.Result = 1;
        g_Log.Add(
                "[ItemShop] (%s)(%s) Purchase Failed (Gift) - Item (%d/%d): not enough money to purchase (need:%d/have:%d)",
                lpObj->AccountID, lpObj->Name, lpMsg->ItemIndex, lpMsg->ItemSubIndex, Iter->wPrice, UserCoin);
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        return;
    }
    ...
    if (Iter->wPackageID > 0) {
        if (Iter->btRandomItemPackage == TRUE) {
            ...
            for (It = this->VecPackageList.begin(); It != this->VecPackageList.end(); It++) {
                if (It->PackageID == Iter->wPackageID) {
                    pItem = (ISHOP_ITEM_PACKAGE *) (cBUFF + sizeof(ISHOP_ITEM_GIFT_PACKAGE) +
                                                    (pMsg->Count * sizeof(ISHOP_ITEM_PACKAGE)));
            ...
            g_Log.Add("[ItemShop] (%s)(%s) Purchase Request (Gift): [Package ID: %d] [Items: %d] [Target:%s]",
                      lpObj->AccountID, lpObj->Name, Iter->wPackageID, pMsg->Count, lpMsg->szName);
            ITEMSHOP_LOG->Output(
                    "[ItemShop] (%s)(%s) Purchase Request (Gift): [Package ID: %d] [Items: %d] [Target:%s]",
                    lpObj->AccountID, lpObj->Name, Iter->wPackageID, pMsg->Count, lpMsg->szName);
        }
    } else {
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Request (Gift): (GUID:%d) (ID:%d) (Price:%d) (CoinType:%d) (Target:%s)",
                  lpObj->AccountID, lpObj->Name, Iter->wItemInfoGUID, Iter->wItemInfoID, Iter->wPrice, Iter->btCoinType,
                  lpMsg->szName);
        ITEMSHOP_LOG->Output(
                "[ItemShop] (%s)(%s) Purchase Request (Gift): (GUID:%d) (ID:%d) (Price:%d) (CoinType:%d) (Target:%s)",
                lpObj->AccountID, lpObj->Name, Iter->wItemInfoGUID, Iter->wItemInfoID, Iter->wPrice, Iter->btCoinType,
                lpMsg->szName);
...
void CItemShop::GCCashItemBuy(LPOBJ lpObj, int ID1, int ID2, int ID3, int Result) {
    ...
    if (Result == 3 || Iter == this->MapItemInfo.end()) {
        g_Log.Add(
                "[ItemShop] (%s)(%s) Purchase Failed (%d) - Item does not found in CashItemList (ItemInfo) (GUID:%d)(ID:%d)",
                lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        ITEMSHOP_LOG->Output(
                "[ItemShop] (%s)(%s) Purchase Failed (%d) - Item does not found in CashItemList (ItemInfo) (GUID:%d)(ID:%d)",
                lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        pMsg.Result = 2;
        ...
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Failed (%d) -  Not enough money (GUID:%d)(ID:%d)", lpObj->AccountID,
                  lpObj->Name, Result, ID2, ID3);
        ITEMSHOP_LOG->Output("[ItemShop] (%s)(%s) Purchase Failed (%d) - Not enough money (GUID:%d)(ID:%d)",
                             lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        pMsg.Result = 1;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
    } else if (Result == 1) {
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Success (%d) (GUID:%d)(ID:%d)", lpObj->AccountID, lpObj->Name, Result,
                  ID2, ID3);
        ITEMSHOP_LOG->Output("[ItemShop] (%s)(%s) Purchase Success (%d) (GUID:%d)(ID:%d)", lpObj->AccountID,
                             lpObj->Name, Result, ID2, ID3);
...
void CItemShop::GCCashItemGift(LPOBJ lpObj, int ID1, int ID2, int ID3, int Result) {
    ...
    if (Result == 3) {
        g_Log.Add(
                "[ItemShop] (%s)(%s) Purchase Failed (Gift) (%d) - Item does not found in CashItemList (ItemInfo) (GUID:%d)(ID:%d)",
                lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        ITEMSHOP_LOG->Output(
                "[ItemShop] (%s)(%s) Purchase Failed (Gift) (%d) - Item does not found in CashItemList (ItemInfo) (GUID:%d)(ID:%d)",
                lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        ...
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Failed (Gift) (%d) -  Not enough money (GUID:%d)(ID:%d)",
                  lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        ITEMSHOP_LOG->Output("[ItemShop] (%s)(%s) Purchase Failed (Gift) (%d) - Not enough money (GUID:%d)(ID:%d)",
                             lpObj->AccountID, lpObj->Name, Result, ID2, ID3);
        pMsg.Result = 1;
        IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
    } else if (Result == 1) {
        g_Log.Add("[ItemShop] (%s)(%s) Purchase Success (Gift) (%d) (GUID:%d)(ID:%d)", lpObj->AccountID, lpObj->Name,
                  Result, ID2, ID3);
        ITEMSHOP_LOG->Output("[ItemShop] (%s)(%s) Purchase Success (Gift) (%d) (GUID:%d)(ID:%d)", lpObj->AccountID,
                             lpObj->Name, Result, ID2, ID3);
...
bool CItemShop::GetRandomItemFromPackage(int PackageID, int &GUID, int &ID) {
    ...
    for (std::vector<ITEMSHOP_PACKAGELIST>::iterator It = this->VecPackageList.begin();
         It != this->VecPackageList.end(); It++) {
...
std::vector<ITEMSHOP_ITEMLIST>::iterator
CItemShop::FindItemInList(int iIndex, int iSubIndex, int iOption, int iCategory) {
    std::vector<ITEMSHOP_ITEMLIST>::iterator It;

    for (It = this->VecItemList.begin(); It != this->VecItemList.end(); It++) {
        if (It->wItemIndex == iIndex && It->wItemSubIndex == iSubIndex && It->wItemOptionSelect == iOption &&
            It->ItemCategory == iCategory) {
            return It;
        }
    }
...
void CItemShop::AddGPMonster(LPOBJ lpMonsterObj) {
    ...
    for (std::vector<GP_PERMONSTER_DATA>::iterator It = this->VecGpMonster.begin();
         It != this->VecGpMonster.end(); It++) {
        if (It->MonsterID == lpMonsterObj->Class &&
            (It->MapNumber == lpMonsterObj->MapNumber || It->MapNumber == 0xFFFF)) {

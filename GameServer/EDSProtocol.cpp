            ...
            if (gObj[n].Name[0] == lpMsg->MemberID[0]) {
                if (strcmp(lpMsg->MemberID, gObj[n].Name) == 0) {
                    ...
                    if (gObj[n].m_PlayerData->lpGuild != NULL) {
                        ...
                        Guild.ConnectUser(gObj[n].m_PlayerData->lpGuild, lpMsg->GuildName, gObj[n].Name, n,
                                          lpMsg->pServer);
...
void DGGuildMasterListRecv(unsigned char *lpData) {
    ...
    for (int n = 0; n < lpCount->Count; n++) {
        ...
        if (Guild.AddMember(szGuildName, szMasterName, -1, lpCount->Count, lpList->btGuildStatus, lpList->pServer) ==
            FALSE) {
            break;
        }
    ...
    for (int n = 0; n < MAX_USER_GUILD; n++) {
        if (lpTargetGuild->Use[n] > 0) {
            if (lpTargetGuild->Index[n] >= 0) {
                ...
                if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION) {
                    _Notice.SendToUser(lpTargetGuild->Index[n], Lang.GetText(0, 184), lpReqGuild->Name,
                                       lpTargetGuild->Name);
                } else if (lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL) {
...
void DGUnionBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT *lpMsg) {
    ...
    for (int n = 0; n < MAX_USER_GUILD; n++) {
        if (lpTargetGuild->Use[n] > 0) {
            if (lpTargetGuild->Index[n] >= 0) {
                TNotice _Notice(1);
                _Notice.SendToUser(lpTargetGuild->Index[n], Lang.GetText(0, 186), lpReqGuild->Name,
                                   lpTargetGuild->Name);
            }
        }
    }
...
void DGRelationShipListRecv(EXSDHP_UNION_RELATIONSHIP_LIST *lpMsg) {
    if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION) {
        ...
        UnionManager.SetGuildUnionMemberList(lpMsg->iUnionMasterGuildNumber, lpMsg->btRelationShipMemberCount,
                                             lpMsg->iRelationShipMember);
    } else if (lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL) {
        UnionManager.AddUnion(lpMsg->iUnionMasterGuildNumber, lpMsg->szUnionMasterGuildName);
        UnionManager.SetGuildRivalMemberList(lpMsg->iUnionMasterGuildNumber, lpMsg->btRelationShipMemberCount,
                                             lpMsg->iRelationShipMember);
    }
}
...
void DGUnionListRecv(LPBYTE aRecv) {
    ...
    PHeadSetW((LPBYTE) lpMsg, 0xE9,
              lpMsg->btCount * sizeof(PMSG_UNIONLIST) + 0x10);    // #warning Check plus operation ( why 0x10 ??? )
...
void DGAnsGensInfo(_tagPMSG_ANS_GENS_INFO_EXDB *lpMsg) {
    ...
    if (gObjIsConnectedGP(iObjIndex)) {
        ...
        g_Log.Add("[GensSystem] Get DB GensInfo [%s][%s] GensName [%s]", lpObj->AccountID, lpObj->Name,
                  g_GensSystem.GetGensInfluenceName(lpObj));
...
void DGAnsRegGensMember(_tagPMSG_ANS_REG_GENS_MEMBER_EXDB *lpMsg) {
    ...
    if (gObjIsConnectedGP(lpObj->m_Index)) {
        ...
        switch (lpMsg->bResult) {
            case 0:
                g_Log.Add("[GensSystem] [%s][%s] Gens Request - Join [%s]", lpObj->AccountID, lpObj->Name,
                          g_GensSystem.GetGensInfluenceName(lpObj));
                break;

            case 1:
                g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Gens be Same : [%s]", lpObj->AccountID,
                          lpObj->Name, g_GensSystem.GetGensInfluenceName(lpObj));
                break;

            case 2:
                g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Below Gens leaving Time", lpObj->AccountID,
                          lpObj->Name);
                break;
            ...
            case 4:
                g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Gens be Different Guild Leader",
                          lpObj->AccountID, lpObj->Name);
                break;

            case 5:
                g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Not Register Guild Leader GuildName [%s]",
                          lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->GuildName);
                break;

            case 6:
                g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Already Partymember", lpObj->AccountID,
                          lpObj->Name);
                break;

            case 7:
                g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Union GuildMaster", lpObj->AccountID,
                          lpObj->Name);
                break;
...
void DGAnsSecedeGensMember(_tagPMSG_ANS_SECEDE_GENS_MEMBER_EXDB *lpMsg) {
    ...
    if (gObjIsConnectedGP(lpObj->m_Index)) {
        switch (lpMsg->bResult) {
            case 0:
                g_Log.Add("[GensSystem] [%s][%s] Gens Request - Leave [%s]", lpObj->AccountID, lpObj->Name,
                          g_GensSystem.GetGensInfluenceName(lpObj));
                break;

            case 1:
                g_Log.Add("[GensSystem] [%s][%s] Gens Leaving Failed to Not Register Gens", lpObj->AccountID,
                          lpObj->Name);
                break;

            case 2:
                g_Log.Add("[GensSystem] [%s][%s] Gens Leaving Failed to GuildMaster GensName [%s]", lpObj->AccountID,
                          lpObj->Name, g_GensSystem.GetGensInfluenceName(lpObj));
                break;

            case 3:
                g_Log.Add("[GensSystem] [%s][%s] Gens Leaving Failed to Not Influence NPC : [%s]", lpObj->AccountID,
                          lpObj->Name, g_GensSystem.GetGensInfluenceName(lpObj));
                break;
...
void DGAnsGensReward(_tagPMSG_ANS_GENS_REWARD_EXDB *lpMsg) {
    ...
    if (gObjIsConnectedGP(iObjIndex)) {
        ...
        g_Log.Add("[GensSystem] Send Gens Ranking Reward [%s][%s] Result [%d]", lpObj->AccountID, lpObj->Name,
                  btResult);
...
void DGAnsRegWantedPartyMember(_stAnsRegWantedPartyMember *lpMsg) {
    ...
    if (nPartyNum >= 0 && lpMsg->nResult == 0) {
        ...
        for (int i = 1; i < nPartyCount; i++) {
            ...
            g_Log.Add("[PMATCHING][GD][0xA4][0x05] LeaderName:%s, MemberName:%s ", pAddMsg.szLeaderName,
                      pAddMsg.szMemberName);
...
void GDReqJoinMemberPartyMatching(char *szMemberName, char *szLeaderName, char *szPassWord, int nUserIndex,
                                  int nUserDBNumber, int nLevel, BYTE btClass, BYTE btRandomParty, BYTE btGensType,
    ...
    for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
        if (lpMsg->stList[n].bUse == TRUE) {
            ...
            g_Log.Add("[PMATCHING][GC][0x42] PartyMember:%s LeaderName:%s, IsCurrentServer:%d (%d/%d life)(%d/%d mana)",
                      lpMsg->stList[n].Name, szLeaderName, nCurrentServer, (int) pList.Life, (int) pList.MaxLife,
                      (int) pList.nMana, (int) pList.nMaxMana);
...
void DGAnsAcceptMemberJoin(_stAnsAddPartyMember *lpMsg) {
    ...
    if (lpMsg->btSendType) {
        ...
        g_Log.Add("[PMATCHING][GC][0xEF][0x05] (%s/Member) resultCode:%d ", gObj[lpMsg->nMemberIndex].Name,
                  pMsg.nResult);
...
void DGFixAddPartyMember(_stAnsAddRealPartyMember *lpMsg) {
    ...
    if (gObj[nLeaderIndex].PartyNumber < 0) {
        gObj[nLeaderIndex].PartyNumber = gParty.Create(nLeaderIndex, gObj[nLeaderIndex].DBNumber,
                                                       gObj[nLeaderIndex].Level);

...
BOOL CRaklionBattleUserMng::AddUserData(int iIndex) {
    if (gObjIsConnected(iIndex) == FALSE) {
        g_Log.Add("[ RAKLION ][ BattleUser ] Add User Fail - Disconnect User [%s][%s]", gObj[iIndex].AccountID,
                  gObj[iIndex].Name);
        return FALSE;
    }
    ...
    g_Log.Add("[ RAKLION ][ Battle User ] Add User [%s][%s] Current Battle User:%d", gObj[iIndex].AccountID,
              gObj[iIndex].Name, m_BattleUser.size());
    return TRUE;
}
...
BOOL CRaklionBattleUserMng::DeleteUserData(int iIndex) {
    ...
    for (int iCount = 0; iCount < m_BattleUser.size(); iCount++) {
        if (m_BattleUser[iCount] == iIndex) {
            ...
            g_Log.Add("[ RAKLION ][ BattleUser ] Delete User - [%s][%s] Current Battle User:%d", gObj[iIndex].AccountID,
                      gObj[iIndex].Name, m_BattleUser.size());
            return TRUE;
        }
    }
...
void CRaklionBattleUserMng::CheckUserState() {
    for (int iCount = 0; iCount < m_BattleUser.size(); iCount++) {
        if (!gObjIsConnected(m_BattleUser[iCount])) {
            ...
            g_Log.Add("[ RAKLION ][ BattleUser ] Delete User - Disconnect [%s][%s]",
        ...
        if (gObj[m_BattleUser[iCount]].MapNumber != MAP_INDEX_HATCHERY &&
            ...
            DeleteUserData(m_BattleUser[iCount]);
            g_Log.Add("[ RAKLION ][ BattleUser ] Delete User - Map Move [%s][%s]", gObj[m_BattleUser[iCount]].AccountID,
                      gObj[m_BattleUser[iCount]].Name);
        }
    }
}
...
BOOL CRaklionBattleUserMng::MoveAllUser(int iGateNumber) {
    ...
    for (int iCount = 0; iCount < m_BattleUser.size(); iCount++) {
        ...
        if (bMoveGateSuccess == TRUE) {
            g_Log.Add("[ RAKLION ][ BattleUser ] [%s][%s] MoveGate(%d)", gObj[m_BattleUser[iCount]].AccountID,
                      gObj[m_BattleUser[iCount]].Name, iGateNumber);
        } else {
            DeleteUserData(m_BattleUser[iCount]);
            g_Log.Add("[ RAKLION ][ BattleUser ] [%s][%s] MoveGate Fail (%d)", gObj[m_BattleUser[iCount]].AccountID,
                      gObj[m_BattleUser[iCount]].Name, iGateNumber);

...
int WzUdp::SendSet(char *ip, int port) {
    ...
    if (this->m_SockAddr.sin_addr.S_un.S_addr == -1) {
        ...
        if (host != 0) {
            memcpy(&this->m_SockAddr.sin_addr.S_un.S_addr, *host->h_addr_list,
                   host->h_length);    // Add Type Structure {$D }
        } else {
            return 0;
        }
    }
...
int WzUdp::SendData(LPBYTE SendData, DWORD nSendDataLen) {
    ...
    Ret = WSASendTo(this->m_Socket, &this->m_PerIoSendData.DataBuf, 1, (unsigned long *) &nSendDataLen, 0,
                    (sockaddr * ) & this->m_SockAddr, 16, &this->m_PerIoSendData.Overlapped,
                    NULL);        // 28 Add Type Structure {$D}
    if (Ret == -1) {
...
BOOL WzUdp::RecvThread() {
    ...
    dwSenderSize = 16;
    while (true) {
        ret = recvfrom(this->m_Socket, (char *) &this->m_Recvbuf[this->m_dwRecvOfs], (4096 - this->m_dwRecvOfs), 0,

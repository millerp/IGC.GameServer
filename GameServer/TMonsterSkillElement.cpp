...
BOOL TMonsterSkillElement::LoadData(LPSTR lpszFileName) {
    ...
    try {
        ...
        for (pugi::xml_node element = main.child("Element"); element; element = element.next_sibling()) {
            ...
            memcpy(TMonsterSkillElement::s_MonsterSkillElementArray[iElementNumber].m_szElementName, szElementName,
                   sizeof(TMonsterSkillElement::s_MonsterSkillElementArray[iElementNumber].m_szElementName));
        }
...
void TMonsterSkillElement::ForceSkillElement(int iIndex, int iTargetIndex) {
    ...
    switch (this->m_iElementType) {
        ...
        case MSE_TYPE_PERCENT_DAMAGE:
            ...
            break;
        default:
            g_Log.AddC(TColor::Red, "[TMonsterSkillElement] ForceSkillElement(): m_iElementType unknown value: %d",
                       this->m_iElementType);
            break;
    }
}
...
BOOL TMonsterSkillElement::ApplyElementHP(int iIndex, int iTargetIndex) {
    ...
    g_Log.AddC(TColor::Red, "(%s) used Heal: (%f) + (%d) -> (%f)", lpTargetObj->Name, lpTargetObj->Life, iIncDecValue,
...
BOOL TMonsterSkillElement::ApplyElementSummon(int iIndex, int iTargetIndex) {
    ...
    if (lpObj->Class == 459 && lpObj->Connected == PLAYER_PLAYING && lpObj->MapNumber == MAP_INDEX_HATCHERY &&
        iIndex == iTargetIndex)//Season 4.5 addon
    {
...
BOOL TMonsterSkillElement::ApplyElementPush(int iIndex, int iTargetIndex) {
    ...
    if (lpObj->Class == 459 && lpObj->Connected == 3 && lpObj->MapNumber == 58 &&
        iIndex == iTargetIndex)//Season 4.5 addon
    {
        return FALSE;
    }
    ...
    if (!gObjCheckUsedBuffEffect(lpTargetObj, BUFFTYPE_POISON)) {
        if (this->m_iNullifiedSkill < 1) {
            if (retResistance(lpTargetObj, 1) == 0) {
                ...
                gObjAddBuffEffect(lpTargetObj, BUFFTYPE_POISON, EFFECTTYPE_POISON_DMG_TICK, 3, 0, 0,
                                  this->m_iIncAndDecValue);
            }
        } else {

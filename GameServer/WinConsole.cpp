    ...
    switch (uMsg) {
        ...
        case 1028:    // 0x404
            ...
            if (g_Console->m_pScriptContext) {
                if (luaL_loadbuffer(g_Console->m_pScriptContext, CWinConsole::m_CommandBuffer,
                                    strlen(CWinConsole::m_CommandBuffer), NULL))
...
void CWinConsole::Write(char const *pString, ...) {
    ...
    if (pString && g_Console && CWinConsole::m_hWnd) {
        ...
        for (int i = 0; i < strlen(szBuffer); ++i) {
            ...
            buf[indx] = 0;
            UINT64 lpdwResult = 0;
            LRESULT lResult = SendMessageTimeoutA(CWinConsole::m_hListBox, LB_ADDSTRING, 0, (LPARAM) buf, SMTO_NORMAL,
                                                  iTimeOut, &lpdwResult);
...
void CWinConsole::Init(HINSTANCE hInstance) {
    ...
    AppendMenuA(m_hSubMenu, 0, IDM_WINCONS_TOPMOST, "���� �׻� �ֻ���(&T)");    // [T]opmost ?
    AppendMenuA(m_hSubMenu, MF_SEPARATOR, 0, 0);
    AppendMenuA(m_hSubMenu, 0, IDM_WINCONS_DEBUG, "�����â ���(&D)");            // [D]ebugOut ?
    AppendMenuA(m_hSubMenu, MF_SEPARATOR, 0, 0);
    AppendMenuA(m_hSubMenu, 0, IDM_WINCONS_SAVE, "�ܼ� ���� ����(&S)");            // [S]ave ?
    AppendMenuA(m_hSubMenu, 0, IDM_WINCONS_CLEAR, "�ܼ� ���� �����(&C)");        // [C]lear ?
    AppendMenuA(m_hSubMenu, MF_SEPARATOR, 0, 0);
    AppendMenuA(m_hSubMenu, 0, IDM_WINCONS_EXIT, "�ܼ� ���� ����(&X)");            // e[X]it ?
    InsertMenuA(m_hMainMenu, 0, MF_POPUP, (UINT_PTR) m_hSubMenu, "�ܼ� ����(&C)"); // no idea

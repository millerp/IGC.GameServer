...
void CChaosCastle::Init() {
    ...
    for (int iTRAP_STEP = 0; iTRAP_STEP < MAX_CC_TRAP_STEP; iTRAP_STEP++) {
        ...
        g_rtPOINT_TRAP[0].left =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][2] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][0]) / 2 + 1;
        g_rtPOINT_TRAP[0].top = ::g_iChaosCastle_DamageAxis[iTRAP_STEP][1];
        g_rtPOINT_TRAP[0].right = ::g_iChaosCastle_DamageAxis[iTRAP_STEP][2];
        g_rtPOINT_TRAP[0].bottom =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][3] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][1]) / 2;

        g_rtPOINT_TRAP[1].left =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][2] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][0]) / 2 + 1;
        g_rtPOINT_TRAP[1].top =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][3] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][1]) / 2 + 1;
        ...
        g_rtPOINT_TRAP[2].top =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][3] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][1]) / 2 + 1;
        g_rtPOINT_TRAP[2].right =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][2] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][0]) / 2;
        ...
        g_rtPOINT_TRAP[3].right =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][2] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][0]) / 2;
        g_rtPOINT_TRAP[3].bottom =
                (::g_iChaosCastle_DamageAxis[iTRAP_STEP][3] + ::g_iChaosCastle_DamageAxis[iTRAP_STEP][1]) / 2;
    }
}
...
void CChaosCastle::Load(LPSTR filename) {
    ...
    this->m_bCC_EVENT_ENABLE =
            g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_CC) == true
...
void CChaosCastle::CheckSync(int iChaosCastleIndex) {
    ...
    if (this->m_vtChaosCastleOpenTime.empty() != false) {
        g_Log.AddC(TColor::Red,
                   "[Chaos Castle] (%d) No Schedule Time Data - Chaos Castle Terminated (m_vtChaosCastleOpenTime.empty())",
                   iChaosCastleIndex);
    ...
    switch (bTIME_CHANGED) {
        case TRUE:
            this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC =
                    (((iMIN_HOUR * 60 * 60) + (iMIN_MINUTE * 60)) -
                     ((sysTime.wHour * 60 * 60) + (sysTime.wMinute * 60) + (sysTime.wSecond))) * 1000;
            break;

        case 2:
            this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC =
                    ((((iMIN_HOUR + 24) * 60 * 60) + (iMIN_MINUTE * 60)) -
                     ((sysTime.wHour * 60 * 60) + (sysTime.wMinute * 60) + (sysTime.wSecond))) * 1000;
            break;

        default:
            g_Log.AddC(TColor::Red,
                       "[Chaos Castle] (%d) No Schedule Time Data - Chaos Castle Terminated (bTIME_CHANGED=%d)",
...
void CChaosCastle::ProcState_Closed(int iChaosCastleIndex) {
    ...
    if (iTICK_MSEC >= 1000) {
        ...
        if (this->IsEventEnabled() != FALSE) {
            if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <=
                (this->m_iCC_TIME_MIN_OPEN * 60 * 1000) &&
            ...
            if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <=
                ...
                (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC / 60000) !=
                this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_NOTIFY_COUNT) {
                this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_NOTIFY_COUNT =
                        this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC / 60000;
                ...
                if (iChaosCastleIndex == 0) {
                    ...
                    TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 126),
                                             this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_NOTIFY_COUNT + 1);
            ...
            if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 30000 &&
                this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC > 0 &&
                this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_MSG_BEFORE_ENTER == false) {
...
void CChaosCastle::ProcState_Playing(int iChaosCastleIndex) {
    ...
    if (iTICK_MSEC >= 1000) {
        ...
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <=
            ((this->m_iCC_TIME_MIN_PLAY * 60 - 30) * 1000) &&
        ...
        // Set Play Quest
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <=
            ((this->m_iCC_TIME_MIN_PLAY * 60 - 60) * 1000) &&
            ...
            this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_FIRST_USER_COUNT = this->ReCalcUserCount(
                    iChaosCastleIndex);
            this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_CURRENT_MONSTER_COUNT = this->SetMonster(
                    iChaosCastleIndex);
        ...
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == true) {
            ...
            if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_TRAP_STATUS !=
                this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_TRAP_OLD_STATUS) {
                ...
                this->SetMapAttrHollow(iChaosCastleIndex,
                                       this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_TRAP_STATUS);
                this->PullObjInnerPlace(iChaosCastleIndex,
                                        this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_TRAP_STATUS);
            }
            ...
            if (bPlayEnded == TRUE) {
                ...
                if (iMonsterCount <= 0) {
                    ...
                    if (iWinnerIndex != -1) {
                        g_Log.Add(
                                "[Chaos Castle] (%d) [%s][%s] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
                                iChaosCastleIndex + 1, gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name,
                                gObj[iWinnerIndex].Class, gObj[iWinnerIndex].Level,
                                gObj[iWinnerIndex].m_PlayerData->LevelUpPoint,
                                gObj[iWinnerIndex].m_PlayerData->Experience, gObj[iWinnerIndex].m_PlayerData->Strength,
                                gObj[iWinnerIndex].m_PlayerData->Dexterity, gObj[iWinnerIndex].m_PlayerData->Vitality,
                                gObj[iWinnerIndex].m_PlayerData->Energy,
                                gObj[iWinnerIndex].Leadership, gObj[iWinnerIndex].MapNumber,
                                gObj[iWinnerIndex].m_PK_Level);
                    ...
                    g_Log.Add("[Chaos Castle] (%d) Has No Winner : Monster Left (%d)",
                              iChaosCastleIndex + 1,
                              this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_CURRENT_MONSTER_COUNT);
                }
    ...
    if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 0) {
        g_Log.Add("[Chaos Castle] (%d) is Over : TIME-OUT (Left User:%d, Monster:%d)",
                  iChaosCastleIndex + 1, this->GetCurPlayUser(iChaosCastleIndex),
                  this->GetMonsterListCount(iChaosCastleIndex));
        ...
        if (iMonsterCount <= 0) {
            ...
            if (this->GetCurrentState(iChaosCastleIndex) == 1) //season 2.5 add-on
            {
                for (int i = 0; i < MAX_CHAOSCASTLE_MONSTER; i++) {
                    ...
                    if (((CC_MONSTER_COUNT < 0) ? FALSE : (CC_MONSTER_COUNT >
                                                           g_ConfigRead.server.GetObjectMaxMonster() - 1) ? FALSE
                                                                                                          : TRUE) ==
                        FALSE) {
                    ...
                    if (gObj[CC_MONSTER_COUNT].Life > 0.0f) {
                        if (CC_MAP_RANGE(gObj[CC_MONSTER_COUNT].MapNumber) != FALSE) {
                            if (gObj[CC_MONSTER_COUNT].Connected > 0) {
                                g_Log.Add("[Chaos Castle][Bug Tracer] (%d) Left Monster AttrInfo %d/%d",
                                          iChaosCastleIndex + 1, gObj[CC_MONSTER_COUNT].X, gObj[CC_MONSTER_COUNT].Y);
                            }
                        }
                    }
                }
...
void CChaosCastle::PullObjInnerPlace(int iChaosCastleIndex, int iTRAP_STEP) {
    ...
    for (int i = 0; i < MAX_CHAOSCASTLE_USER; i++) {
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1) {
            if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber ==
                (this->GetChaosCastleMapNumber(iChaosCastleIndex))) {
                ...
                if (btMapAttr == 8) {
                    ...
                    if (vtMAP_UNTRAP[iUSER_AXIS].empty() == false) {
                        ...
                        if (this->ObjSetPosition(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex,
                                                 ptEMPTY.x, ptEMPTY.y) == TRUE) {
    ...
    for (int iMON = 0; iMON < MAX_CHAOSCASTLE_MONSTER; iMON++) {
        ...
        if (gObj[iMON_INDEX].Life > 0.0f && CC_MAP_RANGE(gObj[iMON_INDEX].MapNumber) != FALSE &&
            gObj[iMON_INDEX].Connected > PLAYER_EMPTY) {
...
void CChaosCastle::SetMapAttrHollow(int iChaosCastleIndex, int iTRAP_STEP) {
    ...
    for (int a = 0; a < MAX_CC_HOLLOW_ZONE; a++) {
        for (int b = ::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][0];
             b <= ::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][2]; b++) {
            for (int c = ::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][1];
                 c <= ::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][3]; c++) {
                int iMapNumber = this->GetChaosCastleMapNumber(iChaosCastleIndex);
...
void CChaosCastle::ProcState_PlayEnd(int iChaosCastleIndex) {
    ...
    if (iTICK_MSEC >= 1000) {
        ...
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 30000 &&
            this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC > 0 &&
            this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_MSG_BEFORE_QUIT == false) {
...
int CChaosCastle::SetMonster(int iChaosCastleIndex) {
    ...
    for (int n = 0; n < this->m_iCC_MOB_CNT_SCRIPT[iChaosCastleIndex] &&
                    iNOW_ADDED_MONSTER_COUNT < iMAX_ADDABLE_MONSTER_COUNT; n++) {
...
void CChaosCastle::AddMonsterList(int iChaosCastleIndex, int iMonsterIndex) {
    ...
    for (int iMON = 0; iMON < MAX_CC_MONSTERS; iMON++) {
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[iMON] == -1) {
            InterlockedExchange((LPLONG) & this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[iMON],
                                iMonsterIndex);
            break;
        }
    }
...
int CChaosCastle::GetMonsterListCount(int iChaosCastleIndex) {
    ...
    for (int i = 0; i < MAX_CHAOSCASTLE_MONSTER; i++) //loc3
    {
        ...
        if (((CC_MONSTER_COUNT < 0) ? FALSE : (CC_MONSTER_COUNT > g_ConfigRead.server.GetObjectMaxMonster() - 1) ? FALSE
                                                                                                                 : TRUE) ==
            FALSE) {
            this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[i] = -1;
            continue;
    ...
    if (g_ConfigRead.server.GetServerType() != SERVER_BATTLECORE) {
        for (int iA = 0; iA < MAX_CC_MONSTER_DROP_ITEM; iA++) {
            for (int iB = 0; iB < ::g_iChaosCastle_MonsterItems[iChaosCastleIndex][iA][1]; iB++) {
                ...
                {
                    int iMONSTER_INDEX = this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[rand() %
                                                                                                          iMAX_MONSTER];
                    ...
                    if (it == this->m_stChaosCastleData[iChaosCastleIndex].m_mapMonsterItemList.end()) {
                        ...
                        this->m_stChaosCastleData[iChaosCastleIndex].m_mapMonsterItemList.insert(
                                std::pair<int, _MONSTER_ITEM_DROP>(iMONSTER_INDEX, MID));
                        break;
                    }
                }
            }
...
void CChaosCastle::SearchNDropMonsterItem(int iChaosCastleIndex, int iMonsterIndex, int iMaxHitUserIndex) {
    ...
    std::map<int, _MONSTER_ITEM_DROP>::iterator it = this->m_stChaosCastleData[iChaosCastleIndex].m_mapMonsterItemList.find(
            iMonsterIndex);
    ...
    for (int i = 0; i < MAX_CHAOSCASTLE_USER; i++) {
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1) {
            if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) != FALSE) {
                if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleIndex !=
                    -1 &&
                    gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleSubIndex !=
                    -1) {
        ...
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1) {
            if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Connected > PLAYER_LOGGED) {
                if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleIndex !=
                    ...
                    gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleSubIndex !=
                    -1) {
                    IOCP.DataSend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex,
                                  (UCHAR * ) & pNotice, pNotice.h.size);
                }
            }
        }
    }
...
void CChaosCastle::SendNoticeState(int iChaosCastleIndex, int iPlayState) {
    ...
    pMsg.wMaxKillMonster = this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_FIRST_USER_COUNT +
                           this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_FIRST_MONSTER_COUNT;
        ...
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1) {
            if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Connected > PLAYER_LOGGED) {
                if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleIndex !=
                    ...
                    -1) {
                    IOCP.DataSend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex,
                                  (UCHAR * ) & pMsg, pMsg.h.size);

                    if (iPlayState == 7) {
                        if (m_stChaosCastleData[iChaosCastleIndex].m_lCC_CURRENT_MONSTER_COUNT == 0) {
                            g_QuestExpProgMng.ChkUserQuestTypeEventMap(QUESTEXP_ASK_CHAOSCASTLE_CLEAR,
...
void CChaosCastle::SendWinMessage(int iChaosCastleIndex, int iWinnerIndex) {
    ...
    if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
        this->GD_Req_Save_KillPoint_UBF(iWinnerIndex, gObj[iWinnerIndex].Name,
                                        3 *
                                        gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cKillUserCount +
                                        2 *
                                        gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cKillMonsterCount +
                                        50,
                                        iChaosCastleIndex);
    }
    ...
    g_QuestExpProgMng.ChkUserQuestTypeEventMap(QUESTEXP_ASK_CHAOSCASTLE_CLEAR, &gObj[iWinnerIndex], iChaosCastleIndex,
                                               0);
            ...
            if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != iWinnerIndex) {
                if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_bSendQuitMsg == false) {
                    if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
                        ...
                        this->GD_Req_Save_KillPoint_UBF(
                                this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,
                    ...
                    if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
                        this->RewardUserKillPointUBF(iChaosCastleIndex,
                                                     gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleSubIndex,
                                                     0);
                    } else {
                        this->RewardUserEXP(iChaosCastleIndex,
                                            gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleSubIndex,
                                            false);
                    }
                    ...
                    g_QuestExpProgMng.ChkUserQuestTypeEventMap(QUESTEXP_ASK_CHAOSCASTLE_CLEAR,
                                                               &gObj[m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex],
                                                               iChaosCastleIndex, 0);
                    GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 129),
                                                     this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex,
                                                     1);
...
void CChaosCastle::ProcessFallUser(int iChaosCastleIndex) {
    ...
    for (int iFALL = 0; iFALL < MAX_CHAOSCASTLE_USER; iFALL++) {
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_lFallUser[iFALL] != -1) {
            ...
            if ((gObj[iFALL_INDEX].MapNumber) == this->GetChaosCastleMapNumber(iChaosCastleIndex) &&
                gObjIsConnected(iFALL_INDEX) && gObj[iFALL_INDEX].Life > 0) {
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp) {
    ...
    if ((gObj[iUserIndex].m_PlayerData->Experience + iAddExp) < gObj[iUserIndex].m_PlayerData->NextExp) {
        ...
        if (g_ConfigRead.data.reset.iBlockLevelUpPointAfterResets == -1 ||
            gObj[iUserIndex].m_PlayerData->m_iResets < g_ConfigRead.data.reset.iBlockLevelUpPointAfterResets) {
            if (gObj[iUserIndex].Class == CLASS_DARKLORD || gObj[iUserIndex].Class == CLASS_MAGUMSA ||
                gObj[iUserIndex].Class == CLASS_RAGEFIGHTER || gObj[iUserIndex].Class == CLASS_GROWLANCER) {
...
int CChaosCastle::CheckEnterLevel(int iIndex, int iLevel) {
    ...
    if (gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_MAGUMSA ||
        ...
        if (gObj[iIndex].Level >= g_sttCHAOSCASTLE_LEVEL[iLevel - 1].iLOWER_BOUND_MAGUMSA &&
            gObj[iIndex].Level <= g_sttCHAOSCASTLE_LEVEL[iLevel - 1].iUPPER_BOUND_MAGUMSA) {
            return 0;
        }
        ...
        if (gObj[iIndex].Level >= g_sttCHAOSCASTLE_LEVEL[iLevel - 1].iLOWER_BOUND &&
            gObj[iIndex].Level <= g_sttCHAOSCASTLE_LEVEL[iLevel - 1].iUPPER_BOUND) {
            return 0;
        }
...
int CChaosCastle::GetRemainTime(int iChaosCastleIndex) {
    ...
    if (this->GetCurrentState(iChaosCastleIndex) == 1) {
        iREMAIN_MINUTE =
                this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC / 60000 - this->m_iCC_TIME_MIN_OPEN + 1;
    } else {
        SYSTEMTIME sysTime;
        GetLocalTime(&sysTime);
        int iMIN_HOUR = 24;
        ...
        switch (bTIME_CHANGED) {
            case 1:
                iREMAIN_MINUTE = (((iMIN_HOUR * 60 * 60 + iMIN_MINUTE * 60) -
                                   (sysTime.wHour * 60 * 60 + sysTime.wMinute * 60 + sysTime.wSecond))) / 60 + 1;
                break;
            case 2:
                iREMAIN_MINUTE = ((((iMIN_HOUR + 24) * 60 * 60 + iMIN_MINUTE * 60) -
                                   (sysTime.wHour * 60 * 60 + sysTime.wMinute * 60 + sysTime.wSecond))) / 60 + 1;
                break;
            default:
                iREMAIN_MINUTE = -1;
...
void CChaosCastle::SafetyCastleZone(int iChaosCastleIndex) {
    ...
    for (int i = g_btCastleSafetyZoneMapXY[iChaosCastleIndex][0];
         i <= g_btCastleSafetyZoneMapXY[iChaosCastleIndex][2]; i++) {
        for (int j = g_btCastleSafetyZoneMapXY[iChaosCastleIndex][1];
             j <= g_btCastleSafetyZoneMapXY[iChaosCastleIndex][3]; j++) {
            MapC[iMapNumber].m_attrbuf[j * 256 + i] |= 1;
        }
    }

    for (int iHOLE_NUM = 0; iHOLE_NUM < 2; iHOLE_NUM++) {
        for (int i = g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][0];
             i <= g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][2]; i++) {
            for (int j = g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][1];
                 j <= g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][3]; j++) {
            ...
            if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
                ...
                if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Life > 0.0) {
                    ...
                    if (btMapAttr == 8) {
                        ...
                        GSProtocol.GCDiePlayerSend(
                                &gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex],
                                this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex, 0, 0);

                        g_Log.Add(
                                "[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
                                iChaosCastleIndex + 1,
                                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
                                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
                                iSX, iSY);

                        this->SendFailMessage(iChaosCastleIndex,
                                              this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex);
    ...
    for (int i = 0; i < MAX_CHAOSCASTLE_USER; i++) {
        if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1) {
            if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
                ...
                int iPAYBACK_MONEY =
                        g_iChaosCastle_EnterCost[iChaosCastleIndex] + g_iChaosCastle_GuardSetValue[iChaosCastleIndex];
                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_PlayerData->Money +=
                        g_iChaosCastle_EnterCost[iChaosCastleIndex] + g_iChaosCastle_GuardSetValue[iChaosCastleIndex];
                int iNOW_MONEY = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_PlayerData->Money;

                GSProtocol.GCMoneySend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex,
                                       gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_PlayerData->Money);
                ...
                IOCP.DataSend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex, (LPBYTE) & Notice,
                              Notice.h.size);

                g_Log.Add(
                        "[Chaos Castle] (%d) [%s][%s] Payback to User ChaosCastle Money (FIRST:%d, PAYBACK:%d, FINAL:%d)",
                        iChaosCastleIndex + 1,
                        gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
                        gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
...
void CChaosCastle::RewardUserEXP(int iChaosCastleIndex, int iChaosCastleSubIndex, BOOL bWinner) {
    ...
    if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex != -1) {
        if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex) ==
            TRUE) {
            if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cChaosCastleIndex ==
                -1 ||
                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cChaosCastleSubIndex ==
                -1) {
                return;
            }
            ...
            int iTOT_EXP = iKILLCOUNT_USER * g_iChaosCastle_ExpTable[iChaosCastleIndex][0] +
                           iKILLCOUNT_MONSTER * g_iChaosCastle_ExpTable[iChaosCastleIndex][1];

            int iREWARD_EXP = this->CalcSendRewardEXP(
                    this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,
            ...
            IOCP.DataSend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,
                          (LPBYTE) & pMsg, pMsg.h.size);

            this->ChaosCastleRank(
                    this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,
                    iKILLCOUNT_USER,
                    iKILLCOUNT_MONSTER, iTOT_EXP, bWinner);

            g_Log.Add(
                    "[Chaos Castle] (%d) [%s][%s] Reward User EXP (USER_KILL:%d, MON_KILL:%d, TOT_EXP:%d, TOT_REWARD_EXP:%d)",
                    iChaosCastleIndex + 1,
                    gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].AccountID,
                    gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].Name,
            ...
            if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
                ...
                if (iSX >= g_iChaosCastle_GroundAxis[0] &&
                    ...
                    if (gObjCheckUsedBuffEffect(
                        ...
                        gObjAddBuffEffect(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex],
                                          BUFFTYPE_FREEZE, 0, 0, 0, 0, 3);
                    }
                }
            }
        }
...
void CChaosCastle::GiveWinnerItem(int iChaosCastleIndex, int iWinnerIndex) {
    ...
    switch (iChaosCastleIndex) {
        case 0:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC1, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
        case 1:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC2, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
        case 2:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC3, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
        case 3:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC4, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
        case 4:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC5, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
        case 5:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC6, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
        case 6:
            g_BagManager.UseBag_GremoryCase(iWinnerIndex, BAG_EVENT, EVENTBAG_CC7, iWinnerIndex, GC_STORAGE_CHARACTER,
                                            GC_REWARD_CHAOS_CASTLE, 30);
            break;
    }
}
...
void CChaosCastle::GD_Req_Save_KillPoint_UBF(int index, char *Name, int KillPoint, int CastleIndex) {
    ...
    g_Log.Add("[CChaosCastle][GD_Req_Save_KillPoint][%s][%s][%s] Save Requested To DB, KillPoint:%d Castle Index:%d",
              gObj[index].AccountID, gObj[index].m_PlayerData->m_RealNameOfUBF, gObj[index].Name, KillPoint,
              CastleIndex);
}
...
void CChaosCastle::RewardUserKillPointUBF(int iChaosCastleIndex, int iChaosCastleSubIndex, bool bWinner) {
    ...
    if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cChaosCastleSubIndex ==
        -1) {
        return;
    }
    ...
    int iKILLCOUNT_USER = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cKillUserCount;
    int iKILLCOUNT_MONSTER = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cKillMonsterCount;
    int iTOT_EXP = (g_iChaosCastle_ExpTable[iChaosCastleIndex][1] * iKILLCOUNT_MONSTER) +
                   (g_iChaosCastle_ExpTable[iChaosCastleIndex][0] * iKILLCOUNT_USER);
    int iREWARD_EXP = g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE ? 0 : this->CalcSendRewardEXP(
            this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,
            iTOT_EXP, iKILLCOUNT_USER, iKILLCOUNT_MONSTER);
    ...
    if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
        ...
        IOCP.DataSend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,
                      (LPBYTE) & pUBFMsg, pUBFMsg.h.size);

        g_Log.Add(
                "[UBF][Chaos Castle] (%d) [%s][%s][%s] GameResult (USER_KILL:%d, MON_KILL:%d, TOT_KILLPOINT:%d, TOT_REWARD_EXP:%d)",
                iChaosCastleIndex + 1,
                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].AccountID,
                gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].Name,

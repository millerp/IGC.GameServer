...
void CImperialGuardian::LoadScript(LPSTR szFileName) {
    ...
    this->m_rewardExp.LoadScript(
            g_ConfigRead.GetPath("\\Events\\ImperialGuardian\\IGC_ImperialGuardian_Reward_Exp.xml"));
...
void CImperialGuardian::LoadItemBag() {
    ...
    this->pEventDungeonItemBag->LoadScript(
            g_ConfigRead.GetPath("\\Events\\ImperialGuardian\\IGC_ImperialGuardian_Drop_General.xml"));
    this->pEventDungeonItemBagGaion->LoadScript(
            g_ConfigRead.GetPath("\\Events\\ImperialGuardian\\IGC_ImperialGuardian_Drop_Gaion.xml"));
    this->pEventDungeonItemBagStatue->LoadScript(
            g_ConfigRead.GetPath("\\Events\\ImperialGuardian\\IGC_ImperialGuardian_Drop_Statue.xml"));
}
...
void CImperialGuardian::ProcBeginTimeAttack(int nZoneIndex) {
    ...
    if (this->m_stZoneInfo[nZoneIndex].m_bGoTimeAttack) {
        ...
        if (!this->m_stZoneInfo[nZoneIndex].m_bSendWarningMsg &&
            this->m_stZoneInfo[nZoneIndex].m_dwRemainTimeAttack <= 60000) {
            char szMsg[255];
        ...
        this->RegenMonster(nZoneIndex, this->m_stZoneInfo[nZoneIndex].m_nMonsterRegenTableIndex,
                           this->m_stZoneInfo[nZoneIndex].m_nMaxUserLevel, false);
        this->GCNotifyRemainTickCount(nZoneIndex, 2, this->m_stZoneInfo[nZoneIndex].m_dwRemainTimeAttack);

        g_Log.Add("[IMPERIALGUARDIAN] BEGIN TIMEATTACK -> [ZONE]:%d", nZoneIndex + 1);
        g_Log.Add("[IMPERIALGUARDIAN] OBJECT COUNT -> [ZONE]:%d [COUNT]:%d", nZoneIndex + 1,
                  this->m_stZoneInfo[nZoneIndex].m_vtMonsterIndexInfo.size());
    }
}
...
void CImperialGuardian::ProcAllWarpNextZone(int nZoneIndex) {
    ...
    if ((nZoneIndex + 1) < 4) {
        for (std::vector<int>::iterator it = this->m_stZoneInfo[nZoneIndex].m_vtUserIndex.begin();
             it != this->m_stZoneInfo[nZoneIndex].m_vtUserIndex.end(); it++) {
    ...
    for (std::vector<int>::iterator it = this->m_stZoneInfo[nZoneIndex].m_vtUserIndex.begin();
        ...
        if (gObj[*it].Connected > 0) {
            if (IMPERIAL_MAP_RANGE(gObj[*it].MapNumber) == TRUE) {
                g_Log.Add("[IMPERIALGUARDIAN] MISSION CLEAR USER ->[AccountID]:%s [Name]:%s", gObj[*it].AccountID,
                          gObj[*it].Name);
            }
        }
    }
            ...
            if ((gObj[i].Authority & 2) != 2 && (gObj[i].Authority & 0x20) != 0x20) {
                if (this->CheckValidUser(i)) {
                    if (this->m_nCheatMode == 0 && gObj[i].PartyNumber < 0) {
                        gObjMoveGate(i, 22);
                        g_Log.Add("[IMPERIALGUARDIAN][ Invalid User ] User is not a party [%s][%s]", gObj[i].AccountID,
                                  gObj[i].Name);
                    }
...
void CImperialGuardian::CGEnterPortal(int nUserIndex, int nDestZoneIndex) {
    ...
    pMsg.m_dwRemainTime = this->m_stZoneInfo[nDestZoneIndex].m_dwRemainWaitPlayerTime +
                          this->m_stZoneInfo[nDestZoneIndex].m_dwRemainLootTime +
                          this->m_stZoneInfo[nDestZoneIndex].m_dwRemainTimeAttack;
    ...
    if (this->m_nCheatMode == TRUE || nDestZoneIndex <= 0 ||
        this->GetCurrentZoneIndex(nUserIndex) == (nDestZoneIndex - 1)) {
        if (this->GetZoneState(nDestZoneIndex) && this->GetZoneState(nDestZoneIndex) != 2) {
            ...
            if (this->m_stZoneInfo[nDestZoneIndex].m_nPartyNumber >= 0 ||
                ...
                if (nDestZoneIndex > 0 && this->m_nCheatMode != TRUE &&
                    !this->m_stZoneInfo[nDestZoneIndex - 1].m_bAblePortal) {
                    char szMsg[256];
                ...
                if (nDestZoneIndex) {
                    if (this->m_stZoneInfo[nDestZoneIndex - 1].m_nMonsterRegenTableIndex < 0 || this->m_nCheatMode) {
                        ...
                        this->m_stZoneInfo[nDestZoneIndex].m_nMonsterRegenTableIndex = this->m_stZoneInfo[
                                nDestZoneIndex - 1].m_nMonsterRegenTableIndex;
                        this->m_stZoneInfo[nDestZoneIndex].m_nMaxUserLevel = this->m_stZoneInfo[nDestZoneIndex -
                                                                                                1].m_nMaxUserLevel;
                    }
                } else {
                ...
                this->RegenMonster(nDestZoneIndex, this->m_stZoneInfo[nDestZoneIndex].m_nMonsterRegenTableIndex,
                                   nMaxLevel, true);
            ...
            int nGateIndex = this->GetGateNumber(nDestZoneIndex,
                                                 this->m_stZoneInfo[nDestZoneIndex].m_nMonsterRegenTableIndex);
            ...
            if (nGateIndex != -1) {
                    ...
                    if (lpObj->pInventory[nEnterItemPos].m_Type != ITEMGET(14, 102) &&
                        ...
                        if (lpObj->pInventory[nEnterItemPos].m_Type == ITEMGET(13, 126) ||
                            ...
                            if (lpObj->pInventory[nEnterItemPos].m_Durability > 0.0) {
                                GSProtocol.GCItemDurSend2(nUserIndex, nEnterItemPos,
                                                          lpObj->pInventory[nEnterItemPos].m_Durability, 0);
                            } else {
                ...
                g_Log.Add("[IMPERIALGUARDIAN] Enter Zone User -> [AccountID]:%s [NAME]:%s [ZONE]:%d", lpObj->AccountID,
                          lpObj->Name, nDestZoneIndex + 1);
                g_Log.Add("[IMPERIALGUARDIAN] [%d ZONE USER COUNT] : %d", nDestZoneIndex + 1,
                          this->GetPlayerCount(nDestZoneIndex));
            }
        }
    }
}
...
void
CImperialGuardian::RegenMonster(int nZoneIndex, int nMonsterRegenTableIndex, int nMaxUserLevel, bool bOnlyCreateGate) {
    if (nZoneIndex < 0 || nZoneIndex > 3) {
    ...
    if (nMonsterRegenTableIndex < 0 || nMonsterRegenTableIndex > 6) {
        g_Log.Add("[IMPERIALGUARDIAN][RegenMonsterFunc] Invalid MonsterRegenTable Index => %d",
                  nMonsterRegenTableIndex);
        return;
    }
    ...
    for (std::vector<_stIFMonsterPos>::iterator It = this->m_vtMonsterPosInfo.begin();
         It != this->m_vtMonsterPosInfo.end(); It++) {
        if (It->m_Zone == nZoneIndex && It->m_Day == nMonsterRegenTableIndex) {
            ...
            if (It->m_Type < 523 || It->m_Type > 528) {
                ...
                g_Log.Add(
                        "[IMPERIALGUARDIAN] AddMonster => %s, MapNumber => %d ,Zone => %d PosX => %d, PosY => %d, Connected => %d, STATE => %d",
                        gObj[nResult].Name, gObj[nResult].MapNumber, gObj[nResult].m_nZoneIndex, gObj[nResult].X,
                        gObj[nResult].Y, gObj[nResult].Connected, gObj[nResult].m_State);
            } else {
...
void CImperialGuardian::InitZone(int nZoneIndex) {
    ...
    this->m_stZoneInfo[nZoneIndex].m_nSendRemainCreatePortal =
            this->m_stZoneInfo[nZoneIndex].m_dwRemainTimeAttack / 60000 + 1;
...
void CImperialGuardian::ProcCheckDungeon(int nZoneIndex) {
    ...
    if (this->GetZoneState(nZoneIndex)) {
        if (this->GetZoneState(nZoneIndex) == 1 && this->m_stZoneInfo[nZoneIndex].m_bStartRegenMonster == true &&
...
void CImperialGuardian::CheckLiveMonster(int nZoneIndex) {
    for (std::vector<_stMonsterIndexInfo>::iterator it = this->m_stZoneInfo[nZoneIndex].m_vtMonsterIndexInfo.begin();
         it != this->m_stZoneInfo[nZoneIndex].m_vtMonsterIndexInfo.end(); it++) {
        if (it->m_bLive == true) {
        ...
        if (gObj[it->m_nIndex].Live == TRUE) {
            if (gObj[it->m_nIndex].m_State != 1 && gObj[it->m_nIndex].m_State != 2) {
                g_Log.Add("[IMPERIALGUARDIAN][RESTORE] [NAME]:%s [INDEX]:%d [TYPE]:%d [STATE]:%d",
                          gObj[it->m_nIndex].Name, it->m_nIndex,
...
int CImperialGuardian::GetCurrentZoneIndex(int nUserNumber) {
    ...
    for (int i = 0; i < MAX_ZONE; i++) {
        for (std::vector<int>::iterator it = this->m_stZoneInfo[i].m_vtUserIndex.begin();
             it != this->m_stZoneInfo[i].m_vtUserIndex.end(); it++) {
            if (*it == nUserNumber) {
                iZoneIndex = i;
                break;
            }
...
void CImperialGuardian::RemoveUserInZone(int nZoneIndex, int nUserNumber) {
    ...
    for (std::vector<int>::iterator it = this->m_stZoneInfo[nZoneIndex].m_vtUserIndex.begin();
        ...
        if (*it == nUserNumber) {
            ...
            g_Log.Add("[IMPERIALGUARDIAN] Leave Player Zone [ZONE]:%d, [AccountID]:%s, [Name]:%s", nZoneIndex,
                      gObj[nUserNumber].AccountID, gObj[nUserNumber].Name);
            break;
        }
    }
...
bool CImperialGuardian::GCNotifyAllZoneClear(int nZoneIndex) {
    ...
    for (int i = 0; i < nCount; i++) {
        ...
        g_Log.Add("[IMPERIALGUARDIAN] [ACCOUNTID]:%s, [NAME]:%s, [Reward Exp] : %d", gObj[nUserIndex[i]].AccountID,
                  gObj[nUserIndex[i]].Name, pMsg.m_nRewardExp);
...
void CImperialGuardian::RollBackUserPos(int nUserNumber) {
    if (gObj[nUserNumber].Type == OBJ_USER && gObj[nUserNumber].Connected > PLAYER_LOGGED) {
        ...
        GSProtocol.GCTeleportSend(&gObj[nUserNumber], -1, gObj[nUserNumber].MapNumber, gObj[nUserNumber].X,
                                  gObj[nUserNumber].Y, gObj[nUserNumber].Dir);
...
bool CImperialGuardian::IsAttackAbleMonster(int nMonsterIndex) {
    for (int i = 0; i < MAX_ZONE; i++) {
        for (std::vector<_stMonsterIndexInfo>::iterator it = this->m_stZoneInfo[i].m_vtMonsterIndexInfo.begin();
             it != this->m_stZoneInfo[i].m_vtMonsterIndexInfo.end(); it++) {
...
void CImperialGuardian::DestroyGate(int nZoneIndex, int nIndex, int nTargetIndex) {
    ...
    g_Log.Add(
            "[IMPERIALGUARDIAN] DestroyGate -> [ZONE]:%d, [AccountID]:%s, [NAME]:%s, [GATE INDEX]:%d, [USER COUNT]:%d",
            nZoneIndex, gObj[nTargetIndex].AccountID, gObj[nTargetIndex].Name, nIndex,
            this->m_stZoneInfo[nZoneIndex].m_vtUserIndex.size());
}
...
bool CImperialGuardian::IsRegPartyUser(int nZoneIndex, int nUserIndex) {
    ...
    for (std::vector<int>::iterator it = this->m_stZoneInfo[nZoneIndex].m_vtRegPartyUser.begin();
         it != this->m_stZoneInfo[nZoneIndex].m_vtRegPartyUser.end(); it++) {
        if (*it == nUserIndex) {
            bFlag = true;
        }
    }
...
void CImperialGuardian::MonsterBaseAct(LPOBJ lpObj) {
    ...
    if (lpObj->m_ActState.Emotion == 1) {
        ...
        if (lpObj->TargetNumber >= 0 && lpObj->PathStartEnd == 0) {
            ...
            if (dis <= attackrange) {
                ...
                if (gObjMonsterGetTargetPos(lpObj) == TRUE) {
                    ...
                    if (lpTargetObj->Connected > PLAYER_LOGGED &&
                        gObjGetTargetPos(lpObj, lpTargetObj->X, lpTargetObj->Y, tx, ty) == TRUE) {
                        if (MAX_MAP_RANGE(lpObj->MapNumber) == FALSE) {
                            g_Log.AddC(TColor::Red, "[ERROR] MAX_MAP_RANGE (gObjMonster) == FALSE (%d)",
                                       lpObj->MapNumber);
                            return;
                        }
        ...
        if (lpObj->m_MoveRange > 0 && !gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_STONE) &&
            !gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_STUN) && !gObjCheckUsedBuffEffect(lpObj, BUFFTYPE_SLEEP) &&

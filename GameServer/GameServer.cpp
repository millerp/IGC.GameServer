...
IDS_APP_TITLE, szTitle, 100);
LoadString(hInstance,
IDS_GAMESERVER, szWindowClass, 100);
...
hInstance = hInstance;
wcex.
hIcon = LoadIcon(hInstance, (LPCTSTR) IDI_NORMAL_V1);
wcex.
hCursor = LoadCursor(NULL, IDC_ARROW);
wcex.
hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
wcex.
lpszMenuName = (LPCSTR) IDR_MENU;
wcex.
lpszClassName = szWindowClass;
wcex.
hIconSm = LoadIcon(wcex.hInstance, (LPCTSTR) IDI_NORMAL_V1);
...
HWND StatusBar = CreateWindowExA(0, STATUSCLASSNAME, NULL, WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, ghWnd, (HMENU) 130,
                                 hInstance, NULL);
ghStatusBar = StatusBar;

int iQueueBarWidths[] = {130, 340, 470, 600, 730, 860, 990, 1120, 1250, 1630, 1760, 1890, -1};
SendMessage(StatusBar, SB_SETPARTS,
11, (LPARAM)iQueueBarWidths);
...
IDD_ABOUT_BOX, hWnd, (DLGPROC)About);
break;
// Players
...
DialogBox(hInst, (LPCTSTR)
IDD_DCMENU, hWnd, (DLGPROC)GSDisconnect);
break;
...
g_LastManStanding.
LoadConfig(g_ConfigRead
.GetPath("\\Events\\IGC_LastManStanding.xml"));
g_LastManStanding.
...
OpenItemScript(g_ConfigRead
.GetPath("\\Items\\IGC_ItemList.xml"));
g_PentagramMixSystem.
LoadMixNeedSourceScript(g_ConfigRead
.GetPath("\\Items\\IGC_PentagramMixNeedSource.xml"));
g_PentagramMixSystem.
LoadJewelOptionScript(g_ConfigRead
.GetPath("\\Items\\IGC_PentagramJewelOptionValue.xml"));
g_PentagramSystem.
LoadDropScript(g_ConfigRead
.GetPath("\\Items\\IGC_PentagramItemDropRate.xml"));
g_PentagramSystem.
LoadJewelOutRate(g_ConfigRead
.GetPath("\\Items\\IGC_PentagramJewelOutRate.xml"));
//g_PentagramSystem.LoadPentagramSetOptionScript(gDirPath.GetNewPath("\\Items\\PentagramSetOption.ini"));
g_PentagramSystem.
LoadPentagramOptionScript(g_ConfigRead
.GetPath("\\Items\\IGC_PentagramOption.xml"));
LoadResetItemList(g_ConfigRead
.GetPath("\\Items\\IGC_ResetItem.xml"));
gSetItemOption.
LoadOptionInfo(g_ConfigRead
.GetPath("\\Items\\IGC_ItemSetOption.xml"));
gSetItemOption.
LoadTypeInfo(g_ConfigRead
.GetPath("\\Items\\IGC_ItemSetType.xml"));
g_SocketOptionSystem.
LoadOptionScript(g_ConfigRead
.GetPath("\\SocketSystem\\IGC_SocketOption.xml"));
g_kJewelOfHarmonySystem.
LoadScript(g_ConfigRead
.GetPath("\\Items\\IGC_HarmonyItem_Option.xml"));
g_kJewelOfHarmonySystem.
LoadScriptOfSmelt(g_ConfigRead
.GetPath("\\Items\\IGC_HarmonyItem_Smelt.xml"));
g_kItemSystemFor380.
Load380ItemOptionInfo(g_ConfigRead
.GetPath("\\Items\\IGC_Item380Option.xml"));
g_ItemAddOption.
Load(g_ConfigRead
.GetPath("\\Items\\IGC_ItemOptionManager.xml"));
break;
...
MagicDamageC.
LogSkillList(g_ConfigRead
.GetPath("\\Skills\\IGC_SkillList.xml"));
break;
case ID_RELOADMAINDATA_WARPS:
gMoveCommand.
Load(g_ConfigRead
.GetPath("\\Warps\\IGC_MoveReq.xml"));
gMoveCommand.
LoadMoveLevel(g_ConfigRead
.GetPath("\\Warps\\IGC_MoveLevel.xml"));
break;
...
g_EventManager.
Load(g_ConfigRead
.GetPath("\\Events\\IGC_InvasionManager.xml"));
...
g_QuestInfo.
LoadQuestInfo(g_ConfigRead
.GetPath("IGC_ClassQuest.xml"));
break;
...
CheckSumFileLoad(gDirPath
.GetNewPath("CheckSum.dat"));
break;
case ID_RELOADMISCDATA_ANTISWEAR:
g_prohibitedSymbols.
LoadSymbolFile(g_ConfigRead
.GetPath("IGC_ProhibitedSymbols.xml"));
SwearFilter.
LoadFile(g_ConfigRead
.GetPath("IGC_ProhibitedWords.xml"));
break;
case ID_RELOADMAINDATA_ITEMDROP:
ItemDrop.
LoadFile(g_ConfigRead
.GetPath("IGC_DropManager.xml"));
SetItemDrop.
LoadFile(g_ConfigRead
.GetPath("IGC_SetItemDropManager.xml"));
break;
case ID_RELOADMISCDATA_EXPMANAGER:
g_ExpManager.
LoadScript(g_ConfigRead
.GetPath("IGC_ExpSystem.xml"));
break;
case ID_RELOADMISCDATA_MAPATTRIBUTE:
g_MapAttr.
LoadFile(g_ConfigRead
.GetPath("IGC_MapAttribute.xml"));
break;
...
g_OffLevel.
LoadFile(g_ConfigRead
.GetPath("\\Plugins\\IGC_OffLevelling.xml"));
g_OffLevel.
LoadSkillDefinitions(g_ConfigRead
.GetPath("\\Skills\\IGC_SkillCategory.xml"));
}
else {
g_Log.
AddC(TColor::Red,
"Your license does not have Offlevel plugin purchased, please disable Off-Level system");
}
break;
...
GMJoinServerConnect(g_ConfigRead
.server.

GetJoinServerIP(),

WM_GM_JOIN_CLIENT_MSG_PROC);
GMDataServerConnect(g_ConfigRead
.server.

GetDataServerIP(),

WM_GM_DATA_CLIENT_MSG_PROC);
ExDataServerConnect(g_ConfigRead
.server.

GetExDBIP(),

WM_GM_EXDATA_CLIENT_MSG_PROC);
break;
}
break;
case
...
{
MessageBox(ghWnd,
"Players are already connected to GameServer \nPlease disconnect all players first", "Close", MB_OK|MB_ICONERROR);
}
else
if (
MessageBox(ghWnd,
"Do you want to close eGameServer?", "Close", MB_YESNO|MB_APPLMODAL) == IDYES)
{
...
wsprintf(szTempText,
"Players: %d/%d", gObjTotalUser, g_ConfigRead.server.

GetObjectMaxUser()

);
SendMessage(ghStatusBar, SB_SETTEXT,
0, (LPARAM)szTempText);

wsprintf(szTempText,
"Monsters: (A/L/M) %d/%d/%d", gObjTotalMonster, gMSetBase.m_Count, g_ConfigRead.server.
...
{
wsprintf(szTempText,
"CC: %d minutes", g_ChaosCastle.GetRemainTime(0));
}

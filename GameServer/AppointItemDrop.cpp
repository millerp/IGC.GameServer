...
void CAppointItemDrop::LoadAppointItemDropScript() {
    ...
    for (pugi::xml_node monster = monsterlist.child("Monster"); monster; monster = monster.next_sibling()) {
        ...
        std::map<int, _ST_APP_MONSTER_ITEMGROUP_LINK>::iterator It = this->m_mapMonsterItemGroupLink.find(
                tmpMonsterItemGroupLink.iMonsterIndex);

        if (It == this->m_mapMonsterItemGroupLink.end()) {
            m_mapMonsterItemGroupLink.insert(
                    std::pair<int, _ST_APP_MONSTER_ITEMGROUP_LINK>(tmpMonsterItemGroupLink.iMonsterIndex,
                                                                   tmpMonsterItemGroupLink));
        } else {
            g_Log.MsgBox("[IGC_AppointItemDrop.xml] Error - link already exists (%d) (%d)",
                         tmpMonsterItemGroupLink.iMonsterIndex, tmpMonsterItemGroupLink.iDropItemGroup);
        }
    }
    ...
    for (pugi::xml_node dropgroup = itemdrop.child("DropGroup"); dropgroup; dropgroup = dropgroup.next_sibling()) {
        ...
        if (tmpDropItemGroup.iDropRateDenominator <= 0) {
            g_Log.MsgBox("[IGC_AppointItemDrop.xml] Error - Denominator is ZERO (Group:%d)",
                         tmpDropItemGroup.iDropItemGroup);
            return;
        }
        ...
        for (pugi::xml_node item = dropgroup.child("Item"); item; item = item.next_sibling()) {
            ...
            if (ItemGetNumberMake(tmpDropItem.iType, tmpDropItem.iIndex) == -1) {
                g_Log.MsgBox("[ !! ERROR !! ]  at 'IGC_AppointItemDrop.xml' Wrong Item Index Found Type:%d, Index:%d",
                             tmpDropItem.iType, tmpDropItem.iIndex);
            }
        ...
        this->m_mapItemDropInfo.insert(
                std::pair<int, _ST_APP_DROP_ITEM_GROUP>(tmpDropItemGroup.iDropItemGroup, tmpDropItemGroup));
    }
...
BOOL CAppointItemDrop::AppointItemDrop(OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpMonsterObj) {
    ...
    std::map<int, _ST_APP_MONSTER_ITEMGROUP_LINK>::iterator iterMonsterItemGroupLink = this->m_mapMonsterItemGroupLink.find(
            lpMonsterObj->Class);
    ...
    std::map<int, _ST_APP_DROP_ITEM_GROUP>::iterator iterMonsterItemDropInfo = m_mapItemDropInfo.find(
            iterMonsterItemGroupLink->second.iDropItemGroup);
    ...
    for (std::vector<_ST_APPOINTITEMDROP_DROP_ITEM>::reverse_iterator iterDropItem = iterMonsterItemDropInfo->second.vecDropItem.rbegin();
         iterDropItem != iterMonsterItemDropInfo->second.vecDropItem.rend(); iterDropItem++) {
        ...
        if (iLeft <= iSelectedDropRate && iSelectedDropRate < iRight) {
            ...
            ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, iX, iY, iType, iLevel, iDur, iSkill, iLuck, iOption3,
                                 iMaxHitUser, byExOption, 0, 0, 0, 0);

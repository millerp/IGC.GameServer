...
bool CSkillUseTime::CheckSkillTime(LPOBJ lpObj, int iSkill) {
    ...
    for (std::vector<SKILL_TIME_INFO>::iterator It = this->m_vtSkillTimeInfo.begin();
         It != this->m_vtSkillTimeInfo.end(); It++) {
                    ...
                    if (this->m_iIsDC) {
                        if (lpObj->m_PlayerData->LastSkillUseCount >= this->m_iNumberOfBadSkillUseDC) {
                            g_Log.AddC(TColor::Red,
                                       "[ANTI-HACK] [%s][%s][%s] Used skill too fast %d times -> Disconnect",

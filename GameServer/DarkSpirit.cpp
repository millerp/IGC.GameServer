...
void CDarkSpirit::Run() {
    ...
    if (masteridx < g_ConfigRead.server.GetObjectStartUserIndex() ||
        masteridx > g_ConfigRead.server.GetObjectMax() - 1) {
...
void CDarkSpirit::ModeNormal() {
    ...
    if (this->m_iMasterIndex < g_ConfigRead.server.GetObjectStartUserIndex() ||
        this->m_iMasterIndex > g_ConfigRead.server.GetObjectMax() - 1) {
        return;
    }
            ...
            if (tObjNum >= 0) {
                ...
                if (gObj[tObjNum].Life > 0.0f && (gObj[tObjNum].Class < 100 || gObj[tObjNum].Class > 110)) {
                    if (lpObj->VpPlayer2[count].type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon < 0) {
                        ...
                        if (gObj[tObjNum].Type == OBJ_USER && g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE &&
                            g_GensSystem.IsPkEnable(lpObj, &gObj[lc85]) == TRUE) {
                            EnableAttack = TRUE;
                        }
...
void CDarkSpirit::ModeAttakTarget() {
    ...
    if (g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE &&
        g_GensSystem.IsPkEnable(lpObj, lpTargetObj) == FALSE) {
        return;
    }
        ...
        if (lpObj->VpPlayer2[count].state != 0) {
            ...
            if (tObjNum >= 0 && aTargetIndex != tObjNum) {
                ...
                if (lpObj->VpPlayer2[count].type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon < 0) {
                    ...
                    if (gObj[tObjNum].Type == OBJ_USER && g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE &&
                        g_GensSystem.IsPkEnable(lpObj, &gObj[tObjNum]) == TRUE) {
                        EnableAttack = TRUE;
                    }
...
void CDarkSpirit::Set(int aIndex, CItem *pPetItem) {
    ...
    g_ConfigRead.m_ItemCalcLua.Generic_Call("DarkSpirit_CalcValues", "ii>iiddiiii", leadership, petitemlevel,
                                            &this->m_AttackDamageMin, &this->m_AttackDamageMax, &this->m_CriticalDamage,
                                            &this->m_ExcellentDamage,
                                            &this->m_DoubleDamage, &this->m_IgnoreEnemy, &this->m_AttackSpeed,
                                            &this->m_SuccessAttackRate);
...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf *lpMagic, int iDamageType, int iActionType) {
    ...
    if (g_ConfigRead.pet.DamageDisableForPet[DAMAGE_OFF_RAVEN] == false) {
        ...
        lpObj->pInventory[1].m_DurabilitySmall +=
                idamage - (idamage * lpObj->m_PlayerData->m_MPSkillOpt.iMpsPetDurDownSpeed / 100);
    ...
    if (lpTargetObj->Live) {
        ...
        if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
            AttackDamage =
                    AttackDamage * g_ConfigRead.calc.PvPDamageRate_DarkSpirit[lpObj->Class][lpTargetObj->Class] / 100;
...
int CDarkSpirit::GetAttackDamage(LPOBJ lpObj, int targetDefense, int crititcaldamage) {
    ...
    if (GetItemKindA(lpObj->pInventory[0].m_Type) == ITEM_KIND_A_WEAPON &&
        GetItemKindB(lpObj->pInventory[0].m_Type) == ITEM_KIND_B_SCEPTER) {

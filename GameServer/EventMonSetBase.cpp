    ...
    for (pugi::xml_node spot = main.child("Spot"); spot; spot = spot.next_sibling()) {
        ...
        for (pugi::xml_node spawn = spot.child("Spawn"); spawn; spawn = spawn.next_sibling()) {
            ...
            for (int i = 0; i < iSpawnCount; i++) {
                ...
                this->m_mapEventSpotData.insert(std::pair < int, boost::shared_ptr <
                                                                 EVENT_MONSTER_POSITION >> (iTotalCount, lpEventPos));
                iTotalCount++;
            }
        }
    }
...
void CEventMonSetBase::Run() {
    ...
    for (std::map < int, boost::shared_ptr < EVENT_MONSTER_POSITION >> ::iterator It = this->m_mapEventSpotData.begin();
            It != this->m_mapEventSpotData.end();
    It++)
    {
        if (time.wDay >= It->second->m_StartDay && time.wDay <= It->second->m_EndDay &&
            time.wHour >= It->second->m_StartHour && time.wHour < It->second->m_EndHour) {
...
bool CEventMonSetBase::IsEventMonster(int iIndex) {
    std::map < int, boost::shared_ptr < EVENT_MONSTER_DATA >> ::iterator
    Iter = this->m_mapMonsterData.find(iIndex);
...
void CEventMonSetBase::SpawnMonster(boost::shared_ptr <EVENT_MONSTER_POSITION> lpSpotData) {
    ...
    if (this->SetPosMonster(result, lpSpotData->m_MapNumber, lpSpotData->m_X, lpSpotData->m_Y, lpSpotData->m_W,
                            lpSpotData->m_H, lpSpotData->m_Dir) == FALSE) {
        gObjDel(result);
        g_Log.Add("error : %s %d", __FILE__, __LINE__);
        return;
    }
    ...
    g_Log.Add("[Event Spawn] Set Monster (%d) (%d) (%d)(%d)(%d)", result, lpSpotData->m_Type, gObj[result].MapNumber,
              gObj[result].X, gObj[result].Y);
}
...
void CEventMonSetBase::RegenMonster(int iIndex) {
    ...
    if (this->GetBoxPosition(Iter->second->lpSpotData->m_MapNumber, Iter->second->lpSpotData->m_X,
                             Iter->second->lpSpotData->m_Y, Iter->second->lpSpotData->m_W,
                             Iter->second->lpSpotData->m_H,
...
void CEventMonSetBase::DeleteAllMonsters() {
    for (std::map < int, boost::shared_ptr < EVENT_MONSTER_DATA >> ::iterator It = this->m_mapMonsterData.begin(); It !=
                                                                                                                   this->m_mapMonsterData.end();
    It++)
    {
...
int
CEventMonSetBase::SetPosMonster(int aIndex, int nMapNumber, int nBeginX, int nBeginY, int nEndX, int nEndY, int nDir) {

...
void CIllusionTemple::ProcState_Closed(int IllusionTempleIndex) {
    ...
    if (TickCount >= 1000) // 1sekunda timera
    {
        ...
        if (gIllusionTempleEvent) {
            if (this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC <= (this->OpenTime * 60 * 1000) &&
            ...
            if (this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC <= (this->OpenTime * 60 * 1000) &&
                ...
                (this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC / 60000) !=
                this->m_IllusionTempleData[IllusionTempleIndex].Notify) {
                this->m_IllusionTempleData[IllusionTempleIndex].Notify =
                        this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC / 60000;
                if (IllusionTempleIndex == 0) {
                    PMSG_NOTICE pNotice;
                    TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0, 288),
                                             this->m_IllusionTempleData[IllusionTempleIndex].Notify + 1);
                    this->SendAllUserAnyMsg((LPBYTE) & pNotice, pNotice.h.size);
                }
            }
            if (this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC <= 30000 &&
                this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC > 0 &&
                this->m_IllusionTempleData[IllusionTempleIndex].BeforeEnter == false) {
        ...
        for (int i = 0; i < MAX_ILLUSION_TEMPLE_USER; ++i) {
            if (this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex != -1) {
                if (gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].MapNumber !=
                    IllusionTempleIndex + MAP_INDEX_ILLUSIONTEMPLE1) {
                    this->DeletePlayer(IllusionTempleIndex,
                                       this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex,
                                       true);
                }
            }
        }
        ...
        if (this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC <= 0 &&
            this->m_IllusionTempleData[IllusionTempleIndex].BeforeSemiPlay == false) {
            if (gIllusionTempleEvent) {
                this->m_IllusionTempleData[IllusionTempleIndex].Open = false;
    ...
    if (gIllusionTempleEvent && this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC <= 0 &&
        this->m_IllusionTempleData[IllusionTempleIndex].BeforeSemiPlay == true) {
...
void CIllusionTemple::CheckSync(int IllusionTempleIndex) {
    ...
    switch (Changed) {
        case 1:
            this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC = (((Hour * 60 * 60) + (Minute * 60)) -
                                                                          ((SysTime.wHour * 60 * 60) +
                                                                           (SysTime.wMinute * 60) +
                                                                           (SysTime.wSecond))) * 1000;
            break;
        case 2:
            this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC = ((((Hour + 24) * 60 * 60) + (Minute * 60)) -
                                                                          ((SysTime.wHour * 60 * 60) +
                                                                           (SysTime.wMinute * 60) +
                                                                           (SysTime.wSecond))) * 1000;
            break;
...
int CIllusionTemple::GetUserLevelRoom(int aIndex) {
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_LEVEL; ++i) {
        if (gObj[aIndex].ChangeUP == 2) {
            return 5;
        } else if (gObj[aIndex].Level >= g_IllusionLevel[i].MinLevel &&
                   gObj[aIndex].Level <= g_IllusionLevel[i].MaxLevel) {
            return i;
        }
    }
    return -1;
...
void CIllusionTemple::EnterToWaitingRoom(int aIndex, int IllusionTempleIndex, int ItemSerial, int ItemID, int ItemPos) {
    ...
    if (gObj[aIndex].m_cIllusionTempleSubIndex == -1) {
        ...
        LogAddC(2,
                "[Illusion Temple] (%d) [%s][%s] CIllusionTemple::EnterToWaitingRoom(): ERROR: m_cIllusionTempleSubIndex == -1",
...
void CIllusionTemple::SetTeams(int IllusionTempleIndex) {
    ...
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_USER; ++i) {
        if (gObjIsConnected(this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex)) {
            if (IT_MAP_RANGE(
                    gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].MapNumber)) {
                if (gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].m_cIllusionTempleIndex ==
                    IllusionTempleIndex) {
                    if (yellow == 5) {
            ...
            if (IT_MAP_RANGE(
                ...
                if (gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].m_cIllusionTempleIndex ==
                    ...
                    if (this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].Team == 0) {
                        ...
                        LogAddTD("[Illusion Temple] (%d) [%s][%s] - Allies Team", IllusionTempleIndex + 1,
                                 gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].AccountID,
                        ...
                        LogAddTD("[Illusion Temple] (%d) [%s][%s] - Illusions Team", IllusionTempleIndex + 1,
                                 gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].AccountID,
                                 gObj[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex].Name);
...
void CIllusionTemple::SetPartyTeam(int IllusionTempleIndex) {
    ...
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAMS; ++i) {
        PartyID = gParty.Create(this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[0].UserIndex,
                                gObj[this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[0].UserIndex].DBNumber,
        ...
        for (int n = 0; n < MAX_ILLUSION_TEMPLE_TEAM_USER; ++n) {
            if (i == 0) {
                if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex != -1) {
                    ...
                    gObjViewportListProtocolCreate(
                            &gObj[this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex]);
                }
            } else if (i == 1) {
                if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex != -1) {
...
void CIllusionTemple::MoveToBattleArea(int IllusionTempleIndex) {
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_USER; ++i) {
        if (this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex != -1) {
            if (this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].Team == 0) {
                gObjMoveGate(this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex,
                             g_IllusionGatesYellow[IllusionTempleIndex].Gate);
            } else {
                gObjMoveGate(this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex,
                             g_IllusionGatesBlue[IllusionTempleIndex].Gate);
            }
        }
    }
}
...
void CIllusionTemple::SetKillPoint(int IllusionTempleIndex, bool ForAll, int aIndex, int KillPoint) {
    ...
    if (ForAll) {
        for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAMS; ++i) {
            for (int n = 0; n < MAX_ILLUSION_TEMPLE_TEAM_USER; ++n) {
                if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex != -1) {
                    pPoint.KillPoint = KillPoint -
                                       gObj[this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex].m_btKillCount;
                    gObj[this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex].m_btKillCount = KillPoint;
                    IOCP.DataSend(this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex,
                                  (LPBYTE) & pPoint, pPoint.h.size);
                }
            }
        }
        return;
...
void CIllusionTemple::ProcState_Playing(int IllusionTempleIndex) {
    ...
    if (Tick >= 1000) {
        ...
        if (this->m_IllusionTempleData[IllusionTempleIndex].RemainMSEC <= 0 &&
            ...
            this->m_IllusionTempleData[IllusionTempleIndex].BeforeSemiPlayEnd == false) {
        ...
        if (this->m_IllusionTempleData[IllusionTempleIndex].PlayStarted == true &&
            this->m_IllusionTempleData[IllusionTempleIndex].BeforeSemiPlayEnd == false) {
            int aIndex;
            for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAMS; ++i) {
                for (int n = 0; n < MAX_ILLUSION_TEMPLE_TEAM_USER; ++n) {
                    if ((aIndex = this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex) !=
                        -1) {
            ...
            if (this->m_IllusionTempleData[IllusionTempleIndex].HeroIndex == -1 ||
                !gObjIsConnected(this->m_IllusionTempleData[IllusionTempleIndex].HeroIndex)) {
                ...
                if (this->m_IllusionTempleData[IllusionTempleIndex].StatueIndex[0] != -1) {
                    if (gObjIsConnected(this->m_IllusionTempleData[IllusionTempleIndex].StatueIndex[0]) == false ||
                        ...
                        if (this->m_IllusionTempleData[IllusionTempleIndex].StatueIndex[1] != -1) {
                            if (gObjIsConnected(this->m_IllusionTempleData[IllusionTempleIndex].StatueIndex[1]) ==
                                false ||
                ...
                for (int i = 0; i < 300; ++i) {
                    if (MapC[MAP_INDEX_ILLUSIONTEMPLE1 + IllusionTempleIndex].m_cItem[i].m_Number ==
                        this->m_IllusionTempleData[IllusionTempleIndex].SacredItemSerial &&
            ...
            if (WinTeam >= 0 || WinTeam == -2) {
                this->m_IllusionTempleData[IllusionTempleIndex].PlayStarted = false;
                this->m_IllusionTempleData[IllusionTempleIndex].WinTeam = this->CheckWinnerTeam(IllusionTempleIndex,
                                                                                                true);
...
void CIllusionTemple::MinimapProcess(int IllusionTempleIndex, int IllusionTempleSubIndex, char Team, int aIndex) {
    ...
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAM_USER; ++i) {
        if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team].TeamMate[i].UserIndex != -1 &&
            this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team].TeamMate[i].UserIndex != aIndex) {
...
int CIllusionTemple::GetCurPlayUser(int IllusionTempleIndex) {
    ...
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAMS; ++i) {
        for (int n = 0; n < MAX_ILLUSION_TEMPLE_TEAM_USER; ++n) {
            if (gObjIsConnected(this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex) ==
                true &&
                gObj[this->m_IllusionTempleData[IllusionTempleIndex].Teams[i].TeamMate[n].UserIndex].MapNumber ==
                MAP_INDEX_ILLUSIONTEMPLE1 + IllusionTempleIndex) {
                cnt++;
            }
        }
    }
...
void CIllusionTemple::CreateStatues(int IllusionTempleIndex) {
    ...
    if (result >= 0) {
        if (Statue % 2 == 0) {
            ...
            gObjSetMonster(result,
                           gMSetBase.m_Mp[this->m_IllusionTempleData[IllusionTempleIndex].StatuePosNum[0]].m_Type);
            gObj[result].DieRegen = 0;
            gObj[result].Live = 1;
            ...
            gObjSetMonster(result,
                           gMSetBase.m_Mp[this->m_IllusionTempleData[IllusionTempleIndex].StatuePosNum[1]].m_Type);
            gObj[result].DieRegen = 0;
            gObj[result].Live = 1;
            gObj[result].m_PosNum = -1;
            gObj[result].m_cIllusionTempleIndex = IllusionTempleIndex;
...
void CIllusionTemple::DropSacredItem(int IllusionTempleIndex, LPOBJ Statue, LPOBJ User) {
    ...
    LogAddTD("[Illusion Temple] (%d) (%s)(%s) dropped Cursed Castle Water", IllusionTempleIndex + 1, User->AccountID,
             User->Name);
            ...
            if (User->pInventory[i].m_Type == ITEMGET(14, 64)) {
                if (User->pInventory[i].m_Number == this->m_IllusionTempleData[IllusionTempleIndex].SacredItemSerial) {
                    if (User->m_Index == this->m_IllusionTempleData[IllusionTempleIndex].HeroIndex) {
                        ...
                        if (Statue->Class == 383) {
                            LogAddTD(
                                    "[Illusion Temple] (%d) (%s)(%s) Scored a point for Allies. A:[%d] I:[%d] UserPoint:[%d]",
                                    IllusionTempleIndex + 1, User->AccountID, User->Name,
                        ...
                        if (Statue->Class == 384) {
                            LogAddTD(
                                    "[Illusion Temple] (%d) (%s)(%s) Scored a point for Illusions. A:[%d] I:[%d] UserPoint:[%d]",
                                    IllusionTempleIndex + 1, User->AccountID, User->Name,
                                    this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Score,
                                    this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].Score,
                                    this->m_IllusionTempleData[IllusionTempleIndex].Teams[User->m_cIllusionTempleTeam].TeamMate[User->m_cIllusionTempleTeamIndex].Score);
    ...
    for (int i = 12; i < 108; ++i) {
        if (gObj[aIndex].pInventory[i].IsItem() == true) {
            if (gObj[aIndex].pInventory[i].m_Type == ITEMGET(14, 64)) {
                if (gObj[aIndex].pInventory[i].m_Number ==
                    this->m_IllusionTempleData[IllusionTempleIndex].SacredItemSerial) {
                    pMsg.Ipos = i;
                    break;
                }
...
int CIllusionTemple::CheckWinnerTeam(int IllusionTempleIndex, bool EndTime) {
    ...
    if ((this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Count <= 0 ||
        ...
        if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Score >= 2 ||
            ...
            if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Score >
                this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].Score) {
                return 0;
            } else {
                return 1;
...
void CIllusionTemple::RankingInfo(int IllusionTempleIndex) {
    ...
    for (int i = 0; i < this->m_IllusionTempleData[IllusionTempleIndex].UserCount; ++i) {
        pData = (PMSG_ILLUSION_STATISTICS_DATA * )(
                buf + sizeof(PMSG_ILLUSION_STATISTICS) + (i * sizeof(PMSG_ILLUSION_STATISTICS_DATA)));
...
void CIllusionTemple::TeleportToSecureArea(int IllusionTempleIndex) {
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAM_USER; ++i) {
        if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].TeamMate[i].UserIndex != -1) {
            gObjMoveGate(this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].TeamMate[i].UserIndex,
                         g_IllusionGatesYellow[IllusionTempleIndex].Gate);
        }
    }
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAM_USER; ++i) {
        if (this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].TeamMate[i].UserIndex != -1) {
            gObjMoveGate(this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].TeamMate[i].UserIndex,
                         g_IllusionGatesBlue[IllusionTempleIndex].Gate);
        }
    }
}
...
void CIllusionTemple::SetState_PlayEnd(int IllusionTempleIndex) {
    ...
    for (int i = 12; i < 108; ++i) {
        if (this->m_IllusionTempleData[IllusionTempleIndex].HeroIndex != -1) {
            if (gObj[this->m_IllusionTempleData[IllusionTempleIndex].HeroIndex].pInventory[i].m_Number ==
                this->m_IllusionTempleData[IllusionTempleIndex].SacredItemSerial) {
    ...
    if (this->m_IllusionTempleData[IllusionTempleIndex].WinTeam == 0) {
        LogAddTD("[Illusion Temple] (%d) Winner Team: Allies [%d]:[%d]", IllusionTempleIndex + 1,
                 this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Score,
                 this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].Score);
    } else if (this->m_IllusionTempleData[IllusionTempleIndex].WinTeam == 1) {
        LogAddTD("[Illusion Temple] (%d) Winner Team: Illusions [%d]:[%d]", IllusionTempleIndex + 1,
                 this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Score,
                 this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].Score);
    } else {
        LogAddTD("[Illusion Temple] (%d) It's a Tie [%d]:[%d]", IllusionTempleIndex + 1,
                 this->m_IllusionTempleData[IllusionTempleIndex].Teams[0].Score,
                 this->m_IllusionTempleData[IllusionTempleIndex].Teams[1].Score);
    }
...
int CIllusionTemple::GetViewportState() {
    ...
    if (this->m_IllusionTempleData[0].State == 1 &&
        this->m_IllusionTempleData[0].RemainMSEC > this->OpenTime * 60 * 1000) {
        return 1;
    } else if (this->m_IllusionTempleData[0].State == 1 &&
               this->m_IllusionTempleData[0].RemainMSEC <= this->OpenTime * 60 * 1000) {
        return 2;
    }
    ...
    if (WaitingRoom) {
        for (int i = 0; i < MAX_ILLUSION_TEMPLE_USER; ++i) {
            if (this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex == aIndex) {
                InterlockedExchange((LPLONG) & this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].Team,
                                    -1);
                InterlockedExchange(
                        (LPLONG) & this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex, -1);
        ...
        if (Team == -1) {
            LogAddC(2, "[Illusion Temple] (%d) CIllusionTemple::DeletePlayer ERROR: Team == -1",
...
int CIllusionTemple::CalcExp(int IllusionTempleIndex, int Team, int TeamIndex) {
    ...
    if (this->m_IllusionTempleData[IllusionTempleIndex].WinTeam == Team) {
        exp = (g_IT_Exp[IllusionTempleIndex].Exp + ((this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team].Score -
                                                     this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team == 0 ? 1
                                                                                                                     : 0].Score) *
                                                    g_IT_PointExp[IllusionTempleIndex].Exp)) * 2;
    } else {
        exp = (g_IT_Exp[IllusionTempleIndex].Exp + (this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team].Score *
                                                    g_IT_PointExp[IllusionTempleIndex].Exp));
    }

    exp += (this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team].TeamMate[TeamIndex].KillCount *
            g_IT_KillExp[IllusionTempleIndex].Exp);
    exp += (this->m_IllusionTempleData[IllusionTempleIndex].Teams[Team].TeamMate[TeamIndex].Score *
            g_IT_BallExp[IllusionTempleIndex].Exp);
...
int CIllusionTemple::GiveRewardExp(int IllusionTempleIndex) {
    ...
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_TEAM_USER; ++i) {
        ...
        int iEXP = this->CalcExp(IllusionTempleIndex, gObj[iUserIndex].m_cIllusionTempleTeam,
                                 gObj[iUserIndex].m_cIllusionTempleTeamIndex);
...
int CIllusionTemple::LevelUp(int UserIndex, int Exp, int Event) {
    ...
    if ((gObj[UserIndex].Experience + Exp) < gObj[UserIndex].NextExp) {
        ...
        if (gObj[UserIndex].Class == CLASS_DARKLORD || gObj[UserIndex].Class == CLASS_MAGUMSA ||
            gObj[UserIndex].Class == CLASS_RAGEFIGHTER) {
...
void CIllusionTemple::DeleteInvitation(int IllusionTempleIndex) {
    for (int i = 0; i < MAX_ILLUSION_TEMPLE_USER; ++i) {
        ...
        if (Item.m_Type == ITEMGET(13, 51)) {
            gObjInventoryDeleteItem(this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex,
                                    this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].ItemPos);
            GSProtocol.GCInventoryItemDeleteSend(
                    this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[i].UserIndex,
...
bool CIllusionTemple::InvitationUsedCheck(LPOBJ lpObj, int ItemPos) {
    ...
    if (this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[IllusionTempleSubIndex].UserIndex ==
        lpObj->m_Index) {
...
void CIllusionTemple::Payback(int aIndex) {
    ...
    GSProtocol.GCItemDurSend2(aIndex,
                              this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[IllusionTempleSubIndex].ItemPos,
                              gObj[aIndex].pInventory[this->m_IllusionTempleData[IllusionTempleIndex].WaitingPlayers[IllusionTempleSubIndex].ItemPos].m_Durability,
                              0);
}

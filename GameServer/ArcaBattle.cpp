    ...
    for (int i = 0; i < 5; i++) {
        if (i != this->m_iSkipObelisk) {
            ...
            g_Log.Add("[ArcaBattle] Send ObeliskInfo iGroupNumber %d iAttr %d iMapIndex %d iPosX %d iPosY %d",
                      ...
                      pMsg.m_stObeliskInfo[i].iMapIndex,
    ...
    for (int i = 0; i < 5; i++) {
        if (this->m_stObeliskState[i].m_iObeliskState == 1) {
            ...
            if (lpGuild) {
                ...
                g_Log.Add("[ArcaBattle] Reward GuildBuff GuildName[%s] BuffIndex[%d]",
                          this->m_stObeliskState[i].m_szOccupyGuildName, wBuffIndex);
            } else {
                g_Log.Add(
                        "[ArcaBattle][Error] GiveRewardBuffWinningGuild() GuildInfo is NULL GuildName : %s ObeliskAttr : %d ObeliskGroup %d",
                        this->m_stObeliskState[i].m_szOccupyGuildName, this->m_stObeliskInfo[i].m_iAttrKind,
                        this->m_stObeliskInfo[i].m_iGroupNumber);
            }
        }
    }
}
...
void CArcaBattle::DGAnsRemoveAllRewardBuff() {
    for (int i = 0; i < this->m_iWinGuildCnt; i++) {
        ...
        g_Log.Add("[ArcaBattle] Remove All Reward Guild Buff GuildName[%s] BuffIndex[%d]",
                  this->m_stABWinGuildInfo[i].szGuildName, wBuffIndex);
        this->m_stABWinGuildInfo[i].Clear();
    }
}
        ...
        if (lpGuild) {
            for (int nGuildCnt = 0; nGuildCnt < lpGuild->Count; nGuildCnt++) {
                if (gObjIsConnected(lpGuild->Index[nGuildCnt]) == TRUE) {
                    this->WinGuildMemberAddBuff(&gObj[lpGuild->Index[nGuildCnt]],
                                                this->m_stABWinGuildInfo[i].wOccupyObelisk);
                }
            }
        }
...
BOOL CArcaBattle::IsArcaBattleOccupyZone(OBJECTSTRUCT *lpObj, int gt) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if ((gt == 418 && this->m_stABWinGuildInfo[i].wObeliskGroup == 1) ||
            (gt == 422 && this->m_stABWinGuildInfo[i].wObeliskGroup == 2) ||
...
int CArcaBattle::CheckSync() {
    ...
    if (wDayOfWeek < 7) {
        if (this->m_iAB_DayOfWeekCycle[wDayOfWeek] == TRUE) {
            ...
            for (std::vector<_tagARCA_BATTLE_START_TIME>::iterator It = this->m_vtArcaBattleOpenTime.begin();
                 It != this->m_vtArcaBattleOpenTime.end(); It++) {
                pRET = *It;
            ...
            if (bTIME_CHANGED == TRUE) {
                this->m_i64_AB_REMAIN_MSEC = 1000 * (60 * iMIN_MINUTE + 3600 * iMIN_HOUR -
                                                     (systime.wSecond + 60 * systime.wMinute + 3600 * systime.wHour));
                this->SetState(2);
                this->GDReqArcaBattleProcMultiCast(2);
                this->m_i64_AB_TICK_COUNT = GetTickCount64();

                g_Log.Add("[ArcaBattle] Sync Start Time. [%I64d] min remain (START HOUR:%d, MIN:%d)",
                          this->m_i64_AB_REMAIN_MSEC / 60000,
                ...
                g_Log.AddC(TColor::Red, "[ArcaBattle] No Schedule Time Data - Arca War Terminated (bTIME_CHANGED=%d)",
                           bTIME_CHANGED);
                this->SetState(0);
                this->GDReqArcaBattleProcMultiCast(0);
                return false;
            ...
            if (GetTickCount64() - this->m_i64_AB_TICK_COUNT_2 > 60000) {
                ...
                if (this->m_i64_AB_REMAIN_MSEC > 900000) {
                    if (this->m_i64_AB_REMAIN_MSEC > 900000 && !((iAB_REMAIN_MIN + 1) % 5)) {
                        ...
                        g_Log.AddC(TColor::Blue, "[ArcaBattle] [State Notify] STATE: MASTER_REG (Left Time:%d)",
                                   this->m_iAB_NOTIFY_COUNT + 1);
                    }
                } else {
                    memset(szMsg, 0x00, sizeof(szMsg));
    ...
    if (i64TICK_MSEC >= 1000) {
        ...
        if (this->m_i64_AB_REMAIN_MSEC <= 60000 * this->m_iAB_RegTimeMin) {
            if (GetTickCount64() - this->m_i64_AB_TICK_COUNT_2 > 60000) {
                ...
                g_Log.Add("[ArcaBattle] Registration to Arca War of guild members is open for %d minutes",
                          this->m_iAB_NOTIFY_COUNT + 1);

                if (this->m_iAB_NOTIFY_COUNT == this->m_iAB_RegTimeMin - 1 ||
                    this->m_iAB_NOTIFY_COUNT == (this->m_iAB_RegTimeMin / 2) - 1) {
...
void CArcaBattle::ProcStateReady() {
    ...
    if (i64TICK_MSEC >= 1000) {
        ...
        if (this->m_iAB_NOTIFY_COUNT > 0) {
            ...
            g_Log.Add("[ArcaBattle] %d remaining seconds to warp to the Arca War Please wait in a safe area.",
                      this->m_iAB_NOTIFY_COUNT);
        }
...
void CArcaBattle::ProcStatePaty() {
    ...
    if (i64TICK_MSEC >= 1000) {
        ...
        if (this->m_i64_AB_REMAIN_MSEC <= 60000 * this->m_iAB_WaitPartyTimeMin &&
            GetTickCount64() - this->m_i64_AB_TICK_COUNT_2 > 60000) {
            this->m_i64_AB_TICK_COUNT_2 = GetTickCount64();
            this->m_iAB_NOTIFY_COUNT = this->m_i64_AB_REMAIN_MSEC / 60000;
...
void CArcaBattle::ProcStatePlaying() {
    ...
    if (i64TICK_MSEC >= 1000) {
        ...
        if (this->m_i64_AB_REMAIN_MSEC >= 60000 * (this->m_iAB_PlayTimeMin - this->m_iAB_PlayNotifyTimeMin) &&
            GetTickCount64() - this->m_i64_AB_TICK_COUNT_SEC >= 70000) {
            this->m_i64_AB_TICK_COUNT_SEC = GetTickCount64();
            this->GCArcaBattleSendNotiMsg(0, 0, 0);
        }

        if (this->m_i64_AB_REMAIN_MSEC <= 60000 * this->m_iAB_PlayTimeMin &&
            GetTickCount64() - this->m_i64_AB_TICK_COUNT_2 >= 60000) {
            this->m_i64_AB_TICK_COUNT_2 = GetTickCount64();
            this->m_iAB_NOTIFY_COUNT = this->m_i64_AB_REMAIN_MSEC / 60000;
...
void CArcaBattle::ProcStateResult() {
    ...
    if (i64TICK_MSEC >= 1000) {
        ...
        if (this->m_i64_AB_REMAIN_MSEC > 0) {
            ...
            g_Log.Add("[ArcaBattle] Arca War has ended after %d seconds users will be warped to village.",
                      this->m_iAB_NOTIFY_COUNT + 1);
        }
        ...
        if (this->m_i64_AB_REMAIN_MSEC <= 0) {
            ...
            int iMapNumber = gGateC.GetGate(426, (short &) sX, (short &) sY, (BYTE &) btMAPNUM, (BYTE &) btDIR,
                                            (short &) sLVL);

            if (iMapNumber >= 0) {
                for (int n = g_ConfigRead.server.GetObjectStartUserIndex();
                     n < g_ConfigRead.server.GetObjectMax(); ++n) {
...
void CArcaBattle::ProcStateChannelClose() {
    ...
    if (i64TICK_MSEC >= 1000) {
        ...
        if (this->m_i64_AB_REMAIN_MSEC <= 60000 * this->m_iAB_ChannelTimeMin &&
            GetTickCount64() - this->m_i64_AB_TICK_COUNT_2 >= 60000) {
            this->m_i64_AB_TICK_COUNT_2 = GetTickCount64();
            this->m_iAB_NOTIFY_COUNT = this->m_i64_AB_REMAIN_MSEC / 60000;
...
BOOL CArcaBattle::EnterArcaBattleEvent(int aIndex) {
    ...
    for (int i = 0; i < this->m_iCurUserCount; i++) {
        if (strcmp(this->m_UserData[i].szCharName, gObj[aIndex].Name) == 0) {
            ...
            g_Log.Add("[ArcaBattle] Entered ArcaBattle [%s] [%s] UserCnt [%d]", gObj[aIndex].AccountID,
                      gObj[aIndex].Name, this->m_iCurUserCount);
    ...
    g_Log.Add("[ArcaBattle] Enter ArcaBattle [%s] [%s] UserCnt [%d]", gObj[aIndex].AccountID, gObj[aIndex].Name,
              this->m_iCurUserCount);
    this->GCArcaBattleUserInfo(aIndex);
...
void CArcaBattle::SendObeliskState() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (this->m_stObeliskState[i].m_iMonIndex >= 0) {
            ...
            pMsg.btObeliskAttribute = (BYTE)
            this->m_stObeliskInfo[i].m_iAttrKind;
            pMsg.btAuraState = 2;
                ...
                for (int j = 0; j < 3; j++) {
                    if (this->m_stObeliskInfo[i].m_stAuraInfo[j].m_iAuraPosX == lpObj->X &&
                        ...
                        g_Log.Add("[ArcaBattle] Obelisk foot hold [on][%s][%s][%s] : Pos X[%d] Y[%d]",
                                  lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->lpGuild->Name, lpObj->X,
                                  lpObj->Y);
                    }
                }
            }
    ...
    for (int i = 0; i < this->m_iObeliskCnt; ++i) {
        ...
        if (iAurReleaseCnt == 3) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                if (gObj[iHoverUserIndex_1].m_PlayerData->GuildNumber !=
                    gObj[iHoverUserIndex_2].m_PlayerData->GuildNumber)
            ...
            if (this->m_stObeliskState[i].m_iAuraState == 2) {
                ...
                if (this->m_stObeliskState[i].m_iAuraReleaseGuildNum != gObj[iUserIndex].m_PlayerData->GuildNumber &&
                    ...
                    _GUILD_INFO_STRUCT *lpGuild = Guild.SearchGuild_Number(
                            this->m_stObeliskState[i].m_iAuraReleaseGuildNum);

                    if (!lpGuild) {
                        g_Log.Add(
                                "[ArcaBattle] Error Can Not Find Guild ChkAuraUserHover() lpGuild NULL GuildNumber : %d",
                                this->m_stObeliskState[i].m_iAuraReleaseGuildNum);
                        return;
                    }
                    this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                                    this->m_stObeliskInfo[i].m_iAttrKind, ARCA_BATTLE_OBELISK_CAPTURING,
                                                    this->m_stObeliskState[i].m_iAuraReleaseGuildNum, lpGuild->Name, 0);
            ...
            if (iAurReleaseCnt > 0) {
                ...
                if (this->m_stObeliskState[i].m_iAuraReleaseGuildNum == -1 ||
                    this->m_stObeliskState[i].m_iObeliskState == 1) {
                    if (this->m_stObeliskState[i].m_iAuraReleaseGuildNum != -1 &&
                        this->m_stObeliskState[i].m_iObeliskState == 1) {
                        this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                                        this->m_stObeliskInfo[i].m_iAttrKind,
                                                        ARCA_BATTLE_OBELISK_CAPTURED,
                                                        this->m_stObeliskState[i].m_iOccupyGuildNum,
                                                        this->m_stObeliskState[i].m_szOccupyGuildName, 0);
                        this->m_stObeliskState[i].m_iAuraReleaseGuildNum = -1;
                    }
                } else {
                    this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                                    this->m_stObeliskInfo[i].m_iAttrKind,
                                                    ARCA_BATTLE_OBELISK_UNCAPTURED, 0, NULL, 0);
                    this->m_stObeliskState[i].m_iAuraReleaseGuildNum = -1;
                }
            }
        }
            ...
            for (int k = 0; k < MAX_VIEWPORT; ++k) {
                if (lpObj->VpPlayer2[k].state == 1 && lpObj->VpPlayer2[k].type == OBJ_USER) {
                    ...
                    for (int j = 0; j < 3; j++) {
                        ...
                        if (gObj[this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex].Type == OBJ_USER &&
                            ObjectMaxRange(this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex)) {
                            if (this->m_stObeliskState[i].m_stAuraState[j].m_btAuraState != 0 &&
                                (this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex != 0 ||
                                 gObj[iTarObjIndex].m_PlayerData->GuildNumber !=
                                 gObj[this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex].m_PlayerData->GuildNumber)) {
...
int CArcaBattle::IsOccupyObelisk(LPOBJ lpObj, int iMonIndex) {
    ...
    for (int i = 0; i < this->m_iObeliskCnt; i++) {
        ...
        if (iMonIndex == this->m_stObeliskInfo[i].m_iMonsterNum) {
            ...
            this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                            this->m_stObeliskInfo[i].m_iAttrKind, ARCA_BATTLE_OBELISK_CAPTURED,
                                            lpObj->m_PlayerData->GuildNumber, lpObj->m_PlayerData->lpGuild->Name, 0);
            this->m_stObeliskState[i].m_iObeliskState = 1;
            ...
            g_Log.Add("[ArcaBattle] Occupy Obelisk In ArcaBattle Progress  [%s][%s][%s] ObeliskGroup:[%d]",
                      lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->GuildName,
                      this->m_stObeliskState[i].m_iGroupNumber);
...
void CArcaBattle::GDReqWinGuildInfoInsert() {
    ...
    for (int i = 0; i < 5; i++) {
        if (this->m_stObeliskState[i].m_iObeliskState == 1) {
            ...
            memcpy(pMsg.m_stABWinGuildInfoDS[iGuildCnt].szGuildName, this->m_stObeliskState[i].m_szOccupyGuildName,
                   MAX_GUILD_LEN + 1);

            g_Log.Add(
                    "[ArcaBattle] Request Win Guild Info Insert GuildName : %s(%d) OccupyObelisk : %d ObeliskGroup : %d",
                    pMsg.m_stABWinGuildInfoDS[iGuildCnt].szGuildName, pMsg.m_stABWinGuildInfoDS[iGuildCnt].dwGuild,
                    pMsg.m_stABWinGuildInfoDS[iGuildCnt].wOccupyObelisk,
                    pMsg.m_stABWinGuildInfoDS[iGuildCnt].wObeliskGroup);
...
void CArcaBattle::SendPlayResult() {
    ...
    for (int i = 0; i < 5; i++) {
        if (this->m_stObeliskState[i].m_iGroupNumber != -1) {
            ...
            memcpy(pMsg.m_stABObeliskOccupy[iCnt].szGuildName, this->m_stObeliskState[i].m_szOccupyGuildName,
                   MAX_GUILD_LEN + 1);

            g_Log.Add("[ArcaBattle] SendPlayResult Occupy Guild Name : [%s] GroupNumber [%d] ObeliskAttribute [%d]",
    ...
    for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
        ...
        if (gObjIsConnected(n) == TRUE && lpObj->Type == OBJ_USER) {
            if (!lpObj->m_PlayerData->lpGuild) {
                g_Log.Add("[ArcaBattle] Error SendPlayResult() lpObj->lpGuild NULL %s %s", lpObj->AccountID,
                          lpObj->Name);
                continue;
            }
            ...
            if (!pUserInfo) {
                g_Log.Add("[ArcaBattle] Error SendPlayResult() pUserInfo NULL %s %s GuildName %s", lpObj->AccountID,
                          lpObj->Name, lpObj->m_PlayerData->GuildName);
                continue;
            }
            ...
            g_Log.Add(
                    "[ArcaBattle] SendPlayResult [%s][%s] BootyCnt[%d] ContributePoint[%d] KillPoint[%d] RewardExp[%d]",
                    gObj[n].AccountID, gObj[n].Name, pMsg.wBootyCnt, pMsg.dwContributePoint, pMsg.dwKillPoint,
                    iRewardExp);
            IOCP.DataSend(lpObj->m_Index, (LPBYTE) & pMsg, pMsg.h.size);
        }
    }
...
void CArcaBattle::RegenMonsterRun() {
    ...
    for (int i = 0; i < 50 && this->m_stMonsterGroupInfo[i].m_nGroupNumber > -1; ++i) {
        ...
        if (i64Tick >= this->m_stMonsterGroupInfo[i].m_i64NextRegenTime) {
            ...
            this->RegenMonster(i);
            this->m_stMonsterGroupInfo[i].m_i64NextRegenTime =
                    GetTickCount64() + 60000 * this->m_stMonsterGroupInfo[i].m_nRegenTimeMin;
        }
    }
}
    ...
    for (int i = 0; i < 255; i++) {
        if (this->m_stMonsterGroupInfo[nGroupNumber].m_stMonsterAccountNumInfo[i].m_nIndex > -1) {
            ...
            if (gObj[nIndex].Live == TRUE) {
                gObjDel(nIndex);
                g_Log.Add("[ArcaBattle][MonsterGroupRegen] Delete Monster - %d ",
...
void CArcaBattle::RegenMonster(int nGroupNumber) {
    ...
    for (int n = 0; n < this->m_stMonsterGroupInfo[nGroupNumber].m_nCnt &&
                    this->m_stMonsterGroupInfo[nGroupNumber].m_nMapNumber != -1; n++) {
        ...
        if (nIndex <= -1 || gObj[nIndex].Live == FALSE) {
            ...
            int nMonsterIndex = this->m_stMonsterPosition[nGroupNumber].m_stMonsterIndexInfo[rand() %
                                                                                             this->m_stMonsterPosition[nGroupNumber].m_nMonsterKindCnt].m_nMonsterIndex;
            this->m_stMonsterGroupInfo[nGroupNumber].m_stMonsterAccountNumInfo[n].m_nIndex = nResult;

            if (nResult < 0) {
                g_Log.Add("[ArcaBattle][MonsterGroupRegen] AddMonster fail!! [GROUP]:%d, [INDEX]:%d", nGroupNumber,
                          nMonsterIndex);
                return;
            }

            if (this->SetPosMonster(nResult, this->m_stMonsterGroupInfo[nGroupNumber].m_nMapNumber,
                                    this->m_stMonsterPosition[nGroupNumber].m_stRegenArea[nRegenArea].m_nBeginX,
                                    this->m_stMonsterPosition[nGroupNumber].m_stRegenArea[nRegenArea].m_nBeginY,
                                    this->m_stMonsterPosition[nGroupNumber].m_stRegenArea[nRegenArea].m_nEndX,
                                    this->m_stMonsterPosition[nGroupNumber].m_stRegenArea[nRegenArea].m_nEndY) ==
                FALSE) {
                gObjDel(nResult);
            ...
            g_Log.Add("[ArcaBattle][MonsterGroupRegen] Group: %d, MonsterIndex: %d, MapNumber: %d, X: %d, Y: %d ",
                      nGroupNumber, nMonsterIndex, this->m_stMonsterGroupInfo[nGroupNumber].m_nMapNumber,
...
void CArcaBattle::AddContributePoint(OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpTargetObj) {
    ...
    if (lpTargetObj->Type == OBJ_USER &&
        (lpTargetObj->Class == 592 || lpTargetObj->Class == 593 || lpTargetObj->Class == 594 ||
         lpTargetObj->Class == 595 || lpTargetObj->Class == 596 || lpTargetObj->Class == 611 ||
         lpTargetObj->Class == 612 || lpTargetObj->Class == 613 || lpTargetObj->Class == 614 ||
         lpTargetObj->Class == 615)) {
    ...
    g_Log.Add("[ArcaBattle] [%s][%s] User Kill ContributePoint[%d] - Result : Lv [%d] / Lv [%d][%s][%s]",
              lpObj->AccountID, lpObj->Name, pUserInfo->m_stAcquiredPoints.dwContributePoints, lpObj->Level,
              lpTargetObj->Level, lpTargetObj->AccountID, lpTargetObj->Name);
}
...
void CArcaBattle::AddKillPoint(OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpTargetObj) {
    ...
    if (lpTargetObj->Type == OBJ_USER) {
        ...
        if (pTargetUserInfo && pTargetUserInfo->bBootyDrop == false) {
            ...
            ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(13, 147), 0, 1, 0, 0, 0,
                                 -1, 0, 0, 0, 0, 0);
        ...
        if (lpTargetObj->Class == 598 || lpTargetObj->Class == 599 || lpTargetObj->Class == 600 ||
            lpTargetObj->Class == 601 || lpTargetObj->Class == 602) {
                ...
                for (int j = 0; j < 255; ++j) {
                    if (lpTargetObj->m_Index == this->m_stMonsterGroupInfo[i].m_stMonsterAccountNumInfo[j].m_nIndex) {
                        if (this->m_stMonsterGroupInfo[i].m_stMonsterAccountNumInfo[j].bItemDrop == false) {
                            ...
                            ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(13, 147),
...
void CArcaBattle::BootyItemGetCnt(OBJECTSTRUCT *lpObj) {
    ...
    if (pUserInfo) {
        ...
        g_Log.Add("[ArcaBattle] BootyItemGetCnt [%s][%s] Total BootyCnt[%d]", lpObj->AccountID, lpObj->Name,
                  pUserInfo->m_stAcquiredPoints.dwBootyCnt);
    }
}
        ...
        if (lpGuild) {
            for (int n = 0; n < 80; n++) {
                if (gObjIsConnected(lpGuild->Index[n]) == TRUE) {
                    ...
                    g_Log.Add("[ArcaBattle] Canceled With Less Than Guild Member - GuildName[%s]",
                              lpMsg->CancelGuildNames[i].szGuildNames);
                }
            }
        }
    }
...
bool CArcaBattle::CGReqMarkReg(int iIndex) {
    ...
    if (!lpGuildInfo) {
        g_Log.Add("[ArcaBattle] [%s][%s] error-L3 : ArcaBattle Mark Reg GuildInfo is NULL", gObj[iIndex].AccountID,
                  gObj[iIndex].Name);
    ...
    if (lpObj->m_PlayerData->GuildStatus != G_MASTER && lpObj->m_PlayerData->GuildStatus != G_SUB_MASTER &&
        lpObj->m_PlayerData->GuildStatus != G_BATTLE_MASTER) {
        this->GCAnsMarkRegErrCode(15, iIndex);
        return false;
    }
...
void CArcaBattle::GCAnsMarkRegErrCode(int iResult, int iIndex) {
    switch (iResult) {
        case 5:
            g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle MarkReg Fail to Time Over", gObj[iIndex].AccountID,
                      gObj[iIndex].Name);
            break;
        case 15:
            g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle MarkReg Fail to No Registration Authority",
                      gObj[iIndex].AccountID, gObj[iIndex].Name);
            break;
        case 16:
            g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle MarkReg Fail to Below Guild Member", gObj[iIndex].AccountID,
                      gObj[iIndex].Name);
            break;
        case 17:
            g_Log.Add("[ArcaBattle] [%s][%s] ArcaBattle MarkReg Fail to Guild Is More than 250 Registered",
                      gObj[iIndex].AccountID, gObj[iIndex].Name);
            break;
    }
...
void CArcaBattle::CGReqMarkRegButtonClick(OBJECTSTRUCT *lpObj) {
    ...
    if (iValidItemCount < 1 || iInvalidItemCount != 0 || iItemPos == -1) {
        g_Log.Add("[ArcaBattle][Mark] - Can Not be Reg Mark [%s][%s] CharClass[%d] ItemNum[%d] ItemName[%s]",
                  lpObj->AccountID, lpObj->Name, lpObj->Class, lpObj->pChaosBox[iItemPos].m_Type,
                  ItemAttribute[lpObj->pChaosBox[iItemPos].m_Type].Name);
    ...
    if (iItemCnt > 255) {
        g_Log.Add("[ArcaBattle][Mark] - Reg Mark Item Cnt Over [%s][%s] ItemCnt[%d]", lpObj->AccountID, lpObj->Name,
                  iItemCnt);
    ...
    g_Log.Add("[ArcaBattle] [%s][%s] ReqMarkReg Cnt [%d] - [%s][%d]",
              lpObj->AccountID, lpObj->Name, iItemCnt, lpObj->m_PlayerData->GuildName,
              lpObj->m_PlayerData->GuildStatus);
...
void CArcaBattle::GCAnsMarkRank(int iIndex, BYTE btRank, DWORD dwMarkCnt, BYTE btTopRankCnt,
                                _stArcaBattleMarkTopRankDS *pArcaBattleMarkTopRank) {
...
void CArcaBattle::GDReqMarkReg(int iIndex, DWORD dwMarkCnt) {
    ...
    if (!lpGuild) {
        g_Log.Add("[ArcaBattle][MarkReg] [%s][%s]  GuildInfo NULL GDReqMarkReg()", gObj[iIndex].AccountID,
                  gObj[iIndex].Name);
        return;
    }
...
void CArcaBattle::DGAnsMarkRank(_tagPMSG_ANS_ARCA_BATTLE_MARK_RANK_DS *lpMsg) {
    this->GCAnsMarkRank(lpMsg->wNumber, lpMsg->btRank, lpMsg->dwMarkCnt, lpMsg->btGuildCnt,
                        lpMsg->ArcaBattleMarkTopRank);
}
...
void CArcaBattle::DGAnsAllGuildMarkCnt(_tagPMSG_ANS_ALL_GUILD_MARK_CNT_DS *lpMsg) {
    ...
    if (wGuildCnt <= 250) {
        for (int i = 0; i < wGuildCnt; i++) {
            g_Log.Add("[ArcaBattle] Guild Name [%s] - ReqMarkReg [%d]", lpMsg->ABAllGuildMark[i].szGuildName,
                      lpMsg->ABAllGuildMark[i].dwMarkCnt);
        }
    }
}
...
void CArcaBattle::CheatABOccupyObelisk(OBJECTSTRUCT *lpObj, int iObeliskGroup) {
    ...
    memcpy(this->m_stObeliskState[iObeliskGroup - 1].m_szOccupyGuildName, lpObj->m_PlayerData->GuildName,
           MAX_GUILD_LEN + 1);
}
...
void CArcaBattle::BootyExchange(OBJECTSTRUCT *lpObj) {
    ...
    if (CheckInventoryEmptySpace(lpObj, 4, 4) == FALSE) {
        g_Log.Add("[ArcaBattle][BootyExchange] - Fail - Not Empty Inventory [%s][%s] CharClass[%d]", lpObj->AccountID,
    ...
    ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, itype, stRewardItemInfo.iItemLevel, stRewardItemInfo.iDurab, 0, 0, 0,
                         lpObj->m_Index, 0, 0, 0, 0, 0);

    g_Log.Add(
            "[ArcaBattle][BootyExchange] - Exchange End [%s][%s] ItemName[%s] ItemNum[%d] Level[%d] Dur[%d] skill[%d] luck[%d] option[%d] ExOpt[%d] SetOption[%d] BootyCnt[%d]",
            lpObj->AccountID, lpObj->Name, ItemAttribute[itype].Name, itype, stRewardItemInfo.iItemLevel,
            stRewardItemInfo.iDurab, 0, 0, 0, 0, 0, iItemCnt);
...
void CArcaBattle::GCArcaBattleUserInfo(int iUserIndex) {
    ...
    for (int i = 0; i < this->m_iABJoinUserCnt; i++) {
        if (!strcmp(lpObj->Name, this->m_stABJoinUserInfo[i].szUserName)) {
            ...
            if (!lpGuild) {
                g_Log.Add("[ArcaBattle] Error Can Not Find Guild GCArcaBattleUserInfo() lpGuild NULL GuildName : %s",
                          this->m_stABJoinUserInfo[i].szGuildName);
                return;
            }
            ...
            for (int j = 0; j < lpGuild->Count; j++) {
                ...
                if (lpGuild->GuildStatus[j] == GUILD_MASTER) {
                    if (ObjectMaxRange(iGuildMembIndex) == true && gObjIsConnected(lpGuildObj) == TRUE &&
                        MAX_MAP_RANGE(lpGuildObj->MapNumber) == TRUE) {
                        BYTE btAttr = MapC[lpGuildObj->MapNumber].GetAttr(lpGuildObj->X, lpGuildObj->Y);

                        if (lpGuildObj->MapNumber == MAP_INDEX_ARCA_WAR ||
                            lpGuildObj->MapNumber == MAP_INDEX_DEBENTER_ARCA_WAR) {
                            if ((btAttr & 1) == 1) {
                                ...
                                memcpy(pMsg.szGuildMasterName, lpGuildObj->Name, MAX_ACCOUNT_LEN + 1);
                                pMsg.dwGuildMasterGateNum = this->GetGuildMasterGateNum(
                                        this->m_stABJoinUserInfo[i].dwGuild);
                                pMsg.btGuildMasterPosX = lpGuildObj->X;
                                pMsg.btGuildMasterPosY = lpGuildObj->Y;
                            }
                        }
                            ...
                            for (int l = 0; l < lpGuild->Count; l++) {
                                ...
                                if (iIndex == -1) {
                                    if (!strcmp(lpGuild->Names[l], this->m_stABJoinUserInfo[k].szUserName)) {
                                        memcpy(stABCurJoinGuildUser.szUserName, this->m_stABJoinUserInfo[k].szUserName,
                                               MAX_ACCOUNT_LEN + 1);
                                    ...
                                    if ((gObj[iIndex].MapNumber == MAP_INDEX_ARCA_WAR ||
                                         gObj[iIndex].MapNumber == MAP_INDEX_DEBENTER_ARCA_WAR) &&
                                        ...
                                        BYTE btAttr = MapC[gObj[iIndex].MapNumber].GetAttr(gObj[iIndex].X,
                                                                                           gObj[iIndex].Y);
...
void CArcaBattle::GCArcaBattleCurrentStatus(int iObeliskIndex, int iAttrKind, BYTE btObeliskState, int iGuildNum,
                                            char *szGuildName, int iUserIndex) {
...
void CArcaBattle::GCArcaBattleSendNotiMsg(BYTE btNoticeType, int iNoticeValue, char *szGuildName) {
    ...
    if (!szGuildName) {
        ...
        if (!lpGuild) {
            g_Log.Add("[ArcaBattle] Error Can Not Find Guild GCArcaBattleSendNotiMsg() lpGuild NULL GuildName : %s",
                      szGuildName);
            return;
        }
        ...
        if (this->m_stObeliskState[i].m_iObeliskState != 0) {
            if (this->m_stObeliskState[i].m_iAuraState == 2) {
                ...
                if (this->m_stObeliskState[i].m_iAuraReleaseGuildNum == this->m_stObeliskState[i].m_iOccupyGuildNum) {
                    this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                                    this->m_stObeliskInfo[i].m_iAttrKind, ARCA_BATTLE_OBELISK_CAPTURED,
                                                    this->m_stObeliskState[i].m_iAuraReleaseGuildNum, lpGuild->Name,
                                                    iUserIndex);
                } else {
                    this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                                    this->m_stObeliskInfo[i].m_iAttrKind, ARCA_BATTLE_OBELISK_CAPTURING,
                ...
                this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber,
                                                this->m_stObeliskInfo[i].m_iAttrKind, ARCA_BATTLE_OBELISK_CAPTURED,
                                                this->m_stObeliskState[i].m_iOccupyGuildNum, lpGuild->Name, iUserIndex);
            }
        } else {
            if (this->m_stObeliskState[i].m_iAuraState == 2) {
                _GUILD_INFO_STRUCT *lpGuild = Guild.SearchGuild_Number(
...
bool CArcaBattle::LoadScriptArcaBattle(char *lpszFileName) {
    ...
    this->m_bDoEvent =
            g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_ARCA) == true
            ? main.attribute("Enable").as_int() : false;
    ...
    for (pugi::xml_node monster = monstergroupmember.child("Monster"); monster; monster = monster.next_sibling()) {
        ...
        this->m_stMonsterPosition[GroupNumber].m_stMonsterIndexInfo[ArrayIndex].m_nMonsterIndex = monster.attribute(
                "Index").as_int();
        ArrayIndex++;
...
int CArcaBattle::LoadScriptBootyMix(char *lpszFileName) {
    ...
    for (pugi::xml_node combination = trophymix.child(
            "Combination"); combination; combination = combination.next_sibling()) {

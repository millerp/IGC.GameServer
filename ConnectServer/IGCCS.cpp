// IGCCS.cpp : Defines the entry point for the application.
//
...
INT_PTR CALLBACK
AuthBox(HWND, UINT, WPARAM, LPARAM
);
...
LoadString(hInstance,
IDC_IGCCS, szWindowClass, MAX_LOADSTRING);
...
char szText[100];
wsprintf(szText,
"[%s] IGC.ConnectServer [TCP-Port: %d][UDP-Port: %d][TCP-Auth Port: %d]", CONNECTSERVER_VERSION, g_ConnectServerPort, g_ConnectServerUDP);
SetWindowText(hWnd, szText
);

IOCP.
CreateGIocp(g_ConnectServerPort);
...
hInstance = hInstance;
wcex.
hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_IGCCS));
wcex.
hCursor = LoadCursor(NULL, IDC_ARROW);
wcex.
hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
wcex.
lpszMenuName = MAKEINTRESOURCE(IDC_IGCCS);
wcex.

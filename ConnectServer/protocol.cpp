...
void SCSendAutoUpdateData(int aIndex, PMSG_CLIENTVERSION *aRecv) {
    ...
    if (aRecv->ClientMainVersion == MainVersion && aRecv->ClientHeadVersion == HeadVersion &&
        aRecv->ClientSubVersion == SubVersion) {
        PMSG_VERSIONOK pMsg;
        ...
        for (short i = 0; i <= 3; ++i) {
            switch (i) {
                case 0 :
                    for (short j = 0; j < 100; ++j) { pMsg.HostURL[j] ^= Keys[j % 3]; }
                    break;
                case 1 :
                    for (short j = 0; j < 20; ++j) { pMsg.FTPLogin[j] ^= Keys[j % 3]; }
                    break;
                case 2 :
                    for (short j = 0; j < 20; ++j) { pMsg.FTPPassword[j] ^= Keys[j % 3]; }
                    break;
                case 3 :
                    for (short j = 0; j < 20; ++j) { pMsg.VersionFile[j] ^= Keys[j % 3]; }
                    break;
            }
        }

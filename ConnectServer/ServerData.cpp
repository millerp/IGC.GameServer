...
void CServerData::LoadServerFile(LPSTR lpszFile) {
    ...
    for (pugi::xml_node server = main.child("Server"); server; server = server.next_sibling()) {
        ...
        g_Log.AddC(TColor::Green,
                   "[Server Data] Loaded Server(s) info CODE: (%d), NAME: (%s), IP: (%s), PORT: (%d), VISIBLE: (%d), INDEX: (%d)",
                   this->m_Servers[count].Code, this->m_Servers[count].Name, this->m_Servers[count].IP,
                   this->m_Servers[count].Port, this->m_Servers[count].Visible, count);

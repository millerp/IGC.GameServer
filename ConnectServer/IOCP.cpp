        ...
        if (CreateListenSocket() == 0) {
            ...
            while (true) {
                ...
                if (UpdateCompletionPort(Accept, ClientIndex, 1) == 0) {
                    g_Log.AddC(TColor::Red, "error-L1: %d %d CreateIoCompletionPort failed with error %d", Accept,
                ...
                nRet = WSARecv(Accept, &Users[ClientIndex].PerSocketContext->IOContext[0].wsabuf, 1,
    ...
    while (true) {
        ...
        if (lpIOContext->IOOperation == 1) {
            ...
            if (RecvDataParse(lpIOContext, lpPerSocketContext->nIndex) == 0) {
                g_Log.AddC(TColor::Red, "error-L1: Socket Header error %d, %d", WSAGetLastError(),
                           lpPerSocketContext->nIndex);
                CloseClient(lpPerSocketContext, 0);
                LeaveCriticalSection(&criti);
                continue;
            }
...
bool CIOCP::RecvDataParse(_PER_IO_CONTEXT *lpIOContext, int uIndex) {
    ...
    while (true) {
        if (recvbuf[lOfs] == 0xC1 ||
            ...
            g_Log.AddC(TColor::Red, "error-L1: Header error (%s %d)lOfs:%d, size:%d", __FILE__, __LINE__, lOfs,
        ...
        if (size <= lpIOContext->nSentBytes) {
            ...
            if (Users[uIndex].PacketCount >= g_MaxPacketPerSec && strcmp(Users[uIndex].IP, g_WhiteListIP)) {
                g_Log.AddC(TColor::Red, "[ANTI-FLOOD] Packets Per Second: %d / %d, IP: %d", Users[uIndex].PacketCount,
                           g_MaxPacketPerSec, Users[uIndex].IP);
...
bool CIOCP::DataSend(int aIndex, unsigned char *lpMsg, DWORD dwSize, bool Encrypt) {
    ...
    if (lpIoCtxt->nWaitIO > 0) {
        if ((lpIoCtxt->nSecondOfs + dwSize) > MAX_IO_BUFFER_SIZE - 1) {
            g_Log.AddC(TColor::Red, "(%d)error-L2: MAX BUFFER OVER %d %d %d", aIndex, lpIoCtxt->nTotalBytes,
    ...
    if (WSASend(Users[aIndex].socket, &lpIoCtxt->wsabuf, 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1) {
        ...
        if (WSAGetLastError() != WSA_IO_PENDING) {
            ...
            if (lpIoCtxt->wsabuf.buf[0] == 0xC1) {
                g_Log.AddC(TColor::Red, "(%d)WSASend(%d) failed with error [%x][%x] %d %s ", __LINE__, aIndex,
...
bool CIOCP::IoSendSecond(_PER_SOCKET_CONTEXT *lpPerSocketContext) {
    ...
    if (WSASend(Users[aIndex].socket, &lpIoCtxt->wsabuf, 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1) {
        if (WSAGetLastError() != WSA_IO_PENDING) {
            g_Log.AddC(TColor::Red, "WSASend(%d) failed with error %d %s ", __LINE__, WSAGetLastError(),
                       Users[aIndex].IP);
            CloseClient(aIndex);
            LeaveCriticalSection(&criti);
            return false;

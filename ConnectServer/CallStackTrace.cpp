    ...
    __try
    {
        ...
        for (UINT i = 0; i < 512; i++) {
            if (StackWalk(0x14C, hProcess, hThread, &stackframe, &context, NULL,
                          SymFunctionTableAccess, (PGET_MODULE_BASE_ROUTINE) CCallStackTrace::GetModuleBase, NULL) ==
                FALSE) {
                break;
            }
...
DWORD __stdcall CCallStackTrace::GetModuleBase(HANDLE hProcess, DWORD address) {
    ...
    if (VirtualQueryEx(hProcess, (LPCVOID) address, &memory_basic_information, sizeof(memory_basic_information)) ==
        FALSE) {
        return 0;
    }

    module_file_name_length = GetModuleFileName((HMODULE) memory_basic_information.AllocationBase, module_file_name,
